...
#if 0                                  /*  ZSS June 2011. Delete useless code after dust has
                                        * settled.  */
...
/***********************************************************************
 * Plugin to accept data from an external process and assemble it into a dataset.  Initial
 *implementation was for the Bruker 3T/60 system at the Medical College of Wisconsin.  Now works
 *with generic external "image source" software.  See README.realtime for info.
 ************************************************************************/
...
/** 29 Jan 2004: allow 100 chars in root_prefix via PREFIX (from 31) [rickr] x-axis of 3-D motion
 * graphs changed from time to reps plot_ts_... functions now use reg_rep for x-axis values
 * reg_graph_xr is no longer scaled by TR added (float *)reg_rep, for graphing with x == rep num
 * added RT_set_grapher_pinnums(), to call more than once added GRAPH_XRANGE and GRAPH_YRANGE
 * command strings for control over the scales of the motion graph if GRAPH_XRANGE and GRAPH_YRANGE
 * commands are both passed, do not display the final (scaled) motion graph          **/
/** 13 Feb 2004: added RT_MAX_PREFIX for incoming PREFIX command     [rickr] if GRAPH_?RANGE is
 * given, disable 'pushing'
 *                   (see plot_ts_xypush()) added GRAPH_EXPR command, to compute and display a
 *single motion curve, instead of the normal six (see p_code, etc., reg_eval, and RT_parser_init())
 *      **/
/** 31 Mar 2004: added ability to send registration parameters       [rickr] If the
 * AFNI_REALTIME_MP_HOST_PORT environment variable is set (as HOST:PORT, e.g. localhost:53214), then
 * the six registration correction parameters will be sent to that host/port via a tcp socket.  This
 * is done only in the case of graphing the 3D registration parameters.
 * added RT_input variables to manage the new socket added RT_mp_comm_...() functions modified yar[]
 *logic in RT_registration_3D_realtime() to pass the registration parameters before adjusting the
...
/** 01 Jun 2009: added ability to have a callback called at each time an update is sent to AFNI --
 * for further processing of some hideous sort, I suppose          [RWCox] **/
/** 02 Jun 2009: added ability to merge multichannel data            [RWCox] **/

/** 02 Jun 2010: added ability to register merged data, and to align channels via the same merge
 * registration parameters [rickr] **/
...
/** 14 Jan 2020: apply regtime to ext reg dset; fix pre-merge align  [rickr] dx,y,z were used for
* multiple things, corrupting the channel merge values                                        **/
/** 23 Jan 2020: added ROIs_and_data mask method for JGC             [rickr] This was given a new
 * mask method (and in rr.py), sending 2 lengths to the receiver, one for #(non-1)ROIs, one for the
...
typedef struct {
    ...
    pid_t               child_info;                  /* pid of child that will give me control
                                                      * information */
    double              child_start_time;            /* 10 Dec 1998: elapsed time when child process
                                                      * was forked */
    ...
    float               xxoff, yyoff, zzoff;         /* offsets (instead of xxorg, etc.) [18 Dec
                                                      * 2002] */
    ...
    /* 01 Aug 2002: these items are now indexed by MAX_CHAN to allow for multiple channel
     * acquisitions */
...
/* RT mask mechods:
 *
 * None                 : nothing sent Motion_Only          : 6 motion parameters, per time point
 *ROI_means            : 6 motion, plus the mean over each ROI index All_Data             : big: 6
 *motion plus index, ijk, xyz, value for each vox All_Data_light       : 6 motion, plus each voxel
 *value (over mask) ROIs_and_data        : 6 motion, each ROI average (for ROI index > 1) each voxel
 *value (for ROI index == 1)
 */
...
/* Cameron Craddock defines to support real-time detrending set up as bit masks to make it easier to
...
# define RT_DETREND_POLORT     0x10           /* remove polynomial regressor determined by
                                               * rtin->detrend_polort */
...
/* Cameron Craddock, init masking without setting up communication */
...
/***********************************************************************
 * Set up the interface to the user 09 Oct 2000: allow all options to be initialized via the
 *environment
 ************************************************************************/
...
PLUGIN_interface * PLUGIN_init(int ncall)
{
    ...
    if (!ALLOW_realtime)
    {
        return NULL;                     /* do nothing if not allowed */
    }
    ...
    if (!THD_filename_pure(ept))
    {
        ept = NULL;
    }
    if (ept != NULL)
    {
        MCW_strncpy(root, ept, THD_MAX_PREFIX);
    }
    ...
    /* Cameron Craddock added to allow show times to be set by GUI interface */
    ...
    if (ept != NULL)
    {
        ...
        if (ii < 0)
        {
            ii = PLUTO_string_index(ept, NREG, REG_strings_ENV);
        }
        if (ii >= 0 && ii < NREG)
        {
            regmode = ii;
    ...
    if (ept != NULL)
    {
        ...
        if (ii < 0)
        {
            ii = PLUTO_string_index(ept, NREG_BASE, REG_BASE_strings_ENV);
        }
        if (ii >= 0 && ii < NREG_BASE)
        {
            g_reg_base_mode = ii;
        }
    }
    ...
    if (RT_chmrg_mode < 0 || RT_chmrg_mode >= N_RT_CHMER_MODES)
    {
        RT_chmrg_mode = 0;
    }
    ...
    if (RTdatamode < 0 || RTdatamode >= N_RT_WRITE_MODES)
    {
        RTdatamode = 0;
    }
    ...
    /* convert the detrend node to the appropriate string array indices */
    rt_detrend_addort = 0;
    if (rt_detrend_mode & RT_DETREND_GM)
    {
        rt_detrend_addort += 1;
    }
    if (rt_detrend_mode & RT_DETREND_MASK)
    {
        rt_detrend_addort += 2;
    }
    ...
    if (rt_detrend_mode & RT_DETREND_FRISTON)
    {
        rt_detrend_motion += 1;
    }
...
char * RT_main(PLUGIN_interface *plint)
{
    ...
    while ((tag = PLUTO_get_optiontag(plint)) != NULL)
    {
        ...
        if (strcmp(tag, "MiscOptions") == 0)
        {
            ...
            /* if coming through the plugin, afni already knows about the dataset, so we do not need
             * to delete it (or we make a plugin copy, as with g_reg_base_dset) */
            idc = PLUTO_get_idcode(plint);
        ...
        if (strcmp(tag, "Detrend") == 0)
        {
            ...
            /* CC if the user specifies detrend to MASK ROIs, make sure that they also specified a
             * MASK under masking, this assumes that masking is interpreted before detrend */
            ...
            /* CC if the user specifies detrend to motion, make sure that they also specified RT
             * motion correction, this assumes that motion correction is interpreted before detrend
             */
            if ((rt_detrend_mode & RT_DETREND_MOTION) &&
                (regmode != 1) && (regmode != 3))
            {
...
/***************************************************************************
 * Listen for an incoming real-time data control connection. Return 1 if the connection is
 *established, return 0 if no connection is ready, return -1 if an error occurs.
 ****************************************************************************/
...
int RT_check_listen(void)
{
...

    {
        ...
        {
/**
 *       if( verbose > 1 ) fprintf(stderr,"RT: still waiting for control data.\n") ;
 **/
        }
        ...
        if (jj > 0 && verbose > 1)
        {
            fprintf(stderr, "RT: control data is present!\n");
        }
...
/* 10 Dec 2002: add keep_ioc_data flag, so as not to close the data channel;
...
void cleanup_rtinp(int keep_ioc_data)
{
    ...
    {
        int kk;
        for (kk = 0 ; kk < rtinp->num_note ; kk++)
        {
            FREEUP(rtinp->note[kk]);
        }
        FREEUP(rtinp->note);
    }
...
/*********************************************************************
 * Real time routine called every so often by Xt. This handles connections from other processes that
 *want to send AFNI data.
 *
 * This is a work process -- it returns False if it wants to be called again, and return True if it
 *is done.  For real-time AFNI-izing, this should always return False.
 **********************************************************************/
...
RwcBoolean RT_worker(XtPointer elvis)
{
    ...
    if (first)
    {
        if (verbose)
        {
            fprintf(stderr, "RT: first call to RT_worker()\n");
        }
        first = 0;
    }
#endif
    ...
    if (rtinp == NULL)          /* not doing real-time input at this time */
    {
        ...
        if (jj == 0)
        {
            return False;                  /* try later if no connection now */
        }
...
#define CHILD_MAX_WAIT    66.6 /* seconds */
    ...
    if (rtinp->child_info > 0)
    {
        ...
        if (jj == 0)                                         /** 10 Dec 1998: child not sending
                                                              * data? **/
        {
            ...
            if (eee != NULL)
            {
                double val = strtod(eee, NULL); if (val >= 1.0)
                {
                    mw = val;
                }
            }
...

        {
            ...
            /* if we already received the first image data, then all the setup info should be OK.
             *  if not, quit */
        ...
        /** if nothing happened (good or bad), will try to read it again on the next time thru **/
    }
...

    {
        ...
        /** if there is no image data for a long time, and there is something new to write out, then
         * write the datasets out again.         **/
    ...
    if (rtinp->no_data)
    {
        ...
        if (strlen(rtinp->name_info) > 0)
        {
            RT_start_child(rtinp);
        }
        ...
        if (g_show_times)
        {
            RT_show_duration("starting to receive realtime data");
        }
        ...
        /* if we have already received the child process info, or if there is no child process info,
         * then all should be OK now */
    ...
    /* 10 Dec 2002: if data stream marked itself for death, then close down the dataset, but don't
     * close down the data stream itself;
     *              instead, create a new RT_input context for reading new metadata+image data from
...
/*--------------------------------------------------------------------------
 * Initialize a new RT_input structure; returns pointer to new structure. Will read from the control
 *channel, which will tell it what data channel to open, and if it needs to fork a child process to
 *gather header info.
 *
 * 10 Dec 2002: new input ioc_data is a pointer to an IOCHAN that should be used for data
 *acquisition:
 *                - if this is not NULL, then the control channel is NOT used
 *                - instead, the RT_input struct is setup to read directly from ioc_data (which
 *should already be good)
 *                - in this case, you can't use an info_command; all the metadata must come from the
 *ioc_data channel
 * ----------------------------------------------------------------------------*/
...
RT_input * new_RT_input(IOCHAN *ioc_data)
{
...

    {
        ...
        /** The data we now have in 'con' should be of the form image_channel_spec\n
         * info_command\n\0 where 'image_channel_spec' is an IOCHAN specifier saying how AFNI should
         * read the image data and 'info_command' is the name of a command to run that will write
         * the dataset control info to stdout (where a child of AFNI will get it using the 'popen'
         * routine). If no 'info_command' is needed (all the dataset info will come thru on the
         *image channel), then the info_command should just be skipped. Each string should be less
         *than NNAME characters long!
         **/
        ...
        /** if more follows, that is a command for a child process (which will not be started until
         * the first image data arrives) **/
    ...
    while (1)
    {
        ...
        else if (ii == 0 && verbose > 1)
        {
            fprintf(stderr, ".");                             /* not good yet */
        }
        else                                                  /* is bad! */
        {
    ...
    /* nuke any .nii, say, and note for later          30 May, 2014 [rickr] This will be set after
     * any ADN_prefix adjustment, because the root_prefix is appended to in many and compounding
     * ways. Unfortunately, this method cannot distinguish between .nii and
     *.nii.gz, which might be what Vinai would prefer.               */
...
/* April 2012 Cameron Craddock initialize variables for RT detrend */
    ...
    if (regmode == REGMODE_3D_ESTIM && reggraph == 0)
    {
        rtin->reg_graph = 1;
    }
...
/* if 'text' ends with 'suffix', return a pointer to that position within 'text' */
static char * ends_at(char *text, char *suffix)
{
    int tlen, slen;
    ...
    if (!strcmp(text + tlen - slen, suffix))
    {
        return text + tlen - slen;
    }
    else
    {
        return NULL;
...
/*-------------------------------------------------------------------------
 * Create a child process to run a command that will send header info back to the parent AFNI
 *process.
 * ---------------------------------------------------------------------------*/
...
void RT_start_child(RT_input *rtin)
{
    ...
    if (rtin == NULL || strlen(rtin->name_info) == 0)
    {
        return;                     /* sanity */
    }
 ...
 *   Run a command, get its output, send it back to the parent, exit. The entire existence of the
 *child is confined to this routine.
 * ---------------------------------------------------------------------------
 * ---------------------------------------------------------------------------*/
...
/****************************************************************************
 * Check the realtime input structure for OK-ness, and set its internal flag. If if it bad, and prt
 *isn't 0, print a message about the error(s).
 *****************************************************************************/
...
void RT_check_info(RT_input *rtin, int prt)
{
    ...
    if (rtin->image_mode)
    {
        ...
        if (rtin->info_ok || !prt)
        {
            return;                         /* if good, or if no print */
        }
        if (!(rtin->nxx > 1))
        {
            EPR("Image x-dimen not > 1");
        }
        if (!(rtin->nyy > 1))
        {
            EPR("Image y-dimen not > 1");
        }
        if (!(AFNI_GOOD_DTYPE(rtin->datum)))
        {
            EPR("Bad datum");
        }
        return;
    ...
    if (!(rtin->dtype > 0))
    {
        EPR("Bad acquisition type");
    }
    if (!(THD_filename_pure(rtin->root_prefix)))
    {
        EPR("Bad prefix");
    }
    if (!(strlen(rtin->root_prefix) < THD_MAX_PREFIX))
    {
        EPR("Overlong prefix");
    }
    if (!(rtin->tr > 0))
    {
        EPR("TR is not positive");
    }
    if (!(rtin->dzz > 0 || rtin->zzfov > 0))
    {
        EPR("Slice thickness not positive");
    }
    if (!(rtin->xxfov > 0))
    {
        EPR("x-FOV not positive");
    }
    if (!(rtin->yyfov > 0))
    {
        EPR("y-FOV not positive");
    }
    if (!(rtin->nxx > 1))
    {
        EPR("Image x-dimen not > 1");
    ...
    if (!(rtin->zorder > 0))
    {
        EPR("Slice ordering illegal");
    }
    if (!(rtin->tpattern > 0))
    {
        EPR("Timing pattern illegal");
    }
    if (rtin->nstimes && rtin->nstimes != rtin->nzz)
    {
        EPR("Num slice times != num slices");
    }
    if (!(rtin->orcxx >= 0))
    {
        EPR("x-orientation illegal");
    }
    if (!(rtin->orcyy >= 0))
    {
        EPR("y-orientation illegal");
    }
    if (!(rtin->orczz >= 0))
    {
        EPR("z-orientation illegal");
    }
    if (!(OR3OK(rtin->orcxx, rtin->orcyy, rtin->orczz)))
    {
        EPR("Inconsistent xyz-orientations");
    }
...
static int RT_mp_comm_alive(int sock, int verb, char *mesg)
{
    ...
    if (!ac)
    {
        if (mesg)
        {
            fprintf(stderr, "** %s: socket has gone bad\n", mesg);
        }
        else
        {
            fprintf(stderr, "** RT_comm socket has gone bad\n");
        }
    }
    ...
    if (verb)     /* check and print extra info */
    {
        ...
        if (rc)
        {
            tr = tcp_recv(sock, buf, 1, MSG_PEEK);
        }
 ...
 *
 * new_tcp_use should be set to either 0 or -1, where -1 means not to allow any init again (this
 *run)
 *
 * return   0 : on success
 *        < 0 : on error
...
static int RT_mp_comm_close(RT_input *rtin, int new_tcp_use)
{
    ...
    if (tcp_alivecheck(rtin->mp_tcp_sd) && tcp_writecheck(rtin->mp_tcp_sd, 0))
    {
        if (verbose > 1)
        {
            fprintf(stderr, "RTM: sending magic_bye...\n");
        }
...
/*---------------------------------------------------------------------------
 * Set non-1 ROI averages and all val==1 mask values.     19 Jan 2020 [rickr] (motion has already
 *been set)
 *
...
/*---------------------------------------------------------------------------
 * Like RT_mp_set_mask_data, but for each masked voxel, send only that voxel data value (so no
 *index, i,j,k, x,y,z).
 *
 * return 0 on success
 * -----------------------------------------------------------------------------*/
...
static int RT_mp_get_mask_aves(RT_input *rtin, int sub)
{
    ...
    {
        source_set = rtin->mrg_dset;
    }
    else
    {
        source_set = rtin->reg_dset;
    }
    ...
    for (c = 1; c < nval_len; c++)
    {
        if (nvals[c])
        {
            rtin->mask_aves[c] /= (double)nvals[c];
        }
        if (verbose > 1)
        {
...
static int RT_mp_mask_free(RT_input *rtin)
{
    if (rtin->mask)
    {
        free(rtin->mask);      rtin->mask = NULL;
    }
    if (rtin->mask_aves)
    {
        free(rtin->mask_aves); rtin->mask_aves = NULL;
    }
    rtin->mask_nset  = 0;
    rtin->mask_nones = 0;
    rtin->mask_nvals = 0;
 ...
 *
 * NOTE: anything applied here that is also in the interface needs to be set as an env var if the
 *interface is applied (so the most recent action is effective).
 *
...
static int RT_mp_getenv(void)
{
    ...
    if (estr != NULL)
    {
        ...
        if (verbose)
        {
            fprintf(stderr, "++ RTM getenv: mvals = %s\n",
                    RT_mask_strings_ENV[g_mask_val_type]);
        }
        if (verbose > 1)
        {
            fprintf(stderr, "-- RTM getenv, set from %s\n", estr);
        }
    }
...
static int RT_mp_comm_init(RT_input *rtin)
{
    ...
    /* set the 'hello' mode (based on the mask values choice) 0 : original magic_hi 1 : magic_hi[3]
     * += 1   -> also send num_extra as int 2 : magic_hi[3] += 2   -> also send num_extra as int for
     *ALL_DATA 3 : magic_hi[3] += 3   -> also send num_extra for ALL_DATA light 4 : magic_hi[3] +=
     * 4   -> also send num_extra for nROIs and nones
     *
     * if we are prepared to send mask info, and hello mode is set, then note to also send
     *mask_nvals (or mask_nset) and possibly nones
     */
    ...
    sin.sin_addr.s_addr = ((struct in_addr *)(hostp->h_addr_list[0]))->s_addr;
...
static int RT_mp_comm_init_vars(RT_input *rtin)
{
    ...
    if (g_mask_dset)
    {
        RT_mp_init_mask(rtin);
    }

    return 0;
}

/* CC 2012; 3 Sep 2014 [rickr] initialize mask independent of the RT_mp_comm_init_vars, e.g. to use
...
static int RT_mp_init_mask(RT_input *rtin)
{
    ...
    if (rtin->mask)
    {
        free(rtin->mask);               /* in case of change */
    }
    ...
    for (c = 0, max = rtin->mask[0]; c < DSET_NVOX(g_mask_dset); c++)
    {
        ...
        if (rtin->mask[c] == 1)
        {
            rtin->mask_nones++;                         /* count vox==1  */
        }
        if (rtin->mask[c] > max)
        {
            max = rtin->mask[c];                        /* track max     */
        }
    }
    ...
    if (verbose)
    {
        fprintf(stderr, "RTM: have %d-value mask\n", max);
    }
...
/*---------------------------------------------------------------------------
 * If AFNI_REALTIME_Mask_Dset is set, make sure g_mask_dset points to it. Let NONE mean the same as
 *not being set.
 *
 * return   0 : on success
...
int RT_mp_rm_env_mask(void)
{
    ...
    if (verbose)
    {
        fprintf(stderr, "RTM: clearing mask dataset name %s\n",
                g_mask_dset_name);
    }
    ...
    if (g_mask_dset)
    {
        if (verbose)
        {
            fprintf(stderr, "RTM: clearing mask dataset\n");
        }
...
/*---------------------------------------------------------------------------
 * Process command strings in the buffer, up to the first '\0' character. Returns the number of
 *characters processed, which will be one more than the index of the first '\0'.  If an error
...
int RT_process_info(int ninfo, char *info, RT_input *rtin)
{
    ...
    if (rtin == NULL || info == NULL || ninfo == 0)
    {
        return -1;
    }
    ...
    for (nend = 0 ; nend < ninfo && info[nend] != '\0' ; nend++)
    {
        ;                                                        /* nada */
    }
    if (nend == ninfo)
    ...
    while (nstart < nend)
    {
        ...
        for ( ; nstart < nend && isspace(info[nstart]) ; nstart++)
        {
            ;                                                      /* nada */
        }
        if (nstart >= nend)
        ...
        if (STARTER("ACQUISITION_TYPE"))
        {
            ...
            if (strcmp(typ, "2D+z") == 0)
            {
                rtin->dtype = DTYPE_2DZ;
            }
            else if (strcmp(typ, "2D+zt") == 0)
            {
                rtin->dtype = DTYPE_2DZT;
            }
            ...
            /* Some troublesome physicist, let's just call him "Tom" (possibly named by his mother,
             * Mrs. Ross), wants control over the ranges in the motion correction graph window.
             *                                                29 Jan 2004 [rickr] */
        }
        ...
        else if (STARTER("GRAPH_YRANGE"))
        {
            ...
            /* Allow the user to specify an expression, to evalue the six motion parameters into
             * one.
             *                                                12 Feb 2004 [rickr] */
        }
        ...
        else if (STARTER("DATUM"))
        {
            ...
            {
                if (strcmp(tstr, MRI_TYPE_name[ii]) == 0)
                {
                    break;
                }
            }

            if (AFNI_GOOD_DTYPE(ii))
            {
                rtin->datum = ii;
            }
            else
        ...
        else if (STARTER("BYTEORDER"))      /* 27 Jun 2003:           [rickr] */
        {
            ...
            if (strncmp(tstr, "LSB_FIRST", 9) == 0)
            {
                bo = LSB_FIRST;
            }
            else if (strncmp(tstr, "MSB_FIRST", 9) == 0)
            {
                bo = MSB_FIRST;
            }
            else
        ...
        else if (STARTER("ZORDER"))         /* ineffective.                   */
        {
            if (!rtin->zorder_lock)
            {
                ...
                if (strcmp(str32, "alt") == 0)
                {
                    rtin->zorder = ZORDER_ALT;
                }
                else if (strcmp(str32, "seq") == 0)
                {
        ...
        else if (STARTER("TPATTERN"))                 /* get timing pattern  */
        {
            if (!rtin->zorder_lock)                   /* 10 May 2005 [rickr] */
            {
                ...
                if (strcmp(str32, "alt+z") == 0)
                {
                    rtin->tpattern = ZORDER_ALT;
                }
                else if (strcmp(str32, "seq+z") == 0)
                {
        ...
        else if (STARTER("XYZAXES"))
        {
            ...
            for (ii = 0 ; ii < 6 ; ii++)
            {
                if (strcmp(xstr, ORIENT_shortstr[ii]) == 0 ||
                    ...
                    strcmp(xstr, ORIENT_tinystr[ii]) == 0)
                {
                    orx = ii;
                }
                ...
                if (strcmp(ystr, ORIENT_shortstr[ii]) == 0 ||
                    ...
                    strcmp(ystr, ORIENT_tinystr[ii]) == 0)
                {
                    ory = ii;
                }
                ...
                if (strcmp(zstr, ORIENT_shortstr[ii]) == 0 ||
                    ...
                    strcmp(zstr, ORIENT_tinystr[ii]) == 0)
                {
                    orz = ii;
                }
            }
        ...
        else if (STARTER("DRIVE_WAIT"))     /* 21 Aug 2008 */
        {
            ...
            else   /* the command is everything after "DRIVE_WAIT " */
            {
                ...
                if (verbose > 1)
                {
                    fprintf(stderr, "RT: command DRIVE_WAIT %s\n", buf + 11);
                }
            }
        }
        else if (STARTER("ECHO_TIMES"))     /* 12 Mar 2015 [rickr] */

        /* just see what we find, like explicit TPATTERN, and verify against num_chan later (being
         * more kind and gentle than I feel) */
        {
    ...
    if (numte > 0)
    {
        ...
        {
            for (jj = 1; jj < rtin->num_chan; jj++)
            {
                rtin->TE[jj] = rtin->TE[0];
            }
        }
...
/*--------------------------------------------------------------------
 * Create the dataset and prepare to receive image data.  Some of this code is taken shamelessly
 *from to3d.c (I wrote it, I can steal it). This routine should only be called when rtin->info_ok is
 *true.
 * ----------------------------------------------------------------------*/
...
void RT_start_dataset(RT_input *rtin)
{
    ...
    if (rtin->num_chan > 1)
    {
        ...
        else if (maxnc > MAX_CONTROLLERS)
        {
            maxnc = MAX_CONTROLLERS;
        }
        ...
        for (cc = 0 ; cc < rtin->num_chan ; cc++)
        {
            if (cc < tc)
            {
                rtin->im3d[cc] = open_controller[cc];       /* next available */
            }
            else
            {
                rtin->im3d[cc] = NULL;                      /* none available */
            }
    ...
    base_ind = num_runs;
    if (AFNI_yesenv("AFNI_REALTIME_reset_output_index"))
    {
        base_ind = 1;
    }
    ...
    /* use __001 etc., as '#' evaluates to a comment and '-' is used for options 27 Jul 2009 [rickr]
     */
    {
...

        {
            for (cc = 0 ; cc < rtin->num_chan ; cc++) /* check each channel prefix */
            {
                ...
                if (!PLUTO_prefix_ok(ccpr))
                {
                    break;                        /* not good? quit cc loop */
                }
            }
            if (cc == rtin->num_chan)
            {
                break;                            /* all were good? we are done */
            }
        }
    }
    ...
    if (rtin->zzfov > 0)
    {
        rtin->dzz = rtin->zzfov / rtin->nzz + rtin->zgap;
    }
    ...
    if (rtin->xcen)
    {
        rtin->xxorg = 0.5 * (rtin->nxx - 1) * rtin->dxx + rtin->xxoff;
    }
    if (rtin->ycen)
    {
        rtin->yyorg = 0.5 * (rtin->nyy - 1) * rtin->dyy + rtin->yyoff;
    }
    if (rtin->zcen)
    {
        rtin->zzorg = 0.5 * (rtin->nzz - 1) * rtin->dzz + rtin->zzoff;
    }

    /* if axis direction is a 'minus', voxel increments are negative */

    if (ORIENT_sign[rtin->orcxx] == '-')
    {
        rtin->dxx = -rtin->dxx;
    }
    if (ORIENT_sign[rtin->orcyy] == '-')
    {
        rtin->dyy = -rtin->dyy;
    }
    if (ORIENT_sign[rtin->orczz] == '-')
    {
        rtin->dzz = -rtin->dzz;
    }

    /* 10 Dec 2002:
     * We now allow direction codes input for x and y as well, so must duplicate the complicated
     *zzorg logic for setting the xxorg and yyorg signs.  This duplicated logic follows the zzdcode
     *stuff, below.                                 */

#if 0
    /* if axis direction is a 'plus', then the origin is in a 'minus' direction, so the origin
     * offset must have its sign changed */  /*** THE OLD WAY ***/

    if (ORIENT_sign[rtin->orcxx] == '+')
    {
        rtin->xxorg = -rtin->xxorg;
    }
    if (ORIENT_sign[rtin->orcyy] == '+')
    {
        rtin->yyorg = -rtin->yyorg;
    }
#endif

    /* z-origin is complex, because the remote program might have given a direction code, or it
     * might not have. We must set the sign of the z-origin based on the sign of the axis direction
     * if no specific direction code is given, otherwise it should be based on the specific
     * direction code (rtin->zzdcode). */
...

    {
        if (ORIENT_sign[rtin->orczz] == '+')
        {
            rtin->zzorg = -rtin->zzorg;
        }
    }
    else     /* have code */

    /* check if code matches the axis direction (or is the opposite, which is also OK).  */
    ...
    if (RT_chmrg_mode > 0)
    {
        ...
        if (rtin->chan_list_str)
        {
            free(rtin->chan_list_str);
        }
        rtin->chan_list_str = nifti_strdup(RT_chmrg_list);
    }

    /* if reg_chan_mode is set, verify there is something to do note: reg_chan_mode specifies to
     * register the mrg_dset, and maybe the channel dsets as followers          19 May 2010 [rickr]
     */
    ...
    if (RT_will_register_merged_dset(rtin))
    {
        if (!rtin->mrg_dset)
        {
            if (verbose > 0)
            {
                fprintf(stderr, "** RTCM: no merge dset to register\n");
            }
    ...
    { /* CC add test here to see if detrending enabled */
        ...
        /* CC just copied this from reg_dset, copies some notes from dataset, (i think) */
        if (rtin->num_note > 0 && rtin->note != NULL)
        {
            for (ii = 0 ; ii < rtin->num_note ; ii++)
            {
    ...
    /** if there is already data stored in the temporary buffer (acquired before this routine was
     * called), then we want to place it into the dataset now.                       **/
    ...
    if (rtin->bufar != NULL)
    {
        ...
        FREE_IMARR(rtin->bufar);                                 /* throw this away like a used
                                                                  * Kleenex */
    ...
    { RT_status *rts = GLOBAL_library.realtime_status;
      ...
      for (cc = 0 ; cc < rtin->num_chan ; cc++)
      {
          rts->dset[cc] = rtin->dset[cc];
      }
      if (rtin->mrg_dset != NULL)
      {
          rts->dset[cc++] = rtin->mrg_dset;
      }
      if (rtin->reg_dset != NULL)
      {
          rts->dset[cc++] = rtin->reg_dset;
      }
      /* CC add in detrend dataset */
      if (rtin->detrend_dset != NULL)
      {
          rts->dset[cc++] = rtin->detrend_dset;
      }
...
/* return whether this is true, whether we will register the merged dataset 27 Oct 2016 [rickr] */
...
/* return whether to merge before registration, after, or not at all 27 Oct 2016 [rickr] */
static int RT_when_to_merge(void)
{
    if (RT_chmrg_mode == RT_CHMER_NONE)
    {
        return RT_CM_NO_MERGE;
    }
    if (RT_chmrg_mode == RT_CHMER_OPT_COMB)
    {
        return RT_CM_MERGE_AFTER_REG;
    }
...
void RT_read_image(RT_input *rtin, char *im)
{
    ...
    /* 10 Dec 2002:
     * Check if the command string is present at the start of im. If it is, then mark rtin for
     *death.                       */
...
/*--------------------------------------------------------------------
 * Read images and put them into place.  Note that if there is any data left in the rtin buffer, it
 *will go into the image first.
 * ----------------------------------------------------------------------*/
...
int RT_process_data(RT_input *rtin)
{
    ...
    while (rtin->nbuf > 0 || iochan_readcheck(rtin->ioc_data, 0) > 0)
    {
...

        {
            ...
            if (rtin->marked_for_death)
            {
                return 0;           /* 10 Dec 2002 */
            }
...
/*-------------------------------------------------------------------
 * Given image data stored in rtin->im, process it into the dataset that is under construction. This
 *routine should only be called after RT_start_dataset has been executed.
 * ---------------------------------------------------------------------*/
...
void RT_process_image(RT_input *rtin)
{
    ...
    if (vdone)
    {
        ...
        /* first time: put this volume in as "substitute" for empty 1st brick later:      add new
         * volume at end of chain                         */
        ...
        /* must also change the number of times recorded [EDIT_add_brick does 'nvals' correctly, but
         * not 'ntt'] */
        ...
        /* Cameron Craddock If a mask is specified, but for some reason the communication fails, or
         * is not initialized, or if real time graphing is turned off then go ahead and calculate
         * the mask averages. */
        ...
        /* MP communication is turned off, but the mask is initialized calculate mask averages */
        if ((rtin->mp_tcp_use != 1) && (rtin->mask_init == 1))
        {
            if (RT_mp_get_mask_aves(rtin, rtin->reg_nvol - 1))
            {
                /* something bad happened, free memory and disable future attempts */
        ...
        * near real time - vinai.                                        */
        ...
        if (RTdatamode) /* if not zero, and writing data in real time   */
        {
            ...
            /* RCR: do we want to consider writing registered datasets, merged and individual
             * channels? */
        }
        ...
        if (rtin->func_code > 0)
        {
            ...
            if (rtin->func_condit == 0)
            {
                ...
                if (jj < 0)
                {
                    rtin->func_code = 0; rtin->func_func = NULL;
                }
...
static int RT_merge(RT_input *rtin, int chan, int tt)
{
    ...
    if (chan + 1 != rtin->num_chan || RT_chmrg_mode == 0)
    {
        return 0;
    }
    ...
    /* note: the channel int list can only be created "now", since we must know how many channels
     * there are to use */
...
void RT_tell_afni(RT_input *rtin, int mode)
{
    ...
    if (mode == TELL_FINAL)    /* moved here 01 Jun 2009 */
    {
        ...
        /* Cameron modification to support detrended dataset, just copying because we do this to
         * every other dataset */
        if (rtin->detrend_dset != NULL)
        {
...
void RT_tell_afni_one(RT_input *rtin, int mode, int cc)
{
    ...
    if (rtin == NULL || !ISVALID_DSET(rtin->dset[cc]))
    {
        return;
    }
    ...
    /* Cameron Craddock modifications to handle real-time detrend, just copied this from the
     * registered dataset code above */
...

    {
        if (im3d != NULL)
        {
            ...
            if (EQUIV_DSETS(rtin->dset[cc], old_anat))
            {
                THD_update_statistics(rtin->dset[cc]);
            }
            else
...

    {
        ...
        for (ii = 0 ; ii < MAX_CONTROLLERS ; ii++)
        {
            ...
            /* Cameron Craddock modified to support realtime detrend, just copied and modified the
             * code above */
    ...
    if (mode == TELL_FINAL)
    {
        ...
        /* Cameron Craddock to support real-time detrending, just copying everything that you do for
         * reg_dset, i hope that this will write out the dataset at the end of the acquisition */
...
/*--------------------------------------------------------------------------
 * This routine is called when no more data will be added to the incoming dataset(s).
 * ----------------------------------------------------------------------------*/
...
void RT_finish_dataset(RT_input *rtin)
{
    ...
    for (cc = 0 ; cc < rtin->num_chan ; cc++)
    {
        ...
        if (rtin->nvol[cc] < 1)
        {
            ...
            /* Cameron Craddock to support real-time detrend, just copying everything (well most
             * things) that you do to reg_dset */
            if (rtin->detrend_dset != NULL)
            {
...
#if 0 /* leave out for now          20 Jun 2012 [rickr] */
      /***** note: doing this results in a white image window in RT (i.e. when registering, at the
       * end of a run) *****/
...
/*---------------------------------------------------------------------------
 * Do 2D registration of all slices at once, when the realtime dataset is all present and accounted
 *for.
 * -----------------------------------------------------------------------------*/
...
void RT_registration_2D_setup(RT_input *rtin)
{
    ...
    if (RT_registration_set_vr_base(rtin))
    {
        return;                                     /* 26 Aug 2009 [rickr] */
    }
    ...
    if (rtin->reg_base_dset)
    {
        bar = DSET_BRICK_ARRAY(rtin->reg_base_dset, 0);
    }
    else
    {
        bar = DSET_BRICK_ARRAY(rtin->dset[g_reg_src_chan],
                               ibase);
    }
...
void RT_registration_2D_onevol(RT_input *rtin, int tt)
{
    ...
    if (rtin->dset[g_reg_src_chan] == NULL || rtin->reg_dset == NULL)
    {
        return;
    }
...
void MTD_killfunc(MEM_topshell_data *mp)
{
    ...
    if (rtinp != NULL && mp == rtinp->mp)
    {
        if (verbose)
        {
            fprintf(stderr, "RT: user killed active realtime graph\n");
        }
        rtinp->mp = NULL;
    }
    else
    {
        if (verbose)
        {
            fprintf(stderr, "RT: user killed inactive realtime graph\n");
        }
    }

    if (mp->userdata != NULL)
    {
        free(mp->userdata); mp->userdata = NULL;
    }
    return;
}
...
void RT_registration_3D_realtime(RT_input *rtin)
{
    ...
    /* check if enough data to setup (0 -> g_reg_src_chan 27 Oct 2016) */
    {
        ...
        if (RT_will_register_merged_dset(rtin) &&
            rtin->reg_base_index >= rtin->mrg_nvol)
        {
            return;
        }
        ...
        if (rtin->reg_graph == 2)
        {
            ...
            if (rtin->mp != NULL)
            {
                rtin->mp->killfunc = (void_func *)MTD_killfunc;
            }
...
/*---------------------------------------------------------------------------
 * Do 3D registration of all slices at once, when the realtime dataset is all present and accounted
 *for.
 * -----------------------------------------------------------------------------*/
 ...
 *
 * The global g_reg_base_dset variable will be set anytime the base needs to be stored, which is:
 *      1. EXTERN: the base/index is from an External dataset In RT_main: nuke and set
 *g_reg_base_dset from the chosen dset. In any case, apply any g_reg_base_dset in new_RT_input.
 *      2. CUR_KEEP: from current run, keep for future Here (set_vr_base), if g_reg_base_dset is
 *NULL, set to current dset and appropriate volume.  If g_reg_base_dset is not NULL, it was done
 *previously. In the previous case, g_reg_base_dset will have been _copied_ to rtin->reg_base_dset
 *in new_RT_input.
 *. NOT done for CUR: current run and index So if rtin->reg_base_dset, either EXTERN or old CUR_KEEP
 *cases.  Else new CUR_KEEP or just CUR.  Note, rtin->reg_base_dset is always a copy.
 * -------------------------------------------------------------------------*/
int RT_registration_set_vr_base(RT_input *rtin)
{
    ...
    if (rtin->reg_base_mode == RT_RBASE_MODE_CUR)
    {
        RETURN(0);                                           /* nothing to do */
    }
    /* note dset to register                     27 May 2010 [rickr] */

    /* rcr OC - applies to 1 and 2 also, can choose channel */
    if (RT_will_register_merged_dset(rtin))
    {
        dset = rtin->mrg_dset;
    }
    ...
    /* If CUR_KEEP, create the global base dataset (deleting any old one). Note that we still do not
     * need to set rtin->reg_base_dset. */
...
void RT_registration_3D_setup(RT_input *rtin)
{
    ...
    if (rtin->reg_base_dset)
    {
        im = DSET_BRICK(rtin->reg_base_dset, 0);
    }
    else
    {
        im = DSET_BRICK(dset, ibase);
    }
    ...
    case REGMODE_3D_ATEND:
        if (verbose > 1)
        {
            fprintf(stderr, "RT: do full registration\n");
        }
        ept = getenv("AFNI_REALTIME_volreg_maxite");
        kk  = VL_MIT;
        if (ept != NULL)
        {
            kk = strtol(ept, NULL, 10); if (kk <= 0)
            {
                kk = VL_MIT;
            }
        }
    ...
    case REGMODE_3D_ESTIM:                 /* just estimate motion */
        if (verbose > 1)
        {
            fprintf(stderr, "RT: just estimate motion\n");
        }
...
void RT_registration_3D_onevol(RT_input *rtin, int tt)
{
    ...
    /* rcr OC - option to set source channel ChannelMerge MergeRegister modes (RT_CM_RMODE_*)
     *      - NONE
     *      - reg merged
     *      - reg M and chans
     *      - reg chans only     (merging probably happens after chan reg) Registration SourceChan
     * -1..(Nchan-1)
     *      - -1 means last channel (since possibly unknown?)
     *      - 0 is default in 0-based list
     *
     *   have g_reg_src_chan from AFNI_REALTIME_Reg_Source_Chan & GUI set RT_chmrg_reg_mode trace
     *references to reg_chan_mode and RT_CM_RMODE_NONE add variable for Registration:SourceChan for
     *RT_CM_RMODE_POST_MRG: merge reg_chan_dset list
     *       - merging requires float data?
     *   update Help with multi-chan, no reg_dset
     *     - allow reg in multi- mode, esp given Src Chan
     *     - still add 'chans only' to MergeRegister list (i.e. register channels without merging)
     *     - again: maybe change MergeRegister to be (just Register?)
     *           - FLAG to register channels
     *           - MERGE: pre-reg or post-reg?
     *        or FLAG? : register chan or merge
     *           - none
     *           - merge pre-reg
     *           - merge and reg chans
     *           - reg chans and merge
    ...
    if (rtin->reg_chan_mode >= RT_CM_RMODE_REG_CHAN)
    {
        ...
        /* is this needed (aaaa5555)?  if so, needs alt var names dx = qim->dx ; dy = qim->dy ; dz =
         * qim->dz ;   * store for channels */
    ...
    roll *= R2DFAC; if (rtin->hax1 < 0)
    {
        roll = -roll;
    }
    pitch *= R2DFAC; if (rtin->hax2 < 0)
    {
        pitch = -pitch;
    }
    yaw *= R2DFAC; if (rtin->hax3 < 0)
    {
        yaw = -yaw;
    }
    ...
    {
    case ORI_R2L_TYPE: ddy = dx; break;

    case ORI_L2R_TYPE: ddy = -dx; break;

    case ORI_P2A_TYPE: ddz = -dx; break;

    case ORI_A2P_TYPE: ddz = dx; break;

    case ORI_I2S_TYPE: ddx = dx; break;

    case ORI_S2I_TYPE: ddx = -dx; break;
    }

    switch (source->daxes->yyorient)
    {
    case ORI_R2L_TYPE: ddy = dy; break;

    case ORI_L2R_TYPE: ddy = -dy; break;

    case ORI_P2A_TYPE: ddz = -dy; break;

    case ORI_A2P_TYPE: ddz = dy; break;

    case ORI_I2S_TYPE: ddx = dy; break;

    case ORI_S2I_TYPE: ddx = -dy; break;
    }

    switch (source->daxes->zzorient)
    {
    case ORI_R2L_TYPE: ddy = dz; break;

    case ORI_L2R_TYPE: ddy = -dz; break;

    case ORI_P2A_TYPE: ddz = -dz; break;

    case ORI_A2P_TYPE: ddz = dz; break;

    case ORI_I2S_TYPE: ddx = dz; break;

    case ORI_S2I_TYPE: ddx = -dz; break;
    }
    ...
    if (verbose > 1)
    {
        fprintf(stderr, "RTCM motion [%d]: %f %f %f %f %f %f\n",
    ...
    if (rim != NULL && rtin->reg_dset != NULL)
    {
        ...
        /*-- if registering channels with respect to 'source' params, store results (no conversion)
         * 2 Jun 2010 [rickr] --*/
        if (rtin->reg_chan_mode >= RT_CM_RMODE_REG_CHAN && outarr)
        {
            for (cc = 0; cc < rtin->num_chan; cc++)
            {
                ...
                if (tt == 0)
                {
                    EDIT_substitute_brick(rtin->reg_chan_dset[cc], 0,
                                          rtin->datum, outarr->imarr[cc]->im);
                }
                else
                {
                    EDIT_add_brick(rtin->reg_chan_dset[cc], rtin->datum,
 ...
 *  mode = (if >= 0) index of sub-brick to process this time (if <  0) one of the MODE codes above:
 *           INIT_MODE:   create a new dataset, and initialize workspaces UPDATE_MODE: put values
 *into dataset bricks FINAL_MODE:  clean up workspaces
 *
 * The return value is 1 if all is OK, -1 if something bad happens.
 *
 * Adapted 27 Apr 1997 from AFNI_fimmer_compute in afni_fimmer.c -- only the computational stuff is
 *left; the display stuff is relegated to another routine.
 * -----------------------------------------------------------------------------*/
...
int RT_fim_recurse(RT_input *rtin, int mode)
{
    ...
    if (mode == INIT_MODE)
    {
        ...
        im3d = rtin->im3d[0];         if (im3d == NULL)
        {
            return -1;
        }
        ref_ts = im3d->fimdata->fimref; if (ref_ts == NULL)
        {
            return -1;
        }
        ...
        /** at this point, ngood_ref = max number of good reference points, and                  it1
         * = index of first point used in first reference **/
    ...
    if (mode == UPDATE_MODE)
    {
        if (nupdt < MIN_UPDT)
        {
            return 1;          /* can't do anything yet */
        }
        ...
        if (ny_ref == 1)
        {
            ...
            /*--- get alpha (coef) into vval, find max value, scale into brick array ---*/
            ...
            /*--- get correlation coefficient (pcor) into vval, scale into brick array (with fixed
             * scaling factor) ---*/
        ...
        else
        {
            ...
            /*--- for each succeeding ref vector, get results into aval and vval, if |vval| >
             * |rbest|, then use that result instead ---*/
            ...
            /*--- at this point, abest and rbest are the best results, so scale them into the
             * dataset bricks ---*/

            topval = 0.0;
            for (iv = 0 ; iv < nvox ; iv++)
            {
                if (fabs(abest[iv]) > topval)
                {
                    topval = fabs(abest[iv]);
                }
            }
    ...
    if (mode < it1)
    {
        return 1;               /* nothing to do before first time */
    }

    /*---- at first time, find values above threshold to fim:
     *       find the mean of the first array, compute the threshold (fthr) from it, make indx[i] be
     *the 3D index of the i-th voxel above threshold ----*/
    ...
    for (it = start_index; it <= mode; it++)
    {
        ...
        if (it >= nx_ref)
        {
            return 1;                           /* can't update without references! */
        }
        ...
        if (nnow > 0)
        {
            nupdt++;            /* at least one ideal vector was used */
        }
    }
...
static int rt_run_drive_wait_commands(rt_string_list *slist)
{
    ...
    if (!slist || !slist->list || slist->len <= 0)
    {
        return 0;
    }
    ...
    for (c = 0; c < drive_wait_list.len; c++)
    {
        ...
        if (verbose > 1)
        {
            fprintf(stderr, "RT: executing DRIVE_WAIT cmd: %s\n", cmd);
        }
...
void RT_test_callback(void *junk)
{
    ...
    if (rts == NULL)
    {
        ERROR_message("bad call to RT_test_callback"); return;
    }
 ...
 * MRI_IMAGE * RT_mergerize(int nds , THD_3dim_dataset **ds , int iv, int * dlist) but pass rtin
 *instead of elements                     12 Mar 2015 [rickr] */
...
MRI_IMAGE * RT_mergerize(RT_input *rtin, int iv)
{
    ...
    if (nds <= 1 || ds == NULL || !ISVALID_DSET(ds[0]))
    {
        return NULL;
    }
    if (iv < 0 || iv >= DSET_NVALS(ds[0]))
    {
        return NULL;
    }
    ...
    case RT_CHMER_OPT_COMB:    /* output datum is always float */
    {
        ...
        for (ii = 0 ; ii < nvox ; ii++)
        {
             ...
             *  """
             *
             *  Nv,Ne,Nt = data.shape ft2s  = t2s[:,np.newaxis] alpha = tes * np.exp(-tes /ft2s)
             *alpha = np.tile(alpha[:,:,np.newaxis],(1,1,Nt)) octs  = np.average(data,axis =
             * 1,weights=alpha) return octs
             *
...
/* RT_detrend_getenv access the AFNI_REALTIME_DETREND_MODE and AFNI_REALTIME_DETREND_POLORT
 * environment variables and copies the results to rtin */
void RT_detrend_getenv(RT_input *rtin)
{
    ...
    if (ept != NULL)
    {
        ...
        /* we put detrend_mode into rtin, so it can be used outside of this function */
        if (ii >= 0 && ii <= 32)
        {
...
/* RT_detrend makes use of Bob's recursive regression method to orthogonalize the received data to a
 * user specified set of regressors. Everything that we need is either in the
 * AFNI_REALTIME_DETREND_MODE or AFNI_REALTIME_DETREND_POLORT environment variables or in the
 * RT_input data structure. This code relies heavily on the code found in the RT_fim_recurse
 * function. */
void RT_detrend(RT_input *rtin, int mode)
{
    /* these are static variables, and their values persist across several call so this function */
    ...
    int nvol = 0; /* counts the number of volumes received, this will be set to either rtin->nvol[0]
                   * or rtin->reg_nvol depending on whether registration is on or off */
    static int     dtype;
    static int     nxyz = 0;
    static int     nvox = 0;
    ...
    /* there is some difference between when data becomes available. If registration is off, we just
     * care that at least one image is received. If online motion correction is selected, then we
     * want to make sure that at least one image has passed through motion correction. There is some
     * descrepencies between these two values depending on whether motion correction is performed to
     * the first image of the series being received, or if registering to an external dataset.
     * Failure to account for this will result in the automask function returning NULL and
     * detrending being turned off. This was a segmentation fault, but I modified mri_automask to
     * just return NULL instead of giving us the finger */
    ...
    if (nvol > 0)
    {
        ...
        if ((detrendIsInitialized == 0))
        {
            ...
            {
                /* motion ort regression specified, but registration not being performed, turn it
                 * off */
                snprintf(rt_errorString, RT_LONG_STR_LEN,
            ...
            if (rtin->detrend_mode & ~RT_DETREND_SMOOTH)
            {
                ...
                /* open ort file, go ahead and do it here, so that we can create a header for the
                 * ort file */
                ...
                /* now we have the number of orts, so we can allocate an array to hold them, since
                 * we are using recursive regression, we only need one timepoint */
                detrend_orts = (float *)malloc(detrend_num_ort * sizeof(float));
                if (detrend_orts == NULL)
                {
                    /* couldn't get memory, so print an error and turn off future detrending */
                    snprintf(rt_errorString, RT_LONG_STR_LEN,
            ...
            if (rtin->detrend_mode & RT_DETREND_SMOOTH)
            {
                /* copied code from mri_blur3D_addfwhm_speedy, would just call this function, but
                 * the voxel dimensions are not correctly set until BRICK is written */
