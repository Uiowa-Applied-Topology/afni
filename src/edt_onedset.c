...
/*--------------------------------------------------------------------
 * routine to edit an input dataset in place according to inputs in "edopt" (see editvol.h).
 *
 * Feb 1996: This routine is much more complex now due to the need to deal with byte, short, float,
 *or complex data in sub-bricks.
 *
 * 30 Nov 1997: added ability to edit a given sub-brick, using the edopt->iv_fim entry
 *
...
void EDIT_one_dataset(THD_3dim_dataset *dset, EDIT_options *edopt)
{
    ...
    if (ISANAT(dset))
    {
        ...
        if (verbose)
        {
            fprintf(stderr, "--- EDIT_one_dataset: fim index = %d\n", iv_fim);
        }
    ...
    if (ISFUNC(dset))
    {
        ...
        if (verbose)
        {
            fprintf(stderr, "--- EDIT_one_dataset: thr index = %d\n", iv_thr);
        }
...
#ifdef AFNI_DEBUG
        { char str[256];
          sprintf(str, "Func dset: iv_fim=%d type=%s fac=%g", iv_fim, MRI_TYPE_name[fim_type],
                  fimfac);
          STATUS(str);
          if (iv_thr >= 0)
          {
              sprintf(str, "Func dset: iv_thr=%d type=%s fac=%g", iv_thr, MRI_TYPE_name[thr_type],
                      thrfac);
              STATUS(str);
          }
        }
    ...
    if (edit_thtoin && iv_thr >= 0)
    {
        ...
        if (verbose)
        {
            fprintf(stderr, "--- EDIT_one_dataset: copy thr over fim\n");
        }

        /****
        *     Find scaling factors for various conversions (0 --> no scaling) scaling    = factor to
        *actually scale data by when copying to new brick new_fimfac = factor to later scale data by
        *when converting to floats
        ****/
    ...
    if (edit_noneg)    /* meaningless for byte and complex */
    {
        ...
        if (verbose)
        {
            fprintf(stderr, "--- EDIT_one_dataset: remove negative values\n");
        }
    ...
    if (edit_abs)    /* meaningless for byte */
    {
        ...
        if (verbose)
        {
            fprintf(stderr, "--- EDIT_one_dataset: take absolute value\n");
        }
    ...
    if (iv_thr >= 0 && (edit_thresh > 0.0 || edit_thresh > edit_thbot))
    {
        if (verbose)
        {
            fprintf(stderr, "--- EDIT_one_dataset: apply threshold\n");
        }
        ...
        case MRI_float: {
            float thrplu, thrmin;
            thrplu = edit_thresh; if (thrfac > 0.0)
            {
                thrplu /= thrfac;
            }
            thrmin = edit_thbot; if (thrfac > 0.0)
            {
                thrmin /= thrfac;
            }
            switch (fim_type)
    ...
    {
        EDIT_blur_allow_fir(0);                                /* 04 Oct 2005 */
    }
...

    {
        if (verbose)
        {
            fprintf(stderr, "--- EDIT_one_dataset: blurring_3D fim\n");
        }
    ...
    else if (edit_blur > 0.0)
    {
        if (verbose)
        {
            fprintf(stderr, "--- EDIT_one_dataset: blurring fim\n");
        }
    ...
    if ((edit_thrblur > 0.0) && (vthr != NULL))
    {
        if (verbose)
        {
            fprintf(stderr, "--- EDIT_one_dataset: blurring threshold\n");
        }
    ...
    if (edit_zvol)
    {
        ...
        if (ix1 > ix2)
        {
            ii = ix1; ix1 = ix2; ix2 = ii;
        }
        if (jy1 > jy2)
        {
            ii = jy1; jy1 = jy2; jy2 = ii;
        }
        if (kz1 > kz2)
        {
            ii = kz1; kz1 = kz2; kz2 = ii;
        }

#ifdef AFNI_DEBUG
        { char str[256];
          sprintf(str, "edit_zvol: x1=%g x2=%g y1=%g y2=%g z1=%g z2=%g",
                  edopt->zv_x1, edopt->zv_x2, edopt->zv_y1, edopt->zv_y2, edopt->zv_z1,
                  edopt->zv_z2);
          STATUS(str);
...

    {
        ...
        if (verbose)
        {
            fprintf(stderr, "--- EDIT_one_dataset: clustering with rmm=%g vmul=%g\n",
                    rmm, vmul);
        }
        ...
        /* ZSS March 03 2010, changed from MCW_find_clusters to NIH_find_clusters to accommodate
         * isomode */
        ...
        if (edit_clust == ECFLAG_DEPTH)
        {
            ...
            for (iclu = 0 ; iclu < clar->num_clu ; iclu++)
            {
                if (clar->clar[iclu] && clar->clar[iclu]->num_pt > 0)
                {
                    ...
                    if (!mask)
                    {
                        mask = (byte *)calloc(nxyz, sizeof(byte));
                    }
                    if (!depth)
                    {
                        depth = (short *)calloc(nxyz, sizeof(short));
                    }
                    if (!mask || !depth)
                    {
                    ...
                    case MRI_complex:
                        for (ii = 0 ; ii < nxyz ; ii++)
                        {
                            if (mask[ii])
                            {
                                cfim[ii].r = cfim[ii].i = (float)depth[ii];
                            }
                        }
                        break;
    ...
    if (filter_opt > FCFLAG_NONE)
    {
        if (verbose)
        {
            fprintf(stderr, "--- EDIT_one_dataset: filtering fim\n");
        }
    ...
    if ((thrfilter_opt > FCFLAG_NONE) && (vthr != NULL))
    {
        if (verbose)
        {
            fprintf(stderr, "--- EDIT_one_dataset: filtering thr\n");
        }
    ...
    if (edit_scale)
    {
        ...
        if (verbose)
        {
            fprintf(stderr, "--- EDIT_one_dataset: scaling fim to max\n");
        }
    ...
    if (edit_mult != 0.0)
    {
        ...
        if (verbose)
        {
            fprintf(stderr, "--- EDIT_one_dataset: multiplying fim\n");
        }
        ...
        case MRI_float: for (ii = 0 ; ii < nxyz ; ii++)
            {
                ffim[ii] *= edit_mult;
            }
            break;
    ...
    if (edopt->zscore)                           /* 07 Jun 1999! How did this get lost? */
    {
        ...
        if (FUNC_IS_STAT(kv) && kv != FUNC_ZT_TYPE)
        {
            ...
            if (verbose)
            {
                fprintf(stderr, "--- EDIT_one_dataset: converting to zscore\n");
            }
