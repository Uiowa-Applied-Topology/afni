...
/*
 * This program performs 2d image registration of slices contained in an AFNI 3d+time dataset.  This
 *program was adapted from plug_imreg.c and imreg.c.
 *
 *
 * File:    2dImReg.c Author:  B. Douglas Ward Date:    04 February 1998
 *
 *
 * Mod:     Added routines to write the registration parameters, and the RMS error, to user
 *specified ASCII files. Date:    20 March 1998
 *
 * Mod:     Added option to change dx and dy output format from pixels to mm. Date:    24 March 1998
 *
 * Mod:     No longer assume that input and base datasets have same length. This problem was
 *reported by Paul Reber. Date:    3 June 1998
 *
 * Mod:     Routine eval_registration extended to include byte and float datum types for base and
 *input datasets. Date:    3 June 1998
 *
 * Mod:     Corrected problem with base image memory deallocation. Date:    20 July 1998
 *
 ...
 *
 * Mod:     If nsl == 0, set num_slices equal to nz.   [4 occurrences] Date:    06 October 2003
 *[rickr]
 *
 * Mod:     allow dx!=dy; use ||dx|-|dy||/|dx| > 0.0001 for failure apply to both square image and
 *against basefile Date:    22 March 2024  [rickr]  (gee, only 20 years since the last change)
 */
 ...
 *
 * Original tests were if d0 != d1, fail.  But if deltas come from an oblique matrix, they will
 *often not be exact.
 *
 ...
 *
 * return 0 or 1 to indicate whether the comparison is true [22 Mar 2024 rickr]
 */
int approx_equal(float d0, float d1, float epsilon, int report_fail)
{
...

{
    ...
    while (nopt < argc)
    {
        ...
        if (strncmp(argv[nopt], "-input", 6) == 0)
        {
            ...
            if (nopt >= argc)
            {
                IR_error("Need argument after -input ");
            }
        ...
        if (strncmp(argv[nopt], "-basefile", 9) == 0)
        {
            ...
            if (nopt >= argc)
            {
                IR_error("Need argument after -basefile ");
            }
        ...
        if (strncmp(argv[nopt], "-base", 5) == 0)
        {
            ...
            if (nopt >= argc)
            {
                IR_error("Need argument after -base ");
            }
        ...
        if (strncmp(argv[nopt], "-prefix", 7) == 0)
        {
            ...
            if (nopt >= argc)
            {
                IR_error("Need argument after -prefix ");
            }
        ...
        if (strncmp(argv[nopt], "-dprefix", 8) == 0)
        {
            ...
            if (nopt >= argc)
            {
                IR_error("Need argument after -dprefix ");
            }
            (*option_data)->dprefix
                = (char *)malloc(sizeof(char) * MAX_NAME_LENGTH);
        ...
        if (strncmp(argv[nopt], "-rprefix", 8) == 0)
        {
            ...
            if (nopt >= argc)
            {
                IR_error("Need argument after -rprefix ");
            }
            (*option_data)->rprefix
                = (char *)malloc(sizeof(char) * MAX_NAME_LENGTH);
        ...
        if (strncmp(argv[nopt], "-fine", 5) == 0)
        {
            ...
            if (nopt + 2 >= argc)
            {
                IR_error("Need 3 arguments after -fine ");
            }
...
/*
 * Routine to make a copy of a dataset, with data attached. This routine is copied directly from
 *afni_plugin.c
 */
...
/*
 * Main routine for this program. If the return string is not NULL, some error transpired, and the
 *program will display the error message.
 */
...
)
{
    ...
    {
        /*     No need to quit, works fine.  ZSS 07 Only if nx >= ny (so fix might be easy).  12 Jan
         * 2010 [rickr] */
        return "***********************************\n"
    ...
    for (kk = 0 ; kk < nz ; kk++)
    {
        ...
        if (!failed)
        {
            DESTROY_IMARR(ims_out);             /* 08 Apr 2008 */
        }
    }
