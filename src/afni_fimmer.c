                            ...
                            XtPointer cd, MCW_choose_cbs *cbs)
{
    ...
    if (!IM3D_VALID(im3d) || im3d->type != AFNI_3DDATA_VIEW)
    {
        EXRETURN;
    }
...
void AFNI_fimmer_setref(Three_D_View *im3d, MRI_IMAGE *tsim)
{
    ...
    /* 12 Nov 1996: fix problem with freeing old ref that might be in the library */
...
void AFNI_fimmer_setort(Three_D_View *im3d, MRI_IMAGE *tsim)
{
    ...
    /* 12 Nov 1996: fix problem with freeing old ort that might be in the library */
                                       ...
                                       THD_session *sess, int code, int ucode)
{
    ...
    /** find the mean of the first array, compute the threshold (fthr) from it, make indx[i] be the
     * 3D index of the i-th voxel above threshold **/
    ...
    if ((code & FIM_ALPHA_MASK) != 0)
    {
        ibr_fim = nbrik; nbrik++;
    }
    if ((code & FIM_BEST_MASK) != 0 && ny_ref > 1)
    {
        ibr_best = nbrik; nbrik++;
    }
    if ((code & FIM_PERC_MASK) != 0)
    {
        ibr_perc = nbrik; nbrik++;
    }
    if ((code & FIM_PAVE_MASK) != 0)
    {
        ibr_pave = nbrik; nbrik++;
    }
    if ((code & FIM_BASE_MASK) != 0)
    {
        ibr_base = nbrik; nbrik++;
    }
    if ((code & FIM_AVER_MASK) != 0)
    {
        ibr_aver = nbrik; nbrik++;
    }
    if ((code & FIM_CORR_MASK) != 0)
    {
        ibr_corr = nbrik; nbrik++;
    }
    if ((code & FIM_PTOP_MASK) != 0)
    {
        ibr_ptop = nbrik; nbrik++;
    }
    if ((code & FIM_TOPL_MASK) != 0)
    {
        ibr_topl = nbrik; nbrik++;
    }
    if ((code & FIM_SIGM_MASK) != 0)
    {
        ibr_sigm = nbrik; nbrik++;
    }
    ...
    if (ny_ref > 1)
    {
        ...
        if (sgbest == NULL)
        {
            ...
            if (ibest != NULL)
            {
                free(ibest);                /* 15 Dec 1997 */
            }
            if (pbest != NULL)
            {
                free(pbest);                /* 16 Jan 1998 */
            }
            if (bbest != NULL)
            {
                free(bbest);                /* 16 Jan 1998 */
            }
            if (pval != NULL)
            {
                free(pval);                 /* 16 Jan 1998 */
            }
            if (bval != NULL)
            {
                free(bval);                 /* 16 Jan 1998 */
            }
            if (paval != NULL)
            {
                free(paval);                /* 08 Sep 1999 */
            }
            if (avval != NULL)
            {
                free(avval);                /* 08 Sep 1999 */
            }
            if (pabest != NULL)
            {
                free(pabest);               /* 08 Sep 1999 */
            }
            if (avbest != NULL)
            {
                free(avbest);               /* 08 Sep 1999 */
            }
            if (ptval != NULL)
            {
                free(ptval);                /* 03 Jan 2000 */
            }
            if (tlval != NULL)
            {
                free(tlval);                /* 03 Jan 2000 */
            }
            if (sgval != NULL)
            {
                free(sgval);                /* 03 Jan 2000 */
            }
            if (ptbest != NULL)
            {
                free(ptbest);               /* 03 Jan 2000 */
            }
            if (tlbest != NULL)
            {
                free(tlbest);               /* 03 Jan 2000 */
            }
            if (sgbest != NULL)
            {
                free(sgbest);               /* 03 Jan 2000 */
            }
            RETURN(NULL);
        }
    }
...

    {
        ...
        if (nnow > 0)
        {
            nupdt++;            /* number of time points that had updates */
        }
#ifndef DONT_USE_METER
...
# define METERIZE(ib)    /*nada*/
    ...
    if (ny_ref == 1)
    {
        ...
        if (ibr_perc >= 0)
        {
            ...
            if (top_perc > 0.0)
            {
                EDIT_clip_float(top_perc, nvox, vval);
            }
    ...
    else
    {
        ...
        for (iv = 0 ; iv < nvox ; iv++)
        {
            ibest[iv] = 1;                            /* 15 Dec 1997 */
        }

        /*--- for each succeeding ref vector, get results into aval and vval, if |vval| > |rbest|,
         * then use that result instead ---*/
        ...
        /*--- at this point, abest and rbest are the best results, so scale them into the dataset
         * bricks ---*/
        ...
        if (ibr_perc >= 0)
        {
            ...
            if (top_perc > 0.0)
            {
                EDIT_clip_float(top_perc, nvox, pbest);
            }

            topval = 0.0;
            for (iv = 0 ; iv < nvox ; iv++)
            {
                if (fabs(pbest[iv]) > topval)
                {
                    topval = fabs(pbest[iv]);
                }
            }
        ...
        if (ibr_pave >= 0)
        {
            ...
            if (top_perc > 0.0)
            {
                EDIT_clip_float(top_perc, nvox, pabest);
            }

            topval = 0.0;
            for (iv = 0 ; iv < nvox ; iv++)
            {
                if (fabs(pabest[iv]) > topval)
                {
                    topval = fabs(pabest[iv]);
                }
            }
        ...
        if (ibr_ptop >= 0)
        {
            ...
            if (top_perc > 0.0)
            {
                EDIT_clip_float(top_perc, nvox, ptbest);
            }

            topval = 0.0;
            for (iv = 0 ; iv < nvox ; iv++)
            {
                if (fabs(ptbest[iv]) > topval)
                {
                    topval = fabs(ptbest[iv]);
                }
            }
        ...
        if (ibr_base >= 0)
        {
            ...
            for (iv = 0 ; iv < nvox ; iv++)
            {
                if (fabs(bbest[iv]) > topval)
                {
                    topval = fabs(bbest[iv]);
                }
            }
        ...
        if (ibr_aver >= 0)
        {
            ...
            for (iv = 0 ; iv < nvox ; iv++)
            {
                if (fabs(avbest[iv]) > topval)
                {
                    topval = fabs(avbest[iv]);
                }
            }
        ...
        if (ibr_topl >= 0)
        {
            ...
            for (iv = 0 ; iv < nvox ; iv++)
            {
                if (fabs(tlbest[iv]) > topval)
                {
                    topval = fabs(tlbest[iv]);
                }
            }
        ...
        if (ibr_sigm >= 0)
        {
            ...
            for (iv = 0 ; iv < nvox ; iv++)
            {
                if (fabs(sgbest[iv]) > topval)
                {
                    topval = fabs(sgbest[iv]);
                }
            }
    ...
    /*** Set the brick factors for the new dataset, no matter how it was computed above.       ***/
...
#define MAXTS      32 /* number of timeseries to process at once */
    ...
    if (ucode != 0)
    {
        ...
        if (nuse == 0)
        {
            goto final_exit;            /* shouldn't happen */
        }
        ...
        /* loop over voxels, assemble time series, call functions to put results in val[], store
         * float outputs in vbr[][]          */
         ...
         *   make short space for it, scale and stored floats into this space , attach it to the
         *output dataset as a new brick */
...
void AFNI_fimmer_menu_CB(Widget w, XtPointer cd, XtPointer cbs)
{
    ...
    /*** Pick a time dependent dataset. This code is mostly taken from AFNI_choose_dataset_CB ***/
...
/*--------------------------------------------------------------------
 * Routines to deal with widgets that are allowed to be active during 'real-time' operations.
 * ----------------------------------------------------------------------*/
...
void AFNI_add_interruptable(Widget w)
{
    ...
    wwin = XtWindowOfObject(w); if (wwin == (Window)NULL)
    {
        return;
    }
...
void AFNI_process_interrupts(Widget w)
{
    ...
    if (GLOBAL_library.interruptables.windows == NULL ||
        GLOBAL_library.interruptables.windows->num == 0)
    {
        return;
    }
                           ...
                           KeyPressMask | KeyReleaseMask, &ev))
    {
        ...
        if (ii == nwin)
        {
            BEEPIT;                          /* beep for non-allowed event */
        }
...
void AFNI_fimmer_fix_optmenu(Three_D_View *im3d)
{
    ...
    if (im3d->fimdata->fimdset == NULL)
    {
        EXRETURN;
    }
    ntime = DSET_NUM_TIMES(im3d->fimdata->fimdset); if (ntime < 2)
    {
        EXRETURN;
    }
