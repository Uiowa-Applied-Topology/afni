...
void FILTER_adaptive(int num, float *vec)
{
    ...
    if (aXX == NULL)
    {
        aXX = (float *)malloc(sizeof(float) * nXX);
    }
...
void DES_despike9(int num, float *vec)
{
    ...
    if (deswks == NULL)
    {
        deswks = (float *)malloc(sizeof(float) * (4 * num));
    }
...
int main(int argc, char *argv[])
{
    ...
    while (nopt < argc && argv[nopt][0] == '-')
    {
            ...
            strcasecmp(argv[nopt], "-inset") == 0)
        {
            ...
            if (old_dset != NULL)
            {
                ERROR_exit("you can't have 2 input datasets!");
            }
    ...
    if (nffunc == 0)
    {
        ERROR_exit("No -filter options given !? :(");
    }
    ...
    if (hwid > 0)
    {
        setup_adaptive_filter(hwid, nvals);
    }
