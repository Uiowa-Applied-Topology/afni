...
THD_3dim_dataset * THD_mean_dataset(int nds, THD_3dim_dataset **dsin, int ivbot, int ivtop,
                                    int verb)
{
    ...
    if (nds <= 0 || dsin == NULL || !ISVALID_DSET(dsin[0]))
    {
        RETURN(NULL);
    }
    if (ivbot < 0)
    ...
    if (ivtop < ivbot || ivtop >= DSET_NVALS(dsin[0]))
    {
        ivtop = DSET_NVALS(dsin[0]) - 1;
    }
    if (ivtop < ivbot)
    {
        RETURN(NULL);
    ...
    for (dd = 0 ; dd < nds ; dd++)
    {
        if (!ISVALID_DSET(dsin[dd]))
        {
            RETURN(NULL);
        }
        if (ivtop >= DSET_NVALS(dsin[dd]))
        {
            RETURN(NULL);
        }
        if (DSET_NX(dsin[dd]) != nx ||
            DSET_NY(dsin[dd]) != ny || DSET_NZ(dsin[dd]) != nz)
        {
            RETURN(NULL);
        }
    }
    ...
    for (ndd = dd = 0 ; dd < nds ; dd++)
    {
        ...
        for (vv = 0 ; vv < nbr ; vv++)
        {
            ...
            {
            default:
                ERROR_message("THD_mean_dataset(): can't use sub-brick with datum=%d",
                              DSET_BRICK_TYPE(dsin[dd], kk));
                ndd--;
                break;

            case MRI_float: {
                float *pp = (float *)DSET_ARRAY(dsin[dd], kk);
                fac = DSET_BRICK_FACTOR(dsin[dd], kk); if (fac == 0.0f)
                {
                    fac = 1.0f;
                }
                for (ii = 0 ; ii < nxyz ; ii++)
