 ...
 *  - The method is to find which permutation of (x,y,z) has the smallest angle to the (i,j,k) axes
 *directions.
 *  - 27 Aug 2003 - RWCox
 * -----------------------------------------------------------------------------*/
...
THD_ivec3 THD_matrix_to_orientation(THD_mat33 R)
{
    ...
    if (detQ == 0.0)
    {
        return vor;               /* shouldn't happen unless user is a dufis or oufis */
    }
...
#if 0
                                fprintf(stderr,
                                        "mattor: vbest=%g (%g) i=%d j=%d k=%d p=%d q=%d r=%d\n",
                                        val,
                                        2.0 * acos(0.5 * sqrt(1.0 + vbest)) * (180.0 / 3.14159265),
                                        i, j, k, p, q, r);
#endif
                            }
                        }
...
float MAT44_angle(mat44 amat, mat44 bmat)
{
    ...
    v1 = MAT44_DET(amat); if (v1 == 0.0f)
    {
        return PI;                                      /* not a valid matrix! */
    }
    v2 = MAT44_DET(bmat); if (v2 == 0.0f)
    {
        return PI;
    }
    if (v1 * v2 < 0.0f)
...
/* Rotation matrix of angle theta (radians) about the axis along vector (ax,ay,az) - right hand
 * rule. [05 Nov 2020 RWC] See https://mathworld.wolfram.com/RodriguesRotationFormula.html
 *//*-------------------------------------------------------------------------*/
...
mat33 THD_mat33_generic_rotation(float theta, float ax, float ay, float az)
{
    ...
    if (theta == 0.0f)
    {
        return mrot;                        /* that was easy */
    }
    aqq = sqrtf(ax * ax + ay * ay + az * az);
    if (aqq == 0.0f)
    {
        return mrot;                        /* hopeless loser user */
    }
