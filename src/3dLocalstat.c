...
int LS_decode_parameters(char *str, float *params)
{
    ...
    for (k = 0; k < ncol; ++k)
    {
        ...
        for (ii = 0; ii < stp - icol[k]; ++ii)
        {
            strbuf[ii] = str[icol[k] + ii + 1];
        }
        strbuf[stp - icol[k]] = '\0';
        /* fprintf(stderr, ">>>%s<<<\n", strbuf); */
...
int main(int argc, char *argv[])
{
    ...
    while (iarg < argc && argv[iarg][0] == '-')
    {
        ...
        if (strcmp(argv[iarg], "-input") == 0)
        {
            if (inset != NULL)
            {
                ERROR_exit("Can't have two -input options");
            }
            if (++iarg >= argc)
            {
        ...
        if (strcmp(argv[iarg], "-label_ext") == 0)
        {
            if (++iarg >= argc)
            {
                ERROR_exit("Need argument after '-label_ext'");
            }
            lab_ext = argv[iarg];
            iarg++; continue;
        }
        ...
        if (strcmp(argv[iarg], "-mask") == 0)
        {
            ...
            {
                ERROR_exit("Can't have two mask inputs");
            }
            ...
            if (mmm < 2)
            {
                ERROR_exit("Mask is too small to process");
            }
            iarg++; continue;
        }
        ...
        if (strcmp(argv[iarg], "-automask") == 0)
        {
            if (mask != NULL)
            {
                ERROR_exit("Can't have -automask and -mask");
            }
            automask = 1;
            iarg++; continue;
        }

        /* Some -stat options must be processed after input and neighborhoods are generated, so
         * brief check here followed by processing below */
        if (strcmp(argv[iarg], "-stat") == 0)
        {
            ...
            if (++iarg >= argc)
            {
                ERROR_exit("Need argument after '-stat'");
            }

            cpt = argv[iarg]; if (*cpt == '-')
            {
                cpt++;
            }
        ...
        if (strcmp(argv[iarg], "-nbhd") == 0)
        {
            ...
            if (ntype > 0)
            {
                ERROR_exit("Can't have 2 '-nbhd' options");
            }
            if (++iarg >= argc)
            {
                ERROR_exit("Need argument after '-nbhd'");
            }
            ...
            if (strncasecmp(cpt, "SPHERE", 6) == 0)
            {
                ...
                if (na == 0.0f)
                {
                    ERROR_exit("Can't have a SPHERE of radius 0");
                }
                ntype = NTYPE_SPHERE;
            }
            ...
            else if (strncasecmp(cpt, "RHDD", 4) == 0)
            {
                ...
                if (na == 0.0f)
                {
                    ERROR_exit("Can't have a RHDD of radius 0");
                }
                ntype = NTYPE_RHDD;
            }
            else if (strncasecmp(cpt, "TOHD", 4) == 0)
            {
                ...
                if (na == 0.0f)
                {
                    ERROR_exit("Can't have a TOHD of radius 0");
                }
                ntype = NTYPE_TOHD;
            }
            else
        ...
        {
            if (strcmp(argv[iarg], "-reduce_restore_grid") == 0)
            {
                restore_grid = 1;
            }
        ...
        if (strcmp(argv[iarg], "-grid_rmode") == 0)
        {
            if (++iarg >= argc)
            {
                ERROR_exit("Need argument after '-grid_rmode'");
            }
        ...
        if (strcmp(argv[iarg], "-fillvalue") == 0)
        {
            if (++iarg >= argc)
            {
                ERROR_exit("Need argument after '-fillvalue'");
            }
        ...
        if (strcmp(argv[iarg], "-unfillvalue") == 0)
        {
            if (++iarg >= argc)
            {
                ERROR_exit("Need argument after '-unfillvalue'");
            }
        ...
        if (strcmp(argv[iarg], "-maskvalue") == 0)
        {
            if (++iarg >= argc)
            {
                ERROR_exit("Need argument after '-maskvalue'");
            }
        ...
        if (strcmp(argv[iarg], "-maskvalue2") == 0)
        {
            if (++iarg >= argc)
            {
                ERROR_exit("Need argument after '-maskvalue2'");
            }
    ...
    if (ncode <= 0)
    {
        ERROR_exit("No '-stat' options given?");
    }
    ...
    while (iarg < argc)
    {
        if (strcmp(argv[iarg], "-stat") == 0)
        {
            ...
            if (strcasecmp(cpt, "mean") == 0)
            {
                code[ncode++] = NSTAT_MEAN;
            }
            else if (strcasecmp(cpt, "stdev") == 0)
            {
                code[ncode++] = NSTAT_SIGMA;
            }
            else if (strcasecmp(cpt, "var") == 0)
            {
                code[ncode++] = NSTAT_VAR;
            }
            else if (strcasecmp(cpt, "cvar") == 0)
            {
                code[ncode++] = NSTAT_CVAR;
            }
            else if (strcasecmp(cpt, "median") == 0)
            {
                code[ncode++] = NSTAT_MEDIAN;
            }
            else if (strcasecmp(cpt, "osfilt") == 0)
            {
                code[ncode++] = NSTAT_OSFILT;
            }
            else if (strcasecmp(cpt, "MAD") == 0)
            {
                code[ncode++] = NSTAT_MAD;
            }
            else if (strcasecmp(cpt, "mode") == 0)
            {
                code[ncode++] = NSTAT_MODE;
            }
            else if (strcasecmp(cpt, "nzmode") == 0)
            {
                code[ncode++] = NSTAT_NZMODE;
            }
            else if (strcasecmp(cpt, "P2skew") == 0)
            {
                code[ncode++] = NSTAT_P2SKEW;
            }
            else if (strcasecmp(cpt, "kurt") == 0)
            {
                code[ncode++] = NSTAT_KURT;
            }
            else if (strcasecmp(cpt, "min") == 0)
            {
                code[ncode++] = NSTAT_MIN;
            }
            else if (strcasecmp(cpt, "max") == 0)
            {
                code[ncode++] = NSTAT_MAX;
            }
            else if (strcasecmp(cpt, "absmax") == 0)
            {
                code[ncode++] = NSTAT_ABSMAX;
            }
            else if (strcasecmp(cpt, "mconex") == 0)
            {
                code[ncode++] = NSTAT_MCONEX;
            }
            else if (strcasecmp(cpt, "num") == 0)
            {
                code[ncode++] = NSTAT_NUM;
            }
            else if (strcasecmp(cpt, "nznum") == 0)
            {
                code[ncode++] = NSTAT_NZNUM;
            }
            else if (strcasecmp(cpt, "fnznum") == 0)
            {
                code[ncode++] = NSTAT_FNZNUM;
            }
            else if (strcasecmp(cpt, "filled") == 0)
            {
                code[ncode++] = NSTAT_FILLED;
            }
            else if (strcasecmp(cpt, "unfilled") == 0)
            {
                code[ncode++] = NSTAT_UNFILLED;
            }
            else if (strcasecmp(cpt, "has_mask") == 0)
            {
                code[ncode++] = NSTAT_MASKED;
            }
            else if (strcasecmp(cpt, "has_mask2") == 0)
            {
                code[ncode++] = NSTAT_MASKED2;
            }
            else if (strcasecmp(cpt, "sum") == 0)
            {
                code[ncode++] = NSTAT_SUM;
            }
            else if (strcasecmp(cpt, "rank") == 0)
            {
                code[ncode++] = NSTAT_RANK;
            }
            else if (strcasecmp(cpt, "frank") == 0)
            {
                code[ncode++] = NSTAT_FRANK;
            }
            else if (strcasecmp(cpt, "rankinv") == 0)
            {
                code[ncode++] = NSTAT_RANKINV;
            }
            else if (strcasecmp(cpt, "frankinv") == 0)
            {
                code[ncode++] = NSTAT_FRANKINV;
            }
            else if (strcasecmp(cpt, "fwhm") == 0)
            {
            ...
            else if (strcasecmp(cpt, "diffs") == 0)
            {
                code[ncode++] = NSTAT_diffs0;
                code[ncode++] = NSTAT_diffs1;
                code[ncode++] = NSTAT_diffs2;
            }
            else if (strcasecmp(cpt, "adiffs") == 0)
            {
                code[ncode++] = NSTAT_adiffs0;
                code[ncode++] = NSTAT_adiffs1;
                code[ncode++] = NSTAT_adiffs2;
            }
            else if (strcasecmp(cpt, "mMP2s") == 0)
            {
                code[ncode++] = NSTAT_mMP2s0;
                code[ncode++] = NSTAT_mMP2s1;
                code[ncode++] = NSTAT_mMP2s2;
            }
            else if (strcasecmp(cpt, "mmMP2s") == 0)
            {
                code[ncode++] = NSTAT_mmMP2s0;
            ...
            else if (strcasecmp(cpt, "list") == 0)
            {
                ...
                for (ipv = 0; ipv < nbhd->num_pt; ++ipv)
                {
                    code[ncode++] = NSTAT_LIST;
                }
            }
            else if (strncasecmp(cpt, "hist", 4) == 0)
            {
                 ...
                 *(int)codeparams[ncode][0],      codeparams[ncode][1], codeparams[ncode][2],
                 *(int)codeparams[ncode][3], (int)codeparams[ncode][4]); */
                stp = (int)codeparams[ncode][3];
                for (ipv = 0; ipv < stp; ++ipv)
                {
                    code[ncode++] = NSTAT_HIST;
                }
            }
            else if (strncasecmp(cpt, "perc", 4) == 0)
            {
                ...
                {
                    stp = codeparams[ncode][2]; if (stp == 0)
                    {
                        stp = strt;
                    }
                }
                ...
                for (ipv = 0; ipv < iizz; ++ipv)
                {
                    code[ncode++] = NSTAT_PERCENTILE;
                }
            }
            else if (strcasecmp(cpt, "fwhmbar") == 0)
            {
                code[ncode++] = NSTAT_FWHMbar;
            }
            else if (strcasecmp(cpt, "fwhmbar12") == 0)
            {
                code[ncode++] = NSTAT_FWHMbar12;
            }
    ...
    if (outset == NULL)
    {
        ERROR_exit("Function THD_localstat() fails?!");
    }
