...
int main(int argc, char *argv[])
{
    ...
    /*---------- I guess I've put this off as long as possible. Have to compare amat#0 vs amat#i for
     * i=1, 2, .... ----------*/
...
static mat44_vec * read_affine_warp_vector(char *cp)
{
    ...
    if (qim->nx != 12)
    {
        ERROR_message("read_affine_warp: nx == %d (!= 12) (this message should never happen!)",
                      qim->nx);
        free(wp); RETURN(mvv);
    }
...
static void glue_affine_warp_vector(mat44_vec **avec, mat44_vec **bvec)
{
    ...
    aa = *avec;
    bb = *bvec;
    if (bb == NULL || bb->nmar <= 0 || bb->mar == NULL)
    {
        return;                                                  /* nothing? */
    }
    nb = bb->nmar;
 ...
 *   3dMaskToASCII q.nii > q.txt The quotes were added manually to q.txt when it was included into
 *this file.
 */
...
static THD_3dim_dataset *default_MNI152_mask_dataset(void)
{
     ...
     * geometry string below is from output of 3dinfo ~/abin/MNI152_2009_template.nii.gz */

    mset = EDIT_geometry_constructor(
        "MATRIX(-1,0,0,96,0,-1,0,132,0,0,1,-78):193,229,193",
        "JeepRubicon");

    /* create binary mask from the big string (64697 char expands to 8530021 voxels) */
    ...
    for (kk = 0 ; kk < nz ; kk++)
    {
        for (jj = 0 ; jj < ny ; jj++)
        {
            for (ii = 0 ; ii < nx ; ii++, qq++)
            {
                ...
                if (ii < nx1 && mask[qq + 1] == 0)
                {
                    continue;
                }
                if (jj > 0 && mask[qq - nx] == 0)
                {
                    continue;
                }
                if (jj < ny1 && mask[qq + nx] == 0)
                {
                    continue;
                }
                if (kk > 0 && mask[qq - nxy] == 0)
                {
                    continue;
                }
                if (kk < nz1 && mask[qq + nxy] == 0)
                {
                    continue;
                }
