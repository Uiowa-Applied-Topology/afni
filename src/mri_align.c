...
/** FINE_FIT:
 *  experimental code for a less smoothed fit at the end RW Cox, 17 July 1995
 **/
...
/*********************************************************************
 * 05 Nov 1997: make the parameters that control the iterations be alterable.
 **********************************************************************/
...
/*---------------------------------------------------------------------
 * Inputs:  imbase = base image that others will align to imwt   = image of weight factors to align
 *to (if NULL, will generate one internally) ims    = array of images to align to imbase code   =
 * inclusive OR of ALIGN_* codes, which are:
 *                     ALIGN_NOITER_CODE   --> no iteration ALIGN_VERBOSE_CODE  --> print progress
 *reports ALIGN_REGISTER_CODE --> return images registered ALIGN_DEBUG_CODE    --> print debugging
 *info
 *
 * Outputs: dx[i] , dy[i] , phi[i] , i=0..(ims->num - 1) =
 *          inputs to mri_rota that will bring ims->imarr[i] into alignment with imbase (dx,dy in
 *pixels, phi in radians).
 *
 *          if( (code & ALIGN_REGISTER_CODE) != 0 ) THEN the return value from this function will be
 *an array of registered images;  otherwise, the return value is NULL.
 *
 * Method:  Iterated differential alignment.  Only works for small displacements (up to 4 pixels,
 *say).
 * -----------------------------------------------------------------------*/
                              ...
                              int code, float dx[], float dy[], float phi[])
{
    ...
    if (verbose)
    {
        printf("-- mri_align_dfspace"); fflush(stdout);
    }
    ...
    if (imwt == NULL)
    {
        imww = mri_to_float(bim);                   /* 28 Oct 1996 */
    }
    else
    {
        imww = mri_to_float(imwt);
    }

    tar = MRI_FLOAT_PTR(imww);
    for (ii = 0 ; ii < nx * ny ; ii++)
    {
        tar[ii] = fabs(tar[ii]);                              /* 16 Nov 1998 */
    }
    ...
    if (use_fine_fit)
    {
        ...
        if (imwt == NULL)
        {
            fine_imww = mri_to_float(bim);                 /* 03 Oct 1997 */
        }
        else
        {
            fine_imww = mri_to_float(imwt);
        }
...
/*----------------------------------------------------------------------------
 * Similar routine to above, but uses temporal filtering to determine how much to alter each pixel.
 *Note that if ALIGN_REGISTER_CODE is not turned on, this routine will produce identical outputs to
 *the dfspace routine.
 * ------------------------------------------------------------------------------*/
                             ...
                             int code, float dx[], float dy[], float phi[])
{
    ...
    /** run the time series code to get the pixel-by-pixel correlation between the displacements and
    ...
    if (verbose)
    {
        printf("-- temporal filtering"); fflush(stdout);
    }
