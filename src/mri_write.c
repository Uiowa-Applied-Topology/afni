...
FILE * fopen_maybe(char *fname)    /* 05 Feb 2008 */
{
    ...
    if (strcmp(tname, "-") == 0 || strcmp(tname, "stdout") == 0 ||
        strncmp(tname, "stdout:", 7) == 0)
    {
        return stdout;
    }

    if (strcmp(tname, "stderr") == 0 ||
        strncmp(tname, "stderr:", 7) == 0)
    {
        return stderr;
    }
    ...
    if (imfile == NULL)
    {
        NI_sleep(33); imfile = fopen(fname, "w");
    }
    if (imfile == NULL)
    {
        NI_sleep(66); imfile = fopen(fname, "w");
    }
    if (imfile == NULL)
    {
        NI_sleep(99); imfile = fopen(fname, "w");
    }
    if (imfile == NULL)
    {
        ERROR_message("Can't open for output: %s", fname);
    }
    return imfile;
}
...
int mri_write_raw(char *fname, MRI_IMAGE *im)
{
    ...
    if (im == NULL || fname == NULL || fname[0] == '\0')
    {
        RETURN(0);
    }
...
int mri_write_filtered(char *fname, MRI_IMAGE *im)     /* 15 Dec 2006 */
{
    ...
    if (*fname == '|')
    {
        fname++;                   /* skip pipe character, if present */
    }
    if (*fname == '\0')
    {
        return 0;
