...
/*! Flip a 3D image.  The (outx,outy,outz) parameters specify the direction of the output axes
 * relative to the input axes:
 *   - +1 => +x
 *   - -1 => -x
 *   - +2 => +y
 *   - -2 => -y
...
MRI_IMAGE * mri_flip3D(int outx, int outy, int outz, MRI_IMAGE *inim)
{
    ...
    if (inim == NULL || outx == 0 || outy == 0 || outz == 0)
    {
        RETURN(NULL);
    }
    ...
    if (outx == 1 && outy == 2 && outz == 3)
    {
        RETURN(mri_copy(inim));                                  /* easy case */
    }
    nxin = inim->nx;
    nyin = inim->ny; nxyin = nxin * nyin;
    nzin = inim->nz;

    /* setup so that i_out = ax + bx*i_in + cx*j_in + dx*k_in, for i_in=0..nxin-1, j_in=0..nyin-1,
     * k_in=0..nzin-1, and then similarly for y and z axes                    */
