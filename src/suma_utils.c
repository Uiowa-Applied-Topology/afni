/*
 * Assortment of generic functions needed to support I/O functions in suma_datasets.c
 */
 ...
 * \param first_dim_first (byte) 1 = first dimension first (column major, for a matrix) only 1 is
 *accepted at the moment
 * \return mxv (SUMA_MX_VEC *) the structure with enough memory allocated to the pointer of
 *interest.
...
SUMA_MX_VEC *SUMA_NewMxNullVec(SUMA_VARTYPE tp, int N_dims, int *dims, byte first_dim_first)
{
    ...
    for (i = N_dims; i < SUMA_MX_VEC_MAX_DIMS; ++i)
    {
        mxv->dims[i] = 1;
    }
    if (mxv->N_vals <= 0)
...
{
    return no_suma_rc_found;
}

/*!
 * Return number of Icosahedron depth (int) depth of subdivision (value of -ld or -rd in
 *CreateIcosahedron) bin (byte) 1 --> equiv to -rd in CreateIcosahedron 0 -->          -ld in
 *CreateIcosahedron what (char) 'v' or 'n' :return number of nodes
 ...
 * $HOME/.sumarc and $HOME/.afnirc if HOME is not defined then try .afnirc and .sumarc Shameless
 *wrapper for AFNI_process_environ
 *
...
void SUMA_process_environ(void)
{
    ...
    if (!homeenv)
    {
        sprintf(sumarc, ".sumarc");
    }
    else
    {
        sprintf(sumarc, "%s/.sumarc", homeenv);
    }
    ...
    if (!homeenv)
    {
        sprintf(sumarc, ".afnirc");
    }
    else
    {
        sprintf(sumarc, "%s/.afnirc", homeenv);
    }
    if (stat(sumarc, &stbuf) != -1)
    {
        if (LocalHead)
    ...
    if (sumarc)
    {
        free(sumarc); sumarc = NULL;
    }                         /* allocated before CommonFields */
...
/*!
 * \brief parse command line arguments for input/output debugging and memory debugging. Use no
 *fancies in this function!
 *
 * This function is to be called after SUMAg_CF has been created, if #ifdef SUMA_COMPILED
 *
 * Default for iotrace = 0 memtrace = 1 Those defaults are common to all apps
 *
 */
...
int SUMA_ParseInput_basics_eng(char *argv[], int argc)
{
    ...
    kar = 1;
    brk = 0;
    while (kar < argc)  /* loop across tracing and debugging command line options */
    {
...
void SUMA_ParseInput_basics_ns(char *argv[], int argc) /* for non-suma programs */
{
    ...
    if (!SUMA_ParseInput_basics_eng(argv, argc))
    {
        return;
    }
...
/*!
 * A function version of macro SUMA_TO_LOWER what gets returned is actually the same pointer s but s
 *now points to a lowercase string.
 */
...
{/*SUMA_StripPath*/
    ...
    if (N_FileName)
    {
        ...
        if (!NotFound && i > -1)
        {
            ...
            for (j = i + 2; j < N_FileName; ++j)
            {
                NewName.FileName[j - i - 2] = FileName[j];
            }
            NewName.FileName[j - i - 2] = '\0';

            /* fprintf(stdout,"All Path (%d chars)/%d: %s\n", (i+2),  strlen(NewName.Path),
             * NewName.Path);
             * fprintf(stdout,"All FileName (%d chars)/%d: %s\n", (N_FileName-i-2),
             *strlen(NewName.FileName), NewName.FileName); */
        }
        else
        {
            NewName.Path     = (char *)SUMA_malloc(sizeof(char) * (N_FileName + 1));
...
char *SUMA_getcwd(void)
{
    ...
    /* In theory, we might have to loop over this until success (if cwd would end up being long).
     * For now, failure returns garbage.    [19 Aug 2022 rickr] Thanks to markjens@github for noting
     * the concern.
     */
 ...
 *    .Path (char *) containing path including last slash. If no path exists, Path is "./"
 *    .AbsPath (char *) containing absolute path, which is the same as .Path if .Path starts with
 * '/'
 *                      or cwd/.Path if .Path does not start with '/'
 *                      AND cwd is specified. If cwd is null then it is determined inside the
 *function.
 *    .Ext (char *) containing extension including the dot. If no extension exists, Ext[0] = '\0'
...
{/*SUMA_ParseFname*/
    ...
    if (N_FileName)
    {
        ...
        while (i > -1 && !FoundPath)
        {
            ...
            {
                iColSel = i; if (iColSel < iFirstSel)
                {
                    iFirstSel = iColSel;
                }
            }
            ...
            {
                iRowSel = i; if (iRowSel < iFirstSel)
                {
                    iFirstSel = iRowSel;
                }
            }
            ...
            {
                iNodeSel = i; if (iNodeSel < iFirstSel)
                {
                    iFirstSel = iNodeSel;
                }
            }
            ...
            {
                iRangeSel = i; if (iRangeSel < iFirstSel)
                {
                    iFirstSel = iRangeSel;
                }
            }
        ...
        else
        {
            ...
            if (ptmp[0] == '.')
            {
                 ...
                 * ... problem noted by Ryan from Princeton if (strstr(NewName->Path,"./") &&
                 * ptmp[1] == '/') ptmp = ptmp+2;
                 * else ptmp = ptmp+1;                                           */
        ...
        /* get the relative path (assumes both end with / )*/
        {
            ...
            while (i < strlen(NewName->RelDir) &&
                   ...
                   NewName->RelDir[i] == NewName->AbsPath[i])
            {
                ++i;
            }
            /* backup i until you hit the last '/' */
            while (i >= 0 && NewName->RelDir[i] != '/')
            {
                --i;
            }
            if (NewName->RelDir[i] == '/')
...
/*!
 * \brief Lazy function calls to get at various parts of a file name without the pains of freeing
 *and allocating. Do NOT free the returned string
 *
 ...
 *
 * Note that the function can leak ONE allocated and filed SUMA_PARSED_NAME in a program's life
 *cycle, unless one calls  SUMA_FnameGet(NULL, NULL) at the end
 *
 * WARNING: You can't use this function more than MAX_NUM_STR_FG times as argument to a *printf
 *command. Otherwise, you'll end up with the repeated strings for call numbers that exceed
 *MAX_NUM_STR_FG!
 */
...
char *SUMA_FnameGet(char *Fname, char *sel, char *cccwd)
{
    ...
    if (!Fname)
    {
        ...
        if (ParsedFname)
        {
            SUMA_Free_Parsed_Name(ParsedFname); ParsedFname = NULL;
        }
    ...
    else
    {
        ...
        if (strcmp(currid, lastid)) /* different name */
        {
            if (ParsedFname)
            {
                SUMA_Free_Parsed_Name(ParsedFname);            /* free the old */
            }
            if (!(ParsedFname = SUMA_ParseFname(Fname, cccwd)))
            {
                SUMA_RETURN(str[istr]);
                                         ...
                                         char *what, char *val)
{
    ...
    else if (!strcmp(what, "view"))
    {
        ...
        if (val[0] != '+')
        {
            sprintf(vval, "+%c%c%c%c", val[0], val[1], val[2], val[3]);
        }
        else
        {
            sprintf(vval, "%c%c%c%c%c", val[0], val[1], val[2], val[3], val[4]);
        }
        if (pn->StorageMode == STORAGE_BY_BRICK ||
...
SUMA_Boolean SUMA_isExtension(char *filename, char *ext)
{
    ...
    while (cnt <= N_ext)
    {
        if (filename[N_filename - cnt] != ext[N_ext - cnt])
        {
            SUMA_RETURN(NOPE);
        }
        ++cnt;
 ...
 * \param Remove (SUMA_Boolean) YUP = Remove extension if found Do nothing if it is not there
 *already NOPE = Add extension if not there Do nothing if it is there already
 * \returns ans (char*) containing modified filename
 *
 * - You must free ans on your own Examples:
 *    {
 *    char *ans=NULL;
...
void *SUMA_Free_Parsed_Name(SUMA_PARSED_NAME *Test)
{
    ...
    if (Test->AbsPath)
    {
        SUMA_free(Test->AbsPath);
    }
    if (Test->RelPath)
    {
        SUMA_free(Test->RelPath);
    }
    if (Test->RelDir)
    {
        SUMA_free(Test->RelDir);
    }
    if (Test->Path)
    {
        SUMA_free(Test->Path);
    ...
    if (Test->FileName)
    {
        SUMA_free(Test->FileName);
    }
    if (Test->FullName)
    {
        SUMA_free(Test->FullName);
    }
    if (Test->Ext)
    {
        SUMA_free(Test->Ext);
    }
    if (Test->FileName_NoExt)
    {
        SUMA_free(Test->FileName_NoExt);
    }
    if (Test->RowSelect)
    {
        SUMA_free(Test->RowSelect);
    }
    if (Test->ColSelect)
    {
        SUMA_free(Test->ColSelect);
    }
    if (Test->NodeSelect)
    {
        SUMA_free(Test->NodeSelect);
    }
    if (Test->RangeSelect)
    {
        SUMA_free(Test->RangeSelect);
    }
    if (Test->NameAsParsed)
    {
        SUMA_free(Test->NameAsParsed);
    }
    if (Test->cwdAsParsed)
    {
        SUMA_free(Test->cwdAsParsed);
    }
    if (Test->ExistsAs)
    {
        SUMA_free(Test->ExistsAs);
    }
...
/*! \brief A function that attempts to find a file that is readable. If *fname has a full path, then
 * returns 1 if readable, 0 otherwise If *fname has no path, Try finding file under SUMAg_cwd/ if
 * found -> 1, else, continue If a search path is given, then that path is searched if file is found
 * -> 1 else -> 0 If no search path is given, then path from user's environment is searched. If file
 *is found, old name is freed and new one put in its place.
 */
int SUMA_search_file(char **fnamep, char *epath)
{
   ...
   #if defined SUMA_COMPILED
    /* else, the hard work, first check with cwd options for suma programs*/
    ...
    if (!epath)
    {
        ...
        if (epath == NULL)
        {
            SUMA_RETURN(NOPE);                   /* nothing left to do */
        }
      #else
...
int SUMA_CleanNumString(char *s, void *p)
{
    ...
    if (LocalHead)
    {
        fprintf(stderr, "%s: string begins:%s:\n", FuncName, s);
    }
    FoundTip = 0;
    ...
    if (LocalHead)
    {
        fprintf(stderr, "%s: Read %d/%d values.\n", FuncName, nd, N);
    }
    if (N != nd)
    {
        SUMA_RETURN(0);
...
int SUMA_NumStringUnits(char *s, int marktip)
{
    ...
    if (0)
    {
        ;   /* order of following else ifs matters */
    }
 ...
 * \return int: This is the number of values read. The function will not register in fv more than N
 *values (to keep from running over preallocated space), but it will return the full number of
 *values found.
 *
 *    -1 in case of error
...
/* Like SUMA_StringToNum but looks for side flags in beginning or end Those would be L or R at the
 * very beginning or very end. Function also deblanks s
 */
...
/*!
 * \brief forces a string to be of a certain length. If truncation is necessary, ... are inserted at
...
/*!
 * \brief padds a string to a certain length. You can use this function to crop a string to the
 *specified number of characters n Padding is done with character cp The original string is not
 *modified.
 *
...
/*!
 * \brief truncates a string to a certain length. Adds ... as the last characters of the string The
...
/*!
 * brief Return a copy of string between quotes q1 and q2 s (char*) input string eop (char *) if !=
 * NULL, stop when s reaches eop, else keep going till end of s if necessary q1 (char) opening
 *quote. If '\0', take opening quote as s[0] after you deblank s q2 (char) closing quote. If '\0',
 *q2 = q1 is_closed (int *) on return, set to 1 if found opening and closing quotes 0 otherwise
 *deblank (int) remove blanks after q1 and before q2 withqotes (int)  if 1 then put quotes back on
 *output
 *
                       ...
                       int *is_closed)
{
    ...
    if (deblank)
    {
        ...
        --op2;
        while (SUMA_IS_BLANK(*op2) && op2 > op)
        {
            --op2;
        }
    ...
    if (withquotes)
    {
        ++op2; SUMA_COPY_TO_STRING(op, op2, strn);
    }
    else
    {
        ++op; SUMA_COPY_TO_STRING(op, op2, strn);
    }
...
/*!
 * Put all arguments between opening and closing quotes into one string The function starts by
 *looking for argv[*kar] that begins with opq If opq is found, it continues looking until it finds
 *argv[K] with ends with cloq. If a closing quote is found, a catenation of all the argvs is
 *returned. Also *kar is updated to indicate the last used argument.
 */
                     ...
                     char *opq, char *cloq, int clearused)
{
    ...
    if (!argv || !N_argv || !kar || *kar >= N_argv || !opq)
    {
        RETURN(aq);
    }
 ...
 * - None of the strings passed to the function are freed.
 *
 ...
 * \param whichTofree (int) 0 free none, 1 free s1 2 free s2 3 free s1 and s2
 ...
 * - When newstring is NULL, space allocated for SS->s is resized to the correct dimension and  a
 *null character is placed at the end.
 * \sa SUMA_SS2S
 */
SUMA_STRING * SUMA_StringAppend(SUMA_STRING *SS, char *newstring)
{
    ...
    if (!SS)
    {
        if (LocalHead)
        {
            fprintf(SUMA_STDERR, "%s: Allocating for SS.\n", FuncName);
        }
...
/*!
 * \brief Appends newstring to string in SS->s while taking care of resizing space allocated for s A
 *variable argument version of SUMA_StringAppend
 *
 * \param SS (SUMA_STRING *) pointer to string structure
 * \param newstring (char *) pointer to string to add to SS
 ...
 *null character is placed at the end.
 *
 * - For this function, the formatted length of newstring should not be > than MAX_APPEND-1 If that
 *occurs, the string will be truncated and no one should get hurt
 *
 * NOTE: DO NOT SEND NULL pointers in the variable argument parts or crashes will occur on SUN Such
 *NULL pointers do not result in null vararg_ptr and cause a seg fault in vsnprintf
 *
...
SUMA_STRING * SUMA_StringAppend_va(SUMA_STRING *SS, char *newstring, ...)
{
    ...
    if (newstring)
    {
        ...
        if (LocalHead)
        {
            ...
            /* It seems that sometimes va_list is a pointer, sometimes not. Thanks to marjens for
             * pointing this out. [19 Aug 2022 rickr] */
 ...
 *
 * The Function can handle an env that returns some character separated list if sep is not NULL For
 *example, say env = {"The, olde, fox"}
 * SUMA_EnvEquals(env,"olde", 0,NULL) returns 0 but SUMA_EnvEquals(env,"olde", 0,",") returns 2
...
int SUMA_EnvEquals(char *env, char *sval, byte ci, char *sep)
{
    ...
    if (!sep)
    {
        if (ci)
        {
            SUMA_RETURN(!(strcasecmp(eee, sval)));
        }
        else
        {
            SUMA_RETURN(!(strcmp(eee, sval)));
        }
    }

    /* need to breakup into subsets */
    if (!(sar = SUMA_NI_decode_string_list(eee, sep)))
    {
        SUMA_RETURN(0);
    }
    if (LocalHead)
...
char * SUMA_env_list_help(int DEFAULT_values, TFORM targ)
{
    ...
    while (se.envhelp)
    {
        ...
        if (!eee)
        {
            userval = SUMA_copy_string(se.envval);
        }
        else
        {
            userval = SUMA_copy_string(eee);
        }
        switch (targ)
        {
        default:
...
/*!
 * If !env return NULL if !sval and env is an environment variable, its value is returned if sval
 *and env then env's value is compared (case insensitive) and NULL is returned if there is no match,
 *else the environment variable value is returned Partial match is allowed DO NOT free the returned
 *pointer
 */
...
/* Find the array index k where NodeIndex[n] == k Function assumes NodeIndex is monotonic ascending
 */
 ...
 *
 * This function performs a binary search.  The indices of the elements in nodeList surrounding
 *target will be stored in (overwrite) seg; thus seg[0]=seg[1]=i implies that an exact match was
 *found at index i.
 ...
 * \return found (SUMA_Boolean) YUP if all passed correctly and target within segment, NOPE
 *otherwise
 *
...
int SUMA_binFind(float *indexList, int N_node, float target, byte ematch)
{
    ...
    if (SUMA_binSearch(indexList, target, seg, ematch))
    {
        return seg[0];
    }
    else
...
int SUMA_ibinFind(int *indexList, int N_node, int target)
{
    ...
    if (SUMA_ibinSearch(indexList, target, seg))
    {
        return seg[0];
    }
    else
...
/*!
 * \brief creates a reordered version of a vector yr = SUMA_reorder(y, isort, N_isort);
 *
 * \param y (int *) vector, if y is NULL what you get back in yr is a copy of isort.
 ...
 * - obviously it's your business to ensure that isort[i] cannot be larger than then number of
 *elements in y
 */
...
/* Careful to only free returned pointer after calling function is done with it. Do not free
