...
int main(int argc, char *argv[])
{
    ...
    if (rv)
    {
        RETURN(rv < 0);     /* only a negative return is considered failure */
    }
    if (check_dims(opts))
    {
        RETURN(1);
    ...
    if (opts->method == T21_METH_4095_WARN ||
        opts->method == T21_METH_4095_GCOUNT)
    {
        RETURN(rv);
    }
...
int write_results(options_t *opts)
{
    ...
    else if (!strcmp(opts->prefix, "-"))
    {
        fp = stdout;
    }
    else if (!strcmp(opts->prefix, "stdout"))
    {
        fp = stdout;
    }
    else if (!strcmp(opts->prefix, "stderr"))
    {
        fp = stderr;
    }
    else
    ...
    if (opts->verb)
    {
        if (fp == stdout)
        {
            INFO_message("writing to stdout...");
        }
        else if (fp == stdout)
        {
            INFO_message("writing to stderr...");
        }
        else if (opts->prefix)
        {
            INFO_message("writing to '%s'...", opts->prefix);
        }
        else
        {
            INFO_message("writing to unknown?!?");
        }
    }
...
int compute_results(options_t *opts)
{
    ...
    if (method == T21_METH_ENORM ||
        ...
        method == T21_METH_S_SRMS)
    {
        RETURN(compute_enorm(opts, method));
    }

    if (method == T21_METH_MDIFF ||
        method == T21_METH_SMDIFF)
    {
        RETURN(compute_meandiff(opts, method));
    }

    if (method == T21_METH_4095_COUNT ||
        method == T21_METH_4095_GCOUNT ||
        method == T21_METH_4095_FRAC ||
        method == T21_METH_4095_WARN)
    {
        RETURN(compute_4095(opts, method));
    }
...
int compute_meandiff(options_t *opts, int method)
{
    ...
    if (method == T21_METH_MDIFF)
    {
        if (opts->verb)
        {
            INFO_message("mean diff uses no scaling");
        }
        dscale = nmask;
    }
    else if (method == T21_METH_SMDIFF)
    {
        if (opts->verb)
        {
            INFO_message("scaled mean diff = mdiff/gmean");
        }
        if (gmean == 0.0)
        {
    ...
    if (opts->verb > 1)
    {
        INFO_message("successfully computed mean diff");
    }
 ...
 *
 * method = 4095_count  : return masked counts 4095_gcount : return masked global count 4095_frac
 *  : return masked fractions 4095_warn   : return limit warning
 *
...
int compute_4095(options_t *opts, int method)
{
    ...
    if (method == T21_METH_4095_FRAC)
    {
        for (tind = 0; tind < nt; tind++)
        {
            opts->result[tind] /= nmask;
        }
    }

    if (opts->verb > 1)
    {
        INFO_message("successfully ran 4095 test");
    }
...
int compute_enorm(options_t *opts, int method)
{
    ...
    if (method == T21_METH_ENORM)
    {
        if (opts->verb)
        {
            INFO_message("writing enorm : uses no scaling");
        }
        dscale = 1.0;
    }
    else if (method == T21_METH_RMS)
    {
        if (opts->verb)
        {
            INFO_message("writing rms = dvars = enorm/sqrt(nvox)");
        }
        dscale = sqrt(nmask);
    }
    else if (method == T21_METH_SRMS || method == T21_METH_S_SRMS)
    {
        if (opts->verb)
        {
            INFO_message("scaled dvars = srms = rms/gmean");
        }
        if (gmean == 0.0)
        {
    ...
    if (opts->verb > 2)
    {
        INFO_message("scaling enorm down by %f", dscale);
    }
    ...
    if (method == T21_METH_S_SRMS)
    {
        ...
        if (opts->verb)
        {
            INFO_message("shift by scaled mean diff %f", mdiff);
        }
        for (tind = 1; tind < nt; tind++)
        {
    ...
    if (opts->verb > 1)
    {
        INFO_message("successfully computed enorm");
    }
...
int fill_mask(options_t *opts)
{
    ...
    if (opts->automask)
    {
        if (opts->verb)
        {
            INFO_message("creating automask...");
        }
    ...
    if (opts->mask_name)
    {
        ...
        if (!mset)
        {
            ERROR_exit("cannot open mask dset '%s'", opts->mask_name);
        }
        nvox = DSET_NVOX(opts->inset);
...
int meth_name_to_index(char *name)
{
    ...
    if (!strcasecmp(name, "enorm"))
    {
        return T21_METH_ENORM;
    }

    if (!strcasecmp(name, "rms") ||
        !strcasecmp(name, "dvars"))
    {
        return T21_METH_RMS;
    }

    if (!strcasecmp(name, "srms") ||
        !strcasecmp(name, "cvar"))
    {
        return T21_METH_SRMS;
    }

    if (!strcasecmp(name, "shift_srms") ||
        !strcasecmp(name, "s_srms"))
    {
        return T21_METH_S_SRMS;
    }

    if (!strcasecmp(name, "mdiff"))
    {
        return T21_METH_MDIFF;
    }

    if (!strcasecmp(name, "smdiff"))
    {
        return T21_METH_SMDIFF;
    }

    if (!strcasecmp(name, "4095_count"))
    {
        return T21_METH_4095_COUNT;
    }
    if (!strcasecmp(name, "4095_gcount"))
    {
        return T21_METH_4095_GCOUNT;
    }
    if (!strcasecmp(name, "4095_frac"))
    {
        return T21_METH_4095_FRAC;
    }
    if (!strcasecmp(name, "4095_warn"))
    {
        return T21_METH_4095_WARN;
    }
...
char * meth_index_to_name(int method)
{
    ...
    if (method == T21_METH_RMS)
    {
        return "rms";
    }
    if (method == T21_METH_SRMS)
    {
        return "srms";
    }
    if (method == T21_METH_S_SRMS)
...
/* ----------------------------------------------------------------------
 * fill the options_t struct return 1 on (acceptable) termination 0 on continue
 *       -1 on error
 */
int process_opts(options_t *opts, int argc, char *argv[])
{
    ...
    while (ac < argc)
    {
        ...
        else if (!strcmp(argv[ac], "-input") || !strcmp(argv[ac], "-inset"))
        {
            if (opts->inset)
            {
                ERROR_exit("-input already applied");
            }
            ...
            if (!opts->inset)
            {
                ERROR_exit("cannot open dset '%s'", argv[ac]);
            }
        ...
        else if (!strcmp(argv[ac], "-mask"))
        {
            if (opts->mask_name)
            {
                ERROR_exit("-mask already applied");
            }
            if (++ac >= argc)
            {
        ...
        else if (!strcmp(argv[ac], "-method"))
        {
            if (++ac >= argc)
            {
                ERROR_exit("need argument after '-method'");
            }
    ...
    if (!opts->inset)
    {
        ERROR_exit("missing -input dataset");
    }
    if (opts->method == T21_METH_UNDEF)
    {
        ERROR_exit("missing -method option");
    }
