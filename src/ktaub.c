...
/* This file contains code to calculate Kendall's Tau in O(N log N) time in a manner similar to the
 * following reference:
 *
 * A Computer Method for Calculating Kendall's Tau with Ungrouped Data William R. Knight Journal of
 *the American Statistical Association, Vol. 61, No. 314, Part 1 (Jun., 1966), pp. 436-439
 *
...
/* Sorts in place, returns the bubble sort distance between the input array and the sorted array.
 */
...
/* This function calculates the Kendall correlation tau_b. The arrays arr1 should be sorted before
 * this call, and arr2 should be re-ordered in lockstep.  This can be done by calling
 * qsort_floatfloat(len,arr1,arr2) for example. Note also that arr1 and arr2 will be modified, so if
 * they need to be preserved, do so before calling this function.
 */
...
/* This function uses a simple O(N^2) implementation.  It probably has a smaller constant and
 * therefore is useful in the small N case, and is also useful for testing the relatively complex
...
#define NSTEP    10
    ...
    /* Test the small N version against a few values obtained from the old version in R.  Only
     * exercising the small N version because the large N version requires the first array to be
     * sorted and the second to be reordered in lockstep before it's called.*/
    {
        float a1[] = { 1, 2, 3, 5, 4 };
        float a2[] = { 1, 2, 3, 3, 5 };
    ...
    /* Now that we're confident that the simple, small N version works, extensively test it against
     * the much more complex and bug-prone O(N log N) version.
     */
    for (i = 0; i < 100; i++)
    {
        int j, len;
        ...
        /* The large N version assumes that the first array is sorted.  This will usually be made
         * true in R before passing the arrays to the C functions.
         */
        insertionSort(a, len);
    ...
    /* Speed test.  Compare the O(N^2) version, which is very similar to R's current impl, to my O(N
     * log N) version.
     */
    {
        float *foo, *bar, *buf;
        ...
        for (N = NBOT ; N <= NTOP ; N += NSTEP)
        {
            ...
            /* Only sorting first array.  Normally the second one would be reordered in lockstep.
             */
#if 1
