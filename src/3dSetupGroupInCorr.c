...
int main(int argc, char *argv[])
{
    ...
    byte *mask = NULL;
    int   mask_nx = 0, mask_ny = 0, mask_nz = 0, nmask = 0, nx = 0, ny = 0, nz = 0,
          nvox = 0;
    NI_element *    nel;
    ...
    while (nopt < argc && argv[nopt][0] == '-')
    {
        ...
        if (strncmp(argv[nopt], "-labels", 6) == 0) /* 14 May 2010 */
        {
            ...
            if (ndset_labels > 0)
            {
                ERROR_exit("Can't use -labels TWICE!");
            }
            if (++nopt >= argc)
            {
                ERROR_exit("need an argument after -labels!");
            }
        ...
        if (strcmp(argv[nopt], "-LRpairs") == 0)
        {
            ...
            if (nopt + 2 >= argc)
            {
                ERROR_exit("Need 2 arguments after '-LRpairs'");
            }
    ...
    if (LRpairs && mask)
    {
        ...
        /* You should get two masks in that case, pass both to THD_2dset_to_vectim and be sure to
         * set Nm[0] and Nm[1] according to the number of non zero values in each of them */
    }
    ...
    if (nvox < 2)
    {
        ERROR_exit("Only 1 voxel in datasets?!");
    }
    ...
    if (fp == NULL)
    {
        ERROR_exit("Can't open file '%s' for output", dfname);
    }
    ...
    if (LRpairs)
    {
        ...
        while (ids < ndset)
        {
            ...
            if (do_byte)
            {
                kvv = fwrite(bv, sizeof(sbyte), nvv, fp);
            }
            else
            {
                kvv = fwrite(sv, sizeof(short), nvv, fp);
            }
            if (kvv < nvv)
            {
                fclose(fp); remove(dfname);
        ...
        for (ids = 0 ; ids < ndset ; ids++)
        {
            ...
            { float sq;
              for (iv = 0 ; iv < mv->nvec ; iv++)
              {
                  ...
                  for (kk = 0 ; kk < mv->nvals ; kk++)
                  {
                      sq += fv[kk] * fv[kk];
                  }
                  if (sq != 0.0f && fabsf(sq - 1.0f) > 0.0001f)
                  {
                      ININFO_message("index=%d sq=%g", iv, sq);
                  }
              }
            }
#endif
