...
/*===========================================================================
 * Routines to rotate/shift a 3D volume of byte pairs using a 4 way shear decomposition and
 * "two-step" interpolation -- RWCox - Oct 2000.
 * =============================================================================*/
...
static void ts_shift_2byte(int n, float af, DTYPE *f)
{
    ...
    if (fabs(af) < TSBOT)
    {
        return;                     /* don't do anything if shift is too small */
    }
    af = -af; ia = (int)af; if (af < 0)
    {
        ia--;                                      /* ia = floor */
    }
    aa = af - ia;
...
{
    int ii, jj, kk;
    int nx1 = nx - 1, nx2 = nx / 2, ny1 = ny - 1, ny2 = ny / 2, nz1 = nz - 1, nz2 = nz / 2,
        nxy = nx * ny;
    DTYPE *r1;
    ...
    for (kk = 0 ; kk < nz ; kk++)      /* for each slice */
    {
...

        {
            for (ii = 0 ; ii < nx ; ii++)
            {
                r1[ii] = VV(SX(ii), SY(jj), kk);
            }
            for (ii = 0 ; ii < nx ; ii++)
            {
                VV(ii, SY(jj), kk) = VV(SX(ii), jj, kk);
            }
            for (ii = 0 ; ii < nx ; ii++)
            {
                VV(ii, jj, kk) = r1[ii];
        ...
        if (ny % 2 == 1)                                           /* central row? */
        {
            for (ii = 0 ; ii < nx ; ii++)
            {
                r1[ii] = VV(SX(ii), jj, kk);                            /* flip it */
            }
            for (ii = 0 ; ii < nx ; ii++)
            {
                VV(ii, jj, kk) = r1[ii];                                /* restore */
            }
        }
    }
...
static void flip_yz(int nx, int ny, int nz, DTYPE *v)
{
    ...
    for (ii = 0 ; ii < nx ; ii++)
    {
        for (kk = 0 ; kk < nz2 ; kk++)
        {
            for (jj = 0 ; jj < ny ; jj++)
            {
                r1[jj] = VV(ii, SY(jj), SZ(kk));
            }
            for (jj = 0 ; jj < ny ; jj++)
            {
                VV(ii, jj, SZ(kk)) = VV(ii, SY(jj), kk);
            }
            for (jj = 0 ; jj < ny ; jj++)
            {
                VV(ii, jj, kk) = r1[jj];
...
static void flip_xz(int nx, int ny, int nz, DTYPE *v)
{
    ...
    for (jj = 0 ; jj < ny ; jj++)
    {
        for (kk = 0 ; kk < nz2 ; kk++)
        {
            for (ii = 0 ; ii < nx ; ii++)
            {
                r1[ii] = VV(SX(ii), jj, SZ(kk));
            }
            for (ii = 0 ; ii < nx ; ii++)
            {
                VV(ii, jj, SZ(kk)) = VV(SX(ii), jj, kk);
            }
            for (ii = 0 ; ii < nx ; ii++)
            {
                VV(ii, jj, kk) = r1[ii];
...
/*---------------------------------------------------------------------------
 * Apply an x-axis shear to a 3D array: x -> x + a*y + b*z + s (dilation factor "f" assumed to be
 *1.0)
 * -----------------------------------------------------------------------------*/
                         ...
                         int nx, int ny, int nz, DTYPE *v)
{
    ...
    st = fabs(a) * ny2 + fabs(b) * nz2 + fabs(s); if (st < TSBOT)
    {
        return;
    }
                         ...
                         int nx, int ny, int nz, DTYPE *v)
{
    ...
    st = fabs(a) * nx2 + fabs(b) * nz2 + fabs(s); if (st < TSBOT)
    {
        return;
    }

    xnum = CACHE / (ny * DSIZE); if (xnum < 1)
    {
        xnum = 1;
    }
                         ...
                         int nx, int ny, int nz, DTYPE *v)
{
    ...
    st = fabs(a) * nx2 + fabs(b) * ny2 + fabs(s); if (st < TSBOT)
    {
        return;
    }

    xnum = CACHE / (nz * DSIZE); if (xnum < 1)
    {
        xnum = 1;
    }
...
/*---------------------------------------------------------------------------
 * Apply a set of shears to a 3D array of byte pairs. Note that we assume that the dilation factors
 *("f") are all 1.
 * -----------------------------------------------------------------------------*/
...
/****************************************************************************
 * Alternative entries, with rotation specified via a 3x3 matrix and shift as a 3-vector -- RWCox -
 * 16 July 2000
 *****************************************************************************/
