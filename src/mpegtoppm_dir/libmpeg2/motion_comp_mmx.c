/*
 * motion_comp_mmx.c Copyright (C) 2000-2002 Michel Lespinasse <walken@zoy.org>
 * Copyright (C) 1999-2000 Aaron Holtzman <aholtzma@ess.engr.uvic.ca>
 *
 * This file is part of mpeg2dec, a free MPEG-2 video stream decoder. See
 *http://libmpeg2.sourceforge.net/ for updates.
 *
 * mpeg2dec is free software; you can redistribute it and/or modify it under the terms of the GNU
 *General Public License as published by the Free Software Foundation; either version 2 of the
 *License, or (at your option) any later version.
 *
 * mpeg2dec is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without
...
/*
 * Motion Compensation frequently needs to average values using the formula (x+y+1)>>1. Both MMXEXT
 *and 3Dnow include one instruction to compute this, but it's been left out of classic MMX.
 *
 * We need to be careful of overflows when doing this computation. Rather than unpacking data to
 *16-bits, which reduces parallelism, we use the following formulas:
 *
 * (x+y)>>1 == (x&y)+((x^y)>>1) (x+y+1)>>1 == (x|y)-((x^y)>>1)
 */
...
/*
 * This code should probably be compiled with loop unrolling (ie, -funroll-loops in gcc)because some
 *of the loops use a small static number of iterations. This was written with the assumption the
 *compiler knows best about when unrolling will help
 */
