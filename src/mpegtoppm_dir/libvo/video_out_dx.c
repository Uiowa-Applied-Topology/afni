/*
 * video_out_dx.c Copyright (C) 2000-2002 Michel Lespinasse <walken@zoy.org>
 *
...
/*
 * DirectDraw GUIDs. Defining them here allows us to get rid of the dxguid library during link.
 */
...
static int create_window(dx_instance_t *instance)
{
    ...
    /* store a directx_instance pointer into the window local storage (for later use in
     * event_handler). We need to use SetWindowLongPtr when it is available in mingw */
