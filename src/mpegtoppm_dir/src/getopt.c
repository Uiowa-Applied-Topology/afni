 ...
 *
 * Copyright (C) 1987, 88, 89, 90, 91, 92, 93, 94, 95, 96, 97, 98, 99, 2000 Free Software
 *Foundation, Inc.
 *
 ...
 *Library; see the file COPYING.LIB.  If not, write to the Free Software Foundation, Inc., 59 Temple
 *Place - Suite 330, Boston, MA 02111-1307, USA.  */

/* This tells Alpha OSF/1 not to define a getopt prototype in <stdio.h>. Ditto for AIX 3.2 and
 * <stdlib.h>.  */
#ifndef _NO_PROTO
# define _NO_PROTO
#endif
...
/* Comment out all this code if we are using the GNU C Library, and are not actually compiling the
 * library itself.  This code is part of the GNU C Library, but also included in many other GNU
 * distributions.  Compiling and linking in this code is a waste when using the GNU C library
 * (especially if it is a shared library).  Rather than having every GNU program understand
 * `configure --with-gnu-libc' and omit the object files, it is simpler to just do this in the
 *source for each such file.  */
...
/* This is for other GNU distributions with internationalized messages. When compiling libc, the _
 * macro is predefined.  */
 ...
 * but it behaves differently for the user, since it allows the user to intersperse the options with
 *the other arguments.
 *
 ...
 *order.
 *
 * Setting the environment variable POSIXLY_CORRECT disables permutation. Then the behavior is
 *completely standard.
 *
...
/* Index in ARGV of the next element to be scanned. This is used for communication to and from the
 * caller and for communication between successive calls to `getopt'.
 *
 * On entry to `getopt', zero means this is the first call; initialize.
 *
 * When `getopt' returns -1, this is the index of the first of the non-option elements that the
 *caller should itself scan.
 *
 * Otherwise, `optind' communicates from one call to the next how much of ARGV has been scanned so
 *far.  */

/* 1003.2 says this must be 1 before any call.  */
int optind = 1;

/* Formerly, initialization of getopt depended on optind==0, which causes problems with re-calling
 * getopt as programs generally don't know that. */

int __getopt_initialized;

/* The next char to be scanned in the option-element in which the last option character we returned
...
/* Set to an option character which was unrecognized. This must be initialized on some systems to
 * avoid linking in the system's own getopt implementation.  */

int optopt = '?';

/* Describe how to deal with options that follow non-option ARGV-elements.
 *
 * If the caller did not specify anything, the default is REQUIRE_ORDER if the environment variable
 *POSIXLY_CORRECT is defined, PERMUTE otherwise.
 *
 * REQUIRE_ORDER means don't recognize them as options;
 * stop option processing when the first non-option is seen. This is what Unix does. This mode of
 *operation is selected by either setting the environment variable POSIXLY_CORRECT, or using `+' as
 *the first character of the list of option characters.
 *
 * PERMUTE is the default.  We permute the contents of ARGV as we scan, so that eventually all the
 ...
 *
 * RETURN_IN_ORDER is an option available to programs that were written to expect options and other
 *ARGV-elements in any order and that care about the ordering of the two.  We describe each
 *non-option ARGV-element as if it were the argument of an option with character code 1. Using `-'
 * as the first character of the list of option characters selects this mode of operation.
 *
 * The special argument `--' forces an end of option-scanning regardless of the value of `ordering'.
 *In the case of RETURN_IN_ORDER, only
...
/* Describe the part of ARGV that contains non-options that have been skipped.  `first_nonopt' is
 * the index in ARGV of the first of them;
...
/* Bash 2.0 gives us an environment variable containing flags indicating ARGV elements that should
 * not be considered arguments.  */
...
/* Make sure the environment variable bash 2.0 puts in the environment is valid for the getopt call
 * we must make sure that the ARGV passed to getopt is that one passed to the process.  */
static void
__attribute__ ((unused))
store_args_and_env(int argc, char *const *argv)
{
    /* XXX This is no good solution.  We should rather copy the args so that we can compare them
     * later.  But we must not use malloc(3).  */
    original_argc = argc;
    original_argv = argv;
}
 ...
 * other is elements [last_nonopt,optind), which contains all the options processed since those
 ...
 * `getopt'
 * is called repeatedly, it returns successively each of the option characters from each of the
 *option elements.
 *
 * If `getopt' finds another option character, it returns that character, updating `optind' and
 ...
 *of the first ARGV-element that is not an option.  (The ARGV-elements have been permuted so that
 ...
 *that is not listed in OPTSTRING, return '?' after printing an error message.  If you set `opterr'
 ...
 *in the same ARGV-element, or the text of the following ARGV-element, is returned in `optarg'. Two
 *colons mean an option that wants an optional arg; if there is text in the current ARGV-element, it
 ...
 *abbreviation is unique or is an exact match for some defined option.  If they have an argument, it
 *follows the option name in the same ARGV-element, separated from the option name by a `=', or else
 *the in next ARGV-element. When `getopt' finds a long-named option, it returns 0 if that option's
 * `flag' field is nonzero, the value of the option's `val' field if the `flag' field is zero.
 *
 * The elements of ARGV aren't really const, because we permute them. But we pretend they're const
...
int  long_only;
{
    ...
    /* Test whether ARGV[optind] points to a non-option argument. Either it does not have option
     * syntax, or there is an environment flag from the shell indicating it is not an option.  The
     * later information is only used when the used in the GNU libc.  */
#ifdef _LIBC
...
# define NONOPTION_P    (argv[optind][0] != '-' || argv[optind][1] == '\0')
    ...
    if (nextchar == NULL || *nextchar == '\0')
    {
        ...
        /* Give FIRST_NONOPT & LAST_NONOPT rational values if OPTIND has been moved back by the user
         * (who may also have changed the arguments).  */
...

    {
        ...
        if (temp[0] == 'W' && temp[1] == ';')
        {
            ...
            if (pfound != NULL)
            {
                ...
                if (*nameend)
                {
                    /* Don't test has_arg with >, because some C compilers don't allow it to be used
                     * on enums.  */
                    if (pfound->has_arg)
                    {
                        optarg = nameend + 1;
