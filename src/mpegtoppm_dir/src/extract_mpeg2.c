/*
 * extract_mpeg2.c Copyright (C) 2000-2002 Michel Lespinasse <walken@zoy.org>
...
static int demux(uint8_t *buf, uint8_t *end, int flags)
{
     ...
     * if "state" == DEMUX_DATA, then we need to copy "bytes" bytes of ES data before the next
     *header.
     * if "state" == DEMUX_SKIP, then we need to skip "bytes" bytes of data before the next header.
     *
     * NEEDBYTES makes sure we have the requested number of bytes for a header. If we dont, it
     *copies what we have into head_buf and returns, so that when we come back with more data we
     *finish decoding this header.
     *
