...
/* For putting 3D images together in a line (1D catenation). Also see mri_cat2D.c for 1D or 2D
 * catenation of 2D images. */
...
/* This xx catvol is modified to allow for NULL input images. At this time [17 Jul 2018], the yy and
 * zz catvol do NOT work with NULLs!
 *//*--------------------------------------------------------------------------*/
...
static MRI_IMAGE * mri_catvol_1D_xx(MRI_IMARR *imar, int na, int nb)
{
    ...
    if (na == nb)
    {
        imout = mri_copy(imin); RETURN(imout);
    }
    ...
    for (nn = na ; nn <= nb ; nn++)
    {
        imin = IMARR_SUBIM(imar, nn); if (imin == NULL)
        {
            continue;
        }
        if (nyin != imin->ny || nzin != imin->nz || datum != imin->kind)
        {
            RETURN(NULL);
        }
        nxout += imin->nx;
...
static MRI_IMAGE * mri_catvol_1D_yy(MRI_IMARR *imar, int na, int nb)
{
    ...
    {
        imin = IMARR_SUBIM(imar, nn);
        if (nxin != imin->nx || nzin != imin->nz || datum != imin->kind)
        {
            RETURN(NULL);
        }
        nyout += imin->ny;
...
static MRI_IMAGE * mri_catvol_1D_zz(MRI_IMARR *imar, int na, int nb)
{
    ...
    {
        imin = IMARR_SUBIM(imar, nn);
        if (nxin != imin->nx || nyin != imin->ny || datum != imin->kind)
        {
            RETURN(NULL);
        }
        nzout += imin->nz;
...
MRI_IMAGE * mri_catvol_1D_ab(MRI_IMARR *imar, int dir, int na, int nb)
{
    ...
    if (dir <= 1)
    {
        imout = mri_catvol_1D_xx(imar, na, nb);
    }
    else if (dir == 2)
    {
        imout = mri_catvol_1D_yy(imar, na, nb);
    }
    else
    {
        imout = mri_catvol_1D_zz(imar, na, nb);
    }
