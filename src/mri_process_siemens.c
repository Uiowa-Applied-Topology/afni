...
/*------------------------------------------------------------------------
 * attempt to process as a Siemens mosaic image        20 Dec 2010 [rickr] (rewritten - try to make
 *this usable for mri_read_dicom.c as well)
 *
 *      Sinfo   : pointer to sexinfo struct to fill     (return data) Sstr    : pointer to sexinfo
 *string to fill     (return data) epos    : pointers to DICOM field strings fname   : file name to
 *read assume  : assume dicom mosaic nx,y,z  : as processed outside, for error checking
 *
 * consider result as mosaic if sexinfo->mosaic_num > 1
 *
 * return  1 if mosaic 0 if apparently not
 *        -1 on fatal error
 */
    ...
    char *fname, int assume, int nx, int ny, int nz)
{
    ...
    if (len <= 0 || loc <= 0)
    {
        ...
        if (verb > 1)
        {
            fprintf(stderr, "-- bad SIEMENS len %d, loc %d\n", len, loc);
        }
        RETURN(0);
    }
    ...
    if (verb > 1)
    {
        fprintf(stderr, "-- SIEMENS len %d, loc %d\n", len, loc);
    }
    ...
    if (!fp)
    {
        if (verb > 0)
        {
            fprintf(stderr, "-- failed to open %s as Siemens\n", fname);
        }
        RETURN(0);
    }
    ...
    *Sstr = extract_bytes_from_file(fp, (off_t)loc, (size_t)len, 1);
        ...
        (!epos[E_ID_IMAGE_TYPE] || !strstr(epos[E_ID_IMAGE_TYPE], "MOSAIC")))
    {
        if (verb > 1)
        {
            fprintf(stderr, "-- file %s, not MOSAIC\n", fname);
        }
        RETURN(0);
    }
    ...
    if (!*Sstr)
    {
        if (verb > 1)
        {
            fprintf(stderr, "-- failed to extract Siemens string\n");
        }
        RETURN(0);
    }
    ...
    /* KRH 25 Jul 2003 try to shrink Sstr between BEGIN and END, particularly if there are 2 BEGINs
     * (require both BEGIN and END) */
    /* Now there might be something between BEGIN and ### ...
     *                                   27 Jan 2012 [dglen, rickr]    */
    s_start = strstr(*Sstr, "### ASCCONV BEGIN");
    if (s_start)
    {
        s_end = strstr(s_start, "### ASCCONV END");
    }
    if (s_end)
    {
        char *s_tmp;
    ...
    /* compute size of mosaic layout as 1st integer whose square is >= # of images in mosaic */

    for (mos_ix = 1; mos_ix * mos_ix < Sinfo->mosaic_num; mos_ix++)
    {
        ;                                                        /* nada */
    }
    Sinfo->mos_ix = mos_iy = mos_ix;

    /* nx should now be mos_ix times (actual image NX), so set mos_nx to that of the image (and
     * similarly for mos_ny) */
    ...
    if (nz > 1)
    {
        if (verb > 0)
        {
            fprintf(stderr, "** DICOM ERROR: %dx%d Mosaic of "
...
/*-------------------------------------------------------------------------
 * read data into image array, based on mosaic (fill image and image array) (from mri_read_dicom.c
 *22 Dec 2010 [rickr]
 *
...
{   /*-- 28 Oct 2002:  is a 2D mosaic --*******************/
    ...
    if (!mi->good)
    {
        if (verb)
        {
            fprintf(stderr, "** apply_z_orient but not mosaic");
        }
        RETURN(1);
    }
...
/*------------------------------------------------------------------------
 * finish filling the orientation string, and get the appropriate z offset (from the code formerly
 *known as mri_read_dicom.c)  22 Dec 2010 [rickr]
 *
 * require:
...
/*! Parse the Siemens extra stuff for mosaic information. Ad hoc, based on sample data and no
 * documentation.
...
int get_siemens_extra_info(char *str, Siemens_extra_info *mi, char **epos)
{
    ...
    if (!mi)
    {
        fprintf(stderr, "** GSEI: missing SEI %p\n", mi); RETURN(1);
    }
    ...
    if (str == NULL || *str == '\0')
    {
        if (verb > 1)
        {
            fprintf(stderr, "-- GSEI: no siemens string\n");
        }
        RETURN(0);
    }
    ...
    * binary section                                     --KRH */
    ...
    /* use the updated ept for new searches      27 Jan 2012 [rickr, dglen] */
    {
        ...
        if (cpt == NULL)
        {
            if (verb > 1)
            {
                fprintf(stderr, "-- GSEI: no sSliceArray.asSlice\n");
            }
            RETURN(0);
        }

        /* interpret next string into snum = slice subscript (0,1,...) name = variable name val  =
         * number of RHS of '=' sign mm   = # of bytes used in scanning the above */
    ...
    while (1)
    {
        if (nn < 3)
        {
            break;                              /* bad conversion set */
        }
        if (snum < 0 || snum >= NMOMAX)
        {
            break;                              /* slice number out of range */
        }

        /* 21 Feb 2003 rework this section to allow for missing coordinates in some mosaic files
         *                                        --KRH  */
        ...
        while (isspace(*cpt))
        {
            cpt++;                              /* skip over whitespace */
        }
        if (cpt - dpt > 16)
        {
            break;                              /* too much space */
        }
        if (strncmp(cpt, "sSliceArray.asSlice[", 20) != 0)
        {
            break;                                              /* bad line */
        }
    ...
    {
        mi->good = 1;
        if (have_x[0] && have_x[1])
        {
            mi->have_data[0] = 1;
        }
        if (have_y[0] && have_y[1])
        {
            mi->have_data[1] = 1;
        }
        if (have_z[0] && have_z[1])
        {
            mi->have_data[2] = 1;
        }
    ...
    if (check_for_3D_acquisition(epos, &mi->mosaic_num))
    {
        RETURN(1);
    }
...
/* adjust mi data if 3D volume acquisition return 0 on success                          2 Jun 2016
 * [rickr,DRG] */
