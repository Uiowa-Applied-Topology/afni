...
/*
 * Program to calculate the delay between FMRI time series and the ideal reference time series. This
 *program, which is a command line version of the plugin Hilbert Delay 98, uses Doug Ward's 3dfim+'s
 *skeleton to read and write the bricks.
 */

/* [PT: Aug 25, 2022] Tweak the program so that some marginal/problem cases don't lead to a failure
 * to output anything. This happened for a user when -nodtrnd was being used, specifically, which
 * might not generally be recommended, but still I think the two changes here (to the value of 'opt'
 * in this file and within an opt-testing IF condition in plug_dleay_V2.h stabilize things more
 *generally.
 *
 */
...
#define ERROR_LONGDELAY         3      /* Could not detect zero crossing before half of time course
                                        * was gone */
...
typedef struct DELAY_options {
    ...
    int    p;                         /* number of parameters in the baseline model plus number of
                                       * ideals */
...
/***********************************************************************
 * Plugin to compute a 3D+time dataset voxelwise delay with respect to a reference waveform
 ************************************************************************/
...

{
    ...
    for (is = 0; is < NBUCKETS; ++is)
    {
        option_data->output_type[is] = -1;
    }
...

{
    ...
    while (nopt < argc)
    {
            ...
            strcmp(argv[nopt], "-Nort") == 0)
        {
            ...
            if (nopt >= argc)
            {
                FIM_error("need argument after -polort ");
            }
        ...
        if (strcmp(argv[nopt], "-input") == 0)
        {
            ...
            if (nopt >= argc)
            {
                FIM_error("need argument after -input ");
            }
        ...
        if (strcmp(argv[nopt], "-mask") == 0)
        {
            ...
            if (nopt >= argc)
            {
                FIM_error("need argument after -mask ");
            }
        ...
        if (strcmp(argv[nopt], "-nfirst") == 0)
        {
            ...
            if (nopt >= argc)
            {
                FIM_error("need argument after -nfirst ");
            }
        ...
        if (strcmp(argv[nopt], "-nlast") == 0)
        {
            ...
            if (nopt >= argc)
            {
                FIM_error("need argument after -nlast ");
            }
            sscanf(argv[nopt], "%d", &ival);
            if (ival < 0)
            {
        ...
        if (strcmp(argv[nopt], "-fs") == 0)
        {
            ...
            if (nopt >= argc)
            {
                FIM_error("need argument after -fs ");
            }
        ...
        if (strcmp(argv[nopt], "-T") == 0)
        {
            ...
            if (nopt >= argc)
            {
                FIM_error("need argument after -T ");
            }
            sscanf(argv[nopt], "%f", &fval);
            if (fval < 0)
            {
        ...
        if (strcmp(argv[nopt], "-ideal_file") == 0)
        {
            ...
            if (nopt >= argc)
            {
                FIM_error("need argument after -ideal_file");
            }
        ...
        if (strcmp(argv[nopt], "-prefix") == 0)
        {
            ...
            if (nopt >= argc)
            {
                FIM_error("need file prefixname after -bucket ");
            }
        ...
        if (strcmp(argv[nopt], "-phzscale") == 0)
        {
            ...
            if (nopt >= argc)
            {
                FIM_error("need argument after -phzscale");
            }
            sscanf(argv[nopt], "%f", &fval);
            if (fval < 0 || fval > 10)
            {
        ...
        if (strcmp(argv[nopt], "-co") == 0)
        {
            ...
            if (nopt >= argc)
            {
                FIM_error("need argument after -co");
            }
            sscanf(argv[nopt], "%f", &fval);
            if (fval < 0)
            {
...
/*
 * Read one time series from specified file name.  This file name may have a column selector
 *attached.
 */
...
/*
 * Read multiple time series from specified file name.  This file name may have a column selector
 *attached.
 */
...
)
{
    ...
    int num_idealts;            /* number of ideal time series */
    int q;                      /* number of parameters in the baseline model */
    int p;                      /* number of parameters in the baseline model plus number of ideals
                                 */
    int               nref_in     = -1;
    int               nmsk        = 0;
    float **          ref_in      = NULL;
    ...
    else if (option_data->input_filename != NULL)
    {
        ...
        *dset_time = THD_open_one_dataset(option_data->input_filename);
        ...
        /* [PT: Aug 23, 2022] set correct matrix dims in option_data struct */
...

{
    ...
    if (num_idealts < 1)
    {
        FIM_error("No ideal time series?");
    }
...

{
    ...
    *option_data = (DELAY_options *)malloc(sizeof(DELAY_options));
    ...
    if (LocalHead)
    {
        fprintf(stderr, "returning from init prog.\n");
    }
}
...

{
    ...
    if (im == NULL)
    {
        FIM_error("Unable to extract data from 3d+time dataset");
    }
...

{
    ...
    int p = -1;                /* number of parameters in the baseline model plus number of ideals
                                */
    ...
    if (DSET_TIMEUNITS(dset) == UNITS_MSEC_TYPE)
    {
        tdelta *= 0.001;
    }
    if (tdelta == 0.0)
    ...
    for (ixyz = 0; ixyz < nxyz; ixyz++)
    {
        ...
        /* [PT: Aug 25, 2022] Set this 'opt' param to 0, so zeros can be output in places of poor
         * fitting by hilbertdelay_V2, rather than scuttling all calcs
         */
        ...
        /*option_data->errcode == 0 inner loop */
        {
            ...
            {
                /* if voxel is truly activated, write results to file without modifying return value
                 */
        ...
        else if (option_data->errcode == ERROR_LONGDELAY)
        {
            ...
            /* Set all the variables to Null and don't set xcorCoef to an impossible value because
             * the data might still be OK
             */
            del      = 0.0;
            xcorCoef = 0.0;
            xcor     = 0.0;
        ...
        else if (option_data->errcode != 0)
        {
            ...
            /* Set all the variables to Null and set xcorCoef to an impossible value */
            del      = 0.0;
...

{
    ...
    /*----- Modify some structural properties.  Note that the nbricks just make empty sub-bricks,
     * without any data attached. -----*/
    ierror = EDIT_dset_items(new_dset,
