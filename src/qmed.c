...
float qmed_float(int n, float *ar)
{
...

    {
        ...
        if (i == mid)             /* good luck */
        {
            if (nodd)
            {
                return pivot;        /* exact middle of array */
            }
        ...
        if (i < mid)
        {
            left = i;            /* throw away bottom partition */
        }
        else
        {
            right = i;           /* throw away top partition    */
        }
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 2)
    {
        printf("Usage: qmed N [m]\n"); exit(0);
    }
    nn = strtol(argv[1], NULL, 10); if (nn == 0)
    {
        exit(1);
    }
    ...
    if (argc == 3)
    {
        mrep = strtol(argv[2], NULL, 10); if (mrep < 1)
        {
            mrep = 1;
        }
    }
