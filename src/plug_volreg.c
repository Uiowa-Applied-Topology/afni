...
/***********************************************************************
 * Plugin to do what 3dvolreg.c does.  Most of the code is adapted from plug_imreg.c, 3dvolreg.c,
 *and plug_realtime.c.
 ************************************************************************/
...
char * VOLREG_main(PLUGIN_interface *plint)
{
    ...
    float              dxtop = 0.0, dytop = 0.0, dztop = 0.0, rolltop = 0.0, yawtop = 0.0,
                       pitchtop = 0.0;
    float dxbot = 0.0, dybot = 0.0, dzbot = 0.0, rollbot = 0.0, yawbot = 0.0,
          pitchbot = 0.0;
    ...
    for (kim = 0 ; kim < imcount ; kim++)
    {
        ...
        roll[kim] *= (180.0 / PI); if (hax1 < 0)
        {
            roll[kim] = -roll[kim];
        }
        pitch[kim] *= (180.0 / PI); if (hax2 < 0)
        {
            pitch[kim] = -pitch[kim];
        }
        yaw[kim] *= (180.0 / PI); if (hax3 < 0)
        {
            yaw[kim] = -yaw[kim];
        }
        ...
        /*-- Attach the registered brick to output dataset, converting it to the correct type, if
         * necessary (the new registered brick in "tim" is stored as floats). --*/
    ...
    if (VL_graph && imcount > 1)
    {
        ...
        dt     = DSET_TIMESTEP(VL_dset); if (dt <= 0.0)
        {
            dt = 1.0;
        }
        for (ii = 0 ; ii < nn ; ii++)
