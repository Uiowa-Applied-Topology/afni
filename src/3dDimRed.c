...
static void DR_atexit(void) /*-- called by exit(): delete chunk_file --*/
{
    if (chunk_file != NULL)
    {
        fclose(chunk_file); chunk_file = NULL;
    }
    if (chunk_fnam != NULL)
    {
...
int main(int argc, char *argv[])
{
    ...
    byte *            mask = NULL;
    int               mask_nx = 0, mask_ny = 0, mask_nz = 0, nmask = 0, *imask = NULL;
    ...
    while (nopt < argc && argv[nopt][0] == '-')
    {
        ...
        if (strcasecmp(argv[nopt], "-1Dcols") == 0)
        {
            ...
            if (argv[nopt][0] == '-')
            {
                ERROR_exit("Illegal argument after %s", argv[nopt - 1]);
            }
            for ( ; nopt < argc && argv[nopt][0] != '-' ; nopt++)
            {
                ...
                if (qim == NULL)
                {
                    ERROR_exit("Can't read from -1Dcols file '%s'", argv[nopt]);
                }
        ...
        if (strcasecmp(argv[nopt], "-rdim") == 0)
        {
            if (++nopt >= argc)
            {
                ERROR_exit("need an argument after -rdim!");
            }
        ...
        if (strcasecmp(argv[nopt], "-automask") == 0)
        {
            if (mask != NULL)
            {
                ERROR_exit("Can't use -mask AND -automask!");
            }
        ...
        if (strcasecmp(argv[nopt], "-mask") == 0)
        {
            ...
            if (nmask < 1)
            {
                ERROR_exit("Mask is empty: nothing to process :-(");
            }
            nopt++; continue;
        }
        ...
        if (strcasecmp(argv[nopt], "-dt") == 0 || strcasecmp(argv[nopt], "-TR") == 0)
        {
            ...
            if (dt <= 0.0f)
            {
                WARNING_message("value after %s illegal!", argv[nopt - 1]);
            }
            nopt++; continue;
        }
        ...
        if (strcasecmp(argv[nopt], "-polort") == 0)
        {
            if (++nopt >= argc)
            {
                ERROR_exit("need an argument after -polort!");
            }
            if (have_freq)
            {
                ERROR_exit("Can't use -polort and -band together!");
            }
        ...
        if (strcasecmp(argv[nopt], "-band") == 0)
        {
            if (++nopt >= argc - 1)
            {
                ERROR_exit("need 2 arguments after -band!");
            }
            if (polort >= 0)
            {
                ERROR_exit("Can't use -band and -polort together!");
            }
            fbot = strtod(argv[nopt++], NULL);
            ftop = strtod(argv[nopt++], NULL);
            if (fbot < 0.0f || ftop <= fbot)
            {
                WARNING_message("values after -band are illegal!");
            }
            else
            {
                have_freq = 1;
        ...
        if (strcasecmp(argv[nopt], "-input") == 0)
        {
            ...
            for ( ; nopt < argc && argv[nopt][0] != '-' ; nopt++)
            {
                ...
                if (kset == NULL)
                {
                    ERROR_exit("Can't read dataset '%s'", argv[nopt]);
                }
    ...
    if (ndset > 0)
    {
        ...
        if (nt < 2)
        {
            ERROR_message("Dataset %s has only 1 point in the time direction!",
                          DSET_HEADNAME(iset));
            nbad++;
        }
    ...
    else if (do_automask)
    {
        ...
        else
        {
            ...
            if (nmask < 1)
            {
                ERROR_message("Automask is too small to process"); nbad++;
            }
        }
    }
    if (mask == NULL)
...
#pragma omp parallel
    ...
    if (verb && nmask > 1)
    {
        INFO_message("Start main voxel loop");
    }
#endif
    ...
    {
        imtop = imbot + chunk; if (imtop > nmask)
        {
            imtop = nmask;
        }
...
#pragma omp for
          ...
          ININFO_message("MCW_MALLOC_status = %s", MCW_MALLOC_status);
          free(ws); free(sval); free(tsar); if (umat != NULL)
          {
              free(umat);
          }
...
#pragma omp critical
    ...
    if (polort == 0)
    {
        for (kk = 0 ; kk < nvv ; kk++)
        {
            THD_const_detrend(nt, INVEC(kk), NULL);
        }
    }
...
#undef  IM
    ...
    if (pp < nv)
    {
        if (umm != NULL)
        {
            AAmemset(umm, 0, sizeof(float) * ntr);
        }
        if (svv != NULL)
        {
            AAmemset(svv, 0, sizeof(float) * nv);
        }
 ...
 *  - a = pointer to input matrix; a[i+j*m] has the (i,j) element (m X n matrix, stored in
 *column-first order)
...
static void DR_svd_double(int m, int n, double *a, double *s, double *u, double *v)
{
     ...
     *  this is needed because the svd() function compiles with rare computational errors on some
