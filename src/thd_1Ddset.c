...
/*! Open a 1D file as an AFNI dataset.  Each column is a separate sub-brick.  Ignore comments at
 * this time.  Data is not loaded into bricks.
 * -------------------------------------------------------------------*/
...
THD_3dim_dataset * THD_open_1D(char *pathname)
{
    ...
    /* note we accept default orientation (RAI) here (no use of ADN_xyzorient), since we actually
...
void THD_write_1D(char *sname, char *pname, THD_3dim_dataset *dset)
{
    ...
    if (!ISVALID_DSET(dset) || !DSET_LOADED(dset))
    {
        EXRETURN;
    }
    ...
    /* 05 Mar 2008: special case to write as a 'pure' .1D file, if filename ends in '.1D' or
     * specifies stdout */
    ...
    {
        if (DSET_BRICK_STATCODE(dset, iv) > 0)
        {
            ii++;
        }
    }
...

    {
        for (iv = 0 ; iv < nv ; iv++)         /* loop over sub-bricks = columns */
        {
            ...
            case MRI_short: {
                ...
                float  fac = DSET_BRICK_FACTOR(dset, iv); if (fac == 0.0)
                {
                    fac = 1.0;
                }
                val[0] = fac * bar[ii];
            ...
            if (do_complex)
            {
                if (binflag)
                {
                    qq = fwrite(cval, sizeof(complex), 1, fp);
                }
                else
                {
                    qq = fprintf(fp, " %g %g", cval[0].r, cval[0].i);
                }
            }
            else
            {
                if (binflag)
                {
                    qq = fwrite(val, sizeof(float), 1, fp);
                }
                else
                {
                    qq = fprintf(fp, " %g", val[0]);
                }
            }
