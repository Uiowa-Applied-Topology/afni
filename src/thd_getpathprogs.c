...
/*! Find an executable in the PATH by its name, if it exists. If not, NULL is returned.  If it
 * exists, a pointer to static storage is returned (i.e., don't free() this pointer!).
 * ------------------------------------------------------------------------------*/
...
char * THD_find_executable(char *ename)
{
    ...
    if (!einit)
    {
        einit = 1; elist = THD_getpathprogs(NULL, 1);
    }
    if (elist == NULL)
    {
        RETURN(NULL);
...
/*! Find afni's bin directory if it exists. If not, NULL is returned.  If it exists, a pointer to
 * the path is returned. Do free it with free()
 * ------------------------------------------------------------------------------*/
...
/*! Find a regular file in the PATH by its name, if it exists. Does not include directories. If not,
 * NULL is returned. If it exists, a pointer to malloc-ed storage is returned (e.g., free it when
 * you are done).
 *
 *  thispath is a user supplied ':' delimited path string of the form somewhere/here:/over/there .
 *If null then path is taken from the env PATH
 * ------------------------------------------------------------------------------*/
...
char * THD_find_regular_file(char *ename, char *thispath)
{
    ...
    if (epath != NULL)
    {
        ...
        /* extract blank delimited strings, use as directory names to get timeseries files */

        do{
            ii = sscanf(elocal + epos, "%s%n", dirname, &id);
            if (ii < 1)
            {
                break;                  /* no read ==> end of work */
            }
            ...
            if (!THD_is_directory(dirname))
            {
                continue;                               /* 25 Feb 2002 */
            }
...
/*
 * Find a file somewhere afniish Do not free returned pointer Empty string means nothing was found
 *if altpath is not NULL, and nimlname does not have an absolute path, altpath is considered before
 *diving into the default locations
 */
char *find_afni_file(char *nimlname, int niname, char *altpath)
{
    ...
    if (epath[0] == '\0')
    {
        RETURN(filestr[icall]);                     /* should not happen */
    }
    if (wami_verb() > 1)
    {
    ...
    if (abpath == NULL)
    {
        RETURN(filestr[icall]);                   /* bad-who has no afni?*/
    }
    if (wami_verb() > 1)
    {
        INFO_message("trying to open %s in path as regular file\n  %s\n",
...
THD_string_array * THD_get_all_afni_executables(void)
{
    ...
    for (ii = 0, iaf = 0; ii < elist->num ; ii++)
    {
        ...
        if (
            ...
            /* skip README files, processed as scripts     20 Jul 2012 [rickr] of note:
             * README.atlas_building starts with the line:
             *          README.atlas_building such process recursion is not good for the system...
             */
        ...
        else
        {
             ...
             * STORAGE_UNDEFINED, smode, LAST_STORAGE_MODE, THD_is_directory(elist->ar[ii])); */
        }
    }
...
THD_string_array * THD_get_all_afni_dsets(void)
{
    ...
    for (ii = 0, iaf = 0; ii < elist->num ; ii++)
    {
            ...
            )
        {
            ...
            /*fprintf(stderr," %d- %s smode %d[%d]%d\n", iaf, etr, STORAGE_UNDEFINED, smode,
             * LAST_STORAGE_MODE); */
        }
        else
        {
            /*fprintf(stderr," skip %s (%s) smode %d[%d]%d\n", elist->ar[ii], af, STORAGE_UNDEFINED,
             * smode, LAST_STORAGE_MODE); */
        }
    }
 ...
 *
 * There is a most unholy relationship between this function and the include line: #include
 *"prog_opts.c"
 *
 * This function is for internal machinations having to do with automatic generation of help web
 *pages. It is not for mass consumption.
 *
 * \param fout (FILE *): Pointer to output stream
 * \param verb (int): verbosity
 * \param thisprog (char *): If not NULL, then update the list of options for program thisprog.
 *Existing other programs option are preserved (appendmode is forced to 1) If NULL, then do this for
 *all programs recognized by THD_get_all_afni_executables()
 * \param appendmode (int): 1 --> Keep existing information about programs not in
 *THD_get_all_afni_executables() or other than  thisprog 0 --> Just output information on programs
 *from THD_get_all_afni_executables(). This is only allowed when thisprog == 0
 */
int progopt_C_array(FILE *fout, int verb, char *thisprog, int appendmode)
{
    ...
    for (ii = 0; ii < progs->num; ++ii)
    {
        if (verb)
        {
            fprintf(stderr, "Prog %d/%d: %s ", ii + 1, progs->num,
...
int phelp_cmd(char *prog, TFORM targ, char cmd[512], char fout[128], int verb)
{
    ...
    case NO_FORMAT:
        hopt = "-h_raw";
        if (!program_supports(prog, hopt, NULL, verb))
        {
            hopt = "-HELP";
        }
        if (!program_supports(prog, hopt, NULL, verb))
...
char *sphelp(char *prog, char **str, TFORM targ, int verb)
{
    ...
    case ASPX:
        if (!(help = sphinxize_prog_shelp(prog, *str, verb)))
        {
            if (verb)
            {
                ERROR_message("Failed to autosphinxize string.");
            }
            RETURN(*str);
        }
        free(*str); *str = help;
...
/* Check the static list in prog_opts.c to find whether or not an option exists for a particular
 * program. Return  1: If program is found and the option exists 0: If program found and option does
 * not exist
...
/*
 * Return 1 if program uprog has option option opt 0 otherwise
 *
 * The function first checks if the program has an entry in array poptslist from file prog_opts.c
 *included above.
 *
 * If an entry is found, the decision is based on whether or not opt is listed for that program.
 *Otherwise, if no entry is found, the function resorts to running the program with option opt
 *followed by value oval (if not NULL). If the program returns a status of 1, OR creates no output
 *in response to the option then the option is considered non-existent.
 *
 * Obviously, this is not a general purpose option checker. It was written for the purpose of
 *checking whether or not a program supports the newfangled -h_raw, etc. options.
 *
 * If uprog is "ALL", the function check all existing programs for opt and returns the total number
 *of programs that seem to support it.
 */
int program_supports(char *uprog, char *opt, char *oval, int verb)
{
    ...
    do {
        ...
        case -1:
            /* DO NOT attempt to query program itself to find whether or not it supports an option.
             * For scripts, that ask apsearch for help when they don't recognize an option, this
             * will cause an ugly recursion. */
            #if 0
            UNIQ_idcode_fill(uid);
...
int prog_complete_command(char *prog, char *ofileu, int shtp)
{
    ...
    /* shell types are:
     *    0: tcsh 1: bash 2: zsh set shtpmax to largest+1 (so 3, now) */
    if (shtp < 0)
    {
        shtp = 0; shtpmax = 3;
    }
...
/* particularly for the case of zsh, set any of {'=', '>', '|'} to space, which should result in
 * them not being part of any option */
static int local_clean_zsh_punct(char *str)
{
    char *cp;
