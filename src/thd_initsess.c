...
/*---------------------------------------------------------------------
 * Given a directory name, read in all the datasets and make a session data structure from them.
 *
 * [28 Jul 2003] Modified to use new THD_session struct, wherein all datasets are in one array (no
 *anat/func distinction).
...
THD_session * THD_init_session(char *sessname)
{
    ...
    { char *env = my_getenv("AFNI_SESSTRAIL"); int tt = 0;
      if (env != NULL)
      {
          tt = strtol(env, NULL, 10);
      }
      env = THD_trailname(sess->sessname, tt);
      tt  = 1 + strlen(env) - THD_MAX_NAME; if (tt < 0)
      {
          tt = 0;
      }
    ...
    { char *env = my_getenv("AFNI_OVERRIDE_VIEW");
      if (env != NULL)
      {
          if (toupper(*env) == 'T')
          {
              qview = VIEW_TALAIRACH_TYPE;
          }
          else if (toupper(*env) == 'O')
          {
              qview = VIEW_ORIGINAL_TYPE;
          }
          else
          {
    ...
    for (ibar = 0 ; ibar < dblk_arrarr->num ; ibar++)
    {
        ...
        if (dset_arr == NULL || dset_arr->num <= 0)
        {
            continue;
        }
    ...
    if (!AFNI_noenv("AFNI_ANALYZE_DATASETS"))
    {
        ...
        if (num_anlz > 0)                              /* got some! */
        {
            ...
            for (ii = 0 ; ii < num_anlz ; ii++)        /* loop over files */
            {
                ...
                sess->num_dsset++;
#ifdef ALLOW_FSL_FEAT
                if (strcmp(DSET_PREFIX(dset), "example_func.hdr") == 0)
                {
                    feat_exf = nds;
                }
                else if (strcmp(DSET_PREFIX(dset), "highres.hdr") == 0)
                {
                    feat_hrs = nds;
                }
                else if (strcmp(DSET_PREFIX(dset), "standard.hdr") == 0)
                {
                    feat_std = nds;
                }
#endif
...

            {
                ...
                if (feat_hrs >= 0)
                {
                    dset_hrs = GET_SESSION_DSET(sess, feat_hrs, 0);            /* And at least   */
                }
                if (feat_std >= 0)
                {
                    dset_std = GET_SESSION_DSET(sess, feat_std, 0);            /* one of these. */
                }
 ...
 *       if( feat_std >= 0 ) dset_std = sess->dsset_xform_table[feat_std][0] ; */
                ...
                if (dset_std != NULL)
                {
                    ...
                    /* 28 Aug 2002:
                     *  (i)  correct orientation of example_func (ii) correct warp for non-DICOM
                     *order of coords in datasets      */
...
#undef FIX_EXF
                        ...
                        if (ix + iy + iz == 6) /* (ix,iy,iz) must be a permutation of (1,2,3) */
                        {
                            ...
                            switch (ix)
                            {
                            case 1: /* example_func x-axis is mainly same as standard x-axis (ax>0)
                                     * or its opposite (ax<0) */
                                if (ax > 0)
                                {
                                    ix = dset_std->daxes->xxorient;
                                }
                                else
                                {
                                    ix = ORIENT_OPPOSITE(dset_std->daxes->xxorient);
                                }
                                break;

                            case 2: /* example_func x-axis is mainly same as standard y-axis (ax>0)
                                     * or its opposite (ax<0) */
                                if (ax > 0)
                                {
                                    ix = dset_std->daxes->yyorient;
                                }
                                else
                                {
                                    ix = ORIENT_OPPOSITE(dset_std->daxes->yyorient);
                                }
                                break;

                            case 3: /* example_func x-axis is mainly same as standard z-axis (ax>0)
                                     * or its opposite (ax<0) */
                                if (ax > 0)
                                {
                                    ix = dset_std->daxes->zzorient;
                                }
                                else
                                {
                                    ix = ORIENT_OPPOSITE(dset_std->daxes->zzorient);
                                }
                                break;
                            }
                            switch (iy)
                            ...
                            case 1: if (ay > 0)
                                {
                                    iy = dset_std->daxes->xxorient;
                                }
                                else
                                {
                                    iy = ORIENT_OPPOSITE(dset_std->daxes->xxorient);
                                }
                                break;

                            case 2: if (ay > 0)
                                {
                                    iy = dset_std->daxes->yyorient;
                                }
                                else
                                {
                                    iy = ORIENT_OPPOSITE(dset_std->daxes->yyorient);
                                }
                                break;

                            case 3: if (ay > 0)
                                {
                                    iy = dset_std->daxes->zzorient;
                                }
                                else
                                {
                                    iy = ORIENT_OPPOSITE(dset_std->daxes->zzorient);
                                }
                                break;
                            }
                            switch (iz)
                            ...
                            case 1: if (az > 0)
                                {
                                    iz = dset_std->daxes->xxorient;
                                }
                                else
                                {
                                    iz = ORIENT_OPPOSITE(dset_std->daxes->xxorient);
                                }
                                break;

                            case 2: if (az > 0)
                                {
                                    iz = dset_std->daxes->yyorient;
                                }
                                else
                                {
                                    iz = ORIENT_OPPOSITE(dset_std->daxes->yyorient);
                                }
                                break;

                            case 3: if (az > 0)
                                {
                                    iz = dset_std->daxes->zzorient;
                                }
                                else
                                {
                                    iz = ORIENT_OPPOSITE(dset_std->daxes->zzorient);
                                }
                                break;
                            }
                            ...
                            EDIT_dset_items(dset_exf, ADN_xyzorient, orixyz, ADN_xyzdel, dxyz,
                                            ADN_none);
#endif
                ...
                /* if we have these warps, then build a hashtable describing their use in
...
/* qset is a temporary array of datasets for sorting, like the dsrow dataset rows of a session.
 *
 * note the original sorting is a very simple extraction of the anat type datasets first followed by
 *funcs (not really a sort)
 *
 * the original sort checks across all views/spaces for an anat dataset and puts that early in the
 *list; otherwise all datasets are added to end
 */
...
/*     dset = sess->dsset_xform_table[ids][iview] ;*/
        ...
        /* might be ANAT in one view and FUNC in another (consider qsort()?) 17 Nov 2011 [rickr,
         * dglen] */
        if (!dset)
        {
            continue;
...
void THD_append_sessions(THD_session *ssa, THD_session *ssb)
{
    ...
    if (!ISVALID_SESSION(ssa) || !ISVALID_SESSION(ssb))
    {
        EXRETURN;
    }
    if (THD_equiv_files(ssa->sessname, ssb->sessname) == 1)
    {
        EXRETURN;
    }
...
THD_session * THD_init_session_recursive(char *dirname)
{
    ...
    /* [PT: 18 Apr 2022] bug fix for this command, would not work as it was previously (see below);
     * might still need help/changing? */
    sprintf(cmd, "find %s -maxdepth 9 -type d", dirname);
    /*sprintf( cmd, "find %s -type d -depth -9", dirname) ; */
    flist = THD_suck_pipe(cmd);
    if (flist == NULL || strlen(flist) < 1)
    {
        RETURN(NULL);
    }
...
/* could sort datasets on alphabetical, case insensitive alphabetical, space names available, view
 * in dataset name, dataset types (anat, func,...) storage file types default here is to sort on
 * case insensitive A=a alphabetic  */
...
/* use uppercase version of strcasecmp for comparison (vs lower case) mostly to avoid __tt...
 * datasets appearing before alphabetic */
static int strucasecmp(const char *s1, const char *s2)
{
    int c1, c2;
