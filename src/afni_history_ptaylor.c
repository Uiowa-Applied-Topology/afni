...
/* Ponderosos:
 * -----------
 * - Every bone of my body shivers with excitement at the thought of adding a type variable :
 * { NA, NEW_PROG, NEW_OPT, NEW_ENV, BUG_FIX }
 * It would allow users to check for new things easily. And it might help eradicate misery from this
 *world. Did I also mention that it can fit on the first line beautfully, right after the short
 *string? WOW!
 * - We should probably start building a program list. To make sure that we have no typos there. We
 *can have users add to that list whenever they introduce a program for the first time. This way
...
/*=====BELOW THIS LINE=====*/
    ...
    { 23, Apr,  2022, PT,   "@afni_refacer_make_master_addendum", MINOR,
      TYPE_GENERAL,
      "Now creates a v2.0 of the shell.",
      "The new shell removes more face.  Asked for by A. Basavaraj.\n" },
    { 23, Apr,  2022, PT,   "@afni_refacer_run",                  MINOR,
      TYPE_NEW_OPT,
    ...
    { 14, Apr,  2022, PT,   "@djunct_modal_smoothing_with_rep",   MAJOR,
      TYPE_BUG_FIX,
    ...
    {  1, Apr,  2022, PT,   "3dZipperZapper",                     MINOR,
       TYPE_NEW_OPT,
    ...
    { 20, Mar,  2022, PT,   "apqc_make_html.py",                  MICRO,
      TYPE_BUG_FIX,
    ...
    { 27, Sep,  2021, PT,   "lesion_align",                       MINOR,
      TYPE_GENERAL,
      "On/about Aug 23, 2021, default label_sizes in image windows changed.",
      "That shrunk fonts down one size; now bump back up @chauffeur_afni calls.\n" },

    { 27, Sep,  2021, PT,   "fat_proc_align_anat_pair",           MINOR,
      TYPE_GENERAL,
      "On/about Aug 23, 2021, default label_sizes in image windows changed.",
      "That shrunk fonts down one size; now bump back up @chauffeur_afni calls.\n" },

    { 27, Sep,  2021, PT,   "fat_proc_map_to_dti",                MINOR,
      TYPE_GENERAL,
      "On/about Aug 23, 2021, default label_sizes in image windows changed.",
      "That shrunk fonts down one size; now bump back up @chauffeur_afni calls.\n" },

    { 27, Sep,  2021, PT,   "adjunct_apqc_tsnr_general",          MINOR,
      TYPE_GENERAL,
      "On/about Aug 23, 2021, default label_sizes in image windows changed.",
      "That shrunk fonts down one size; now bump back up @chauffeur_afni calls.\n" },

    { 27, Sep,  2021, PT,   "fat_proc_convert_dcm_anat",          MINOR,
      TYPE_GENERAL,
      "On/about Aug 23, 2021, default label_sizes in image windows changed.",
      "That shrunk fonts down one size; now bump back up @chauffeur_afni calls.\n" },

    { 27, Sep,  2021, PT,   "@SSwarper",                          MINOR,
      TYPE_GENERAL,
      "On/about Aug 23, 2021, default label_sizes in image windows changed.",
      "That shrunk fonts down one size; now bump back up @chauffeur_afni calls.\n" },

    { 27, Sep,  2021, PT,   "fat_proc_imit2w_from_t1w",           MINOR,
      TYPE_GENERAL,
      "On/about Aug 23, 2021, default label_sizes in image windows changed.",
      "That shrunk fonts down one size; now bump back up @chauffeur_afni calls.\n" },

    { 27, Sep,  2021, PT,   "@animal_warper",                     MINOR,
      TYPE_GENERAL,
      "On/about Aug 23, 2021, default label_sizes in image windows changed.",
      "That shrunk fonts down one size; now bump back up @chauffeur_afni calls.\n" },

    { 27, Sep,  2021, PT,   "fat_proc_dwi_to_dt",                 MINOR,
      TYPE_GENERAL,
      "On/about Aug 23, 2021, default label_sizes in image windows changed.",
      "That shrunk fonts down one size; now bump back up @chauffeur_afni calls.\n" },

    { 27, Sep,  2021, PT,   "@djunct_4d_imager",                  MINOR,
      TYPE_GENERAL,
      "On/about Aug 23, 2021, default label_sizes in image windows changed.",
      "That shrunk fonts down one size; now bump back up @chauffeur_afni calls.\n" },

    { 27, Sep,  2021, PT,   "adjunct_suma_fs_mask_and_qc",        MINOR,
      TYPE_GENERAL,
      "On/about Aug 23, 2021, default label_sizes in image windows changed.",
      "That shrunk fonts down one size; now bump back up @chauffeur_afni calls.\n" },

    { 27, Sep,  2021, PT,   "@chauffeur_afni",                    MINOR,
      TYPE_GENERAL,
      "On/about Aug 23, 2021, default label_sizes in image windows changed.",
      "That shrunk fonts down one size; now bump back up @chauffeur_afni calls.\n" },

    { 27, Sep,  2021, PT,   "fat_proc_axialize_anat",             MINOR,
      TYPE_GENERAL,
      "On/about Aug 23, 2021, default label_sizes in image windows changed.",
      "That shrunk fonts down one size; now bump back up @chauffeur_afni calls.\n" },

    { 27, Sep,  2021, PT,   "@djunct_overlap_check",              MINOR,
      TYPE_GENERAL,
      "On/about Aug 23, 2021, default label_sizes in image windows changed.",
      "That shrunk fonts down one size; now bump back up @chauffeur_afni calls.\n" },

    { 27, Sep,  2021, PT,   "fat_proc_decmap",                    MINOR,
      TYPE_GENERAL,
      "On/about Aug 23, 2021, default label_sizes in image windows changed.",
      "That shrunk fonts down one size; now bump back up @chauffeur_afni calls.\n" },

    { 27, Sep,  2021, PT,   "@afni_refacer_run",                  MINOR,
      TYPE_GENERAL,
      "On/about Aug 23, 2021, default label_sizes in image windows changed.",
      "That shrunk fonts down one size; now bump back up @chauffeur_afni calls.\n" },

    { 27, Sep,  2021, PT,   "@djunct_edgy_align_check",           MINOR,
      TYPE_GENERAL,
      "On/about Aug 23, 2021, default label_sizes in image windows changed.",
      "That shrunk fonts down one size; now bump back up @chauffeur_afni calls.\n" },

    { 27, Sep,  2021, PT,   "apqc_make_tcsh.py",                  MINOR,
      TYPE_GENERAL,
      "On/about Aug 23, 2021, default label_sizes in image windows changed.",
      "That shrunk fonts down one size; now bump back up @chauffeur_afni calls.\n" },

    { 23, Sep,  2021, PT,   "@epi_b0_corr.py",                    MINOR,
      TYPE_NEW_OPT,
      "Had been missing the internal processing of option '-epi_pe_bwpp'.",
      "... which has now been added in.\n" },

    { 23, Sep,  2021, PT,   "@chauffeur_afni",                    MINOR,
      TYPE_NEW_OPT,
    ...
    { 21, Sep,  2021, PT,   "apqc_make_tcsh.py",                  MINOR,
      TYPE_NEW_OPT,
    ...
    { 21, Sep,  2021, PT,   "adjunct_apqc_tsnr_general",          MINOR,
      TYPE_NEW_OPT,
      "Add in new opts to turn off images in particular view planes being made.",
      "These are '-no_cor', '-no_axi', '-no_sag'.  First will help APQC.\n" },

    { 21, Sep,  2021, PT,   "@djunct_edgy_align_check",           MINOR,
      TYPE_NEW_OPT,
      "Add in new opts to turn off images in particular view planes being made.",
      "These are '-no_cor', '-no_axi', '-no_sag'.  First will help APQC.\n" },

    { 21, Sep,  2021, PT,   "@chauffeur_afni",                    MINOR,
      TYPE_NEW_OPT,
      "Add in new opts to turn off images in particular view planes being made.",
      "These are '-no_cor', '-no_axi', '-no_sag'.  First will help APQC.\n" },

    { 20, Sep,  2021, PT,   "@grayplot",                          MINOR,
      TYPE_GENERAL,
      "apsearchize.",
      "Make help visible in editor with '@grayplot -hview'.\n" },

    { 20, Sep,  2021, PT,   "3dGrayplot",                         MINOR,
      TYPE_NEW_OPT,
      "Add '-raw_with_bounds ..' to display raw values in arbitrary interval.",
      "Maybe most useful if data have been scaled.\n" },

    {  9, Sep,  2021, PT,   "@4Daverage",                         MICRO,
       TYPE_GENERAL,
       "Shebang changed from csh to tcsh.",
       "For uniformity/simplicity, and to avoid issues on occasional system.\n" },

    {  9, Sep,  2021, PT,   "@FindAfniDsetPath",                  MICRO,
       TYPE_GENERAL,
       "Shebang changed from csh to tcsh.",
       "For uniformity/simplicity, and to avoid issues on occasional system.\n" },

    {  9, Sep,  2021, PT,   "@Purify_1D",                         MICRO,
       TYPE_GENERAL,
       "Shebang changed from csh to tcsh.",
       "For uniformity/simplicity, and to avoid issues on occasional system.\n" },

    {  9, Sep,  2021, PT,   "@RenamePanga",                       MICRO,
       TYPE_GENERAL,
       "Shebang changed from csh to tcsh.",
       "For uniformity/simplicity, and to avoid issues on occasional system.\n" },

    {  9, Sep,  2021, PT,   "@djunct_edgy_align_check",           MICRO,
       TYPE_GENERAL,
       "Shebang changed from csh to tcsh.",
       "For uniformity/simplicity, and to avoid issues on occasional system.\n" },

    {  9, Sep,  2021, PT,   "@djunct_glue_imgs_vert",             MICRO,
       TYPE_GENERAL,
       "Shebang changed from csh to tcsh.",
       "For uniformity/simplicity, and to avoid issues on occasional system.\n" },

    {  9, Sep,  2021, PT,   "@djunct_overlap_check",              MICRO,
       TYPE_GENERAL,
       "Shebang changed from csh to tcsh.",
       "For uniformity/simplicity, and to avoid issues on occasional system.\n" },

    {  9, Sep,  2021, PT,   "@djunct_anonymize",                  MICRO,
       TYPE_GENERAL,
       "Shebang changed from csh to tcsh.",
       "For uniformity/simplicity, and to avoid issues on occasional system.\n" },

    { 31, Aug,  2021, PT,   "@afni_refacer_make_master",          MINOR,
      TYPE_GENERAL,
      "Change default cost function to 'lpa', from 'ls'.",
      "Should be better? There is an opt to change, as necessary.\n" },

    { 30, Aug,  2021, PT,   "3dNetCorr",                          MINOR,
      TYPE_NEW_OPT,
      "New opts: '-all_roi_zeros' and '-automask_off'.",
      "Basically, N ROIs can have NxN mat, even if ROI ave is all zeros.\n" },

    { 22, Aug,  2021, PT,   "lib_afni1D.py",                      MICRO,
      TYPE_BUG_FIX,
      "Afni1D.uncensor_from_vector() had a syntax error in one print call').",
      "Was missing a %, now fixed.\n" },

    { 10, Aug,  2021, PT,   "fat_proc_align_anat_pair",           MINOR,
      TYPE_MODIFY,
      "No longer worry about even/odd slice output (and remove '-no_fs_prep').",
      "Dealt with an old non-issue.\n" },

    { 29, July, 2021, PT,   "@djunct_overlap_check",              MINOR,
      TYPE_NEW_OPT,
      "Add in @chauffeur_afni functionality:  -edgy_ulay.",
      NULL },

    { 27, Jul,  2021, PT,   "afni-general",                       MINOR,
      TYPE_BUG_FIX,
    ...
    { 24, June, 2021, PT,   "@djunct_overlap_check",              MINOR,
      TYPE_GENERAL,
    ...
    { 18, June, 2021, PT,   "@Install_SURFLAYERS_DEMO1",          MINOR,
      TYPE_GENERAL,
      "Created by Sam Torrisi.  Help added, temp placeholder data now in place.",
      "Will add full demo data soon...\n" },

    { 15, June, 2021, PT,   "@radial_correlate",                  MINOR,
      TYPE_BUG_FIX,
      "Minor bug fix (never hit?), and avoid single line 'if' conds.",
      "Latter to avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "@clean_help_dir",                    MINOR,
      TYPE_GENERAL,
      "Avoid single line 'if' conds.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "@RetinoProc",                        MINOR,
      TYPE_GENERAL,
      "Avoid single line 'if' conds.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "@AddEdge",                           MINOR,
      TYPE_GENERAL,
      "Avoid single line 'if' conds.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "@SUMA_Make_Spec_Caret",              MINOR,
      TYPE_GENERAL,
      "Avoid single line 'if' conds.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "@SUMA_Make_Spec_FS",                 MINOR,
      TYPE_GENERAL,
      "Avoid single line 'if' conds.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "@build_afni_Xlib",                   MINOR,
      TYPE_GENERAL,
      "Avoid single line 'if' conds.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "@fix_FSsphere",                      MINOR,
      TYPE_GENERAL,
      "Avoid single line 'if' conds.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "@toMNI_Qwarpar",                     MINOR,
      TYPE_GENERAL,
      "Avoid single line 'if' conds.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "ap_run_simple_rest.tcsh",            MINOR,
      TYPE_GENERAL,
      "Avoid single line 'if' conds.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "@Reorder",                           MINOR,
      TYPE_GENERAL,
      "Avoid single line 'if' conds.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "@SUMA_Make_Spec_SF",                 MINOR,
      TYPE_GENERAL,
      "Avoid single line 'if' conds.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "@T1scale",                           MINOR,
      TYPE_GENERAL,
      "Avoid single line 'if' conds.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "@afni.run.me",                       MINOR,
      TYPE_GENERAL,
      "Avoid single line 'if' conds.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "@diff.files",                        MINOR,
      TYPE_GENERAL,
      "Avoid single line 'if' conds.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "@get.afni.version",                  MINOR,
      TYPE_GENERAL,
      "Avoid single line 'if' conds.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "@grayplot",                          MINOR,
      TYPE_GENERAL,
      "Avoid single line 'if' conds.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "@parse_afni_name",                   MINOR,
      TYPE_GENERAL,
      "Avoid single line 'if' conds.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "@parse_name",                        MINOR,
      TYPE_GENERAL,
      "Avoid single line 'if' conds.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "@simulate_motion",                   MINOR,
      TYPE_GENERAL,
      "Avoid single line 'if' conds.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "@update.afni.binaries",              MINOR,
      TYPE_GENERAL,
      "Avoid single line 'if' conds.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "@2dwarper.Allin",                    MINOR,
      TYPE_GENERAL,
      "Avoid single line 'if' conds.",
      "To avoid badness in some (older?) tcsh versions.\n" },
    ...
    { 15, June, 2021, PT,   "@ExamineGenFeatDists",               MINOR,
      TYPE_GENERAL,
      "Avoid single line 'if' conds.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "@FS_roi_label",                      MINOR,
      TYPE_GENERAL,
      "Avoid single line 'if' conds.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "@Install_InstaCorr_Demo",            MINOR,
      TYPE_GENERAL,
      "Avoid single line 'if' conds.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "@MakeLabelTable",                    MINOR,
      TYPE_GENERAL,
      "Avoid single line 'if' conds.",
      "To avoid badness in some (older?) tcsh versions.\n" },
    ...
    { 15, June, 2021, PT,   "@ScaleVolume",                       MINOR,
      TYPE_GENERAL,
      "Avoid single line 'if' conds.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "@afni_refacer_make_master",          MINOR,
      TYPE_GENERAL,
      "Avoid single line 'if' conds.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "@afni_refacer_make_onebigA12",       MINOR,
      TYPE_GENERAL,
      "Avoid single line 'if' conds.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "@auto_tlrc",                         MINOR,
      TYPE_GENERAL,
      "Avoid single line 'if' conds.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "@clip_volume",                       MINOR,
      TYPE_GENERAL,
      "Avoid single line 'if' conds.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "@compute_gcor",                      MINOR,
      TYPE_GENERAL,
      "Avoid single line 'if' conds.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "@diff.tree",                         MINOR,
      TYPE_GENERAL,
      "Avoid single line 'if' conds.",
      "To avoid badness in some (older?) tcsh versions.\n" },
    ...
    { 15, June, 2021, PT,   "@move.to.series.dirs",               MINOR,
      TYPE_GENERAL,
      "Avoid single line 'if' conds.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "@snapshot_volreg",                   MINOR,
      TYPE_GENERAL,
      "Avoid single line 'if' conds.",
      "To avoid badness in some (older?) tcsh versions.\n" },
    ...
    { 15, June, 2021, PT,   "@SUMA_AlignToExperiment",            MINOR,
      TYPE_GENERAL,
      "Clean up some spacing; avoid single line 'if' conds; use unaliased rm.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "lesion_align",                       MINOR,
      TYPE_GENERAL,
      "Put spaces in if-conditions after 'if'.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "fat_proc_map_to_dti",                MINOR,
      TYPE_GENERAL,
      "Put spaces in if-conditions after 'if'.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "fat_proc_imit2w_from_t1w",           MINOR,
      TYPE_GENERAL,
      "Put spaces in if-conditions after 'if'.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "fat_proc_dwi_to_dt",                 MINOR,
      TYPE_GENERAL,
      "Put spaces in if-conditions after 'if'.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "fat_proc_decmap",                    MINOR,
      TYPE_GENERAL,
      "Put spaces in if-conditions after 'if'.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "fat_proc_convert_dcm_anat",          MINOR,
      TYPE_GENERAL,
      "Put spaces in if-conditions after 'if'.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "fat_proc_axialize_anat",             MINOR,
      TYPE_GENERAL,
      "Put spaces in if-conditions after 'if'.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "fat_proc_align_anat_pair",           MINOR,
      TYPE_GENERAL,
      "Put spaces in if-conditions after 'if'.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "@toMNI_Awarp",                       MINOR,
      TYPE_GENERAL,
      "Put spaces in if-conditions after 'if'.",
      "To avoid badness in some (older?) tcsh versions.\n" },
    ...
    { 15, June, 2021, PT,   "@measure_in2out",                    MINOR,
      TYPE_GENERAL,
      "Put spaces in if-conditions after 'if'.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "@measure_erosion_thick",             MINOR,
      TYPE_GENERAL,
      "Put spaces in if-conditions after 'if'.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "@measure_bb_thick",                  MINOR,
      TYPE_GENERAL,
      "Put spaces in if-conditions after 'if'.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "@djunct_slice_space",                MINOR,
      TYPE_GENERAL,
      "Put spaces in if-conditions after 'if'.",
      "To avoid badness in some (older?) tcsh versions.\n" },
    ...
    { 15, June, 2021, PT,   "@chauffeur_afni",                    MINOR,
      TYPE_GENERAL,
      "Put spaces in if-conditions after 'if'.",
      "To avoid badness in some (older?) tcsh versions.\n" },
    ...
    { 15, June, 2021, PT,   "@animal_warper",                     MINOR,
      TYPE_GENERAL,
      "Put spaces in if-conditions after 'if'.",
      "To avoid badness in some (older?) tcsh versions.\n" },
    ...
    { 15, June, 2021, PT,   "@Install_ClustScat_Demo",            MINOR,
      TYPE_GENERAL,
      "Put spaces in if-conditions after 'if'.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "@GradFlipTest",                      MINOR,
      TYPE_GENERAL,
      "Put spaces in if-conditions after 'if'.",
      "To avoid badness in some (older?) tcsh versions.\n" },

    { 15, June, 2021, PT,   "@FindAfniDsetPath",                  MINOR,
      TYPE_GENERAL,
      "Put spaces in if-conditions after 'if'.",
      "To avoid badness in some (older?) tcsh versions.\n" },
    ...
    { 15, June, 2021, PT,   "@Atlasize",                          MINOR,
      TYPE_GENERAL,
      "Put spaces in if-conditions after 'if'.",
      "To avoid badness in some (older?) tcsh versions.\n" },
    ...
    { 15, June, 2021, PT,   "@global_parse",                      MINOR,
      TYPE_GENERAL,
      "Put spaces in if-conditions after 'if' and before 'then'.",
      "To avoid badness in some (older?) tcsh versions.\n" },
    ...
    { 15, June, 2021, PT,   "@djunct_glue_imgs_vert",             MINOR,
      TYPE_GENERAL,
      "Put spaces in if-conditions after 'if' and before 'then'.",
      "To avoid badness in some (older?) tcsh versions.\n" },
    ...
    { 11, May,  2021, PT,   "@chauffeur_afni",                    MICRO,
      TYPE_GENERAL,
      "Set env var to turn off NIFTI warnings.",
      "That is, AFNI_NIFTI_TYPE_WARN -> NO.\n" },

    { 11, May,  2021, PT,   "apqc_make_tcsh.py",                  MICRO,
      TYPE_GENERAL,
    ...
    { 16, Apr,  2021, PT,   "adjunct_apqc_tsnr_general",          MINOR,
      TYPE_NEW_OPT,
      "More options from @chauffeur_afni here.",
      "Tryin' to make nicer images.\n" },

    { 16, Apr,  2021, PT,   "adjunct_apqc_tsnr_with_mask",        MINOR,
      TYPE_REMOVE,
      "This program has been superseded by: adjunct_apqc_tsnr_general.",
      "The new version is more... general.\n" },

    { 16, Apr,  2021, PT,   "adjunct_apqc_tsnr_no_mask",          MINOR,
      TYPE_REMOVE,
      "This program has been superseded by: adjunct_apqc_tsnr_general.",
      "The new version is more... general.\n" },

    { 16, Apr,  2021, PT,   "apqc_make_tcsh.py",                  MICRO,
      TYPE_GENERAL,
    ...
    { 23, Mar,  2021, PT,   "3dBrickStat",                        MICRO,
      TYPE_GENERAL,
      "Uniformize internal spacing. Should be no change in behavior.",
      "Just a few comments stretch far still.\n" },

    { 16, Mar,  2021, PT,   "@djunct_overlap_check",              MICRO,
      TYPE_NEW_OPT,
      "Set env AFNI_COMPRESSOR to NONE.",
      "Avoid minor badnesses occasionally.\n" },

    { 16, Mar,  2021, PT,   "@djunct_4d_imager",                  MICRO,
      TYPE_NEW_OPT,
      "Set env AFNI_COMPRESSOR to NONE.",
      "Avoid minor badnesses occasionally.\n" },

    { 16, Mar,  2021, PT,   "@djunct_edgy_align_check",           MICRO,
      TYPE_NEW_OPT,
      "Set env AFNI_COMPRESSOR to NONE.",
      "Avoid minor badnesses occasionally.\n" },

    { 10, Mar,  2021, PT,   "adjunct_apqc_tsnr_with_mask",        MINOR,
      TYPE_NEW_OPT,
      "Add in more control features, so can apply in more cases.",
      "Basically just allowing more chauffeur control.\n" },

    {  8, Mar,  2021, PT,   "3dRSFC",                             MICRO,
       TYPE_MODIFY,
       "I/O strings now up to THD_MAX_NAME length; requested by L Waller.",
       NULL },

    {  8, Mar,  2021, PT,   "map_TrackID",                        MICRO,
       TYPE_MODIFY,
       "I/O strings now up to THD_MAX_NAME length; requested by L Waller.",
       NULL },

    {  8, Mar,  2021, PT,   "3dZipperZapper",                     MICRO,
       TYPE_MODIFY,
       "I/O strings now up to THD_MAX_NAME length; requested by L Waller.",
       NULL },

    {  8, Mar,  2021, PT,   "3dNetCorr",                          MICRO,
       TYPE_MODIFY,
       "I/O strings now up to THD_MAX_NAME length; requested by L Waller.",
       NULL },

    {  8, Mar,  2021, PT,   "3dDWUncert",                         MICRO,
       TYPE_MODIFY,
       "I/O strings now up to THD_MAX_NAME length; requested by L Waller.",
       NULL },

    {  8, Mar,  2021, PT,   "3dLombScargle",                      MICRO,
       TYPE_MODIFY,
       "I/O strings now up to THD_MAX_NAME length; requested by L Waller.",
       NULL },

    {  8, Mar,  2021, PT,   "3dMatch",                            MICRO,
       TYPE_MODIFY,
       "I/O strings now up to THD_MAX_NAME length; requested by L Waller.",
       NULL },

    {  8, Mar,  2021, PT,   "3dROIMaker",                         MICRO,
       TYPE_MODIFY,
       "I/O strings now up to THD_MAX_NAME length; requested by L Waller.",
       NULL },

    {  8, Mar,  2021, PT,   "3ddot_beta",                         MICRO,
       TYPE_MODIFY,
       "I/O strings now up to THD_MAX_NAME length; requested by L Waller.",
       NULL },

    {  8, Mar,  2021, PT,   "3dAmpToRSFC",                        MICRO,
       TYPE_MODIFY,
       "I/O strings now up to THD_MAX_NAME length; requested by L Waller.",
       NULL },

    {  8, Mar,  2021, PT,   "3dReHo",                             MICRO,
       TYPE_MODIFY,
       "I/O strings now up to THD_MAX_NAME length; requested by L Waller.",
       NULL },

    {  8, Mar,  2021, PT,   "3dTrackID",                          MICRO,
       TYPE_MODIFY,
       "I/O strings now up to THD_MAX_NAME length; requested by L Waller.",
       NULL },

    {  6, Mar,  2021, PT,   "@fat_tract_colorize",                MINOR,
       TYPE_GENERAL,
       "Run a bit more quietly, setting ENV vars and GUI opts.",
       "Fewer warnings and messages.  And tweak help to be more useful.\n" },

    {  6, Mar,  2021, PT,   "@chauffeur_afni",                    MINOR,
       TYPE_GENERAL,
       "Run a bit more quietly, setting ENV vars and GUI opts.",
       "Fewer warnings and messages.\n" },

    {  6, Mar,  2021, PT,   "@snapshot_volreg",                   MINOR,
       TYPE_GENERAL,
       "Run a bit more quietly, setting ENV vars and GUI opts.",
       "Fewer warnings and messages.\n" },

    {  5, Mar,  2021, PT,   "apqc_make_tcsh.py",                  MINOR,
       TYPE_GENERAL,
    ...
    {  3, Mar,  2021, PT,   "@djunct_modal_smoothing_with_rep",   MINOR,
       TYPE_BUG_FIX,
    ...
    { 24, Feb,  2021, PT,   "apqc_make_tcsh.py",                  MINOR,
      TYPE_GENERAL,
    ...
    {  9, Feb,  2021, PT,   "adjunct_suma_fs_mask_and_qc",        MINOR,
       TYPE_GENERAL,
    ...
    { 22, Dec,  2020, PT,   "@animal_warper",                     MINOR,
      TYPE_GENERAL,
    ...
    {  6, Nov,  2020, PT,   "adjunct_tort_plot_dp_align",         MINOR,
       TYPE_NEW_PROG,
    ...
    { 19, Oct,  2020, PT,   "@djunct_overlap_check",              MAJOR,
      TYPE_NEW_PROG,
    ...
    { 27, July, 2019, PT,   "3dDWUncert",                         MINOR,
      TYPE_GENERAL,
      "Insert a couple ifdefs around OMP functionality.",
      "This should allow program to compile even without OpenMP.\n" },

    { 15, July, 2020, PT,   "3dNetCorr",                          MINOR,
      TYPE_GENERAL,
      "Moved header dep of suma_suma.h -> suma_objs.h.",
      "Should be no output change.\n" },

    { 15, July, 2020, PT,   "3dTORTOISEtoHere",                   MINOR,
      TYPE_GENERAL,
      "Moved header dep of suma_suma.h -> suma_objs.h.",
      "Should be no output change.\n" },

    { 15, July, 2020, PT,   "3ddot_beta",                         MINOR,
      TYPE_GENERAL,
      "Moved header dep of suma_suma.h -> suma_objs.h.",
      "Should be no output change.\n" },

    { 15, July, 2020, PT,   "3dTrackID",                          MINOR,
      TYPE_GENERAL,
      "Moved header dep of suma_suma.h -> suma_objs.h.",
      "Should be no output change.\n" },

    { 15, July, 2020, PT,   "3dEigsToDT",                         MINOR,
      TYPE_GENERAL,
      "Moved header dep of suma_suma.h -> suma_objs.h.",
      "Should be no output change.\n" },

    { 15, July, 2020, PT,   "3dDTtoNoisyDWI",                     MINOR,
      TYPE_GENERAL,
    ...
    { 10, June, 2020, PT,   "convert_cdiflist_to_grads.py",       MAJOR,
      TYPE_NEW_PROG,
    ...
    {  1, June, 2020, PT,   "apqc_make_tcsh.py",                  MINOR,
       TYPE_GENERAL,
    ...
    { 30, May,  2020, PT,   "@djunct_modal_smoothing_with_rep",   MINOR,
      TYPE_GENERAL,
      "Now use *.nii.gz files for all intermeds, not *.nii.",
      "Works better with @animal_warper this way.\n" },

    { 30, May,  2020, PT,   "@djunct_modal_smoothing_with_rep",   MAJOR,
      TYPE_NEW_PROG,
    ...
    { 30, May,  2020, PT,   "adjunct_aw_tableize_roi_info.py",    MINOR,
      TYPE_GENERAL,
    ...
    { 24, Apr,  2020, PT,   "3dClusterize",                       MINOR,
      TYPE_GENERAL,
    ...
    { 23, Apr,  2020, PT,   "@chauffeur_afni",                    MINOR,
      TYPE_NEW_OPT,
    ...
    { 27, Mar,  2020, PT,   "apqc_make_tcsh.py",                  MICRO,
      TYPE_GENERAL,
      "Rearrange variable/function definitions in afnipy libs (no more interdep).",
      "All changes just 'under the hood'---should be no output differences.\n" },

    { 12, Mar,  2020, PT,   "3dReHo",                             MINOR,
      TYPE_GENERAL,
    ...
    { 24, Jan,  2020, PT,   "@afni_refacer_run",                  MAJOR,
      TYPE_GENERAL,
    ...
    { 21, Jan,  2020, PT,   "apqc_make_tcsh.py",                  MINOR,
      TYPE_GENERAL,
    ...
    { 23, Oct,  2019, PT,   "check_dset_for_fs.py",               MAJOR,
      TYPE_NEW_PROG,
    ...
    { 22, Oct,  2019, PT,   "@djunct_edgy_align_check",           MICRO,
      TYPE_GENERAL,
      "Adjusted help file.",
      "Had given wrong name for opt.\n" },

    { 21, Oct,  2019, PT,   "afni",                               MICRO,
      TYPE_NEW_OPT,
    ...
    {  2, Oct,  2019, PT,   "1dDW_Grad_o_Mat++",                  MINOR,
       TYPE_GENERAL,
    ...
    {  4, Sep,  2019, PT,   "@chauffeur_afni",                    MINOR,
       TYPE_NEW_OPT,
    ...
    {  4, Sep,  2019, PT,   "@SUMA_Make_Spec_FS",                 MINOR,
       TYPE_NEW_OPT,
    ...
    {  8, July, 2019, PT,   "@chauffeur_afni",                    MICRO,
       TYPE_GENERAL,
    ...
    { 22, May,  2019, PT,   "@djunct_json_value.py",              MINOR,
      TYPE_NEW_PROG,
      "Tiny program to extract values from JSONs.",
      "Just used by apqc_make_tcsh.py.\n" },

    { 14, May,  2019, PT,   "@chauffeur_afni",                    MINOR,
      TYPE_GENERAL,
    ...
    { 12, Feb,  2019, PT,   "@djunct_4d_slices_to_3d_vol",        MICRO,
      TYPE_GENERAL,
    ...
    { 12, Feb,  2019, PT,   "@djunct_4d_imager",                  MICRO,
      TYPE_GENERAL,
      "Change under the hood: new way to check for validity of input dset.",
      "Should be boring an have no effect on output; just more stable check.\n" },

    { 12, Feb,  2019, PT,   "@GradFlipTest",                      MICRO,
      TYPE_GENERAL,
      "Change under the hood: new way to check for validity of input dset.",
      "Should be boring an have no effect on output; just more stable check.\n" },

    { 12, Feb,  2019, PT,   "@xyz_to_ijk",                        MICRO,
      TYPE_GENERAL,
      "Change under the hood: new way to check for validity of input dset.",
      "Should be boring an have no effect on output; just more stable check.\n" },

    { 12, Feb,  2019, PT,   "p2dsetstat",                         MICRO,
      TYPE_GENERAL,
      "Change under the hood: new way to check for validity of input dset.",
      "Should be boring an have no effect on output; just more stable check.\n" },

    { 12, Feb,  2019, PT,   "@chauffeur_afni",                    MICRO,
      TYPE_GENERAL,
      "Change under the hood: new way to check for validity of input dset.",
      "Should be boring an have no effect on output; just more stable check.\n" },

    { 12, Feb,  2019, PT,   "fat_procs",                          MICRO,
      TYPE_GENERAL,
      "Change under the hood: new way to check for validity of input dset.",
      "Should be boring an have no effect on output; just more stable check.\n" },

    { 11, Feb,  2019, PT,   "@SSwarper",                          MINOR,
      TYPE_NEW_OPT,
    ...
    {  5, Dec,  2018, PT,   "apqc_make_tcsh.py",                  MICRO,
       TYPE_GENERAL,
    ...
    { 20, Nov,  2018, PT,   "@chauffeur_afni",                    MINOR,
      TYPE_BUG_FIX,
    ...
    { 21, Oct,  2018, PT,   "apqc_make_tcsh.py",                  MICRO,
      TYPE_GENERAL,
      "Include 'enorm' and 'outlier' string labels in basic 1dplot.",
      "Clarify plot...\n" },

    {  6, Nov,  2018, PT,   "@chauffeur_afni",                    MINOR,
       TYPE_BUG_FIX,
    ...
    { 28, Aug,  2018, PT,   "@xyz_to_ijk",                        MINOR,
      TYPE_NEW_PROG,
      "Helper program to be able to convert xyz coors to ijk indices.",
      "Supplementary program.\n" },

    { 15, Mar,  2018, PT,   "fat_proc_convert_dwis",              MINOR,
      TYPE_NEW_OPT,
    ...
    {  1, July, 2018, PT,   "@snapshot_volreg",                   MINOR,
       TYPE_GENERAL,
    ...
    { 26, June, 2018, PT,   "fat_proc_axialize_anat",             MINOR,
      TYPE_NEW_OPT,
    ...
    { 22, Apr,  2018, PT,   "fat_proc_axialize_anat",             MINOR,
      TYPE_BUG_FIX,
    ...
    { 20, Sep,  2017, PT,   "1dDW_Grad_o_Mat++",                  MINOR,
      TYPE_NEW_OPT,
    ...
    { 30, Mar,  2017, PT,   "lib_fat_funcs.py",                   MICRO,
      TYPE_BUG_FIX,
    ...
    { 16, Nov,  2016, PT,   "1dDW_Grad_o_Mat",                    MINOR,
      TYPE_GENERAL,
    ...
    { 16, Nov,  2016, PT,   "3dDWUncert",                         MINOR,
      TYPE_GENERAL,
    ...
    {  5, Nov,  2014, PT,   "3dROIMaker",                         MAJOR,
       TYPE_NEW_ENV,
    ...
    {  6, Jun,  2014, PT,   "3dEigsToDT",                         MICRO,
       TYPE_BUG_FIX,
    ...
    { 29, Apr,  2014, PT,   "3dROIMaker",                         MINOR,
      TYPE_NEW_OPT,
    ...
    { 29, Apr,  2014, PT,   "3dNetCorr",                          MINOR,
      TYPE_NEW_OPT,
    ...
    { 21, Aug,  2013, PT,   "3dTrackID",                          MINOR,
      TYPE_MODIFY,
    ...
    { 14, Mar,  2013, PT,   "3dProbTrackID",                      MINOR,
      TYPE_MODIFY,
    ...
    { 14, Mar,  2013, PT,   "3dNetCorr",                          SUPER,
      TYPE_NEW_PROG,
    ...
    { 22, Feb,  2013, PT,   "3dDWUncert",                         MICRO,
      TYPE_BUG_FIX,
      "Free as well as DELETE a dset.",
      "Ultraminor change.\n" },

    { 22, Feb,  2013, PT,   "rsfc",                               MICRO,
      TYPE_BUG_FIX,
    ...
    { 99,   99,   99, NULL, NULL,
      99, 99, NULL, NULL }                 /** the end (do not delete) **/
};
