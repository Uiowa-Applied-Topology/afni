 ...
 * on Tue, Jan 23 2001. See the file "ChangeLog" for details of what was changed.
 *
 *
 ...
 *
 * Portions Copyright (c) 1999 Apple Computer, Inc.  All Rights Reserved.  This file contains
 *Original Code and/or Modifications of Original Code as defined in and that are subject to the
 *Apple Public Source License Version 1.1 (the "License").  You may not use this file except in
 ...
 *PARTICULAR PURPOSE OR NON- INFRINGEMENT.  Please see the License for the specific language
...
/*
 * NSMakePrivateModulePublic() is not part of the public dyld API so we define it here.  The
 *internal dyld function pointer for __dyld_NSMakePrivateModulePublic is returned so thats all that
 *maters to get the functionality need to implement the dlopen() interfaces.
 */
    ...
    int mode)
{
    ...
    /*
     * A NULL path is to indicate the caller wants a handle for the main program.
     */
    if (path == NULL)
    {
    ...
    /*
     * If we don't want an unshared handle see if we already have a handle for this path.
     */
    if ((mode & RTLD_UNSHARED) != RTLD_UNSHARED)
    {
        ...
        while (p != NULL)
        {
            if (p->dev == stat_buf.st_dev && p->ino == stat_buf.st_ino)
            {
                ...
                /*
                 * We have already created a handle for this path.  The caller might be trying to
                 *promote an RTLD_LOCAL handle to a RTLD_GLOBAL.  Or just looking it up with
                 *RTLD_NOLOAD.
                 */
    ...
    /*
     * We do not have a handle for this path if we were just trying to look it up return NULL to
     *indicate we don't have it.
     */
    ...
    /*
     * If the handle is to be global promote the handle.  It is done this way to avoid multiply
     *defined symbols.
     */
