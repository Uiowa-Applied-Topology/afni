...
/*----------------------------------------------------------------
 * Given a directory name, and a header filename, create a datablock that corresponds (return NULL
 *if impossible).
 * ------------------------------------------------------------------*/
...
THD_datablock * THD_init_one_datablock(char *dirname, char *headname)
{
    ...
    if (dirname == NULL || strlen(dirname) == 0 ||
        headname == NULL || strlen(headname) == 0)
    {
        RETURN(NULL);
    }
    ...
    if (strlen(prefix) == 0 ||
        strstr(headname, DATASET_HEADER_SUFFIX) == NULL)
    {
        RETURN(NULL);
    }
...
int THD_datablock_from_atr(THD_datablock *dblk, char *dirname, char *headname)
{
    ...
    else
    {
        if (headname != NULL)
        {
            MCW_strncpy(prefix, headname, THD_MAX_NAME);
        }
        THD_init_diskptr_names(dkptr, dirname, NULL, prefix, view_type, True);
    }
    ...
    if (dkptr->storage_mode == STORAGE_UNDEFINED)  /* ZSS: Oct. 2011 the next line was being called
                                                    * all the time before */
    {
    ...
    /** Now create an empty shell of the "brick" == the data structure that will hold all the voxel
     * data.  Note that all datablocks will have a brick, even if they never actually contain data
     * themselves (are only warp-on-demand).
     *
     *  If the BRICK_TYPES input attribute doesn't exist, then all sub-bricks are shorts.  This
     *makes the code work with old-style datasets, which were always made up of shorts.
     **/
    ...
    /* if the data is not on disk, the flag remains at DATABLOCK_MEM_UNDEFINED, otherwise the flag
     * says how the memory for the bricks is to be created. */
    ...
    if (dkptr->storage_mode == STORAGE_BY_BRICK)
    {
         ...
         *    data is compressed, data is not in native byte order, or user explicitly forbids use
         *of mmap   */
    ...
    if (atr_labs != NULL && atr_labs->nch > 0)   /* create labels from attribute */
    {
...

        {
            ...
            if (ngood > 0)
            {
                ...
                if (ngood > THD_MAX_SBLABEL)
                {
                    ngood = THD_MAX_SBLABEL;
                }
    ...
    else             /*--- the olde way to get ye brick stataux parameters ---*/
    {
        ...
        if (atr_flt != NULL && atr_flt->nfl >= 3)
        {
             ...
             *   sub-brick-index  statcode  no.-of-values value ... value sub-brick-index  statcode
             *no.-of-values value ... value, etc. */
...
/*!  Given a 12 parameter affine transform created a la 3dWarpDrive's WARPDRIVE_MATVEC_INV_000000,
 * turn it into a WARP_DATA type array of 30 parameters. See matlab function MATVEC_to_WARP.m for
 * more inspiration.
 *
 * ZSS: June 06
 * ---------------------------------------------------------------------*/
...
int Matvec_2_WarpData(ATR_float *atr_flo, THD_affine_warp *ww, float *wdv)
{
    ...
    if (wdv)
    {
        ...
        /* bot and top are filled as to not cause trouble for Talairach bounding box at a minimum
         * from:
...
int THD_WarpData_From_3dWarpDrive(THD_3dim_dataset *dset, ATR_float *atr_flt)
{
    ...
    /* If you have a warp, you must have a warp_parent However, previous versions of @auto_tlrc did
     * not set that, so use some defaults */
    if (strlen(dset->warp_parent_name) <= 0 &&
        ISZERO_IDCODE(dset->warp_parent_idcode))
    {
        if (dbg)
        {
            fprintf(stderr, "Assigning a dummy warp parent name\n");
        }
...
/*! Apply attributes to modify an existing datablock. Only some attributes have an effect.
 *  09 May 2005 -- written to support NIfTI-ization, by allowing attributes to be applied AFTER a
 *dataset is created.
 * -----------------------------------------------------------------------------*/
...
void THD_datablock_apply_atr(THD_3dim_dataset *dset)
{
    ...
    blk = dset->dblk;  if (blk == NULL)
    {
        EXRETURN;
    }
    nvals = blk->nvals;  if (nvals <= 0)
    {
        EXRETURN;
    }
    daxes = dset->daxes;  if (daxes == NULL)
    {
        EXRETURN;
    }
    dkptr = blk->diskptr;
    ...
    if (ATR_IS_STR(ATRNAME_BRICK_KEYWORDS))
    {
        ...
        {
            THD_init_datablock_keywords(blk);
        }
    ...
    else                                       /* But perhaps there is a little something from auto
                                                * talairaching
                                                *
                                                *                                              ZSS,
                                                *June 06 */
...
#undef TF
        ...
        for (ii = 0 ; ii < ntag ; ii++)
        {
            if (jj < nch)
            {
                ...
                if (kk > 0)
                {
                    TAG_SETLABEL(dset->tagset->tag[ii], atr_str->ch + jj);
                }
                else
                {
    ...
    if (atr_int != NULL && atr_flt2 != NULL)
    {
        ...
        if (dset->taxis->nsl > 0)
        {
            ...
            else
            {
                ...
                /* also correct partial TAXIS_FLOATS here, in case of NIFTI ext (for 3drefit
                 * -Tslices of NIFTI)           [3 Feb 2025 rickr] */
...
/*----------------------------------------------------------------
 * Initialize the brick structure of a datablock.  This will contain no data for now, but provides a
 *place for it to go. This routine presumes that the datablock is already set up with the dimension
 *information, etc.
 *
 * Inputs ntype and btype determine the types of data in the sub-bricks. If btype == NULL, then the
 *type code for all sub-bricks is given by ntype;
 *  If ntype < 0,     then the type code for all sub-bricks is taken from the datablock pointed to
 *by (THD_datablock *) btype;
 *  If btype != NULL  then the type code for each sub-brick
 *    && ntype > 0      is taken from the array pointed to by (int *) btype.
 * ------------------------------------------------------------------*/
...
void THD_init_datablock_brick(THD_datablock *dblk, int ntype, void *btype)
{
    ...
    nvals = dblk->nvals; if (nvals < 1)
    {
        EXRETURN;                                   /* something wrong */
    }
