...
/* fim   = image to warp wimar = contains 3 images with index warps mode  = interpolation method
 */
...
/* jim    = base image iimar  = 4 images: [0] = source image, [1..3] = derivatives of source wimar
 *  = 3 images with current index warp dblur  = blurring to apply to deltim output = 3 images with
 *additional index warp
 */
...
/* INFO_message("Enter GW_deltim") ; */
    ...
    tim  = GW_warpim(ixim, wimar, MRI_LINEAR); mri_free(ixim); ixim = tim;
    ixar = MRI_FLOAT_PTR(ixim);
    tim  = GW_warpim(iyim, wimar, MRI_LINEAR); mri_free(iyim); iyim = tim;
    iyar = MRI_FLOAT_PTR(iyim);
    tim  = GW_warpim(izim, wimar, MRI_LINEAR); mri_free(izim); izim = tim;
    izar = MRI_FLOAT_PTR(izim);
...
/* iim    = source image jim    = base image wimar  = base index warp [3] deltar = additional index
 * warp [3] ds     = amount of deltar to apply
 */
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 3)
    {
        printf("gwarp [-blur b s] base source\n"); exit(0);
    }
    ...
    bset = THD_open_dataset(argv[iarg++]); if (!ISVALID_DSET(bset))
    {
        ERROR_exit("bset");
    }
    DSET_load(bset); bim = mri_to_float(DSET_BRICK(bset, 0)); DSET_unload(bset);

    iset = THD_open_dataset(argv[iarg++]); if (!ISVALID_DSET(iset))
    {
        ERROR_exit("iset");
    }
