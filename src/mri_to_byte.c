...
/* compute a byte mask from the input image     11 Jan 2008 [rickr] (this is akin to THD_makemask)
 *                                   */
byte * mri_to_bytemask(MRI_IMAGE *maskim, float mask_bot, float mask_top)
{
    ...
    case MRI_byte: {
        ...
        if (mask_bot <= mask_top)
        {
            ...
            if (mask_bot >= maxval || mask_top <= -maxval)
            {
                empty = 1;
            }
