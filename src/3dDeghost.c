...
int main(int argc, char *argv[])
{
    ...
    while (iarg < argc && argv[iarg][0] == '-')
    {
        ...
        if (strcasecmp(argv[iarg], "-FPS") == 0) /* stolen from 3dAllineate.c */
        {
            ...
            if (strlen(fps) < 3)
            {
                ERROR_exit("Code '%s' after '%s' is too short",
                           fps, argv[iarg - 1]);
            }
            switch (fps[0])
            {
            ...
            if (fe + pe + se != 6)
            {
                ERROR_exit("Code '%s' after '%s' is nonsensical",
                           fps, argv[iarg - 1]);
            }
            iarg++; continue;
        }
    ...
    {
        orfilt_len = nvals / 2; if (orfilt_len % 2 == 0)
        {
            orfilt_len++;
        }
    }
    ...
    if (outset == NULL)
    {
        ERROR_exit("THD_deghoster fails :-(((");
    }
    if (filset != NULL)
    {
        DSET_delete(filset);
...
void orfilt_vector(int nvec, float *vec)
{
    ...
    if (orfilt_len == 0)
    {
        if (ar != NULL)
        {
            free(ar); nar = 0; ar = NULL;
        }
        if (vv != NULL)
        {
            free(vv); nvv = 0; vv = NULL;
        }
        return;
    }
    if (orfilt_len == 1)
    ...
    for (ii = 0 ; ii < nvec ; ii++)
    {
        ibot = ii - nby2; if (ibot < 0)
        {
            ibot = 0;
        }
        itop = ii + nby2; if (itop > nv1)
        {
            itop = nv1;
        }
        nii = itop - ibot + 1;
...
/*** Given Iy, Iyn, theta (in form of cos and sin), and noise level d, find M that is the solution
 * to
 *
 *    F(M) =    Iy*cos(theta)^2 / sqrt( M^2 * cos(theta)^2 + d^2 )
 + Iyn*sin(theta)^2 / sqrt( M^2 * sin(theta)^2 + d^2 ) - 1 = 0
 +
 +   If d=0, M is solved for analytically (mzero).  If d > 0, then 3 steps of Newton's method are
 +used -- unless d is too big, in which case M=0 is the return value.
 *//*------------------------------------------------------------------------***/
...
#define COMPUTE_ff_df(mm)                                                            \
        { mq = (mm) * (mm); mcd = sqrt(mq * ctq + dq); msd = sqrt(mq * stq + dq);    \
          ...
          df = -iy * (mm) * ctq * ctq / (mcd * mcd * mcd) - iyn * (mm) * stq * stq / \
               (msd * msd * msd); }
...
float find_mhat(float iy, float iyn, float ct, float st, float d)
{
    ...
    if (d <= 0.0f)
    {
        return mzero;                          /* easiest case */
    }
    if (d >= iy * ctq + iyn * stq)
    {
        return 0.0f;                           /* unlikely */
    }
    COMPUTE_ff_df(mzero);
    if (ff >= 0.0f || df >= 0.0f)
    {
        return mzero;                             /* should not happen */
    }
...
/*** Given Iy, Iyn, theta (in form of cos and sin), and noise level d, find My and Myn that are the
 * solution to
 *
 *     Iy^2  = My^2 * cos(theta)^2 + Myn^2 * sin(theta)^2 + d^2 Iyn^2 = My^2 * sin(theta)^2 + Myn^2
 *cos(theta)^2 + d^2
 *//*------------------------------------------------------------------------***/
...
float_pair find_mpair(float iy, float iyn, float ct, float st, float d)
{
    ...
    den = ctq * ctq - stq * stq; if (den <= 0.0f)
    {
        return result;                                          /* bad inputs */
    }
    iyt = iy * iy - dq; if (iyt < 0.0f)
    {
        iyt = 0.0f;
    }
    iynt = iyn * iyn - dq; if (iynt < 0.0f)
    {
        iynt = 0.0f;
    }

    my = (iyt * ctq - iynt * stq) / den; if (my < 0.0f)
    {
        my = 0.0f;
    }
    myn = (-iyt * stq + iynt * ctq) / den; if (myn < 0.0f)
    {
        myn = 0.0f;
    }
...
/** ININFO_message("  bvec=%g gvec=%g ctvec=%g stvec=%g ==> mhat=%g e1=%g e2=%g", bvec[ii],gvec[ii]
 * , ctvec[ii],stvec[ii] , mhat,e1,e2) ; **/
    }
...
byte * DEG_automask_image(MRI_IMAGE *im)
{
    ...
    {
        if (!bmask[ii] && cmask[ii] && iar[ii] < cval)
        {
            cmask[ii] = 0;
        }
    }
...
#define IJK(f, p, s)    ((f) * df + (p) * dp + (s) * ds)
    ...
    for (ss = 0 ; ss < ns ; ss++)
    {
        /* make copy of brain mask in this slice, then edit it down to voxels in the brain whose N/2
         * point is outside the brain (smask) */
        ...
        for (iim = nsm = pp = 0 ; pp < np ; pp++)
        {
            ...
            for (ff = 0 ; ff < nf ; ff++, iim++)
            {
                ...
                if (sm)
                {
                    xvec[nsm] = xvim[iim]; yvec[nsm] = yvim[iim]; nsm++;
                }
            }
        }
        ...
        /* smask is now the mask of brain voxels whose Nyquist ghost locations are NOT in the brain
         * mask */
        ...
        for (vv = 0 ; vv < nv ; vv++)
        {
            ...
            /* extract the vector of image values in smask, and the vector of image values at the
             * ghost locations */
            ...
            for (iim = ism = pp = 0 ; pp < np ; pp++)
            {
                ...
                for (ff = 0 ; ff < nf ; ff++, iim++)
                {
                    ...
                    if (smask[iim])
                    {
                        bvec[ism] = bvim[iim]; gvec[ism++] = gvim[iim];
                    }
                }
            }
        ...
        for (vv = 0 ; vv < nv ; vv++)
        {
            ...
            if (thet1_t[vv] == 0.0f)
            {
                continue;                   /* ghost amplitude is 0 ==> skip this time point */
            }
            if (verb > 1)
            {
             ...
             *  (a) inside the smask                  == voxel in brain, N/2 ghost isn't (b) not in
             *the smask but in the bmask == voxel && N/2 ghost are in brain (c) otherwise
             *                         == voxel is unimportant effluvium  */
