    ...
    float max_dist, int mode)
{
    ...
    do {
        ...
        if (ijk == nxyz)
        {
            break;                              /* didn't find any nonzero point! */
        }
        ...
        case ISOVALUE_MODE: { /* for each point in cluster:
                               * check points offset by mask for nonzero entries in fim enter those
                               *into cluster continue until end of cluster is reached (note that
                               *cluster is expanding as we progress) */
            switch (ftype)
            {
            case MRI_short:
                for (icl = 0 ; icl < clust->num_pt ; icl++)
                {
                    ...
                    for (jma = 0 ; jma < mnum ; jma++)
                    {
                        ...
                        km = kc + mask->k[jma];
                        if (im < 0 || im >= nx ||
                            jm < 0 || jm >= ny || km < 0 || km >= nz)
                        {
                            continue;
                        }

                        ijkma = THREE_TO_IJK(im, jm, km, nx, nxy);
                        if (ijkma < ijk_last || ijkma >= nxyz || sfar[ijkma] != fimv)
                        {
                            continue;
                        }
            ...
            case MRI_byte:
                for (icl = 0 ; icl < clust->num_pt ; icl++)
                {
                    ...
                    for (jma = 0 ; jma < mnum ; jma++)
                    {
                        ...
                        ijkma = THREE_TO_IJK(im, jm, km, nx, nxy);
                        if (ijkma < ijk_last || ijkma >= nxyz || bfar[ijkma] != fimv)
                        {
                            continue;
                        }
            ...
            case MRI_float:
                for (icl = 0 ; icl < clust->num_pt ; icl++)
                {
                    ...
                    for (jma = 0 ; jma < mnum ; jma++)
                    {
                        ...
                        ijkma = THREE_TO_IJK(im, jm, km, nx, nxy);
                        if (ijkma < ijk_last || ijkma >= nxyz || ffar[ijkma] != fimv)
                        {
                            continue;
                        }
