 ...
 *
 *  Wbilin(x) = y  = inv[ I   + C    x ] [ A   x  + b  ] i         ij    ijk  k     jk  k    j
 *
 * where the summation convention applies to repeated indexes. There are 39 parameters: 3 in vector
 *b, 3x3 in matrix A, and 3x3x3 in tensor C.  If C=0, this is an affine warp, obviously.
 *
 * Such a transformation has the advantage that it's inverse is also a bilinear transformation.  For
 *small |x|, Taylor series clearly shows that this is similar to a general quadratic transformation,
 *with the extra advantage of being analytically invertible.
 *
 * Functions are provided to invert a bilinear warp, as well as to multiply it (pre- and post-) with
 *an affine warp.  Also, to apply a bilinear warp to a set of points.
 *
 * These functions do not depend on other code in the AFNI suite. Important entry points for the
 *programmer using this stuff:
 *
 * BL_invert_warp()             = invert a bilinear warp BL_warp_from_params()        = create a
 *bilinear warp from parameters BL_params_from_warp()        = extract 39 parameters to an array
 *BL_extract_affine_warp()     = get the affine (A,b) part out BL_affine_from_12_params()   = make
 *affine transform BL_affine_from_12_elements() = make affine transform BL_bilinear_x_affine()
 *       = catenate bilinear warp with affine warp
 *                                = Wbilin(Waffine(x)) BL_affine_x_bilinear()       = catenate
 *affine warp with bilinear warp
 *                                = Waffine(Wbilin(x)) BL_print_standard_warp()     = print bilinear
 *warp coefficients nicely BL_apply_warp()              = apply bilinear warp to some (x,y,z) points
 *
 ...
 * stuff in 3ddata.h, et cetera, with some names changes, to provide a self-contained file for
 *dealing with 3D bilinear transformations.
 ...
 * rotate about axis #ff, from axis #aa toward #bb, where ff, aa, and bb are a permutation of
 * {0,1,2} */
...
/* load the top 3 rows of a mat44 matrix, and set the 4th row to [ 0 0 0 1], as required */
...
/* The stuff below is stolen directly from mri_genalign.c et cetera. Only the serial numbers have
 * been filed off to protect the innocent.
 *//*------------------------------------------------------------------------*/
...
static BLmat44 BL_affine44(float *parvec)
{
    ...
    a = parvec[9]; if (fabsf(a) > 0.3333f)
    {
        a = 0.0f;
    }
    b = parvec[10]; if (fabsf(b) > 0.3333f)
    {
        b = 0.0f;
    }
    c = parvec[11]; if (fabsf(c) > 0.3333f)
    {
        c = 0.0f;
    }
...
static INLINE BLmat BL_matinv(BLmat mmm)
{
    ...
    if (det == 0.0f)
    {
        memset(&ttt, 0, sizeof(BLmat)); return ttt;
    }
    det = 1.0f / det;
...
/* Convert a general-form bilinear warp to a standard-form warp. The concept of general-form and
 * standard-form warps simplifies the discussion and coding of affine+bilinear catenation -- see the
 * math notes.
 *//*------------------------------------------------------------------------*/
...
/* Create a bilinear warp from parameters. Possible npar values:
 *  -12 ==> affine only; tensor=0 [params = 12 matrix values] 12 ==> affine only; tensor=0 [params =
 * shifts, angles, scales, shears]
 *  -39 ==> standard bilinear [1st 12 params = numerator matrix values] 39 ==> standard bilinear
 *[1st 12 params = shifts, angle, etc. ] 43 ==> 3dAllineate / 3dWarpDrive bilinear (with cen vector
 *and fac) God only knows what will happen to you if npar takes on any other value!
 *//*------------------------------------------------------------------------*/
 ...
 *   0 ==> tensor is all zero ==> warp is really affine 1 ==> tensor is nonzero only for diagonal
 *i==j elements 2 ==> tensor is nonzero in off-diagonal elements
 *//*------------------------------------------------------------------------*/
...
int BL_warp_tensor_status(BL_standard_warp wi)
{
    ...
    if (zero_tot == 27)
    {
        return 0;                   /* all tensor elements are zero */
    }
    if (zero_off == 18)
    {
        return 1;                   /* all off-diagonal elements zero */
    }
...
/* Apply a standard-form bilinear warp to a bunch of points. Special cases for pure affine and
 * diagonal denominator tensors are coded for efficiency. To use with OpenMP, #include this file
 * into an OpenMP-ized code.
 *//*------------------------------------------------------------------------*/
                   ...
                   float *xo, float *yo, float *zo)
{
    ...
    /* find status of denominator tensor 0 ==> it is zero (warp is pure affine) 1 ==> it is nonzero
     * only on diagonal elements: c[i][j][k] for i==j 2 ==> it is nonzero in some off diagonal
     * elements (the hardest case) */
...
int main(int argc, char *argv[])
{
    ...
    for (ii = 0 ; ii < npar ; ii++)
    {
        par[ii] = (float)strtod(argv[ii + 1], NULL);
    }
    for (      ; ii < PDIM ; ii++)
    {
        par[ii] = 0.0f;
