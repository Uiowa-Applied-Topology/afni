...
int THD_is_prefix_ondisk(char *pathname, int stripsels)
{
    ...
    if (stripsels)
    {
        ...
        for (ii = strlen(ppp) - 1; ii >= 0; --ii)
        {
            ...
            else if (ppp[ii] == '[' && open1)
            {
                ppp[ii] = '\0';
            }
            else if (ppp[ii] == '>' && !open2)
            {
                open2 = 1;
            }
            else if (ppp[ii] == '<' && open2)
...
int THD_mkdir(char *pathname)    /* 19 Dec 2002 */
{
    ...
    if (!THD_filename_ok(pathname))
    {
        return 0;
    }
    if (THD_is_ondisk(pathname))
    {
        return THD_is_directory(pathname);
    }
    ...
    while (1)
    {
        /* advance ii to point to end of next path segment, at the next '/' character, or at the end
         * of pnam */
 ...
 *  -1 file modification predates the specified day 0 same as specified day 1 newer 2 for error in
 *input or file not found              */
...
/* retrieve custom atlas directory and make sure it is present Do not free returned pointer. Failure
 * is a NULL pointer */
...
/* Return the name of the help directory, no guarantee that it exists Consider THD_get_helpdir() Do
 * not free returned pointer. Failure results in an empty string.
 */
char *THD_helpdir(byte withslash)
{
    ...
    if (withslash)
    {
        snprintf(sout[icall], 600 * sizeof(char), "%s/.afni/help/", home);
    }
    else
    {
...
/* Return the name of the data directory, no guarantee that it exists Consider THD_get_datadir() Do
 * not free returned pointer. Failure results in an empty string.
 */
char *THD_datadir(byte withslash)
{
    ...
    if (withslash)
    {
        snprintf(sout[icall], 600 * sizeof(char), "%s/.afni/data/", home);
    }
    else
    {
        snprintf(sout[icall], 599 * sizeof(char), "%s/.afni/data", home);
    }

    return sout[icall];
}

/* retrieve help directory and make sure it is present Do not free returned pointer. Failure is a
 * NULL pointer */
...
/* retrieve data directory and make sure it is present Do not free returned pointer. Failure is a
 * NULL pointer */
...
int THD_is_executable(char *pathname)    /* 26 Jun 2001 */
{
    ...
    ii = stat(pathname, &buf); if (ii)
    {
        return 0;
    }
    ii = (buf.st_mode & S_IXOTH) != 0; if (ii)
    {
        return ii;
    }
...
int THD_equiv_files(char *path1, char *path2)
{
    ...
    ii = stat(path1, &buf1); if (ii != 0)
    {
        return -1;
    }
    ii = stat(path2, &buf2); if (ii != 0)
    {
        return -1;
    }
 ...
 *   - the lev=2 trailing name is "author/of/AFNI", and so on. That is, "lev" is the number of
 *directory names above the last name to keep.  The pointer returned is to some place in the middle
 *of fname; that is, this is not a malloc()-ed string, so don't try to free() it!.
 * -------------------------------------------------------------------*/
...
char * THD_trailname(char *fname, int lev)
{
    ...
    if (fname == NULL || (flen = strlen(fname)) <= 1)
    {
        return fname;
    }
    ...
    if (fname[fpos - 1] == '/')
    {
        fpos--;                         /* skip trailing slash */
    }

    /* fpos   = index of latest character I've accepted, fpos-1 = index of next character to
     * examine, flev   = number of directory levels found so far */
...
/*! Return the path to fname. Do not free returned string.
 */
char * THD_filepath(char *fname)
{
    ...
    if (!(ppp = THD_trailname(fname, 0)))
    {
        return pname[icall];
    }
    if (!(lend = ppp - fname))
    {
        return pname[icall];                    /* no path on name */
    }
    if (lend >= THD_MAX_NAME - 1)
    {
        ERROR_message("Path name too long. Returning './' as the file path :-(");
...
int THD_filename_fix(char *name)    /* 04 Feb 2010 */
{
    ...
    ll = strlen(name); if (ll == 0)
    {
        return -1;
    }
    for (nfix = ii = 0 ; ii < ll ; ii++)
    {
        if (!THD_character_ok(name[ii]))
        {
            name[ii] = '_'; nfix++;
        }
    }
 ...
 *
 * The filename can have a '/' in it.  To insist that there be not '/', use THD_filename_pure(). The
 *list of crummy characters can be inferred from the source code.
 */
...
int THD_filename_ok(char *name)    /* 24 Apr 1997 */
{
    ...
    if (AFNI_yesenv("AFNI_ALLOW_ARBITRARY_FILENAMES"))
    {
        return 1;                                                 /* 08 Apr 2009 */
    }
    for (ii = 0 ; ii < ll ; ii++)
    {
...
/*! Check a list of dataset names for duplicates. Return value is number of duplicates found.
...
int THD_check_for_duplicates(int ns, char **sar, int flag)
{
    ...
    for (ii = 0 ; ii < ns - 1 ; ii++)
    {
        ...
        if (strcmp(di + li - 5, ".HEAD") == 0)
        {
            di[li - 5] = '\0';
        }
        else if (strcmp(di + li - 5, ".BRIK") == 0)
        {
            di[li - 5] = '\0';
        }
        else if (strcmp(di + li - 8, ".BRIK.gz") == 0)
        {
            di[li - 8] = '\0';
        }
        else if (strcmp(di + li - 7, ".nii.gz") == 0)
        {
            di[li - 3] = '\0';
        }
        else if (strcmp(di + li - 1, ".") == 0)
        {
            di[li - 1] = '\0';
        }
        ...
        for (jj = ii + 1 ; jj < ns ; jj++)
        {
            ...
            if (strcmp(dj + lj - 5, ".HEAD") == 0)
            {
                dj[lj - 5] = '\0';
            }
            else if (strcmp(dj + lj - 5, ".BRIK") == 0)
            {
                dj[lj - 5] = '\0';
            }
            else if (strcmp(dj + lj - 8, ".BRIK.gz") == 0)
            {
                dj[lj - 8] = '\0';
            }
            else if (strcmp(dj + lj - 7, ".nii.gz") == 0)
            {
                dj[lj - 3] = '\0';
            }
            else if (strcmp(dj + lj - 1, ".") == 0)
            {
                dj[lj - 1] = '\0';
            }
            ...
            if (strcmp(di, dj) == 0)
            {
                ...
                if (verb)
                {
                    WARNING_message("Datasets '%s' and '%s' are the same?!?",
                                    sar[ii], sar[jj]);
                }
            }
            free(dj);
