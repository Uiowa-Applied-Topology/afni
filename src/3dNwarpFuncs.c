...
int main(int argc, char *argv[])
{
    ...
    while (iarg < argc && argv[iarg][0] == '-')
    {
        ...
        if (strcasecmp(argv[iarg], "-nwarp") == 0 || strcasecmp(argv[iarg], "-warp") == 0)
        {
            ...
            if (dset_nwarp == NULL)
            {
                ERROR_exit("can't open warp dataset '%s' :-(", argv[iarg]);
            }
            if (DSET_NVALS(dset_nwarp) < 3)
            {
                ERROR_exit("dataset '%s' isn't a 3D warp", argv[iarg]);
            }
            iarg++; continue;
        }
    ...
    if (do_bulk)
    {
        bim = (float *)calloc(sizeof(float), nxyz);
    }
    if (do_shear)
    {
        sim = (float *)calloc(sizeof(float), nxyz);
    }
    if (do_vort)
    {
        vim = (float *)calloc(sizeof(float), nxyz);
    }
