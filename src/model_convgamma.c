 ...
 *
 *       t0 : time offset before response begins k  : amplitude r  : rise parameter (exponent on
 *polynomial component) b  : fall parameter (exponent on decay component)
 *
 * The above reference function is then convolved with the time series specified by
 *AFNI_CONVMODEL_REF, which might just be a TR-locked binary time series specifying onset events. In
 *such a case, the returned time series would be the sum of f(t), starting at each of the
 *"events" in AFNI_CONVMODEL_REF.
 *
 * Convolving with AFNI_CONVMODEL_REF could also handle events of different durations (spanning
 *multiple consecutive onset time points), as well as events with varying (but known) relative
 *magnitudes, akin to ampiltude modulation.
 *----------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------
 * Function to set the reference time series, with which the model function is convolved to produce
 *the simulated data.
 * ------------------------------------------------------------------------*/
...
void conv_set_ref(int num, float *ref)
{
    if (num > 0 && ref != NULL)  /*** if have inputs, make space & copy in ***/
    {
        ...
        if (refts != NULL)
        {
            free(refts); refts = NULL; free(refin); refin = NULL;
        }
        ...
        for (ii = 0, refnz = 0 ; ii < num ; ii++)        /* build list of nonzero */
        {
            if (refts[ii] != 0)
            {
                refin[refnz++] = ii;                     /* points in refts */
            }
        }
        if (refnz == 0)
        {
                ...
                float **x_array, float *ts_array)
{
    ...
    for (ii = 0 ; ii < refnz ; ii++)
    {
        ...
        jtop = ts_length - kk; if (jtop > nid_top)
        {
            jtop = nid_top + 1;
        }
