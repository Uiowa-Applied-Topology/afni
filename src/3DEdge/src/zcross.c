...
/*
 * `sign' of the zero-crossings if > 0, zero-crossings are chosen in the `positive' region ie are
 *points M such that I(M) > 0 et I(M+v) <= 0 if < 0 zero-crossings are chosen in the `negative'
 * region ie are points M such that I(M) < 0 et I(M+v) >= 0
 */
...
int Extract_PositiveZeroCrossing_2D(void *bufferIn,
                                    ...
                                    int *bufferDims)
{
    /* les passage par zeros sont definis par I(M) > 0 et I(M+v) <= 0 on marque M
     *
     * Dans un contexte de detection de contours (ex laplacien) cela marque dans les zones sombres
     *(par rapport a clair)
     */
    ...
    /*
     * // the (bufferDims[1]-1) first rows for ( y=0, iy=0; y<bufferDims[1]-1; y++, iy+=dx )
     *
     *   // the (dx-1) first points of the row for ( x=0; x<dx-1; x++ )
...
#define _POSITIVE_ZERO_CROSSINGS_(TYPE)    {                                                \
            ...
            for (z = 0, iz = 0; z < bufferDims[2]; z++, iz += dxy) {                        \
                for (y = 0, iy = 0; y < bufferDims[1] - 1; y++, iy += dx) {                 \
                    for (x = 0; x < dx - 1; x++) {                                          \
                        if (theBuf[iz + iy + x] > 0) {                                      \
                            if (theBuf[iz + iy + x + 1] <= 0 ||                             \
                                theBuf[iz + iy + x + dx] <= 0) resBuf[iz + iy + x] = 1;     \
                        } else {                                                            \
