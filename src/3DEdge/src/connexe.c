 ...
 *   - tmpBuf was erroneously set to inputBuf when typeOut was equal to either USHORT or SSHORT, it
 *is corrected and set to outputBuf.
 *
 ...
 *   - add parameters (connectivity, minimal size of connected components, maximum number of
 *connected components) and related procedures.
 ...
 *   - add the case ( maxNumberOfConnectedComponent == 1 ) which was obviously forgotten in
 *InternalConnectedComponentsExtraction().
 *
 *
 */
...
/* the size of the table of equivalence is fixed (it is easier to manage) and adapted to the the
 * maximal number of labels i.e. 65535 as labels are stored in an (unsigned short int) array
 */
 ...
 */
/*
 * static void SetBinaryLabelToValidComponent( typeConnectedComponent *components, int typeOut, int
 *binaryLabel );
 */
 ...
 *
 * the input image is relabeled with the connected components labels
 */
                           ...
                           double highThreshold)
{
    return HysteresisThresholdingWithAllParams(inputBuf, typeIn, outputBuf, typeOut, theDim,
                                               ...
                                               _connectivity_,
                                               _minimum_size_of_components_,
                                               (int)1, /* at least one point above the high
                                                        * threshold in each connected component */
                                               _maximum_number_of_components_,
                                               (int)1 /* output is binary */);
}

/* FROM NOW ON, we should not find any static global variable except _VERBOSE_
 */
                                          ...
                                          int outputIsBinary)
{
    ...
    components = (typeConnectedComponent *)malloc(_EQUIVALENCE_ARRAY_SIZE_ *
                                                  sizeof(typeConnectedComponent));
    if (components == (typeConnectedComponent *)NULL)
    {
        if ((typeOut != USHORT) && (typeOut != SSHORT))
        {
            free(tmpBuf);
        }
        if (_VERBOSE_)
                                        ...
                                        int outputIsBinary)
{
    ...
    if (lowThreshold >= 0.0)
    {
        iLowThreshold = (int)(lowThreshold + 0.5);
    }
    else
    {
        iLowThreshold = (int)(lowThreshold - 0.5);
    ...
    {
        iHighThreshold = (int)(highThreshold + 0.5);
    }
    else
    {
        iHighThreshold = (int)(highThreshold - 0.5);
    ...
    /* initialisation
     *
     * iLowThreshold and iHighThreshold are the nearest integer values of lowThreshold and
     *highThreshold.
     */
    resBuf = tmpBuf;
    switch (typeIn)
    ...
    case UCHAR:
    {
        ...
        for (i = 0; i < v; i++, theBuf++, resBuf++)
        {
            if (*theBuf >= iHighThreshold)
            {
                *resBuf = _hig_value_;
            }
            else if (*theBuf >= iLowThreshold)
            {
                *resBuf = _low_value_;
            }
            else
    ...
    case FLOAT:
    {
        ...
        for (i = 0; i < v; i++, theBuf++, resBuf++)
        {
            if (*theBuf >= highThreshold)
            {
                *resBuf = _hig_value_;
            }
            else if (*theBuf >= lowThreshold)
            {
                *resBuf = _low_value_;
            }
            else
 ...
 * - 0           : the background
 * - _low_value_ : points with values >= _low_value_ are considered for inclusion in a connected
 *component
 * - _hig_value_ : only connected components containing at least one point of value >= _hig_value_
 *are kept
 *
 * This way, this procedure may be used for different task, according some preprocessing of the
 *original buffer into *inputBuf
 * - extraction of connected components
 *   => all points to be considered are set to _hig_value_
 * - hysteresis thresholding
 *   => point above the low threshold (and below the high one) are set to _low_value, point above
 *the high threshold are set to _hig_value_
 * - connected components containing seeds
 *   => all points to be considered are set to _low_value_ seeds are set to _hig_value_
 *
 * after computation, it will contain labels of connected components several labels may correspond
 *to the same connected component, this is indicated by the array *cc.
 *
 * *theDim : dimensions of the buffer
 *
 * connectivity : the connectivity used for the components available: 4, 8, 6, 10, 18, and 26 4 and
 *8 are 2D connectivities, while 6, 10, 18 and 26 are 3D. 10 corresponds to the union of 6 and 8.
 *
 * Once the computation is done, some connected components may be discarded:
 * - the ones which do not contain a point with value >= _hig_value_
 * - some others according to the two last arguments
 *
 * minSize: minimum size of a connected components connected components with a size < minSize will
 *be discarded.
 *
 * nbCC: maximal number of connected components if nbCC > 0 Among the remaining components (the
 *little ones are already being discarded), only the nbCC largest ones are kept.
 *
 * *cc contains all the information about the result cc[0].label : number of valid connected
 *component for each point i, after computation, cc[ inputbuf[i] ].label is 0 if the component is
 *discarded is the right label else.
 *
 */

static int InternalConnectedComponentsExtraction(unsigned short int *inputBuf,

                                                 /* input buffer, should contains at most 3 values
                                                 ...
                                                 /* equivalence array, should be already allocated
                                                  */
                                                 int connectivity,
                                                 ...
                                                 /* requested minimum number of points above the low
                                                  * value to validate the connected component */
                                                 int minNumberOfPtsAboveHigh,

                                                 /* requested minimum number of points above the
                                                  * high value to validate the connected component
                                                  */
                                                 int maxNumberOfConnectedComponent,

                                                 /* maximal number of connected components. If this
                                                  * value is <= 0 or >= _MAXIMAL_LABEL_VALUE_
                                                  * (=65535), all the valid connected components are
                                                  * kept. For other values, the components are
                                                  * sorted with respect to their size and only the
                                                  * largest ones are kept. As a side effect, if the
                                                  * number of valid connected components is less
                                                  * than the maximal number of connected components,
                                                  * the labeling gives the order with respect to the
                                                  * size.
                                                  */
                                                 int outputIsBinary
                                                  ...
                                                  * component labeling. It is only used when we have
                                                  * to manage a label overflow, i.e.
                                                 ...
                                                 )
{
    ...
    /*
     * Computation of a point's label : current_label If this point has some neighbors (i.e.
     * nb_neighbors > 0) then 1. Its label is the minimum value of all the neighbors labels 2. We
     *report the equivalences between all the labels If not (i.e. nb_neighbors == 0) then we use a
     *new label (potential case of error : label overflow)
     */
#define _LABEL_COMPUTATION          { \
            if (nb_neighbors > 0) {   \
                ...
                if (used_labels > (_EQUIVALENCE_ARRAY_SIZE_ - 1)) { \
                    ...
                    tmpCc = LabelsOverflowManagement(inputBuf, theDim, cc, local_connexite, \
                                                     minNumberOfPtsAboveLow,                \
                                                     minNumberOfPtsAboveHigh,               \
                    ...
                    if (tmpCc == (typeConnectedComponent *)NULL || \
                        ...
                        if (_VERBOSE_)                                                             \
                        fprintf(stderr,                                                            \
                                "%s: too much used labels for connected components computation\n", \
                                proc);                                                             \
    ...
    /*
     * Generic tests of all the neighbors we make sure that they belong to the image. We check if
     *the neighbor has a label (value > 0) We are looking for the equivalence classe (i.e. cc[ value
...
#define _GENERIC_TEST               {                                                  \
    ...
    /*
     * Specific tests for the upper row of the neighborhood of a point We check if the neighbor is
     *connected to the current point if the neighbor belongs to the image if the neighbor has a
     *label
     */
#define _UPPER_ROW_TESTS            {                                                                \
            if (y > 0) {                                                                             \
                for (i = -1; i <= 1; i++) {                                                          \
                    ...
                    label_neighbors[nb_neighbors++] = cc[(int)(*(theBuf +                            \
                                                                 array_offset[1 + i][0][1]))].label; \
                }                                                                                    \
            }                                                                                        \
}

    /*
     * Specific test for the left neighbor of a point We should test if (array_offset[0][1][1] != 0)
     *but we do not because the point (x-1,y,z) always belongs to the neighborhoods we use.
     */
#define _LEFT_NEIGHBOR_TEST         {                                         \
     ...
     * - the validity of the connect component is checked (with respect to the current value)
     */
#define _ADDITIONAL_INFORMATIONS    {                     \
     ...
     *
     * We will attribute to each point a value (a label). According to this value and to the
     *equivalence table, we know the equivalence classe of the point (i.e. cc[ value ].label).
     *
     * The computation is divided into two parts : the first slice and the others slices. This
     *allows to avoid some tests and to spare some time.
     *
     ...
     *   if the point is on the image border. This point is different for the first slice and the
     *other ones.
     *   first slice  -> if ( y == 0 ) _INSIDEY_ = 0;
     *   other slices -> if ( (y == 0) || (y == dimy1) ) _INSIDEY_ = 0;
     *   => ( _INSIDEY_ == 1 ) && ( x > 0 ) && ( x < dimx1 )
     * - depending on the above checking, we compute the number of neighbors (nb_neighbors) and
     *extract all the neighbors' values (label_neighbors[]), either without additionnal tests
     *(_GENERIC_TEST), or with additionnal tests for the neighbors in the upper slice, in the upper
     *row (_UPPER_ROW_TESTS) or the left neighbor (_LEFT_NEIGHBOR_TEST). The two offsets structures
     *(offsets[] and array_offset[][][]) are necessary because of this choice.
     * - Once we have the number of neighbors and their values, we can compute the point's label
     *(_LABEL_COMPUTATION)
     *   - if the point have neighbors, its label will be the minimum of all labels, and we report
     *the equivalence between this minimum label and the other labels in the neighborhood inside the
     *equivalence table.
     *   - if the point does not have any neighbor, we use a new label.
     *     we increment the number of used label (++used_labels) and we initialize a new label.
     *WARNING: an error may occur if such initialization is not possible (it depends on the type
     *used for the image).
     * - Once the point's label is computed, we report this value in the image, increment the
     *connected component size and test if connected component will be valid (current_value >=
     *_hig_value_)
     */
    ...
    nb_offsets      = InitNeighborsOffsets(array_offset, offsets, dimx, dimx * dimy,
                                           local_connexite);
        ...
        for (i = 1; i <= used_labels; i++)
        {
            if (cc[i].label == i)
            {
                    ...
                    (cc[i].pointsAboveLowThreshold >= minNumberOfPtsAboveLow))
                {
                    ...
                    else if (cc[valid_labels].pointsAboveLowThreshold <
                             cc[i].pointsAboveLowThreshold)
                    {
                        valid_labels = i;
                    }
                }
    ...
    /*
     * counting and re-labeling all the valid connected components this is not optimal, but more
     *elegant because it may be used in several further cases.
     *
     * The representative label of a equivalence's classe (i.e. a connected component) is
     *characterized by ( cc[ i ].label == i ). We have to check if this connected component is valid
     *i.e.
     ...
     *   => (cc[ i ].pointsAboveLowThreshold >= minNumberOfPtsAboveLow) ) if yes, we increment the
     *number of valid connected components (++valid_labels) and give this value as a new label for
     *the connected component.
     * if no, we give 0 as a new label and as a new size.
     *
     * If a label is not representative of its equivalence's classe (i.e. cc[ i ].label < i), we
     *give to it the new value of its representative label (which may be 0 if the connected
     *component is not valid). Recall that the representative label of an equivalence's classe is
     *always parsed before the other labels of the same equivalence's classe. Id. for the size.
     *
     * To keep recognizing the valid classes, the representative labels have the numbers of points,
    ...
    /*
     * At this point, all valid components have been renumbered, and have the right size.
     */
    ...
    /*
     * output: the n largest connected component maxNumberOfConnectedComponent > 1 we will sort the
     *valid connected components (# = valid_labels) with respect to their size
     */
    if (maxNumberOfConnectedComponent > 1)
    {
        validCc = (typeConnectedComponent *)malloc((valid_labels + 1) *
                                                   sizeof(typeConnectedComponent));
        if (validCc == (typeConnectedComponent *)NULL)
        {
            ...
            {
                fprintf(stderr,
                        "%s: allocation failed for auxiliary array (to sort connected components)\n",
                        proc);
            }
            return -1;
        }

        /* store all the valid connected components in an array A valid connected component is
         * recognized by (cc[ i ].pointsAboveHighThreshold >=
         * minNumberOfPtsAboveHigh) or (cc[ i ].pointsAboveLowThreshold >= minNumberOfPtsAboveLow)
         *
        ...
        /* At this point, we want to keep the connected components represented by validCc[i].label
         * for i=1...min(maxNumberOfConnectedComponent,valid_labels).
         *
         * and to reject the connected components represented by validCc[i].label for
         *i=min(maxNumberOfConnectedComponent,valid_labels)+1 ... valid_labels
         *
         * the labels (validCc[i].label) are in the range 1...valid_labels
         *
         * If (valid_labels <= maxNumberOfConnectedComponent) all connected components are kept, but
         *they are sorted with respect to size.
         */

        /* we mark by cc[ i ].pointsAboveHighThreshold = 'index after sort'
         * the connected components to be kept and by cc[ j ].pointsAboveHighThreshold = 0 the
         *connected components to be removed
         */
        ...
        /* if maxNumberOfConnectedComponent >= valid_labels, the following does not matter
         */
        ...
        /* here, for i=1...valid_labels if ( validCc[ i ].pointsAboveHighThreshold > 0 ) the
         * connected component is to be kept and its index should be validCc[ i
         * ].pointsAboveHighThreshold (its index after sort) instead of validCc[ i
         * ].label
         */
         ...
         */
        cc[0].label = (valid_labels >
                       maxNumberOfConnectedComponent) ? maxNumberOfConnectedComponent :
                      valid_labels;
        cc[0].pointsAboveHighThreshold = total_labels;
        cc[0].pointsAboveLowThreshold  = used_labels;
        return 1;
...
/* calcule les offsets pour connaitre les voisins du point courant selon la connexite. Retourne le
 * nombre de voisins.
 */
                                        ...
                                        int dimz)
{
    ...
    /* seules les connexites 26, 18, 10, 6 (en 3D) et 8 et 4 (en 2D) sont permises
     */
    switch (connectivity)
    {
    default: break;
 ...
 *
 * Pour les composantes deja entierement traitees, on determine si elles sont valides ou non.
 * => on recupere les labels des composantes invalidees.
 *
 * Pour les composantes valides, et les composantes non entierement traitees, on leur attribue a
 *toutes un seul label, et on recupere ainsi tous les labels intermediaires.
 */
static typeConnectedComponent * LabelsOverflowManagement(unsigned short int *inputBuf,

                                                         /* input buffer, it is already labeled
                                                          * until the point (x,y,z) not included
                                                          */
                                                         int *theDim,
                                                         /* dimension of the input buffer */
                                                         typeConnectedComponent *cc,
                                                         ...
                                                         int x, int y, int z)
{
     ...
     * -> (xlast, ylast, zlast)
     *
     * (x,y,z) represente le point courant pour lequel l'attribution d'un nouveau label a echoue
     *
     * => on cherche le dernier point traite avec succes (ce peut etre un point du fond)
     */
    ...
    /* pour certaines composantes (classes d'equivalence) on sait deja si il faut les garder ou non,
     * ce sont celles qui n'ont plus de voisins dans les points qui restent a parcourir.
     *
     * Determination du dernier point
     * -> (xnum, ynum, znum) pouvant appartenir a l'une de ces composantes
     *
     * On ne regarde que les cas non-degeneres, c-a-d ou le dernier point traite n'appartient pas a
     *un bord.
     */
    znum = ynum = xnum = -1;
     ...
     *
     * 1. Si (xnum, ynum, znum) est valide, alors les composantes dont tous les points se trouvent
     *entre (0,0,0) et (xnum, ynum, znum) inclus sont deja entierement traitees et on peut verifier
     *leur validite Les autres (dont au moins un point se trouve entre (xnum, ynum, znum)+1 et
     *(xlast, ylast, zlast) inclus sont conservees telles quelles.
     * 2. sinon toutes les composantes sont conservees telles quelles sans test de validation.
     */
    ...
    /* on regarde les labels et on cherche les representants des classes d'equivalences (labels[i] =
     * i) 1. si la composante peut encore grandir (completelyProcessed = 0) => on la garde 2. sinon,
     *on teste sa validite
     *
     * si l'output est binaire, on garde la meme valeur pour toutes les composantes valides (on fait
     *comme si c'etait une seule composante)
     */
    ...
    for (i = 1; i <= (*used_labels); i++)
    {
        ...
        if (cc[i].label == i)
        {
            ...
            else
            {
                    ...
                    (cc[i].pointsAboveLowThreshold >= minNumberOfPtsAboveLow))
                {
                    ...
                    /* mode binaire, on recupere le numero de la premiere composante complete, et on
                     * affectera ce numero a TOUTES les composantes valides
                     */
                    if (outputIsBinary != 0)
                    {
                        if (valid_comps == 1)
                    ...
                    /* mode etiquettes:
                     * on garde la composante telle quelle et on incremente les labels attribues
                     */
                    else
                    {
                        cc[i].label = ++valid_labels;
                ...
                /* on peut ne plus considerer la composante elle n'est pas valide
                 */
                else
                {
                    cc[i].label = 0;
        ...
        /* ce n'est pas le label representant la classe d'equivalence on recupere le nouveau label
         * de celle-ci
         */
        else
        {
    ...
    {
        fprintf(stderr,
                "%s: previous number of used labels: %d\n\
...
\t number of already valid components: %d",
        ...
        {
            fprintf(stderr,
                    "%s: last point for valid component: %3d %3d %3d\n\
...
\t current point:                  %3d %3d %3d",
    ...
    /* on remonte le nombre de labels utilises et la nouvelle table d'equivalence
     */
    *used_labels = valid_labels;
    return components;
}

/* Sorting an array of 'connected component' structures with respect to size adaptation of the
 * quicksort
 */
static void SortConnectedComponents(typeConnectedComponent *tab,
                                    int left,
...
/* Relabel a buffer with respect to an array of connected components descriptors
 */
static int RelabelConnectedComponents(void *inputBuf,
                                      bufferType typeIn,
                                      int *theDim,
                                      ...
                                      int binaryLabel)
{
    ...
    /* binary output ?
     * if yes, we choose as the single output value the maximal value, depending on the type, if no
     *value was specified
     */
    if (outputIsBinary != 0)
    {
                                               ...
                                               int *theDim)
{
    ...
    /* ici, on a theCC[i] qui est la ieme composante selon la taille, et qui a le label
     * theCC[i].label
     * => on range les labels dans size theCC[ theCC[i].label ].size = i pour faire le changement
     */
