...
int main(int argc, char *argv[])
{
    ...
    for (i = 1; i < argc; i++)
    {
        if (argv[i][0] == '-')
        {
                     ...
                     (strcmp(argv[i], "-lt") == 0))
            {
                ...
                if (i >= argc)
                {
                    ErrorMessage("parsing -low-threshold...\n", 0);
                }
                     ...
                     (strcmp(argv[i], "-ht") == 0))
            {
                ...
                if (i >= argc)
                {
                    ErrorMessage("parsing -high-threshold...\n", 0);
                }
                     ...
                     (strcmp(argv[i], "-ms") == 0))
            {
                ...
                if (i >= argc)
                {
                    ErrorMessage("parsing -minimal-size...\n", 0);
                }
    ...
    bufferOut = (void *)malloc(bufferDims[0] * bufferDims[1] * bufferDims[2] *
    ...
    /*
     * to change the connectivity -> Connexe_SetConnectivity to change the minimal size of the
     *connected components to be kept.
     *                            -> Connexe_SetMinimumSizeOfComponents to change the maximum number
     *of the connected components to be kept.
     ...
     * => HysteresisThresholdingWithAllParams() see connexe.h
     */
