 ...
 * DESCRIPTION:
 *
 * recursive filtering of a buffer (a [1,2,3]D array) according that the filtering is separable
 *
 *
 *
 * AUTHOR:
 ...
 * ADDITIONS, CHANGES
 *
 * * Jul 6 1999 (Gregoire Malandain) a bug in RecursiveFilterOnBuffer (*&%^@$^ cut and paste)
 *
 */
                            ...
                            recursiveFilterType filterType)
{
    ...
    /*
     * theZ0 : smoothed         along Z theZ1 : first derivative along Z theZ  : first derivative
     *along Z, smoothed along X and Y theZZ : second derivative along Z, smoothed along X and Y
     */
                            ...
                            recursiveFilterType filterType)
{
    ...
    /*
     * obviously, we need to perform the computation with float or double values. For this reason,
     *we allocate an auxiliary buffer if the output buffer is not of type float or double.
     */
     ...
     * If its type is FLOAT or DOUBLE, then yes. If not, we have to allocate an auxiliary buffer.
     */
     ...
     * If its type is FLOAT or DOUBLE, then yes. If not, we convert it into the buffer bufferResult,
     *and this last buffer is the bufferToBeProcessed.
     */
    ...
    /*
     * Tue Jul  6 19:15:15 MET DST 1999 (gregoire.malandainoire Malandain) changes 3 x dimx -> dimx,
     *dimy, dimz
     */
    ...
    /*
     * Allocations of work arrays. We will use them to process each line.
     */
    ...
    /*
     * From now, typeToBeProcessed is either FLOAT or DOUBLE so is typeResult.
     */
            ...
            if (filterCoefs[0] > 0.0)
            {
                ...
                for (z = 0; z < dimz; z++)
                {
                    for (y = 0; y < dimy; y++)
                    {
                         ...
                         * Processing the line.
                         */
                        if (RecursiveFilter1D(RFC, theLine, resLine, tmpLine, resLine,
                                              lengthX) == 0)
                        {
                            if (_VERBOSE_ != 0)
                            {
                ...
                /*
                 * The next buffer to be processed is the buffer bufferResult.
                 */
                bufferToBeProcessed = bufferResult;
                typeToBeProcessed   = typeResult;
        ...
        if (derivatives[1] != NODERIVATIVE)
        {
            if (filterCoefs[1] > 0.0)
            {
                ...
                for (z = 0; z < dimz; z++)
                {
                    ...
                    /*
                     * Going to the first point of the next Y line which is the first Y line of the
                     *next slice.
                     *
                     * The pointer r[32,64]firstPoint[Result] has already been increased by dimx. To
                     *reach the first point of the next slice, we have to increase it by
                     *(dimx*dimy)-dimx.
                     */
                    switch (typeToBeProcessed)
                    {
