...
/*
 * epsilon value to decide of the interpolation type. If one derivative's absolute value is larger
 *than this epsilon (close to one), then we use the nearest value else we perform a [bi,tri]linear
 *interpolation.
 */
                               ...
                               recursiveFilterType filterType)
{
    ...
    /*
     * Allocation of auxiliary buffer. We need a slice buffer for each gradients' component plus one
     *for the modulus.
     */
     ...
     *
     * For each slice, we compute both the X and Y components of the gradient, its modulus, and we
     *suppress the non-maxima of the gradient. Finally, we put the result in the buffer bufferOut.
     *
     * An other solution may consist in computing the  X and Y components of the gradient for the
     *whole 3D buffer, and performing the non-maxima suppression slice by slice.
     */
    for (z = 0; z < bufferDims[2]; z++)
    {
        ...
        /*
         * sliceIn points towards the slice #z of the buffer bufferIn.
         */
        switch (typeIn)
        {
        case UCHAR:
        ...
        /*
         * computing the X and Y component of the gradient.
         */
        ...
        /*
         * Suppression of the non maxima of the gradient in the direction of the gradient.
         *
         * If the type of the result buffer bufferOut is FLOAT, then we compute directly the result
         *into the slice #z of the result buffer. Else, we compute the suppression of the non maxima
         *into the gx buffer, and we convert it into the result buffer type.
         */
        if (typeOut == FLOAT)
        {
                               ...
                               recursiveFilterType filterType)
{
     ...
     */
    /*
     * gx[0] points toward the X gradient of the current slice gx[0] points toward the X gradient of
     *the next slice
     */
    ...
    /*
     * norme[0] points toward the gradient modulus of the previous slice norme[1] points toward the
     *gradient modulus of the current slice norme[2] points toward the gradient modulus of the next
     *slice
     */
     ...
     *
     * We need a 3D buffer for the Z component of the gradient, plus a 3D buffer for the 3D buffer
     *smoothed along Z, plus 7 2D buffers for the X component of the gradient in both the current
     *and the next slices, idem for the Y component, idem for the modulus plus one 2D buffer for the
     *modulua in the previous slice.
     *
     * If the buffer bufferOut is of type FLOAT, we use it as the Z component of the gradient.
     *
     * This Z component will be used to stored the extrema of the gradient.
     */
    ...
    /*
     * Computation of the Z component of the gradient. Computation of the input buffer smoothed
     *along Z.
     */
     ...
     *
     * - computation of the X component of the gradient for that slice
    ...
    for (z = 1; z < bufferDims[2] - 1; z++)
    {
        ...
        /*
         * gx[0] and gy[0] are the X and Y components of the gradient of the current slice.
         * gx[1] and gy[1] are the X and Y components of the gradient of the next slice.
         * norme[0] is the gradient modulus of the previous slice, norme[1] is the gradient modulus
         *of the current slice, norme[2] is the gradient modulus of the next slice.
         */
        /*
     ...
     * last slice
     *
     * Components and modulus of the gradient are already computed.
     *
    ...
    /*
     * conversion of the buffer bufferZderivated of type FLOAT into the buffer bufferOut.
     */
...
{
    /*
     * the buffer norme[0] contains the gradient modulus of the previous slice, the buffer norme[1]
     *the ones of the slice under study, while norme[2] contains the ones of the next slice.
     */
    /*
     * dimensions
     */
    ...
    /*
     * we set the image border to zero. First the borders along X direction, second, the borders
     ...
     */
    /*
     * Pointers are set to the first point to be processed.
     */
    ...
    for (y = 1; y < dimyMinusOne; y++, fl_max += 2, fl_pt1 += 2, fl_pt2 += 2, fl_nor += 2)
    {
        for (x = 1; x < dimxMinusOne; x++, fl_max++, fl_pt1++, fl_pt2++, fl_nor++)
        {
            ...
            /*
             * First point to be interpolated. It is the current point + an unitary vector in the
             *direction of the gradient. It must be inside the image.
             */
            ...
            /*
             * Upper left corner, coordinates of the point to be interpolated with respect to this
             *corner.
             */
            ...
            /*
             * bilinear interpolation of the gradient modulus norme[x_point_to_be_interpolated,
            ...
            /*
             * We compare the modulus of the point with the interpolated modulus. It must be larger
             *to be still considered as a potential gradient extrema.
             *
             * Here, we consider that it is strictly superior. The next comparison will be superior
             *or equal. This way, the extrema is in the light part of the image. By inverting both
             *tests, we can put it in the dark side of the image.
             */
            if (*fl_nor <= interpolated_norme)
            {
                *fl_max = 0.0;
            ...
            /*
             * Second point to be interpolated. It is the current point - an unitary vector in the
             *direction of the gradient. It must be inside the image.
             */
            x_point_to_be_interpolated = (double)x - normalized_gx;
            y_point_to_be_interpolated = (double)y - normalized_gy;
            ...
            /* we do not recompute the coefficients dx = x_point_to_be_interpolated -
             * (double)x_upper_left_corner;
             * dy = y_point_to_be_interpolated - (double)y_upper_left_corner;
             * dxdy = dx * dy;
             */
            /*
             ...
             *   norme[1,1] * c00
             *
             * WARNING: it works only if the cases where one derivative is close to -/+ 1 are
             *already be independently processed, else it may lead to errors.
             */
            /* we do not recompute the coefficients c00 = 1.0 - dx - dy + dxdy;
             * c10 = dx - dxdy;
             * c01 = dy - dxdy;
             * c11 = dxdy;
             * fl_upper_left = norme + (x_upper_left_corner + y_upper_left_corner * dimx);
            ...
            /*
             * Last test to decide whether or not we have an extrema
             */
            if (*fl_nor < interpolated_norme)
            {
                *fl_max = 0.0;
                                        ...
                                        int *bufferDims)
{
    ...
    for (y = 1; y < dimyMinusOne;
         y++, fl_max += 2, fl_pt1 += 2, fl_pt2 += 2, fl_pt3 += 2, fl_nor += 2)
    {
        for (x = 1; x < dimxMinusOne; x++, fl_max++, fl_pt1++, fl_pt2++, fl_pt3++, fl_nor++)
        {
            ...
            /*
             * trilinear interpolation of the gradient modulus norme[x_point_to_be_interpolated,
             *y_point_to_be_interpolated, z_point_to_be_interpolated] =
             ...
             *
             *
             * WARNING: as in the 2D case it works only if the cases where one derivative is close
             *to -/+ 1 are already be independently processed, else it may lead to errors.
             */
            /* we do not recompute the coefficients dxdy = dx * dy;
             * dydz = dy * dz;
             * dxdz = dx * dz;
             * c111 = dxdy * dz;
             * c011 = dydz - c111;
