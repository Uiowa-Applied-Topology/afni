...
/*********************************************************************
*  These are the template routines to fill in a slice of data in a new image array from an old image
*brick array, warped on demand.
*
*  To create actual routines for this purpose, you must compile the file with the preprocessor
*symbol DTYPE set to one of the following types:
*
*     byte short int float double complex rgbyte rgba
*
*     cc -c -DDTYPE=short afni_slice.c mv -f afni_slice.o afni_slice_short.o
*
*  In the example above, the resulting routines will be named
*
*     AFNI_lmap_to_xslice_short AFNI_lmap_to_yslice_short AFNI_lmap_to_zslice_short AFNI_br2sl_short
*
*  and will take as input pointers bold and bslice, pointers of type "short *".
*********************************************************************/
...
/*******************************************************************
 *   To add a new DTYPE, you must add definitions in mrilib.h, and define the following macros for
 *the new type:
...
#define FMAD4_short(a, d1, b, d2, c, d3, d, d4, \
                    e)    (e) = (a) * (d1) + (b) * (d2) + (c) * (d3) + (d) * (d4)
...
/** ALOOP: inner loop statements for the actual NN assignments, and there is no need to check for
 * being outside the input brick _GEN:  for the general (non-parallel) case _PAR:  for the case when
 * the inner loop is parallel to an input brick axis **/
...
#define NN_ALOOP_GEN                                                                               \
        do{                                                                                        \
            ...
            if (PRINT_TRACING) { char str[256];                                                    \
                                 sprintf(str, "out_ind=%d xi_old=%d yj_old=%d zk_old=%d", out_ind, \
                                         xi_old, yj_old, zk_old);                                  \
                                 STATUS(str); }                                                    \
            bslice[out_ind++] = bold[IBASE(xi_old, yj_old, zk_old)];                               \
        } while (0)
...
/** BLOOP: assign values to the ib array that will be used to index into the input brick for rapid
 * access;
...
/** CLOOP: like the ALOOP, but for when we must test if the desired point may be outside the input
 * brick array **/
...
/*---------------------------------------------------------------------
 * routine to apply a linear mapping to a dataset and fill in a single fixed-x slice from a 3D
 *dataset:
 *
 *   map        = linear mapping from old to new (bot & top field contain index limits in new)
 *resam_mode = type of interpolation to do in old
 *
 *   old_daxes  = axis information of old dataset bold       = pointer to old dataset's 3D data
 *brick array
 *
 *   new_daxes  = axis information of new dataset xi_fix     = fixed x index in new dataset bslice
 *     = pointer to nynew * nznew array to hold slice
 *
 * Cognate routines for yslice (output is nznew * nxnew: Y-ZX) and zslize (output is nxnew * nynew:
 * Z-XY) follow this one directly.
 * -----------------------------------------------------------------------*/
                ...
                THD_dataxes *new_daxes, int xi_fix, DTYPE *bslice)
{
    ...
    if (xi_fix < xi_bot || xi_fix > xi_top)
    {
        EXRETURN;                                       /* map doesn't apply! */
    }
    if (bold == NULL || bslice == NULL)
    {
        STATUS("NULL pointers?!"); EXRETURN;
    }
    ...
    case RESAM_NN_TYPE: {
         ...
         *  if so, can use a version of the resampling loop that does not need to check each voxel
         *for being inside -- hopefully, this will execute more quickly **/
        ...
        {
            char str[256];
            sprintf(str, "fxi_bot=%g  fxi_top=%g  fxi_min=%g  fxi_max=%g", fxi_bot, fxi_top,
                    fxi_min, fxi_max);
            STATUS(str);
            sprintf(str, "fyj_bot=%g  fyj_top=%g  fyj_min=%g  fyj_max=%g", fyj_bot, fyj_top,
                    fyj_min, fyj_max);
            STATUS(str);
            sprintf(str, "fzk_bot=%g  fzk_top=%g  fzk_min=%g  fzk_max=%g", fzk_bot, fzk_top,
                    fzk_min, fzk_max);
            STATUS(str);
        }
#endif
...
#define IND_top     TWO_TWO(IND_NAME, _top) /* inner loop index max  */
...

        {
            ...
            /** thz = flag that indicates which of the steps df??_inner are zero. If two of them are
             * zero, then the inner loop is parallel to one of the input brick axes, and so data may
             * be pulled out in a very efficient fashion.  In such a case, precompute the indexes
             * for the inner loop:
             *
             *        the BLOOP macros load array ib, which holds the inner loop computed indexes
             *for each inner loop position.
             *        ub = upper bound value for ib array value to still be inside input brick
             *array.
            ...
            if (PRINT_TRACING)
            {
                ...
                if (any_outside)
                {
                    sprintf(str, "NN resample has some outside: thz=%d tho=%d", thz, tho);
                }
                else
                {
                    sprintf(str, "NN resample has all inside: thz=%d thd=%d", thz, tho);
                }
                STATUS(str);
            ...
            case XY_ZERO:
                ...
                for (IND = IND_bot ; IND <= IND_top ; IND++)
                {
                    NN_BLOOP_XY_ZERO(IND);
                }
                break;
            ...
            for (OUD = OUD_bot ; OUD <= OUD_top ; OUD++)
            {
                 ...
                 *     all inside, inner loop not parallel to any input axis all inside, inner loop
                 *parallel to input brick z-axis all inside, inner loop parallel to input brick
                 *y-axis all inside, inner loop parallel to input brick x-axis
                 *
                 *   and then the 4 same cases repeated when not all desired points are inside the
                 *input brick.  Each of these is coded separately for efficiency.  This is important
                 *for rapid re-display of results during interactive imaging. ***/
 ...
 ***/
                ...
                /* interpolate in the z direction and put the result into the output array!
...
/* use the ANSI token-merge operator ## to create an interpolating macro for the x-direction, at
 * each offset in y (j) and z (k)    */
 ...
 ***/
                 ...
                 * (the +0.5 is to force rounding of the result) */
/***
 *             bslice[out_ind++] = 0.5 + CP_FACTOR ( zwt_m1 * f_km1 + zwt_00 * f_k00
                ...
                THD_dataxes *new_daxes, int yj_fix, DTYPE *bslice)
{
    ...
    if (yj_fix < yj_bot || yj_fix > yj_top)
    {
        EXRETURN;                                       /* map doesn't apply! */
    }
    if (bold == NULL || bslice == NULL)
...
#define IND_top     TWO_TWO(IND_NAME, _top) /* inner loop index max  */
...

        {
            ...
            if (PRINT_TRACING)
            {
                ...
                if (any_outside)
                {
                    sprintf(str, "NN resample has some outside: thz = %d", thz);
                }
                else
                {
                    sprintf(str, "NN resample has all inside: thz = %d", thz);
                }
                STATUS(str);
                sprintf(str, "OUD_bot=%d  OUD_top=%d  nxold=%d nyold=%d nzold=%d",
                        OUD_bot, OUD_top, nxold, nyold, nzold); STATUS(str);
                ...
                THD_dataxes *new_daxes, int zk_fix, DTYPE *bslice)
{
    ...
    if (zk_fix < zk_bot || zk_fix > zk_top)
    {
        EXRETURN;                                       /* map doesn't apply! */
    }
    if (bold == NULL || bslice == NULL)
 ...
 *   nxx,nyy,nzz = brick dimensions fixed_axis  = 1, 2, or 3 (x, y, or z) fixed_index = subscript of
 *chosen slice in the fixed_axis direction bold        = pointer to brick array bslice      =
 * pointer to area to get slice from brick array fixed_axis = 1 --> nyy * nzz 2 --> nzz * nxx 3 -->
 * nxx * nyy
 * -----------------------------------------------------------------------*/
