 ...
 *
 * FD_bricks are a relic of the very earliest code that evolved into AFNI.  By the time I invented
 *THD_3dim_datasets, I had enough code in place to keep this data structure alive.  Basically, it
 *exists only to provide a structure that enables fast copying of data out of a 3D array into 2D
 *arrays.
 * -------------------------------------------------------------------------*/
...
FD_brick ** THD_setup_bricks(THD_3dim_dataset *dset)
{
     ...
     * the top option is the way I think it ought to be, so that in the axial and coronal views,
...
/*----------------------------------------------------------------------
 * 07 Dec 2001 - orient an FD brick in any legal way (e.g., orients = "RAI" for standard axial)
 * ------------------------------------------------------------------------*/
 ...
 *  ax_n = 1,2,3 for displaying the x,y,z axis along the n-th display direction (n=1 --> across, n=2
 * --> down, n=3 --> slices ) negative value means reverse
 *
                                     ...
                                     int ax_1, int ax_2, int ax_3)
{
    ...
    /*-- assign original dimensions to arrays, then pick out the permuted dimensions --*/
