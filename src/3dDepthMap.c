 ...
 * ver = 2.0;  date = Nov 29, 2021
 ...
 +lib_EDT.py with P Lauren's concurrent work on a C version (which had been compared/developed in
 +part with
 + the aforementioned lib_EDT.py).
 +
 + ver = 2.1;  date = Dec 1, 2021
 + [PT] Rearrange how main run_EDT_3D works, and how calc_EDT_3D() is called. Mostly, we want to
 +make it easier for other programs to use the latter func
 +
 + ver = 2.2;  date = Dec 1, 2021
 + [PT] Bug fix: had delta set incorrectly in 2 out of 3 calc_EDT_3d_dim?() funcs, because was using
 +*DZ* everywhere.
 +
 + ver = 2.3;  date = Dec 8, 2021
 + [PT] new opt, '-only2D ..' so that EDT can be calced in only 2D, if desired.  For DRG, may he use
 +it well.
 +
 + ver = 2.4;  date = Dec 9, 2021
 + [PT] internal tweak: allocate some tmp arrays less, doesn't really make much difference in speed
 +:(
 +
 ...
 + [PT] -binary_only opt now working well (faster code running for special case of binary input
 +mask)
 +
 ...
 + - ones that are specifically the 'general' multiROI alg have "_GEN" as part of their name, to
 +distinguish them from _BIN ones (latter, only for binary input mask)
 +
               ...
               int argc, char *argv[])
{
    ...
    if (opts.rimify)
    {
        /* the "rim" dset overlaps the original at the rims, will just be zeroed out elsewhere.
         *  dset_rc will have same type as dset_roi */
