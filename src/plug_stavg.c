...
char * STAVG_main(PLUGIN_interface *plint)
{
    ...
    if (stimim == NULL)
    {
        return "Please specify stimulus timing";
    }
    ...
    if (RMB_DEBUG)
    {
        fprintf(stderr, "Done with avg_epochs\n");
    }
    ...
    /*------- The output is now in fout[kk][ii], for kk=0..maxlength-1 , ii=0..nvox-1. We must now
     * put this into the output dataset -------*/
...
/*---------------------------------------------------------------*/
{
    ...
    if (RMB_DEBUG)
    {
        fprintf(stderr, "Start stavg\n");
    }

    /* determine number of epochs to average */
    if (RMB_DEBUG)
    {
        fprintf(stderr, "Determining number of epochs...");
    }
    numepochs = 1;
    ...
    if (RMB_DEBUG)
    {
        fprintf(stderr, "Set array of epoch lengths...");
    }
    pEpochLength = (int *)malloc(sizeof(int) * numepochs);
    for (ne = 0; ne < numepochs; ne++)
    {
        pEpochLength[ne] = 0;
    }
    ne = 0;
    ...
    if (RMB_DEBUG)
    {
        fprintf(stderr, "Set array of time markers...");
    }
    ...
    for (ne = 0; ne < (numepochs - 1); ne++)
    {
        ...
        if (pEpochLength[ne + 1] > maxlength)
        {
            maxlength = pEpochLength[ne + 1];
        }
        if (pEpochLength[ne + 1] < minlength)
        {
            minlength = pEpochLength[ne + 1];
        }
    }
    if (RMB_DEBUG)
    {
    ...
    if (RMB_DEBUG)
    {
        fprintf(stderr, "Start averaging...");
    }
    for (te = 0; te < maxlength; te++)
    {
        pNumAvg[te] = 0.0;
        ...
        case _STAVG_METH_SIGMA:
            ...
            /* sigma statistic is actually the unbiased standard deviation calculated with this
    ...
    if (RMB_DEBUG)
    {
        fprintf(stderr, "convert to output...");
    }
    for (te = 0; te < maxlength; te++)
    {
        for (ii = 0; ii < nvox; ii++)
