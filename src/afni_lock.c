...
void AFNI_time_lock_carryout(Three_D_View *im3d)
{
    ...
    if (GLOBAL_library.ignore_lock)
    {
        EXRETURN;                               /* ordered not to do anything */
    }
    if (!GLOBAL_library.time_lock)
    {
        EXRETURN;                               /* don't lock time */
    }
    ii = AFNI_controller_index(im3d);           /* which one am I? */

    if (ii < 0)
    {
        EXRETURN;                               /* nobody? bad input! */
    }
    if (((1 << ii) & glock) == 0)
    {
        EXRETURN;                               /* input not locked */
    }
    /* something to do? */
     ...
     *   for those that ARE open, ARE NOT the current one, and ARE locked, jump to the new time
     *index */
...
void AFNI_space_lock_carryout(Three_D_View *im3d)
{
    ...
    /* loop through other controllers:
     *   for those that ARE open, ARE NOT the current one, and ARE locked, transform the above
     *vector to the controller's dataset, and then jump to that point */
...
void AFNI_set_all_thrlock_bboxes(Three_D_View *im3d, int bval)
{
    ...
    if (bval < 0)
    {
        bval = 1 << AFNI_thresh_lock_env_val();
    }
...
void AFNI_thresh_lock_carryout(Three_D_View *im3d)
{
    ...
    /* loop through other controllers:
     *   for those that ARE open, ARE NOT the current one, and ARE locked, set the new threshold */
...
void AFNI_equate_pbars(Three_D_View *lh3d, Three_D_View *rh3d)
{
    ...
    if (!IM3D_OPEN(lh3d) || !IM3D_OPEN(rh3d))
    {
        EXRETURN;
    }
    ...
    if (MCW_pbars_equivalent(lbar, rbar))
    {
        EXRETURN;                                    /* 07 Jul 2014 */
    }
    cc = AFNI_controller_index(lh3d); if (cc < 0)
    {
        EXRETURN;
    }
            ...
            if (qq3d->vinfo->func_thresh_top == im3d->vinfo->func_thresh_top)
            {
                    ...
                    DSET_BRICK_STATCODE(qq3d->fim_now, qq3d->vinfo->thr_index) > 0)
                {
                    tval = THD_pval_to_stat(pval,
                                            DSET_BRICK_STATCODE(qq3d->fim_now,
                                                                qq3d->vinfo->thr_index),
                                            DSET_BRICK_STATAUX(qq3d->fim_now,
                                                               qq3d->vinfo->thr_index));
...
{
    if (AFNI_yesenv("AFNI_RANGE_LOCK"))
    {
        return 1;
    }
    if (PBAR_FULLRANGE && AFNI_check_pbar_lock())
    {
        return 1;
    }
    return 0;
...
void AFNI_range_lock_carryout(Three_D_View *im3d)
{
    ...
    for (nn = cc = 0 ; cc < MAX_CONTROLLERS ; cc++)
    {
        ...
        if (qq3d != im3d && IM3D_OPEN(qq3d) && ((1 << cc) & glock) != 0)
        {
            nn++;
        }
    }
    ...
    for (cc = 0 ; cc < MAX_CONTROLLERS ; cc++)
    {
...

        {
            if (qq3d == im3d && /* no need to set current if not autoRanged */
                MCW_val_bbox(im3d->vwid->func->range_bbox) == 0)
            {
                continue;
            }
...
void AFNI_zoompan_lock_carryout(Three_D_View *im3d)
{
    ...
    if (!GLOBAL_library.zoompan_lock)
    {
        EXRETURN;
    }
    ...
    /* loop through other controllers:
     *   for those that ARE open, ARE NOT the current one, and ARE locked, set zoom and pan to match
