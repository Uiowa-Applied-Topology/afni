...
/*! Open an ANALYZE .hdr file as an unpopulated AFNI dataset. It will be populated from the .img
 * file, in THD_load_analyze().
 * -------------------------------------------------------------------*/
...
THD_3dim_dataset * THD_open_analyze(char *hname)
{
    ...
    ii = strlen(hname); if (ii < 5)
    {
        RETURN(NULL);
    }
    if (strcmp(hname + ii - 4, ".hdr") != 0)
    {
        RETURN(NULL);
    }
    ...
    dx = fabs(hdr.dime.pixdim[1]); if (dx == 0.0)
    {
        dx = 1.0;
    }
    dy = fabs(hdr.dime.pixdim[2]); if (dy == 0.0)
    {
        dy = 1.0;
    }
    dz = fabs(hdr.dime.pixdim[3]); if (dz == 0.0)
    {
        dz = 1.0;
    }
    dt = fabs(hdr.dime.pixdim[4]); if (dt == 0.0 || nt == 1)
    {
        dt = 1.0;
    }
    ...
    { short xyzuv[5], xx, yy, zz, aa, bb;
      ...
      if (doswap)
      {
          swap_2(&(xyzuv[0])); swap_2(&(xyzuv[1])); swap_2(&(xyzuv[2])); swap_2(&(xyzuv[3]));
          swap_2(&(xyzuv[4]));
      }
                                                   ...
                                                   * can be complete rubbish. Not reliable. */
      {
          ...
          if (xx > 0 && xx < nx - 1 &&
              ...
              zz > 0 && zz < nz - 1)
          {
              spmorg = 1; spmxx = xx - 1; spmyy = yy - 1; spmzz = zz - 1;
          }
    ...
    {                           /* ZSS Dec 15 03 */
        ...
        {
            static int nwarn = 0;
            if (strcmp(vie, "tlrc") == 0)
            {
                iview = VIEW_TALAIRACH_TYPE;
            }
            else if (strcmp(vie, "orig") == 0)
            {
                iview = VIEW_ORIGINAL_TYPE;
            }
            else if (strcmp(vie, "acpc") == 0)
            {
