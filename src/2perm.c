...
int main(int argc, char *argv[])
{
    ...
    while (iarg < argc && argv[iarg][0] == '-')
    {
        if (strcasecmp(argv[iarg], "-prefix") == 0)
        {
            ...
            {
                ERROR_exit("2perm: illegal prefix");
            }
            iarg++; continue;
        }
    ...
    if (arr == NULL)
    {
        ERROR_exit("2perm: bad inputs -- can't create subsets");
    }
    ar1 = arr[0]; ar2 = arr[1];
    ...
    if (fp == NULL)
    {
        ERROR_exit("2perm: Can't open file %s for output!", fnam);
    }
    if (docomma)
    {
        for (jj = 0 ; jj < n1 - 1 ; jj++)
