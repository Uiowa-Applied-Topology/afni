...
double norm_3shear(MCW_3shear sh)
{
    ...
    {
        jj  = sh.ax[ii];
        val = fabs(sh.scl[ii][(jj + 1) % 3]); if (val > top)
        {
            top = val;
        }
        val = fabs(sh.scl[ii][(jj + 2) % 3]); if (val > top)
        {
            top = val;
        }
    }
...
/*------------------------------------------------------------------------
 * Permute the axes of a shear structure so that what was in order (0,1,2) is now in order
 *(ox1,ox2,ox3). If P is the 3x3 matrix [ delta(i,ox{j+1}) ] (i,j=0,1,2), then
 *
 *                                      T [output shear] = P [input shear] P
 * --------------------------------------------------------------------------*/
...
MCW_3shear permute_3shear(MCW_3shear shin, int ox1, int ox2, int ox3)
{
    ...
    if (!ISVALID_3SHEAR(shin))
    {
        INVALIDATE_3SHEAR(shout); return shout;
    }
 ...
 *
 * where             [ f b c ]              [ 1 0 0 ]              [ 1 0 0 ] Sx[b,c,f] = [ 0 1 0 ]
 *Sy[a,c,f] = [ a f b ]  Sz[a,b,f] = [ 0 f 0 ] [ 0 0 0 ]              [ 0 0 1 ]              [ a b 1
 *]
 *
 * "f" is a stretching/shrinking factor applied with the 1D shear. If det[Q] = 1 , then f = 1 and
 *these are pure shears.
 *
 * In addition, a shift is allowed for, so that the total coordinate transformation being decomposed
 *is really
 *
 *     [ x ]          [ x ]     [ xdel ] [ y ]   =  [Q] [ y ]   + [ ydel ] [ z ]          [ z ] [
 * zdel ] new            old
 *
 * The output shifts are applied to the last three transformations, so that the factoring produces
 *this sequence of operations:
 *
 *     [ x ]           [ x ] [ y ]   = [Sx1] [ y ] [ z ]           [ z ] 1               old
 *
 *     [ x ]           [ x ]    [ 0  ] [ y ]   = [Sy]  [ y ]  + [ dy ] [ z ]           [ z ]    [ 0
 *  ] 2               1
 *
 *     [ x ]           [ x ]    [ 0  ] [ y ]   = [Sz]  [ y ]  + [ 0  ] [ z ]           [ z ]    [ dz
 * ] 3               2
 *
 *     [ x ]           [ x ]    [ dx ] [ y ]   = [Sx2] [ y ]  + [ 0  ] [ z ]           [ z ]    [ 0
 *  ] new             3
 *
 * The function returns a set of shears in the MCW_3shear struct type.
 *
 * The C code for generating the parameters (f,bx2,cx2,az,bz,ay,cy,bx1,cx1,dx,dy,cz) was generated
 *by the following Maple V Release 4 script:
 *
 ...
 *
 *  ee := { seq( SS[i,1]-QQ[i,1] , i=1..3 ) , seq( SS[i,2]-QQ[i,2] , i=1..3 ) , seq( SS[i,3]-QQ[i,3]
 *, i=1..3 ) , seq( SS[i,4]-QQ[i,4] , i=1..3 )  } :
 *
...
MCW_3shear shear_xzyx(THD_dmat33 *q, THD_dfvec3 *xyzdel)
{
    ...
    t107 = t11 * t36;
    t109 = t38 * t22;
    t113 = t86 * t87 - t57 * t90 + 2.0 * t50 * t92 + 2.0 * t37 * t94 + t96 * t22 * t3 + t96 * q31 *
           t8 - 3.0 *
           t24 - t96 * q22 * t102 + 3.0 * t30 + t27 - 2.0 * t36 * t26 * t3 - t19 - t62 * q32 *
           t107 - 2.0 * t37 * t109 - t26 *
           q21 * t64;
    t118 = q32 * q22;
    t119 = t118 * q11;
    t121 = q11 * t36;
    ...
    t145 = -q11 * t15 * t17 * q31 + t23 * t89 + t86 * t119 + t121 * t28 - t123 * t55 + t125 * t45 +
           t1 *
           t127 - t129 * t66 + t131 * t132 + t134 * q13 * q22 - t9 * t134 - 2.0 * t36 * t22 * t8 -
           t131 * t141 - t11 * t127 +
           2.0 * t47 * t125;
    ...
    t185 = xdel * q21;
    t190 = ydel * q11;
    t193 = -ydel * q22 * t51 * t26 - t157 * q23 * t134 - t160 * t129 * q33 + t164 * t119 + t163 *
           t54 -
           t167 * q21 * q22 * q12 + ydel * t134 * t3 + t157 * q21 * q33 * t26 - t167 * t6 - 3.0 *
           ydel * q21 * t75 * q32 +
           t167 * t8 + 3.0 * ydel * t15 * t17 * q22 * q31 + t185 * t20 * t26 - ydel * t16 * t18 -
           t190 * t28 * q31;
    t194 = zdel * q21;
    t195 = t125 * q12;
    t203 = xdel * t18;
    ...
    t233 = t194 * t195 + t194 * t22 * t12 + t160 * t94 - t194 * q32 * t7 * t22 + t203 * q31 * t15 -
           2.0 *
           t206 * t207 * q32 - t210 * t107 - t194 * t75 * q11 + t194 * q31 * t20 * q11 * t36 +
           t210 * t11 * q31 - t220 *
           t221 * q31 - t194 * t224 + t160 * t207 * q12 + ydel * t25 * t26 - t230 * t8 - t160 *
           t109;
    t238 = t194 * t36;
    t240 = ydel * t96;
    t241 = t240 * q21;
    ...
    t264 = -t203 * t36 * t15 + t230 * t12 + 2.0 * t238 * t61 - t241 * t141 - t240 * q22 * t102 +
           t220 *
           t221 * t36 - t160 * q31 * t87 + 2.0 * t206 * t36 * t29 * q32 - 2.0 * t252 * t22 * t8 +
           t164 * t87 + t190 * t36 *
           t17 * t15 - t240 * t67 - 2.0 * t252 * t224 + 2.0 * t252 * q22 * t92 + t241 * t132;
    t267 = t160 * t36;
    t269 = ydel * q31;
    t275 = t252 * q21;
    t292 = -2.0 * t238 * t67 + t240 * t69 + 2.0 * t267 * t132 - t269 * q32 * t90 - t185 * t36 *
           t20 * t22
           + 2.0 * t275 * t94 + t230 * t10 + 2.0 * t238 * t69 + 2.0 * t252 * t195 - t230 * t4 -
           2.0 * t275 * t109 - 2.0 *
           t267 * t141 - 2.0 * t238 * t70 + t240 * q31 * t8 - t269 * q21 * t118 * t121 + t160 *
           t96 * q13 * q32;
...
/*---------------------------------------------------------------------------------------
 * Decompose transformation (q,xyzdel) into a set of 4 shears, whose axis order is ox1,ox2,ox3,ox1;
 * that is, q = S    S    S    S ox1  ox3  ox2  ox1
 * -----------------------------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------------------
 * Find the "best" shear decomposition (smallest stretching factors). Input matrix q should have
 *det(q)=1.
 * -------------------------------------------------------------------------------------*/
...
#endif
        shr.ax[3]  = 0; shr.scl[3][0] = 1.0; shr.scl[3][1] = 0.0; shr.scl[3][2] = 0.0;
        shr.sft[3] = dx;
        shr.ax[2]  = 2; shr.scl[2][2] = 1.0; shr.scl[2][0] = 0.0; shr.scl[2][1] = 0.0;
        shr.sft[2] = dz;
        shr.ax[1]  = 1; shr.scl[1][1] = 1.0; shr.scl[1][0] = 0.0; shr.scl[1][2] = 0.0;
        shr.sft[1] = dy;
        shr.ax[0]  = 0; shr.scl[0][0] = 1.0; shr.scl[0][1] = 0.0; shr.scl[0][2] = 0.0;
        shr.sft[0] = 0.0;
...
/*--------------------------------------------------------------------------
 * Compute a set of shears to carry out a rotation + shift. The rotation is specified as the
 *composition of elementary rotations about 3 axes.
 * ----------------------------------------------------------------------------*/
                        ...
                        double xdel, double ydel, double zdel)
{
    ...
    if (DMAT_TRACE(q) < 1.0)
    {
        ...
        if (top < q.mat[1][1])
        {
            top = q.mat[1][1]; itop = 1;
        }
        if (top < q.mat[2][2])
        {
            top = q.mat[2][2]; itop = 2;
        }
        switch (itop)
        {
        default:
...
/*--------------------------------------------------------------------------
 * Compute a set of shears to carry out a rotation + shift (note the shift is always DELTA_AFTER in
 *this code).
 * ----------------------------------------------------------------------------*/
...
/*=========================================================================
 * Routines to compute the rotation+translation to best align a set of 3D points to one another --
 * RWCox - 16 Jul 2000.
 * ===========================================================================*/

/*---------------------------------------------------------------------
 * Compute        T [inmat]  [inmat]
 * -----------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------
 * Compute the eigensolution of the symmetric matrix inmat; that is, orthogonal [X] and diagonal [D]
 *such that
 *
 *      [inmat] [X] = [X] [D]
 * ----------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------
 * Compute                  pp [      T       ] [ inmat  inmat ]   for some power pp
 * -----------------------------------------------------------------------*/
...
THD_dmat33 DMAT_pow(THD_dmat33 inmat, double pp)
{
    ...
    if (pp == 0.0)
    {
        LOAD_DIAG_DMAT(out, 1, 1, 1); return out;
    }
...
/*---------------------------------------------------------------------
 * Compute the rotation         T [V] [U] from the matrix, where                     T [inmat] = [U]
 *[D] [V] is the SVD of the input matrix.  We actually calculate it as
 *
 *                   -1/2 [     T      ]           T [inmat  inmat]    [inmat]
 *
...
/*  Alternative calculation to above, computing U and V directly, as eigenmatrices from symmetric
 * eigenvalue problems. This avoids a problem that arises when inmat is singular.
 * -----------------------------------------------------------------------*/
...
/*  Yet another alternative calculation to above, computing U and V even more directly, from the SVD
 * itself.  [22 Oct 2004]
 * -----------------------------------------------------------------------*/
 ...
 *
 * true in the (weighted) least squares sense.  If ww == NULL, then weights of all 1 are used.
 *Method follows KS Arun, TS Huang, and SD Blostein, IEEE PAMI, 9:698-700, 1987 and uses the
 *routines above to compute the matrix [R].
 * -----------------------------------------------------------------------*/
...
THD_dvecmat DLSQ_rot_trans(int n, THD_dfvec3 *xx, THD_dfvec3 *yy, double *ww)
{
    ...
    ZZME(out);
    if (n < 3 || xx == NULL || yy == NULL)
    {
        LOAD_ZERO_DMAT(out.mm); return out;
    }
...
/*! Compute a transformation to take one set of vectors into another. That is, find matrix R and
 * vector B so that
 *
 *   yy = [R] xx + V   (k=0..n-1) k        k
 *
 *  is true in the unweighted least squares sense.  Here, we restrict R to be a scalar multiple of
 *an orthgonal matrix.
 * ----------------------------------------------------------------------------*/
