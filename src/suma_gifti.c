...
NI_group * afni_open_gifti_surf(char *fname, int read_data)
{
    ...
    if (GP->verb > 2)
    {
        fprintf(stderr, "-- NI_read_gifti from '%s'\n", fname);
    }
...
static NI_group *gifti_surf_to_afni_surf(gifti_image *gim)
{
    ...
    if (rai_convert)
    {
        ...
        {
            flip_float_triples((float *)nelxyz->vec[0], rows);
        }
    }
    ...
    if (dan)
    {
        ...
        if (rai_convert)
        {
            flip_float_triples((float *)nelnormals->vec[0], rows);
        }
    }
...
static gifti_image *afni_surf_to_gifti_surf(NI_group *aSO)
{
    ...
    /* basic step - create empty image (with a version string) from gifti_create_image */
    ...
    if (rai_convert)
    {
        flip_float_triples((float *)da->data, da->dims[0]);
    }
