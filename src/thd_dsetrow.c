...
int THD_get_dset_rowcount(THD_3dim_dataset *dset, int dcode)
{
    ...
    {
    case  1:
    case -1: return DSET_NX(dset);             /* good */

    case  2:
    case -2: return DSET_NY(dset);             /* good */

    case  3:
    case -3: return DSET_NZ(dset);             /* good */
    }
    return 0;                                  /* bad */
}
...
/*! Extract a row from a dataset sub-brick in the direction given by dcode. The 3-index of a voxel
 * from the row is in (xx,yy,zz).  The return value is a pointer to a malloc()-ed array, whose type
 * is given by DSET_BRICK_TYPE(dset,ival) and whose length is given by
 * THD_get_dset_rowcount(dset,dcode).  If NULL is returned, something bad happened. N.B.: dcode < 0
 * ==> data is extracted in the reverse direction.
 * -----------------------------------------------------------------------------*/
                             ...
                             int dcode, int xx, int yy, int zz)
{
    ...
    nrow = THD_get_dset_rowcount(dset, dcode);
    kind = DSET_BRICK_TYPE(dset, ival);
    fac  = DSET_BRICK_FACTOR(dset, ival); if (fac <= 0.0f)
    {
        fac = 1.0f;
    }
