...
int main(int argc, char *argv[])
{
    ...
    {
        Error_Exit("Cannot use ANALYZE dataset");
    }
    if (DSET_IS_1D(dset))
    {
        Error_Exit("Cannot use .1D dataset");
    }
    if (DSET_IS_3D(dset))
    {
        Error_Exit("Cannot use .3D dataset");
    }
    if (DSET_IS_CTFMRI(dset))
    {
        Error_Exit("Cannot use CTF dataset");
    }
    ...
    do {
        ...
        /* find the largest note to delete, since numbering for those > than deleted changes */
