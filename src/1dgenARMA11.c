 ...
 *    [ 1 lam lam*rho lam*rho^2 lam*rho^3 ... ] which is the ARMA(1,1) model with the AR parameter a
 * = rho, and the MA parameter b such that (b+a)*(1+a*b)/(1+2*a*b+b*b) = lam. For reasonable models
 * of FMRI noise, 0 < lam < rho < 0.9. The maximum bandwidth of the matrix is chosen so that the
 * last correlation element is about 0.01.
 * tau[i] is the 'true' time index of the i-th data point.  This lets you allow for censoring and
 *for inter-run gaps. If tau==NULL, tau[i] is taken to be i -- that is, no censoring/gaps.
 *//*------------------------------------------------------------------------*/
...
rcmat * rcmat_arma11(int nt, int *tau, MTYPE rho, MTYPE lam)
{
    ...
    if (rho > 0.99)
    {
        rho = 0.99;                      /* max allowed NN correlation */
    }
    else if (rho < -0.99)
    {
        rho = -0.99;
    ...
    for (ii = 1 ; ii < nt ; ii++)
    {
        ...
        jbot = ii - bmax; if (jbot < 0)
        {
            jbot = 0;                            /* earliest allow index */
        }
        for (jj = jbot ; jj < ii ; jj++)         /* scan to find bandwidth */
        {
            ...
            if (jtt <= bmax)
            {
                break;                          /* if in OK region, stop now */
            }
        }
        ...
        for (jj = jbot ; jj < ii ; jj++)          /* compute off diagonal entries */
        {
            ...
            if (jtt == 1)
            {
                rii[jj] = lam;                           /* lag==1 means lam */
            }
            else if (jtt > 1)
            {
                rii[jj] = lam * pow(rho, jtt - 1.0);
            }
        }
    }
...
/*! Setup sparse banded correlation matrix (as an rcmat struct) for a general shift-invariate
 * structure, whose lagged correlations are given in corvec.
 * tau[i] is the 'true' time index of the i-th data point.  This lets you allow for censoring and
 *for inter-run gaps. If tau==NULL, tau[i] is taken to be i -- that is, no censoring/gaps.
 *//*------------------------------------------------------------------------*/
                     ...
                     double a, double r1, double t1, double vrt)
{
    ...
    { int ii; double ssum = 0.0;
      ...
      {
          fprintf(stderr, " %g", corvec->ar[ii]); if (ii > 0)
          {
              ssum += fabs(corvec->ar[ii]);
          }
      }
...
int main(int argc, char *argv[])
{
    ...
    while (iarg < argc)
    {
        if (strcasecmp(argv[iarg], "-ARMA31") == 0) /* 01 Jul 2020 */
        {
            if (iarg + 4 >= argc)
            {
                ERROR_exit("Need 4 arguments after option '%s'", argv[iarg]);
            }
        ...
        if (strcasecmp(argv[iarg], "-ARMA51") == 0) /* 01 Jul 2020 */
        {
            if (iarg + 6 >= argc)
            {
                ERROR_exit("Need 6 arguments after option '%s'", argv[iarg]);
            }
        ...
        if (strcmp(argv[iarg], "-num") == 0 || strcmp(argv[iarg], "-len") == 0)
        {
            ...
            if (nlen < 3)
            {
                ERROR_exit("Time series length must be at least 3!");
            }
            iarg++; continue;
        }
        ...
        if (strcmp(argv[iarg], "-nvec") == 0)
        {
            ...
            if (nvec < 1)
            {
                ERROR_exit("Number of vectors must be at least 1!");
            }
            iarg++; continue;
        }
        ...
        if (strcmp(argv[iarg], "-sig") == 0)
        {
            ...
            if (sig <= 0.0)
            {
                ERROR_exit("Value after '-sig' should be positive!");
            }
            iarg++; continue;
        }
        ...
        if (strcmp(argv[iarg], "-a") == 0)
        {
            ...
            if (fabs(aa) >= 0.95)
            {
                ERROR_exit("Largest allowed value of a = 0.95");
            }
            iarg++; continue;
        }
        ...
        if (strcmp(argv[iarg], "-b") == 0)
        {
            ...
            if (fabs(bb) >= 0.95)
            {
                ERROR_exit("Largest allowed value of b = 0.95");
            }
            iarg++; continue;
        }
        ...
        if (strcmp(argv[iarg], "-lam") == 0)
        {
            ...
            if (fabs(lam) >= 0.95)
            {
                ERROR_exit("Largest allowed value of lam = 0.95");
            }
            iarg++; continue;
        }
        ...
        if (strcmp(argv[iarg], "-CORcut") == 0)
        {
            ...
            if (corcut <= 0.0 || corcut > 0.1)
            {
                ERROR_exit("Illegal value after '-CORcut'");
            }
            iarg++; continue;
        }
    ...
    if (nlen <= 3)
    {
        ERROR_message("Didn't you give the -num/-len option?"); nbad++;
    }
    ...
    if (do_arma11)
    {
        if (aa <= -1.0)
        {
            ERROR_message("Didn't you give the -a option?"); nbad++;
        }
        if (bb <= -1.0 && lam <= -1.0)
        {
        ...
        if (nbad > 0)
        {
            ERROR_exit("Can't continue past the above problem%s",
                       (nbad == 1)? "\0" : "s");
        }
    ...
    if (rcm == NULL)
    {
        ERROR_exit("Can't setup matrix?!");                /* should be impossible */
    }
    kk = rcmat_choleski(rcm);
    if (kk > 0)
    {
        ERROR_exit("Choleski fails at row %d", kk);
    }
