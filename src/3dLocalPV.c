...
int main(int argc, char *argv[])
{
    ...
    while (iarg < argc && argv[iarg][0] == '-')
    {
        ...
        if (strcmp(argv[iarg], "-prefix2") == 0)
        {
            if (++iarg >= argc)
            {
                ERROR_exit("Need argument after '-prefix2'");
            }
        ...
        if (strcmp(argv[iarg], "-evprefix") == 0)
        {
            if (++iarg >= argc)
            {
                ERROR_exit("Need argument after '-evprefix'");
            }
...
#pragma omp critical
          { if (vstep && kk % vstep == vstep - 1)
         {
          ...
          if (do_vnorm)
          {
              ...
              {
                  tsar = nbar + nt * vv;
                  for (sum = 0.0f, ii = 0 ; ii < nt ; ii++)
                  {
                      sum += tsar[ii] * tsar[ii];
                  }
                  if (sum > 0.0f)
                  {
                      for (sum = 1.0f / sqrtf(sum), ii = 0 ; ii < nt ; ii++)
                      {
                          tsar[ii] *= sum;
                      }
                  }
