...
int main(int argc, char *argv[])
{
    ...
    while (iarg < argc && argv[iarg][0] == '-')
    {
        ...
        if (strcmp(argv[iarg], "-polort") == 0)
        {
            ...
            if (++iarg >= argc)
            {
                ERROR_exit("Need argument after '-polort'");
            }
...
#pragma omp parallel if( nmask > 666 )  /* parallelization 16 Jul 2009 [disabled] */
    { int        kk, xx, yy, zz, vv, ii;
      ...
      if (rebase)
      {
          coef = (float *)malloc(sizeof(float) * (polort + 1));
      }
      else
      {
          coef = NULL;
      ...
      for (kk = 0 ; kk < nxyz ; kk++)
      {
          ...
          if (imar == NULL)
          {
              if (!use_nonmask)
              {
                  ERROR_message("get failure #%d", kk);
              }
              continue;
          }
          if (polort >= 0)
          ...
          if (domean)
          {
              ...
              if (pim == NULL)
              {
                  ERROR_message("mav failure #%d", kk); continue;
              }
          }
          else
          {
              ...
              if (pim == NULL)
              {
                  ERROR_message("mpv failure #%d", kk); continue;
              }
          }
          DESTROY_IMARR(imar);
                                    ...
                                    int xx, int yy, int zz, MCW_cluster *nbhd)
{
    ...
    kk = xx + yy * nx + zz * nxy; if (kk < 0 || kk >= nxyz)
    {
        return NULL;
    }
    ...
    if (nvox == 0)
    {
        free(ivox); return NULL;
    }                                             /* no voxels to extract */
...
MRI_IMAGE * mri_principal_vector(MRI_IMARR *imar)
{
    ...
    nvec = IMARR_COUNT(imar);       if (nvec < 1)
    {
        return NULL;
    }
    nx = IMARR_SUBIM(imar, 0)->nx; if (nx < 1)
    {
        return NULL;
    }
...
#define X(i, j)    amat[(i) + (j) * nvec] /* nvec X nx matrix */
    ...
    nev = nx * nvec;
    for (jj = 1 ; jj < nev && amat[jj] == amat[0] ; jj++)
    {
        ;                                                  /*nada*/
    }
    if (jj == nev)
    ...
    if (jj <= 0)
    {
        free(sval); free(umat); free(amat); return NULL;
    }
    nev = jj;
