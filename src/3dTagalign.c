...
int main(int argc, char *argv[])
{
    ...
    while (iarg < argc && argv[iarg][0] == '-')
    {
        ...
        if (strcmp(argv[iarg], "-master") == 0)
        {
            if (mset != NULL)
            {
                ERREX("Can only have one -master option");
            }
            if (++iarg >= argc)
            {
                ERREX("Need an argument after -master");
            }
            ...
            if (mset == NULL)
            {
                ERREX("Can't open -master dataset");
            }
            if (mset->tagset == NULL)
            {
                ERREX("No tags in -master dataset");
            }
            if (TAGLIST_COUNT(mset->tagset) < 3)
            {
                ERREX("Not enough tags in -master dataset");
            }
            ...
            if (nvec < 3)
            {
                ERREX("Not enough tags set in -master dataset");
            }
            ...
            if (verb)
            {
                fprintf(stderr, "++ Found %d tags in -master dataset\n", nvec);
            }
        ...
        if (strcmp(argv[iarg], "-wtval") == 0)
        {
            if (ww != NULL)
            {
                ERREX("Can't have -wtval after -wt1D");
            }
            wtval++;
            iarg++; continue;
        }
        ...
        if (strcmp(argv[iarg], "-wt1D") == 0)
        {
            ...
            if (wtval)
            {
                ERREX("Can't have -wt1D after -wtval");
            }
            if (ww != NULL)
            {
                ERREX("Can't have two -wt1D options!");
            }
            if (++iarg >= argc)
            {
                ERREX("Need an argument after -wt1D");
            }
            ...
            if (wtim == NULL)
            {
                ERREX("Can't read -wtim file");
            }
            if (wtim->ny > 1)
            {
                ERREX("-wtim file has more than one column");
            }
            ...
            for (ii = 0 ; ii < nww ; ii++)
            {
                ...
                if (ww[ii] < 0.0)
                {
                    ERREX("Negative value found in -wt1D file");
                }
            }
        ...
        if (strcmp(argv[iarg], "-prefix") == 0)
        {
            if (++iarg >= argc)
            {
                ERREX("Need an argument after -prefix");
            }
            prefix = argv[iarg];
            if (!THD_filename_ok(prefix))
            {
                ERREX("-prefix string is illegal");
            }
            iarg++; continue;
        }
        ...
        if (strcmp(argv[iarg], "-matvec") == 0)
        {
            if (++iarg >= argc)
            {
                ERREX("Need an argument after -matvec");
            }
            mfile = argv[iarg];
            if (!THD_filename_ok(mfile))
            {
                ERREX("-matvec string is illegal");
            }
            iarg++; continue;
        }
        ...
        if (strcmp(argv[iarg], "-tagset") == 0)
        {
            if (++iarg >= argc)
            {
                ERREX("Need an argument after -tagset");
            }
            tfile = argv[iarg];
            if (!THD_filename_ok(tfile))
            {
                ERREX("-tagset string is illegal");
            }

            if (mytagset != NULL)
            {
                ERREX("Can only have one -tagset option");
            }
            mytagset = (THD_usertaglist *)calloc(1, sizeof(THD_usertaglist));
            if (!mytagset)
            {
                ERREX("insufficient RAM for taglist");
            }
            TAG_read(tfile);
    ...
    if (mset == NULL)
    {
        ERREX("No -master option found on command line");
    }

#if 0
    if (ww != NULL && nww < nvec)
    {
        ERREX("Not enough weights found in -wt1D file");
    }
    ...
    if (wtval)
    {
        ...
        for (ii = jj = 0 ; ii < TAGLIST_COUNT(mset->tagset) ; ii++)
        {
            if (TAG_SET(TAGLIST_SUBTAG(mset->tagset, ii)))
            {
                ...
                if (ww[jj] < 0.0)
                {
                    ERREX("Negative value found in -master tag values");
                }
                jj++;
            }
        }
    ...
    if (dset == NULL)
    {
        ERREX("Can't open input dataset");
    }
    ...
    if (dset->tagset == NULL)
    {
        ERREX("No tags in input dataset");
    }
    ...
    /*-- 21 Apr 2003: transformation can be done the old way (a la 3drotate), or the new way (a la
     * 3dWarp).                          --*/

#if 0
    if (!use_3dWarp)           /**** the old way ****/

    /*-- now must scramble the rotation matrix and translation vector from Dicom coordinate order to
     * dataset brick order --*/

    {
        ...
        /*-- now create the output dataset by screwing with the input dataset (this code is adapted
         * from 3drotate.c)                           --*/
        ...
        if (verb)
        {
            fprintf(stderr, "++ computing output BRIK");
        }
    ...
    {  /**** the new way: use 3dWarp type transformation ****/
        ...
        if (keeptags)
        {
            ...
            if (!oset->tagset)
            {
                ERREX("insufficient RAM to copy taglist");
            }
...
/*---------------------------------------------------------------------
 * This produces a permutation-like matrix that transforms from brick axis coordinates to Dicom
 *order coordinates.
 * -----------------------------------------------------------------------*/
...
static void TAG_read(char *str)           /* copied from plug_tag.c */
{
    ...
    if (cpt == NULL)
    {
        ...
        if (!cpt)
        {
            ERREX("insufficient RAM to append .tag to tag file name");
        }
        strcpy(cpt, str);
        if (cpt[ii - 1] != '.')
        {
    ...
    while (ntog < MAX_TAG_NUM)
    {
        ...
        jj   = 0;
        while (jj < lbuf && isspace(buf[jj]))
        {
            jj++;
        }
        if (jj == lbuf)
        {
            continue;                     /* was a blank line => skip */
        }
        if (buf[jj] == '#')
        {
            continue;                     /* comment */
