...
/*! Input is a dataset axes struct and an orientation code. Output is an int saying which dataset
 * axis is this code.
 *   -  +1 => dataset +x-axis
...
int THD_get_axis_direction(THD_dataxes *daxes, int orient_code)
{
    ...
    if (daxes->xxorient == orient_code)
    {
        return 1;
    }
    if (ORIENT_OPPOSITE(daxes->xxorient) == orient_code)
    {
        return -1;
    }
    if (daxes->yyorient == orient_code)
    {
        return 2;
    }
    if (ORIENT_OPPOSITE(daxes->yyorient) == orient_code)
    {
        return -2;
    }
    if (daxes->zzorient == orient_code)
    {
        return 3;
    }
    if (ORIENT_OPPOSITE(daxes->zzorient) == orient_code)
    {
        return -3;
    }
    return 0;
 ...
 *    - return 0 on success, 1 on error (ostr is just char *, the 4 is a reminder)   23 Jan 2013
 *[rickr] */
 ...
 *    - return 0 on success, 1 on error (ostr is just char *, the 7 is a reminder)   23 Jan 2013
 *[rickr] */
