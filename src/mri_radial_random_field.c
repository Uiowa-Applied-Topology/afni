...
/*** This file is intended to be #include-d into another source file, in particular into
 * 3dClustSim.c -- for optimization and OpenMP-ization ***/
...
static float rfunc_falsi_step(float *parm, float val, float rlo, float rhi)
{
    ...
    if (dv == 0.0f || fabsf(dv) < 0.005f * (fabsf(val - v0) + fabsf(val - v1)))
    {
        return rlo;
    }
...
/* What it says. The dimensions are assumed to be compatible with csfft_cox(). The array tar, if not
 * NULL, must be at least as long as MAX(ny,nz).
 *//*--------------------------------------------------------------------------*/
...
static void mri_fft3D_inplace(MRI_IMAGE *fim, complex *tar)
{
    ...
    if (fim == NULL || fim->kind != MRI_complex)
    {
        return;
    }
    ...
    CFFT_setup;  /* for the OMP version of csfft, which allows csfft_cox_OMP to be called inside a
                  * thread */
    ...
    for (jj = 0 ; jj < ny ; jj++)
    {
        for (ii = 0 ; ii < nx ; ii++)
        {
            ...
            for (kk = 0 ; kk < nz ; kk++)
            {
                qar[kk] = far[qq + kk * nxy];
            }
            CFFT(-1, nz, qar);
            for (kk = 0 ; kk < nz ; kk++)
            {
                far[qq + kk * nxy] = qar[kk];
            }
        }
    }
...
/* Given a noise field size and the kernel function parameters, determine the grid size for the FFTs
 * to perform, including some buffering for edge effects of the kernel function. Output grid sizes
 * are even and compatible with csfft_cox().
 *//*--------------------------------------------------------------------------*/
...
/* Create the kernel smoothing function, then FFT it, and clip/shrink it as far as reasonable.  The
 * basic method for simulating the noise random field is then (1) create an iid N(0,1) Gaussian
 * random field in FFT space (2) multiply this field by the radial weight function from here (3) FFT
 * to real space (4) [in 3dClustSim] truncate back to desired 3D grid and normalize the standard
 * deviation This method relies on the fact that the Fourier transform of white noise is also white
 * noise (if this isn't obvious to you, do the algebra).
 *
 * Note that nx,ny,nz must be compatible with csfft_cox(), which will be easiest if
 *get_random_field_size() is used to set them.
 *//*--------------------------------------------------------------------------*/
                                      ...
                                      float *parm)
{
    ...
    int        nxy = nx * ny, ii, jj, kk, nxh = nx / 2, nyh = ny / 2, nzh = nz / 2,
               nxyh = nxh * nyh;
    ...
    /* note we compute sqrt(FFT(wim)) since we are creating the weight for the noise
...
/* Create TWO instances of a random field. Result has 0 mean but stdev depends on what wtim
 * contains.
 *   nx,ny,nz = dimensions of output wtim     = weight image in FFT space tar      = FFT workspace -
 * at least MAX(ny,nz) long (or NULL) xran     = random seed vector Why TWO? It is easiest to just
 *fill the FFT space with the complex random Gaussians, then FFT to real space -- giving us a
 *complex random field with the desired ACF(r).  Then in 3dClustSim, we'll just use this pair of
 *results alternately.
 *//*--------------------------------------------------------------------------*/
...
#undef  CXRAN
#define CXRAN(i, j, k)                                                 \
        (zr = ww * zgaussian_sss(xran), zi = ww * zgaussian_sss(xran), \
         CXAR(i, j, k) = CMPLX(zr, zi))
