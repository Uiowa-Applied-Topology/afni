...
/*** modified January 1994 to include FIM-like "ignore" locations in reference time series.  The
 * mechanism is to use a weighted inner product everywhere -- now, the weights are 0 or 1, but can
...
void main(int argc, char *argv[])
{
    ...
    do {
        ...
        if (tempim == NULL)
        {
            exit(1);                       /* check for errors on read */
        }
        if (kk >= nimax)
        {
