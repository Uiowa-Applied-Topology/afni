 ...
 *   menu_name = label for menu nbrik     = number of values returned by user_func user_func =
 * function to call for each data time series user_data = extra data to go to user_func
 *
 * void user_func( int n, float *ts, void *user_data, int nbrik, void *v )
 *
 *   n         = length of time series ts[]      = time series data user_data = whatever you want
 *(same as above) nbrik     = same as above val       = (float *) v = output array of values val[]
 *     = user_func should fill val[0..nbrik-1] with values to save (presumably computed from ts[])
 *
 * user_func should not destroy the data in ts[], since it will be reused if more than one fimfunc
 *is used at a time.
 *
 * Before the first call with time series data, user_func will be called like so:
 *   user_func( n,NULL,user_data,nbrik, FIMdata *fd ) ;
 * where FIMdata is the type typedef struct {
 *      MRI_IMAGE *ref_ts , *ort_ts ;
 *      int nvox , ignore , polort ;
 *   } FIMdata ;
 * and fd is used to pass in the parameters set by the user:
 *    fd->ref_ts = float image of reference ("ideal") time series fd->ref_ts->nx = length of data
 *fd->ref_ts->ny = number of ref time series MRI_FLOAT_PTR(fd->ref_ts) = pointer to ref timeseries
 *data fd->ort_ts = similarly for the orts (but this may be NULL) fd->nvox   = number of voxels that
 *will be passed in
 *               = number of "normal" calls to user_data fd->ignore = ignore setting (but ignored
 *data is included in ts[]) fd->polort = polort setting (but no detrending is done to ts[]) N.B.:
 * FIMdata is typedef-ed in afni.h.
 *
 * After the last (nvox-th) time series is processed, user_func will be called one last time, with
 *the form user_func( -k,NULL,user_data,nbrik, dset ) ;
 * where dset is a pointer to the dataset (THD_3dim_dataset *) that has just been constructed.  The
 *value k (negative of the first argument) will be the index of the first sub-brick in the dataset
 * - it contains the data from val[0].  If desired, you can use this final call to set sub-brick
 *parameters for sub-bricks k through k+nbrik-1 (for val[0] .. val[nbrik-1]).
 *
 * Both special calls have the second argument NULL, which is how user_func can distinguish them
 *from a normal call with timeseries data in ts[].
 *
 * The initialization call will have the first argument be the number of points in the timeseries to
 *come ('n') - this will be positive.  The final call will have the first argument be the negative
 *of the sub-brick index of the first sub-brick created by this routine.  This value will be
 *non-positive (will be 0 or less), which is how the initialization and final calls can be
 *distinguished.  In each of these cases, the last argument must be properly cast to the correct
 *pointer type before being used.
 *
 * A sample fimfunc for the Spearman rank correlation, spearman_fimfunc(), is given at the end of
 *this file.
 * --------------------------------------------------------------------------------*/
                           ...
                           generic_func *user_func, void *user_data)
{
    ...
    if (menu_name == NULL || menu_name[0] == '\0' ||
        nbrik <= 0 || user_func == NULL)
    {
        EXRETURN;                                               /* bad inputs! */
    }
    if (num == sizeof(int))
    {
        EXRETURN;                       /* too many already! */
    }
    if (num == 0)
    {
                                    ...
                                    int nr, float *rv, float **r)
{
    ...
    if (nr == 1)
    {
        return spearman_rank_corr(n, x, rv[0], r[0]);
    }

    xv = spearman_rank_prepare(n, x); if (xv <= 0.0)
    {
        return 0.0;
    }
                               ...
                               int nr, float *rv, float **r)
{
    ...
    if (nr == 1)
    {
        return quadrant_corr(n, x, rv[0], r[0]);
    }

    xv = quadrant_corr_prepare(n, x); if (xv <= 0.0)
    {
        return 0.0;
    }
...
void spearman_fimfunc(int n, float *ts, void *ud, int nb, void *val)
{
    ...
    if (ts == NULL)
    {
        ...
        if (n > 0)
        {
            ...
            for (nkeep = 0, ii = ignore ; ii < ntsc ; ii++) /* for each time point */
            {
                ...
                if (kk == nref)
                {
                    keep[nkeep++] = ii;               /* mark if all are OK */
                }
            }
    ...
    if (ncall % 1000 == 0)
    {
        fprintf(stderr, "spearman_fimfunc: ncall=%d\n", ncall);
    }
#endif
    ...
    if (nkeep < ntsc)
    {
        for (ii = 0 ; ii < nkeep ; ii++)
        {
            tsc[ii] = ts[keep[ii]];                           /* the hard way */
        }
    }
    else
    {
...
void quadrant_fimfunc(int n, float *ts, void *ud, int nb, void *val)
{
    ...
    if (ncall % 1000 == 0)
    {
        fprintf(stderr, "quadrant_fimfunc: ncall=%d\n", ncall);
    }
#endif
