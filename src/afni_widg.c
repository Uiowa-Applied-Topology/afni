...
/** if USE_OPTMENUS is defined, then option menus will be used in place of MCW_arrowvals wherever
 * possible **/
...
/*---------------------------------------------------------------------
 * Make all the rest of the widgets for a Three_D_View (after the toplevel shell has been created)
 * -----------------------------------------------------------------------*/
...
static char *AFNI_perc_bbox_label[1]  = { "%" };
...
#ifdef USING_LESSTIF
    /* In Lesstif, using form spacing, shifts the top left corner in unsightly ways. Little spacing
     * looks better    Lesstif Patrol Jan 09*/
...
#define RGB_TO_PIXMAP(data, pnam)                                               \
      ...
      if (afni16_pixmap[num_entry - 1] == XmUNSPECIFIED_PIXMAP && AFNI_yesenv("AFNI_LOGO16"))
      {
          ...
          if (ic < 0)
          {
              ic = DC_find_closest_overlay_color(im3d->dc, getenv("AFNI_LOGO16_FOREGROUND"));
          }
          if (ic < 0)
          {
              ic = DC_find_closest_overlay_color(im3d->dc, fgn[(num_entry - 1) % 7]);
          }
          if (ic >= 0)
          {
              fg16 = im3d->dc->ovc->pix_ov[ic];
          }
          ...
          if (ic < 0)
          {
              ic = DC_find_closest_overlay_color(im3d->dc, getenv("AFNI_LOGO16_BACKGROUND"));
          }
#if 0
          if (ic < 0)
          {
              ic = im3d->dc->ovc->ov_darkest;
          }
#endif
          if (ic >= 0)
          {
              bg16 = im3d->dc->ovc->pix_ov[ic];
          }
...
/***
 * XtAddCallback( imag->popmenu , XmNunmapCallback , AFNI_imag_pop_CB , im3d ) ;
 ***/
    ...
    /*--- create button to Move crosshairs to position in atlas, formerly "Talairach to" ---*/
...
/*      if( TT_retrieve_atlas_dset("TT_Daemon",0) ){ */
    ...
    /**** 1 Aug 1996:
     *    put crosshair gap into a horizontal rowcol so can attach a toggle for periodic montages
     *here, too ****/
...
#define zxy_3DIM    "Coronal "
    ...
    if (AFNI_yesenv("AFNI_SEE_OVERLAY"))
    {
...
void AFNI_make_wid2(Three_D_View *im3d)
{
    ...
    /*----- create radio buttons in pairs, one in the rowcol and one in the popmenu;
     *      N.B.: they are created unmanaged and will be managed later, depending on defined marks
     *for the view -----*/
    ...
    { int       ib;
      ...
      for (ib = 0 ; ib < MARKS_MAXNUM ; ib++)
      {
          ...
          /* we set XmNindicatorSize resource for the popup to same as the menu above, since menu
           * default size is tiny and looks bad! */
...
#define SELECTOR_HEIGHT    240
    ...
    { static char *onofflabel[]    = { "Use Threshold?" };
      ...
      MCW_set_widget_bg(func->thr_pvalue_pb, PVALUE_COLOR, 0); } /*---- end of thr_menu creation for
                                                                  * top of threshold slider ----*/
...
#define SCALE_EXTRA    66
    ...
    { Widget qqq; int iqqq;
      ...
      { char *eee = getenv("AFNI_THRESH_BIGSTEP");    /* 09 May 2003 */
        if (eee != NULL)
        {
            iqqq = strtol(eee, NULL, 10); if (iqqq > 0)
            {
                sstep = iqqq;
            }
        }
...
#define PVAL_POPUP
    ...
    { float pmin, pmax;   /* posfunc added 3/21/95 */
      ...
      if (im3d->dc->visual_class == TrueColor)
      {
          ...
          if (eee == NULL)
          {
              eee = getenv("AFNI_COLOR_SCALE_DEFAULT");
          }
...
#define VEDIT_NOPT       (VEDIT_LAST_VALUE + 1)
    ...
    func->perc_bbox->parent = (XtPointer)im3d;

    MCW_set_bbox(func->perc_bbox,
    ...
    /* 25 Jul 2001: a toggle box to show the Atlas regions set by AFNI_ATLAS_COLORS */
...
/***
 *    XtAddCallback( prog->hidden_menu , XmNunmapCallback , AFNI_hidden_CB , im3d ) ;
 ***/

/***
 *    XtVaSetValues( prog->hidden_menu , XmNspacing      , 0 , XmNmarginHeight , 0 , XmNmarginWidth
 *, 0 , NULL ) ;
 ***/
...
void AFNI_redraw_controller(Three_D_View *im3d)
{
    ...
    if (im3d->g123)
    {
        forceExpose(im3d->g123->fdw_graph, 0);
    }
    if (im3d->g231)
    {
        forceExpose(im3d->g231->fdw_graph, 0);
    }
    if (im3d->g312)
    {
        forceExpose(im3d->g312->fdw_graph, 0);
    }
#else
    /* this code is more brute force, but does work if forceExpose does not */
    if (im3d->g123)
    {
        GRA_handle_keypress(im3d->g123, " ", NULL);
    }
    if (im3d->g231)
    {
        GRA_handle_keypress(im3d->g231, " ", NULL);
    }
    if (im3d->g312)
    {
        GRA_handle_keypress(im3d->g312, " ", NULL);
    }
#endif
...
int AFNI_controller_index(Three_D_View *im3d)
{
    ...
    for (ii = 0 ; ii < MAX_CONTROLLERS ; ii++)
    {
        if (GLOBAL_library.controllers[ii] == im3d)
        {
            RETURN(ii);
        }
    }
...
/*---------------------------------------------------------------------------
 * Create and return a new controller for AFNI. Inputs:
 * "shell" is a top level shell, to contain the entire widget hierarchy. If this shell is NULL, then
 *the routine will create it. Normally, you would pass in the results of XtVaAppInitialize as the
 *first controller's shell, and let the routine create any subsequent shells needed.
 * "dc" must be created by MCW_new_DC before this routine.  It contains all the information
 *pertaining to the X11 display.
 * "im3d_type" is one of AFNI_3DDATA_VIEW or AFNI_IMAGES_VIEW.  The former is for viewing 3D
 *datasets; the latter is a restricted version for viewing images only (mostly obsolete now -- use
 *program aiv instead).
 * -----------------------------------------------------------------------------*/
...
static float DSET_bigositiness(THD_3dim_dataset *dset)   /* 07 Dec 2001 */
{
    ...
    if (DSET_COMPRESSED(dset) || DSET_IS_MINC(dset))
    {
        bb *= 1.666f;
    }
...
void AFNI_initialize_controller(Three_D_View *im3d)
{
    ...
    if (im3d->brand_new && sss)
    {
        ...
        case 1:
            ...
            if (mm < 1.e+33)
            {
                im3d->vinfo->func_num = im3d->vinfo->anat_num = jb;
            }
            break;
        }
    }
    else if (im3d->brand_new)     /* 29 Jul 2003 */
    {
        ...
        if (jj < im3d->ss_now->num_dsset)
        {
            im3d->vinfo->anat_num = jj;
        }
    ...
    /*--- 11/23/94 addition: scan for a good view in the initial setup (to allow for input of
     * datasets w/o +orig view) */

    for (ii = 0 ; ii <= LAST_VIEW_TYPE ; ii++)
    {
        if (ISVALID_DSET(im3d->anat_dset[ii]))
        {
            break;
        }
    }
...
/*      im3d->fim_dset[ii] = im3d->ss_now->dsset_xform_table[im3d->vinfo->func_num][ii] ;*/
    ...
    if (ii <= 0)
    {
        ii = (int)AFNI_numenv("AFNI_INDEX_STRIDE");
    }
    if (ii > 0)
    {
        AFNI_time_index_set_fstep(im3d, ii);
    }
...
/*---------------------------------------------------------------------------
 * 23 Sep 2000: A hack to allow the creation of a specific controller, instead of letting
 *AFNI_clone_controller_CB choose the index.
 * -----------------------------------------------------------------------------*/
...
void AFNI_make_controller(int cii)
{
    ...
    if (cii < 0 || cii >= MAX_CONTROLLERS ||
        IM3D_VALID(GLOBAL_library.controllers[cii]))
    {
        EXRETURN;
    }
...
void AFNI_clone_controller_CB(Widget wcall, XtPointer cd, XtPointer cbs)
{
    ...
    /** 23 Sep 2000: if cii_override is set, use that for the index of the controller to create **/
    ...
    if (cii_override < 0)
    {
        ...
        for (ii = 0 ; ii < MAX_CONTROLLERS ; ii++)
        {
            ...
            if (IM3D_VALID(im3d) && !im3d->opened)
            {
                break;                                     /* found it */
            }
        }
...
/*-------------------------------------------------------------------------
 * 04 Nov 1996: make a menubar to control the coordinate locking of the various controller windows
...
void AFNI_vedit_CB(MCW_arrowval *av, XtPointer cd)
{
    ...
    if (!im3d->vwid->imag->pop_instacorr_pb ||
        !im3d->vwid->imag->pop_icorrjump_pb)   /*This happens when running afni with -im option ZSS
                                                * Aug 31 09*/
    {
        EXRETURN;
    }
...
/*   if (!AFNI_yesenv("AFNI_CMAP_AUTO")) RETURN(0);*/
    ...
    if (AFNI_noenv("AFNI_CMAP_AUTO") || AFNI_noenv("AFNI_PBAR_AUTO"))
    {
        RETURN(0);
    }
    im3d = (Three_D_View *)vp_im3d;
 ...
 */     }
        ...
        /* Problem is that when you switch back to a non  ROI dset, you are stuck with the ROI deal.
         * Perhaps one should bite the bullet and create a structure that preserves the last
         * colormap setup for a dset. Hmmm, got to discuss this with Bob, Daniel, and Rick */
    }
                                        ...
                                        "ATLAS_LABEL_TABLE")))
    {
        ...
        {
            /* usually atlases don't have a pbar_name, but it does not hurt to check, allowing for
             * its potential use someday */
 ...
 */         }
    ...
    {
        /*
         * Here one could guess at the moment (See is_integral_dset). But that can be time consuming
         *if integer data are stored as float. It is better to have the dataset flagged by a special
         *type in the header.
         */
...
/*---------------------------------------------------------------------------
 * A set of functions to handle thresholding based on percentile of threshold sub-brick.
 * -----------------------------------------------------------------------------*/
...
void flush_3Dview_sort(Three_D_View *im3d, char *sel)  /* ZSS April 26 2012 */
{
    if (im3d && im3d->vinfo)
    {
        flush_vinfo_sort(im3d->vinfo, sel);
    }
    return;
}
...
float get_3Dview_func_thresh(Three_D_View *im3d, int apply_power)
{
    ...
    if ((im3d->cont_perc_thr) && DSET_ONDISK(im3d->fim_now))
    {
         ...
         *        "or when a new on the fly set is computed as in Insta stuff.\n"
         *        "Perc. %f, thresh now: %f\n", im3d->fim_now, im3d->vinfo->thr_index,
         *im3d->vinfo->func_threshold,thresh ); */
    }
    else
    {
...
void reset_mnito(struct Three_D_View *im3d)
{
    ...
    if (!im3d->vwid->imag->pop_mnito_pb)
    {
        return;
    }
