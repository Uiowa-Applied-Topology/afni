...
void THD_automask_set_gradualize(int n)
{
    gradualize = n;
}
...
static int cheapo = 0;
void THD_automask_set_cheapo(int n)
{
    cheapo = n;
}                                                    /* 13 Aug 2007 */
...
void THD_automask_set_onlypos(int n)
{
    onlypos = n;
}                                                      /* 09 Nov 2020 */
...
int mask_intersect_count(int nvox, byte *mmm, byte *nnn)
{
    ...
    if (nvox <= 0 || mmm == NULL || nnn == NULL)
    {
        return 0;
    }
    for (nint = ii = 0 ; ii < nvox ; ii++)
    {
        nint += (mmm[ii] && nnn[ii]);
    }
    return nint;
...
int mask_union_count(int nvox, byte *mmm, byte *nnn)
{
    ...
    if (mmm == NULL && nnn != NULL)
    {
        return mask_count(nvox, nnn);
    }
    if (mmm != NULL && nnn == NULL)
    {
        return mask_count(nvox, mmm);
    }
    for (nint = ii = 0 ; ii < nvox ; ii++)
    {
        nint += (mmm[ii] || nnn[ii]);
...
/*
 * Apply a mask dataset to all voxels and across all sub-bricks of a dataset (migrated from the
 *mothership, 3dAutomask.c.
 */
...
byte * THD_automask(THD_3dim_dataset *dset)
{
    ...
    if (onlypos)
    {
        medim = THD_avepos_brick(dset);
    }
    else
    {
        medim = THD_aveabs_brick(dset);
    }
    if (medim == NULL)
    {
        RETURN(NULL);
...
/*! Make a byte mask from the average of an array of 3D images. We assume that they all have the
 * same (nx,ny,nz) dimensions.
 * -----------------------------------------------------------------------*/
...
byte * mri_automask_imarr(MRI_IMARR *imar)    /* 18 Nov 2004 */
{
    ...
    for (jj = 0 ; jj < nim ; jj++)
    {
        ...
        if (tim->kind != MRI_float)
        {
            qim = mri_to_float(tim);
        }
        else
        {
            qim = tim;
...
/*! Make a byte mask from an image (3D).  Adapted from THD_automask() to make it possible to do this
...
byte * mri_automask_image(MRI_IMAGE *im)
{
    ...
    if (im->nz == 1)
    {
        RETURN(mri_automask_image2D(im));                  /* 12 Jun 2010 */
    }
    ...
    if (verb)
    {
        ININFO_message("Fixed clip level = %f\n", clip_val);
    }
    ...
    else
    {
        ...
        if (verb)
        {
            ININFO_message("Used gradual clip level = %f .. %f", cbot, ctop);
        }
        mri_free(cim);
    }
    ...
    if (verb)
    {
        ININFO_message("Number voxels above clip level = %d\n", nmm);
    }
    if (im != medim && (!exterior_clip || nmm == 0))
    {
        mri_free(medim); medim = NULL;
    }
    if (nmm == 0)
    {
        cheapo = 0; RETURN(mmm);
    ...
    /* 18 Apr 2002: now erode the resulting volume (to break off any thinly attached pieces) */
    ...
    if (verb)
    {
        ININFO_message("Clustering non-brain voxels ...\n");
    }
    THD_mask_clust(nx, ny, nz, mmm);

    /* mask is now 1 for non-brain voxels;
     * if we want to clip off voxels neighboring the non-brain mask AND whose values are below
     *clip_val, do so now     */
    ...
    if (verb)
    {
        ININFO_message("Mask now has %d voxels\n", mask_count(nvox, mmm));
    }
...
/*! Find voxels not in the mask, but neighboring it, that are also below the clip threshold in
 * mar[].  Add these to the mask. Repeat until done.  Return value is number of voxels added.
 * -----------------------------------------------------------------------*/
        ...
        for (kk = 1 ; kk < nz - 1 ; kk++)
        {
            ...
            for (jj = 1 ; jj < ny - 1 ; jj++)
            {
                ...
                for (ii = 1 ; ii < nx - 1 ; ii++)
                {
                    i3 = ii + j3;
                    if (mmm[i3] ||                                /* in mask */
                        (mar[i3] >= clip_val && mar[i3] <= tclip))
                    {
                        continue;                                 /* or is OK */
                    }

                    /* If here, voxel IS NOT in mask, and IS below threshold. If any neighbors are
                     * also in mask, then add it to mask. */

                    if (mmm[i3 - 1] || mmm[i3 + 1] ||
                        mmm[i3 - nx] || mmm[i3 + nx] ||
                        mmm[i3 - nxy] || mmm[i3 + nxy])
                    {
                        mmm[i3] = 1; nnew++;
                    }
                }
...
/*! Fill in a byte mask.  Filling is done by looking to each side (plus/minus) of a non-filled
 * voxel, and seeing if there is a filled voxel on both sides.  This looking is done parallel to the
 * x-, y-, and z-axes, out to distance nside voxels.
...
int THD_mask_fillin_once(int nx, int ny, int nz, byte *mmm, int nside)
{
    ...
    nsx = (nx - 1) / 2; if (nsx > nside)
    {
        nsx = nside;
    }
    nsy = (ny - 1) / 2; if (nsy > nside)
    {
        nsy = nside;
    }
    nsz = (nz - 1) / 2; if (nsz > nside)
    {
        nsz = nside;
    }
...
#define XMIN11(q)    XMIN10(q) || mmm[q - 11]
                ...
                case 4:
                    if ((mmm[iv + 1] || mmm[iv + 2] || mmm[iv + 3] || mmm[iv + 4]) &&
                        (mmm[iv - 1] || mmm[iv - 2] || mmm[iv - 3] || mmm[iv - 4]))
                    {
                        FILLVOX;
                    }
                    break;
                ...
                case 3:
                    if ((mmm[iv + nx] || mmm[iv + nx2] || mmm[iv + nx3]) &&
                        (mmm[iv - nx] || mmm[iv - nx2] || mmm[iv - nx3]))
                    {
                        FILLVOX;
                    }
                    break;
                ...
                case 4:
                    if ((mmm[iv + nx] || mmm[iv + nx2] || mmm[iv + nx3] || mmm[iv + nx4]) &&
                        (mmm[iv - nx] || mmm[iv - nx2] || mmm[iv - nx3] || mmm[iv - nx4]))
                    {
                        FILLVOX;
                    }
                    break;
                ...
                case 3:
                    if ((mmm[iv + nxy] || mmm[iv + nxy2] || mmm[iv + nxy3]) &&
                        (mmm[iv - nxy] || mmm[iv - nxy2] || mmm[iv - nxy3]))
                    {
                        FILLVOX;
                    }
                    break;
                ...
                case 4:
                    if ((mmm[iv + nxy] || mmm[iv + nxy2] || mmm[iv + nxy3] || mmm[iv + nxy4]) &&
                        (mmm[iv - nxy] || mmm[iv - nxy2] || mmm[iv - nxy3] || mmm[iv - nxy4]))
                    {
                        FILLVOX;
                    }
                    break;
...
/*! Fill in a byte mask, repeatedly until it doesn't fill any more. Return value is number of voxels
 * filled.
 * ------------------------------------------------------------------------*/
 ...
 *
 *  A hole is defined as a connected set of zero voxels that does not reach a volume edge.
 *
 *  method: use less than two volumes of extra memory to be fast (one volume is to make 0/1 copy of
 *first, so values are known) make 0/1 copy of volume for each voxel if set, continue find connected
 *set of zero voxels, setting to 2 if this does not contain an edge voxel, reset to 1 clear all edge
 *voxels (set any 2 to 0)
 *
...
/* rcr - pass ivec3 check_dir (data axis directions) */
    ...
    for (voxel = 0; voxel < nvox; voxel++)
    {
        ...
        if (Cset.num < 1)
        {
            ERROR_message("TFMH: FCS failure"); RETURN(-1);
        }
        ...
        if (!has_edge)
        {
            ...
            if (verb > 1)
            {
                INFO_message("found hole of %d voxels", Cset.num);
            }
        ...
        {
            INFO_message("found edge set of %d voxels", Cset.num);
        }
    }
    ...
    if (verb)
    {
        INFO_message("filled %d holes (%d voxels)", nholes, nfilled);
    }
...
/*! find connected set of zero voxels, and note whether this is an edge set (whether any voxel is at
 * a volume edge)
 *
 *  modify new set of bmask values to 2
 *
 *  dirs is a restriction of connected sets - only search in those dirs (basically to allow for
 *planar hole filling)
 *
 *  method:
 *     set voxel init as search list while search list is not empty init new search list for each
 *voxel in list for each zero neighbor (6, 18 or 26?  start with 6) (restrict neighbors to axis
 *dirs) set and add to new list current list = new list
 *
                              ...
                              THD_ivec3 *dirs, int_list *IL)
{
    ...
    if (init_int_list(&SL1, 1000) != 1000)
    {
        RETURN(-1);
    }
    if (init_int_list(&SL2, 1000) != 1000)
    {
        RETURN(-1);
    }
    ...
    if (add_to_int_list(spa, voxel, 1000) < 0)
    {
        RETURN(-1);
    }
        ...
        for (index = 0; index < spa->num; index++)
        {
            ...
            if (!has_edge)
            {
                if (dirs)
                {
                    ...
                    else if (doj && (j == 0 || j == ny - 1))
                    {
                        has_edge = 1;
                    }
                    else if (dok && (k == 0 || k == nz - 1))
                    {
                        has_edge = 1;
                    }
                }
             ...
             *    if neighbor is not off edge and is not set set and add to next search list
             */
            if (doi)
            {
                testvox = newvox - 1;
...
void THD_mask_clust(int nx, int ny, int nz, byte *mmm)
{
    ...
    if (verb)
    {
        ININFO_message("Clustering voxels ...");
    }

    ijk_last = 0; if (dall < DALL)
    {
        dall = DALL;
    }
    while (1)
    {
         ...
         *    check neighboring points for nonzero entries in mmm enter those into cluster (and
         *clear them in mmm) continue until end of cluster is reached (note that cluster is
         *expanding as we progress)
         * --*/
    ...
    if (verb)
    {
        ININFO_message("Largest cluster has %d voxels\n", nbest);
    }
...
/*! Erode away nonzero voxels that aren't neighbored by mostly other nonzero voxels.  Then (maybe)
 * restore those that were eroded that are neighbors of survivors. The neighbors are the 6,18,26
 * voxels closest in 3D (facing, edge and corner neighbors) by specifying NN=1,2 or 3. The byte
 * array passed here is returned with voxel elements zeroed
 * ----------------------------------------------------------------------------*/
void THD_mask_erode(int nx, int ny, int nz, byte *mmm, int redilate, byte NN)
{
    ...
    if (verb && jj > 0)
    {
        ININFO_message("Eroded   %d voxels\n", jj);
    }

    /* optionally re-dilate eroded voxels that are next to survivors */

    /* useful for cleaning up 1 voxel deep necks that are eroded in at least two directions */
    if (redilate)
    {
        ...
        if (verb && jj > 0)
        {
            ININFO_message("Restored %d eroded voxels\n", jj);
        }
    } /* end of redilate */
...
/*! Erode away nonzero voxels that aren't neighbored by mostly other nonzero voxels.
 *
 *  Pass nerode to match ndil from THD_mask_dilate. For each masked voxel, count unmasked neighbors.
 *If there are at least nerode, then erode the voxel.
 *
 *  Pass NN to specify NN=1,2,3, again to match THD_mask_dilate. The nerode count will be restricted
 *by the given NN level.
 *                                                       19 May 2020 [rickr]
 *
 *  This allows for symmetric dilate/erode operations. It is symmetric with THD_mask_dilate.
 *                 7 May 2012 [rickr]
 * ----------------------------------------------------------------------------*/
                        ...
                        int NN)
{
    ...
    if (NN >= 3)
    {
        nmax = 26;                  /* 3x3x3 box minus center      */
    }
    else if (NN == 2)
    {
        nmax = 18;                  /* minus the 8 corners         */
    }
    else
    {
        nmax = 6;                   /* count only face neighbors   */
    }
    ...
    /* then switch logic from (>= nerode zero neighbors) to (<= (nmax-nerode) set ones) */
    nmask = nmax - nerode;

    nnn = (byte *)calloc(sizeof(byte), nxyz); /* mask of eroded voxels */
    if (!nnn)
    {
        EXRETURN;                             /* death to Ming! */
    }
...
/*! Generalization of THD_mask_erode(), to peel away multiple layers and then redilate.
 * ----------------------------------------------------------------------------*/
            ...
            for (jj = 0 ; jj < ny ; jj++)
            {
                ...
                for (ii = 0 ; ii < nx ; ii++)
                {
                    if (mmm[ii + jy + kz]) /* count nonzero nbhrs */
                    {
                        ...
                        if (num < realpeelthr)
                        {
                            nnn[ii + jy + kz] = bpp;      /* mark to erode */
                        }
                    }
                }
            }
                ...
                for (ii = 0 ; ii < nx ; ii++)
                {
                    if (nnn[ii + jy + kz] >= bpp && !mmm[ii + jy + kz]) /* was eroded before */
                    {
                        ...
                        qqq[ii + jy + kz] = /* count any surviving nbhrs */
                                            mmm[im + jy + km]
                                            + mmm[ii + jm + km] + mmm[ii + jy + km] +
                                            mmm[ii + jp + km]
                                            + mmm[ip + jy + km]
                                            + mmm[im + jm + kz] + mmm[im + jy + kz] +
                                            mmm[im + jp + kz]
                                            + mmm[ii + jm + kz] + mmm[ii + jp + kz]
                                            + mmm[ip + jm + kz] + mmm[ip + jy + kz] +
                                            mmm[ip + jp + kz]
                                            + mmm[im + jy + kp]
                                            + mmm[ii + jm + kp] + mmm[ii + jy + kp] +
                                            mmm[ii + jp + kp]
                                            + mmm[ip + jy + kp];
                    }
                }
...
/*! Dilate a mask - that is, fill in zero voxels that have at least ndil neighbors in the mask.  The
 * neighbors are the 8/18/26 voxels closest in 3D defined by the NN Nearest Neighbor mode
 * NN1=facing,NN2=edge,NN3=corners Past default has been NN2.
 * not sure about usefulness of ndil threshold, but keeping it because it's used across many
 *programs now  [DRG] Return value is number of voxels added to the mask.
 * ----------------------------------------------------------------------------*/
...
void THD_autobbox_noexpand(int c)
{
    bbox_noexpand = c;
}
 ...
 *  [xm..xp] will be box for x index, etc. Send in a prefix and you shall get a cropped volume back.
 *Else you get NULL and you will like it!
 * -----------------------------------------------------------------------*/
                                ...
                                char *prefix)
{
    ...
    medim = THD_maxabs_brick(dset); if (medim == NULL)
    {
        RETURN(NULL);
    }
    ...
    if (prefix)
    {
        ...
        xb = *xm - bbox_npad; if (bbox_noexpand && xb < 0)
        {
            xb = 0;
        }
        xt = *xp + bbox_npad; if (bbox_noexpand && xt >= nx)
        {
            xt = nx - 1;
        }
        yb = *ym - bbox_npad; if (bbox_noexpand && yb < 0)
        {
            yb = 0;
        }
        yt = *yp + bbox_npad; if (bbox_noexpand && yt >= ny)
        {
            yt = ny - 1;
        }
        zb = *zm - bbox_npad; if (bbox_noexpand && zb < 0)
        {
            zb = 0;
        }
        zt = *zp + bbox_npad; if (bbox_noexpand && zt >= nz)
        {
            zt = nz - 1;
        }
...
/*! For each plane direction, return the first and last index that have nonzero voxels in that
 * plane.
 *
 *  If an early error, the last positions will be -1. If the box is empty, the last positions will
 *be -1.
 * -----------------------------------------------------------------------*/
                  ...
                  int *xm, int *xp, int *ym, int *yp, int *zm, int *zp)
{
    ...
    /* For each plane direction, find the first and last index that have nonzero voxels in that
     * plane */
                       ...
                       int *xm, int *xp, int *ym, int *yp, int *zm, int *zp)
{
    ...
    if (qim->kind != MRI_byte)
    {
        EXRETURN;                         /* bad input */
    }
    mmm = MRI_BYTE_PTR(qim); if (qim == NULL)
    {
        EXRETURN;
    }
...
/*!
 * Return a vector representing the number of layers peeled to reach a voxel in mask.  ZSS March 02
 *2010 nn is 1,2 or 3 for nearest neighbor mode NN1, NN2, NN3
 */
                      ...
                      byte preservemask, short *usethisdepth, byte NN)
{
    ...
    if (!cpmask)
    {
        if (verb)
        {
            ERROR_message("NULL mask (or mask copy) pointer");
        }
        return NULL;
    }
    ...
    else
    {
        if (!(depth = (short *)calloc(nxyz, sizeof(short))))
        {
            if (verb)
            {
                ERROR_message("Failed to allocate for %d shorts", nxyz);
            }
            return NULL;
        }
    }
    if (!depth)
    {
        if (verb)
        {
            ERROR_message("NULL depth vector");
        }
        return NULL;
    }
    ...
    while (ncpmask > 0)
    {
        ...
        if (verb)
        {
            INFO_message("Peeled %d voxels from mask of %d (now %d)\n",
                         np, ncpmask,
                         ncpmask - np);
        }
...
/*! Find the biggest cluster of nonzeros in the byte mask mmm, and keep all clusters at least kfrac
 * times that size (0 < kfrac <= 1, duh).
 *//*----------------------------------------------------------------*/
...
void THD_mask_clust2D(int nx, int ny, float kfrac, byte *mmm)
{
    ...
    while (1)
    {
        ...
        if (nnow > nbest)
        {
            nbest = nnow;               /* nbest = size of biggest cluster */
        }
    } /* loop ends when all nonzero points are clustered */
...
byte * mri_automask_image2D(MRI_IMAGE *im)    /* 12 Jun 2010 */
{
    ...
    if (nmm == 0)
    {
        free(mmm); RETURN(NULL);
    }                                            /* should not happen */
    if (nmm <= 2 || nmm == nvox)
    {
        RETURN(mmm);                              /* very unlikely */
    }
