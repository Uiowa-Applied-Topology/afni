...
/* This function is for testing a gltsym string for basic validity. It returns a string that
 * contains the messages generated during the tests.
 * varlist = space (or comma or semicolon) delimited list of variable names gltsym  = expression for
 *GLT If NULL is returned, there were no problems detected.  [01 May 2015]
 *//*--------------------------------------------------------------------------*/
    ...
    for (nrang = vv = 0 ; vv < vsar->num ; vv++)
    {
        ...
        if (!isalpha(vnam[0]))
        {
            outbuf = THD_zzprintf(outbuf,
                                  "** ERROR: Variable name '%s' doesn't start with alphabetic character\n",
                                  vnam);
            nbad++; nerr++;
        }
...
/*! Expand a string like "Fred 2*Jed -Ned[1..3]" into a float vector that comprises the weights for
 * each beta element in 3dDeconvolve.
 *
 ...
 *
 *  The set of structs in rang[] should collectively span global indexes from 0..nlast (inclusive).
 *The returned floatvec will have nlast+1 entries.
 * ------------------------------------------------------------------------------*/
...
floatvecvec * SYM_expand_ranges(int nlast, int nrang, SYM_irange *rang, char *str)
{
    ...
    if (nrang < 1 || rang == NULL || str == NULL || *str == '\0')
    {
        RETURN(NULL);
    }
    ...
    for (ii = 0 ; str[ii] != '\0' && isspace(str[ii]) ; ii++)
    {
        ;                                      /*nada*/
    }
    ...
    for (ss = 0 ; ss < sar->num ; ss++)
    {
        ...
        if (strlen(qstr) == 1 && (*qstr == '+' || *qstr == '-'))
        {
            ...
            else                                   /* this is a "-" */
            {
                ...
                {
                    ERROR_message("-gltsym: isolated '%s' is followed by NULL entry in? IGNORING!",
                                  qstr);
                    qstr[0] = '\0'; nerr++;
                }
                ...
                {
                    ERROR_message(
                        "-gltsym: isolated '-' is followed by '%s' which is already signed? IGNORING!",
                        qpt);
                    qstr[0] = '\0'; nerr++;
                }
    ...
    else                 /* have multiple outputs */
    {
        for (iv = 0 ; iv < nvec ; iv++)
        {
            ...
            {
                if (fvv->fvar[iv].ar[gg] == 0.0f)
                {
                    fvv->fvar[iv].ar[gg] = fv->ar[gg];
                }
            }
        }
        KILL_floatvec(fv);
