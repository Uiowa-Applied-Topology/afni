...
static int wfverb = 0;
void Warpfield_set_verbose(int vv)
{
    wfverb = vv;
}
...
void Warpfield_destroy(Warpfield *wf)
{
    ...
    if (wf->bpar != NULL)
    {
        wf->bset(-1.0f, NULL, wf->flags, wf->bpar);
    }
...
/*! Compute coefficients in wf so that the basis function expansion evaluated at (xi,yi,zi) is a
 * least squares fit to (xw,yw,zw).
 *//*-------------------------------------------------------------------------*/
                       ...
                       float *xw, float *yw, float *zw)
{
    ...
    /* ncol = number of variables to solve for nrow = number of equations being solved */
...
#define P(i, j)    iar[(i) + (j) * ncol] /* i=0..ncol-1 , j=0..nrow-1 */
    ...
    /* create matrix, each column of which is a basis function evaluated at the (xi,yi,zi) points */
    ...
    /* apply pseudo-inverse to (xw,yw,zw) points, to get coefficients for each basis function */
    ...
    /* compute the RMS error between the fitted basis functions evaluated at the (xi,yi,zi) points
     * and the (xw,yw,zw) points */
...
Warpfield * Warpfield_inverse(Warpfield *wf, float *rmserr)
{
    ...
    for (ii = 0 ; ii < NG ; ii++)
    {
        gg[ii] = (2.0f / PI) * asinf(-1.0f + (ii + 0.499f) * dg);
    }
...
int cmp_fvm(const fvm *v, const fvm *w)     /* for sorting */
{
    ...
    cc = CFV(v->c, w->c); if (cc)
    {
        return cc;
    }
    cc = CFV(v->b, w->b); if (cc)
    {
        return cc;
    }
    ...
    for (pp = kk = 0 ; kk < qq ; kk++)
    {
        for (jj = 0 ; jj < qq ; jj++)
        {
            for (ii = 0 ; ii < qq ; ii++)
            {
                if (ii + jj + kk <= 1)
                {
                    continue;         /* skip the lowest orders */
                }
...
void * Warpfield_prodfun_setup(float order, int *nfun, int flags, void *vp)
{
    ...
    if (nfun == NULL || order < 2.0f || order > MAXORD)
    {
        return NULL;
    }
...
#define P9                                                                                        \
        (2.4609375f + (-36.09375f + (140.765625f + (-201.09375f + 94.9609375f * xq) * xq) * xq) * \
         xq) * x
...
#define G9    (-0.2734375f + (3.28125f + (-10.828125f + (13.40625f - 5.5859375f * xq) * xq) * xq) * \
               xq) * x
                       ...
                       int npt, float *x, float *y, float *z, float *val)
{
    ...
    if (first)
    {
        INFO_message("Warpfield trigfun initialized"); first = 0;
    }
                      ...
                      int npt, float *x, float *y, float *z, float *val)
{
    ...
    if (first)
    {
        INFO_message("Warpfield legfun initialized"); first = 0;
    }
...
/* Note that if SKIPAFF is turned on, then (xo,yo,zo) inputs are added to, but if SKIPAFF is off,
 * then (xo,yo,zo) inputs are cast aside like trash.
 *//*-------------------------------------------------------------------------*/
