...
/*---------------------------------------------------------------------
 * 01 Feb 2001: determine if 2 datasets don't match in some way for voxel-by-voxel comparison.
 *  return = 0 if they are good in all ways return = binary OR of MISMATCH_ codes (cf. 3ddata.h) if
 *something in their dataxes don't match A generalization of EQUIV_GRIDS
 * -----------------------------------------------------------------------*/
...
int THD_dataset_mismatch(THD_3dim_dataset *ds1, THD_3dim_dataset *ds2)
{
    ...
    if (!ISVALID_DSET(ds1) || !ISVALID_DSET(ds2))
    {
        RETURN(-1);
    }
    ...
    if (fabs(dax1->xxdel - dax2->xxdel) > 0.01 * fabs(dax1->xxdel) ||
        ...
        fabs(dax1->zzdel - dax2->zzdel) > 0.01 * fabs(dax1->zzdel))
    {
        code |= MISMATCH_DELTA;
    }
    ...
    if (dax1->xxorient != dax2->xxorient ||
        ...
        dax1->zzorient != dax2->zzorient)
    {
        code |= MISMATCH_ORIENT;
    }
...
/*---------------------------------------------------------------------
 * 23 Feb 2012: Return the absolute value of the difference between two volumes, divided by the
 *number of voxels and the number of sub-bricks. Voxels that are zero in both sets are not counted.
 *Comparisons are done after conversion of data to double return = -1.0 ERROR
...
double THD_diff_vol_vals(THD_3dim_dataset *d1, THD_3dim_dataset *d2, int scl)
{
    ...
    if (!EQUIV_GRIDS(d1, d2))
    {
        RETURN(-1.0);
    }
    if (DSET_NVALS(d1) != DSET_NVALS(d2))
    {
        RETURN(-1.0);
    }
...
/*---------------------------------------------------------------------
 * 07 Dec 2021: Return the total number of differing voxels within the given tolerance.
 *  return = -1 ERROR
 *         =  0 Exactly the same
