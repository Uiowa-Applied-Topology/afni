...
int main(int argc, char *argv[])
{
    ...
    while (iarg < argc && argv[iarg][0] == '-')
    {
        if (strcmp(argv[iarg], "-mask") == 0)
        {
            ...
            if (mask != NULL)
            {
                ERROR_exit("Can't have two '-mask' options");
            }
            mset = THD_open_dataset(argv[iarg]);
            if (mset == NULL)
            {
                ERROR_exit("Can't open mask dataset '%s'", argv[iarg]);
            }
        ...
        if (strcmp(argv[iarg], "-input") == 0)
        {
            if (inset != NULL)
            {
                ERROR_exit("Can't use 2 '-input' options");
            }
            iarg++; if (iarg >= argc)
            {
        ...
        if (strcmp(argv[iarg], "-matrix") == 0)
        {
            ...
            if (imat != NULL)
            {
                ERROR_exit("Can't use 2 '-matrix' options");
            }
            iarg++; if (iarg >= argc)
            {
                ERROR_exit("Need argument after '-matrix'");
            }
                ...
                (strchr(argv[iarg], ' ') == NULL && argv[iarg][0] != '&'))
            {
                ...
                if (imat == NULL)
                {
                    ERROR_exit("Can't read matrix file '%s'", argv[iarg]);
                }
                mop = "Read in";
            }
            else
            {
                ...
                if (imat == NULL)
                {
                    ERROR_exit("Can't evaluate matrix expression");
                }
                mop = "Calculated";
            }
