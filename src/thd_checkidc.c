...
/*----------------------------------------------------------------
 *  Check to see if there are any duplicate ID codes in the datasets stored herein.
...
void THD_check_idcodes(THD_sessionlist *ssl)
{
    ...
    {
        sess = ssl->ssar[iss]; if (!ISVALID_SESSION(sess))
        {
            continue;
        }
...
/*         dset = sess->dsset_xform_table[idd][ivv] ;*/
    ...
    for (iss = idd = 0 ; idd < dsnum - 1 ; idd++)
    {
        ...
        {
            if (DUPLICATE_DSETS(dsl[idd], dsl[jdd])) /* 20 Dec 2001: change EQUIV_IDCODES() to
                                                      * DUPLICATE_DSETS() */
            {
                fprintf(stderr,
