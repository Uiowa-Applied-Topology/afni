...
void mri_matrix_evalrpn_verb(int i)
{
    verb_rpn = i;
}
...
#define A(i, j)    amat[(i) + (j) * nr] /* nr X nc */
    ...
    {
        val = (int)amat[ii];
        if (val != amat[ii] || fabsf(val) > 99.0f)
        {
            break;
        }
    }
    ...
    for (ii = 0 ; ii < nr ; ii++)
    {
        for (jj = 0 ; jj < nc ; jj++)
        {
            if (ipr)
            {
                fprintf(fp, " %3d", (int)A(ii, jj));
            }
            else
            {
                fprintf(fp, " %11.5g", A(ii, jj));
            }
        }
        fprintf(fp, "\n");
    }
...
MRI_IMAGE * mri_matrix_mult(MRI_IMAGE *ima, MRI_IMAGE *imb)
{
    ...
    if (ima->kind != MRI_float || imb->kind != MRI_float)
    {
        RETURN(NULL);
    }
...
/*! Compute the product of two matrices, stored in 2D float images. The first matrix is transposed.
 * -------------------------------------------------------------------------*/
...
/*! Compute the product of two matrices, stored in 2D float images. The second matrix is transposed.
 * -------------------------------------------------------------------------*/
...
/*! Multiply matrix in ima by scalar fa, matrix in imb by scalar fb, and add the results.  Used for
 * general purpose matrix add/subtract.
 * -------------------------------------------------------------------------*/
...
#pragma omp critical (STDERR)
        { if (first)
          {
              ERROR_message("SVD fails in mri_matrix_svprod()!\n");
          }
        }
        free(sval); first = 0;
        RETURN(imp);
...
void mri_matrix_psinv_svd(int i)
{
    force_svd = i;
}
...
/*! Compute the pseudo-inverse of a matrix stored in a 2D float image. If the input is mXn, the
 * output is nXm.  wt[] is an optional array of positive weights, m of them.  The result can be used
 * to solve the weighted least squares problem [imc] [b] = [v] where [b] is an n-vector and [v] is
 * an m-vector, where m >= n. If alpha > 0, then the actual matrix calculated is
 *                        -1 [imc' imc + alpha I]   imc'    (where ' = transpose)
 *
...
MRI_IMAGE * mri_matrix_psinv(MRI_IMAGE *imc, float *wt, float alpha)
{
    ...
    if (PRINT_TRACING)
    {
        char str[222]; sprintf(str, "m=%d n=%d", m, n); STATUS(str);
    }
...
#define V(i, j)    vmat[(i) + (j) * n]
    ...
    for (ii = 0 ; ii < n ; ii++)
    {
        for (kk = 0   ; kk < m && A(kk, ii) == 0.0 ; kk++)
        {
            ;                                            /*nada*/
        }
        kbot[ii] = kk;
        for (kk = m - 1 ; kk >= 0 && A(kk, ii) == 0.0 ; kk--)
        {
            ;                                            /*nada*/
        }
        ktop[ii] = kk;
    ...
    for (ii = 0 ; ii < n ; ii++)      /* form normal equations */
    {
        ...
        for (jj = 0 ; jj <= ii ; jj++)
        {
            jbot = kbot[jj]; if (ibot > jbot)
            {
                jbot = ibot;
            }
            jtop = ktop[jj]; if (itop < jtop)
            {
                jtop = itop;
            }
            sum = 0.0;
            for (kk = jbot ; kk <= jtop ; kk++)
            {
                sum += A(kk, ii) * A(kk, jj);
            }
            V(ii, jj) = sum;
...
#pragma omp critical (STDERR)
    ...
    del = PSINV_EPS * smax * smax; if (del < alp)
    {
        del = alp;
    }
    for (ii = 0 ; ii < n ; ii++)
...
/*! The output matrix is the orthogonal projection onto the linear space spanned by the columns of
 * the input imc.  If pout != 0, then instead it is the orthogonal projection onto the complement of
 * this space. If the input is NxM, the output is NxN.  Note that the matrix output by this function
 * will be symmetric.                          [10 Apr 2006]
 * ------------------------------------------------------------------------------*/
...
/*! Return both the pseudo-inverse and the orthogonal projection. If [C] is N x M, then the psinv is
 * M x N and the ortproj is N x N. */
...
MRI_IMARR * mri_matrix_psinv_pair(MRI_IMAGE *imc, float alpha)
{
    ...
    if (m < 1 || n < 1)
    {
        RETURN(NULL);                   /* bad users should be shot! */
    }
    rmat = MRI_FLOAT_PTR(imc);
...
#pragma omp critical (STDERR)
        { if (first)
          {
              ERROR_message("SVD fails in mri_matrix_psinv_pair()!\n");
          }
        }
        free(xfac); free(sval); first = 0;
        free(vmat); free(umat); RETURN(NULL);
...
/*! Return the sqrt(eigenvalues) of NxN matrix [C'C], scaled to diagonal 1. The output points to a
 * 1D image as vector of floats, of length N=C->ny. This image should be mri_free()-ed when you are
 * done with it.
 * -----------------------------------------------------------------------------*/
...
/*! imt = time series to detrend in place (can be more than 1 vector in here) imq = array of orts to
 * remove (imq->nx must equal imt->nx) imp = mri_matrix_psinv(imq)   (imp is imq->ny X imq->nx)
 *//*--------------------------------------------------------------------------*/
...
void mri_matrix_detrend(MRI_IMAGE *imt, MRI_IMAGE *imq, MRI_IMAGE *imp)
{
    ...
    if (imt == NULL || imq == NULL || imp == NULL)
    {
        EXRETURN;
    }

    nlen = imt->nx; if (nlen != imq->nx)
    {
        EXRETURN;
    }
    nort = imq->ny; if (imp->nx != nort || imp->ny != nlen)
    {
        EXRETURN;
    }
    nvec = imt->ny;
...
MRI_IMAGE * mri_matrix_sqrt(MRI_IMAGE *imc)    /* 30 Jul 2007 */
{
    ...
    nn = imc->nx; if (nn != imc->ny)
    {
        RETURN(NULL);
    }
    if (nn == 1)
    {
        car = MRI_FLOAT_PTR(imc); if (car[0] < 0.0f)
        {
            RETURN(NULL);
        }
    ...
    csiz = mri_matrix_size(imc); if (csiz <= 0.0f)
    {
        RETURN(imz);
    }
    imy = mri_copy(imc);
    ...
    for (ite = 0 ; ite < 50 ; ite++)
    {
        ...
        {
            imzinv = mri_matrix_psinv(imz, NULL, 0.0f);
        }
        if (imzinv == NULL)
        {
            ERROR_message("mri_matrix_sqrt() fails at ite=%d", ite);
...
MRI_IMAGE * mri_matrix_pcvector(MRI_IMAGE *imx)
{
    ...
    if (imx == NULL || imx->kind != MRI_float)
    {
        RETURN(NULL);
    }
...
double Plegendre(double x, int m)
{
    ...
    case 8: return ((((6435.0 * x * x - 12012.0) * x * x + 6930.0) * x * x - 1260.0) * x * x +
                    35.0) / 128.0;
    ...
    case 13:
        return (0.29326171875e1 + (-0.87978515625e2 + (0.7478173828125e3 +
                                                       (-0.270638671875e4 +
                                                        (0.47361767578125e4 + (-0.3961166015625e4
                                                                               +
                                                                               0.12696044921875e4 *
                                                                               x * x) * x * x) * x *
                                                        x) * x * x) * x * x)
        ...
        return -0.1854705810546875e0 + (0.3171546936035156e2 +
                                        (-0.8880331420898438e3 + (0.9531555725097656e4 +
                                                                  (-0.5106190567016602e5 +
                                                                   (0.1531857170104980e6 +
                                                                    (-
                                                                     0.2692355026245117e6 +
                                                                     (0.2751527664184570e6 +
                                                                      (
                                                                          -
                                                                          0.1513340215301514e6 +
                                                                          0.34618893814086910e5 *
                                                                          x * x) * x * x)
                                                                     * x *
                                                                     x) * x * x) * x * x) * x * x) *
                                         x * x) * x * x) * x * x;
...
static int matrix_name_lookup(char *nam)
{
    ...
    for (ii = 0 ; ii < IMARR_COUNT(matar) ; ii++)
    {
        ...
        if (im != NULL && strcmp(nam, im->name) == 0)
        {
            return ii;
        }
    }
...
static int char_check(char ccc)
{
    return ccc == '&' || ccc == '@' || ccc == '%';
}

static int command_check(char *str, char *cmd)
{
    if (str == NULL || cmd == NULL || *str == '\0' || *cmd == '\0')
    {
        return 0;
    }
    if (char_check(str[0]) == 0)
    {
        return 0;
    }
    return strncasecmp(str + 1, cmd, strlen(cmd)) == 0;
}
...
MRI_IMAGE * mri_matrix_evalrpn(char *expr)
{
    ...
    if (verb_rpn)
    {
        fprintf(stderr, "++ mri_matrix_evalrpn('%s')\n", expr);
    }
    ...
    for (ss = 0 ; ss < sar->num ; ss++)
    {
        ...
        else if (strcmp(cmd, "'") == 0 || command_check(cmd, "trans") || command_check(cmd, "'"))
        {
            if (nstk < 1)
            {
                ERREX("no matrix");
        ...
        else if (command_check(cmd, "read("))
        {
            ...
            *bp = '\0';
            ...
            if (heq)
            {
                matrix_name_assign(bp + 2, imc);
            }
            free(buf);
        }

        else if (command_check(cmd, "read4x4Xform("))
        {
        ...
        else if (command_check(cmd, "write("))
        {
            char *buf = strdup(cmd + 7), *bp;
            if (nstk < 1)
            {
                free(buf); ERREX("no matrix");
            }
            for (bp = buf ; *bp != '\0' && *bp != ')' ; bp++)
            {
                ;                                          /*nada*/
        ...
        else if (command_check(cmd, "ident("))
        {
            int nn; char *cv = cmd + 7;
            DECODE_VALUE(cv, nn);
            if (nn <= 0)
            {
                ERREX("illegal dimension");
            }
        ...
        else if (command_check(cmd, "Sqrt"))
        {
            if (nstk < 1)
            {
                ERREX("no matrix");
        ...
        else if (command_check(cmd, "Pproj"))
        {
            if (nstk < 1)
            {
                ERREX("no matrix");
        ...
        else if (command_check(cmd, "Qproj"))
        {
            if (nstk < 1)
            {
                ERREX("no matrix");
        ...
        else if (strcmp(cmd, "*") == 0 || command_check(cmd, "mult"))
        {
            if (nstk < 2)
            {
                ERREX("no matrices");
        ...
        else if (strcmp(cmd, "+") == 0 || command_check(cmd, "add"))
        {
            if (nstk < 2)
            {
                ERREX("no matrices");
        ...
        else if (strcmp(cmd, "-") == 0 || command_check(cmd, "sub"))
        {
            if (nstk < 2)
            {
                ERREX("no matrices");
        ...
        else if (command_check(cmd, "swap"))
        {
            if (nstk < 2)
            {
                ERREX("no matrices");
        ...
        else if (cmd[0] == '\'' || cmd[0] == '"')
        {
            ...
            for (ii = 0 ; buf[ii] != cmd[0] && buf[ii] != '\0' ; ii++)
            {
                ;                                                   /*nada*/
            }
    ...
    if (verb_rpn)
    {
        fprintf(stderr, "++ exit mri_matrix_evalrpn\n");
    }
    ...
    if (nstk > 0)
    {
        ima = mri_to_float(IMARR_SUBIM(imstk, nstk - 1));
    }
    else
    {
        ima = NULL;
