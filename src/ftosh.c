...
void FTOSH_getopts(int argc, char *argv[])
{
    ...
    while (nopt < argc && argv[nopt][0] == '-')
    {
        ...
        if (strncmp(argv[nopt], "-prefix", 5) == 0)
        {
            if (++nopt >= argc)
            {
                FTOSH_syntax("-prefix needs a name!");
            }
        ...
        if (strncmp(argv[nopt], "-suffix", 5) == 0)
        {
            if (++nopt >= argc)
            {
                FTOSH_syntax("-suffix needs a name!");
            }
        ...
        if (strncmp(argv[nopt], "-scale", 5) == 0)
        {
            ...
            if (++nopt >= argc)
            {
                FTOSH_syntax("-scale needs a value!");
            }
        ...
        if (strncmp(argv[nopt], "-base", 5) == 0)
        {
            ...
            if (++nopt >= argc)
            {
                FTOSH_syntax("-base needs a value!");
            }
            FT_bval = strtod(argv[nopt], &ch);
            if (*ch != '\0')
            {
                FTOSH_syntax("value after -base is illegal!");
            }
            nopt++; continue; /* skip to next arg */
        }
        ...
        if (strncmp(argv[nopt], "-top", 4) == 0)
        {
            ...
            if (++nopt >= argc)
            {
                FTOSH_syntax("-top needs a value!");
            }
        ...
        if (strncmp(argv[nopt], "-start", 5) == 0)
        {
            ...
            if (++nopt >= argc)
            {
                FTOSH_syntax("-start needs a value!");
            }
        ...
        if (strncmp(argv[nopt], "-step", 5) == 0)
        {
            ...
            if (++nopt >= argc)
            {
                FTOSH_syntax("-step needs a value!");
            }
    ...
    if (FT_imts == NULL)
    {
        FTOSH_syntax("cannot continue without input images!");
    }
