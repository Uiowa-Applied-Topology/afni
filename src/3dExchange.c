...
int main(int argc, char *argv[])
{
    ...
    if (!dset)
    {
        ERROR_exit("Error - no dataset given!");
    }
    if (!mapstr)
    {
        ERROR_exit("Error - no mapping file given!");
    }
    ...
    if (floatize)
    {
        EDIT_floatize_dataset(new_dset);
    }
    ...
    for (i = 0; i < dset->dblk->nvals; i++)       /* for each sub-brik in dataset */
    {
        ...
        case MRI_short: {
            for (k = 0; k < nvox; k++)
            {
                ...
                else
                {
                    ...
                    {
                        outsar[k] = (short)(find_float_in_list(voxval, srclist, replacelist,
                                                               nx) / fac);
        ...
        case MRI_byte: {
            for (k = 0; k < nvox; k++)
            {
                ...
                else
                {
                    ...
                    {
                        outbar[k] = (byte)(find_float_in_list(voxval, srclist, replacelist,
                                                              nx) / fac);
        ...
        /* change storage mode, this way prefix will determine format of output dset */
...
/* return floating point value in replacelist wherever voxval matches value in srclist */
static float find_float_in_list(float voxval,
                                float *srclist, float *replacelist, int nx)
{
    int i;
    ...
    /* return the original value if not in list - could change this to some other behavior */
    return voxval;
}
