                     ...
                     XmHTMLAnchorCallbackStruct *cbs)
{
    ...
    case ANCHOR_HTTP: {                               /* external http links */
        ...
        if (first == 1)
        {
            webb = GetAfniWebBrowser(); first = 2;
        }
...
static char * htmlize(char *msg)
{
    ...
    if (strncmp(msg, "<html>", 6) == 0)
    {
        RETURN(msg);                       /* already HTML format */
    }
...
/* This is a callback to deal with some refresh problems that should have been handled by the XmHTML
 * library. For now this call is not needed. It looks like the patching of XmHTML did the trick ZSS
 * March 2012 */
...
/* Open a window with an XmHTML widget containing msg. If msg starts with "file:", then it indicates
 * a file to read and display. Otherwise, it is the content of the page directly.
 *//*--------------------------------------------------------------------------*/
...

{
    ...
    swid = WidthOfScreen(XtScreen(wpar)) - 222; if (swid > 799)
    {
        swid = 799;
    }
    shi = HeightOfScreen(XtScreen(wpar)) - 222; if (shi > 899)
    {
        shi = 899;
    }
...
#if 0                                     /* This was needed to deal with some refreshing problems
                                           * when the scrollbar was moved. The patch in XmHTML seems
                                           * to have do the trick. These are left here should we
                                           * need to reuse them someday */
...
char * convert_text_to_html(char *txt)
{
    ...
    while (*tin != '\0')
    {
        ...
        if (HTTP_check(tin))
        {
            ...
            for (hend = 7 ; tin[hend] != '\0' && !isspace(tin[hend]) ; hend++)
            {
                ;                                                          /*nada*/
            }
        ...
        if (HTTPS_check(tin))
        {
            ...
            for (hend = 8 ; tin[hend] != '\0' && !isspace(tin[hend]) ; hend++)
            {
                ;                                                          /*nada*/
            }
            /* insert hyperlink here*/
        ...
        if (cc == ' ')
        {
            EMIT_string("&nbsp;");                    /* put in HTML escapes */
        }
        else if (cc == '&')
        {
            EMIT_string("&amp;");                     /* for special cases */
        }
        else if (cc == '<')
        {
            EMIT_string("&lt;");
        ...
        {
            EMIT_char(cc);                            /* perfectly normal character */
        }
        tin++;
    }
