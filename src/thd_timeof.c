...
float THD_timeof_vox(int it, int nvox, THD_3dim_dataset *dset)
{
    ...
    if (!ISVALID_DSET(dset) || !ISVALID_TIMEAXIS(dset->taxis))
    {
        return 0.0;
    }
    ...
    if (dset->taxis->nsl <= 0 || dset->taxis->toff_sl == NULL)
    {
        return tof;
    }
...
float * TS_parse_tpattern(int nzz, float TR, char *tpattern)
{
    ...
    else if (tpattern[0] == '@')
    {
        ...
        else if (tim->nx < nzz && tim->ny < nzz && (tim->nx * tim->ny < nzz))
        {
            ...
            if (nv == 0)
            {
                nv = (tim->nx > tim->ny) ? tim->nx : tim->ny;
            }
