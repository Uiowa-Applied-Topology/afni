...
double waveform_FILE(double t)     /* 23 Aug 2005 */
{
    ...
    nn = (int)tf;
    tf = tf - (double)nn;
    if (nn < 0 || nn > FILE_nval)
    {
        return 0.0;
    }

    /* Changed (tf < 0.0001) to (tf && tf < 0.0001) Otherwise an extra repetition would occur if tf
     * is exactly 0. To illustrate:
     *    waver -dt 1 -FILE 1 '1D:1 1' -tstim 0 used to return: 1 1 1 0 (bad) and   waver -dt 1
     * -FILE 1 '1D:1 1' -tstim 0.001 used to return: 0 1 0.001 0 0 (which is OK) With the new
     *change:
     *    waver -dt 1 -FILE 1 '1D:1 1' -tstim 0 returns: 1 1 0 0     (OK) and   waver -dt 1 -FILE 1
     * '1D:1 1' -tstim 0.001 returns: 0 1 0.001 0 0 (OK) R&Z   */
    if (nn == FILE_nval)
    {
        return (tf && tf < 0.0001) ? (double)FILE_val[nn - 1] : 0.0;
    }

    /* The if statement is to guard against reading nn+1 when nn is equal to FILE_nval -1 R&Z   */
    if (nn == FILE_nval - 1)
    {
        return (1.0 - tf) * FILE_val[nn];
    }
    else
    {
...
void Process_Options(int argc, char *argv[])
{
    ...
    while (nopt < argc)
    {
        ...
        if (strncmp(argv[nopt], "-FILE", 4) == 0) /* 23 Aug 2005 */
        {
            ...
            if (nopt + 2 >= argc)
            {
                ERROR_exit("need 2 arguments after -FILE!");
            }
            FILE_dt = strtod(argv[++nopt], NULL);
            if (FILE_dt <= 0.0)
            {
                ERROR_exit("-FILE '%s' is illegal 'dt' value", argv[nopt]);
            }
            fim = mri_read_1D(argv[++nopt]);
            if (fim == NULL)
            {
                ERROR_exit("Can't read file '%s' for -FILE", argv[nopt]);
            }
            FILE_nval = fim->nx;
            if (FILE_nval < 2)
            {
                ERROR_exit("File '%s' too short for -FILE", argv[nopt]);
            }
        ...
        if (strncmp(argv[nopt], "-gamb", 5) == 0)
        {
            if (nopt + 1 >= argc)
            {
                ERROR_exit("need argument after -gamb");
            }
            GAM_power = strtod(argv[nopt + 1], NULL);
            if (GAM_power <= 0.0)
            {
                ERROR_exit("non-positive value after -gamb");
            }
            waveform_type = GAM_TYPE;
            nopt++; nopt++; continue;
        }
        ...
        if (strncmp(argv[nopt], "-gamc", 5) == 0)
        {
            ...
            if (GAM_time <= 0.0)
            {
                ERROR_exit("non-positive value after -gamc");
            }
            waveform_type = GAM_TYPE;
            nopt++; nopt++; continue;
        }
        ...
        if (strncmp(argv[nopt], "-del", 4) == 0)
        {
            ...
            if (WAV_delay_time < 0.0)
            {
                ERROR_exit("negative value after -del");
            }
            waveform_type = WAV_TYPE;
            nopt++; nopt++; continue;
        }
        ...
        if (strncmp(argv[nopt], "-ris", 4) == 0)
        {
            ...
            if (WAV_rise_time <= 0.0)
            {
                ERROR_exit("non-positive value after -ris");
            }
            waveform_type = WAV_TYPE;
            nopt++; nopt++; continue;
        }
        ...
        if (strncmp(argv[nopt], "-fal", 4) == 0)
        {
            ...
            if (WAV_fall_time <= 0.0)
            {
                ERROR_exit("non-positive value after -fal");
            }
            waveform_type = WAV_TYPE;
            nopt++; nopt++; continue;
        }
        ...
        if (strncmp(argv[nopt], "-res", 4) == 0)
        {
            ...
            if (WAV_restore_time <= 0.0)
            {
                ERROR_exit("non-positive value after -res");
            }
            waveform_type = WAV_TYPE;
            nopt++; nopt++; continue;
        }
        ...
        if (strncmp(argv[nopt], "-dt", 3) == 0 || strncmp(argv[nopt], "-TR", 3) == 0)
        {
            ...
            if (WAV_dt <= 0.0)
            {
                ERROR_exit("non-positive value after -dt");
            }
            nopt++; nopt++; continue;
        }
        ...
        if (strncmp(argv[nopt], "-inp", 4) == 0)
        {
            ...
            if (tsim == NULL)
            {
                ERROR_exit("can't read -input file '%s'", argv[nopt + 1]);
            }

            IN_npts = tsim->nx;
            IN_ts   = (double *)malloc(sizeof(double) * IN_npts);
            tsar    = MRI_FLOAT_PTR(tsim);
            for (ii = 0 ; ii < IN_npts ; ii++)
            {
                IN_ts[ii] = tsar[ii];
            }
            mri_free(tsim);
        ...
        if (strcmp(argv[nopt], "-tstim") == 0) /* 16 May 2001 */
        {
            ...
            while (iopt < argc && argv[iopt][0] != '-')
            {
                ...
                /* formats:  start            ==> instantaneous impulse start%duration   ==>
                 * extended duration start:end        ==> extended duration can also have
                 * "/amplitude" afterwards to scale result */
        ...
        if (strcmp(argv[nopt], "-when") == 0)  /* 08 Apr 2002 */
        {
            ...
            if (nbt < 1)
            {
                ERROR_exit("No ranges after -when?");
            }
