...
/* CC define nodes for histogram data structure that we will use for sparsity thresholding */
...
hist_node_head * free_histogram(hist_node_head *histogram, int nhistnodes)
{
    ...
    /* only try to free the histogram if we have reason to believe that it exists */
    if (histogram != NULL)
    {
        /* iterate through the histogram bins */
        for (kout = 0; kout < nhistnodes; kout++)
        {
            /* if there is a linked list for this bin, iterate over it and free the list */
        ...
        /* all of the linked lists should be empty, now free the bin array */
        if (histogram != NULL)
        {
            free(histogram);
...
/* 3dDegreeCentrality was created from 3dAutoTCorrelate by R. Cameron Craddock */

/*----------------------------------------------------------------*/
/**** Include these here for potential optimization for OpenMP ****/
/*----------------------------------------------------------------*/

/*! Pearson correlation of x[] and y[] (x and y are NOT modified. And we know ahead of time that the
 * time series have 0 mean and L2 norm 1.
 *//*--------------------------------------------------------------*/
...
int main(int argc, char *argv[])
{
    ...
    while (nopt < argc && argv[nopt][0] == '-')
    {
        ...
        if (strcmp(argv[nopt], "-sparsity") == 0)
        {
            ...
            {
                WARNING_message(
                    "Sparsity %3.2f%% is large and will require alot of memory and time, consider using a smaller value. ",
                    val);
            }
            dosparsity = 1;
    ...
    if (nopt >= argc)
    {
        ERROR_exit("Need a dataset on command line!?");
    }
    ...
    if (mset)
    {
        ...
        if (nmask < 2)
        {
            ERROR_exit("Only %d voxels in -mask, exiting...", nmask);
        }
    ...
    else if (do_autoclip)
    {
        ...
        if (nmask < 2)
        {
            ERROR_exit("Only %d voxels in -automask!", nmask);
        }
    }
    ...
    /* CC calculate the total number of possible correlations, will be useful down the road */
    ...
    if (xvectim == NULL)
    {
        ERROR_exit("Can't create vectim?!");
    }
     ...
     * avoid duplicating memory usage ---*/
    ...
    /* -- CC create arrays to hold degree and weighted centrality while they are being calculated --
     */
    if (dosparsity == 0)
    {
    ...
    /* -- CC if we are using a sparsity threshold, build a histogram to calculate the threshold */
    if (dosparsity == 1)
    {
        ...
        if ((histogram = (hist_node_head *)malloc(nhistnodes * sizeof(hist_node_head))) == NULL)
        {
            ...
            if (weightedDC)
            {
                free(weightedDC); weightedDC = NULL;
            }
        ...
        for (kout = 0; kout < nhistnodes; kout++)
        {
            ...
            /*INFO_message("Hist bin %d [%3.3f, %3.3f) [%d, %p]\n", kout, histogram[ kout ].bin_low,
             * histogram[ kout ].bin_high, histogram[ kout ].nbin, histogram[ kout ].nodes );*/
        }
    }
    ...
    {
        INFO_message(
            "Calculating degree centrality with threshold = %f and sparsity = %3.2f%% (%d)\n",
...
#pragma omp parallel if( nmask > 999 )
    {
        ...
        if (ithr == 0)
        {
            INFO_message("%d OpenMP threads started", nthr);
        }
#else
        ithr = 0; nthr = 1;
#endif

        /*-- For the progress tracker, we want to print out 50 numbers, figure out a number of loop
         * iterations that will make this easy */
        vstep = (int)(nmask / (nthr * 50.0f) + 0.901f); vii = 0;
        if ((MEM_STAT == 0) && (ithr == 0))
        {
...
#pragma omp for schedule(static, 1)
...

        {
            ...
            {
                vii++; if (vii % vstep == vstep / 2 && MEM_STAT == 0)
                {
                    vstep_print();
                }
            }
            ...
            /* try to make calculation more efficient by only calculating the unique correlations */
...
/* update degree centrality values, hopefully the pragma will handle mutual exclusion */
#pragma omp critical(dataupdate)
                {
                    ...
                    if (car > thresh)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                /* this error should indicate a programming error and should not
                                 * happen */
                                WARNING_message("Node index %d is out of range [%d,%d)!",
                                                new_node_idx,
                                                bottom_node_idx, nhistnodes);
                            }
                            else
                            {
                                /* create a node to add to the histogram, try to use a recycled node
                                 * to save time and memory */
                                if (recycled_nodes == NULL)
                                {
                                    new_node = (hist_node *)calloc(1, sizeof(hist_node));

                                    /* -- update running memory estimate to reflect memory
                                     * allocation */
                                    INC_MEM_STATS(sizeof(hist_node), "hist nodes");
                                    if ((totNumCor % (1024 * 1024)) == 0)
                                    {
                                        PRINT_MEM_STATS("hist nodes");
                                    }
                                }
                                else
                                {
                                ...
                                else
                                {
                                    ...
                                    /* see if there are enough correlations in the histogram for the
                                     * sparsity */
                                    if ((totNumCor - histogram[bottom_node_idx].nbin) > nretain)
                                    {
                                        /* push the histogram nodes onto the list of recycled nodes,
                                         * it could be that this hist bin is empty, in which case we
                                         * have nothing to add */
                                        ...
                                        else
                                        {
                                            ...
                                            {
                                                WARNING_message(
                                                    "Trying to remove histogram bin that contains %d values, but whose head or tail pointer is NULL\n",
    ...
    if (abuc)
    {
        EDIT_dset_items(cset,
                        ADN_prefix, prefix,
                        ADN_nvals, nsubbriks,            /* 2 subbricks, degree and weighted
                                                          * centrality */
    ...
    /* CC this sets the subbrik scaling factor, which we will probably want to do again after we
     * calculate the voxel values */
    ...
    else
    {
         ...
         *    at first we add in values a whole bin at the time until we get to a point where we
         *need to add in a partial bin, then we create a new histogram to sort the values in the bin
         *and then add those bins at a time */
        kout = nhistnodes - 1;
        ...
        /* if we haven't used all of the correlations that are available, go through and add a
         * subset of the voxels from the remaining bin */
        if ((nretain > 0) && (kout >= 0))
        {
            ...
            h2binwidth = (((1.0 + binwidth / ((float)h2nbins)) * histogram[kout].bin_high) -
                          histogram[kout].bin_low) /
                         ((float)h2nbins);
            ...
            for (kin = 0; kin < h2nbins; kin++)
            {
                 ...
                 * ].bin_low, histogram2[ kin ].bin_high, histogram2[ kin ].nbin, histogram2[ kin
                 * ].nodes );*/
            }
            ...
            while (histogram[kout].nodes != NULL)
            {
                ...
                h2ndx = (int)floor((double)(hptr->corr - histogram[kout].bin_low) /
                                   (double)h2binwidth);
                ...
                else
                {
                    WARNING_message("h2ndx %d is not in range [0,%d) :: %.10f,%.10f\n", h2ndx,
                                    h2nbins, hptr->corr, histogram[kout].bin_low);
                }
            }
            ...
            {
                int nbins_rem = 0;
                for (ii = 0; ii < nhistnodes; ii++)
                {
                    nbins_rem += histogram[ii].nbin;
                }
                if (histogram != NULL)
                {
                    histogram = free_histogram(histogram, nhistnodes);
            ...
            while ((nretain > 0) && (kin >= 0))
            {
                ...
                while (hptr != NULL)
                {
                    ...
                    if ((ii >= DSET_NVOX(cset)) || (jj >= DSET_NVOX(cset)))
                    {
                        ...
                        {
                            WARNING_message(
                                "Avoiding bodset, wodset overflow (jj) %d > %d\n (%s,%d)\n",
                                jj, DSET_NVOX(cset), __FILE__, __LINE__);
                        }
                    }
                    else
        ...
        {
            WARNING_message(
                "Was not able to meet goal of %d (%3.2f%%) correlations, %d (%3.2f%%) correlations passed the threshold of %3.2f, maybe you need to change the threshold or the desired sparsity?",
                ngoal, 100.0 * ((float)ngoal) / ((float)totPosCor), totNumCor,
                100.0 * ((float)totNumCor) / ((float)totPosCor), thresh);
        }
    }
    ...
    /* increment our memory stats, since we are relying on the header for this information, we
     * update the stats before actually freeing the memory */
