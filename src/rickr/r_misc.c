 ...
 *
 * Convert up to 8 hex digits into a single unsigned long integer. The input string must consist of
 *hex type characters, without a leading 0x.  i.e. '0'->'9', 'a'->'f' and 'A'->'F'.
 *
 ...
 *
 * Convert the low-order number of bytes from the unsigned long into a null-terminated string hex
 *characters (strlen() is twice the number of converted bytes).
 *
 * We input the option to pad the string with zeros if the number is too small for the requested
 *number of bytes.
 *
...
/* allocate and catenate the strings into one long one, using the given separator string 12 May,
 * 2014 [rickr] */
char * cat_strings(char *slist[], int nstr, char *sepstr)
{
    ...
    for (sind = 1; sind < nstr; sind++)
    {
        if (slist[sind])
        {
            total += strlen(slist[sind]) + seplen;
        }
    }
...
/* display sequence of slist strings to FILE stream, from bot to top (follow with newline)
 *
 * example: disp_strings(stderr, "-- prior args: ", argc, argv, nopt-3, nopt-1, " ", 1);
 *
 *      fp      : FILE pointer to write to nstr    : total number of strings in list, just for
 *checking slist   : array of strings bot     : bottom index to display top     : top index to
 *display sepstr  : separation string, e.g. " ", "\n", "\n   "
 *
