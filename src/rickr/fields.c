...
char * field_type_str(int type)
{
    ...
    if (type == FT_DT_STRING)
    {
        return "FT_DT_STRING";
    }
    if (type == FT_DT_POINTER)
    {
        return "FT_DT_POINTER";
    }
    if (type == FT_DT_CHAR_PTR)
    {
        return "FT_DT_CHAR_PTR"; /* longest: 14 */
    }
               ...
               int header, int hex)
{
    ...
    for (c = 0; c < nfields; c++, fp++)
    {
        ...
        case FT_DT_STRING:
        {
            ...
            if (hex)
            {
                disp_raw_data(charp, fp->type, fp->len, ' ', 1, hex);
            }
            else
            {
                fprintf(stdout, "%.*s\n", fp->len, charp);
                       ...
                       field_s *fields, int flen)
{
    ...
    for (lc = 0; lc < names->len; lc++)
    {
        ...
        {
            if (strcmp(names->list[lc], fp->name) == 0)
            {
                break;
            }
        }
