...
int main(int argc, char *argv[])
{
    ...
    if (gopts.swap == 2)
    {
        swap_2(data, gopts.block_len / 2);
    }
    else if (gopts.swap == 4)
    {
        swap_4(data, gopts.block_len / 4);
    }
    ...
    while (!done && (gopts.nblocks <= 0 || nblocks < gopts.nblocks))
    {
        ...
        if (len > 0)
        {
            done = send_serial(&gopts, &gcontr, data, len);
        }
        else
        {
            done = 1;
    ...
    if (gopts.debug)
    {
        fprintf(stderr, "-d wrote %d blocks of data\n", nblocks);
    }
...
int set_data(opts_t *opts, control_t *contr, char *data)
{
    ...
    if (c < 0)
    {
        fprintf(stderr, "** read nothing from input stream\n");
    }
 ...
 * display usage
 *
 * SW_USE_SHORT   : most basic usage info SW_USE_LONG    : display complete description of program
 *and options SW_USE_VER     : display version and compile date
 * ----------------------------------------------------------------------
 */
int usage(char *prog, int level)
{
