...
/* Keep a copy of a single image/volume as the size to send for end of run (maybe lost at commit
 * 030341058c7c759639ddb26ed910661e73e887d5, 4 Jan 2011, when no longer sending via alloc'd
 * im_store.x_im).  The afni server will attempt to read the same image size that it has been, so
 * sending less would cause it to hang.
 *
 * Allocate xim of size xim_size, required to be >= xim_last_vsize. But always send using
 *xim_last_vsize.       6 Sep 2017 [rickr]
 */
...
/*****************************************************************************/
{
    ...
    if (ac->mode <= 0 || ac->mode == AFNI_CONTINUE_MODE)
    {
        return 0;
    }
    ...
    if (ac->mode == AFNI_OPEN_DATA_MODE)
    {
        ...
        else
        {
            ...
            else
            {
                if (debug > 1)
                {
                    fprintf(stderr, "ART: Entering AFNI_CATCHUP_MODE.\n");
                }
    ...
    if (ac->mode == AFNI_CATCHUP_MODE)
    {
        ...
        else if (ii > 0)                        /* can now send data to AFNI! */
        {
            ...
            if (debug > 1)
            {
                fprintf(stderr, "ART: Entering AFNI_CONTINUE_MODE.\n");
            }
        }
    }
 ...
 *
 * If the (run,seq) pair is new, send a message to afni. There is image space in x_im.
 *----------------------------------------------------------------------
 */
int ART_send_end_of_run(ART_comm *ac, int run, int seq, int debug)
{
    ...
    if (ac->state != ART_STATE_IN_USE)
    {
        if (debug > 3)
        {
            fprintf(stderr, "ART: end of run, but not in use\n");
        }
        return 0;
    }
    ...
    if (xim_size < xim_last_vsize)
    {
        if (debug > 1)
        {
            fprintf(stderr, "ART: xim alloc: %d -> %d bytes\n",
                    xim_size, xim_last_vsize);
        }
        xim_size = xim_last_vsize;
        if (!xim)
        {
            xim = malloc(xim_size * sizeof(char));
        }
        else
        {
            xim = realloc(xim, xim_size * sizeof(char));
        }
    }
...
int ART_open_afni_link(ART_comm *ac, int num_tries, int again, int debug)
{
    ...
    if (debug > 1)
    {
        fprintf(stderr, "ART: starting I/O to afni\n");
    }
...
int ART_send_control_info(ART_comm *ac, vol_t *v, int debug)
{
    ...
    if (v->minfo.im_is_volume)
    {
        if (debug > 1)
        {
            fprintf(stderr, "ART COMM: mosaic/volume of %d slices\n",
                    v->minfo.nslices);
        }
        nim = v->minfo.nslices;
    }
    ...
    /* NEW: 3D+timing is sorted by volume (see num_chan>1) but has slice timing info (3D+t does not)
     * 3 Aug 2015 */
