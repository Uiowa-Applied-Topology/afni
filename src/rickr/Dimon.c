 ...
 *
 *     This program is intended to be run during a scanning session on a GE scanner, to monitor the
 *collection of 2D image files. The user will be notified of any missing slice or any slice that is
 *acquired out of order.
 *
 *     It is recommended that the user runs 'Dimon' when scanning begins, and then watches for error
 *messages during the scanning session.  The user should terminate the program whey they are done
 *with all runs.
 *
 *     - When acquiring GEMS 5.x I-files, users should run Dimon once for the entire scanning
 *session.
 *
 *     - When acquiring DICOM image files, users should run Dimon once per run.
 *
 *     At the present time, the user must use <ctrl-c> to terminate the program.
 *
 *   usage: Dimon [options] -infile_pattern input_file_format
 *
 *   examples:    Dimon -infile_pattern 's12345/i*'
 *                Dimon -help Dimon -version Dimon -infile_prefix s12345/i -gert_create_dataset
 *Dimon -infile_list my_files.txt -quit Dimon -infile_prefix 's12345/i' -rt -host pickle -quit
 *----------------------------------------------------------------------
 */
...
/* modulo sorting: sorting on parameter P, given g_mod_sort/_base This is intended to group as
 * echo-major order of slices per time point. One time point probably has multiple echoes.
 *
 *    major = (P-g_mod_sort_base) // g_mod_sort   (offset time point) minor = (P-g_mod_sort_base) %
 * g_mod_sort   (slice within time point)
 */
...
{
    if (ftype == IFM_IM_FTYPE_DICOM)
    {
        return "DICOM";
    }
    else if (ftype == IFM_IM_FTYPE_GEMS5)
    {
        return "GEMS5";
    }
    else if (ftype == IFM_IM_FTYPE_AFNI)
    {
        return "AFNI";
    }
...
{
    if (!name || !strcmp(name, "DICOM"))
    {
        p->ftype = IFM_IM_FTYPE_DICOM;
    }
    else if (!strcmp(name, "GEMS"))
    {
        p->ftype = IFM_IM_FTYPE_GEMS5;
    }
    else if (!strcmp(name, "AFNI"))
    {
        p->ftype = IFM_IM_FTYPE_AFNI;
    }
    else
    {
 ...
 *
 * This function runs until either a volume is located, or an error occurs.
 *
...
static int find_first_volume(vol_t *v, param_t *p, ART_comm *ac)
{
    ...
    while (ret_val == 0)
    {
        ...
        {
            /* no_wait: if ret_val ever repeats (including start at 0), fail (might go 40, 80,
             * 120...)          16 Feb 2012 [rickr] */
            if (p->opts.no_wait)
            {
                if (nfiles == 0)     /* no new files read */
                {
                    ...
                    {
                        fprintf(stderr, "   (consider -save_details)\n");
                    }
                    fputc('\n', stderr);
                    return -1;
                }
...
int update_max2read(param_t *p, int max)
{
    ...
    /* if doing modulo sorting and it is set, possibly increase max2read to be a multiple of the
     * modulo value */
    if (g_mod_sort > 0)
    {
...
static int find_more_volumes(vol_t *v0, param_t *p, ART_comm *ac)
{
    ...
    if (p->opts.sleep_vol > 0)
    {
        nap_time = p->opts.sleep_vol;
    }
    else
    {
        nap_time = ms_nap_time;
    }
    ...
    {
        max_naps = tr_naps * IFM_MAX_RUN_NAPS;
    }
    ...
    while (!done)
    {
        ...
        while (vmrval == 1)
        {
            ...
            if (gD.level > 2)
            {
                fprintf(stderr, "-- vol match returns %d\n", vmrval);
            }

            if (vmrval < -3)
            {
                return vmrval;                      /* bail on fatal error */
            }
            else if (vmrval == -3)
            {
                continue;                        /* new starting point */
            }
                ...
                (p->opts.nt > 1 && seq_num >= p->opts.nt))
            {
                ...
                if (gD.level > 2)
                {
                    disp_obl_info("+d new run obl info: ");
                }
                if (gD.level > 0)
                {
                    fprintf(stderr, "\n-- run %d: %d ", run, seq_num);
        ...
        if (vmrval == -1)
        {
            continue;                   /* no napping before next check */
        }

        /* wait while no fatal error and no full volume, also, wait if we have not processed new
         * images (full volume error?)*/
...
int nap_for_ms(int ms)
{
    if (gD.level > 2)
    {
        fprintf(stderr, "-d napping for %d ms...\n", ms);
    }
    iochan_sleep(ms);
    return 0;
}
 ...
 *
 * state:     0 :  < 2 slices found (in the past) 1 : >= 2 slices (new 'bound') 2 : >= 2 slices
 *(repeated 'bound', may be one volume run)
 *
 * return:   -2 : on programming error
 *           -1 : on data error 0 : on success - no volume yet 1 : on success - volume read
 *----------------------------------------------------------------------
 */
    ...
    int *state)                         /* assumed state of search        */
{
    ...
    if (bound == prev_bound && first == prev_first)
    {
        if (*state == 1)
        {
            ...
            if (bound_cnt >= MAX_SEARCH_FAILURES)
            {
                *state = 2;
            }
        }
    ...
    if (rv == 1)
    {
        ...
        if (read_obl_info)
        {
            ...
            if (gD.level > 2)
            {
                disp_obl_info("postV mri_read_dicom_get_obliquity ");
            }
        }
    ...
    {
        return -1;                          /* had some reported error */
    }
}

/* updates states from first to last (if new state is not TO_PROC and we have an image, free it) */
 ...
 *           -1 : found direction change 0 : no error, no volume yet 1 : found volume
 *----------------------------------------------------------------------
 */
int check_one_volume(param_t *p, int bound, int state,
                     int *r_first, int *r_last, float *r_delta)
    ...
    *r_delta = -1.0;
    ...
    if ((fabs(delta) < gD_epsilon) || (run1 != run0))
    {
        ...
        if (!IM_IS_GEMS(p->ftype))
        {
            if (gD.level > 1)
            {
                fprintf(stderr, "+d found single slice volume\n");
            }
    ...
    /* also, modify detla to be an average across 'valid' dz, to prevent exacerbating a poor initial
     * value (for B Benson)      30 Aug 2011 */
    zsum   = 0.0;
    zcount = 0;
    ...
    *r_delta = delta;  /* was just delta  30 Aug 2011 */
    ...
    if (fabs(fp->geh.zoff - p->fim_o[first].geh.zoff) < gD_epsilon)
    {
        ...
        if (gD.level > 1)
        {
            ...
            if (errs)
            {
                fprintf(stderr, "   (recovered after skipping images)\n");
            }
        }
        return 1;  /* success */
    }

    /* If we are still waiting for the same location, either keep waiting or figure there is only
     * one volume to read. */
    if ((fabs(dz - delta) < gD_epsilon) && (run1 == run0))
    {
        if (state < 2)
        {
            return 0;              /* not done yet, keep waiting */
        }
 ...
 *
 * Setting num_slices prevents early termination of the first volume when the first slice of second
 *volume is seen before the entire first volume.
 *----------------------------------------------------------------------
 */
 ...
 *           -1 : small error: no nap needed 0 : success:     no volume yet 1 : success:     volume
 *read
 *----------------------------------------------------------------------
 */
static int volume_match(vol_t *vin, vol_t *vout, param_t *p)
{
    ...
    for (count = 0; count < vin->nim; count++)
    {
        ...
        if (!check_error(&retry, vin->geh.tr, estr))
        {
            return -2;
        }
 ...
 *
 * return:  < 0  : on programming error 0  : do not report error yet 1  : report error
 *----------------------------------------------------------------------
 */
static int check_error(int *retry, float tr, char *note)
{
 ...
 *    - a hard limit on number read would be possible with frequent reading (so a TR's worth could
 *be accumulated, say)
 * - apply any sort based on image info: fim_o => fim->s
 * - process unprocessed images
 *
 * return: < 0 : on error 0 : nothing to process 1 : something new happened
 *----------------------------------------------------------------------
 */
static int read_image_files(param_t *p)
{
    ...
    if (p->fim_skip < 0)
    {
        return 0;                   /* still waiting for starting file */
    }
    ...
    if (gD.level > 3)
    {
        idisp_param_t("end read_image_files : ", p);
    }
 ...
 *    - states <= DONE should trickle to the beginning (every such image should be an error
 *condition)
...
static int make_sorted_fim_list(param_t *p)
{
    ...
    /* The first fim_start pointers should be left untouched. There should be no _DONE structures
     * above that point, as fim_start should be incremented with completed volumes. Sort the
     * remaining pointers pulling any "completed" ones down early (FAILED, SKIP, UNKNOWN, DONE
     * (should not happen)).
     *
    ...
    if (n2sort <= 0)
    {
        if (gD.level > 2)
        {
            fprintf(stderr, "-- fim_o: no sorting to do\n");
        }
        return 0;
    }
    ...
    if (method == IFM_SORT_UNSPEC)
    {
        ...
        {
            method = IFM_SORT_ACQ_TIME;
        }
        else if (p->opts.sort_num_suff)
        {
            method = IFM_SORT_NUM_SUFF;
        }
        else if (p->opts.dicom_org)
        {
            method = IFM_SORT_DEFAULT;
        }
    }
    ...
    if (p->opts.order_as_zt)
    {
        finfo_order_as_zt(p, fp, n2sort);
    }

    /* after sorting, increment starting index to first TO_PROC (or later) image (if not found,
     * fim_start should be left unchanged) */
...
/* change order from slice-major (time changes fast, slices slow) to time-major order (slices change
 * fast, time slow)
 *
...
int sort_by_geme_index(param_t *p)
{
    ...
    if (gD.level > 2)
    {
        fprintf(stderr, "-- sorting by GE ME index :%s...\n",
                p->opts.sort_method);
    }
    n2sort = p->nfim - p->fim_start;
    if (n2sort <= 0)
    ...
    if (sort_method(p->opts.sort_method) == IFM_SORT_GEME_XNAT)
    {
        ...
        if (gD.level > 2)
        {
            g_nwarn_dupe_rin = 10;               /* juuuust to verify */
        }
    ...
    if (geme_set_range_n_state(p, p->fim_start, &min, &max, &snacq))
    {
        return 0;
    }
    ngeme = max - min + 1;
    ...
    if (ngeme > 10000)
    {
        if (gD.level > 0)
        {
            fprintf(stderr, "** SBGI: bad ngeme = %d\n", ngeme);
        }
        return 0;
    }
    if (nwarn == 0)
    ...
    if (s_geme_min == -1)
    {
        if (gD.level > 1)
        {
            fprintf(stderr, "-- have initial geme_min = %d\n", min);
        }
        s_geme_min = min;
    }
 ...
 *      mebase = geme_index - memin set sindex to list[mebase]*ngeme + mebase (offset sindex by
 *p->fim_start) list[mebase]++
 *
...
int geme_set_sort_indices(param_t *p, int_list *ilist, int ngeme, int memin)
{
    ...
    {
        if (ilist->list[ind] < nfull)
        {
            nfull = ilist->list[ind];
        }
    }
...
/* if g_mod_sort is 0, try to init, see if an update is needed This is currently used only for
 * IFM_SORT_ECHO_RIN or GEME_RIN.
 *    - sort by RIN
...
/* return a g_mod_sort val and base for ECHO_RIN method NOTE: rin is basically slice index (into one
 * volume/echo) NOTE: all n2proc images should be past state TO_READ
 *
 ...
 *
 *  fim list is assumed to be sorted by RIN already n2proc is number of images in PROC state
 *(starting from fim_start)
 *
                                    ...
                                    int *gm_sort, int *gm_base)
{
    ...
    /* if we do not have what we came for (and NO wait), there must only be one volume, set
     * next_base according to what we have (one more than the previous index) */
     ...
     *   - want nechos*nrin to equal next_base-rin_base (else missing slices?) (do not allow NE*NG
     *>= 2*(next_base-rin_base : fail 3 times?)
     */
...
/* return a g_mod_sort val and base for GEME_RIN method NOTE: geme_index is basically slice index
 * (into one volume/echo) NOTE: all n2proc images should be past state TO_READ
 *
                                    ...
                                    int *gm_sort, int *gm_base)
{
     ...
     *   - want nechos*ngeme to equal next_base-rin_base (else missing slices?) (do not allow NE*NG
     *>= 2*(next_base-rin_base : fail 3 times?)
     */
    /* note evaluated echoes and slices */
    necho       = max_echo - min_echo + 1;
...
/* for all images, if state == TO_PROC or state == TO_SORT, set state to TO_SORT if nacq == -1, try
 * to init from gex.ge_nim_acq
 *
 * note: there should really not be any TO_PROCs from previous sorts, as anything set to that should
 *have a complete volume
 *
                           ...
                           int *nacq)
{
    ...
    if (*nacq < 0)
    {
        *nacq = guess_predefined_nacq(p, ind);
    }
 ...
 *
 * if a repeat of min or max is found, min: back up until to previous RIN max: go forward until
 *different RIN (else, return nim-1)
 */
int geme_find_block_end(param_t *p, int start)
{
    ...
    if (meind == memin)
    {
        ...
        {
            while (c > start &&
                   p->fim_o[c].geh.index == p->fim_o[c2].geh.index)
            {
                c--;
            }
        }
...
int nfim2proc(param_t *p)
{
    ...
    if (p->nfim <= 0 || p->fim_start >= p->nfim)
    {
        return 0;
    }
    ...
    {
        if (p->fim_o[ind].state != IFM_FSTATE_TO_PROC)
        {
            break;
        }
    }
 ...
 *
 * - if states differ, sort on state (state values are arranged to make sorting appropriate)
...
int compare_finfo_t(const void *v0, const void *v1)
{
    ...
    if (p0->state != IFM_FSTATE_TO_PROC)
    {
...
static int append_new_files_to_fim_list(param_t *p)
{
    ...
    while (pind < p->fnames_prev.num)
    {
        ...
        {
            nadded++;
            if (append_new_finfo_entry(p, f_cur[cind]))
            {
                return -1;
            }
            cind++;
 ...
 *
 * The fim_o list gets sorted starting at fim_start. Note that append should always work.
 *
 */
int append_new_finfo_entry(param_t *p, char *fname)
{
    if (gD.level > 3)
    {
        fprintf(stderr, "++ creating new finfo for %s\n", fname);
    }
 ...
 *
 * populate fnames_prev and _cur possibly set fnames_done
 *
 * return -1 on error 0 if nothing was done 1 something was read
 *----------------------------------------------------------------------
 */
static int get_sorted_file_names(param_t *p)
{
    ...
    if (gD.level > 3)
    {
        fprintf(stderr, "-- getting sorted fnames...\n");
    }
    ...
    {
        if (read_file_list(p->opts.infile_list, &fnames, &nfiles))
        {
            return -1;
        }
    ...
    if (gD.level > 2)
    {
        fprintf(stderr, "-- sorted fnames: from %d to %d\n",
 ...
 * read_file_list:
 *
 * Get the list of files from the input file. Populate fnames and nfiles.
 *
 * return 0 on success
 *----------------------------------------------------------------------
 */
...
static int read_new_images(param_t *p)
{
    ...
    if (p->nfim <= 0)
    {
        return 0;                               /* no names to read */
    }
    if (p->fim_skip < 0)
    {
        return 0;                               /* waiting to start */
    }
    if (p->nfim <= p->fim_skip)
    {
        return 0;                               /* waiting for more names */
    }
    ...
    /* if g_mod_sort, set more2read so resulting TO_PROC totals would be a multiple of g_mod_osrt */
    ...
    for (index = p->fim_skip; index < p->nfim; index++, fp++)
    {
        ...
        if (rv == 0)    /* success */
        {
            ...
            if (gD.level > 3)
            {
                idisp_finfo_t("++ have new image ", fp);
            }
    ...
    if (gD.level > 1)
    {
        fprintf(stderr, "-- RNI: read %d new images (nerrs = %d)\n",
                nread, nerrs);
    }
...
static int sort_by_num_suff(char **names, int nnames)
{
    ...
    if (gD.level > 2)
    {
        fprintf(stderr, "-- sort_by_num_suff...\n");
    }

    if (!names || nnames < 0 || !*names)
    {
        return -1;
    }
    if (nnames == 0)
    ...
    if (gD.level > 2)
    {
        fprintf(stderr, "-- sort_by_num_suff: errs = %d\n", errs);
    }
...
int compare_by_num_suff(const void *v0, const void *v1)
{
    ...
    if (n0 == DEATH_VALUE || n1 == DEATH_VALUE)
    {
        return -2;                                           /* error */
    }
    if (n0 < n1)
...
/*----------------------------------------------------------------------
 *  find a numerical suffix (previously required to be non-negative) 6 May 2010 [rickr]
 *
 *  return DEATH_VALUE : some error else        : numerical suffix
 *----------------------------------------------------------------------
 */
static int get_num_suffix(char *str)
{
    ...
    if (!cp || (cp >= (str + len - 1)) || !isdigit(*(cp + 1)))
    {
        ...
        /* so we are either looking at the beginning of the final integer, or did not find one */
        if (!isdigit(*cp))
        {
            return DEATH_VALUE;
        }
    ...
    if (len != 1)
    {
        return DEATH_VALUE;               /* bad things man, bad things */
    }
    return val;
}
...
int compare_finfo(const void *v0, const void *v1)
{
    ...
    if (g_sort_by_atime)
    {
        if (h0->atime < h1->atime)
        {
            g_sort_type |= 4; return -dir;
        }
        else if (h0->atime > h1->atime)
        {
            g_sort_type |= 4; return dir;
        }
    }
    ...
    if (h0->im_index < h1->im_index)
    {
        g_sort_type |= 8; return -dir;
    }
    else if (h0->im_index > h1->im_index)
    {
        g_sort_type |= 8; return dir;
    }

    /* 0020 0013: REL Instance Number */
    if (h0->index < h1->index)
    {
        g_sort_type |= 16; return -dir;
    }
    else if (h0->index > h1->index)
    {
        g_sort_type |= 16; return dir;
    }
...
/*----------------------------------------------------------------------
 * more direct comparison by RIN (after other bits) sort by:
 *       state run rin (can be modulo)
 *----------------------------------------------------------------------
 */
int compare_by_rin(const void *v0, const void *v1)
{
    ...
    /* possibly do modulo sorting (a time index should contain g_mod_sort slice images) (g_mod_sort
     * should be nslices_per_time_point = nechoes*nslices_per_vol)
     *
    ...
    else if (p0->geh.index > p1->geh.index)
    {
        g_sort_type |= 16; return dir;
    }
...
int compare_by_sindex(const void *v0, const void *v1)
{
    ...
    /* state major sort */
    if (f0->state != f1->state)
    {
        return f0->state - f1->state;
    }
 ...
 * init_options:
 *
 *     1. check usage: Imon -start_dir DIR 2. do initial allocation of data structures
 *
 * return  0 on success, with continuation 1 on success, but terminate
 *        -1 on error
 *----------------------------------------------------------------------
 */
static int init_options(param_t *p, ART_comm *A, int argc, char *argv[])
{
    ...
    if (set_ftype(p, p->opts.file_type))
    {
        return -1;
    }
    ...
    {
        if (dir_expansion_form(p->opts.start_dir, &p->glob_dir))
        {
            return -1;
        }
    }
    ...
    if (p->opts.rev_sort_dir)
    {
        rglob_set_sort_dir(-1);
    }
    if (p->opts.rev_org_dir)
    {
        g_dicom_sort_dir = -1;
    }
...
int sort_method(char *method)
{
    if (!method)
    {
        return IFM_SORT_UNSPEC;
    }

    if (!strcmp(method, "none"))
    {
        return IFM_SORT_NONE;
    }
    if (!strcmp(method, "acq_time"))
    {
        return IFM_SORT_ACQ_TIME;
    }
    if (!strcmp(method, "default"))
    {
        return IFM_SORT_DEFAULT;
    }
    if (!strcmp(method, "geme_index"))
    {
        return IFM_SORT_GEME;
    }
    if (!strcmp(method, "geme_suid"))
    {
        return IFM_SORT_GEME_SUID;
    }
    if (!strcmp(method, "geme_xnat"))
    {
        return IFM_SORT_GEME_XNAT;
    }
    if (!strcmp(method, "num_suffix"))
    {
        return IFM_SORT_NUM_SUFF;
    }
    if (!strcmp(method, "zposn"))
    {
        return IFM_SORT_ZPOSN;
    }
    if (!strcmp(method, "rin"))
    {
        return IFM_SORT_RIN;
    }
    if (!strcmp(method, "echo_rin"))
    {
        return IFM_SORT_ECHO_RIN;
    }
    if (!strcmp(method, "geme_rin"))
    {
        return IFM_SORT_GEME_RIN;
    }
 ...
 *  dir_expansion_form
 *
 *  sin    : must be in the form "...0[01]n", where n is a digit (note that "...0[01]n/..." is okay)
 *sexp   : memory is allocated for the output string of the form: "...[0-9][02468]n/I.*"
 *
...
static int read_afni_image(char *pathname, finfo_t *fp, int get_data)
{
    ...
    if (gD.level > 3)
    {
        idisp_mosaic_info("AFNI volume ", mp);
    }
...
static int read_dicom_image(char *pathname, finfo_t *fp, int get_data)
{
    ...
    if (obl_info_set == 2 && read_obl_info)
    {
        ...
        if (gD.level > 2)
        {
            disp_obl_info("post mri_read_dicom_get_obliquity ");
        }
    }
     ...
     *
     * Process the times by calling populate_g_siemens_times() after reading a single DICOM file. If
     *valid, g_siemens_timing_nused and g_siemens_timing_times[] will be set. Use
     *valid_g_siemens_times() to check against nslices and TR.    */
    if (check_timing)
    {
        check_timing = 0;
        ...
        if (gD.level > 2)
        {
            mri_sst_set_verb(tverb);                 /* and reset verb */
        }
    }
 ...
 *
 *  If image is already allocated, just check that bytes is correct. Else, allocate it.
 *
 ...
 *
 *  If a mosaic, copy all images into a sequence of them else, just copy the single image over.
 *
 *  return: 0 on success 1 on failure
 *----------------------------------------------------------------------
 */
static int copy_image_data(finfo_t *fp, MRI_IMARR *imarr)
{
    ...
    if (imarr->num > 1 && !fp->minfo.im_is_volume && nwarn > 0)
    {
        ...
        if (!nwarn)
        {
            fprintf(stderr, "   no more warnings will be printed...\n");
        }
    }
                         ...
                         int get_image, int need_memory)
{
    ...
    fread(str, 1, 4, imfile);                                             /* check for "IMGF" at
                                                                           * start of file */
    ...
    if (hdroff > 0 && hdroff + 256 < length)  /* can read from image header */
    {
        ...
        dy = fabs(xyz[4] - xyz[1]); if (dy > dx)
        {
            ii = 2; dx = dy;
        }
        dz = fabs(xyz[5] - xyz[2]); if (dz > dx)
        {
            ii = 3;
        }
        dx = xyz[ii + 2] - xyz[ii - 1]; if (dx < 0.)
        {
            ii = -ii;
        }
        switch (ii)
        ...
        dy = fabs(xyz[7] - xyz[4]); if (dy > dx)
        {
            jj = 2; dx = dy;
        }
        dz = fabs(xyz[8] - xyz[5]); if (dz > dx)
        {
            jj = 3;
        }
        dx = xyz[jj + 5] - xyz[jj + 2]; if (dx < 0.)
        {
            jj = -jj;
        }
        switch (jj)
        ...
        /* zmodify: get User Variable 17, a likely indicator of a new scan, info by S. Marrett,
         * location from S. Inati's matlab function GE_readHeaderImage.m
         */
...
static int idisp_param_t(char *info, param_t *p)
{
    ...
    if (info)
    {
        fputs(info, stream); fputc(' ', stream);
    }
...
static char * image_state_string(int state)
{
    if (state == IFM_FSTATE_FAILED)
    {
        return "FSTATE_FAILED";
    }
    else if (state == IFM_FSTATE_SKIP)
    {
        return "FSTATE_SKIP";
    }
    else if (state == IFM_FSTATE_UNKNOWN)
    {
        return "FSTATE_UNKNOWN";
    }
    else if (state == IFM_FSTATE_DONE)
    {
        return "FSTATE_DONE";
    }
    else if (state == IFM_FSTATE_TO_PROC)
    {
        return "FSTATE_TO_PROC";
    }
    else if (state == IFM_FSTATE_TO_SORT)
    {
        return "FSTATE_TO_SORT";
    }
    else if (state == IFM_FSTATE_TO_READ)
    {
        return "FSTATE_TO_READ";
    }
...
static char * ftype_string(int ftype)
{
    if (ftype == IFM_IM_FTYPE_GEMS5)
    {
        return "GEMS";
    }
    else if (ftype == IFM_IM_FTYPE_DICOM)
    {
        return "DICOM";
    }
    else if (ftype == IFM_IM_FTYPE_AFNI)
...
static int create_gert_script(stats_t *s, param_t *p)
{
    ...
    if (IM_IS_DICOM(p->ftype))
    {
        return create_gert_dicom(s, p);
    }
    else
    {
        return create_gert_reco(s, &p->opts);
    }
}
...
static int create_gert_dicom(stats_t *s, param_t *p)
{
     ...
     * (default is "FROM_IMAGE" if siemens timing info). Check that siemens timing info has correct
     *nz.      15 Apr 2011  */
    ...
    for (c = 0, num_valid = 0; c < s->nused; c++)
    {
        if (s->runs[c].volumes > 0)
        {
            if (first_run < 0)
            {
                first_run = c;                  /* note first valid run */
            }
            num_valid++;
        }
    }
    ...
    for (c = 0; c < s->nused; c++)
    {
        ...
        if (s->runs[c].volumes > 1)
        {
            ...
            if (opts->gert_nz)
            {
                nslices = opts->gert_nz;
            }
            else if (s->mos_nslices > 1)
            {
                nslices = s->mos_nslices;
            }
        ...
        if (opts->gert_outdir)
        {
            char *suffix = "";
            /* reconcile write_as_nifti and NIFTI prefix */
            if (nifti_find_file_extension(pname))
            {
                suffix = "";
            }
            else if (opts->gert_format == 1)
            {
                suffix = ".nii";
            }
            else
...
int create_dimon_file_lists(param_t *p, char *ret_fname, stats_t *ST, int rind)
{
    ...
    for (cind = 0; cind < nchan; cind++)
    {
        ...
        if (nchan > 1)
        {
            sprintf(fname + fbase_len, ".chan.%0*d",
...
FILE * get_file_pointer(char *fname, char *mesg, int index)
{
    ...
    else  /* actually try to open */
    {
        if (index)
        {
            ...
            if (mesg)
            {
                sprintf(fnew, "%s.%d.%s.txt", fname, index, mesg);
            }
            else
            {
                sprintf(fnew, "%s.%d.txt", fname, index);
            }
            if (gD.level > 0)
            {
                fprintf(stderr, "-- writing details to %s...\n", fnew);
            }
        }
        else
        {
    ...
    if (gD.level > 2)
    {
        fprintf(stderr, "+d opening file for '%s'...\n",
 ...
 *
 *    details:    0 - output just a file list 1 - output a table of many details 2 - output details
 *to verify GEME index sorting
 *
...
static int create_file_list(param_t *p, char *fname, int details, char *mesg)
{
    ...
    if (gD.level > 0)
    {
        fprintf(stderr, "-- writing file list to %s...\n",
                CHECK_NULL_STR(fname));
    }
    if (mesg)
    {
        fputs(mesg, stderr); fputs(" : ", stderr);
    }
    ...
    if (gD.level > 2)
    {
        fprintf(stderr, "-- file list written for %s\n",
...
int test_sop_iuid_index_order(param_t *p)
{
    ...
    if (!err)
    {
        fprintf(stderr, "++ %d IUID's are sequential, yay\n", p->nfim);
    }
 ...
 *
 * The hope is that 1000*major+minor could be used to index the echo set, though not individual
 *slices within that set.  It would mean that iuid_major/minor could be used as a pre-sort for
 *geme_index.
 *
 * So the final iuid should be the ordered *slice* index for the first slice in one set of echoes.
 *
 * Finally, to that we add the geme_index (offset from its base), to return an overall and unique
 *slice index for the entire run.
 */
int get_sop_iuid_index(ge_extras *ep)
{
    ...
    /* -- this could almost be a global slice index, except that it seems to stem from asynchronous
     * writing, and might be out of order within a single echo set */
    iuid = 1000 * (iuid_major - base_major) + (iuid_minor - base_minor);

    /* truncate to multiple of base_ngene */

    /* -- so to correct for iuid not be quite accurate, we first truncate to a multiple of ngeme */
    if (base_ngeme > 0)
    {
        iuid -= (iuid % base_ngeme);
    }

    /* return addition of geme (with offset) */
    /* -- before adding the geme_index offset to that ngeme multiple */

    /* -- this we hope to be a global, incrementing slice index, ordered by: echo set index, echo
     * number, volume slice index */
    return iuid + geme_index - base_geme;
}

/*----------------------------------------------------------------------
 * remove trailing zeros from string of printed float return  1 if something was cleared 0 if not
 *----------------------------------------------------------------------*/
static int clear_float_zeros(char *str)
{
...
static int show_run_stats(stats_t *s)
{
    ...
    if (s->mos_nslices > 1)
    {
        if (IM_IS_AFNI(gP.ftype))
        {
            tstr = "(AFNI volume)";
        }
        else if (IM_IS_DICOM(gP.ftype))
        {
            tstr = "(DICOM mosaic)";
        }
        else
...
/* ----------------------------------------------------------------------
 * given tr (in seconds), return a sleep time in ms (approx. 1.1*TR), to make it likely that data is
 *found
 *
...
static int nap_time_in_ms(float t1, float t2)
{
    ...
    if (nap_time < 10)
    {
        return 10;                              /* 10 ms is minimum */
    }
    if (nap_time > 30000)
    {
        return 30000;                           /* 30 sec is maximum */
    }
    if (gD.level > 1)
    {
 ...
 * find_next_zoff
 *
 * Given p->fim_o, search from index start for an image with geh.zoff equal to zoff.
 *
...
/* ----------------------------------------------------------------------
 * Given:  run     > 0 seq_num > 0 naps tr_naps naps_time    in ms
 *
 * If naps is too big, and the run is incomplete, print an obnoxious warning message to the user.
 *
 ...
 *
 * returns:
 *          2 : run is stalled - message printed 1 : apparent end of a run 0 : no stall, or if a
                             ...
                             int nap_time)
{
    ...
    if ((run < 1) || (seq_num < 1) || (naps <= max_naps))
    {
        return 0;
    }
 ...
 *
 * Note our byte order (LSB_FIRST or MSB_FIRST). If gex.swap is set, reverse it.  If user wants
 *opposite, reverse it.
 *
 * return   0 : success
 *         -1 : on error
...
/* ----------------------------------------------------------------------
 * Use gex.kk to figure out the z orientation, and complete the v->geh.orients string.
 *
...
static int set_starting_fim(param_t *p)
{
    ...
    if (sd)
    {
        ...
        {
            if (!strncmp(sd, flp[index].fname, dlen))
            {
                found = 1; break;
            }
        }
    ...
    if (gD.level > 2)
    {
        if (found)
        {
            fprintf(stderr, "-d found starting entry %s at index %d\n",
                    flp[index].fname, index);
        }
        else
        {
            fprintf(stderr, "-d no starting entry match found (yet)...\n");
        }
    }
 ...
 *
 * dir    - 3 character starting directory           (e.g. 003) suff   - 10 char (max) trailing
 *I-file suffix     (e.g. 017, for I.017) path   - first file in run                        (e.g.
 * 003/I.017)
 *
 * return  0 : success
 *        -1 : failure
...
int disp_obl_info(char *mesg)
{
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        for (j = 0; j < 4; j++)
        {
            fprintf(stderr, "%10.4f  ", obl_info.Tr_dicom[i][j]);
        }
        fputc('\n', stderr);
    }
