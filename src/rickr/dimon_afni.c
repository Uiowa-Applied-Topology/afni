 ...
 * dimon_afni.c
 *
 * This is mostly mri_read_dicom.c, plus some functions solen from other files.  It is for reading
 *Dicom files, but without having to link libmri.a (and also libXm, to be specific).
 *
 * May, 2005 [rickr] : changes from mri_read_dicom.c:
 *
 *   - inserted some of the MRILIB globals, but using DI_MRL (search)
 *   - added (formerly external) functions: swap_twobytes(), swap_fourbytes(), set_endianosity(),
 *mri_possibly_dicom()
...
/* modify mri_read_dicom for Dimon's purposes   April 2005 [rickr] note: return a single MRI_IMAGE,
 * not an MRI_IMARR */
MRI_IMAGE * r_mri_read_dicom(char *fname, int debug, void **data)
{
    ...
    if (!mri_possibly_dicom(fname))
    {
        if (debug > 2)
        {
            fprintf(stderr, "** MRD: mri_possibly_dicom() failure\n");
        }
    ...
    if (ppp == NULL)
    {
        if (debug > 2)
        {
            fprintf(stderr, "** MRD: mri_dicom_hdr() failure\n");
        }
    ...
    { unsigned int psiz, fsiz;
      ...
      if (fsiz < psiz)
      {
          if (debug > 2)
          {
              fprintf(stderr, "** MRD: fsiz < psiz (%d,%d)\n", fsiz, psiz);
          }
          free(ppp);
          RETURN(NULL);
      }
    ...
    if (epos[E_SAMPLES_PER_PIXEL] != NULL)
    {
        ...
        if (ddd == NULL)
        {
            if (debug > 2)
            {
                fprintf(stderr, "** MRD: missing E_SAMPLES_PER_PIXEL\n");
            }
            free(ppp);
            RETURN(NULL);
        }
        ...
        if (ii != 1)
        {
            if (debug > 2)
            {
                fprintf(stderr, "** MRD: SAM_PER_PIX != 1, %d\n", ii);
            }
            free(ppp);
            RETURN(NULL);
        }
    ...
    if (epos[E_PHOTOMETRIC_INTERPRETATION] != NULL)
    {
        ...
        if (ddd == NULL)
        {
            if (debug > 2)
            {
                fprintf(stderr, "** MRD: photometric not monochrome\n");
            }
            free(ppp);
            RETURN(NULL);
        }
    ...
    if (ddd == NULL)
    {
        if (debug > 2)
        {
            fprintf(stderr, "** MRD: missing BITS_ALLOCATED\n");
        }
        free(ppp);
        RETURN(NULL);
    }
    ...
    default:
        if (debug > 2)
        {
            fprintf(stderr, "** MRD: bad bpp value, %d\n", bpp);
        }
    ...
    if (debug > 3)
    {
        fprintf(stderr, "-d dicom: datum %d\n", datum);
    }
...
#define NWMAX    2
    ...
    if (ddd == NULL)                  /* Oops: ROWS is ny and COLUMNS is nx! */
    {
        if (debug > 2)
        {
            fprintf(stderr, "** MRD: missing E_ROWS\n");
        }
        free(ppp);
        RETURN(NULL);
    }
    ...
    if (ddd == NULL)
    {
        if (debug > 2)
        {
            fprintf(stderr, "** MRD: missing E_COLUMNS\n");
        }
        free(ppp);
        RETURN(NULL);
    }
    ...
    if (epos[E_NUMBER_OF_FRAMES] != NULL)
    {
        ...
        if (debug > 3)
        {
            fprintf(stderr, "-d number of frames = %d\n", nz);
        }
    }
    ...
    if (nz == 0)
    {
        nz = plen / (bpp * nx * ny);         /* compute from image array size */
    }
    if (nz == 0)
    {
        if (debug > 2)
        {
            fprintf(stderr, "** MRD: bad nz = %d\n", nz);
        }
        free(ppp);
        RETURN(NULL);
    }
     ...
     *                instead, use the Siemens extra info in epos[E_SIEMENS_2].
     *                     --*/
        ...
        epos[E_SIEMENS_2] != NULL)
    {
        if (debug > 3)
        {
            fprintf(stderr, "-d ignoring sexinfo\n");
        }
    }
    ...
    if (debug > 3)
    {
        fprintf(stderr, "-d dicom: using dxyz = %f, %f, %f\n", dx, dy, dz);
    }
    ...
    if (epos[E_REPETITION_TIME] != NULL)
    {
        ...
        if (ddd != NULL)
        {
            ...
            if (debug > 3)
            {
                fprintf(stderr, "-d dicom: rep. time dt = %f sec.\n", dt);
            }
        }
    }
    ...
    if (epos[E_RS_STUDY_NUM] != NULL)
    {
        ...
        if (ddd != NULL)
        {
            sscanf(ddd + 2, "%d", &gr_dimon_stuff.study);
        }
    }
    ...
    if (epos[E_RS_SERIES_NUM] != NULL)
    {
        ...
        if (ddd != NULL)
        {
            sscanf(ddd + 2, "%d", &gr_dimon_stuff.series);
        }
    }
    ...
    if (epos[E_RS_IMAGE_NUM] != NULL)
    {
        ...
        if (ddd != NULL)
        {
            sscanf(ddd + 2, "%d", &gr_dimon_stuff.image);
        }
    }
    ...
    if (epos[E_RS_IMAGE_INDEX] != NULL)
    {
        ...
        if (ddd != NULL)
        {
            sscanf(ddd + 2, "%d", &gr_dimon_stuff.image_index);
        }
    }
    ...
    if (epos[E_ID_ACQUISITION_TIME] != NULL)
    {
        ...
        if (ddd != NULL)
        {
            sscanf(ddd + 2, "%f", &gr_dimon_stuff.acq_time);
        }
    }
    ...
    if (fp == NULL)
    {
        if (debug > 2)
        {
            fprintf(stderr, "** MRD: failed to open file '%s'\n", fname);
        }
        free(ppp);
        RETURN(NULL);
    }
    ...
    if (epos[E_IMAGE_ORIENTATION] != NULL)
    {
        ...
        if (ddd != NULL)
        {
...

            {
                ...
                dxx = fabs(xc1); ior = 1;
                dyy = fabs(xc2); if (dyy > dxx)
                {
                    ior = 2; dxx = dyy;
                }
                dzz = fabs(xc3); if (dzz > dxx)
                {
                    ior = 3;
                }
                dxx = DI_MRL_xcos[ior - 1]; if (dxx < 0.)
                {
                    ior = -ior;
                }
                ...
                dxx = fabs(yc1); jor = 1;
                dyy = fabs(yc2); if (dyy > dxx)
                {
                    jor = 2; dxx = dyy;
                }
                dzz = fabs(yc3); if (dzz > dxx)
                {
                    jor = 3;
                }
                dyy = DI_MRL_ycos[jor - 1]; if (dyy < 0.)
                {
                    jor = -jor;
                }
    ...
    /** use image position vector to set offsets, and (2cd time in) the z-axis orientation **/
    ...
    {  /* old mri_read_dicom: line 982 */
        ...
        /* fill the orients string */
        {
...

            {
                ...
                if (qoff < 0)
                {
                    kor = -kor;        /* kor determines z-axis orientation */
                }
#if 0
    ...
    /** 23 Dec 2002:
     *  use slice location value to set z-offset, and (2cd time in) the z-axis orientation
     *  -- only try this if image position vector (code above) isn't present AND if we don't have a
     *mosaic image (which already did this stuff)
     *  -- shouldn't be necessary, since slice location is deprecated        **/

    /* if this is here, use it for additional accuracy (it must basically match current zo) */
    ...
    { /* use three warning counters here */
        ...
        else
        {
            ...
            if (qq != 1)
            {
                if (!nwarn0)
                {
                    fprintf(stderr, "** failed to extract SLICE_LOCATION\n");
                }
                nwarn0++;
            }
            else
            {
                ...
                if (fabs(zz - im->zo) > gD_epsilon) /* 20 Nov 2006 [rickr] */
                {
                    ...
                    /* apply IMAGE_LOCATION and continue         29 Dec 2005 [rickr] free(ppp);
                     * free(im); if( data ){ free(*data); *data = NULL; }
                     * RETURN(NULL); */
                    nwarn2++;
                }
                else
...
static void Clear_obl_info(oblique_info *obl_info)
{
    ...
    if (g_debug > 3)
    {
        fprintf(stderr, "-- clearing oblique_info struct\n");
    }
...
static void Fill_obl_info(oblique_info *obl_info, char **epos)
{
    ...
    if (g_debug > 3)
    {
        fprintf(stderr, "-- filling oblique_info struct\n");
    }
    ...
    if (obl_info_set)
    {
        if (g_debug > 2)
        {
            fprintf(stderr, "-- obl_info_set: 1 --> 2\n");
        }
        if (g_debug > 3)
        {
            xyz = obl_info->dfpos1.xyz;
    ...
    if (g_debug > 2)
    {
        fprintf(stderr, "++ obl_info_set: pass 1 done\n");
    }
...
/*   double dotp, angle, aangle;*/
    ...
    if (g_debug > 3)
    {
        fprintf(stderr, "++ ComputeObliquity...\n");
    }

    /* compute cross product of image orientation vectors*/
    vec3 = CROSS_FVEC3(obl_info->xvec, obl_info->yvec);

    /* compute dfpos (difference between first and second ImagePositionPatient fields as a vector */
    ...
    if (g_debug > 3)
    {
        disp_obl_info("++ ComputeObliquity xform: ");
    }
...
int mri_read_dicom_get_obliquity(float *Tr, int verb)
{
    ...
    if (verb)
    {
        fprintf(stderr, "-- MRDGO: obl_info_set = %d\n", obl_info_set);
    }
    ...
    if (verb)
    {
        disp_obl_info("++ MRD_get_obl xform: ");
    }
...
static float get_dz(char **epos)
{
    ...
    if (!check_env)
    {
        eee = getenv("AFNI_SLICE_SPACING_IS_GAP"); check_env = 0;
    }
...
#define GFAC    0.99
        ...
        {
            if (sp > 0.0 && sp < GFAC * th)
            {
                dz = sp + th;                      /* the stupid GE way again */
            }
...
/*! Normally we will find the first occurrence of each element in the text. Maybe the user wants to
 * use the last, instead.     10 Apr 2009 [rickr]
 *---------------------------------------------------------------------------*/
