 ...
 *collection of I-files.  The user will be notified of any missing slice or any slice that is
 *acquired out of order.
 *
 *     It is recommended that the user runs 'Imon' before scanning begins, and then watches for
 *error messages during the scanning session.  The user should terminate the program whey they are
 *done with all runs.
 *
 ...
 *
 *   examples:    Imon -start_dir 003 Imon -help Imon -version Imon -start_dir 003 -GERT_Reco2 Imon
 *-start_dir 003 -debug 2 Imon -start_dir 003 -nt 120 Imon -start_dir 003 -rt -host pickle -swap
    ...
    while (ret_val == 0)
    {
        ...
        if (ret_val > 0)
        {
            ...
            if ((ret_val == 0) && (p->nused > (max_im_alloc / 2)))
            {
                 ...
                 * of our available memory, request more, making sure there is enough for a volume,
                 *despite the previous max_im_alloc limitation.
                 */
                max_im_alloc *= 2;
            }
        }
