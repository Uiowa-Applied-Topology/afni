...
static int find_first_volume(vol_t *v, param_t *p, ART_comm *ac)
{
    ...
    while (ret_val == 0)
    {
        ...
        if (gD.level > 1)
        {
            fprintf(stderr, "-- RGEFa: read %d files (total %d)\n",
                    nfiles, total_files);
        }
        ...
        if (ret_val > 0)
        {
            ...
            /* If we don't have a volume yet, but have used "too much" of our available memory,
             * request more, making sure there is enough for a volume, despite the previous
             * max_im_alloc limitation.  */
            if ((ret_val == 0) && (p->nused > (max_im_alloc / 2)))
            {
                /* if the user has specified num_slices and after more than 5 volumes worth we still
                 * have not found our first, call this a fatal error */
...
static int find_more_volumes(vol_t *v0, param_t *p, ART_comm *ac)
{
    ...
    while (!done)
    {
        ...
        while ((ret_val == 1) || (ret_val == -1))
        {
            ...
            if (ret_val < -1)
            {
                return ret_val;                 /* bail out on fatal error */
            }
        ...
        if (gD.level > 1)
        {
            fprintf(stderr, "-- RGEFb: read %d files\n", ret_val);
        }
               ...
               prev_images == next_im)   /* implies ret_val >= nim */
        {
            ...
            if (gD.level > 1)
            {
                fprintf(stderr, "-- RGEFc: read %d files\n", ret_val);
            }
        }
    ...
    int *state)                         /* assumed state of search        */
{
    ...
    {
        /* We have gone in the wrong direction.  This means that the starting slice was not the
         * first in the volume.  Try restarting from the current position.  */
        fprintf(stderr, "\n"
                "*************************************************\n"
                     ...
                     int *r_first, int *r_last, float *r_delta)
{
    ...
    *r_delta = delta;  /* was just delta  30 Aug 2011 */
    ...
    /* Also, if we are still waiting for the same location, but are in state 2, then we seem to have
     * only a single volume to read. */
 ...
 *           -1 : recoverable data error 0 : on success - no volume yet 1 : on success - volume read
 *----------------------------------------------------------------------
 */
static int volume_match(vol_t *vin, vol_t *vout, param_t *p, int start)
{
 ...
 *     - read GE structures
 *
 * return:       < 0  : on error files read : on success
 *----------------------------------------------------------------------
 */
    ...
    int max)                    /* max number of files to scan     */
{
    ...
    /* clear away old file list, unless we are using the DICOM organizer or have a given file_list
     */
    ...
    else
    {
        if (p->opts.infile_list)
        {
            if (org_todo)
            {
                ...
                if (p->opts.dicom_org && dicom_order_files(p))
                {
                    return -1;
                }
                if (p->opts.flist_file)
                {
                    create_flist_file(p);
                }
 ...
 * scan_ge_files:
 *
 * Starting at index 'next' of 'fnames', scan 'nfiles' files, filling the 'flist' array.
 *----------------------------------------------------------------------
 */
 ...
 *
 *   create an array of finfo_t structs sort the structures by run/index use the new list to sort
 *p->fnames
 *
...
static int dicom_order_files(param_t *p)
{
    ...
    if (p->opts.sort_acq_time)
    {
        if (gD.level > 0)
        {
            fprintf(stderr, "++ will sort by acquisition time\n");
        }
        g_sort_by_atime = 1;
    }
    ...
    for (c = 0; c < p->nfiles; c++)
    {
        ...
        else
        {
            ...
            if (gD.level > 0)
            {
                fprintf(stderr, "%c%c%c%c%c %3d%%", 8, 8, 8, 8, 8, scount);
            }
        }
    }
    if (gD.level > 0)
    {
        fprintf(stderr, "%c%c%c%c%c 100%%\n", 8, 8, 8, 8, 8);
    }
    if (gD.level > 0)
    {
        fprintf(stderr, "++ found %d DICOM files\n", dcount);
    }
    ...
    if (gD.level > 1)
    {
        fprintf(stderr, "-d dicom_org complete\n");
    }
 ...
 *
 * return  0 on success 1 on success, but terminate
 *        -1 on error
 *----------------------------------------------------------------------
 */
...
static int read_afni_image(char *pathname, finfo_t *fp, int get_data)
{
    ...
    if (fp->geh.tr == 0.0 && gP.opts.tr > 0.0)
    {
        fp->geh.tr = gP.opts.tr;
    }
    ...
    if (gD.level > 3)
    {
        idisp_mosaic_info("AFNI volume ", &fp->minfo);
    }
...
static int idisp_param_t(char *info, param_t *p)
{
    if (info)
    {
        fputs(info, stdout); fputc(' ', stdout);
    }
...
static int idisp_vol_t(char *info, vol_t *v)
{
    if (info)
    {
        fputs(info, stderr); fputc(' ', stderr);
    }
 ...
 * find_next_zoff
 *
 * Given p->flist, search from index start for an image with geh.zoff equal to zoff.
 *
 ...
 *
 * return  -1 : on error 0 : nothing needed 1 : memory successfully created
 * ----------------------------------------------------------------------
 */
static int check_im_store_space(im_store_t *is, int num_images)
{
 ...
 *
 * return  -1   : failed to find it else : index
 * ----------------------------------------------------------------------
 */
static int find_fl_file_index(param_t *p)
{
    ...
    if (gD.level > 2)
    {
        if (found)
        {
            fprintf(stderr, "-d found match at entry %d: %s\n", index, *nlp);
        }
        else
        {
            fprintf(stderr, "-d no match found (yet)...\n");
        }
    }
