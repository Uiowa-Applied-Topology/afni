...
/*-------------------------------------------------------------------
 * Fill in the blanks (zeros) in a row of shorts that are at most maxgap in length, and are bounded
 *by the same value. Return value is number of filled-in blanks.
 * ---------------------------------------------------------------------*/
...
static int THD_rowfillin_short(int nrow, short *row, int maxgap)
{
    ...
    for (ii = 0 ; ii < nrow && row[ii] == 0 ; ii++)
    {
        ;                                           /* nada */
    }
    if (ii == nrow)
    {
        return nfill;                                 /*** was all zeros ***/
    }
    ...
    while (1)
    {
        ...
        for ( ; ii < nrow && row[ii] != 0 ; ii++)
        {
            ;                                     /* nada */
        }
        if (ii == nrow)
        {
            return nfill;                      /*** didn't find any zero ***/
        }
#if 0
        ...
        for (jj = ii + 1 ; jj < nrow && row[jj] == 0 ; jj++)
        {
            ;                                             /* nada */
        }
        if (jj == nrow)
        {
            return nfill;                  /*** was all zeros to the end ***/
        }
#if 0
...
/*---------------------------------------------------------------------------
 * Do the fillin thing on each 1D row from a dataset sub-brick. Return value is number of voxels
 *filled in (-1 if an error transpired).
 * -----------------------------------------------------------------------------*/
...
int THD_dataset_rowfillin(THD_3dim_dataset *dset, int ival, int dcode, int maxgap)
{
    ...
    if (kind != MRI_short && kind != MRI_byte && kind != MRI_float)
    {
        RETURN(-1);                                                               /* bad */
    }
    ...
    {
    case 1:
    case -1: ytop = ny; ztop = nz; break;

    case 2:
    case -2: xtop = nx; ztop = nz; break;

    case 3:
    case -3: xtop = nx; ytop = ny; break;
    }
