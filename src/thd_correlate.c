...
float spearman_rank_corr(int n, float *x, float rv, float *r)
{
    ...
    xv = spearman_rank_prepare(n, x); if (xv <= 0.0f)
    {
        return 0.0f;
    }
...
/*! To do quadrant correlation of x[] with r[], first do rv = quadrant_corr_prepare(n,r) ;
 *  then corr = quadrant_corr(n,x,rv,r) ;
...
float quadrant_corr(int n, float *x, float rv, float *r)
{
    ...
    xv = quadrant_corr_prepare(n, x); if (xv <= 0.0f)
    {
        return 0.0f;
    }
...
float quantile_prepare(int n, float *a)
{
    ...
    if (jf <= 2.0f)
    {
        return spearman_rank_prepare(n, a);
    }
    jf = 1.0f / jf;
...
float THD_quantile_corr(int n, float *x, float *y)
{
    ...
    xv = quantile_prepare(n, x); if (xv <= 0.0f)
    {
        return 0.0f;
    }
    yv = quantile_prepare(n, y); if (yv <= 0.0f)
    {
        return 0.0f;
    }
...
void tictactoe_set_thresh(float bb, float tt)
{
    if (bb >= 0.0f && bb < tt && tt <= 1.0f)
    {
        ttt_bot = bb; ttt_top = tt;
    }
    else
    {
        ttt_bot = 0.3333333f; ttt_top = 0.6666667f;
    }
}
...
/*! To do tictactoe correlation of x[] with r[], first do rv = tictactoe_corr_prepare(n,r) ;
 *  then corr = tictactoe_corr(n,x,rv,r) ;
 *  Note that these 2 routines are destructive (r and x are modified).
 * -------------------------------------------------------------------------------*/
...
float tictactoe_corr(int n, float *x, float rv, float *r)
{
    ...
    xv = tictactoe_corr_prepare(n, x); if (xv <= 0.0f)
    {
        return 0.0f;
    }
...
float THD_tictactoe_corr(int n, float *x, float *y)
{
    ...
    yv = tictactoe_corr_prepare(n, y); if (yv <= 0.0f)
    {
        return 0.0f;
    }
    for (ii = 0, ss = 0.0f ; ii < n ; ii++)
...
/*! Returns a float_triple with (a,b,r) where y = a*x + b is the L2 regression result and r =
 * Pearson correlation coeff. For bootstrapping, ix[i] is the i-th index in x[] and y[] to use. For
 *non-bootstrapping, pass in ix==NULL.
 *//*-----------------------------------------------------------------------*/
 ...
 *   rrr = correlation coefficient, 2.5% level, 97.5% level  [in that order] aaa = regression 'a'
 * coefficient, in y=ax+b             [as .a .b .c  ] bbb = regression 'b' coefficient [components
 *  ]
 *//*-------------------------------------------------------------------------*/
                           ...
                           float_triple *bbb)
{
    ...
    if (n < 5 || x == NULL || y == NULL)
    {
        return;
    }
    if (rrr == NULL && aaa == NULL && bbb == NULL)
    {
        return;
    }
...
/* [PT: Nov 9, 2022] weighted Pearson corr--double time! */
double THD_pearson_corrd_wt(int n, double *x, double *y, float *wt)
{
    double xv = 0.0, yv = 0.0, xy = 0.0, vv, ww;
    double xm = 0.0, ym = 0.0, ws = 0.0;
    int    ii;

    if (wt == NULL)
    {
        return THD_pearson_corrd(n, x, y);
    }
...
#if 0
    ININFO_message("THD_pearson_corrd_wt: n=%d ws=%g xm=%g ym=%g xv=%g yv=%g xy=%g",
                   n, ws, xm, ym, xv, yv, xy);
#endif
...
float mri_spearman_corr(MRI_IMAGE *im, MRI_IMAGE *jm)
{
    ...
    if (im == NULL || jm == NULL || im->nvox != jm->nvox)
    {
        return 0.0f;
    }
...
float THD_eta_squared(int n, float *x, float *y)
{
    ...
    if (num < 0.0f || denom <= 0.0f || num >= denom)
    {
        return 0.0f;
    }
...
double THD_eta_squared_masked(int n, float *x, float *y, byte *mask)
{
    ...
    if (num < 0.0 || denom <= 0.0 || num >= denom)
    {
        return 0.0;
    }
...
/*! dice coefficient (allow for mask)          27 Oct 2015 [rickr] (float input)
 *
 ...
 * values below bot and above top are not used histogram can have bot and top bins unequal, and
 *in-between ones equal (if use_xyclip != 0) histogram can have all unequal bin sizes (if nxybin >
 * 2)
 * ----------------------------------------------------------------------------*/
...
static int nhbin = 0;
void set_2Dhist_hbin(int nn)
{
    nhbin = nn; clear_2Dhist();
}

int  get_2Dhist_hbin(void)
{
    return nhbin;
}
 ...
 *    - nb = number of bins; if nb < 3, equal sized bins will be used and the internal bin arrays
 *will be cleared
...
void set_2Dhist_xybin(int nb, float *xb, float *yb)
{
    ...
    if (nb > 2 && xb != NULL && yb != NULL)
    {
        ...
        {
            if (xb[ii - 1] >= xb[ii] || yb[ii - 1] > yb[ii])
            {
                break;
            }
        }
...
static float_pair clipate(int nval, float *xar)
{
    ...
    if (nq < 666)
    {
        rr.a = 1.0f; rr.b = 0.0f; mri_free(qim); RETURN(rr);
    }
    mmm = mri_min(qim);
...
void set_2Dhist_xyclip(int nval, float *xval, float *yval)
{
    ...
    use_xyclip = 0;
    if (nval < 666 || xval == NULL || yval == NULL)
    {
        EXRETURN;
    }
...
static int eqhighate(int nb, int nval, float *xar, float *xb)
{
    ...
    /** scan for plateaus = runs of constant values longer than nominal bin width, at the bottom and
     * top **/

    xbot = xb[0] = xd[0];
    xtop = xb[nb] = xd[nxd - 1]; if (xtop <= xbot)
    {
        free(xd); return 0;
    }
    bsiz = (nxd / nb);
    ...
    for (pp = 1 ; pp < nxd && xd[pp] < xtest ; pp++)
    {
        ;                                            /*nada*/
    }
    if (pp == nxd)
    ...
    for (pp = nxd - 2 ; pp > 0 && xd[pp] > xtest ; pp--)
    {
        ;                                              /*nada*/
    }
    if (pp <= pbot)
...
void set_2Dhist_xybin_eqhigh(int nb, int nval, float *xval, float *yval)
{
    ...
    FREEIF(xbin); FREEIF(ybin); nxybin = 0;
    if (nb < 3 || nval < 9 * nb || xval == NULL || yval == NULL)
    {
        return;
    }
...
/*! Sets pointers to the bins in 2D histogram-ization, and return value is the number of bins (if 0,
 * equal size bins are in use). Do NOT screw with the contents of the pointers: these are internal
 * arrays!
...
/*! Load 2D histogram of x[0..n-1] and y[0..n-1], each point optionally weighted by w[0..n-1]
 * (weights are all 1 if w==NULL). Used in the histogram-based measures of dependence between x[]
 * and y[i]. If something is bad on input, nbin is set to 0.  Otherwise, these global variables are
 * set:
 ...
 *    - Histogram can be retrieved by retrieve_2Dhist() and can be erased by clear_2Dhist()
 ...
 *    - x[] values outside the range xbot..xtop (inclusive) or outside the unequal bins set in
 *set_2Dhist_xybin() (if applicable) will not be used in the histogram; mutatis mutandum for y[]
 * ----------------------------------------------------------------------------*/
                         ...
                         float ybot, float ytop, float *y, float *w, int addon)
{
    ...
    else
    {
        ...
        if (xbot >= xtop)
        {
            clear_2Dhist(); free(good); EXRETURN;
        }
...

    {
        ...
        if (PRINT_TRACING)
        {
            char str[256];
            sprintf(str,
                    "equal size bins: xbot=%g xtop=%g ybot=%g ytop=%g nbin=%d nval=%d ngood=%d",
...

    {
        ...
        for (ii = 0 ; ii < n ; ii++)
        {
            ...
            {
                xx = 1.0f + (xx - xbc) * xi; jj = (int)xx; xx = xx - jj;
            }
            yy = y[ii];
            if (yy < ybc)
            {
            ...
            {
                yy = 1.0f + (yy - ybc) * yi; kk = (int)yy; yy = yy - kk;
            }
...

    {
        ...
        maxsort = 32 * nxybin; if (maxsort > 2048)
        {
            maxsort = 2048;
        }
        ...
        for (ib = 0 ; ib < n ; )
        {
            ...
            if (nb == 0)
            {
                break;       /* didn't find any good data ==> done! */
            }

            /* sort the extracted x data values in xdup[], and keep track of where they came from in
             * xn[] */

            qsort_floatint(nb, xdup, xn);

            /* scan through sorted xdup[] values, which will go into bin #kk which is xb..xt;
             * store the bin index and the fractional location within the bin;
             * the reason for doing the sorting is that finding the bin index kk will be efficient
            ...
            for (ii = 0 ; ii < nb ; ii++)
            {
                ...
                {
                    for (kk++ ; kk + 1 < nxybin && val > xbin[kk + 1] ; kk++)
                    {
                        ;                                           /*nada*/
                    }
            ...
            for (ii = 0 ; ii < nb ; ii++)
            {
                ...
                {
                    for (kk++ ; kk + 1 < nxybin && val > ybin[kk + 1] ; kk++)
                    {
                        ;                                           /*nada*/
                    }
                                  ...
                                  float ybot, float ytop, float *y)
{
...

    {
        ...
        maxsort = 16 * nxybin; if (maxsort > 512)
        {
            maxsort = 512;
        }
        xdup = (float *)malloc(sizeof(float) * maxsort);
        ...
        for (gg = ib = 0 ; ib < n ; )
        {
            ...
            if (nb == 0)
            {
                break;       /* didn't find any good data ==> done done! */
            }
...
void THD_correlate_ignore_zerozero(int z)
{
    ignore_zz = z;
}
                          ...
                          float ybot, float ytop, float *y, float *w)
{
    ...
    if (nbin <= 0 || nww <= 0)
    {
        return 0.0f;                          /* something bad happened! */
    }
    normalize_2Dhist();
...
/*! Compute the normalized mutual info between two vectors, sort of. Actually, returns H(x,y) / [
 * H(x)+H(y) ], which should be small if x and y are redundant and should be large if they are
 *independent.
 * ----------------------------------------------------------------------------*/
...
void THD_corr_ratio_mode(int mm)
{
    cr_mode = mm;
}
                         ...
                         float ybot, float ytop, float *y, float *w)
{
    ...
    if (cr_mode == 2)
    {
        return 1.0f - 0.5f * (xrat + yrat);            /** additive **/
    }
...
/*! Compute the Hellinger metric, mutual info, normalized MI, and symmetrized correlation ratio, and
 * return all 4 (in that order) using the 1D and 2D histograms from build_2Dhist().
 *
 *  The first 3 values all measure the closeness of the joint histogram to the product of the
 *marginals:
 ...
 *    - NMI is larger when the joint is closer to the marginal's product Correlation ratio
 *(symmetrized by addition == CRA) is larger when the two variables are nonlinearly correlated.
 *
 *  As measures of association (generalized correlation): more closely associated variables
 *correspond to larger Hellinger and MI and CRA, and to smaller NMI.
 *//*------------------------------------------------------------------------*/
                            ...
                            float ybot, float ytop, float *y, float *w)
{
    ...
    if (nbin <= 0 || nww <= 0)
    {
        return hmc;                          /* something bad happened! */
    }
    normalize_2Dhist();
 ...
 * with the binary 0-1 variable. That is, does knowing a value is in group 0 or 1 help predict it?
 *//*-------------------------------------------------------------------------*/
...
float_pair THD_binary_mutinfo(int n0, float *y0, int n1, float *y1)
{
    ...
    if (n0 < nbin || y0 == NULL || n1 < nbin || y1 == NULL)
    {
        return mmii;
    }
...
/*! Rank orders a collection of float arrays; ties get the average rank. The output overwrites the
 * input.
 *//*-------------------------------------------------------------------------*/
...
void rank_order_float_arrays(int nar, int *nn, float **aa)
{
    ...
    /* c[kk] now contains the global rank of the kk-th element, but c[] perhaps needs to modified to
     * allow for ties ==> average rank */
...
/* Given a set of bootstrap replicates, find the bias-corrected (BC) estimates of the 1-alpha
 * confidence interval and the central value.
 * estim = actual estimate from the data alpha = 0.05 is a typical value nboot = number of
 *replicates in eboot[] -- at least MAX(100,10/alpha) eboot = array of replicates; will be sorted on
 *output The return value (rval) is a triple:
 * rval.a = lower edge of confidence interval rval.b = middle value = bias-corrected estimate rval.c
 *= upper edge of confidence interval If all values are returned as 0, the inputs were bad bad bad.
 *//*--------------------------------------------------------------------------*/
                                   ...
                                   int nboot, float *eboot)
{
    ...
    if (nboot < 100 || eboot == NULL)
    {
        RETURN(rval);                                           /* bad user */
    }
    if (alpha <= 0.001f || alpha >= 0.9f)
    {
        alpha = 0.05f;                                      /* stupid user */
    }
    alpha *= 0.5f;                                          /* 1-sided tail */
    if ((int)(alpha * nboot) < 5)
    {
        alpha = 5.0f / nboot;                         /* upward adjust */
    }
    zal = PHINV(alpha);                               /* should be negative */

    qsort_float(nboot, eboot);                        /* increasing order */

    for (ii = 0 ; ii < nboot && eboot[ii] < estim ; ii++)
    {
        ;                                                             /*nada*/
    }
    if (ii <= 1 || ii >= nboot - 1)
    {
        RETURN(rval);                               /* crummy data */
    }
    ...
    ii = (int)pp; pp = pp - ii; if (ii >= nboot - 1)
    {
        ii = nboot - 2;
    }
...
float THD_bootstrap_biascorr(float estim, int nboot, float *eboot)
{
    ...
    if (nboot < 50 || eboot == NULL)
    {
        return estim;                                   /* bad user */
    }
    qsort_float(nboot, eboot);                          /* increasing order */
...
#pragma omp critical (MALLOC)
    ...
    else
    {
        for (ii = 0 ; ii < nlen ; ii++)
        {
            uvec[ii] = xbar[ii]; vvec[ii] = ybar[ii];
        }
    }
...
#pragma omp critical (MALLOC)
    { free(vvec); free(uvec); free(rboot);
      free(yar); free(xar); free(ybar); free(xbar); if (use_pv)
      {
          free(pvw);
      }
    }
