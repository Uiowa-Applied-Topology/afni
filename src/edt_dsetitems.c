...
/*! Edit some internals of a dataset.  Notice that it is possible to create a dataset which is
 * inconsistent.  Note also that some operations cannot be performed on a dataset containing actual
 * data -- for example, you cannot change the types of sub-bricks if they already are attached to
 * data arrays!
 *
...
int EDIT_dset_items(THD_3dim_dataset *dset, ...)
{
    ...
    do{
        ...
        default: {
            ...
            if (iv >= 0 && iv < ADN_ONE_STEP)
            {
                ...
                if (brick_label_one != NULL)
                {
                    new_brick_label_one = 1;
                }
                break; /* exit switch */
            }
            ...
            if (iv >= 0 && iv < ADN_ONE_STEP)
            {
                ...
                if (brick_stataux_one != NULL)
                {
                    new_brick_stataux_one = 1;
                }
                break; /* exit switch */
            }
        ...
        case ADN_prefix:   /* processed later */
            ...
            if (prefix != NULL)
            {
                ...
                {
                    /* if defined by prefix, apply, else if current is not a surface, clear (i.e.
                     * assume AFNI type) */
        ...
        case ADN_directory_name:   /* processed later */
            ...
            {
                EDERR("illegal new directory_name");
            }
            break;
        ...
        case ADN_datum_all:   /* processed later */
            ...
            if (AFNI_GOOD_DTYPE(datum_all))
            {
                new_datum_all = 1;
            }
            else
            {
                EDERR("illegal new datum_all");
            }
            break;
        ...
        case ADN_nvals:   /* processed later */
            ...
            {
                EDERR("illegal new nvals");
            }
            break;

        /** these commands affect the spatial axes, which may also influence size of the data bricks
         * **/

        case ADN_ijk_to_dicom:   /* processed later [19 Dec 2005] */
            ijk_to_dicom = va_arg(vararg_ptr, mat44);
            if (ISVALID_MAT44(ijk_to_dicom))
            {
                new_ijk_to_dicom = 1;
            }
            else
            {
                EDERR("illegal new ijk_to_dicom");
            }
            break;
        ...
        case ADN_tunits:   /* processed later */
            ...
            {
                EDERR("illegal new taxis tunits");
            }
            break;
        ...
        /** these commands affect the interpretation of the dataset (e.g., is it functional or
         * anatomical, which view type, ...) **/
    ...
    if (new_prefix || new_directory_name || new_view_type)
    {
        ...
        if (DSET_IS_1D(dset) || DSET_IS_3D(dset))         /* 21 Mar 2003 */
        {
            ...
            if (DSET_IS_1D(dset) || (DSET_NY(dset) == 1 && DSET_NZ(dset) == 1))
            {
                if (!STRING_HAS_SUFFIX(fname, ".1D") &&
                    !STRING_HAS_SUFFIX(fname, ".1D.dset"))
                {
                    strcat(fname, ".1D");
                }
        ...
        if (DSET_IS_NIML(dset))         /* 07 Jun 2006 [rickr] */
        {
            ...
            int   ll    = strlen(fname);
            STRING_DEVIEW_DEEXT_BRICK(fname);
            if (!STRING_HAS_SUFFIX(fname, ".niml"))
            {
                strcat(fname, ".niml");
            }
            fname = dset->dblk->diskptr->prefix; /* also adjust prefix */
        ...
        if (DSET_IS_NI_SURF_DSET(dset)) /* 28 Jun 2006 [rickr] */
        {
            ...
            if (!STRING_HAS_SUFFIX(fname, ".niml.dset"))
            {
                strcat(fname, ".niml.dset");
            }
            fname = dset->dblk->diskptr->prefix; /* also adjust prefix */
            if (!STRING_HAS_SUFFIX(fname, ".niml.dset"))
            {
        ...
        if (nprefix != NULL && STRING_HAS_SUFFIX(nprefix, ".hdr"))
        {
            ...
            if (!STRING_HAS_SUFFIX(fname, ".img"))
            {
                strcat(fname, ".img");
            }
    ...
    if (redo_daxes)
    {
        ...
        {
#if 0
            INFO_message("old daxes orient = %d %d %d", daxes->xxorient, daxes->yyorient,
                         daxes->zzorient);
#endif
...
#if 0
            INFO_message("new daxes orient = %d %d %d", daxes->xxorient, daxes->yyorient,
                         daxes->zzorient);
#endif
        }
    ...
    /*---- Add a single brick stataux value. The input is a float array formatted like so:
     ...
     *           <value>    is an auxiliary statistical parameter needed for data of type <statcode>
     *                ----*/
    ...
    if (!new_ntt)
    {
        ntt = ISVALID_TIMEAXIS(dset->taxis) ? dset->taxis->ntt : 0;
    }
    ...
    if (redo_taxis)
    {
        ...
        if (new_toff_sl)
        {
            for (ii = 0 ; ii < taxis->nsl ; ii++)
            {
                taxis->toff_sl[ii] = toff_sl[ii];
            }
