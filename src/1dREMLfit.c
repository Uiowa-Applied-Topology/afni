...
int main(int argc, char *argv[])
{
    ...
    while (iarg < argc && argv[iarg][0] == '-')
    {
        ...
        if (strcmp(argv[iarg], "-matrix") == 0)
        {
            if (nelmat != NULL)
            {
                ERROR_exit("More than 1 -matrix option!");
            }
    ...
    if (cgl != NULL)
    {
        ...
        if (rst != NULL)
        {
            ...
            if (riar == NULL)
            {
                ERROR_exit("-matrix 'RunStart' badly formatted?");
            }
        ...
        {
            jj = goodlist[ii];
            for ( ; rnum + 1 < Nruns && jj >= runs[rnum + 1] ; rnum++)
            {
                ;                                               /*nada*/
            }
