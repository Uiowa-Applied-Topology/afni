/*****************************************************************************
 * Major portions of this software are copyrighted by the Medical College of Wisconsin, 1994-2001,
 *and are released under the Gnu General Public License, Version 2.  See the file README.Copyright
 *for details.
 ******************************************************************************/

/*
 * This program calculates three-factor analysis of variance for 3 dimensional AFNI data sets.
 *
 * File:    3dANOVA3.c Author:  B. D. Ward Date:    29 January 1997
 *
 * Mod:     Extensive changes required to implement the 'bucket' dataset. Date:    30 December 1997
 *
 * Mod:     Separated 3dANOVA.h and 3dANOVA.lib files. Date:    5 January 1998
 *
 * Mod:     Continuation of 'bucket' dataset changes. Date:    9 January 1998
 *
 * Mod:     Added software for statistical tests of individual cell means. Date:    27 October 1998
 *
 * Mod:     Added changes for incorporating History notes. Date:    09 September 1999
 *
 * Mod:     Replaced dataset input code with calls to THD_open_dataset, to allow operator selection
 *of individual sub-bricks for input. Date:    03 December 1999
 *
 * Mod:     Added call to AFNI_logger. Date:    15 August 2001
 *
 * Mod:     Modified routine write_afni_data of 3dANOVA.lib so that all output subbricks will now
 *have the scaled short integer format. Date:    14 March 2002
 *
 * Mod:     Set MAX_NAME_LENGTH equal to THD_MAX_NAME. Date:    02 December 2002
 *
 * Mod:     In get_options(), calloc and free n (stack space is limited). Date:    19 July 2004
 *[rickr]
 *
 * Mod:     Update computation of sums of squares for a/bmean, a/bdiff and a/bcontr, for type 4
 *ANOVAs.  See calc_mean_sum2_acontr(). Similar update for model type 5.  See calc_type5_acontr()
 *and calc_type5_bcontr().  Cleaned up compiler warnings. Date:    23 Sep 2005 [rickr,gangc]
 *
 * Mod:     Added options -aBcontr and -Abcontr for types 4 and 5. The contrast and t-stat files
 *will go at the end of any bucket. Changed some EPSILON values from e-10 to e-15. Date:    17 Oct
 *2005 [rickr,gangc]
 *
 * Mod:     Fixed -help typo, num_Abcontr assignment and df in calc_Abc(). Date:    27 Oct 2005
 *[rickr,gangc]
 *
 * Mod:     Allow old computations to be done via -old_method.  This affects calc_ameans, bmeans,
 *adiff, bdiff, acontr, bcontr, and for the types 4 and 5 models, only (not type 1). Date:    22 Nov
 *2005 [rickr]
 *
 * Mod:     The -old_method option requires -OK. Added -assume_sph and a check that the contrasts
 *sum to zero.
 *          (if so, use old_method) Date:    02 Dec 2005 [rickr]
 *
 * Mod:     Added -aBdiff, -Abdiff and -abmean options and routines. Replaced
 *calc_mean_sum2_acontr() with calc_type4_acontr(), to avoid intermediate storage of data as floats
 *(by absorbing the calculate_t_from_sums() operation). Replaced calc_mean_sum2_bcontr() with
 *calc_type4_bcontr(). Removed calculate_t_from_sums(). Do checks against EPSILON before sqrt(), in
 *case < 0. Date:    28 Dec 2005 [rickr]
 *
 * Mod:     Bothered to actually assign df_prod before using it in calc_type4_bcontr().  Fixed
 * -aBdiff label.  Thanks to Debbie at U-Toronto. Date:    31 Jan 2005 [rickr]
 *
 * Mod:     Small help update, including example. Date:    09 Feb 2006 [rickr]
 */
...
void get_options(int argc, char **argv, anova_options *option_data)
{
    ...
    while (nopt < argc)
    {
        ...
        if (strncmp(argv[nopt], "-datum", 5) == 0)
        {
            if (++nopt >= argc)
            {
                ANOVA_error("need an argument after -datum!");
            }
        ...
        if (strncmp(argv[nopt], "-session", 5) == 0)
        {
            ...
            if (nopt >= argc)
            {
                ANOVA_error("need argument after -session!");
            }
        ...
        if (strncmp(argv[nopt], "-voxel", 6) == 0)
        {
            ...
            if (nopt >= argc)
            {
                ANOVA_error("need argument after -voxel ");
            }
        ...
        if (strncmp(argv[nopt], "-type", 5) == 0)
        {
            ...
            if (nopt >= argc)
            {
                ANOVA_error("need argument after -type ");
            }
            sscanf(argv[nopt], "%d", &ival);
            if ((ival < 1) || (ival > 5))
            {
        ...
        /*   if -old_method if -OK, all contrasts are okay else if -assume_sph, contrasts adding to
         * 0 are okay else complain and fail
         *
         *   bits: -old_method = 001, -OK = 010, -assume_sph = 100
         *
         *   valid bit patterns:
         *       000 - use the new method 011 - use the old method 101 - use the old method (only
         *allows zero-sum contrasts)
         * ------------------------------------------------------------*/
        ...
        if (strncmp(argv[nopt], "-alevels", 5) == 0)
        {
            ...
            if (nopt >= argc)
            {
                ANOVA_error("need argument after -alevels ");
            }
            sscanf(argv[nopt], "%d", &ival);
        ...
        if (strncmp(argv[nopt], "-blevels", 5) == 0)
        {
            ...
            if (nopt >= argc)
            {
                ANOVA_error("need argument after -blevels ");
            }
            sscanf(argv[nopt], "%d", &ival);
            if ((ival <= 0) || (ival > MAX_LEVELS))
            {
        ...
        if (strncmp(argv[nopt], "-clevels", 5) == 0)
        {
            ...
            if (nopt >= argc)
            {
                ANOVA_error("need argument after -clevels ");
            }
            sscanf(argv[nopt], "%d", &ival);
            if ((ival <= 0) || (ival > MAX_LEVELS))
            {
        ...
        if (strncmp(argv[nopt], "-dset", 5) == 0)
        {
            ...
            if (nopt + 3 >= argc)
            {
                ANOVA_error("need 4 arguments after -dset ");
            }
            sscanf(argv[nopt], "%d", &ival);
        ...
        if (strncmp(argv[nopt], "-fa", 5) == 0)
        {
            ...
            if (nopt >= argc)
            {
                ANOVA_error("need argument after -fa ");
            }
            option_data->nfa    = 1;
        ...
        if (strncmp(argv[nopt], "-fb", 5) == 0)
        {
            ...
            if (nopt >= argc)
            {
                ANOVA_error("need argument after -fb ");
            }
            option_data->nfb    = 1;
        ...
        if (strncmp(argv[nopt], "-fc", 5) == 0)
        {
            ...
            if (nopt >= argc)
            {
                ANOVA_error("need argument after -fc ");
            }
            option_data->nfc    = 1;
        ...
        if (strncmp(argv[nopt], "-fab", 5) == 0)
        {
            ...
            if (nopt >= argc)
            {
                ANOVA_error("need argument after -fab ");
            }
            option_data->nfab    = 1;
        ...
        if (strncmp(argv[nopt], "-fac", 5) == 0)
        {
            ...
            if (nopt >= argc)
            {
                ANOVA_error("need argument after -fac ");
            }
            option_data->nfac    = 1;
        ...
        if (strncmp(argv[nopt], "-fbc", 5) == 0)
        {
            ...
            if (nopt >= argc)
            {
                ANOVA_error("need argument after -fbc ");
            }
            option_data->nfbc    = 1;
        ...
        if (strncmp(argv[nopt], "-fabc", 5) == 0)
        {
            ...
            if (nopt >= argc)
            {
                ANOVA_error("need argument after -fabc ");
            }
            option_data->nfabc    = 1;
        ...
        if (strncmp(argv[nopt], "-amean", 5) == 0)
        {
            ...
            if (nopt + 1 >= argc)
            {
                ANOVA_error("need 2 arguments after -amean ");
            }
        ...
        if (strncmp(argv[nopt], "-bmean", 5) == 0)
        {
            ...
            if (nopt + 1 >= argc)
            {
                ANOVA_error("need 2 arguments after -bmean ");
            }
        ...
        if (strncmp(argv[nopt], "-cmean", 5) == 0)
        {
            ...
            if (nopt + 1 >= argc)
            {
                ANOVA_error("need 2 arguments after -cmean ");
            }
        ...
        if (strncmp(argv[nopt], "-xmean", 5) == 0)
        {
            ...
            if (nopt + 3 >= argc)
            {
                ANOVA_error("need 4 arguments after -xmean ");
            }
        ...
        if (strncmp(argv[nopt], "-adiff", 5) == 0)
        {
            ...
            if (nopt + 2 >= argc)
            {
                ANOVA_error("need 3 arguments after -adiff ");
            }
        ...
        if (strncmp(argv[nopt], "-bdiff", 5) == 0)
        {
            ...
            if (nopt + 2 >= argc)
            {
                ANOVA_error("need 3 arguments after -bdiff ");
            }
        ...
        if (strncmp(argv[nopt], "-cdiff", 5) == 0)
        {
            ...
            if (nopt + 2 >= argc)
            {
                ANOVA_error("need 3 arguments after -cdiff ");
            }
        ...
        if (strncmp(argv[nopt], "-xdiff", 5) == 0)
        {
            ...
            if (nopt + 6 >= argc)
            {
                ANOVA_error("need 7 arguments after -xdiff ");
            }
        ...
        if (strncmp(argv[nopt], "-bucket", 4) == 0)
        {
            ...
            if (nopt >= argc)
            {
                ANOVA_error("need argument after -bucket ");
            }
...
void check_for_valid_inputs(anova_options *option_data)
{
    ...
    if (option_data->n < 1)
    {
        ANOVA_error("sample size is too small");
    }
    ...
    case 1:
        if (n == 1)
        {
            ANOVA_error("sample size is too small for Model 1");
        }
        if (option_data->num_aBcontr > 0)
        {
...
void initialize(int argc, char **argv, anova_options **option_data)
{
    ...
    *option_data = (anova_options *)malloc(sizeof(anova_options));
    ...
    if ((*option_data)->diskspace)
    {
        check_disk_space(*option_data);
    }
}
...
/*
 * Routine to sum over the specified set of observations. The output is returned in ysum.
 */
...
/*                                                    8 Sep 2005 [rickr,gangc] routine to compute
 * the mean and t-stat for a type 4 A contrast
 *
 * In the case of an A contrast, the mean is computed across B and any multiple observations, first.
 *The contrast is then computed over the result.  So this 4-D data is reduced to 2-D, with 1 for the
 *contrast, and one for the sum and sum of squares.
 *
 *    mean = sum_over_k[ (sum_over_i_in_contrast[c_i * y_i.k.])   ] / K sum2 = sum_over_k[
 * (sum_over_i_in_contrast[c_i * y_i.k.])^2 ]
 *
 *    t = mean * sqrt[     df ( df + 1 )      ] [ ---------------------- ] [ sum_sq - (df+1)*mean^2
 * ]
 *
 * Note that y_i.j. is the mean over B and any multiple observations. Note also that b may be 1 if
 *bindex is in [0,B-1], else B is equal to #blevels.
 *
 * For accuracy, sums are computed using doubles, then copied to float. The contrast is passed in to
 *allow for more uses of this function.
 */
                       ...
                       int bindex, int df, float *mean, float *tmean)
{
    ...
    if (bindex < 0)
    {
        nvals = n * b;          /* collapse across B and n */
    }
    else
    {
        nvals = n;              /* collapse across n, at the given B level */
    }
    /*----- allocate memory space for calculations -----*/
    ...
    for (ixyz = 0; ixyz < nxyz; ixyz++)
    {
        ...
        {
            tmean[ixyz] = dmean * sqrt(df_prod / dval);
        }
    }
...
/*                                                   20 Sep 2005 [rickr,gangc] routine to compute
 * the mean and tstat of a type-5 A contrast
 *
 * t      = L / sqrt(1/c * SL2 * sum_c2) L      = sum_i(contr[i] * Y_i...) SL     = 1/w(c-1) *
 * sum_i[ step(|contr[i]|) * sum_k(Y_i.k. - Y_i...)^2 ] sum_c2 = sum_i(contr[i]^2)
 *
...
/*                                                   23 Sep 2005 [rickr,gangc] routine to compute
 * the mean and tstat of a type-5 B contrast
 *
 * t  = L / sqrt(1/ac * SL ) L  = sum_j(contr[j] * Y_.j..) SL = 1/a(c-1) * sum_i[ sum_k[ (L_ik -
 * L_i)^2 ] ]
...
/*                                                    8 Sep 2005 [rickr,gangc] routine to compute
 * the mean and t-stat for a type 4 B contrast
 *
 * In the case of a B contrast, the mean is computed across A and any multiple observations, first.
 *The contrast is then computed over the result.  So this 4-D data is reduced to 2-D, with 1 for the
 *contrast, and one for the sum and sum of squares.
 *
 *    mean = sum_over_k[ (sum_over_i_in_contrast[c_j * y_.jk.])   ] / K sum2 = sum_over_k[
 * (sum_over_i_in_contrast[c_j * y_.jk.])^2 ]
 *
 *    t = mean * sqrt[     df ( df + 1 )      ] [ ---------------------- ] [ sum_sq - (df+1)*mean^2
 * ]
 *
 * Note that y_.jk. is the mean over A and any multiple observations. Note also that a may be 1 if
 *aindex is in [0,A-1], else a is equal to #alevels.
 *
 * For accuracy, sums are computed using doubles, then copied to float. The contrast is passed in to
 *allow for more uses of this function.
 */
                       ...
                       int aindex, int df, float *mean, float *tmean)
{
    ...
    if (aindex < 0)
    {
        nvals = n * a;          /* collapse across A and n */
    }
    else
    {
        nvals = n;              /* collapse across n, at the given A level */
    }
    /*----- allocate memory space for calculations -----*/
    dsum   = (double *)malloc(sizeof(double) * nxyz);
    dsum2  = (double *)malloc(sizeof(double) * nxyz);
...
/*
 * Routine to calculate SS0.  Result is stored in temporary output file ss0.3danova3.
 */
...
/*
 * Routine to calculate SSI.  Result is stored in temporary output file ssi.3danova3.
 */
...
/*
 * Routine to calculate SSJ.  Result is stored in temporary output file ssj.3danova3.
 */
...
/*
 * Routine to calculate SSK.  Result is stored in temporary output file ssk.3danova3.
 */
...
/*
 * Routine to calculate SSIJ.  Result is stored in temporary output file ssij.3danova3.
 */
...
/*
 * Routine to calculate SSIK.  Result is stored in temporary output file ssik.3danova3.
 */
...
/*
 * Routine to calculate SSJK.  Result is stored in temporary output file ssjk.3danova3.
 */
...
/*
 * Routine to calculate SSIJK.  Result is stored in temporary output file ssijk.3danova3.
 */
...
/*
 * Routine to sum the squares of all observations. The sum is stored (temporarily) in disk file
 *ssijkm.3danova3.
 */
...
/*
 * Routine to calculate the total sum of squares (SSTO). The output is stored (temporarily) in file
 *ssto.3danova3.
 */
...
/*
 * Routine to calculate the error sum of squares (SSE). The output is stored (temporarily) in file
 *sse.3danova3.
 */
...
/*
 * Routine to calculate the sum of squares due to factor A (SSA). The output is stored (temporarily)
 *in file ssa.3danova3.
 */
...
/*
 * Routine to calculate the sum of squares due to factor B (SSB). The output is stored (temporarily)
 *in file ssb.3danova3.
 */
...
/*
 * Routine to calculate the sum of squares due to factor C (SSC). The output is stored (temporarily)
 *in file ssc.3danova3.
 */
...
/*
 * Routine to calculate the A*B interaction sum of squares (SSAB). The output is stored
 *(temporarily) in file ssab.3danova3.
 */
...
/*
 * Routine to calculate the A*C interaction sum of squares (SSAC). The output is stored
 *(temporarily) in file ssac.3danova3.
 */
...
/*
 * Routine to calculate the factor C(A) sum of squares (SSC(A)). The output is stored (temporarily)
 *in file ssca.3danova3.
 */
...
/*
 * Routine to calculate the B*C interaction sum of squares (SSBC). The output is stored
 *(temporarily) in file ssbc.3danova3.
 */
...
/*
 * Routine to calculate the A*B*C interaction sum of squares (SSABC). The output is stored
 *(temporarily) in file ssabc.3danova3.
 */
...
/*
 * Routine to calculate the B*C(A) interaction sum of squares (SSBC(A)). The output is stored
 *(temporarily) in file ssbca.3danova3.
 */
 ...
 *
 *
 * Model 1:   F(A) = MSA / MSE Model 4:   F(A) = MSA / MSAC Model 5:   F(A) = MSA / MSC(A)
 *
 *     where MSA    = SSA / (a-1) MSE    = SSE / abc(n-1) MSAC   = SSAC / (a-1)(c-1) MSC(A) = SSC(A)
 * / a(c-1)
 *
 * The output is stored as a 2 sub-brick AFNI data set.  The first sub-brick contains the square
 *root of MSA (mean sum of squares due to factor A), and the second sub-brick contains the
 ...
 *
 *
 * Model 1:   F(B) = MSB / MSE Model 3:   F(B) = MSB / MSBC Model 4:   F(B) = MSB / MSBC Model 5:
 *   F(B) = MSB / MSBC(A)
 *
 *     where MSB     = SSB / (b-1) MSE     = SSE / abc(n-1) MSBC    = SSBC / (b-1)(c-1) MSBC(A) =
 * SSBC(A) / a(b-1)(c-1)
 *
 * The output is stored as a 2 sub-brick AFNI data set.  The first sub-brick contains the square
 *root of MSB (mean sum of squares due to factor B), and the second sub-brick contains the
 *corresponding F-statistic.
 */
 ...
 *
 *
 * Model 1:   F(C)    = MSC / MSE Model 3:   F(C)    = MSC / MSBC Model 4:   F(C)    = MSC / MSE
 *Model 5:   F(C(A)) = MSC(A) / MSE
 *
 *     where MSC     = SSC / (c-1) MSE     = SSE / abc(n-1) MSBC    = SSBC / (b-1)(c-1) MSC(A)  =
 * SSC(A) / a(c-1)
 *
 * The output is stored as a 2 sub-brick AFNI data set.  The first sub-brick contains the square
 *root of MSC (mean sum of squares due to factor C), and the second sub-brick contains the
 *corresponding F-statistic.
 */
 ...
 *
 *
 * Model 1:   F(AB) = MSAB / MSE Model 2:   F(AB) = MSAB / MSABC Model 3:   F(AB) = MSAB / MSABC
 *Model 4:   F(AB) = MSAB / MSABC Model 5:   F(AB) = MSAB / MSBC(A)
 *
 *     where MSAB    = SSAB / (a-1)(b-1) MSE     = SSE / abc(n-1) MSABC   = SSABC / (a-1)(b-1)(c-1)
 *MSBC(A) = SSBC(A) / a(b-1)(c-1)
 *
 * The output is stored as a 2 sub-brick AFNI data set.  The first sub-brick contains the square
 *root of MSAB (mean sum of squares due to interaction), and the second sub-brick contains the
 *corresponding F-statistic.
 */
 ...
 *
 * Model 1:   F(BC)    = MSBC / MSE Model 2:   F(BC)    = MSBC / MSABC Model 3:   F(BC)    = MSBC /
 * MSE Model 4:   F(BC)    = MSBC / MSE Model 5:   F(BC(A)) = MSBC(A) / MSE
 *
 *     where MSBC    = SSBC / (b-1)(c-1) MSE     = SSE / abc(n-1) MSABC   = SSABC / (a-1)(b-1)(c-1)
 *MSBC(A) = SSBC(A) / a(b-1)(c-1)
 *
 *
 * The output is stored as a 2 sub-brick AFNI data set.  The first sub-brick contains the square
 *root of MSBC (mean sum of squares due to interaction), and the second sub-brick contains the
 *corresponding F-statistic.
 */
 ...
 *
 * Model 1:   F(AC) = MSAC / MSE Model 2:   F(AC) = MSAC / MSABC Model 3:   F(AC) = MSAC / MSABC
 *Model 4:   F(AC) = MSAC / MSE
 *
 *     where MSAC  = SSAC / (a-1)(c-1) MSE     = SSE / abc(n-1) MSABC   = SSABC / (a-1)(b-1)(c-1)
 *
 * The output is stored as a 2 sub-brick AFNI data set.  The first sub-brick contains the square
 *root of MSAC (mean sum of squares due to interaction), and the second sub-brick contains the
 *corresponding F-statistic.
 */
 ...
 *
 * Model 1:   F(ABC) = MSABC / MSE Model 2:   F(ABC) = MSABC / MSE Model 3:   F(ABC) = MSABC / MSE
 *Model 4:   F(ABC) = MSABC / MSE
 *
 *     where MSABC   = SSABC / (a-1)(b-1)(c-1) MSE     = SSE / abc(n-1)
 *
 * The output is stored as a 2 sub-brick AFNI data set.  The first sub-brick contains the square
 *root of MSAC (mean sum of squares due to interaction), and the second sub-brick contains the
 *corresponding F-statistic.
...
/*
 * The following are the old calculate_* functions, before the changes by rickr/gangc.  The function
 *names have been prepended by old_, allowing the addition of an option to call those functions,
 *instead of the new varieties.  This seems to be the cleanest way to allow users to compare results
 *between the old vs. new functions.  Putting multiple tests in each function would be quite messy.
 *
 * Note that the different functionality exists for types 4 and 5 of a/b means, a/b diff, and a/b
 *contr.
 *                                                 22 Nov 2005 [rickr]
 */
...
/*
 * Routine to calculate the mean treatment effect for factor A at the user specified treatment
 *level.  The output is stored as a 2 sub-brick AFNI data set. The first sub-brick contains the
 *estimated mean at this treatment level, and the second sub-brick contains the corresponding
 *t-statistic.
 */
...
/*
 * Routine to calculate the mean treatment effect for factor B at the user specified treatment
 *level.  The output is stored as a 2 sub-brick AFNI data set. The first sub-brick contains the
 *estimated mean at this treatment level, and the second sub-brick contains the corresponding
 *t-statistic.
 */
...
/*
 * Routine to estimate the difference in the means between two user specified treatment levels for
 *factor A.  The output is a 2 sub-brick AFNI data set. The first sub-brick contains the estimated
 *difference in the means. The second sub-brick contains the corresponding t-statistic.
 */
...
/*
 * Routine to estimate the difference in the means between two user specified treatment levels for
 *factor B.  The output is a 2 sub-brick AFNI data set. The first sub-brick contains the estimated
 *difference in the means. The second sub-brick contains the corresponding t-statistic.
 */
...
/*
 * Routine to estimate a user specified contrast in treatment levels for factor A.  The output is
 *stored as a 2 sub-brick AFNI data set.  The first sub-brick contains the estimated contrast.  The
...
/*
 * Routine to estimate a user specified contrast in treatment levels for factor B.  The output is
 *stored as a 2 sub-brick AFNI data set.  The first sub-brick contains the estimated contrast.  The
 *second sub-brick contains the corresponding t-statistic.
 */
...
/*
 * Routine to calculate the mean treatment effect for factor C at the user specified treatment
 *level.  The output is stored as a 2 sub-brick AFNI data set. The first sub-brick contains the
 *estimated mean at this treatment level, and the second sub-brick contains the corresponding
 *t-statistic.
 */
...
/*
 * Routine to calculate the mean for an individual cell (treatment). The output is stored as a 2
 *sub-brick AFNI data set. The first sub-brick contains the estimated mean for this cell, and the
 *second sub-brick contains the corresponding t-statistic.
 */
...
/*
 * Routine to estimate the difference in the means between two user specified treatment levels for
 *factor C.  The output is a 2 sub-brick AFNI data set. The first sub-brick contains the estimated
 *difference in the means. The second sub-brick contains the corresponding t-statistic.
 */
...
/*
 * Routine to estimate the difference in the cell means between two user specified cells
 *(treatments).  The output is a 2 sub-brick AFNI data set. The first sub-brick contains the
 *estimated difference in the cell means. The second sub-brick contains the corresponding
 *t-statistic.
 */
...
void calculate_acontrasts(anova_options *option_data)
{
    ...
    for (icontr = 0; icontr < num_contr; icontr++)
    {
        ...
        else /* model == 5 */
        {
            ...
            for (i = 0, df = 0; i < a; i++)
            {
                if (option_data->acontr[icontr][i])
                {
                    df++;
                }
            }
...
/*
 * Routine to estimate a user specified contrast in treatment levels for factor C.  The output is
 *stored as a 2 sub-brick AFNI data set.  The first sub-brick contains the estimated contrast.  The
 *second sub-brick contains the corresponding t-statistic.
 */
...
/*
 * Routine to estimate a user specified 2nd order contrast in treatment levels for factor A, at a
 *fixed B level.  The output is stored as a 2 sub-brick AFNI data set.  The first sub-brick contains
 *the estimated contrast.  The second sub-brick contains the corresponding t-statistic.
 */
...
/*
 * Routine to estimate a user specified contrast in treatment levels for factor B, at a fixed A
 *level.  The output is stored as a 2 sub-brick AFNI data set.  The first sub-brick contains the
 *estimated contrast. The second sub-brick contains the corresponding t-statistic.
 */
...
/*
 * Routine to estimate the difference between 2 levels of factor A, at a fixed level of factor B.
 *                                      19 Dec 2005 [rickr]
 *
 */
...
/*
 * Routine to estimate the difference between 2 levels of factor B, at a fixed level of factor A.
 *                                      19 Dec 2005 [rickr]
 *
 */
void calculate_Abdiffs(anova_options *option_data)
...
/*
 * Routine to estimate each mean given a fixed level a factor A and a fixed level of factor B.
 *                                   19 Dec 2005 [rickr]
 *
 */
...
void calculate_anova(anova_options *option_data)
{
    ...
    if (option_data->model != 5)
    {
        calculate_ssk(option_data);
    }
    calculate_ssij(option_data);
    calculate_ssik(option_data);
    if (option_data->model != 5)
    {
        calculate_ssjk(option_data);
    }
    calculate_ssijk(option_data);
    if (option_data->n != 1)
    {
        calculate_ssijkm(option_data);
    }
...
void analyze_results(anova_options *option_data)
{
    ...
    if (option_data->num_ameans)
    {
        if (option_data->old_method)
        {
            old_calculate_ameans(option_data);
        }
        else
        {
            calculate_ameans(option_data);
        }
    }
    ...
    if (option_data->num_bmeans)
    {
        if (option_data->old_method)
        {
            old_calculate_bmeans(option_data);
        }
        else
        {
            calculate_bmeans(option_data);
        }
    }
    ...
    {
        calculate_cmeans(option_data);
    }

    /*-----  estimate cell means  -----*/
    if (option_data->num_xmeans)
    {
        calculate_xmeans(option_data);
    }
    ...
    if (option_data->num_adiffs)
    {
        if (option_data->old_method)
        {
            old_calculate_adifferences(option_data);
        }
        else
        {
            calculate_adifferences(option_data);
        }
    }
    ...
    if (option_data->num_bdiffs)
    {
        if (option_data->old_method)
        {
            old_calculate_bdifferences(option_data);
        }
        else
        {
            calculate_bdifferences(option_data);
        }
    }
    ...
    {
        calculate_cdifferences(option_data);
    }

    /*-----  estimate differences in cell means  -----*/
    if (option_data->num_xdiffs)
    {
        calculate_xdifferences(option_data);
    }
    ...
    if (option_data->num_acontr)
    {
        if (option_data->old_method)
        {
            old_calculate_acontrasts(option_data);
        }
        else
        {
            calculate_acontrasts(option_data);
        }
    }
    ...
    if (option_data->num_bcontr)
    {
        if (option_data->old_method)
        {
            old_calculate_bcontrasts(option_data);
        }
        else
        {
            calculate_bcontrasts(option_data);
        }
    }
    ...
    {
        calculate_ccontrasts(option_data);
    }

    /*-----  estimate level contrasts for factor A at fixed B level -----*/
    if (option_data->num_aBcontr)
    {
        calculate_aBcontrasts(option_data);
    }
    ...
    if (option_data->num_aBdiffs)
    {
        calculate_aBdiffs(option_data);
    }
    ...
    if (option_data->num_abmeans)
    {
        calculate_abmeans(option_data);
    }
}
...

{
    ...
    if (DSET_IS_1D(dset))
    {
        USE_1D_filenames(1);                        /* 14 Mar 2003 */
    }
    else if (DSET_IS_3D(dset))
    {
        USE_1D_filenames(3);                        /* 21 Mar 2003 */
    }
    ...
    /*----- invoke program 3dbucket to generate bucket type output dataset by concatenating previous
     * output files -----*/
...
/*
 * Routine to release memory and remove any remaining temporary data files. If the '-bucket' option
 *has been used, create the bucket dataset and dispose of the 2 sub-brick datasets.
 */
...
void terminate(anova_options *option_data)
{
    ...
    /*----- if 'bucket' dataset was created, remove the individual 2-subbrick data files -----*/
    if (option_data->bucket_filename != NULL)
    {
        /*----- read first dataset -----*/
        dset = THD_open_dataset(option_data->first_dataset);
...
int main(int argc, char **argv)
{
    ...
    /*if( (option_data->model == 4 || option_data->model == 5)
     * && !option_data->old_method ) fprintf(stderr,"\n"
