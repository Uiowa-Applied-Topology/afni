...
/***********************************************************************
 * Plugin to edit/view dataset notes. Makes a custom interface.
 * 01 Sep 1999 - RWCox
 ************************************************************************/
...
/***********************************************************************
 * Set up the interface to the user.  Note that we bypass the normal interface creation, and simply
 *have the menu selection directly call the main function, which will create a custom set of
 *interface widgets the first time in.
 ************************************************************************/
...
static void NOTES_make_widgets(void)
{
    ...
    if (max_tlines < 1)
    {
        ...
        else if (max_tlines > 99)
        {
            max_tlines = 99;                           /* 15 Sep 1999 */
        }
    }
    ...
    /**** make an initial Note window and modify it to be readonly - for the History ****/
...
{
    if (dset != NULL && num_notes > 0)
    {
        NOTES_save_CB(NULL, NULL, NULL);
    }
 ...
 *  - must be in current session Much of this code is adapted from PLUG_choose_dataset_CB.
 * ---------------------------------------------------------------------*/
