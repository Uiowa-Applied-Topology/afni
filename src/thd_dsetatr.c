...
/*! Set attributes from the dataset to the datablock, preparing for output to someplace or other.
 * Adapted from elements of the earlier editions of THD_write_3dim_dataset() and
 * THD_write_datablock() -- 09 Mar 2005.
 * ---------------------------------------------------------------------*/
...
/* DUMP_MAT44("thd_dsetatr: set ijk_to_dicom_real",daxes->ijk_to_dicom_real) ; */
    ...
    for (ii = MAX_STAT_AUX - 1; ii >= 0 && dset->stat_aux[ii] == 0.0; ii--)
    {
        ;          /* nada */
    }
...
#define TF(i, j)    vtag[(i) * NFPER + (j)]
    if (dset->tagset != NULL && dset->tagset->num > 0)
    {
        ...
        for (ii = 0 ; ii < ntag ; ii++)
        {
            ...
            if (dset->tagset->tag[ii].set)
            {
                TF(ii, 4) = dset->tagset->tag[ii].ti;
            }
            else
            {
                TF(ii, 4) = -1.0;
        ...
        /* malloc->calloc : uninitialized bytes sent to THD_set_char_atr can put garbage in header
         * and irritate A Nugent    [4 Jun 2019 rickr] */
...
#undef TF
    ...
    for (id = 2 ; id < ATRSIZE_DATASET_RANK ; id++)
    {
        atrank[id] = 0;
    }
            ...
            for (ibr = 0 ; ibr < blk->nvals ; ibr++)
            {
                ...
                if (FUNC_IS_STAT(jv))
                {
                    ...
                    if (blk->brick_stataux[ibr] != NULL)
                    {
                        if (np > 0)
                        {
                            p1 = blk->brick_stataux[ibr][0];
                        }
                        if (np > 1)
                        {
                            p2 = blk->brick_stataux[ibr][1];
                        }
                        if (np > 2)
                        {
                            p3 = blk->brick_stataux[ibr][2];
                        }
                    }
    ...
    if (anonymize)
    {
        THD_anonymize_dset(dset);              /* 08 Jul 2005 */
    }
    EXRETURN;
}
 ...
 *
 * if bindex >= 0, return the string for just that sub-brick otherwise,      return the string for
 *all sub-bricks
 *---------------------------------------------------------------------- */
char * THD_make_statsym_string(THD_3dim_dataset *dset, int bindex)
{
    ...
    if (!blk->brick_statcode || !blk->brick_stataux)
    {
        RETURN(NULL);
    }
    ...
    if (!statsym)
    {
        fprintf(stderr, "** TMSS: calloc failure\n"); RETURN(NULL);
    }
    ...
    for (ind = bot; ind <= top; ind++)
    {
        ...
        if (FUNC_IS_STAT(code))
        {
            ...
            if (blk->brick_stataux[ind])
            {
                if (np > 0)
                {
                    p1 = blk->brick_stataux[ind][0];
                }
                if (np > 1)
                {
                    p2 = blk->brick_stataux[ind][1];
                }
                if (np > 2)
                {
                    p3 = blk->brick_stataux[ind][2];
                }
            }
