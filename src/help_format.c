...
/* old web help and new sphinx one    16 Mar 2020 [rickr] [PT: 27 Aug 2024] update the sphinx web
 * link */
...
int WSUB_check(char *str)
{
    ...
    {
        if (strncmp(str, wsub[qq], lwsub[qq]) == 0)
        {
            return qq;
        }
    }
...
int WIGN_check(char *str)
{
    ...
    {
        if (strncmp(str, wign[qq], lwign[qq]) == 0)
        {
            return qq;
        }
    }
...
int need_expansion(char *buf)
{
    ...
    {
        hpt = strstr(buf, wsub[jj]); if (hpt != NULL)
        {
            return 1;
        }
    }
...
void complex_echo_line(char *buf)
{
    ...
    while (*buf != '\0')
    {
        ...
        if (is_http || is_https)
        {
            ...
            for (hend = hll ; buf[hend] != '\0' && !isspace(buf[hend]) ; hend++)
            {
                ;                                                            /*nada*/
            }
        ...
        if (isspace(cc))
        {
            printf("&nbsp;");                    /* put in HTML escapes */
        }
        else if (cc == '&')
        {
            printf("&amp;");                     /* for special cases */
        }
        else if (cc == '<')
        {
            printf("&lt;");
        ...
        {
            printf("%c", cc);       /* perfectly normal character */
        }
...
void setup_wsub(int nskip, char **skip)
{
    ...
    for (ii = 0 ; ii < nwign ; ii++)
    {
        lwign[ii] = -strlen(wign[ii]);
    }
...
static void qsrec_intchar(int n, int *ar, char **iar, int cutoff)
{
    ...
    while (mst > 0)
    {
        ...
        if (a[left] > a[i])
        {
            QS_SWAPF(a[left], a[i]); QS_SWAPV(ia[left], ia[i]);
        }
        if (a[left] > a[right])
        {
            QS_SWAPF(a[left], a[right]); QS_SWAPV(ia[left], ia[right]);
        }
        if (a[i] > a[right])
        {
            QS_SWAPF(a[right], a[i]); QS_SWAPV(ia[right], ia[i]);
        }
