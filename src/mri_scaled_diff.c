...
/*! Return a measure of the difference between 2 images bim and nim, possibly with a mask to
 * indicate which voxels to include.
 *
 *  The difference is defined as dd below:
 *
 *  ss = min sum  [ bim[i] - a * nim[i] ]**2 a   i
 *
 *  dd = sqrt( ss / #voxels )  i.e., the RMS difference.
 *
 *  That is, we least squares fit image nim to bim by a scale factor, and the residual determines
 *the difference.  The minimizing value of
 *  'a' is given by
 *
 *  a = BN / NN  where BN = sum [ bim[i]*nim[i] ] NN = sum [ nim[i]**2     ]
 *
 ...
 *  - If bim!=0 and nim=0, then the return value is computed with a=0 (that is, it will be the RMS
 *value of bim)
 * ------------------------------------------------------------------------*/
...
float mri_scaled_diff(MRI_IMAGE *bim, MRI_IMAGE *nim, MRI_IMAGE *msk)
{
    ...
    if (bim == NULL || nim == NULL)
    {
        RETURN(-1.0f);
    }

    nvox = bim->nvox; if (nim->nvox != nvox)
    {
        RETURN(-1.0f);
    }
