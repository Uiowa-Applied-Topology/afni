/***
 * This sample program shows how to send volumes of data to AFNI through a TCP/IP socket, using the
 *NIML interface.  RWCox - Jul 2009
 ***/
...
int main(int argc, char *argv[])
{
    ...
    while (iarg < argc && argv[iarg][0] == '-')
    {
        ...
        {
            dt = (int)strtod(argv[++iarg], NULL); if (dt < 9)
            {
                dt = 9;
            }
            iarg++; continue;
    ...
    if (!DSET_LOADED(dset))
    {
        ERROR_exit("Can't load dataset '%s'", argv[iarg]);
    }
    ...
    if (verbose)
    {
        INFO_message("geometry string = '%s'", geomstr);
    }
    ...
    /* loop until AFNI connects (answers the call), printing a '.' every 1/2 second to keep the user
     * happy */
    ...
    /* all sub-bricks in the input dataset will be sent to be sub-brick #0 in the dataset inside
     * AFNI
     * -- if you don't want this behavior, and want the dataset inside AFNI to keep growing, then
     *don't set this attribute! */

    if (!do_accum)
    {
        NI_set_attribute(nel, "index", "0");
    }
    ...
    if (verbose)
    {
        INFO_message("Starting sub-brick loop");
    }
    ...
    for (kk = 0 ; kk < nval ; kk++)
    {
        ...
        if (verbose)
        {
            ININFO_message("Sent %d bytes for sub-brick #%d in %d ms",
                           nn, kk, ctnew - ctold);
        }
