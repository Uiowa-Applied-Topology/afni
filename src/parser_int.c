...
/*------------------------------------------------------------------
 * Input  = expression string Output = structure containing information about how to evaluate the
 *expression; should be free-d when unneeded;
...
PARSER_code * PARSER_generate_code(char *expression)
{
    ...
    if (first)
    {
        srand48((long)time(NULL) + (long)getpid()); first = 0;
    }
    if (expression == NULL)
    {
        return NULL;
    ...
    {
        cc = expression[ii];
        if (!isspace(cc) && !iscntrl(cc))
        {
            exp[jj++] = cc;
        }
    }
    exp[jj] = '\0';
    nexp    = strlen(exp); if (nexp == 0)
    {
        return NULL;
    }
...
/*---------------------------------------------------------------
 * pc   = code to evaluate expression atoz = double [26] containing values for variables A,B,...,Z
 * -----------------------------------------------------------------*/
...
/*----------------------------------------------------------------------
 * Return 1 if the given code uses the symbol given by the first character of sym, otherwise return
 *0 - 15 Sep 1999 - RWCox.
 * ------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------
 * pc    = code to evaluate expression atoz  = double * [26] containing values for variables A..Z nv
 *    = length of vectors vout  = double [nv]; will get the output of the expression evaluated using
 *the values in atoz
 *
 * The only reason for calling this routine instead of PARSER_evaluate_one nv different times is
 *efficiency.
 * ------------------------------------------------------------------------*/
                            ...
                            int nv, double vout[])
{
    ...
    for (aa = 0 ; aa < 26 ; aa++)
    {
        ...
        else
        {
            if (zerovec == NULL)
            {
                zerovec = (doublereal *)calloc(sizeof(doublereal), nv);
            }
            myatoz[aa] = zerovec;
        }
    }
 ...
 *   expr = expression - first symbol found will be the variable nt   = number of points tz   =
 * value of first point (the variable) dt   = spacing between points vec  = pointer to pre-allocated
 *output location of length nt Return value is 1 for good results, 0 for errors.
 *
...
doublereal rhddc2_(doublereal *x, doublereal *y, doublereal *z)
{
    ...
    xx = fabs((double)*x); if (xx >= 2.0)
    {
        return 0.0;                                      /* way too big */
    }
    yy = fabs((double)*y); if (yy >= 2.0)
    {
        return 0.0;
    }
    zz = fabs((double)*z); if (zz >= 2.0)
    {
        return 0.0;
    }
...
doublereal acfwxm_(doublereal *apar, doublereal *bpar, doublereal *cpar, doublereal *flev)
{
    ...
    if (ACFaa > 1.0 || ACFaa < 0.0)
    {
        return 0.0;
    }
    if (ACFbb <= 0.0 || ACFcc <= 0.0)
    {
        return 0.0;
    }
    if (ACFflev <= 0.0 || ACFflev >= 1.0)
    {
        return 0.0;
    }
    ...
    rhalf = minimize_in_1D(0.50 * rhalf, 2.00 * rhalf, acfwxm_cost);
    rhalf = minimize_in_1D(0.80 * rhalf, 1.25 * rhalf, acfwxm_cost);
...
static void pfit_allocate_atoz(int nval, float *depar)
{
    ...
    for (jj = 0 ; jj < natoz ; jj++)
    {
        dval[jj] = (double)depar[jj];
    }
    return;
}
...
static double pfit_ufunc(int nnn, double *par)
{
    ...
    for (tt = 0 ; tt < natoz ; tt++)
    {
        ...
        if (!isfinite(dval[tt]))
        {
            ERROR_message("pfit_ufunc: dval[%d] = %g", tt, dval[tt]);
        }
        if (!isfinite(eval[tt]))
        {
            ERROR_message("pfit_ufunc: eval[%d] = %g", tt, eval[tt]);
        }
#endif
        rrr = dval[tt] - eval[tt];
        if (mcode == 1)
 ...
 *   nval    = number of time series points indval  = array of independent values [nval] depval  =
 * array of dependent (to be fit) values [nval] expr    = expression to be fit indet   = single
 *character indicating which variable name in expr corresponds to indval parbot  = Array of length
 *[26]; parbot[j] indicates the smallest allowed value for the j-th variable ("a" is j=0, etc).
 *   partop  = Array of length [26]; partop[j] indicates the largest allowed value for the j-th
 *variable.
 ++ If the j-th variable is not used in expr, then parbot[j] and partop[j] are ignored.
 ++ parbot[j] and partop[j] are also ignored for the 'indet' variable.
 ++ Otherwise, if parbot[j] >= partop[j], then the j-th variable value is fixed at parbot[j].
 ++ If parbot[j] < partop[j], then this is the range of values that will be searched for the j-th
 ++parameter.
 ++  parout  = Array of length [26]; gets the output values of all estimated parameters.
 ++  meth    = 1 for L1 fitting, 2 for L2 fitting wtar    = weight array [nval] -- can be NULL
 ++
 ++  The return value is a malloc()-ed float array of the fitted time series (e.g., for plotting on
 ++top of the data depval[]).
 ++
 ++  float xar[100] , yar[100] , parbot[26] , partop[26] , parout[26] , *yfit ;
 ++  parbot[0] = 0.0f ; partop[0] = 30.0f ;
 ++  parbot[1] = partop[1] = 0.7f ;
 ++  partop[2] = -5.0f ; partop[2] = 5.0f ;
 ++  yfit = PARSER_fitter( 100 , xar , yar ,
 ++                        "c*sin(a*x+b)" , "x" , parbot , partop , parout , 2 , NULL ) ;
 ++
                      ...
                      int meth, float *wtar)
{
    ...
    for (jj = 0 ; jj < 26 ; jj++)
    {
        ...
        cjj = 'A' + jj;
        if (!PARSER_has_symbol(&cjj, pcode))
        {
            continue;
        }
        if (parbot[jj] < partop[jj])
        {
            ...
            {
                ERROR_message("PARSER_fitter: parbot[%d] = %g  partop = %g", jj, parbot[jj],
                              partop[jj]);
                nbad++;
            }
        }
    ...
    for (aa = 0 ; aa < nfix ; aa++)
    {
        pfit_fill_atoz(jfix[aa], vfix[aa]);
    }
    ...
    for (aa = 0 ; aa < nfix  ; aa++)
    {
        parout[jfix[aa]] = vfix[aa];
    }
    for (aa = 0 ; aa < nfree ; aa++)
    {
        parout[jfree[aa]] = pval[aa];
    }
    ...
    for (aa = 0 ; aa < nval ; aa++)
    {
        fitts[aa] = (float)eval[aa];
    }

    pfit_free_atoz(); if (wtard != NULL)
    {
        free(wtard);
    }
    return fitts;
