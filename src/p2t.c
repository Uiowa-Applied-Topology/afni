...
/*----------------------------------------------------------------------
 * code for inverse of central t distribution Inputs: p  = double sided tail probability nu =
 * degrees of freedom Output: T such that P( |t| > T ) = p
 *
 * This version is only good for nu >= 5, since it uses the approximations in Abramowitz and Stegun,
 * Eq. 26.7.5 (p. 949).
 * ------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------
 * compute log of complete beta function, using the Unix math library's log gamma function.  If this
 * is not available, tough luck.
 * -----------------------------------------------------------------*/
 ...
 *
 *   computes incomplete beta function ratio for arguments x between zero and one, p and q positive.
...
/*----------------------------------------------------------------
 *  algorithm as 109 appl. statist. (1977), vol.26, no.1 (replacing algorithm as 64  appl. statist.
 * (1973), vol.22, no.3)
 *
 *  Remark AS R83 and the correction in vol40(1) p.236 have been incorporated in this version.
 *
 *  Computes inverse of the incomplete beta function ratio for given positive values of the
 * arguments p and q, alpha between zero and one.
 *  log of complete beta function, beta, is assumed to be known.
 * ------------------------------------------------------------------*/
...
double incbeta_inverse(double alpha, double p, double q, double beta)
{
    ...
    if (p <= ZERO || q <= ZERO || alpha < ZERO || alpha > ONE)
    {
        return -1.0;
    }
...
#if 0
    iex = -5.0 / (pp * pp) - 1.0 / (a * a) - 13.0; if (iex < SAE)
    {
        iex = SAE;
    }
    acu = pow(10.0, iex);
...
lab11:
    ...
    if (prev <= acu || y * y <= acu || fabs(xinbta - tx) < fpu)
    {
        goto lab12;
    }
    xinbta = tx;
