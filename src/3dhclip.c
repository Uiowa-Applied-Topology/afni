...
int main(int argc, char *argv[])
{
    ...
    while (iarg < argc && argv[iarg][0] == '-')
    {
        if (strcmp(argv[iarg], "-bper") == 0)
        {
            ...
            if (bper < 1 || bper > 99)
            {
                fprintf(stderr, "** Illegal -bper value\n"); exit(1);
            }
            iarg++; continue;
        }
        ...
        if (strcmp(argv[iarg], "-bmin") == 0)
        {
            ...
            if (bmin < 0)
            {
                fprintf(stderr, "** Illegal -bmin value\n"); exit(1);
            }
            iarg++; continue;
        }
    ...
    if (sfim == NULL)
    {
        fprintf(stderr, "** Can't load dataset brick\n"); exit(1);
    }
    ...
    if (kk == 0)
    {
        fprintf(stderr, "** All voxels are zero!\n"); exit(1);
    }
    nbin = kk + 1;
    ...
    if (nbot >= nbin - 9)
    {
        fprintf(stderr, "** Base point on histogram too high\n"); exit(1);
    }
    ...
    if (nmax == 0)
    {
        fprintf(stderr, "** No maxima above base point\n"); exit(1);
    }
