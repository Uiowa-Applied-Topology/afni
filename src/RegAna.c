 ...
 *
 * File:    RegAna.c Author:  B. Douglas Ward Date:    02 September 1998
 *
 * Mod:     Restructured matrix calculations to improve execution speed. Date:    16 December 1998
 *
 * Mod:     Added routines for matrix calculation of general linear tests. Date:    02 July 1999
 *
 ...
 *
 * Mod:     If use_psinv is 1, use matrix_psinv() instead of inversion. Date     19 Jul 2004 --
 * RWCox
 *
...

{
    ...
    for (i = 0; i < xtxinv.rows; i++)
    {
        ...
        else if (num < -MAXT * stddev)
        {
            tstat = -MAXT;
        }
        else if (stddev < EPSILON)
        {
            tstat = 0.0;
        }
        else
...

{
    ...
    msreg = (sser - ssef) / (p - q);    if (msreg < 0.0)
    {
        msreg = 0.0;
    }
    msef = ssef / (n - p);            if (msef < 0.0)
    {
        msef = 0.0;
    }
