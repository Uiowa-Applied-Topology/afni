...
void UC_read_opts(int argc, char *argv[])
{
    ...
    while (nopt < argc && argv[nopt][0] == '-')
    {
        ...
        if (strncmp(argv[nopt], "-prefix", 6) == 0)
        {
            ...
            if (nopt >= argc)
            {
                UC_syntax("-prefix needs an argument!");
            }
        ...
        if (strncmp(argv[nopt], "-mask", 5) == 0)
        {
            ...
            if (nopt >= argc)
            {
                UC_syntax("need arguments after -mask!");
            }
            mset = THD_open_dataset(argv[nopt]);
            if (mset == NULL)
            {
                UC_syntax("can't open -mask dataset!");
            }
            ...
            if (UC_mask == NULL)
            {
                UC_syntax("can't use -mask dataset!");
            }
            UC_mask_hits = THD_countmask(UC_mask_nvox, UC_mask);
            if (UC_mask_hits == 0)
            {
                UC_syntax("mask is all zeros!");
            }
            if (!UC_be_quiet)
            {
        ...
        if (strcmp(argv[nopt], "-ptail") == 0)
        {
            if (++nopt >= argc)
            {
                UC_syntax("-ptail needs an argument!");
            }
    ...
    if (nopt >= argc)
    {
        UC_syntax("no input dataset name?");
    }
...
float UC_unusuality(int ndim, float *ref, int nvec, float **vec)
{
    ...
    if (ndim < 4 || nvec < 4 || ref == NULL || vec == NULL)
    {
        return 0.0;
    }
...
int main(int argc, char *argv[])
{
    ...
    if (!UC_be_quiet)
    {
        printf("--- computing u"); fflush(stdout);
    }
