...
int cluster_alphaindex_64(int csize, int nz, float fw, float pv)
{
    ...
    else if (fw > fwhm[NFWHM - 1])
    {
        return -1;                                       /** bad input fw **/
    }
    if (pv < pval[0])
    {
        pv = pval[0];
    }
    else if (pv > pval[NPVAL - 1])
    {
        return -1; /** bad input pv */
    }
    iz = nz - 1;   /* slice index in nx064 array*/

    /* find FWHM index so that fwhm[kf] >= fw >= fwhm[kf-1] */

    for (kf = 1 ; kf < NFWHM && fw > fwhm[kf] ; kf++)
    {
        ;                                            /*nada */
    }
    if (kf == NFWHM)
    ...
    for (lp = 1 ; lp < NPVAL && pv > pval[lp] ; lp++)
    {
        ;                                            /*nada*/
    }
    if (lp == NPVAL)
    ...
    for (ja = 0 ; ja < NALPH ; ja++)
    {
        ...
        if (csize >= cc)
        {
            return ja;                     /* significance at this alpha */
        }
    }
