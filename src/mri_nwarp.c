...
/***-----------------------------------------------------------------------
 * Potential housecleaning chores:           [Dec 2018] DONE * remove the USE_HLOADER code forever
 *DONE * remove the ALLOW_DUPLO code forever DONE * don't need ALLOW_QMODE #define - that is, make
 *this permanent;
 *  DONE    and same for ALLOW_PLUSMINUS
 *
 *  BUT  * don't remove/alter ALLOW_BASIS5 and ALLOW_INEDGE yet;
 *          still on the fence about these, and the former is a lot of complex code that shouldn't
 *be cast aside just yet these options aren't useful in general, but there may be some cases where
 *they make a difference (or maybe not)
 *
 *  TBD  * Remove external slopes and replace with affine transformation that applies internally and
 *externally TBD  * Remove 3dNwarpCalc functionality, as it becomes complicated when the affine part
 *of the warp is separated from the small wiggly (nonlinear) part -- the grid domain of a warp
 *sometimes has to change when an operation (e.g., inverse) is performed.
 * ---------------------------------------------------------------------------***/
...
/** Note that the functions needed only in 3dQwarp (4000+ lines of code) are only compiled if macro
 * ALLOW_QWARP is #define-d -- see 3dQwarp.c. Search ahead for string Q1 for the table of contents
 * for this code.
 *//*........................................................................*/
 ...
 *
 *   (C1) Prolegomenon, Definitions, and Background (C2) Functions for computing external slopes for
 *a warp [DELETED] (C3) Functions for elementary operations on warps (C4) Functions for extending or
 *truncating a warp grid size (C5) Functions to compute various 'norms' (size estimates) of a warp
 *(C6) Functions that do conversions between index warps and dataset warps (C7) Functions to compute
 *auxiliary functions of an index warp, including stuff for the 3dQwarp penalties, and the
 *3dNwarpFuncs output (C8) Functions for interpolating all 3 components of an index warp at once
 *(C9) Functions to carry out warp compositions (C10) Functions to invert an index warp in its
 *entirety (C11) Functions to compute the 'square root' of a index warp [USEFUL?] (C12) Functions
 *for taking stuff from 3dAllineate -nwarp output and producing a warp (obsolete, or nearly so --
 * DO NOT USE!) (C13) Functions for interpolating images (C14) Functions used in 3dNwarpXYZ.c to
 *warp a small number of points given by x,y,z triples, rather than warping a whole grid at once
 *(C15) Functions to warp a dataset, given a warp defined by a dataset (C16) Reverse Polish Notation
 *warp calculator (3dNwarpCalc program) [DELETED] (C17) Functions for reading warps and
 *inverting/catenating them right away (for the '-nwarp' input of various 3dNwarp programs)
 *
 *  The following functions are only compiled if ALLOW_QWARP is #define-d. Note that these functions
 *deal ONLY with the 'small wiggly' part of an index warp, not the affine part.
 *
 *   (Q1) Introduction to 3dQwarp, and Global variables for 3dQwarp-ing (includes an outline of the
 *sequence of function calls for 3dQwarp) (Q2) Basis functions (cubic and quintic) for warping (Q3)
 *Functions to create a patch warp from basis functions and parameters (Q4) Evaluate the
 *incrementally warped source image at the combination of the current global warp and the local
 *patch warp (Q5) Functions to evaluate the warp penalty and cost function (Q6) Functions to setup
 *global variables for warp optimization process (Q7) Function that actually optimizes one
 *incremental patch warp (Q8) Functions that drive the warp searching process by looping over
 *patches of shrinking sizes (Q9) All the above functions copied and edited for plusminus warping,
 *which requires doubling up on the calculations in places as two datasets are being warped now
 * -----------------------------------------------------------------------------*/
/*:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::*/
/*===========================================================================*/
...
/*---------- (C1) Prolegomenon -----------------------------------------------
 *
 * This file contains the AFNI functions for nonlinear warping, which are wrapped up in various ways
 *in the programs 3dQwarp.c and 3dNwarp*.c;
 * as mentioned earlier, this file is intended to be #include-d into a program, although it is also
 *compiled into the AFNI libmri.  The 3dQwarp functions are only compiled if macro ALLOW_QWARP is
 * #define-d before this file is #include-d, since they are only used in the 3dQwarp.c program and
 *they comprise about a lot of this file.
 *
 * --------------- Definition of an Index Warp Struct ---------------------------
 *
 * The functions herein mostly operate on IndexWarp3D structs.  Such a struct represents an "index
 *warp in 3D" -- that is, it maps indexes and not coordinates.  Most functions that operate on these
 *things start with the string 'IW3D_'.  The basic struct is pretty simple (from mri_nwarp.h):
 *
 *    typedef struct {
 *      int    nx ,  ny ,  nz ;  -- Grid dimensions float *xd , *yd , *zd ;  -- Displacements (in
 *index space) float *hv , *je , *se ;  -- Various auxiliary volumes int use_amat ;           --
 * Whether to use amat mat44 amat , amati ;     -- Affine component of warp (and its inverse)
 *       -- stuff below here is for conversion to/from 3D dataset format --
 *      mat44 cmat , imat ;      -- cmat: i->x ; imat: x->i char *geomstring ;       -- grid
 *geometry of 3D dataset int view ;               -- view/space of the dataset
 *    } IndexWarp3D ;
 *
 *    nx, ny, nz = dimensions of the grid arrays (nz==1 is allowed) xd, yd, zd = index displacements
 * = arrays of size nx*ny*nz;
 *                 the voxel at 3D index (i,j,k) is mapped to i + xd[i+j*nx+k*nx*ny] j +
 * yd[i+j*nx+k*nx*ny] k + zd[i+j*nx+k*nx*ny] hv         = volume of the warped cube (hexahedron)
 *[i,i+1] X [j,j+1] X [k,k+1] je         = bulk volume distortion energy (part of 3dQwarp penalty)
 *se         = shear and vorticity energy (other part of penalty);
 *               --> hv, je, se are usually NULL, and are what gets calculated in 3dNwarpFuncs via
 *function IW3D_load_bsv() amat       = matrix-vector [M c] to modify displacements:
 *                       [ i + xd(i,j,k) ] [M] [ j + yd(i,j,k) ] + [c] [ k + zd(i,j,k) ] when
 *(i,j,k) is outside the (0..nx-1,0..ny-1,0..nz-1) grid, (xd,yd,zd)=(0,0,0) and amat still supplies
 *the warp.
 *    amati      = inverse of amat
 *
 *       The components below are for relating the IndexWarp3D struct to a 'real' space 3D dataset:
 *
 *    cmat       = matrix to get DICOM x,y,z coordinates from i,j,k imat       = inverse of cmat
 *(convert coordinate to index) geomstring = string to represent geometry of 3D dataset that
 *corresponds to this index warp; if the index warp was created directly from a dataset via function
 *IW3D_from_dataset(), then that's when geomstring is set, otherwise it will be set using function
 *IW3D_adopt_dataset() -- along with the other dataset-ish stuff (cmat, imat, view).
 *    view       = view code for that dataset (should be upgraded to space)
 *
 * There is also a struct IndexWarp3D_pair, which holds 2 warps, one the forward warp and one the
 *inverse warp via function IW3D_invert().  This struct doesn't get used much, as it turns out.  At
 *one time, I thought I would always keep the warp inverse around, at least for input and output to
 *datasets (cf. infra), but that didn't happen.
 *
 * --------------- Warps stored as 3D Datasets -----------------------------------
 *
 * Externally, warps are stored as 3D datasets, with (at least) 3 sub-bricks:
 * one for each of the x, y, and z displacements.  A warp dataset is created from an IndexWarp3D
 *struct using function IW3D_to_dataset(), with the geomstring component providing the key
 *information about how to lay out the 3D dataset grid.  The xd, yd, zd index displacements are
 *converted to DICOM x,y,z displacements in mm using the cmat component.  Note, however, that the
 *dataset itself may not be stored in DICOM RAI order, but that the displacements always refer to
 *DICOM ordered vectors.  This can be a little confusing, so beware.
 *
 * Incidentally, storing a warp as a set of displacements has a number of advantages over storing
 *the actual displaced locations (i.e., indexes or coordinates).  For one, the identity warp is just
 *a big collection of zeros.  For another, interpolation to a different grid is simpler.
 *
 * -------------- Understanding Warps and Displacements --------------------------
 *
 * Since warps are stored as displacements, the actual transformation of (index) coordinates is
 *
 *   i -> i + xd[i,j,k]  (loosely speaking) j -> j + yd[i,j,k] k -> k + zd[i,j,k]
 *
 * so you will see various places where the addition of the original coordinates is needed; for
 *example, when composing 2 warps in function IW3D_compose(), we have to compute B(A(x)), where the
 *warps are defined by A(x) = x + a(x) and B(x) = x + b(x), so B(A(x)) = x+a(x) + b(x+a(x))
 * -- and so b() must be interpolated at these non-integer index values --
 * which explains why the output warp is computed as a(x)+b(x+a(x)) -- the double appearance of a(x)
 *is required by this storage mechanism.
 *
 * In a more complicated case, in inverting an index warp, the following composition is needed in
 *function IW3D_invert_newt(): B(2x-A(B(x))). This computation is carried out by the following
 *sequence of steps A(x)              = x + a(x)  [by definition] B(x)              = x + b(x)  [by
 *definition] A(B(x))           = x + b(x) + a(x+b(x)) 2x - A(B(x))      = x - b(x) - a(x+b(x)) B(
 * 2x - A(B(x)) ) = x - b(x) - a(x+b(x)) + b(x-b(x)-a(x+b(x))) You should understand how this
 *expansion works in order to be able to understand any of the warp manipulation functions.
 *
 * --------------- Forward and Inverse Warps -------------------------------------
 *
 * A nonlinear warp stores the displacements (in indexes or DICOM mm) from a base dataset grid to a
 *source dataset grid.  For computing the source dataset warped to the base dataset grid, these
 *displacements are needed, so that for each grid point in the output (warped) dataset, the
 *corresponding location in the source dataset can be found.  That is, this 'forward' warp is good
 *for finding where a given point in the base dataset maps to in the source dataset. However, for
 *finding where a given point in the source dataset maps to in the base dataset, the 'inverse' warp
 *is needed.
 *
 * Another way of thinking about warps is to imagine the forward warp reaching out and pulling the
 *source back to the same location as the base dataset. Don't think of it as pushing the source to
 *the base! That is what the inverse warp does, but since datasets live on a regular 3D grid,
 *pushing a source grid point to the base dataset will almost surely not end up on a base dataset
 *grid point, and so for most purposes 'pushing' isn't desired.
 *
 * --------------- OpenMP code ---------------------------------------------------
 *
 * A large number of compute intensive functions use OpenMP to parallelize the lengthy computations.
 *This makes the source code somewhat more complicated and less transparent than it could otherwise
 *be.  If you need some tutorial on OpenMP, start with
...
/* The following macros are for loading and unloading the extra slope variables for extending a warp
...
#define ES_DECLARE_FLOATS                                                                          \
        float es_xd_xp = 0.0f, es_xd_xm = 0.0f, es_xd_yp = 0.0f, es_xd_ym = 0.0f, es_xd_zp = 0.0f, \
              es_xd_zm = 0.0f,                                                                     \
              es_yd_xp = 0.0f, es_yd_xm = 0.0f, es_yd_yp = 0.0f, es_yd_ym = 0.0f, es_yd_zp = 0.0f, \
              es_yd_zm = 0.0f,                                                                     \
              es_zd_xp = 0.0f, es_zd_xm = 0.0f, es_zd_yp = 0.0f, es_zd_ym = 0.0f, es_zd_zp = 0.0f, \
              es_zd_zm = 0.0f
...
/* The following functions are for providing a linear extension to a warp, based on the final 3..5
 * layers in the warp. These macros compute the slope based on 3, 4, or 5 points.
 *//*-------------------------------------------------------------------------*/
 ...
 * that is, the slope is calculated using 3, 4, or 5 points (one from each vector), and then these
 *are averaged to give the output. In this library, these vectors are formed from the outer layers
 *of the faces of the warp in function IW3D_load_external_slopes().
 *//*-------------------------------------------------------------------------*/
...
/* Compute the external slopes in a warp, using the mean_slope() function above applied to each
 * face, extracting the vectors from the face's layers. Code for deciphering the slope names, where
 * 'Q' is 'x' or 'y' or 'z'
 *    es = external slope Qd = Q direction displacement Qm = along the Q face in the minus direction
 *Qp = along the Q face in the plus direction Note that the slopes are always in the positive
 *direction of the coordinate being changes, even at the minus faces.  For example, es_xd_xm > 0
 *means that for the x-index being negative (outside the 'xm' face), then the x-displacement ('xd')
 *will get a negative addition of es_xd_xm * x-index (multiplying the positive slope times the
 *negative index).
 *
...
void IW3D_load_external_slopes(IndexWarp3D *AA)
{
    ...
    for (pp = 0 ; pp < 5 ; pp++)
    {
        vec[pp] = (float *)malloc(sizeof(float) * nbig);
    }
...
#define DD(i, j, k)    dar[(i) + (j) * nx + (k) * nxy] /* access dar[i,j,k] */
    ...
    for (pp = 0 ; pp < nvec ; pp++)               /* at x=nx-1 face */
    {
        for (ss = qq = 0 ; qq < ny ; qq++)
        {
            for (rr = 0 ; rr < nz ; rr++)
            {
                vec[pp][ss++] = DD(nx - 1 - pp, qq, rr);
            }
        }
    }

    /* below, we have to negate the mean_slope() output since we loaded the faces in the order of
     * decreasing x-index */
    AA->es_xd_xp = -mean_slope(nvec, veclen, vec);      /* x plus face */

    /** below here, edited copies for all combinations of displacement vectors (xd,yd,zd) and face
     * (xm,xp,ym,yp,zm,zp) **/
...
/* Very similar to above, but for warps stored in a dataset; here, the external slopes (in mm per
 * voxel) are stored in the output vector. This is used in the 3dNwarpXYZ.c functions defined below
 * in (C14), but not elsewhere.
 *//*-------------------------------------------------------------------------*/
...
floatvec * THD_nwarp_external_slopes(THD_3dim_dataset *dset_nwarp)
{
    ...
    if (DSET_BRICK_TYPE(dset_nwarp, 0) != MRI_float)
    {
        RETURN(NULL);
    }
    DSET_load(dset_nwarp); if (!DSET_LOADED(dset_nwarp))
    {
        RETURN(NULL);
    }
 ...
 *      creation, destruction, copying, scaling, etc.  Plus warp pair stuff. These functions operate
 *on the displacements (xd,yd,zd) = the
...
void IW3D_zero_fill(IndexWarp3D *AA)
{
    ...
    if (AA->xd != NULL)
    {
        AAmemset(AA->xd, 0, nbyt);
    }
    if (AA->yd != NULL)
    {
        AAmemset(AA->yd, 0, nbyt);
    }
    if (AA->zd != NULL)
    {
        AAmemset(AA->zd, 0, nbyt);
    }
    if (AA->hv != NULL)
    {
        AAmemset(AA->hv, 0, nbyt);
    }
    if (AA->je != NULL)
    {
        AAmemset(AA->je, 0, nbyt);
    }
    if (AA->se != NULL)
    {
        AAmemset(AA->se, 0, nbyt);
    }
    EXRETURN;
}
...
int IW3D_is_zero(IndexWarp3D *AA)
{
    ...
    for (ii = 0 ; ii < nvox ; ii++)
    {
        if (xd[ii] != 0.0f || yd[ii] != 0.0f || zd[ii] != 0.0f)
        {
            return 0;
        }
    }
...
void IW3D_adopt_dataset(IndexWarp3D *AA, THD_3dim_dataset *dset)
{
    ...
    {
        AA->geomstring = NULL;                 /* should not happen */
    }
...
/* Warp pairs aren't used much; at one time, I thought they'd be used more (keep a copy of the
 * inverse warp along with the forward warp at all times), but that just turned out not to happen.
 *//*-------------------------------------------------------------------------*/
...
IndexWarp3D * IW3D_sum(IndexWarp3D *AA, float Afac, IndexWarp3D *BB, float Bfac)
{
    ...
    if (BB->nx * BB->ny * BB->nz != nxyz)
    {
        RETURN(NULL);          /* bad inputs! */
    }
...
/* Extend (and/or truncate) an index warp. Extension outside the original grid is via linear
 * extrapolation, unless zpad != 0, in which case extensions of the displacements outside the
 * original grid are just set to zero.      ?????
 *//*-------------------------------------------------------------------------*/
                          ...
                          int nzbot, int nztop, int zpad)
{
    ...
    nxnew = nxold + nxbot + nxtop; if (nxnew < 1)
    {
        RETURN(NULL);
    }
    nynew = nyold + nybot + nytop; if (nynew < 1)
    {
        RETURN(NULL);
    }
    nznew = nzold + nzbot + nztop; if (nznew < 1)
    {
        RETURN(NULL);
    }
...
#define BR(qd, i, j, k)    BB->qd[(i) + (j) * nxnew + (k) * nxynew] /* (i,j,k) in BB array */
    ...
    {
        int nxyold = nxold * nyold, nxynew = nxnew * nynew, nxo1 = nxold - 1, nyo1 = nyold - 1,
            nzo1 = nzold - 1;
...
#pragma omp parallel if( nznew > 16 )
        { int   ii, jj, kk, iq, jq, kq, di, dj, dk;
          ...
          for (kk = 0 ; kk < nznew ; kk++)
          {
              ...
              if (kq < 0)
              {
                  dk  = kq; kq = 0; Exz = AA->es_xd_zm * dk; Eyz = AA->es_yd_zm * dk;
                  Ezz = AA->es_zd_zm * dk;
              }
              else if (kq > nzo1)
              {
                  dk  = kq - nzo1; kq = nzo1; Exz = AA->es_xd_zp * dk; Eyz = AA->es_yd_zp * dk;
                  Ezz = AA->es_zd_zp * dk;
              }
              for (jj = 0 ; jj < nynew ; jj++)
              {
                  ...
                  if (jq < 0)
                  {
                      dj  = jq; jq = 0; Exy = AA->es_xd_ym * dj; Eyy = AA->es_yd_ym * dj;
                      Ezy = AA->es_zd_ym * dj;
                  }
                  else if (jq > nyo1)
                  {
                      dj  = jq - nyo1; jq = nyo1; Exy = AA->es_xd_yp * dj; Eyy = AA->es_yd_yp * dj;
                      Ezy = AA->es_zd_yp * dj;
                  }
                  for (ii = 0 ; ii < nxnew ; ii++)
                  {
                      ...
                      if (iq < 0)
                      {
                          di  = iq; iq = 0; Exx = AA->es_xd_xm * di; Eyx = AA->es_yd_xm * di;
                          Ezx = AA->es_zd_xm * di;
                      }
                      else if (iq > nxo1)
                      {
                          di  = iq - nxo1; iq = nxo1; Exx = AA->es_xd_xp * di;
                          Eyx = AA->es_yd_xp * di; Ezx = AA->es_zd_xp * di;
                      }
                      if (di || dj || dk)
                      {
...
/*----------------------------------------------------------------------------------*/
    ...
    /* the extended warp requires a modified geometry string, done in the most brute force way
     * anyone could ever imagine:
     *   make a dataset (header only) = qset zero pad it (header only)    = adset the new index warp
     *adopts adset's geometry throw these temporary datasets away as untouchable trash */
...
/* This function used to be more impressive, but now just calls the one above to do its work -- but
 * there is no extrapolation (zpad==1).
 *//*-------------------------------------------------------------------------*/
 ...
 * this is done the brute force way, by conversion to an index warp, extension of that, and then
 *conversion back to a dataset struct. Note that there is no option for zero-padded extension.
 *//*-------------------------------------------------------------------------*/
                                    ...
                                    int nzbot, int nztop)
{
    ...
    {
        ERROR_message(
            "Warp extend(%d,%d,%d,%d,%d,%d) fails: can't convert dataset %s to IW3D format",
            nxbot, nxtop, nybot, nytop, nzbot, nztop,
            DSET_BRIKNAME(dset_nwarp));
        RETURN(NULL);
    }
    ...
    {
        ERROR_message(
            "Warp extend(%d,%d,%d,%d,%d,%d) fails: can't extend IW3D format of dataset %s",
            nxbot, nxtop, nybot, nytop, nzbot, nztop,
            DSET_BRIKNAME(dset_nwarp));
        RETURN(NULL);
    }
    ...
    {
        ERROR_message(
            "Warp extend(%d,%d,%d,%d,%d,%d) fails: can't convert IW3D format of %s back to dataset",
            nxbot, nxtop, nybot, nytop, nzbot, nztop,
            DSET_BRIKNAME(dset_nwarp));
    }
    RETURN(qset);
 ...
 *  A) the box that includes all displacements > dthr B) the box that includes the displaced
 *locations of all voxels with displacement > dthr It is possible (if unlikely) that the box will
 *actually be bigger than the warp grid, due to condition B.
 *//*-------------------------------------------------------------------------*/
...
int_sextet IW3D_warpbox(IndexWarp3D *AA, float fac, float dthr)
{
    ...
    for (qq = 0 ; qq < nxyz ; qq++)
    {
        ...
        if (qmm[qq] == 0)
        {
            continue;               /* not in mask ==> don't check */
        }
    ...
    if (xbot < ibot)
    {
        ibot = (int)floorf(xbot);
    }
    if (ybot < jbot)
    {
        jbot = (int)floorf(ybot);
    }
    if (zbot < kbot)
    {
        kbot = (int)floorf(zbot);
    }
    if (xtop > itop)
    {
        itop = (int)ceilf(xtop);
...
void IW3D_bounding_box_clear(IndexWarp3D *AA, float thresh)
{
    ...
    if (ibot == 0 && itop == 0 && jbot == 0 && jtop == 0 && kbot == 0 && ktop == 0)
    {
        EXRETURN;
    }
...
/* (C5) The functions below compute various 'norms' (size estimates) of an index warp.  These
 * comprise pretty simple code.          */
/*===========================================================================*/
...
float_triple THD_nwarp_maxdisp(THD_3dim_dataset *nset)
{
    ...
    if (DSET_BRICK_TYPE(nset, 0) != MRI_float)
    {
        RETURN(dxyz);
    }
    DSET_load(nset); if (!DSET_LOADED(nset))
    {
        RETURN(dxyz);
    }
    ...
    for (ii = 0 ; ii < nvox ; ii++)
    {
        val = fabsf(xd[ii]); if (val > dx)
        {
            dx = val;
        }
        val = fabsf(yd[ii]); if (val > dy)
        {
            dy = val;
        }
        val = fabsf(zd[ii]); if (val > dz)
        {
            dz = val;
        }
    }
 ...
 *   empty != 0 ==> displacements will be all zero ivs   != 0 ==> extract sub-bricks [ivs..ivs+2]
 *for the displacements Normal usage has empty = ivs = 0. See IW3D_to_dataset() for the reverse
 *operation.      ?????
 *//*--------------------------------------------------------------------------*/
...
IndexWarp3D * IW3D_from_dataset(THD_3dim_dataset *dset, int empty, int ivs)
{
    ...
    if (!empty)
    {
        if (DSET_NVALS(dset) < 3 + ivs)
        {
            RETURN(NULL);
        }
        if (!DSET_LOADED(dset))
        {
            DSET_load(dset); if (!DSET_LOADED(dset))
            {
                RETURN(NULL);
            }
        }
    ...
    /* coordinate transformation, needed to take mm displacements in the dataset to voxel index
     * displacements in the index warp */
    ...
    {
        AA->geomstring = NULL;                 /* should never happen! */
    }
    AA->view = dset->view_type;
...
/* Create a warp pair from a single dataset (not used anywhere, I think). Uses sub-bricks 0-2 for
 * the forward warp, and 3-5 for the inverse warp. If sub-bricks 3-5 aren't there, then the inverse
 * warp is created.
 *//*--------------------------------------------------------------------------*/
...
/* Convert an index warp to a 3D dataset of spatial displacmements in mm. See IW3D_from_dataset()
 * for the reverse operation. See IW3D_to_index_dataset() to convert index displacmeents without
 * scaling to mm.
 *//*--------------------------------------------------------------------------*/
...
THD_3dim_dataset * IW3D_to_dataset(IndexWarp3D *AA, char *prefix)
{
    ...
    if (AA->geomstring == NULL)
    {
        ...
        if (gstr == NULL)
        {
            RETURN(NULL);               /* should not transpire */
        }
        AA->geomstring = gstr;
    }
...
/* (C7) Functions below compute auxiliary functions of an index warp, including stuff for the
 * 3dQwarp penalties, and the 3dNwarpFuncs stuff (which are closely related to the 3dQwarp
 * penalties).                */
/*============================================================================*/

/*===========================================================================*/

/* The function below was put here for computation of the eigenvalues of the strain tensor, but
 * wasn't actually needed -- since all that is needed is the sum of their squares, which is more
 * easily computed as the trace of the square of the matrix.  Left in here as a curiosity.
 ...
 *   - Input matrix is [  a[0] a[1] a[2] ] [  a[1] a[3] a[4] ] [  a[2] a[4] a[5] ]
...
/* Compute the bulk and shear energies from DISPLACEMENTS at corners. Loosely based on
 * http://en.wikipedia.org/wiki/Neo-Hookean_solid.
 *   d000 is the 3D displacement at corner 000, etc.
 *   where corner lmn (each index l,m,n is 0 or 1) refers to the corner offset by l in the x
 *direction, by m in the y direction, and by n in the z direction, from the 000 corner. This
 *function is used in computation of the 3dQwarp penalty;
                                           ...
                                           float_triple d011, float_triple d111)
{
     ...
     * the 0.5 factor take the average of 2 first differences relative to the oppposite (000 and
     *111) corners of the hexahedron; for example, fxy = derivative of y displacement with respect
     *to x, computed by averaging 1st differences in the x direction at the 000 corner =
     * DB(d100,d000) and at the 111 corner = DB(d111,d011) since the differences in the corner
     *indexes in the DB() macro are in the first index = x direction code. Note that this matrix
    ...
    /* trace of matrix square = sum of eigenvalue squares of matrix, which in turn gives the shear
     * energy of the displacement tensor */
    ...
    II = (II + VV) / jcb - 3.0f; if (II < 0.0f)
    {
        II = 0.0f;
    }

    /* compute energies (penalties) into the two components of the output -- will be summed later */
 ...
 * return value is the sum of all the energies (AKA the warp penalty). There are two optional
 *deformation energy arrays stored inside an index warp:
 *   je = Jacobian energy = energy of bulk compression or dilation se = Shear energy = energy of
 *shearing and twisting The penalty for 3dQwarp is computed from each voxel energy E by
 *max(je-1,0)^4 + max(se-1,0)^4 In this way, small deformations are penalty-free, and then
 *voxel-wise energies over 1 become heavily penalized. Search below for the "HPEN_"
...
float IW3D_load_energy(IndexWarp3D *AA)
{
    ...
    jea = AA->je; if (jea == NULL)
    {
        jea = AA->je = (float *)calloc(nxyz, sizeof(float));
    }
    sea = AA->se; if (sea == NULL)
    {
        sea = AA->se = (float *)calloc(nxyz, sizeof(float));
    }
...
#pragma omp for
      for (qq = 0 ; qq < nxyz ; qq++)
      {
          ...
          ev  = jea[qq] - Hpen_cut; if (ev > 0.0f)
          {
              esum += (ev * ev) * (ev * ev);
          }
          ev = sea[qq] - Hpen_cut; if (ev > 0.0f)
          {
              esum += (ev * ev) * (ev * ev);
          }
      }
    ...
    for (ii = 0 ; ii < nthmax ; ii++)
    {
        enout += dhaar[ii];                              /* final summation */
    }
    RETURN(enout);
}
...
/* Compute the bulk, shear, and vorticity deformations, from DISPLACEMENTS at corners, for one
 * voxel.
 *//*--------------------------------------------------------------------------*/
...
#pragma omp for
      for (ii = 0 ; ii < njs ; ii++)
      {
          ev = je[ii] - Hpen_cut; if (ev > 0.0)
          {
              dh += (ev * ev) * (ev * ev);
          }
          ev = se[ii] - Hpen_cut; if (ev > 0.0)
          {
              dh += (ev * ev) * (ev * ev);
          }
      }
    ...
    for (esum = 0.0, qq = 0 ; qq < nthmax ; qq++)
    {
        esum += dhaar[qq];
    }
    return esum;
...
/* Volume of a hexahedron (distorted cube) given by 8 corners. Looking down from the top, the bottom
 * plane points are numbered so:
 *     2 -- 3
 |    |  and the top plane is similar (add 4 to each index), 0 -- 1  with point #(i+4) 'above' point
 |#i.
 *//*--------------------------------------------------------------------------*/
...
/* Load the volumes of each hexahedral element in the displaced grid. An undistorted voxel will get
 * volume 1, since AA is a unitless warp. */
...
float IW3D_load_hexvol(IndexWarp3D *AA, float *hv)
{
    ...
    if (hva == NULL)
    {
        ...
        if (hva == NULL)
        {
            hva = AA->hv = (float *)calloc(nxyz, sizeof(float));
        }
    }
                          ...
                          int ibot, int itop, int jbot, int jtop, int kbot, int ktop)
{
    ...
    if (hva == NULL)
    {
        hva = AA->hv = (float *)calloc(nxy * nz, sizeof(float));
    }
...
/* (C8) The following functions are for interpolating all 3 components of an index warp at one time,
 * and are recklessly adapted from mri_genalign_util.c;
 ...
 *    nxx, nyy, nzz = grid dimensions of displacement arrays aar, bar, car = displacement arrays
 *use_es        = flag to use external slope array esar          = external slope array npp
 *           = number of points at which to interpolate ip, jp, kp    = float indexes at which to
 *interpolate uar, var, war = output arrays [npp]
 *//*-------------------------------------------------------------------------*/
...
#pragma omp for
        for (pp = 0 ; pp < npp ; pp++)         /* loop over output points */
        {
            ...
            else             /* using external slopes */
            {
                ...
                if (xx < 0.0f)
                {
                    eex = xx; ix = 0; fx = 0.0f; aem++; Exx = es_xd_xm; Eyx = es_yd_xm;
                    Ezx = es_zd_xm;
                }
                else if (xx < nx1)
                {
                    eex = 0.0f; ix = (int)xx; fx = xx - ix;
                }
                else
                {
                    eex = xx - nx1; ix = nx2; fx = 1.0f; aem++; Exx = es_xd_xp; Eyx = es_yd_xp;
                    Ezx = es_zd_xp;
                }
                if (yy < 0.0f)
                {
                    eey = yy; jy = 0; fy = 0.0f; aem++; Exy = es_xd_ym; Eyy = es_yd_ym;
                    Ezy = es_zd_ym;
                }
                else if (yy < ny1)
                {
                    eey = 0.0f; jy = (int)yy; fy = yy - jy;
                }
                else
                {
                    eey = yy - ny1; jy = ny2; fy = 1.0f; aem++; Exy = es_xd_yp; Eyy = es_yd_yp;
                    Ezy = es_zd_yp;
                }
                if (zz < 0.0f)
                {
                    eez = zz; kz = 0; fz = 0.0f; aem++; Exz = es_xd_zm; Eyz = es_yd_zm;
                    Ezz = es_zd_zm;
                }
                else if (zz < nz1)
                {
                    eez = 0.0f; kz = (int)zz; fz = zz - kz;
                }
                else
                {
                    eez = zz - nz1; kz = nz2; fz = 1.0f; aem++; Exz = es_xd_zp; Eyz = es_yd_zp;
                    Ezz = es_zd_zp;
                }
                if (aem)
                {
                    uex = Exx * eex + Exy * eey + Exz * eez; /* eex = how far past edge of warp, in
                                                              * x-direction (etc.) */
                    vex = Eyx * eex + Eyy * eey + Eyz * eez; /* Eab = external slope for 'a'
                                                              * displacement, */
                    wex = Ezx * eex + Ezy * eey + Ezz * eez; /*       along the 'b' direction, for a
                                                              * and b = x or y or z */
                }
                else
                {
            ...
            ix_00 = ix; ix_p1 = ix_00 + 1; /* at this point, we are 'fx' between indexes ix_00 and
                                            * ix_p1 */
...
++ Taper function wtap(r) is defined to be 1 for 0 <= r <= WCUT and for WCUT < r < 1 is a raised
++cosine dropping down to wtap(r=1) = 0. This choice was made to keep the variance smoothing
++artifact low.
...
#pragma omp parallel if( npp > 333 )
    {
        int nx = nxx, ny = nyy, nz = nzz, nxy = nx * ny, nxyz = nxy * nz, nxyz1 = nxyz - 1, pp,
            ix, jy, kz;
...
#pragma omp for
        for (pp = 0 ; pp < npp ; pp++)
        {
            ...
            if (!ues)
            {
                if (xx < 0.0f)
                {
                    ix = 0; fx = 0.0f; outside = 1;
                }
                else if (xx < nx1)
                {
                    ix = (int)xx; fx = xx - ix; outside = 0;
                }
                else
                {
                    ix = nx2; fx = 1.0f; outside = 1;
                }
                if (yy < 0.0f)
                {
                    jy = 0; fy = 0.0f; outside = 1;
                }
                else if (yy < ny1)
                {
                    jy = (int)yy; fy = yy - jy; outside = 0;
                }
                else
                {
                    jy = ny2; fy = 1.0f; outside = 1;
                }
                if (zz < 0.0f)
                {
                    kz = 0; fz = 0.0f; outside = 1;
                }
                else if (zz < nz1)
                {
                    kz = (int)zz; fz = zz - kz; outside = 0;
                }
                else
                {
                    kz = nz2; fz = 1.0f; outside = 1;
                }
            }
            else
            {
            ...
            for (jj = -IRAD + 1 ; jj <= IRAD ; jj++) /* do interps for */
            {
                ...
                for (kk = -IRAD + 1 ; kk <= IRAD ; kk++)
                {
                    ...
                    ajk[jj + (IRAD - 1)][kk + (IRAD - 1)] =
                        AW(0) + AW(1) + AW(2) + AW(3) + AW(4) + AW(5) + AW(6) + AW(7) + AW(8) +
                        AW(9);
            ...
            {
                for (asum = bsum = csum = 0.0f, jj = -IRAD + 1 ; jj < IRAD ; jj += 2) /* unrolled by
                                                                                       * 2 */
                {
...
/*! Interpolate using quintic method -- see _linear function for relevant comments; this is between
 * linear and wsinc5 in accuracy and speed.
 *//*-------------------------------------------------------------------------*/
...
#pragma omp for
        for (pp = 0 ; pp < npp ; pp++)
        {
            ...
            /* compute indexes from which to interpolate (-2,-1,0,+1,+2,+3), but clipped to lie
             * inside input image volume                 */
 ...
 * -- no interpolation is needed for this operation, since the matrix B can just be applied to the
 *warp displacement from A
...
IndexWarp3D * IW3D_compose_w1m2(IndexWarp3D *AA, mat44 BB, int icode)
{
     ...
     *   MAT33_VEC is used to apply the 3x3 matrix in BL to [xda,yda,zda], which ignores the shifts
     *in the 4x4 matrix, but will take care of rotations (say) MAT44_VEC is used to apply the 4x4
     *matrix in BI = BL-I to the base location [ii,jj,kk], which will include the shifts and any
     *bulk rotation of the grid; the identity matrix I is subtracted so that we just get
     *displacements from [ii,jj,kk] I hope the above helps elucidate the slightly mysterious code
     *below. */
 ...
 * -- in this function, interplation IS necessary, since the displacement from B(x) will not be
 *exactly on a grid point.
 * -- 'm1w2' means 'matrix #1, warp #2'      ?????
 *
 *   A(x)    = x + delta(x) B(x)    = B x                                     [matrix multiply]
 *A(B(x)) = Bx + delta(Bx) = x + (B-I)x + delta(Bx) So the displacements for A(B(x)) are (B-I)x +
 * delta(Bx). In the loops below:
 *   Step 1 = compute Bx for each x=(ii,jj,kk) Step 2 = interpolate delta at the Bx locations =
 * compute delta(Bx) Step 3 = add Bx-x to delta(Bx) to get the final displacements for m1w2
 *//*-------------------------------------------------------------------------*/
...
IndexWarp3D * IW3D_compose_m1w2(mat44 BB, IndexWarp3D *AA, int icode)
{
...

    {
         ...
         *         these are the Bx locations needed for the next step, stored in the temp arrays
         *xq[], yq[], and zq[].     */

        AFNI_OMP_START;
#pragma omp parallel if( qtop-pp > 1111 )
        { int qq, ii, jj, kk;
#pragma ivdep                                                                   /* for Intel icc
                                                                                 * compiler */
#pragma omp for
          for (qq = pp ; qq < qtop ; qq++)                                      /* get x=(ii,jj,kk),
                                                                                 * then */
          {
        ...
        /* step 2: Interpolate A() warp index displacements at B(x) locations, and save in output
         * displacement arrays xdc[], ydc[], and zdc[].   */

        if (0 /*AA->use_es*/)
        {
            ES_PACK(AA, esar);                  /* load external slopes */
        }
        ...
        /* step 3: Add in the B(x) displacements to get the total index displacement from each
         * original position: B(x)-x + A(x+B(x)) */
...
IndexWarp3D * IW3D_compose(IndexWarp3D *AA, IndexWarp3D *BB, int icode)
{
    ...
    if (AA == NULL)
    {
        CC = IW3D_copy(BB, 1.0f); RETURN(CC);
    }
    else if (BB == NULL)
    {
        CC = IW3D_copy(AA, 1.0f); RETURN(CC);
    }

    nx = AA->nx; ny = AA->ny; nz = AA->nz; nxy = nx * ny; nxyz = nxy * nz;

    if (nx != BB->nx || ny != BB->ny || nz != BB->nz)
    {
        RETURN(NULL);
    }
...
#pragma omp for
        ...
        /* Add in the A() displacements to get the total index displacement from each original
         * position: A(x) + B(x+A(x)) */
...
IndexWarp3D * IW3D_2pow(IndexWarp3D *AA, int lev)
{
    ...
    if (lev == 1)
    {
        BB = IW3D_compose(AA, AA, MRI_LINEAR); RETURN(BB);
    }
...
#pragma omp for
            ...
            /* Interpolate B() warp index displacements, at the B() locations, into the C() warp */
            ...
            /* Add in the B() displacements to get the total index displacement from each original
             * position: B(x) + B(x+B(x)) */
...
/* Compute B( 2x - A(B(x)) ) = Newton step for computing Ainv(x), given an initial estimate of the
 * inverse in B(x). Note that the fixed point of this iteration is B( 2x - A(B(x)) ) = B(x) 2x -
 * A(B(x))      = x x                 = A(B(X)  -->> B(x) is inverse of A(x)
 * -- actually, a damping factor is applied so that the output is actually C(x) = (1-inewtfac) *
 * B(x) + inewtfac * B( 2x - A(B(x)) ) ;
 *    the purpose of this is to reduce any instabilities in the iteration.
 * -- the calling function may adjust inewtfac up or down
 * -- set inewtfix to 1 to avoid internal adjustment of inewtfac to start off small and increase
 *when the iteration converges well
 * -- this function computes one step of the iteration; IW3D_invert() drives this to produce the
...
IndexWarp3D * IW3D_invert_newt(IndexWarp3D *AA, IndexWarp3D *BB, int icode)
{
    ...
    /* Warps are stored as voxel index displacements, so we have A(x)              = x + a(x)  [not
     * needed below] B(x)              = x + b(x) A(B(x))           = x + b(x) + a(x+b(x)) 2x -
     * A(B(x))      = x - b(x) - a(x+b(x)) B( 2x - A(B(x)) ) = x - b(x) - a(x+b(x)) +
     * b(x-b(x)-a(x+b(x))) These steps are carried out in the inner loops below:           */
...
/* Find the inverse warp BB(x) to AA(x).  If not NULL, BBinit is the initial estimate for BB(x).
 *  icode tells how to interpolate warps.   ?????
 *//*-------------------------------------------------------------------------*/
...
IndexWarp3D * IW3D_invert(IndexWarp3D *AA, IndexWarp3D *BBinit, int icode)
{
    ...
    if (icode == MRI_WSINC5)
    {
        icode = MRI_QUINTIC;                       /* wsinc5 is too slow */
    }
    /* for many iterations */
    ...
    if (verb_nww)
    {
        ININFO_message(" -- invert max|AA|=%f", normAA);
    }
    ...
    if (BBinit == NULL)                         /* approximate inverse by power iteration */
    {
        ...
        for (ii = 0 ; ii < pp ; ii++) /* compute BB = [I-qq*a(x)]^pp */
        {
            if (verb_nww > 1)
            {
                ININFO_message("  - init step %d", ii + 1);
            }
            else if (Hverb && !verb_nww)
            {
                fprintf(stderr, "*");
            }
    ...
    if (!inewtfix)                                /* initialize Newton */
    {
        ...
        if (inewtfac > 0.333f)
        {
            inewtfac = 0.333f;                   /* on size of input AA */
        }
    }
    ...
    if (getenv("AFNI_NWARP_SWITCHVAL") != NULL)
    {
        ...
        if (switchval <= 0.0002f)
        {
            switchval = 0.0002f;
        }
        else if (switchval >= 0.0100f)
        {
            switchval = 0.0100f;
        }
    }
...

    {
        ...
        if (verb_nww)
        {
            ININFO_message("  - iterate %d nrat=%f", ++nnewt, nrat);
        }
        ...
        if (jcode != icode && nrat < switchval) /* switch to better interp? */
        {
            ...
            if (verb_nww)
            {
                ININFO_message("  - switching from linear interp");
            }
            else if (Hverb)
            {
                fprintf(stderr, ":");
        ...
        if (nrat < 0.0001f) /* maybe we're done already? */
        {
            if (verb_nww)
            {
                ININFO_message(" -- iteration converged");
            }
            RETURN(BB);     /* converged! */
        }
        ...
        if (!inewtfix)
        {
            ...
            {
                nss = 0; inewtfac *= 1.234f; if (inewtfac > 0.678901f)
                {
                    inewtfac = 0.678901f;
                }
                if (verb_nww > 1)
                {
                    ININFO_message("  - switch to inewtfac=%f", inewtfac);
                }
            }
...
/* Invert a dataset that represents a warp. This is done the brute force way, by conversion to an
 * index warp, inversion of that, and then conversion back to a dataset struct.
 *//*-------------------------------------------------------------------------*/
...
/* (C11) Functions to compute the 'square root' of a warp. The square root is probably not actually
 * very useful (especially given the 3dQwarp -plusminus option).
 *   [[The old code to do this was excised completely on 05 Feb 2021]] */
/*============================================================================*/

/*---------------------------------------------------------------------------*/

/* The following iterates on pairs of warps to produce the sqrt and sqrtinv at the same time.
 *    ?????
 *//*-------------------------------------------------------------------------*/
...
IndexWarp3D_pair * IW3D_sqrtpair(IndexWarp3D *AA, int icode)
{
    ...
    for (nite = 0 ; nite < 39 ; nite++)
    {
        ...
        if (verb_nww)
        {
            ININFO_message("*** sqrtpair: nite=%d  nrat=%g", nite, nrat);
        }
        ...
        if (nite > 2 && nrat > orat * 0.99f)
        {
            break;                                /* not converging at all? */
        }
    }

    if (verb_nww)
    {
        INFO_message("*** sqrtpair: exit after %d iterations", nite + 1);
    }
...
/* (C12) These functions are for taking stuff from 3dAllineate -nwarp output. Since that option is
 * obsolete, these functions are probably useless. ????? */
/*============================================================================*/
...
IndexWarp3D * IW3D_from_poly(int npar, float *par, IndexWarp3D *WW)
{
    ...
    if (verb_nww > 1)
    {
        ININFO_message("  - warp name = '%s' has %d parameters", wname, npar);
    }
...
/* Apply a warp to a source image 'sim', and stick values into output 'fim'. Apply to a sub-volume
 * ibot..itop, jbot..jtop, kbot..ktop (inclusive).
 *//*--------------------------------------------------------------------------*/
                            ...
                            int kbot, int ktop, int code, float fac)
{
    ...
    if (fim == NULL || fim->kind != MRI_float)
    {
        EXRETURN;
    }

    nx = AA->nx; ny = AA->ny; nz = AA->nz; nxy = nx * ny;
    if (sim->nx != nx || sim->ny != ny || sim->nz != nz)
    {
        EXRETURN;
    }
    if (fim->nx != nx || fim->ny != ny || fim->nz != nz)
    {
        EXRETURN;
    }
    ...
    nii = itop - ibot + 1; if (nii < 1)
    {
        EXRETURN;
    }
    njj = jtop - jbot + 1; if (njj < 1)
    {
        EXRETURN;
    }
    nkk = ktop - kbot + 1; if (nkk < 1)
    {
        EXRETURN;
    }
                ...
                int code, void *outar)
{
    ...
    case MRI_complex: {
        ...
        for (ii = 0 ; ii < np ; ii++)
        {
            car[ii].r = far[ii]; car[ii].i = gar[ii];
        }
...
/* (C14) Functions used in 3dNwarpXYZ to warp a limited number of points given by x,y,z triples,
 * rather than warping a whole grid at once.                */
/*============================================================================*/
...
#pragma omp parallel if( npt > 333 )
    { int   pp;
      ...
      /* loop over input points, transforming them to indexes, interpolating to that location, then
       * adding the displacements to get the outputs  */
...
#pragma omp for
      for (pp = 0 ; pp < npt ; pp++)
      {
          ...
          aem = 0;                                                /* flag for usage of external
                                                                   * slopes */
          if (xx < 0.0f)
          {
              eex = xx; ix = 0; fx = 0.0f; aem++; Exx = es_xd_xm; Eyx = es_yd_xm; Ezx = es_zd_xm;
          }
          else if (xx < nx1)
          {
              eex = 0.0f; ix = (int)xx; fx = xx - ix;
          ...
          if (yy < 0.0f)
          {
              eey = yy; jy = 0; fy = 0.0f; aem++; Exy = es_xd_ym; Eyy = es_yd_ym; Ezy = es_zd_ym;
          }
          else if (yy < ny1)
          {
              eey = 0.0f; jy = (int)yy; fy = yy - jy;
          ...
          if (zz < 0.0f)
          {
              eez = zz; kz = 0; fz = 0.0f; aem++; Exz = es_xd_zm; Eyz = es_yd_zm; Ezz = es_zd_zm;
          }
          else if (zz < nz1)
          {
              eez = 0.0f; kz = (int)zz; fz = zz - kz;
          ...
          if (aem)
          {
              ...
              vex  = Eyx * eex + Eyy * eey + Eyz * eez; /* Eab = external slope for 'a'
                                                         * displacement, */
              wex  = Ezx * eex + Ezy * eey + Ezz * eez; /*       along the 'b' direction, for a and
                                                         * b = x or y or z */
          ...
          ix_00 = ix; ix_p1 = ix_00 + 1;                 /* at this point, we are 'fx' between
                                                          * indexes ix_00 and ix_p1 */
          jy_00 = jy; jy_p1 = jy_00 + 1;                 /* et cetera */
          kz_00 = kz; kz_p1 = kz_00 + 1;
          wt_00 = (1.0f - fx) * dfac; wt_p1 = fx * dfac; /* weights for ix_00 and ix_p1 points for
                                                          * linear interp */
...
#pragma omp for
      for (pp = 0 ; pp < npt ; pp++)
      {
          ...
          /* compute indexes from which to interpolate (-2,-1,0,+1,+2,+3), but clipped to lie inside
           * input image volume                 */
...
/*  Forward warp a collection of DICOM xyz coordinates. Return value is number of points computed
 * (should be npt). Negative is an error code.
 *//*--------------------------------------------------------------------------*/
...
/*  Inverse warp a collection of DICOM xyz coordinates. Return value is number of points computed
 * (should be npt). Negative is an error code. This code is now just used as the initialization for
 * the final iteration to find the best inverse point.  The method used herein is backward tracing,
 * nstep short steps.
 *//*--------------------------------------------------------------------------*/
...
/* Find the inverse warp of the set of input x,y,z coordinates. NOTE that dfac is not used in this
 * implementation (assumed to be 1)!!!
 *//*--------------------------------------------------------------------------*/
...
/* (C15) Functions to warp a dataset: a setup function and then the actual warper.  Note that index
 * warps are not really used in this computation!    */
/*============================================================================*/

/*----------------------------------------------------------------------------*/

/* Take a warp dataset, and resample it to a new grid. The new grid is defined by a geometry string
 * -- see function EDIT_get_geometry_string().  The format of the string is
 *   "MATRIX(b11,b12,b13,b14,b21,b22,b23,b24,b31,b32,b33,b34):nx,ny,nz"
 * where the bij values comprise the 3x4 matrix that takes indices (i,j,k) to coordinates (x,y,z).
 *//*--------------------------------------------------------------------------*/
...
THD_3dim_dataset * THD_nwarp_regrid(THD_3dim_dataset *inwarp, char *geomstring)
{
    ...
    DSET_load(inwarp); if (!DSET_LOADED(inwarp))
    {
        RETURN(NULL);
    }
...
/* Do the setup to warp images given bimar    = array of DICOM (x,y,z) deltas
 *                = 3D warp displacement function in base image space;
 *   use_amat = if nonzero, use the amat matrix amat     = matrix to transform (x,y,z) AFTER the
 *bimar deltas cmat_bim = matrix to transform indexes (ib,jb,kb) to DICOM (xb,yb,zb), in base image
 *space cmat_src = similar matrix for source dataset (to be warped from) cmat_out = similar matrix
 *for output dataset (to be warped to);
 *                for most purposes, cmat_out will either be cmat_bim (to get the source image
 *warped to the base grid) or will be cmat_src (warp the source image on its own grid)
 *
 * foreach (io,jo,ko) in output dataset do {
 *   (xo,yo,zo) =    [cmat_out](io,jo,ko)     -- transform indexes to coords (ib,jb,kb) =
 * inv[cmat_bim](xo,yo,zo)     -- transform coords to indexes (xs,ys,zs) =  (xo,yo,zo)
 *                 -- compute warped coords
 + bimar interpolated at (ib,jb,kb) (is,js,ks) = inv[cmat_src](xs,ys,zs)     -- compute warped
 +indexes
 + }
 +
 + The output is the array of images of (is,js,ks) = indexes in the source dataset, for each point
 +to interpolate to in the output dataset (io,jo,ko).
 + (N.B.: this is NOT an IndexWarp3D struct!). This set of images can be used, in turn, to
 +interpolate a src grid image to an output grid warped image via THD_interp_floatim().
 *//*--------------------------------------------------------------------------*/
                            ...
                            int nx_out, int ny_out, int nz_out)
{
    ...
    /* compute indexes of each point in output image (the _out grid) in the warp space (the _bim
     * grid) */
    ...
    if (!MAT44_FLEQ(cmat_bim, cmat_out)) /* output & base grids not the same */
    {
        ...
        /* cmat_out takes (i,j,k):out to (x,y,z) tmat     takes (x,y,z)     to (i,j,k):base so
         * imat_out_to_bim takes (i,j,k):out to (i,j,k):base */
...
#pragma omp for
    ...
    /* now interpolate the warp delta volumes from the bim grid to the out grid, using the indexes
     * computed just above;
...
/* Warp a dataset dset_src using the dset_nwarp dataset to control the displacements, patterning the
 * output after dset_mast.
 *//*--------------------------------------------------------------------------*/
                                     ...
                                     MRI_IMAGE *amatim)
{
    ...
    if (dset_nwarp == NULL || dset_src == NULL)
    {
        RETURN(NULL);
    }
    ...
    DSET_load(dset_src); if (!DSET_LOADED(dset_src))
    {
        RETURN(NULL);
    }

    nvals = DSET_NVALS(dset_src); if (nvals > nvlim && nvlim > 0)
    {
        nvals = nvlim;
    }
    ...
    if (dset_mast == NULL)
    {
        dset_mast = dset_src;               /* default master */
    }
    if (prefix == NULL || *prefix == '\0')  /* fake up a prefix */
    {
        ...
        cpt = strcasestr(prefix, ".nii"); if (cpt != NULL)
        {
            *cpt = '\0';
        }
        strcat(prefix, "_nwarp"); if (cpt != NULL)
        {
            strcat(prefix, ".nii");
        }
    }
    ...
    for (iv = 0 ; iv < nvals ; iv++)
    {
        ...
        if (verb_nww && iv == 0)
        {
            fprintf(stderr, "++ Warping dataset: ");
        }
...
/* This function is used to tell THD_nwarp_dataset_array() to save all generated warps into datasets
...
/* Warp a dataset dset_src using the Nwarp_catlist nwc to control the displacements, patterning the
 * output after dset_mast.  [21 Oct 2014]
 *//*--------------------------------------------------------------------------*/
    ...
    float dxyz_mast, float wfac, int nvlim)
{
    ...
    if (nwc == NULL || dset_src == NULL)
    {
        RETURN(NULL);
    }

    numds = dset_src->num; if (numds <= 0)
    {
        RETURN(NULL);
    }
    ...
    for (kds = 0 ; kds < numds ; kds++)        /* loop over input datasets */
    {
        ...
        nvals = DSET_NVALS(dset_sss); if (nvals > nvmax)
        {
            nvmax = nvals;
        }
    ...
    if (dset_mast == NULL)
    {
        dset_mast = DSET_IN_3DARR(dset_src, 0);                 /* default master */
    }
    ...
    if (prefix == NULL)                                         /* fake up some prefixes */
    {
        ...
        for (kds = 0 ; kds < numds ; kds++)
        {
            ...
            cpt = strcasestr(prefix[kds], ".nii"); if (cpt != NULL)
            {
                *cpt = '\0';
            }
            strcat(prefix[kds], "_Nwarp"); if (cpt != NULL)
            {
                strcat(prefix[kds], ".nii");
            }
        }
    }
    ...
    if (next < 32)
    {
        next = 32;               /* warp extension, just for luck */
    }
#endif
    ...
    if (verb_nww)
    {
        fprintf(stderr, "++ Warping:");            /* start progress meter */
    }

    /****** Loop over output sub-bricks, create the warp dataset for that 'time' index, then apply
     * it *****/
    ...
    if (verb_nww > 1)
    {
        fprintf(stderr, "[nvar=%d]", nwc->nvar);
    }
#endif
    ...
    for (iv = 0 ; iv < nvmax ; iv++)
    {
        ...
        /*** toss any old warps */
        {
            ...
            if (verb_nww > 1)
            {
                fprintf(stderr, " a%s", MEMORY_SHORT);
            }
#endif
            ...
            if (verb_nww > 1)
            {
                fprintf(stderr, "b%s", MEMORY_SHORT);
            }
#endif
            ...
            if (verb_nww > 1)
            {
                fprintf(stderr, "c%s", MEMORY_SHORT);
            }
#endif
            fim = THD_extract_float_brick(0, dset_qwarp); ADDTO_IMARR(imar_nwarp, fim);
            fim = THD_extract_float_brick(1, dset_qwarp); ADDTO_IMARR(imar_nwarp, fim);
            ...
            if (QWARP_prefix != NULL)
            {
                ...
                if (verb_nww)
                {
                    ININFO_message(" wrote warp dataset %s", DSET_BRIKNAME(dset_qwarp));
                }
            }
            ...
            if (verb_nww > 1)
            {
                fprintf(stderr, "d%s", MEMORY_SHORT);
            }
#endif
        ...
        {
            dset_sss = DSET_IN_3DARR(dset_src, kds);
            if (DSET_NVALS(dset_sss) < iv)
            {
                continue;                          /* dataset is done already */
            }
            dset_ooo = DSET_IN_3DARR(dset_out, kds);
#ifdef DEBUG_CATLIST
            if (verb_nww > 1)
    ...
    if (verb_nww > 1)
    {
        fprintf(stderr, " z%s", MEMORY_SHORT);
    }
#endif
    DSET_delete(dset_nwarp); DSET_delete(dset_qwarp);
    DESTROY_IMARR(imar_nwarp); DESTROY_IMARR(imar_src);
...
THD_3dim_dataset * NwarpCalcRPN(char *expr, char *prefix, int icode, int acode)
{
    ...
    if (verb_nww)
    {
        INFO_message("NwarpCalcRPN('%s')", expr);
    }
    ...
    for (ss = 0 ; ss < sar->num ; ss++)
    {
        ...
        if (verb_nww)
        {
            ININFO_message(" + stack size=%d  next operation='%s'", nstk, cmd);
        }
            ...
            strncasecmp(cmd, "&readwarp(", 10) == 0)
        {
            ...
            buf = strdup(bp + 1);
            for (bp = buf ; *bp != '\0' && *bp != ')' ; bp++)
            {
                ;                                         /*nada*/
            }
            if (*bp == ')')
            {
                *bp = '\0';           /* delete trailing ) */
            }
        ...
        else if (strncasecmp(cmd, "&readpoly(", 10) == 0)
        {
            ...
            if (nstk < 1)
            {
                free(buf); ERREX("nothing on stack -- needed for template");
            }
            for (bp = buf ; *bp != '\0' && *bp != ')' ; bp++)
            {
                ;                                         /*nada*/
        ...
        else if (strncasecmp(cmd, "&write(", 7) == 0)
        {
            ...
            if (nstk < 1)
            {
                free(buf); ERREX("nothing on stack");
            }
            for (bp = buf ; *bp != '\0' && *bp != ')' ; bp++)
            {
                ;                                         /*nada*/
            ...
            if (sname != NULL)
            {
                MCW_strncpy(dset->atlas_space, sname, THD_MAX_NAME);
            }
            DSET_write(dset);
            if (verb_nww)
            {
                ININFO_message(" -- wrote dataset %s", DSET_BRIKNAME(dset));
            }
        ...
        else if (strncasecmp(cmd, "&swap", 5) == 0) /* modified 06 May 2013 */
        {
            ...
            if (bp != NULL)
            {
                ...
                if (nAA < 0 || nBB < 0 || nAA == nBB)
                {
                    ERREX("illegal values in &swap");
                }
            }
            nAA++; nBB++;
            if (nstk < MAX(nAA, nBB))
            {
                ERREX("stack too short for &swap");
            }
        ...
        else if (strcasecmp(cmd, "&sqrtinv") == 0 || strcasecmp(cmd, "&invsqrt") == 0)
        {
            ...
            if (AA == NULL)
            {
                ERREX("inverse square root failed :-(");
            }
            IW3D_destroy(iwstk[nstk - 1]); iwstk[nstk - 1] = AA;
            if (verb_nww)
            {
        ...
        else if (strncasecmp(cmd, "&sum", 4) == 0)
        {
            ...
            if (nstk < 2)
            {
                ERREX("stack is too small");
            }
            if (bp != NULL)
            {
                sscanf(bp + 1, "%f,%f", &alpha, &beta);
            }
        ...
        else if (strncasecmp(cmd, "&apply(", 7) == 0)
        {
            ...
            if (*bp != ',')
            {
                free(buf); ERREX("no comma for prefix");
            }
            *bp = '\0'; pref = bp + 1; /* delete comma */
            if (!THD_filename_ok(pref))
            {
                free(buf); ERREX("illegal prefix");
            }
            AA             = iwstk[nstk - 1]; FREEIFNN(AA->geomstring);
            AA->geomstring = strdup(geomstring); AA->cmat = cmat; AA->imat = imat;
            ...
            oset = THD_nwarp_dataset(wset, iset, NULL, pref, icode, acode, 0.0f, 1.0f, 999999999,
                                     NULL);
            tross_Copy_History(iset, oset);
            sprintf(mess, "NwarpCalcRPN '%s'", cmd); tross_Append_History(oset, mess);
            if (sname != NULL)
            {
                MCW_strncpy(oset->atlas_space, sname, THD_MAX_NAME);
            }
            DSET_delete(iset); DSET_delete(wset); DSET_write(oset);
            if (verb_nww)
            {
                ININFO_message(" -- wrote dataset %s", DSET_BRIKNAME(oset));
            }
    ...
    if (verb_nww)
    {
        INFO_message("end of evaluation loop");
    }
...
char * CW_get_saved_geomstring(void)
{
    return CW_saved_geomstring;
}

int    CW_get_saved_expad(void)
{
    return CW_saved_expad;
}
...
float_triple M44_max_shifts(mat44_vec *mvv)
{
    ...
    if (mvv == NULL || mvv->nmar == 0 || mvv->mar == NULL)
    {
        return xyz;
    }
    ...
    {
        mmm = mvv->mar[ii];
        dx  = fabsf(mmm.m[0][3]); if (dx > dxm)
        {
            dxm = dx;
        }
        dy = fabsf(mmm.m[1][3]); if (dy > dym)
        {
            dym = dy;
        }
        dz = fabsf(mmm.m[2][3]); if (dz > dzm)
        {
            dzm = dz;
        }
    }
...
mat44_vec * CW_read_affine_warp(char *cp)
{
    ...
    if (qim->nx == 3 && qim->ny == 4)                                       /* single matrix in 3x4
                                                                             * 'Xat.1D' format? */
    {
    ...
    if (qim->nx != 12)
    {
        ERROR_message("CW_read_affine_warp: nx == %d != 12 (this message should never happen!)",
                      qim->nx);
        free(wp); RETURN(mvv);
    }
...
mat44 CW_read_affine_warp_OLD(char *cp)
{
    ...
    {
        WARNING_message("Affine warp file '%s' has %d matrices: using only first one", cp,
                        mvv->nmar);
    }
...
/* Read a warp from a dataset -- don't convert to index warp (yet). Allows for all the weird
 * pre-processing prefixes, like INV, SQRT, etc.
 *//*--------------------------------------------------------------------------*/
...
THD_3dim_dataset * CW_read_dataset_warp(char *cp)
{
     ...
     *   RL: or LR: for x-direction only AP: or PA: for y-direction only IS: or SI: for z-direction
     *only VEC:a,b,c: for arbitrary direction Can also have have a scale factor for the dataset, as
     *in RL:0.7:datasetname   or   VEC:1,1,1:-0.5:datasetname Note that vector (a,b,c) will be
     *L2-normalized to a unit direction */
        ...
        strncasecmp(wp, "VEC:", 4) == 0 || strncasecmp(wp, "UNI:", 4) == 0)
    {
        ...
        {
        case 'R':
        case 'L':  vx = 1.0f; vy = vz = 0.0f; break;

        case 'A':
        case 'P':  vy = 1.0f; vx = vz = 0.0f; break;

        case 'I':
        case 'S':  vz = 1.0f; vx = vy = 0.0f; break;
        ...
        if (!DSET_LOADED(eset))
        {
            ERROR_message("Can't load dataset from file '%s'", up); free(wp); DSET_delete(eset);
            RETURN(NULL);
        }
...
/* Load one warp into the nn-th static data array, converting to index warp (if it's not a matrix),
...
void CW_load_one_warp(int nn, char *cp)
{
    ...
    if (nn > CW_nwtop)
    {
        CW_nwtop = nn;                  /* CW_nwtop = largest index thus far */
    }
    ...
    if (CW_geomstring == NULL)
    {
        ...
        CW_nx         = AA->nx; CW_ny = AA->ny; CW_nz = AA->nz; CW_cmat = AA->cmat;
        CW_imat       = AA->imat;
        CW_dx         = fabsf(DSET_DX(dset)); CW_dy = fabsf(DSET_DY(dset));
        CW_dz         = fabsf(DSET_DZ(dset));
 ...
 *   "warp1 warp2 warp3"
 * and return the dataset that instantiates warp3(warp2(warp1(x))). This is how the '-nwarp' option
 *in 3dNwarp*.c is implemented, except for 3dNwarpApply, which uses IW3D_read_nwarp_catlist().
 *
 * N.B.: The nonlinear warps input this function  must on the same grids!
 *       For a function that does not have this limitation, see the more complicated
 *IW3D_read_nwarp_catlist() -- some distance below.
 *//*--------------------------------------------------------------------------*/
...
THD_3dim_dataset * IW3D_read_catenated_warp(char *cstr)
{
...
#if 0
    { int qq = AFNI_numenv("CW_EXPAD"); if (qq >= 0)
      {
          CW_saved_expad = qq;
      }
    }
    ...
    for (ii = 0 ; ii < CW_nwtop ; ii++)
    {
...

        {
            ...
            if (CW_iwfac[ii] != 1.0f)
            {
                IW3D_scale(CW_iwarp[ii], CW_iwfac[ii]);
            }
#endif
...
void IW3D_destroy_nwarp_catlist(Nwarp_catlist *nwc)
{
    ...
    if (nwc->awarp != NULL)
    {
        for (ii = 0 ; ii < nwc->ncat ; ii++)
        {
            if (nwc->awarp[ii] != NULL)
            {
                DESTROY_mat44_vec(nwc->awarp[ii]);
            }
        }
    }
    if (nwc->actual_geomstring != NULL)
    {
        free(nwc->actual_geomstring);
    }
    if (nwc->master_geomstring != NULL)
    {
        free(nwc->master_geomstring);
    }
    free(nwc); return;
}
...
/* Set the 'master' geometry of the catlist.  Please note that the actual geometry of the warps
 * inside will be padded from this.  If gs is NULL, then the function will make something up
 * instead, which you don't want.
 *//*--------------------------------------------------------------------------*/
...
void IW3D_set_geometry_nwarp_catlist(Nwarp_catlist *nwc, char *gsin)
{
    ...
    if (!ISVALID_GEOMETRY_STRING(gs))
    {
        ...
        for (jj = 1 ; jj < nw ; jj++)
        {
            if (EDIT_geometry_string_diff(gset[0], gset[jj]) > 0.01f)
            {
                break;
            }
        }
    ...
    if (nwc->xshift > 0.0f || nwc->yshift > 0.0f || nwc->zshift > 0.0f || CW_extra_pad > 0)
    {
        ...
        if (xpad > 0)
        {
            hs = EDIT_geometry_string_pad(gs, xpad);
        }
        else
        {
            hs = strdup(gs);
    ...
    for (ii = 0 ; ii < nwc->ncat ; ii++)
    {
        ...
        if (verb_nww > 1)
        {
            ININFO_message("-- warp #%d geometry = %s %s", ii, gg, MEMORY_SHORT);
        }
#endif
        if (EDIT_geometry_string_diff(hs, gg) > 0.01f)
        {
            ...
            if (verb_nww > 1)
            {
                ININFO_message("   regridding warp #%d to geometry = %s", ii, hs);
            }
#endif
        ...
        {
            ININFO_message("   do not need to regrid warp #%d", ii);
        }
#endif
...
/* Go over a Nwarp_catlist struct and collapse/reduce operations that aren't dependent on the 'time'
 * index. Note that if there aren't any multi-matrix 'time dependent' transformations, this should
 *reduce the result down to a single warp dataset.
 *//*--------------------------------------------------------------------------*/
...
Try_It_Again_Dude:  /* target for loop back when ndone > 0 */
    ...
    if (verb_nww > 1)
    {
        INFO_message("-- IW3D_reduce_nwarp_catlist -- start loop %s", MEMORY_SHORT);
    }
#endif
    ...
    for (ndone = ii = 0 ; ii < nwc->ncat - 1 ; ii = jj)
    {
        ...
        if (jj == nwc->ncat)
        {
            break;                  /* nothing above, so we are done */
        }
#ifdef DEBUG_CATLIST
        if (verb_nww > 1)
        {
        ...
        if (NWC_awarp(nwc, ii) != NULL && NWC_awarp(nwc, jj) != NULL)
        {
            ...
            if (verb_nww > 1)
            {
                ININFO_message(".. IW3D_reduce_nwarp_catlist: Matrix[%d]-Matrix[%d] ii=%d jj=%d",
                               nii, njj, ii, jj);
            }
#endif
        ...
        if (doall && NWC_awarp(nwc, ii) != NULL && NWC_nwarp(nwc, jj) != NULL)
        {
            ...
            if (mvii->nmar == 1)
            {
                ...
                if (verb_nww > 1)
                {
                    ININFO_message(".. IW3D_reduce_nwarp_catlist: Matrix[1]-warp ii=%d jj=%d", ii,
                                   jj);
                }
#endif
            ...
            {
                ININFO_message(
                    ".. IW3D_reduce_nwarp_catlist: Matrix[%d]-warp ii=%d jj=%d -- skipped",
                    mvii->nmar, ii, jj);
            }
#endif
            continue;
        ...
        if (doall && NWC_nwarp(nwc, ii) != NULL && NWC_awarp(nwc, jj) != NULL)
        {
            ...
            {
                ININFO_message("IW3D_reduce_nwarp_catlist: warp-Matrix[%d] ii=%d jj=%d -- skipped",
                               mvjj->nmar, ii, jj);
            }
#endif
            continue;
    ...
    if (verb_nww > 1)
    {
        ININFO_message("-- loop reduction operation count = %d %s", ndone, MEMORY_SHORT);
    }
#endif
    totaldone += ndone;
    if (ndone > 0 || !doall)
    {
        doall = 1; goto Try_It_Again_Dude;
    }
    ...
    for (ii = 0 ; ii < nwc->ncat ; ii++)
    {
        ...
        for (jj = ii + 1 ; jj < nwc->ncat ; jj++)
        {
            if (nwc->awarp != NULL)
            {
                nwc->awarp[jj - 1] = nwc->awarp[jj];
            }
            if (nwc->nwarp != NULL)
            {
                nwc->nwarp[jj - 1] = nwc->nwarp[jj];
            }
        }
    ...
    /** More general way to deal with inversion would be to (a) reverse the order of all the warps,
     * and (b) invert them all (including multi-line matrices) This would save having to do
     * inversions in IW3D_from_nwarp_catlist **/
...
THD_3dim_dataset * IW3D_from_nwarp_catlist(Nwarp_catlist *nwc, int vind)
{
    ...
    if (nwc->ncat == 1)        /* return a copy, so it can be deleted */
    {
        ...
        {
            qset = THD_nwarp_invert(oset);                              /* not possible */
        }
        else
        {
            qset = EDIT_full_copy(oset, "Copy_Of_Nwarp");
        }
    ...
    if (verb_nww > 1)
    {
        fprintf(stderr, "{nwarp_catlist[%d]", vind);
    }
#endif
...

    {
        if (NWC_awarp(nwc, ii) != NULL) /* matrix to apply */
        {
            ...
            if (verb_nww > 2)
            {
                ...
                fprintf(stderr, " [new mat=%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f]", a11, a12, a13,
                        a14, a21, a22, a23, a24, a31, a32, a33, a34);
            }
            ...
            else                                           /* instead, apply matrix to nonlinear
                                                            * warp */
            {
#ifdef DEBUG_CATLIST
                if (verb_nww > 1)
                {
                    fprintf(stderr, " [warp*matrix]");
                }
#endif
                tarp = IW3D_compose_w1m2(warp, smat, CW_interp);
                IW3D_destroy(warp); warp = tarp;
...
/* Construct an Nwarp_catlist from a list of strings (filenames). This function is used in
 * 3dNwarpApply.
 *//*--------------------------------------------------------------------------*/
...
Nwarp_catlist * IW3D_read_nwarp_catlist(char *cstr)
{
    ...
    for (ii = 0 ; ii < csar->num ; ii++)         /* read them all in, Frank */
    {
        ...
        if (strcasestr(cp, ".1D") != NULL || strcasestr(cp, ".txt") != NULL)
        {
            ...
            if (mvv->nmar > nwc->nvar)
            {
                nwc->nvar = mvv->nmar; /* update of max */
            }
...

        {
            ...
            if (verb_nww > 1)
            {
                ININFO_message("  max displacements = %g %g %g", dxyz.a, dxyz.b, dxyz.c);
            }
#endif
            dxmax += dxyz.a; dymax += dxyz.b; dzmax += dxyz.c;
    ...
    if (verb_nww > 1)
    {
        ININFO_message("--- Totalized max displacements = %g %g %g", dxmax, dymax, dzmax);
    }
#endif
    ...
    if (!gs_mismatch)
    {
        ...
        if (verb_nww > 1 && ii > 0)
        {
            ININFO_message("Reduced catlist by %d steps", ii);
        }
#endif
    }
...
/** The basic 3dQwarp run is an invocation of function IW3D_warp_s2bim() (s2bim == "source to base
 * image"), which returns a struct combining the final index warp and warped source image.  The
 * outline of function calls follows (some minor calls are omitted -- I'm sure you'll figure them
 * out):
 *
 *   (0) 3dQwarp main() == sets up the global variables (cf. infra) from the options and inputs,
 *calls IW3D_warp_s2bim(), and then writes the outputs (1) IW3D_warp_s2bim() == driver function for
 *IW3D_warpomatic() and applies the warp to the source image (2a) IW3D_warpomatic() == produces the
 *optimized warp, by driving IW3D_improve_warp() over global and then local (overlapping) patches
 *(3a) IW3D_setup_for_improvement() == sets up data for warp optimizing;
 *                                           called at IW3D_warpomatic() start (4)
 *IW3D_load_energy() == compute 'energy' fields for global warp as it is now, for warp penalty later
 *                                 -- if the initial warp (S2BIM_iwarp) is not NULL, then this step
 *is important (3b) IW3D_improve_warp() == called many many times over shrinking patches; this
 *function optimizes the incremental warp Hwarp and then updates the global warp Haawarp -- the
 *eventual output of IW3D_warpomatic() (4a) H?warp_setup_basis() == sets up basis polynomials for
 *patch warp (? == Q for quintic, C for cubic) (4b) INCOR_create() == sets up struct for incremental
 *'correlation'
 *                              calculations -- first computes with the data outside the optimizing
 *patch and later gets finalized each time the patch is updated (4c) HPEN_addup() == initialize warp
 *penalty sum for points outside the current patch (from the 'energy' fields) (4c)
 *powell_newuoa_con() == Powell NEWUOA constrained optimizer which actually optimizes Hwarp by
 *playing with the coefficients of the warp basis polynomials (cf. powell_int.c) (5)
 *IW3D_scalar_costfun() == function being optimized (5a) Hwarp_apply() == computes patch warp Hwarp,
 *composes it with current global warp Haawarp, and then applies it to source image to produce the
 *patch data (using linear interpolation) (6) H?warp_eval_X() == evaluate the patch warp given the
 *current parameters (? == Q or C; X == A or B, depending on the size of the patch);
 * uses data setup from H?warp_setup_basis() (5b) INCOR_evaluate() == completes the 'incomplete'
 * correlation using the patch data just computed;
 *                                  this is the measurement of image matching (5c) HPEN_penalty() ==
 * finalizes the penalty given the patch warp (6) IW3D_load_energy() = computes the 'energy' of the
 *patch warp (3c) IW3D_cleanup_improvement() == free workspaces created in
 *IW3D_setup_for_improvement() (2b) IW3D_warp_floatim() == produces the warped source image (using
 *wsinc5 interpolation)
 *
 *  There is an unfinished function THD_warpomatic() which was intended to provide a simple way to
 *warp one dataset to another, but that was never completed.  At present, you have to use
 *IW3D_warp_s2bim(), which requires MRI_IMAGEs as the inputs, not datasets.
 *
 *  3dQwarp is *supposed* to work with 2D images (nz=1), and there is code to allow for that case --
 * but it has never been tested! (OK, once.)
 *
 *  The INCOR_* functions are in thd_incorrelate.c (which is #include-d far far above), and handles
 * "incomplete correlation" calculations, where part of the data is fixed (the part outside the
 *patch being optimized), and part is changing (the part inside the current patch).  For speed, the
 *computations for the fixed part are only done once -- via INCOR_addto() in function
 *IW3D_improve_warp() before the patch optimization is done. The "correlation" (cost function) is
 *completed via INCOR_evaluate() in function IW3D_scalar_costfun() -- which is what
 *powell_newuoa_con() tries to minimize.
 *//**------------------------------------------------------------------------**/

/**--------------------------------------------------------------------------**/

/** Many (but not all) global variables below start with 'H', which is my notation for the warp
 * patch being optimized;
...
/*** We allow 2 types of polynomial patches: cubics and quintics. Quintic patches have 3*3*3*3=81
 * parameters, and so take much more time to evaluate and optimize than cubic patches, which have
 * only 3*2*2*2 = 24 parameters. Quintic patches are C2 (2 continuous derivatives), while cubic
 * patches are C1 (1 continuous derivative). ***/
...
static IndexWarp3D *AHwarp = NULL;                         /* local version of global warp = A(H(x))
                                                            */
static int need_AH         = 1;                            /* flag to compute AHwarp (not always
                                                            * needed) */
...
#define HSAVE_ADDTO(iww, inn)                                                                   \
        do{ if (Hsave_callback_func != NULL) {                                                  \
                ...
                Hsave_iwarp = (IndexWarp3D **)realloc(Hsave_iwarp,                              \
                                                      sizeof(IndexWarp3D *) * (Hsave_num + 1)); \
                Hsave_iname = (char **)realloc(Hsave_iname,                                     \
                                               sizeof(char *) * (Hsave_num + 1));               \
...
/* This signal handler will longjmp() back into the "it failed" branch of the setjmp() conditional
 * where the optimizer/OpenMP sometimes freezes up -- if Hquitting_do_jump is set. Otherwise, it
 * just returns to where the program was when the signal was invoked. It also sets Hquitting on,
 * which is the flag for the warping control function IW3D_warpomatic() to end work NOW NOW NOW (but
 * gracefully). */
...
void IW3D_signal_quit(int sig)
{
    ...
    {
        fff = 1;                         /* stop recursion (shouldn't happen) */
    }
#ifdef USE_OMP
    ...
    if (Hquitting_do_jump)
    {
        Hquitting_do_jump = 0; longjmp(Hquitting_jmp_buf, 666);
    }
...
int IW3D_munge_flags(int nx, int ny, int nz, int flags)
{
    ...
    /* don't allow x-displacements if x size is too small, or if displacements aren't allowed to
     * depend on x coordinate */
    ...
    if ((flags & NWARP_NODISP_FLAG) == NWARP_NODISP_FLAG)
    {
        flags = -1;
    }
...
/* C1 Hermite cubic basis functions over [-1..1] -- that is, the 2 functions and their first
 * derivatives go to 0 at x=1 and x=-1. The first function (ee.a) is such that f(0)=1, and f'(0)=0.
 * The second function (ee.b) is such that f(0)=0, and f'(0)=6.75. Scale factors are adjusted so
 * that the functions' peak values are all 1. Return value is a float_pair comprising the 2 function
 * values. This set of functions is the default for 3dQwarp patches.
 *//*--------------------------------------------------------------------------*/
 ...
 * sinc() is defined far above as sinc(x) = sin(PI*x)/(PI*x)  x >= 0 This basis function 'set' has
 *only one function, so in 3D there are just 3 parameters to optimize; the purpose of this is to
 *provide the very first alignment step at lev=0 to adjust alignment crudely and quickly.
 *//*--------------------------------------------------------------------------*/
...
/* The following functions add 3 more functions to the C1 basis set, for 'p' type refinement of the
 * warp -- HCwarp_eval_'basisX' provides X basis functions, for X=3, 4, 5.  These additional
 * functions are chosen to provide more oscillations in the basic [-1..1] interval.
 *   function #1 (ee.a) = no internal zero crossings function #2 (ee.b) =  1 internal zero crossing
 *function #3 (ee.c) =  2 internal zero crossings function #4 (ee.d) =  3 internal zero crossings
 *function #5 (ee.e) =  4 internal zero crossings Functions #3-5 integrate to 0 over the interval
 *[0..1] (orthogonal to 1). All functions are scaled so that their peak value is 1. Function #5 is
 *also orthogonal to x over [0..1].              [02 Nov 2015]
 *
 * These functions don't seem to be worth the extra CPU time they require;
 * they provide more detailed warping within a patch, but at the cost of optimizing over more
 *parameters. It seems generally better simply to run down to smaller patches instead.
 *//*--------------------------------------------------------------------------*/
...
/* This func does the MRI_CUBIC_PLUS_3 (AKA cubc105) basis functions, which also include the PLUS_1
 * and PLUS_2 funcs
 *//*..........................................................*/
...
## \rm e?.1D foreach ii ( `count_afni -dig 1 0 200` ) set xx = `ccalc "-1+0.01*$ii"`
##   set aa = `ccalc "abs($xx)"`
...
##   echo $ea >> ea.1D echo $eb >> eb.1D echo $ec >> ec.1D echo $ed >> ed.1D echo $ee >> ee.1D end
...
/* C2 Hermite quintic basis functions over [-1..1] -- that is, the 3 functions and their first 2
 * derivatives go to 0 at x=1 and x=-1. The first function (ee.a) is such that f(0) != 0, and
 * f'(0)=f''(0)=0. The second function (ee.b) is such that f(0)=f''(0)=0, and f'(0) != 0. The third
 * function (ee.c) is such fhat f(0)=f'(0)=0, and f''(0) != 0. Scale factors are adjusted so that
 * the functions' peak values are all 1. Return value is a float_triple comprising the 3 function
 * values.
 *
 * These functions are used at lev=0, and if Qonly or Qfinal or WORKHARD options are enabled in
 *3dQwarp. They provide more detail within a patch warp than the cubic polynomials, but at the cost
 *of CPU time.
 *//*--------------------------------------------------------------------------*/
...
/* Macro to compute grid location coefficients inside interval x in [-1,1]. The leftmost index (that
 * maps to x=-1) is ILEFT. The rightmost index (that maps to x=+1) is IRGHT(n), where 'n' is the
 * number of grid points, indexed from i=0 to i=n-1. Outputs of COMPUTE_CAB() are ca and cb such
 * that x = ca +cb*i. This function is so we can use the cubic and quintic eval_basis() funcs above,
 * which are defined over the domain [-1,1], when setting up the warp basis arrays over a range of
 * integer indexes.
 *//*--------------------------------------------------------------------------*/

/* Rational possibilities here are:
 *          ILEFT = 0    IRGHT = n-1    :: 0 at last grid point ILEFT = -1/2 IRGHT = n-1/2  :: 0
 *halfway outside last grid point ILEFT = -1   IRGHT = n      :: 0 at next grid point outside patch
...
void HCwarp_setup_basis(int nx, int ny, int nz, int flags)
{
    ...
    /* if not going to use all 3D displacements, create map from active set of parameters to total
     * set of parameters:
     *   Hparmap[j] = index into list 0..23 of the 'true' parameter location;
     * We have to do this since the Powell optimization function requires input of all parameters in
     *a contiguous array, but we want to keep them in their logical place even if some of them
     *aren't being used, so this mapping will let us translate between these arrays (if needed) */
    ...
    if ((Hflags & NWARP_NODISP_FLAG) != 0)   /* not all params being used */
    {
        ...
        if (Hbasis_code == MRI_CUBIC)             /* 8 params per dimension */
        {
            ...
            if (!(Hflags & NWARP_NOYDIS_FLAG))
            {
                for (ii = 0 ; ii < 8 ; ii++)
                {
                    Hparmap[pm++] = ii + 16;                  /* z params */
                }
            }
            Hnparmap = pm;
            if (Hnparmap == 24)
            {
                free(Hparmap); Hparmap = NULL;
            }
        }
    ...
    /* if everything is already cool from a previous call, just zero out the warp patches and
     * vamoose the ranch */
    ...
    if (Hflags < 0)
    {
        EXRETURN;               /* this should not happen */
    }
...
#pragma ivdep  /* for Intel icc compiler */
    ...
    /* create empty patch warp, to be populated in HCwarp_load, given these basis function arrays
     * and the warp parameters */
...
/* Setup cubic basis arrays for each dimension for patch = nx X ny X nz, but with some extra basis
 * functions (1 or 2 or 3) given by nplus. This function ALWAYS sets up 3D basis arrays for speed,
 * so only should be used on small-ish patches -- say 25x25x25 or smaller.      [05 Nov 2015]
 *//*--------------------------------------------------------------------------*/
...
int HCwarp_setup_basis345(int nx, int ny, int nz, int flags, int nplus)
{
    ...
    if ((Hflags & NWARP_NODISP_FLAG) != 0)   /* not all params being used */
    {
        ...
        if (!(Hflags & NWARP_NOYDIS_FLAG))
        {
            for (ii = 0 ; ii < nparm ; ii++)
            {
                Hparmap[pm++] = ii + 2 * nparm;                     /* z params */
            }
        }
        Hnparmap = pm;
        if (Hnparmap == 3 * nparm)
...
void HSCwarp_setup_basis(int nx, int ny, int nz, int flags)
{
    ...
    if (bbbsar != NULL)
    {
        free(bbbsar); nbbsxyz = 0; bbbsar = NULL;
    }
...
/** These functions look complicated, partly because there are 2 sets of functions and partly
 * because there are separate cases for quintic and cubic bases, with special sub-cases further
 * broken out for increased efficiency.  But the underlying code is actually pretty simple:
 *
...
/*** Add routine for basis5 functions, which always use 3D basis arrays [05 Nov 2015] **/
...
void HCwarp_eval_A(int qq, float *xx, float *yy, float *zz)
{
    ...
    /* in this function, the 'b' values (3D warp components) are evaluated as tensor products of the
     * underlying 1D functions */
    ...
    if (Hdox)
    {
        *xx = dxci *
              (b0zb0yb0x * Hxpar[0] + b1zb0yb0x * Hxpar[1] + b0zb1yb0x * Hxpar[2]
               ...
               + b0zb1yb1x * Hxpar[6] + b1zb1yb1x * Hxpar[7]);
    }
    else
    {
        *xx = 0.0f;
    ...
    if (Hdoy)
    {
        *yy = dyci *
              (b0zb0yb0x * Hypar[0] + b1zb0yb0x * Hypar[1] + b0zb1yb0x * Hypar[2]
               ...
               + b0zb1yb1x * Hypar[6] + b1zb1yb1x * Hypar[7]);
    }
    else
    {
        *yy = 0.0f;
    ...
    if (Hdoz)
    {
        *zz = dzci *
              (b0zb0yb0x * Hzpar[0] + b1zb0yb0x * Hzpar[1] + b0zb1yb0x * Hzpar[2]
               ...
               + b0zb1yb1x * Hzpar[6] + b1zb1yb1x * Hzpar[7]);
    }
    else
    {
        *zz = 0.0f;
...
void HCwarp_eval_B(int qq, float *xx, float *yy, float *zz)
{
    ...
    /* in this function, the 8 'b' values (warp components) were pre-evaluated in the bbbcar arrays,
     * and so just need to be extracted; this method is faster, but obviously takes more memory */
    ...
    if (Hdox)
    {
        *xx = dxci *
              (b0zb0yb0x * Hxpar[0] + b1zb0yb0x * Hxpar[1]
               + b0zb1yb0x * Hxpar[2] + b0zb0yb1x * Hxpar[3]);
    }
    else
    {
        *xx = 0.0f;
    ...
    if (Hdoy)
    {
        *yy = dyci *
              (b0zb0yb0x * Hypar[0] + b1zb0yb0x * Hypar[1]
               + b0zb1yb0x * Hypar[2] + b0zb0yb1x * Hypar[3]);
    }
    else
    {
        *yy = 0.0f;
    ...
    if (Hdoz)
    {
        *zz = dzci *
              (b0zb0yb0x * Hzpar[0] + b1zb0yb0x * Hzpar[1]
               + b0zb1yb0x * Hzpar[2] + b0zb0yb1x * Hzpar[3]);
    }
    else
    {
        *zz = 0.0f;
    ...
    if (Hdox)
    {
        *xx = dxqi *
              (b0zb0yb0x * Hxpar[0] + b1zb0yb0x * Hxpar[1] + b2zb0yb0x * Hxpar[2]
               ...
               + b0zb2yb2x * Hxpar[24] + b1zb2yb2x * Hxpar[25] + b2zb2yb2x * Hxpar[26]);
    }
    else
    {
        *xx = 0.0f;
    ...
    if (Hdoy)
    {
        *yy = dyqi *
              (b0zb0yb0x * Hypar[0] + b1zb0yb0x * Hypar[1] + b2zb0yb0x * Hypar[2]
               ...
               + b0zb2yb2x * Hypar[24] + b1zb2yb2x * Hypar[25] + b2zb2yb2x * Hypar[26]);
    }
    else
    {
        *yy = 0.0f;
    ...
    if (Hdoz)
    {
        *zz = dzqi *
              (b0zb0yb0x * Hzpar[0] + b1zb0yb0x * Hzpar[1] + b2zb0yb0x * Hzpar[2]
               ...
               + b0zb2yb2x * Hzpar[24] + b1zb2yb2x * Hzpar[25] + b2zb2yb2x * Hzpar[26]);
    }
    else
    {
        *zz = 0.0f;
...
void HQwarp_eval_B(int qq, float *xx, float *yy, float *zz)
{
    ...
    {
        t1 += bbbqar[jj][qq] * Hxpar[jj] + bbbqar[jj + 1][qq] * Hxpar[jj + 1] + bbbqar[jj + 2][qq] *
              Hxpar[jj + 2];
        t2 += bbbqar[jj][qq] * Hypar[jj] + bbbqar[jj + 1][qq] * Hypar[jj + 1] + bbbqar[jj + 2][qq] *
              Hypar[jj + 2];
        t3 += bbbqar[jj][qq] * Hzpar[jj] + bbbqar[jj + 1][qq] * Hzpar[jj + 1] + bbbqar[jj + 2][qq] *
              Hzpar[jj + 2];
    }
    *xx = (Hdox) ? t1 : 0.0f;
    *yy = (Hdoy) ? t2 : 0.0f;
...
/* This just uses 30 parameters = the set of basis functions whose tensor indexes add up to only 0,
 * 1, or 2 */
...
void HQwarp_eval_BMM(int qq, float *xx, float *yy, float *zz)
{
    ...
    /**................................................................... Note that the
     * bbbqar[indexes] in the assignments that follow this comment are not sequential, because we
     * are only using elements whose sum of basis funcs add up to 0, 1, or 2, and these are the
     * definitions of bbbqar from far above:
     *     bbbqar[ 0][qq] = bq0z[kk]*bq0y[jj]*bq0x[ii];    0 0 0 bbbqar[ 1][qq] =
     * bq1z[kk]*bq0y[jj]*bq0x[ii];    1 0 0 bbbqar[ 2][qq] = bq2z[kk]*bq0y[jj]*bq0x[ii];    2 0 0
     *bbbqar[ 3][qq] = bq0z[kk]*bq1y[jj]*bq0x[ii];    0 1 0 bbbqar[ 4][qq] =
     * bq1z[kk]*bq1y[jj]*bq0x[ii];    1 1 0 bbbqar[ 6][qq] = bq0z[kk]*bq2y[jj]*bq0x[ii];    0 2 0
     *bbbqar[ 9][qq] = bq0z[kk]*bq0y[jj]*bq1x[ii];    0 0 1 bbbqar[10][qq] =
     * bq1z[kk]*bq0y[jj]*bq1x[ii];    1 0 1 bbbqar[12][qq] = bq0z[kk]*bq1y[jj]*bq1x[ii];    0 1 1
...
/* The _basis3, _basis4, _basis5 functions below had their basic elements generated by the following
 * tcsh script, followed by manual editing to make things look good.  The example below is for
 * basis5 (nb5=5) -- the way 3dQwarp is set up at present, only basis5 can be executed (-5final).
 *
 ...
 *
 *     echo "float"
 *     foreach rr ( `count_afni -dig 1 0 $nbb` ) foreach qq ( `count_afni -dig 1 0 $nbb` ) foreach
 *pp ( `count_afni -dig 1 0 $nbb` ) echo " b${pp}zb${qq}yb${rr}x,"
 *         end end end
 *
 *     echo
 *
 *     foreach rr ( `count_afni -dig 1 0 $nbb` ) foreach qq ( `count_afni -dig 1 0 $nbb` ) foreach
 *pp ( `count_afni -dig 1 0 $nbb` )
 *           @ ss = $pp + $qq * $nb5 + $rr * $nb5 * $nb5 echo "b${pp}zb${qq}yb${rr}x =
 * bbbcar[$ss][qq] ;"
 *         end end end
 *
 *     echo
 ...
 *     foreach rr ( `count_afni -dig 1 0 $nbb` ) foreach qq ( `count_afni -dig 1 0 $nbb` ) foreach
 *pp ( `count_afni -dig 1 0 $nbb` )
 *           @ ss = $pp + $qq * $nb5 + $rr * $nb5 * $nb5 echo " + b${pp}zb${qq}yb${rr}x*Hxpar[$ss]"
 *         end end end
 *//*--------------------------------------------------------------------------*/
...
/** (Q4) Evaluate the incrementally warped source image at the combination of the current global
 * warp and the patch warp
 *
...
/* Evaluate Hsrcim[ Haawarp(Hwarp(x)) ] into the val[] array. Note that Haawarp is a global warp for
 * Hsrcim, whereas Hwarp is just a local patch. On output, the val[] array contains the linearly
 * interpolated warped image values over this patch. Also evaluate the composed warp
 * Haawarp[Hwarp(x)] into AHwarp for future utility.  AHwarp is a local patch that fits into Haawarp
 * later, when it needs to be updated at the end of a patch optimization step.
...
void Hwarp_apply(float *val)
{
    ...
    if (Hsrcim == NULL || Haawarp == NULL || val == NULL || Hwarp == NULL)
    {
        EXRETURN;
    }
    ...
    if (WARP_IS_QUINTIC(Hbasis_code))
    {
        nbx = nbqx; nby = nbqy; nbz = nbqz;
    }
    else if (WARP_IS_CUBIC(Hbasis_code))
    {
        nbx = nbcx; nby = nbcy; nbz = nbcz;
    }
    else /* sincc warp case */
    {
        nbx = nbsx; nby = nbsy; nbz = nbsz;
    }

    nbxy = nbx * nby; nbxyz = nbxy * nbz;

    /* choose which parameter-to-patch warp function to use, given the choice of basis functions and
     * whether 3D basis arrays (bbb[cqs]ar) exist; Heval(qq) will give us the warp displacements at
     * location qq, given the parameters defining the warp - stored in H[xyz]par. */
...
#pragma ivdep                                                         /* for Intel icc compiler */
#pragma omp for                                                       /* parallel-ized loop over
                                                                       * index into patch */
                                                                       ...
                                                                       */
      {
          ...
          /* get Hwarp-ed indexes into the global warp Haawarp; e.g., xq = Hibot + ii + hxd[qq]
           * because the Hwarp output index warp location is computed as Hwarp_x(x,y,z) = x + hxd
           * and we also have to add in Hibot to get a global index for use in Haawarp */

          /* we don't need to worry about xq < 0 (etc.), since the warp patch does not run up to the
           * edge of the volume */
          ...
          /* linearly interpolate in Haawarp (global warp we are updating) to get Haawarp
           * displacements at this Hwarp warped location; later, we then interpolate at THESE
           * displacements to get the warped image value */
           ...
           *     = updated warp in this patch xq  = index in srcim for output interpolation to get
           *val
           *
           * Awarp_x(x,y,z) = x + Axd(x,y,z) , so Awarp_x( Hwarp(x,y,z) ) = Hwarp(x,y,z) +
           * Axd(interpolated)
           *                         = Hibot + ii + hxd + Axd(interpolated) Below, bxd = hxd +
           * Axd(interpolated) = displacement for AHwarp. Later, xq  = bxd + Hibot + ii = actual
           *location of warped point */
...
/* Compute the penalty as the pre-computed penalty for the part of the warp outside the current
 * patch (Hpen_sum), plus the penalty in the current patch, as computed from AHwarp.  Hpen_sum is
 * initialized in function IW3D_improve_warp().
 *//*-------------------------------------------------------------------------*/
                              ...
                              float *xs, float *ys, float *zs)
{
    ...
    for (nn = ii = 0 ; ii < npt ; ii++)
    {
        ...
        if (ib < 0.0f || ib >= nx1 ||
            jb < 0.0f || jb >= ny1 || kb < 0.0f || kb >= nz1)
        {
            continue;
        }
        if (is < 0.0f || is >= nx1 ||
            js < 0.0f || js >= ny1 || ks < 0.0f || ks >= nz1)
        {
            continue;
        }
...
/* Warp the input xyz indexes given Haawarp and AHwarp. The input indexes will have been edited to
 * be inside the Haawarp box, so external slopes are not needed. Note that the input points are the
 * base image locations, which will be warped to the source image locations for comparison. The only
 * tricky part is to use AHwarp inside its box, otherwise use Haawarp.
 *//*--------------------------------------------------------------------------*/
                         ...
                         float *xut, float *yut, float *zut)
{
     ...
     * use pointer arithmetic fu to make them seem based at (i,j,k)=(0,0,0) [Note: could easily fail
     *badly if on a segmented memory architecture] */
...
#define XINT(aaa, j, k, dca, \
             dcb)       wt_00 *aaa[dca][IJK(ix_00, j, k)] + wt_p1 * aaa[dcb][IJK(ix_p1, j, k)]
...
/* This is the function which will actually be minimized (via Powell's NEWUOA); and as such is the
 * very core of 3dQwarp!
 *//*--------------------------------------------------------------------------*/
...
double IW3D_scalar_costfun(int npar, double *dpar)
{
    ...
    /* -- the first case in the '?' expressions is when the patch is smaller than the whole volume;
    ...
    if (Hnegate)
    {
        cost = -cost;      /* change the sign? (for minimization) */
    }
    ...
    if (save_H_zero)
    {
        Hcostt_zero = cost; save_H_zero = 0;
    }                                                          /* 10 Mar 2020 */
    ...
    if (Hfirsttime)   /* just for fun fun fun in the sun sun sun */
    {
        if (Hverb)
        {
            fprintf(stderr, "[first cost=%.5f]%c", cost, ((Hverb > 1) ? '\n' : ' '));
        }
...
MRI_IMAGE * IW3D_medianfilter(MRI_IMAGE *imin, float irad)
{
    ...
    if (imin == NULL || imin->kind != MRI_float)
    {
        RETURN(NULL);
    }
...
#pragma omp for
      for (ijk = 0 ; ijk < nxyz ; ijk++)
      {
          ...
          {
              ip = ii + di[dd]; if (ip < 0 || ip >= nx)
              {
                  continue;
              }
              jp = jj + dj[dd]; if (jp < 0 || jp >= ny)
              {
                  continue;
              }
              kp = kk + dk[dd]; if (kp < 0 || kp >= nz)
              {
                  continue;
              }
          ...
          if (nt > 0)
          {
              fout[ijk] = qmed_float(nt, tmp);          /* cf. cs_qmed.c */
          }
      }
#pragma omp critical
      { free(tmp); } }
...
/* Macro to compute actual blur radius from the progressive blur (pb) and the fixed blur (fb) radii
 */
                             ...
                             MRI_IMAGE *inim, char *lab)
{
    ...
    if (fixed_blur == 0.0f && prog_blur == 0.0f)
    {
        RETURN(NULL);
    }
...
#define PBLUR_BASE(i1, i2, j1, j2, k1, k2)                                                    \
        do{ if (Hpblur_b > 0.0f) {                                                            \
                ...
                Hbasim_blur = IW3D_do_blurring(Hblur_b, Hpblur_b,                             \
                                               i2 - i1 + 1, j2 - j1 + 1, k2 - k1 + 1, Hbasim, \
                                               "base");                                       \
            } } while (0)
...
#define PBLUR_SOURCE(i1, i2, j1, j2, k1, k2)                                                  \
        do{ if (Hpblur_s > 0.0f) {                                                            \
                ...
                Hsrcim_blur = IW3D_do_blurring(Hblur_s, Hpblur_s,                             \
                                               i2 - i1 + 1, j2 - j1 + 1, k2 - k1 + 1, Hsrcim, \
                                               "source");                                     \
            } } while (0)

/*----------------------------------------------------------------------------*/

/* Sets a bunch of global workspace variables, prior to iteratively improving the warp with function
 * IW3D_improve_warp() */
                                ...
                                int meth_code, int warp_flags)
{
    ...
    {
        Hblur_b = 0.1f;                                      /* if doing progressive blur, */
    }
    if (Hpblur_s > 0.0f && Hblur_s == 0.0f)
    {
        Hblur_s = 0.1f;                                      /* constant blurs mustn't be 0 */
    }
    Hbasim_blur = IW3D_do_blurring(Hblur_b, Hpblur_b,
...

    {
        ...
        if (Hverb > 1)
        {
            ININFO_message("   %d voxels in mask (out of %d = %.2f%%)",
        ...
        if (nexc > 0)
        {
            ININFO_message("-emask excluded %d voxels", nexc);
        }
    }
...

    {
        ...
        for (Hwbar = nwb = nexc = ii = 0 ; ii < Hnxyz ; ii++)
        {
            if (Hemask != NULL && Hemask[ii])
            {
                wbfar[ii] = 0.0f; Hbmask[ii] = 0; nexc++;
            }
            else
            {
    ...
    /* special case: don't try to over-minimize the Pearson costs (e.g., the base and source images
    ...
    /*-- INCOR method uses 2Dhist functions (iii==2), or is the clipped Pearson method (iii==3), or
     * is a local Pearson method (iii==4), so setup some parameters (into Hmpar) for later use --*/
    ...
    if (iii == 2 || iii == 3)
    {
        ...
        else if (iii == 3)        /* clipped Pearson setup */
        {
            ...
            d1           = 0.5f * (xyc.a - xym.a);                                                /*
                                                                                                   ...
                                                                                                   *min
                                                                                                   */
            dif          = MIN(d1, d2); Hmpar->ar[1] = xyc.a - dif;                               /*
                                                                                                   *
                                                                                                   *
                                                                                                   *
                                                                                                   *xdbot
                                                                                                   */
            d1           = 0.5f * (xym.b - xyc.b); dif = MIN(d1, d2); Hmpar->ar[2] = xyc.b + dif; /*
                                                                                                   *
                                                                                                   *
                                                                                                   *
                                                                                                   *xdtop
            ...
            d1           = 0.5f * (xyc.c - xym.c); dif = MIN(d1, d2); Hmpar->ar[3] = xyc.c - dif; /*
                                                                                                   *
                                                                                                   *
                                                                                                   *
                                                                                                   *ydbot
                                                                                                   */
            d1           = 0.5f * (xym.d - xyc.d); dif = MIN(d1, d2); Hmpar->ar[4] = xyc.d + dif; /*
                                                                                                   *
                                                                                                   *
                                                                                                   *
                                                                                                   *ydtop
                                                                                                   */
            Hmpar->ar[5] = xyc.a; Hmpar->ar[6] = xyc.b;                                           /*
                                                                                                   *
                                                                                                   *
                                                                                                   *
                                                                                                   *xcbot
                                                                                                   *xctop
                                                                                                   */
            Hmpar->ar[7] = xyc.c; Hmpar->ar[8] = xyc.d;                                           /*
                                                                                                   *
                                                                                                   *
                                                                                                   *
                                                                                                   *ycbot
...
/* Given a global warp Haawarp, improve it locally over a rectangular patch. The patch is ibot..itop
 * X jbot..jtop X kbot..ktop (inclusive). Also, keep up-to-date the copy of the warped source image
 * Haasrcim.
 *//*--------------------------------------------------------------------------*/
...
/* set optimization parameter range to be a ball ("sphere"), in parameter space  */
...
/* set optimization parameter range to be a box in parameter space [default]     */
                      ...
                      int ibot, int itop, int jbot, int jtop, int kbot, int ktop)
{
    ...
    if (nxh < NGMIN && nyh < NGMIN && nzh < NGMIN)
    {
        Hskipped++;  RETURN(0);
    }
    ...
    {
        if (Hverb > 2)                                                      /* only print for very
                                                                             * verbose cases,
                                                                             * otherwise just skip
                                                                             * silently */
        {
    ...
    /*-- The max displacement scales are set from the combination of parameters that gave
     * non-singular patch warps with "OK" distortion levels. These scales were selected by running
     * program warping_test a number of times.
     *
     *   Hfactor allows the iteration to scale these down at finer levels, but that wasn't needed
     *after the introduction of the warp penalty. However, the Hfactor_q param in 3dQwarp allow for
     *this capability.  --*/
    ...
    /*-- The LITE version of quintic and cubic polynomials omit the highest order tensor products,
    * which don't add much to the result but do add extra parameters and extra CPU time. The idea
    * for doing this came to me one early morning in Dec 2018 in Guangzhou China, when my jet lag
    * made me wander around in the dark rather than lie in the small hotel room I'd been given. The
    * results: program runs nearly twice as fast, and results aren't significantly different. --*/

    switch (warp_code)
    {
    default:                              /* This was the original warping method, long ago and far
                                           * away */
    case MRI_CUBIC:
        ...
        if (ballopt)
        {
            Hbasis_parmax = 0.0790 * Hfactor;       /* 13 Jan 2015 */
        }
    ...
    case MRI_QUINTIC:
        ...
        if (ballopt)
        {
            Hbasis_parmax = 0.0611 * Hfactor;                  /* 13 Jan 2015 */
        }
        Hnpar = 81;
        prad  = 0.333;
    ...
    if (Hpen_use)
    {
        Hpen_sum = 0.0; /* need to compute warp distortion penalty */
    }
...
#define RESTORE_WBFAR                                     \
    ...
    if (Hnval < Hnxyz)                                /* initialize correlation from   */
    {
        ...
        /* initialize the 'correlation' from the data that won't be changing (i.e., data from
         * outside the local patch)
...
#if 0
    if (WORKHARD(Hlev_now) || SUPERHARD(Hlev_now))
    {
        itmax -= Hnparmap;
    }
#endif
...
/* INFO_message("itmax = %d",itmax) ; */
    ...
    /* Note use of setjmp() here to allow the optimizer to get broken out of by the QUIT or ALRM
     * signal, which can be necessary if OpenMP gets into some race condition
     *
     * How setjmp/longjmp works:
     *  1) The initial call below to setjmp() will return 0, so the "normal" code will be executed
     *after the if().
     *  2) If the ALRM or QUIT signal happen, the signal handler IW3D_signal_quit() is invoked.
     *  3) That handler will call longjmp() which will set things up so that it looks like setjmp()
     *return a nonzero value, and at that point the "bad news"
     *     branch of the if() will be taken.
     *  4) If the signal handler is not invoked, then the program continues normally. Also see
     *https://en.wikipedia.org/wiki/Setjmp.h for more explanation */
...

    {
        ...
        if (asec < 99)
        {
            asec = 99;                     /* min num seconds to wait */
        }
        else if (asec > 1888)
        {
            asec = 1888;   /* max num seconds to wait */
        }
    ...
    if (Hquitting)
    {
        RETURN(0);              /* this code probably redundantly pleonastic */
    }
    /******* iter = number of iterations actually used in this patch *******/

    if (iter > Hnparmap)
    {
        Hnpar_sum += Hnparmap;                   /* number of parameters used so far */
    }
    if (Hverb > 3)
    {
        powell_set_verbose(0);
    }
    ...
    for (pp = 0, kk = kbot ; kk <= ktop ; kk++)
    {
        for (jj = jbot ; jj <= jtop ; jj++)
        {
            ...
            {
                qq      = ii + jj * Hnx + kk * Hnxy;
                sar[qq] = Hwval[pp];                                     /* load into warped source
                                                                          * image */
                ...
                if (Aje[qq] > jt)
                {
                    jt = Aje[qq];             /* largest 'j' penalty in the patch */
                }
                if (Ase[qq] > st)
                {
                    st = Ase[qq];             /* largest 's' penalty */
                }
            }
        }
    }
    ...
    if (Hverb > 1)   /* detailed overview of what went down in this patch */
    {
        ...
        if (Hxyzmatch_num > 0)
        {
            sprintf(cbuf, "xyzmatch pen=%g", Hxyzmatch_cost);
        }
        else
        {
            strcpy(cbuf, "\0");
...
static float Hfactor_from_patchsize_ratio(float prat)
{
    ...
    if (Hfactor_q >= 1.0f || Hfactor_q < 0.1f || prat >= 1.0f)
    {
        return 1.0f;
    }
    if (alpha == 0.0f)
    {
        alpha = logf(Hfactor_q) / logf(0.1f);
    }
 ...
 *   bim      = base image wbim      = weight for base image (can be NULL) sim      = source image
 *(to be warped) meth_code = matching code for INCOR warp_flags = special cases (e.g., no
 *x-displacements) Also, a vast number of H... global variables can be set to affect the way this
 *function works. This function is called from IW3D_warp_s2bim(), which is the function called in
 *turn from 3dQwarp.c -- the face to the world at large. Return value is the index warp as
 *optimized.
 *
 * 3dQwarp is basically a front end for this function. The strategy of patch shrinkage (lev) and
 *looping was derived from (a) wild ass guesses and random emotional states (b) memories of
 *multigrid methods I learned back in the Dark Ages (c) experiences with 3dQwarp
 *//*-------------------------------------------------------------------------*/
                              ...
                              int meth_code, int warp_flags)
{
    ...
    /* This whole mode selection thing is a little messy, as the choice of basis functions is
     * influenced by a bunch of 3dQwarp options, and so some of these settings might be altered
    ...
    if (Huse_sincc)
    {
        cmode = zmode = zmode2 = MRI_SINCC;
    }                                                                   /* Nov 2019 */
    if (Huse_cubic_lite)
    {
        cmode = zmode = zmode2 = MRI_CUBIC_LITE;
    }                                                                   /* Dec 2018 */
    if (Huse_quintic_lite)
    {
        qmode = MRI_QUINTIC_LITE;
    }
    ...
    /* We want patch from imin..imax (etc), BUT must extend it a little on each edge to allow for
     * the warp going to 0 at the edges; thus, we add xwid voxels on each end (if possible) -- etc
     * for the other directions. Also, we make sure the 'global' patch is at least 1 voxel back from
     * the edge of the volume (so the warp really IS zero at the edges). */

    xwid = (imax - imin) / 8; ywid = (jmax - jmin) / 8; zwid = (kmax - kmin) / 8;
    ibbb = MAX(1, imin - xwid); jbbb = MAX(1, jmin - ywid); kbbb = MAX(1, kmin - zwid);
    ittt = MIN(Hnx - 2, imax + xwid); jttt = MIN(Hny - 2, jmax + ywid);
    kttt = MIN(Hnz - 2, kmax + zwid);
    ...
    diii   = ittt - ibbb + 1; djjj = jttt - jbbb + 1; dkkk = kttt - kbbb + 1; /* sizes of this patch
                                                                               */
    iter   = MAX(diii, djjj); iter = MAX(iter, dkkk);                         /* largest size of a
                                                                               * patch edge */
    psize0 = (float)iter;                                                     /* for Hfactor
                                                                               * modification later
                                                                               */
    if (iter < NGMIN)
    {
    ...
    {
        INFO_message(
            "AFNI warpomatic: %d x %d x %d volume ; autobbox = %d..%d %d..%d %d..%d [clock=%s]",
            Hnx, Hny, Hnz, imin, imax, jmin, jmax, kmin, kmax,
            nice_time_string(NI_clock_time()));
    }
    if (Hverb > 1)
    {
        MEMORY_CHECK("starting");
    }
    ...
    /* Hznoq is set in 3dQwarp by the '-noQ' option (not in the help) */
    {
        ...
        if (Hverb == 1)
        {
            fprintf(stderr, "lev=0 %d..%d %d..%d %d..%d: ", ibbb, ittt, jbbb, jttt, kbbb, kttt);
        }

        /* The goal at lev=0 is to get the large scale alignment as good as possible, as quickly as
         * possible. To do this, successively more complicated (more parameters) incremental warps
         * are used over this global patch. This strategy evolved over time, and seems to work
         * reasonably well. I discovered that repeating a given type of warp to try for better
         * optimization (e.g., 2 calls with CUBIC_LITE) seldom did much of value and consumed CPU
         * time, so now each type of warp gets just a single optimizing call at lev=0. The sequence
         * is SINCC(3), CUBIC_LITE(12), CUBIC(24), QUINTIC_LITE(30), QUINTIC(81). */
        ...
        if (Hquitting)
        {
            goto DoneDoneDone;            /* signal to quit was sent */
        }
        ...
        {
            sprintf(warplab, "Lev0.%04dx%04dx%04d", ittt - ibbb + 1, jttt - jbbb + 1,
                    kttt - kbbb + 1);
            HSAVE_ADDTO(Haawarp, warplab);
        }
        if (Hverb == 1)
        {
            fprintf(stderr, " done [cost:%.5f==>%.5f]\n", Hfirstcost, Hcost);
        }
    }
     ...
     * normal situation (code determines patch grid sizes) is !HAVE_HGRID. Below, we set ngmin = the
     *minimum patch size that we will shrink to. */
    ...
    if (!HAVE_HGRID)
    {
        ...
        if (ngmin < NGMIN)
        {
            ngmin = NGMIN;                       /* can't go below this! */
        }
        else if (ngmin % 2 == 0)
        {
            ngmin--;                             /* must be odd */
    ...
    if (ngmin >= Hnx && ngmin >= Hny && ngmin >= Hnz)
    {
        goto DoneDoneDone;
    }
     ...
     * -- That is, the default progress of patch dimensions shrinks by factor of 0.75 at each lev,
    ...
    if (Hshrink > 1.0f)
    {
        Hshrink = 1.0f / Hshrink;
    }
    if (Hshrink < 0.444f || Hshrink > 0.888f)
    {
        Hshrink = 0.749999f;
    }
    ...
    if (HAVE_HGRID && Hgridlist_num - 1 < leve)
    {
        leve = Hgridlist_num - 1;
    }

    /* if '-gridlist' was used and the user inputs a '0', that means to use the default patch size
     * at lev=1, which we save now */

    if (HAVE_HGRID)
    {
        xwid = (Hnx + 1) * 0.75f; if (xwid % 2 == 0)
        {
            xwid++;
        }
        ywid = (Hny + 1) * 0.75f; if (ywid % 2 == 0)
        {
            ywid++;
        }
        zwid = (Hnz + 1) * 0.75f; if (zwid % 2 == 0)
        {
            zwid++;
        }
        if (xwid > ngmin)
        ...
        if (ywid > ngmin && ywid < hgzero)
        {
            hgzero = ywid;
        }
        if (zwid > ngmin && zwid < hgzero)
        {
            hgzero = zwid;
        }
        if (hgzero == 0)
        {
            hgzero = ngmin;             /* should not happen */
        }
    }
    ...
    for (lev = levs ; lev <= leve && !levdone ; lev++)
    {
        ...
        if (!HAVE_HGRID)                      /* the olden way */
        {
            ...
            xwid = (Hnx + 1) * flev; if (xwid % 2 == 0)
            {
                xwid++;                                  /* patch sizes must be odd */
            }
            ywid = (Hny + 1) * flev; if (ywid % 2 == 0)
            {
                ywid++;
            }
            zwid = (Hnz + 1) * flev; if (zwid % 2 == 0)
            {
                zwid++;
            }
#else                        /* the new way, shrinking from lev=0 grid size :) */
            xwid = xwid0 * flev; if (xwid % 2 == 0)
            {
                xwid++;                                  /* 04 Jan 2019 */
            }
            ywid = ywid0 * flev; if (ywid % 2 == 0)
            {
                ywid++;
            }
            zwid = zwid0 * flev; if (zwid % 2 == 0)
            {
                zwid++;
            }
#endif
        ...
        else             /* the new-fangled way [31 Dec 2014] via '-gridlist' */
        {
            ...
            if (xwid == 0)
            {
                xwid = ywid = zwid = hgzero;
            }
            if (xwid < NGMIN)
            {
                goto DoneDoneDone;             /* past the end of the Hgridlist array? */
            }
        }
        psize = MAX(xwid, ywid); if (psize < zwid)
        {
            psize = zwid;
        }
        ...
        if (!dox && !doy && !doz) /* exit immediately if nothing to do (shrank too far) */
        {
            if (Hverb > 1)
            {
                ININFO_message("  ---------  lev=%d xwid=%d ywid=%d zwid=%d -- BREAK", lev, xwid,
                               ywid, zwid);
                ININFO_message("  .........  xwid=%d ywid=%d zwid=%d ngmin=%d", xwid, ywid, zwid,
                               ngmin);
            }
            break;
        }
...

        {
            ...
            glev = ywid / (float)ngmin; if (flev < glev)
            {
                flev = glev;                                         /* largest ratio */
            }
            glev = zwid / (float)ngmin; if (flev < glev)
            {
                flev = glev;                               /* of ?wid to ngmin */
            }
            if (flev > 1.0f && flev * Hshrink <= 1.00001f) /* if next time we would shrink */
            {
                if (xwid > ngmin)
                {
                    xwid = ngmin;                      /* below ngmin, shrink to ngmin NOW */
                }
                if (ywid > ngmin)
                {
                    ywid = ngmin;
        ...
        xdel = (xwid - 1) / 2; if (xdel == 0)
        {
            xdel = 1;
        }
        ydel = (ywid - 1) / 2; if (ydel == 0)
        {
            ydel = 1;
        }
        zdel = (zwid - 1) / 2; if (zdel == 0)
        {
            zdel = 1;
        }
        ...
        ibbb = imin - xdel / 4 - 1; if (ibbb <= 0)
        {
            ibbb = 1;
        }
        jbbb = jmin - ydel / 4 - 1; if (jbbb <= 0)
        {
            jbbb = 1;
        }
        kbbb = kmin - zdel / 4 - 1; if (kbbb <= 0)
        {
            kbbb = 1;
        }
        ittt = imax + xdel / 4 + 1; if (ittt >= Hnx - 1)
        {
            ittt = Hnx - 2;
        }
        jttt = jmax + ydel / 4 + 1; if (jttt >= Hny - 1)
        {
            jttt = Hny - 2;
        }
        kttt = kmax + zdel / 4 + 1; if (kttt >= Hnz - 1)
        {
            kttt = Hnz - 2;
        }
        if (Hnx == 1)
        ...
        {
            zmode2 = qmode;
        }                                                              /* OK, quintic on workhard */

        num_xyz = ((int64_t)xwid) * ((int64_t)ywid) * ((int64_t)zwid); /* 06 Nov 2019 */

        if (Huse_sincc)                                                /* Huse_sincc is disabled at
                                                                        * this time */
        {
            if (num_xyz >= MIN_SINCC)
            {
                cmode = zmode = zmode2 = MRI_SINCC;
            }
            else
        ...
        /* workhard option allows for multiple optimization passes at each level, superhard option
         * allows for multiple passes in each direction of sweeping thru the patches (there are 2
         * sweeping directions) */
        ...
        if (Hpblur_b > 0.0f || Hpblur_b > 0.0f)
        {
            Hfirsttime = 1;
        }
        ...
        {
            ININFO_message(
                "  .........  lev=%d xwid=%d ywid=%d zwid=%d Hfac=%g penfac=%g %s %s [clock=%s]",
                lev, xwid, ywid, zwid, Hfactor,
                (Hpen_use)?Hpen_fff:0.0f,
                (levdone   ? "FINAL"  : "\0"),
                (nlevr > 1 ? "WORKHARD" : "\0"),
                nice_time_string(NI_clock_time()));
            ININFO_message("  .........  volume = %d..%d  %d..%d  %d..%d", ibbb, ittt, jbbb, jttt,
                           kbbb, kttt);
        }
        else if (Hverb == 1)
        {
            fprintf(stderr, "lev=%d patch=%dx%dx%d [clock=%s]", lev, xwid, ywid, zwid,
                    nice_time_string(NI_clock_time()));
        }
        if (Hverb > 1)
        {
            MEMORY_CHECK("continuing");
        }
        Hdone = Hskipped = 0;
...

        {
            ...
            for (isup = 0 ; isup < nsup ; isup++) /* working superhard? do this twice! */
            {
                ...
                for (kdon = 0, kbot = kbbb ; !kdon ; kbot += dkkk) /* loop over z direction of
                                                                    * patches */
                {
                    ktop = kbot + zwid - 1;                        /* top edge of patch: maybe edit
                                                                    * it down or up */
                    if (ktop >= kttt)
                    {
                        ktop = kttt; kbot = ktop + 1 - zwid; kdon = 1;
                    }                                                               /* downsize?*/
                    else if (ktop >= kttt - zwid / 4)
                    {
                        ktop = kttt; kdon = 1;
                    }                                                  /* extend patch to end */
                    ...
                    for (jdon = 0, jbot = jbbb ; !jdon ; jbot += djjj) /* y direction loop */
                    {
                        ...
                        if (jtop >= jttt)
                        {
                            jtop = jttt; jbot = jtop + 1 - ywid; jdon = 1;
                        }
                        else if (jtop >= jttt - ywid / 4)
                        {
                            jtop = jttt; jdon = 1;
                        }
                        ...
                        for (idon = 0, ibot = ibbb ; !idon ; ibot += diii) /* x direction loop */
                        {
                            ...
                            if (itop >= ittt)
                            {
                                itop = ittt; ibot = itop + 1 - xwid; idon = 1;
                            }
                            else if (itop >= ittt - xwid / 4)
                            {
                                itop = ittt; idon = 1;
                            }
        ...
        /* Instead, do the patch sweeping from top to bot, kji order. Otherwise, very similar to the
         * above, so comments are fewer */

        if (lev % 2 == 0 || nlevr > 1)
        {
            if (nlevr > 1 && Hverb == 1)
            {
                fprintf(stderr, ":[cost=%.5f]:", Hcost);
            }
            if (do_qfinal || nlevr > 1)
            {
                BALLOPT;
            }
            ...
            for (isup = 0 ; isup < nsup ; isup++)  /* superhard loops? */
            {
                ...
                for (idon = 0, itop = ittt ; !idon ; itop -= diii)
                {
                    ...
                    if (ibot <= ibbb)
                    {
                        ibot = ibbb; itop = ibot + xwid - 1; idon = 1;
                    }
                    else if (ibot <= ibbb + xwid / 4)
                    {
                        ibot = ibbb; idon = 1;
                    }
                    ...
                    for (jdon = 0, jtop = jttt ; !jdon ; jtop -= djjj)
                    {
                        ...
                        if (jbot <= jbbb)
                        {
                            jbot = jbbb; jtop = jbot + ywid - 1; jdon = 1;
                        }
                        else if (jbot <= jbbb + ywid / 4)
                        {
                            jbot = jbbb; jdon = 1;
                        }
                        ...
                        for (kdon = 0, ktop = kttt ; !kdon ; ktop -= dkkk)
                        {
                            ...
                            if (kbot <= kbbb)
                            {
                                kbot = kbbb; ktop = kbot + zwid - 1; kdon = 1;
                            }
                            else if (kbot <= kbbb + zwid / 4)
                            {
                                kbot = kbbb; kdon = 1;
                            }

                            Hcostold = Hcost;
                            iter     = IW3D_improve_warp(zmodeX, ibot, itop, jbot, jtop, kbot,
                                                         ktop);
                            if (Hquitting)
                            {
                                goto DoneDoneDone; /* signal to quit was sent */
        ...
        /* However, if nothing was done at this level, try one more time, centered on the autobox,
         * just for form's sake. This can happen at lev=1 when none of the big patches overlap the
         * weight volume significantly -- it's not common, but does happen. */

        if (Hdone == 0)
        {
            ibot = (imin + imax - xwid) / 2; if (ibot < 0)
            {
                ibot = 0;
            }
            jbot = (jmin + jmax - ywid) / 2; if (jbot < 0)
            {
                jbot = 0;
            }
            kbot = (kmin + kmax - zwid) / 2; if (kbot < 0)
            {
                kbot = 0;
            }
            itop = ibot + xwid - 1; if (itop >= Hnx)
            {
                itop = Hnx - 1;
            }
            jtop = jbot + ywid - 1; if (jtop >= Hny)
            {
                jtop = Hny - 1;
            }
            ktop = kbot + zwid - 1; if (ktop >= Hnz)
            {
                ktop = Hnz - 1;
            }
            Hforce = 1;
                                  ...
                                  int interp_code, int meth_code, int warp_flags)
{
    ...
    {
        ININFO_message("  -- Hshrink set to %.6f", Hshrink);
    }
                                  ...
                                  byte *bmask, byte *emask)
{
    ...
    else
    {
        ...
        oset = THD_nwarp_dataset(wset, sset, bset, "Mercotan", MRI_LINEAR, Himeth, 0.0f, 1.0f, 1,
                                 NULL);
        DSET_delete(wset);
...
#pragma omp for
                                                                       ...
                                                                       */
      {
          ...
          if (!need_val && !need_AH)
          {
              valp[qq] = valm[qq] = 0.0f; continue;
          }

          Heval(qq, hxd + qq, hyd + qq, hzd + qq); /* if warp not loaded, evaluate it now */

          /* get Hwarp-ed indexes into Haawarp; e.g., xq = Hibot + ii + hxd[qq] because the Hwarp
           * output index warp location is computed as Hwarp_x(x,y,z) = x + hxd and we also have to
           * add in Hibot to get a global index for use in Haawarp */

#if 0
          xq = Hibot + ii + hxd[qq]; if (xq < -0.499f)
          {
              xq = -0.499f;
          }
          else if (xq > nAxh)
          ...
          yq = Hjbot + jj + hyd[qq]; if (yq < -0.499f)
          {
              yq = -0.499f;
          }
          else if (yq > nAyh)
          ...
          zq = Hkbot + kk + hzd[qq]; if (zq < -0.499f)
          {
              zq = -0.499f;
          }
          else if (zq > nAzh)
          ...
          /* bxd = x-displacements for AHwarp = Awarp(Hwarp()) xq  = index in srcim for output
           * interpolation to get val */
...
#define RESTORE_WBFAR                                     \
    ...
    /* AHwarp gets loaded into Haawarp and Hwval_plus into Haasrcim_plus and Hwval_minus into
     * Haabasim_minus */
                                        ...
                                        int meth_code, int warp_flags)
{
    ...
    ittt = MIN(Hnx - 1, imax + xwid); jttt = MIN(Hny - 1, jmax + ywid);
    kttt = MIN(Hnz - 1, kmax + zwid);
    ...
    if (Hlev_start == 0)             /* top level = global warps */
    {
        ...
        if (nlevr)
        {
            ...
            {
                BALLOPT;
                (void)IW3D_improve_warp_plusminus(MRI_CUBIC_PLUS_3, ibbb, ittt, jbbb, jttt, kbbb,
                                                  kttt);
                HCwarp_setup_basis345(0, 0, 0, 0, 0); /* cleanup */
                if (Hquitting)
                {
    ...
    for (lev = levs ; lev <= leve && !levdone ; lev++)
    {
        ...
        ibbb = imin - xdel / 2 - 1; if (ibbb < 0)
        {
            ibbb = 0;
        }
        jbbb = jmin - ydel / 2 - 1; if (jbbb < 0)
        {
            jbbb = 0;
        }
        kbbb = kmin - zdel / 2 - 1; if (kbbb < 0)
        {
            kbbb = 0;
        }
        ittt = imax + xdel / 2 + 1; if (ittt >= Hnx)
        {
            ittt = Hnx - 1;
        }
        jttt = jmax + ydel / 2 + 1; if (jttt >= Hny)
        {
            jttt = Hny - 1;
        }
        kttt = kmax + zdel / 2 + 1; if (kttt >= Hnz)
        {
            kttt = Hnz - 1;
        }
                ...
                for (kdon = 0, kbot = kbbb ; !kdon ; kbot += dkkk)
                {
                    ...
                    for (jdon = 0, jbot = jbbb ; !jdon ; jbot += djjj)
                    {
                        ...
                        for (idon = 0, ibot = ibbb ; !idon ; ibot += diii)
                        {
                            ...
                            Hcostold = Hcost;
                            iter     = IW3D_improve_warp_plusminus(zmode, ibot, itop, jbot, jtop,
                                                                   kbot, ktop);
                            if (Hquitting)
                            {
                                goto DoneDoneDone; /* signal to quit was sent */
                ...
                for (idon = 0, itop = ittt ; !idon ; itop -= diii)
                {
                    ...
                    for (jdon = 0, jtop = jttt ; !jdon ; jtop -= djjj)
                    {
                        ...
                        for (kdon = 0, ktop = kttt ; !kdon ; ktop -= dkkk)
                        {
                            ...
                            Hcostold = Hcost;
                            iter     = IW3D_improve_warp_plusminus(zmodeX, ibot, itop, jbot, jtop,
                                                                   kbot, ktop);
                            if (Hquitting)
                            {
                                goto DoneDoneDone; /* signal to quit was sent */
...
/* Function to compute 'normal' 3dQwarp result from the plusminus warps. From the 3dQwarp help:
 *    Define Wp(x) = x+dis(x) and Wm(x) = x-dis(x). Then since base(Wm(x)) matches source(Wp(x)), by
 *substituting INV(Wm(x)) wherever we see x, we have base(x) matches source(Wp(INV(Wm(x))));
 *    that is, the warp V(x) that one would get from the 'usual' way of running 3dQwarp is V(x) =
 * Wp(INV(Wm(x))). The code to do this is in 3dQWarp.c ('-pmBASE' option) and so this function is
