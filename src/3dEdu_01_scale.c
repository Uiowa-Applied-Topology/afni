/*
 * This is an educational/starter program for writing your own AFNI program.
 *
 * To ensure this program is compiled, see how it is added in Makefile.INCLUDE (details of which
 *might change depending on dependencies used here) by searching for instances of
 * '3dEdu_01_scale'.
 */
 ...
 *
 * The files in "double quotes" are first searched for in this source file's local directory, and
 *those in <angle brackets> are searched for in "a sequence of implementation-defined places"
 * (according to C standard).
 *
 * In practice, AFNI-created header files and those distributed with the AFNI source code are
 *included in "double quotes", while standard C libraries and external dependencies are included in
 * <angle brackets>.
 */
#include <stdio.h>
#include <stdlib.h>
 ...
 * NOTE: HELP TEXT
 *
 * The following function displays help for the program.  We try to keep the content width <=80
 *chars for readability.
 *
 * Each option should be listed with a single hyphen.  AFNI's apsearch program will parse all
 *program help files and enable tab-autocompletion of program options from the command line. It will
 *recognize an option as a string starting with '-' that has only white space to its left. Hence, we
 *list options like:
 *
 ...
 *
 * Note: it is useful to clearly distinguish between option flags and options that take one or more
 *arguments, as above.
 *
 * Section headings should have '~1~' to the right of the title, and subsections should have a
 * '~2~'.  During documentation building, this simple syntax will be parsed to create a table of
 *contents and prettier title for the RST/Sphinx-based documentation.  As an output example, see:
 ...
 *
 * Below we should some basic I/O and other functionality for dealing with datasets.  We list the
 *files where most of those functions and macros are found.
 *
 * - Note 1 -
 * If I want to find where a function is defined in the large AFNI codebase, I will often use
 *something like the following grep:
 *
 *    grep -H FUNC_NAME *.c  | grep TYPE_RETURNED
 *
 * If I can't tell the type returned from usage, then I might grep through header files first to
 *find it:
 *
 *   grep -H FUNC_NAME *.h
 *
 * ... and then use that new-found TYPE_RETURNED information to refine the grep through *.c files.
 *
 * One might have to resort to recursive searching, if the function might be defined in the SUMA/ or
 *other subdirectory, such as with:
 *
 *   grep -r FUNC_NAME .  | grep TYPE_RETURNED
 *
 * - Note 2 -
 * Most DSET_*(...) functionalities are macros that are defined in 3ddata.h.
 */
int main(int argc, char *argv[])
{
    /*
     * Initializing+declaring variables.  Pointers should be set to NULL, to allow for easy
     *checking+usage+freeing later.
     */
    ...
    /*
     * THD_3dim_dataset: One of the main AFNI input datastructures.  It can actually can be used
     *quite generally beyond 'just' 3D vols. This is one way that so many 3d* programs run usefully
     *on 3D or 4D volumes, or surfaces, or (transposed) *.1D files, ...
     *
    ...
    /*
     * Examples of ways to check for required inputs, as well as checking consistency of some (e.g.,
     *if dsets have the same grid).
     *
     * NB: the *_message() functions are useful for communicating with the user.  These are each
     *defined in debugtrace.c.
     */
    ...
    /*
     * These DSET_*(...) macros are defined in 3ddata.h---along with many other macros to access
     *THD_3dim_dataset elements.
     */
    nx    = DSET_NX(dset_inp);
    ny    = DSET_NY(dset_inp);
    nz    = DSET_NZ(dset_inp);
    ...
    /*
     * Go through the array making some calc: here, get the [idx]th value from [0]th subvolume of
     *the 'dset_mask' and 'dset_inp'
     * datasets.
     *
    ...
    /*
     * Prepare header for output by copying that of input, and then changing items as necessary.
     *
     * For:
     ...
     +
     + NB: EDIT_dset_items() is the *only* way you should change elements in a THD_3dmin_dataset
     +structure.
     */
    ...
    /*
     * Note the use of both DSET_delete() and free() with THD_3dim_dataset type (and also above).
     *The first cleans up the internal dataset contents, and the latter frees the pointer itself.
     */
    if (dset_inp)
    {
