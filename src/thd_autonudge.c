                        ...
                        float step, int xstep, int ystep, int zstep, int code)
{
    ...
    int *      ov, *ovp, ovx = 2 * xstep + 1, ovy = 2 * ystep + 1, ovz = 2 * zstep + 1,
        ovxy = ovx * ovy;
    ...
    if (iv1.ijk[0] != iv2.ijk[0])
    {
        xant = 0;                                 /* epi x-axis */
    }
    else if (iv1.ijk[1] != iv2.ijk[1])
    {
        xant = 1;                                 /* in ant    */
    }
    else if (iv1.ijk[2] != iv2.ijk[2])
    {
        xant = 2;
    }
    else
    ...
    /*-- for each origin shift in the xant,yant directions, shift origin of ant dataset, then
     * compare to epi dataset --*/
    ...
    for (pp = 0 ; pp < ovx ; pp++)   /* loop over shifts in 3 directions */
    {
        for (qq = 0 ; qq < ovy ; qq++)
        {
            for (rr = 0 ; rr < ovz ; rr++)
            {
                if (verb)
                {
                    fprintf(stderr, "  starting shift %2d %2d %2d ",
                ...
                /*-- foreach voxel in epi dataset, find how much of it is filled by nonzero ant
                 * voxels --*/
                ...
                for (kk = 0 ; kk < nzepi ; kk++)
                {
                    ...
                    for (jj = 0 ; jj < nyepi ; jj++)
                    {
                        ...
                        for (ii = 0 ; ii < nxepi ; ii++)
                        {
                            ...
                            x1 = xorgepi + dxepi * (ii - 0.5);
                            x2 = xorgepi + dxepi * (ii + 0.49999);
                            ...
                            f1 = (xx1 - xorgant) / dxant + 0.49999;
                            f2 = (xx2 - xorgant) / dxant + 0.49999;
                            if (f1 > f2)
                            {
                                tx = f1; f1 = f2; f2 = tx;
                            }
                            ...
                            g1 = (yy1 - yorgant) / dyant + 0.49999;
                            g2 = (yy2 - yorgant) / dyant + 0.49999;
                            if (g1 > g2)
                            {
                                ty = g1; g1 = g2; g2 = ty;
                            }
                            ...
                            h1 = (zz1 - zorgant) / dzant + 0.49999;
                            h2 = (zz2 - zorgant) / dzant + 0.49999;
                            if (h1 > h2)
                            {
                                tz = h1; h1 = h2; h2 = tz;
                            }
...
#if 0
                            fprintf(stderr,
                                    " epi=%d %d %d  ant=%6.2f..%6.2f %6.2f..%6.2f %6.2f..%6.2f vsum=%6.2f\n",
