/* Trying to isolate SUMA's help functions so that they can be used from AFNI */
#include <stdlib.h>
#include <stdio.h>
#include <assert.h>
#include <string.h>
...
static char * DocumentedWidgets = NULL; /*!< Widget names for which a Sphinx documentation entry has
                                         * been created */
char *SUMA_get_DocumentedWidgets(void)
{
    return DocumentedWidgets;
}
...
char * SUMA_hkf_eng(char *keyi, TFORM target, char *cm)
{
    ...
    default:
    case TXT:   /* SUMA */
        /* Actually COMMA, PERIOD, STAR mods are not needed, leave for posterity.*/
    ...
    case SPX:   /* Sphinx */
        ...
        snprintf(s, 511, "\n.. _%s%s%s:\n\n:ref:`%s %s<%s%s%s>`",
         ...
         #else
        /* Brute force, and a pain, as you can see below. Left here as illustration for 'raw html
         * use */
        /* Note that I endup with two labels for the key, one as an html permalink and another
         * sphinx one preceding the text of the first line. The reason this was done has to do with
         * how the help for each key is defined explicitly in a series of SUMA_StringAppend() calls.
         * I could skip the second (sphinx) label but then my html page would have ':' at the
         * beginning of each line. The solution would be to store the help for each key much as I do
         * for each widget and make SUMA_hkf() take the 1st line and body (a parallel to the widget
         * hint and help) as options. That's a lot of tediousness I don't care for quite yet. */
...
char * SUMA_gsf_eng(char *uwname, TFORM target, char **hintout, char **helpout)
{
    ...
    /* make a copy, uwname is likely a static pointer from a convenience function. Could change
     * underneath you */
    ...
    case WEB:
        ...
        /* DO NOT rely on SUMA_is_Documented_Widget() outside of targ == WEB */
        {
            if (!SUMA_is_Documented_Widget(lnm))
            {
                /* Not all widget in a table have entries so get rid of .c??, .r??, .[*], or .[*,*]
                 * and try again. */
                ntip = strlen(lnm) - 1;
                if (ntip > 4)
                {
                    while (ntip > 0 && lnm[ntip] != '.')
                    {
                        --ntip;
                    }
                }
        ...
        else      /* Either a container widget or a widget for which no match was found */
        /*  backup until you find a container widget then try for it */
        {
            ...
            {
                /* Try some guessing. Before March 4 2015 only headings had permalinks created by
                 * SPHINX. Now I manually insert permalinks for the vast majority of the widgets.
                 * Still, some widgets such as table cells, or frames, might not have their own
                 * entries so we try to get help from their container
                 */
    ...
    case SPX:   /* Sphinx */
        ...
        case 1:      /* actual widget */
            ...
            snprintf(s, 511, "\n"
                ...
                #else
            /* Brute force, and a pain, as you can see below. Left here as illustration for 'raw
             * html use */
            /* The "only::" directives below are not necessary if we are only producing html output.
             * I kept them in should we build other than html in the future */
...
/*
 * Register help for a widget. This is to replace all individual calls to MCW_register_help and
 *_hint.
 *
 * Pointer to help is copied so make sure help is not freed.
 *
 * Widget help becoming centralized to help with auto generation of help webpage
 */
                                       ...
                                       char *hint, char *help)
{
    ...
    if (w)
    {
        if (help)
        {
            ...
            /* DO not free s, MCW_register_help uses the pointer as data to the help callback */
            MCW_register_help(w, s);
        }
        if (hint)
        {
            /* Just make a copy of the hint and don't worry about what got passed! */
 ...
 * \param format (int) 0: Default 1: Sphinx format
 * \param helpout (char **): If Not NULL, this will contain the pointer to a copy of the help string
 *formatted per format. Note that if helpout, *helpout must be NULL at the function call. You must
 *also free *helpout when done with it.
 * \param hintout (char **): If Not NULL, this will contain the pointer to a copy of the hint string
 *formatted per format.
 *                           if (hintout), *hintout must be NULL at function call. You must also
 *free *hintout when done with it.
 * \param whelp_off (int ): If non-zero, number of blank characters by which to offset the lines of
 *a widget's help string
 * \return gwh (GUI_WIDGET_HELP *)  A pointer to the structure containing the widget help. NULL if
 *nothing was found.
 */
                                   ...
                                   int whelp_off)
{
    ...
    if (!gname)
    {
        SUMA_S_Err("NULL name"); SUMA_RETURN(gwhc);
    }
    ...
    /* Find name in list. Note that no attempt at fast search is done here even though the list is
     * alphabetical... */
    gwhc = NULL;
    do {
        if (!el)
...
GUI_WIDGET_HELP *SUMA_Get_Widget_Help(RwcWidget w)
{
    ...
    if (!w)
    {
        SUMA_S_Err("NULL w"); SUMA_RETURN(gwhc);
    }
    ...
    if (!gwhc && (s = XtName(w)))  /* Try matching the hint to the widget name, This is done for
                                    * container widgets */
    {
        el = NULL;
        do {
                           ...
                           RwcWidget widget, int type)
{
    ...
    gwh->w    = (void *)widget;
    gwh->type = type; /* 1, regular widget, 0, container widget, used for organizing the help, 2, a
                       * way to insert an html URI for widgets not individually tracked in the help.
                       * For now this is only done for table entries. */
    ...
    do {
                         ...
                         SUMA_Name_GUI_Help(gwh))) == 0)
        {
            ...
            if (LocalHead || !(sstmp = strstr(WhinedNames, buf)))
            {
                ...
                if (!sstmp)
                {
                    SUMA_strncat(WhinedNames, buf, 1023);
                }
                if (LocalHead)
                {
...
char *SUMA_All_GUI_Help_Info(DList *dl, int detail, int format)
{
    ...
    else
    {
        ...
        do {
            ...
            if (!gwh)
            {
                SUMA_StringAppend(SS, "NULL widget data!");
            }
            else
            {
...
SUMA_Boolean SUMA_is_Documented_Widget(char *wname)
{
    ...
    if (strstr(DocumentedWidgets, wname))
    {
        SUMA_RETURN(YUP);
    }
