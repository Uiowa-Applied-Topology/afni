...
MRI_IMAGE * mri_fft2D(MRI_IMAGE *im, int mode)
{
    ...
    for (jj = 0 ; jj < ny ; jj++)
    {
        for (ii = 0 ; ii < nx   ; ii++)
        {
            opt->r = cpt->r; opt->i = cpt->i; opt++; cpt++;
        }
        for (      ; ii < nxup ; ii++)
        {
            opt->r = opt->i = 0.0; opt++;
    ...
    for (ii = 0 ; ii < nxup ; ii++)
    {
        for (jj = 0 ; jj < nyup ; jj++)
        {
            cxar[jj] = outar[ii + jj * nxup];
        }
        csfft_cox(mode, nyup, cxar);
        for (jj = 0 ; jj < nyup ; jj++)
        {
            outar[ii + jj * nxup] = cxar[jj];
        }
    }
...
XtPointer PLUGIN_imseq_getim(int n, int type, XtPointer handle)
{
    ...
    if (type == isqCR_getimage || type == isqCR_getqimage)
    {
        ...
        if (imp->im != NULL)
        {
            im = mri_to_mri(imp->im->kind, imp->im);
        }
        return (XtPointer)im;
    }
...
int PH_loadim(char *str)
{
    ...
    if (str[0] == '\0' || strncmp(str, "black", 5) == 0)
    {
        if (doA)
        {
            P_ima_null = CONST; P_ima_val = 0.0; Mfree(P_ima);
        }
        else
        {
            P_imb_null = CONST; P_imb_val = 0.0; Mfree(P_imb);
        }
    }
    else if (strncmp(str, "const=", 6) == 0)
    {
        ...
        if (doA)
        {
            P_ima_null = CONST; P_ima_val = val; Mfree(P_ima);
        }
        else
        {
            P_imb_null = CONST; P_imb_val = val; Mfree(P_imb);
        }
    }
    else if (strncmp(str, "noise=", 6) == 0)
    {
        ...
        if (doA)
        {
            P_ima_null = NOISE; P_ima_val = val; Mfree(P_ima);
        }
        else
        {
            P_imb_null = NOISE; P_imb_val = val; Mfree(P_imb);
        }
    }
    else
    {
        ...
        if (doA)
        {
            Mfree(P_ima); P_ima = im;
        }
        else
        {
            Mfree(P_imb); P_imb = im;
        }
    }
    ...
    P_nx = MAX(nxa, nxb); if (P_nx == 0)
    {
        P_nx = 256;
    }
    P_ny = MAX(nya, nyb); if (P_ny == 0)
    {
        P_ny = 256;
    }
    ...
    if (P_ima == NULL)
    {
        P_ima = PH_fakeim(P_nx, P_ny, P_ima_null, P_ima_val);
    }
    if (P_imb == NULL)
    {
        P_imb = PH_fakeim(P_nx, P_ny, P_imb_null, P_imb_val);
    }
