...
/*-------------------------------------------------------------------
 * 08 Feb 2001: read a matrix+vector from a file - RWCox 14 Feb 2001: modified to add "-rotate ..."
 * mode 10 Aug 2005: add MATRIX(...)
 * ---------------------------------------------------------------------*/
...
THD_dvecmat THD_read_dvecmat(char *fname, int invert)
{
    ...
    if (strstr(fname, "::") != NULL)  /*== read from dataset header ==*/
    {
        ...
        *cc = '\0'; cc += 2; /* dname = dataset name ; cc = attribute name */
        ...
        if (strstr(cc, "IJK_TO_CARD_DICOM")) /* return the matrix that turns i,j,k to mm RAI ZSS May
                                              * 07*/
        {
        ...
        else
        {
            ...
            if (ss != NULL)
            {
                *ss = '\0'; ss++; iss = (int)strtol(ss, NULL, 10); if (iss < 0)
                {
                    iss = 0;
                }
            }
...

            {
                ...
                /* Need to include VOLREG_CENTER_OLD, VOLREG_CENTER_BASE, ROTATE_CENTER_OLD,
                 * ROTATE_CENTER_BASE, if applicable. */
