...
/*!  Return value is the number of spikes that were squashed (bigger spikes count more).
 *//*-----------------------------------------------------------------------*/
...
/* Count spikes. Then count steps as spikes in the first difference
...
int main(int argc, char *argv[])
{
    ...
    if (ntime < 11)
    {
        ERROR_exit("Input dataset is too short!");
    }
    ...
    if (mrv == NULL)
    {
        ERROR_exit("Can't load time series data!?");
    }
    DSET_unload(inset);
