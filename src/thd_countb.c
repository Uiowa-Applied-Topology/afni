...
/*****************************************************************
 * A datablock contains the information needed to access the data in memory.
 ******************************************************************/

/*---------------------------------------------------------------
 * Count the number of in-memory sub-bricks actually stored in a datablock at this instant.  Returns
 * -1 if an error.
 * -----------------------------------------------------------------*/
...
int THD_count_databricks(THD_datablock *dblk)
{
    ...
    if (!ISVALID_DATABLOCK(dblk) || dblk->brick == NULL)
    {
        return -1;
    }
...
int THD_count_potential_databricks(THD_datablock *dblk)
{
    ...
    for (ibr = 0 ; ibr < dblk->nvals ; ibr++)
    {
        if (MRI_HAS_DATA(DBLK_BRICK(dblk, ibr)))
        {
            count++;
        }
    }
...
int THD_subset_loaded(THD_3dim_dataset *dset, int nb, int *bb)
{
    ...
    if (!ISVALID_DSET(dset) || nb <= 0 || bb == NULL)
    {
        return 0;
    }
    ...
    for (ii = 0 ; ii < nb ; ii++)
    {
        kk = bb[ii]; if (kk < 0 || kk >= nvals)
        {
            return 0;
        }
        if (!DSET_BRICK_LOADED(dset, kk))
        {
            return 0;
        }
    }
