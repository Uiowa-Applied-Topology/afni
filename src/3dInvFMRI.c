...
#define METHOD_K    11
    ...
    while (iarg < argc)
    {
        ...
        if (strcmp(argv[iarg], "-fir5") == 0)
        {
            if (nmed > 0)
            {
                WARNING_message("Ignoring -fir5 in favor of -median5");
            }
            else
            {
                nfir = 5;
        ...
        if (strcmp(argv[iarg], "-median5") == 0)
        {
            if (nfir > 0)
            {
                WARNING_message("Ignoring -median5 in favor of -fir5");
            }
            else
            {
                nmed = 5;
        ...
        if (strcmp(argv[iarg], "-data") == 0 || strcmp(argv[iarg], "-input") == 0)
        {
            if (yset != NULL)
            {
                ERROR_exit("Can't input 2 3D+time datasets");
            }
            ...
            if (nt < 2)
            {
                ERROR_exit("Only 1 sub-brick in dataset %s", argv[iarg]);
            }
            nxyz = DSET_NVOX(yset);
            iarg++; continue;
        }
        ...
        if (strcmp(argv[iarg], "-map") == 0)
        {
            if (aset != NULL)
            {
                ERROR_exit("Can't input 2 -map datasets");
            }
        ...
        if (strcmp(argv[iarg], "-mapwt") == 0)
        {
            if (wset != NULL)
            {
                ERROR_exit("Can't input 2 -mapwt datasets");
            }
        ...
        if (strcmp(argv[iarg], "-mask") == 0)
        {
            if (mset != NULL)
            {
                ERROR_exit("Can't input 2 -mask datasets");
            }
    ...
    if (yset == NULL)
    {
        ERROR_exit("No input 3D+time dataset?!");
    }
    if (aset == NULL)
    {
        ERROR_exit("No input FMRI -map dataset?!");
    }
...
#define Q(i, j)    qar[(i) + (j) * qim->nx] /* qim->nx X qim->ny */
        ...
        /* compute pseudo-inverse of baseline matrix, so we can project it out from the data time
         * series */
...
#define A(i, j)    aar[(i) + (j) * nvox] /* nvox X nparam */
    ...
    for (jj = 0 ; jj < nparam ; jj++)
    {
        for (ii = kk = 0 ; ii < nxyz ; ii++)
        {
            if (GOOD(ii))
            {
                A(kk, jj) = THD_get_voxel(aset, ii, jj); kk++;
            }
        }
    }
    DSET_unload(aset);
        ...
        for (jj = 0 ; jj < nwt ; jj++)
        {
            for (ii = kk = 0 ; ii < nxyz ; ii++)
            {
                if (GOOD(ii))
                {
                    ...
                    if (WT(kk, jj) > 0.0f)
                    {
                        numpos++; WT(kk, jj) = sqrt(WT(kk, jj));
                    }
                    else if (WT(kk, jj) < 0.0f)
                    {
                        numneg++; WT(kk, jj) = 0.0f;
                    }
                    kk++;
                }
            }
...
#define Y(i, j)    yar[(i) + (j) * nt] /* nt X nvox */
    ...
    for (ii = 0 ; ii < nt ; ii++)
    {
        for (jj = kk = 0 ; jj < nxyz ; jj++)
        {
            if (GOOD(jj))
            {
                Y(ii, kk) = THD_get_voxel(yset, jj, ii); kk++;
            }
        }
    }
    DSET_unload(yset);
...
#define T(i, j)    tar[(i) + (j) * nt]    /* nt X nvox */
    ...
    /***** At this point:
     *        matrix A is in aim, matrix Z is in yim. Solve for V into vim, using the chosen method
     * *****/
    ...
    case METHOD_C:
        ...
        if (wim != NULL)
        {
            WARNING_message("Ignoring -mapwt dataset");
        }
        pfim = mri_matrix_psinv(aim, NULL, alpha); /* nparam X nvox */
        if (pfim == NULL)
        {
            ERROR_exit("mri_matrix_psinv() fails");
        }
        mri_free(aim);
...
#define XX(i)    (((i) < 0) ? far[-(i)] : ((i) > np1) ? far[np2 - (i)] : far[(i)])
    ...
    for (ii = 0 ; ii < npt ; ii++)
    {
        for (sum = 0.0f, jj = 0 ; jj < ntap ; jj++)
        {
            sum += wt[jj] * XX(ii - nt2 + jj);
        }
        x[ii] = sum;
    }
