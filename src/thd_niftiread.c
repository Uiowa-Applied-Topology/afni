 ...
 *
 * [PT,DRG: Mar 12, 2019] some fixes for [qs]form behavior.  Still need to look at [qs]form=2 case,
 *changing space/views.
 *
 */
...
THD_3dim_dataset * THD_open_nifti(char *pathname)
{
    ...
    { /* set the nifti_io debug level       8 Apr 2005 [rickr] */
        ...
        if (ept != NULL)
        {
            nifti_set_debug_level(atoi(ept));
        }
    }
     ...
     * if we are scaling, or if the data type in the NIfTI file is something AFNI can't handle, then
    ...
    {
        /* do not scale if either slope or inter is zero slope==0 : treat as unset (rather than
         * constant data) inter==0 : use slope as brick_fac */

        /* Any NIFTI scalar previously implied conversion to float. No longer scale if inter==0 &&
         * slope!=0 (set brick_fac). Thanks to C Caballero and S Moia for reporting this.
         *                                      26 Jan 2021 [rickr] */
    ...
    if (ppp != NULL)
    {
        ...
        {
            WARNING_message("Illegal AFNI_NIFTI_PRIORITY='%s'", ppp);
        }
    }
    ...
    /* KRH 07/11/05 -- adding ability to choose spatial transform from the options of qform, sform,
     * bothform, or noform.
     *
     * If sform is present, it will be used.
     *
     * If qform is absent, but sform present, then the sform will be modified to be an orthogonal
     *rotation and used.
     *
     * If both qform and sform are absent, then we will have an error.
     *
    ...
    if ((nim->qform_code > 0) && (nim->sform_code > 0))
    {
        if (form_priority == 'Q')
        {
            use_qform = 1; use_sform = 0;
        }
        else
        {
            use_qform = 0; use_sform = 1;
        }
    }
    ...
    if (use_qform)
    {
        ...
        /* determine orientation from the qto_xyz matrix, which transforms (i,j,k) voxel indexes to
...
#define MIN3(a, b, c)    ((MINNUM(a, b)) < (MINNUM(a, c)) ? (MINNUM(a, b)):(MINNUM(a, c)))
        ...
        xmax = MAX3(fabs(nim->sto_xyz.m[0][0]), fabs(nim->sto_xyz.m[1][0]),
                    fabs(nim->sto_xyz.m[2][0])) / dxtmp;

        dytmp = sqrt(nim->sto_xyz.m[0][1] * nim->sto_xyz.m[0][1] +
                     nim->sto_xyz.m[1][1] * nim->sto_xyz.m[1][1] +
                     nim->sto_xyz.m[2][1] * nim->sto_xyz.m[2][1]);

        ymax = MAX3(fabs(nim->sto_xyz.m[0][1]), fabs(nim->sto_xyz.m[1][1]),
                    fabs(nim->sto_xyz.m[2][1])) / dytmp;

        dztmp = sqrt(nim->sto_xyz.m[0][2] * nim->sto_xyz.m[0][2] +
                     nim->sto_xyz.m[1][2] * nim->sto_xyz.m[1][2] +
                     nim->sto_xyz.m[2][2] * nim->sto_xyz.m[2][2]);

        zmax = MAX3(fabs(nim->sto_xyz.m[0][2]), fabs(nim->sto_xyz.m[1][2]),
                    fabs(nim->sto_xyz.m[2][2])) / dztmp;
...

    {
        ...
        /* if pixdim data are present, use them in order to set pixel dimensions.  otherwise, set
         * the dimensions to 1 unit.         */
    ...
    /* You need to set the path too before, if you loaded ~/tmp/joe.nii the path appeared to be
     * ./joe.nii, troubling in multiple instances. ZSS Dec 2011 */
...

    {
            ...
            nim->slice_end < nim->nz)
        {
            ...
            if (nim->time_units == NIFTI_UNITS_MSEC)
            {
                nim->slice_duration *= 0.001;
            }
            else if (nim->time_units == NIFTI_UNITS_USEC)
            {
                nim->slice_duration *= 1.e-6;
            }
    ...
    /* note: this MUST be after EDIT_dset_items(ADN_datum_all), as it will clear any existing values
     * 8 Mar 2021 [rickr] */
     ...
     *               if so, load attributes from it and then edit the dataset appropriately **/
 ...
 *
 * Modify logic to return on the "failed" tests, rather than having a set of nested if's to proceed.
 *                [15 Nov 2021 rickr]
 */
static int THD_nifti_process_afni_ext(THD_3dim_dataset *dset,
                                      nifti_image *nim, char *pathnew)
{
    ...
    else                      /* search in group for proper element */
    {
        ...
        {
            nngr = (NI_group *)nnini[0]; NI_free(nnini);
        }
    }
...
/*-----------------------------------------------------------------
 * Load a NIFTI dataset's data into memory (called from THD_load_datablock in thd_loaddblk.c)
 *  - RWC: modified 07 Apr 2005 to read data bricks via nifti_io.c
 *    'NBL' functions, rather than directly from disk
 *  - RCR: modified 02 Sep 2022 to return status return 0 on success, else an error
 * -------------------------------------------------------------------*/
...
int THD_load_nifti(THD_datablock *dblk)
{
    ...
    /*-- determine if we need to copy the data from the bricks as loaded above because of a type
     * conversion --*/
    ...
    nv = dkptr->nvals;  if (nv > NBL.nbricks)
    {
        nv = NBL.nbricks;
    }
    ...
    if (!need_copy)
    {
        ...
        if (nerr > 0)
        {
            WARNING_message("file %s: corrected %d float errors\n",
...

    {
        ...
        for (ibr = 0 ; ibr < nv ; ibr++)
        {
            if (DBLK_ARRAY(dblk, ibr) == NULL)                   /* make space */
            {
                ...
                if (ptr == NULL)
                {
                    ERROR_message("malloc fails for NIfTI sub-brick #%d", ibr);
                }
                mri_fix_data_pointer(ptr, DBLK_BRICK(dblk, ibr)); /* sub-brick! */
            }
            ptr = DBLK_ARRAY(dblk, ibr); if (ptr == NULL)
            {
                break;                       /* bad news!! */
            }
...
#define CPF(ityp)    do{ ityp *sar = (ityp *)nbuf; float *far = (float *)ptr;      \
    ...
    /* do not scale if either slope or inter is zero slope==0 : treat as unset (rather than constant
     * data) inter==0 : use slope as brick_fac (unless need_copy is applied, then apply slope/inter)
     */

    /* Any NIFTI scalar previously implied conversion to float. No longer scale if inter==0 &&
     * slope!=0 (set brick_fac). Thanks to C Caballero and S Moia for reporting this.
     *                                      26 Jan 2021 [rickr] Also, if need_copy, apply any finite
     *scl_slope, regardless of whether scl_inter == 0. Thanks to @liningpan on github for reporting
     *this.
     *                                      17 Apr 2024 [rickr] */
    ...
    /* rcr - replace this with DBLK_IS_RANGE_MASTERED to also check for csv list
...
static int NIFTI_code_to_view(int code, char *atlas_space)
{
     ...
     * SFORM     SPACE    VIEW
     * -----     -----    ----
     * 1 (or 0)  ORIG     orig 4         MNI      tlrc 3         TLRC     tlrc
     * ?2 (->1)  ACPC     acpc 5         A_TEMP   tlrc
     *
     */
