 ...
 *   rot    = one of the MRI_ROT_ codes (see mrilib.h) mirror = whether to left-right mirror after
 *rotation Note that if no rotation is desired, the input and the output point to the same place!
 * ---------------------------------------------------------------------*/
...
MRI_IMAGE * mri_flippo(int rot, int mirror, MRI_IMAGE *im)
{
     ...
     *   nxout = x dimen in output nyout = y dimen in output d1    = stepsize in original data in
     *the new x direction d2    = stepsize in original data in the new y direction s1    = index in
     *original data of new (0,0) point
     *
