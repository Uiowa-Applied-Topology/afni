...
MRI_IMARR * mri_fvect_to_imarr(MRI_IMAGE *inim)
{
    ...
    if (inim == NULL || inim->kind != MRI_fvect)
    {
        RETURN(NULL);
    }

    var = mri_data_pointer(inim); if (var == NULL)
    {
        RETURN(NULL);
    }

    vd = inim->vdim; if (vd <= 0)
    {
        RETURN(NULL);
    }
    nvox = inim->nvox;
...
MRI_IMAGE * mri_imarr_to_fvect(MRI_IMARR *imar)
{
    ...
    for (kk = 0 ; kk < vd ; kk++)
    {
        ...
        if (aim->kind != MRI_float)
        {
            aim = mri_to_float(aim);
        }
        aar = (float *)aim->im;
        for (ii = 0 ; ii < nvox ; ii++)
        {
...
/*
 * Turn an array of vectors into an MRI_IMAGE. Each vector k (vecs[k]) is a pointer to vec_len
 *floats.
