...
/* Changes made Jun 2016 to make these functions work with OpenMP. Macros in Aomp.h for allocating
 * per-thread data are used in place of the older malloc() calls.  The global userfun is also made
 * per-thread, so in principal different threads could be optimizing different functions (not just
 * the same function with different data).     */
/*---------------------------------------------------------------------------*/
...
static void xreduce(int n, double *x)
{
    ...
    for (ii = 0 ; ii < n ; ii++)
    {
        if (!isfinite(x[ii]) || x[ii] < -9.9f || x[ii] > 9.9f)
        {
            x[ii] = 0.5f;
        }
        else
...
int calfun_(integer *n, doublereal *x, doublereal *fun)
{
    ...
    if (AO_VALUE(scalx) == SC_BOX) /* in this case, inputs x[] are in range 0..1,  */
    {
        ...
        if (verb > 2)
        {
            ...
            for (ii = 0 ; ii < *n ; ii++)
            {
                fprintf(stderr, "%7.4f", sx[ii]); if (ii + 1 < *n)
                {
                    fprintf(stderr, ",");
                }
            }
 ...
 *  - ufunc   = function to minimize: inputs are number of variables (n) and current array of
 *variables (x) at which to evaluate
 *
 *  Return value is number of function calls to ufunc actually done. If return is negative,
 *something bad happened.
 *
 *  MJD Powell, "The NEWUOA software for unconstrained optimization without derivatives", Technical
 *report DAMTP 2004/NA08, Cambridge University Numerical Analysis Group. See:
 * http://www.ii.uib.no/~lennart/drgrad/Powell2004.pdf
 *
 *  For constrained optimization (via trickery), see powell_newuoa_con() [OLD] and
 *powell_newuoa_constrained() [NEW].
 * ------------------------------------------------------------------------------*/
                  ...
                  int maxcall, double (*ufunc)(int, double *))
{
    ...
    if (maxcall < 10 + 5 * ndim)
    {
        maxcall = 10 + 5 * ndim;
    }

    n   = ndim;
    npt = (int)(mfac * n + afac); if (npt < n + 2)
    {
        npt = n + 2;
    }
    icode = (n + 1) * (n + 2) / 2; if (npt > icode)
    {
        npt = icode;
    }
    maxfun = maxcall;
...
void powell_newuoa_set_con_box(void)
{
    con_meth = SC_BOX;
}

void powell_newuoa_set_con_ball(void)
{
    con_meth = SC_BALL;
}

int  powell_newuoa_get_con(void)
{
    return con_meth;
}

void powell_newuoa_set_con(int c)
{
    con_meth = c;
}
...
/*! Similar to powell_newuoa(), but with constraints on the variables, and (if nrand > 0) a random
 * search for the starting vector (the initial vector on input in x[] is also tested to see if it is
 * 'best' for starting). Note the units on rstart and rend are relative (between 0 and 1).
 *
                      ...
                      int maxcall, double (*ufunc)(int, double *))
{
     ...
     *   (b) in calfun_(), if an input variable drifts outside the 0..1 range, bring it back into
     *that range with the PRED01() macro;
     *   (c) then scale that 0..1 value back to the 'true' value before calling ufunc() to evaluate
     *objective function. -------*/

    /*** The above comment describes the original SC_BOX method of dealing with constraints.  The
     * newer SC_BALL method keeps the scaled parameters inside the ball of radius 0.5 centered at
    ...
    {
        sxmin[ii] = xbot[ii];
        sxsiz[ii] = xtop[ii] - xbot[ii]; if (sxsiz[ii] <= 0.0)
        {
            sxsiz[ii] = 1.0;
        }
...
#pragma omp critical
        { if (seed)
          {
              srand48((long)time(NULL) + (long)getpid()); seed = 0;
          }
        }
 ...
 *  - nrand  = if nrand > 0, then it is the number of random vectors inside the constraint
 *xbot..xtop region to evaluate as potential starting vectors, in addition to the vector x[]
 *  - nkeep  = the number of random vectors to keep for the first round of optimization (in addition
 *to the initial x[] vector)
 *  - ntry   = the number of vectors to keep for the second round of optimization
 *  - rstart = size of initial search region around initial value of x units are relative to the
 *search size xbot..xtop (if input as 0, 0.10 will be used)
 *  - rend   = size of final search region = desired accuracy (if input as 0, 0.0001 will be used)
 *  - maxcall= max number of times to call ufunc in any one newuoa() call in the second round of
 *optimization
 *  - ufunc  = function to minimize: inputs are number of variables (n) and current array of
 *variables (x) at which to evaluate
 *  - Sequence of calculations:
 *    - Evaluate ufunc() at the input x[] vector
 *    - Also evaluate ufunc() at nrand random vectors in the constraint domain xbot..xtop
 *    - Keep the nkeep+1 best random vectors plus the input x[] vector, and do a little bit of
 *NEWUOA optimization on each of them (this is the first round optimization) -- however, vectors
 *that are too close to each other will not be used (cf. DTHRESH)
 *    - From those nkeep+1 vectors output by the first round, do the full optimization (this is the
 *second round) on the ntry best starting vectors
 *    - The output vector of the second round that has the smallest value of ufunc() is returned in
 *x[]; the corresponding value of ufunc() is returned in *cost
 ...
 *    - Tentative suggestion: nrand=13*ndim nkeep=15 ntry=5; however, it is hard to have a general
 *rule, since cost functions can be funky.
 *
 *  Return value is number of function calls to ufunc actually done. If return is negative,
 *something bad happened.
 *
 *  The best vector found in the second round of optimization is returned in the x[] array.
 *
 *  MJD Powell, "The NEWUOA software for unconstrained optimization without derivatives", Technical
 *report DAMTP 2004/NA08, Cambridge University Numerical Analysis Group. See:
 *  http://www.ii.uib.no/~lennart/drgrad/Powell2004.pdf
                              ...
                              int maxcall, double (*ufunc)(int, double *))
{
    ...
    for (tt = 0 ; tt < nx01 ; tt++)
    {
        x01val[tt] = BIGVAL;                           /* mark as unready */
    }
...
#define CALFUN_ERROR(xxx)                                                \
    ...
    CALFUN_ERROR(x01[0]); if (!isfinite(x01val[0]))
    {
        x01val[0] = BIGVAL;
    }
...
#pragma omp critical
        ...
        /* Step 1: search nrand start vectors, keeping the nkeep-th best values we find on the way.
        ...
        for (qq = 0 ; qq < nrand ; qq++)
        {
            ...
            CALFUN_ERROR(xtest); if (!isfinite(ftest))
            {
                ftest = BIGVAL;
            }
            for (tt = 1 ; tt <= nkeep ; tt++)                                   /* is this better
                                                                                 * than what */
            {
                if (ftest < x01val[tt])                                         /* we've seen thus
                                                                                 * far? */
                {
                    for (jj = nkeep - 1 ; jj >= tt ; jj--)                      /* push those above
                                                                                 * #tt up */
                    {
                    ...
                    x01val[tt] = ftest;
                    break;                                            /* breaking out of 'tt' loop,
                                                                      * having put xtest in list */
                }
            }
        ...
        for (tt = 0 ; tt <= nkeep && x01val[tt] < BIGVAL ; tt++)
        {
            ;       /* nada */
        }
        ...
        for (tt = 0 ; tt < nkeep ; tt++)
        {
            ...
            CALFUN_ERROR(x01[tt]); if (!isfinite(x01val[tt]))
            {
                x01val[tt] = BIGVAL;
            }
            if (x01val[tt] < vbest)
            {
                vbest = x01val[tt]; tbest = tt;
            }
            if (verb > 1)
            {
                ININFO_message("%2d: cost = %g %c nfunc=%d", tt, x01val[tt], (tbest == tt)?'*':' ',
                               icode);
            }
        }
        ...
        /* Step 2c: cast out those that are too close to better vectors in the max-norm (we always
         * keep the best vector in x01[0]) */
...
#define DTHRESH    0.05 /* max-norm distance threshold for vector reject */
        ...
        if (nkeep == 0)
        {
            ERROR_message("Powell: nothing survived 1st round :-(");
        }
    ...
    for (ii = 0 ; ii < ndim ; ii++)
    {
        x01[tbest][ii] = PRED01(x01[tbest][ii]);
    }
    if (AO_VALUE(scalx) != SC_BOX)
    {
        xreduce(ndim, x01[tbest]);
    }
...
/* Function for minimizing over just one parameter, since powell_newuoa requires at least 2
 * dimensions. Method: repeated subdivision into NCUT pieces, focus on the best piece and the
 * interval around it, then da capo. Brute force, but it works -- usually.
 *//*-------------------------------------------------------------------------*/
                         ...
                         int maxcall, double (*ufunc)(int, double *))
{
    ...
    {
        for ( ; POW_gcd(pstep, ndim) > 1 ; pstep++)
        {
            ;                                     /*nada*/
        }
    }
    ...
    do{
        ...
        for (ii = 1 ; ii < nper ; ii++)
        {
            mapin[ii] = (mapin[ii - 1] + pstep) % ndim;
        }
        for (ii = 0 ; ii < nper ; ii++)
        {
            xloc[ii] = mapar[mapin[ii]];
        }
