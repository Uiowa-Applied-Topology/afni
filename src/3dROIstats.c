...
int main(int argc, char *argv[])
{
    ...
    while (narg < argc && argv[narg][0] == '-')
    {
        ...
        if (strcmp(argv[narg], "-float_format") == 0)
        {
            ...
            if (!strcmp(argv[narg], "float"))
            {
                pstr_apply = pstr_float;
            }
            else if (!strcmp(argv[narg], "pretty"))
            {
                pstr_apply = pstr_pretty;
            }
            else if (!strcmp(argv[narg], "sci"))
            {
                pstr_apply = pstr_sci;
            }
            else if (strchr(argv[narg], '%'))
            {
                pstr_apply = argv[narg];
            }
            else
            {
         ...
         * if( narg+2 >= argc ) Error_Exit("-dsub option needs 2 following arguments!\n")  ;
         *
        ...
        if (non_zero[i])
        {
            ...
            if (!quiet && !summary && !force_num_ROI)
            {
                ...
                if (pcxyz)
                {
                    for (k = 0; k < 3; ++k)
                    {
                        fprintf(stdout, "\t%spc0%c_%s ", wpc, 'x' + k, sklab);
                    }
                    for (k = 0; k < 3; ++k)
                    {
                        fprintf(stdout, "\t%spc1%c_%s ", wpc, 'x' + k, sklab);
                    }
                    for (k = 0; k < 3; ++k)
                    {
                        fprintf(stdout, "\t%spc2%c_%s ", wpc, 'x' + k, sklab);
                    }
                    for (k = 0; k < 3; ++k)
                    {
                        fprintf(stdout, "\t%seig%d_%s ", wpc, k, sklab);
                    }
                    if (pcxyz == 2)
                    {
                ...
                if (nzpcxyz)
                {
                    for (k = 0; k < 3; ++k)
                    {
                        fprintf(stdout, "\t%sNZpc0%c_%s ", wpc, 'x' + k, sklab);
                    }
                    for (k = 0; k < 3; ++k)
                    {
                        fprintf(stdout, "\t%sNZpc1%c_%s ", wpc, 'x' + k, sklab);
                    }
                    for (k = 0; k < 3; ++k)
                    {
                        fprintf(stdout, "\t%sNZpc2%c_%s ", wpc, 'x' + k, sklab);
                    }
                    for (k = 0; k < 3; ++k)
                    {
                        fprintf(stdout, "\t%sNZeig%d_%s ", wpc, k, sklab);
                    }
                    if (nzpcxyz == 2)
                    {
    ...
    /* Now, num_ROI is the number of ROIs from the mask to deal with, and
     * non_zero[mask_data[i]+32768] can be used as in index to a 0..num_ROI array */
    ...
    for (; narg < argc; narg++)
    {
        ...
        /* Previously, float dsets would unload/mallocize/reload every volume (mallocize was because
         * dataset values would be altered if ffac!=0). That might have been (with the hope) to free
         * volumes as they are done. Now, NIFTI datasets re-read from disk due to this.  Why the
         * change?
        ...
        for (brik = 0; brik < DSET_NVALS(input_dset); brik++)
        {
            ...
            case MRI_float:
            {
                ...
                else
                {
                    ...
                    /*                               4 Mar 2021 [rickr] DSET_unload(input_dset);
            ...
            for (i = 0; i < nvox; i++)
            {
                if (mask_data[i])
                {
                    ...
                    if (minmax)
                    {
                        if (input_data[i] < min[ROI])
                        {
                            min[ROI] = input_data[i];
                        }
                        if (input_data[i] > max[ROI])
                        {
                            max[ROI] = input_data[i];
                        }
                    }
                            ...
                            if (mask_data[i] && ROI == non_zero[mask_data[i] + 32768])
                            {
                                if (pcxyz)
                                {
                                    AFNI_1D_to_3D_index(i, vi.ijk[0], vi.ijk[1], vi.ijk[2],
                                                        nvi, nvij);
                                    fv3 = THD_3dind_to_dicomm_no_wod(input_dset,
                                                                     vi);
            ...
            if (!quiet && !summary)
            {
                if (nobriklab)
                {
                    ...
                    {
                        fprintf(stdout, "%s_%d\t\t", argv[narg], brik);
                    }
                    else
                    {
                ...
                if (disp1d == 1)
                {
                    fprintf(stdout, "#%s\t%d[%-.9s]\n\t\t",
                            argv[narg], brik, DSET_BRICK_LABEL(input_dset, brik));
                }
                else if (disp1d == 2)
                {
                    fprintf(stdout, "%s_%d[%-.9s]\t\t",
                            argv[narg], brik, DSET_BRICK_LABEL(input_dset, brik));
                }
                else if (longnames == 1)
                {
                    fprintf(stdout, "%s\t%d[%-.50s]",
                            argv[narg], brik, DSET_BRICK_LABEL(input_dset, brik));
                }
                else
                {
            ...
            if (!summary)
            {
                for (i = 0; i < num_ROI; i++)
                {
                    if (voxels[i]) /* possible if the numROI option is used - 5/00 */
                    {
                        if (mean)
                        {
                            RS_DISP_FLOAT(-1, sum[i] / (double)voxels[i]);
                        }
                        if (nzmean)
                        {
            ...
            else
            {
                for (i = 0; i < num_ROI; i++)
                {
                    if (nzmean)
                    {
                        sumallbriks[i] += nzvoxels[i] ? (double)(nzsum[i] /
                                                                 (double)nzvoxels[i]) : 0.0;
                    }
                    else
                    {
