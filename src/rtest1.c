...
void normalize_doublevector(int n, double *far)
{
    ...
    for (ff = 0.0, ii = 0 ; ii < n ; ii++)
    {
        ff += far[ii];
    }
    ff /= n;
    for (gg = 0.0, ii = 0 ; ii < n ; ii++)
    {
        gg += SQR((far[ii] - ff));
    }
    if (gg <= 0.0)
...
int main(int argc, char *argv[])
{
    ...
    if (mdev < BBOT)
    {
        fprintf(stderr, "M=%d is too small!\n", mdev); exit(1);
    }
    jsize = (int)strtod(argv[nopt++], NULL);
    if (jsize < BBOT)
    {
        fprintf(stderr, "J=%d is too small!\n", jsize); exit(1);
    }
