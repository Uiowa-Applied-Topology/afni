...
void elvis_CB(Widget w, XtPointer cd, XtPointer cb)
{
    ...
    if (cbs->reason != XmCR_EXPOSE || ev->count > 0)
    {
        return;
    }
...
/*---------------------------------------------------------------------------
 * Create an XImage from an RGB image.  Adapted from program "xv". The output of this can be
 *XPutImage-d to a window.
 * -----------------------------------------------------------------------------*/
...
XImage * rgb_to_XImage(Display *dis, XVisualInfo *vin, MRI_IMAGE *im)
{
...

    {
        ...
        /* now put all colors for row #i into output image array, allowing for most possible
         * bytes/pixel and byte ordering stupidities */
