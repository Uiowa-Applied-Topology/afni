...
PLUGIN_interface * PLUGIN_init(int ncall)
{
    ...
    if (ncall > 3)
    {
        return NULL;              /* generate interfaces for ncall 0-3 */
    }
    if (ncall == 1)
    {
        return TSGEN_init();               /* interface # 1 */
    }
    if (ncall == 2)
    {
        return EXP0D_init();               /* interface # 2 */
    }
#ifdef ALLOW_LOMO
    if (ncall == 3)
    {
        return LOMOR_init();               /* interface # 3 */
    }
#else
    if (ncall == 3)
    {
...
char * LSQ_main(PLUGIN_interface *plint)
{
    ...
    nrsin = nrts = ntsim = 0;
    do {
        str = PLUTO_get_optiontag(plint); if (str == NULL)
        {
            break;
        }
        ...
        else if (strcmp(str, "Timeseries") == 0)
        {
            ...
            for (ii = ignore ; ii < tsim[ntsim]->nx && tsar[ii] >= WAY_BIG ; ii++)
            {
                ;                                                                /* nada */
            }
            ignore = ii;
...
void LSQ_worker(int nt, double dt, float *vec, int dofit, char **label)
{
    ...
    if (nlen <= nref)
    {
        return;                 /* do nothing if not enough data to fit */
    }

    /** if data vectors are new length, or have a new number of reference vectors, or have a new
     * time step and need sinusoids, or the initialize flag is set, then reinitialize reference
     * vectors and Choleski factor **/
...
void EXP0D_worker(int num, float *vec)
{
    ...
    if (num <= 0 || vec == NULL || exp0d_pc == NULL)
    {
        return;
    }
...
int lomo_regress(int N, int m, int *yin, int *x)
{
    ...
    if (N < 3 || m < 3 || m >= N || yin == NULL || x == NULL)
    {
        return -1;
    }
    ...
    y = (int *)malloc(sizeof(int) * N); if (y == NULL)
    {
        return -1;
    }
    base = yin[0];
...
/************************ eof main FLOMOR loop *******************************/
    ...
    if (cost < maxcost)
    {
        ...
        for (n = N - 2; n >= 0; n--)
        {
            fprintf(stderr, "  [%d] = %d", n, trellis(pv, pl, n + 1).prevstate->value);
            fflush(stderr);
