 ...
 *      zthr_1sid = zthresh(     pthr ) zthr_2sid = zthresh( 0.5*pthr )  */
...
int main(int argc, char *argv[])
{
    ...
    while (nopt < argc && argv[nopt][0] == '-')
    {
        ...
        if (strcasecmp(argv[nopt], "-mthresh") == 0)
        {
            ...
            if (!STRING_HAS_SUFFIX(argv[nopt], ".niml")) /*--- read cluster-FOM dataset ---*/
            {
                ...
                if (atr->nfl < 5)
                {
                    ERROR_exit("-mthresh dataset MULTI_THRESHOLDS attribute is too short [%d] :(",
                               atr->nfl);
                }
                afl = atr->fl;
                ...
                if (verb)
                {
                    ...
                    ININFO_message(
                        "     clustering NN=%d  thresholding=%d-sided  %d thresholds  %d hpows",
                        nnlev, nnsid, nzthr, nhpow);
                }
                if (nnlev < 1 || nnlev > 3)
                {
                    ERROR_message("illegal nnlev=%d", nnlev); nbad++;
                }
                if (nnsid < 1 || nnsid > 2)
                {
                    ERROR_message("illegal sided=%d", nnsid); nbad++;
                }
                if (nzthr < 1)
                {
                    ERROR_message("illegal nzthr=%d", nzthr); nbad++;
                }
                if (nhpow < 1)
                {
                    ERROR_message("illegal nhpow=%d", nhpow); nbad++;
                }
                if (nbad)
                {
                    ERROR_exit("Can't continue after the above problems");
                }
            }
...

            {
                ...
                else
                {
                    ...
                    if (nhpow != nv1)
                    {
                        ERROR_message("data element in %s mismatch in FOM numbers", argv[nopt]);
                        nbad++;
                    }
                }
    ...
    if (tindex < 0 || tindex >= nval)
    {
        ...
        {
            if (DSET_BRICK_STATCODE(iset, ival) == FUNC_ZT_TYPE)
            {
                break;
            }
        }
        if (ival >= nval)
        {
            for (ival = 0 ; ival < nval ; ival++)
            {
                if (DSET_BRICK_STATCODE(iset, ival) == FUNC_TT_TYPE)
                {
                    break;
                }
            }
        ...
        if (verb)
        {
            INFO_message("threshold default index set to %d", ival);
        }
    }
    ...
    if (fim == NULL)
    {
        ERROR_exit("Can't get sub-brick %d to threshold", tindex);
    }
...
void mri_apply_mask(MRI_IMAGE *inim, MRI_IMAGE *bim)
{
    ...
    if (inim == NULL || bim == NULL || bim->kind != MRI_byte)
    {
        EXRETURN;
    }

    msk  = MRI_BYTE_PTR(bim);
    nvox = inim->nvox; if (bim->nvox != nvox)
    {
        EXRETURN;
    }
