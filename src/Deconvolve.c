 ...
 *
 * File:    Deconvolve.c Author:  B. Douglas Ward Date:    31 August 1998
 *
 * Mod:     Restructured matrix calculations to improve execution speed. Date:    16 December 1998
 *
 * Mod:     Report mean square error from full model. Date:    04 January 1999
 *
 * Mod:     Earlier termination if unable to invert X matrix.
 *         (Avoids redundant error messages.) Date:    06 January 1999
 *
 * Mod:     Modifications for matrix calculation of general linear tests. Date:    02 July 1999
 *
 * Mod:     Additional statistical output (partial R^2 statistics). Date:    07 September 1999
 *
 * Mod:     Allow reading of multiple input stimulus functions from a single file by selection of
 *individual columns. Date:    09 November 1999
 *
 * Mod:     Added options for writing the fitted full model time series (-fitts) and the residual
 *error time series (-errts) to 3d+time datasets. Date:    22 November 1999
 *
 * Mod:     Added -censor option to allow operator to eliminate individual time points from the
 *analysis. Date:    21 June 2000
 *
 * Mod:     Added -censor option to allow operator to eliminate individual time points from the
 *analysis. Date:    21 June 2000
 *
 * Mod:     Added screen display of p-values. Date:    22 June 2000
 *
 * Mod:     Added -glt_label option for labeling the GLT matrix. Date:    23 June 2000
 *
 * Mod:     Added -concat option for analysis of a concatenated 3d+time dataset. Date:    26 June
 *2000
 *
 * Mod:     Increased size of screen output buffer. Date:    27 July 2000
 *
 * Mod:     Additional output with -nodata option (norm.std.dev.'s for GLT linear constraints).
 *Date:    11 August 2000
 *
 * Mod:     Added -stim_nptr option, to allow input stim functions to be sampled at a multiple of
 *the 1/TR rate. Date:    02 January 2001
 *
 * Mod:     Changes to eliminate constraints on number of stimulus time series, number of
 *regressors, number of GLT's, and number of GLT linear constraints. Date:    10 May 2001
 *
 * Mod:     Made fstat_t2p() a static function to avoid conflicts on CYGWIN. Date:    08 Jan 2002 -
 * RWCox
 *
 * Mod:     Added static function student_t2p() for display of p-values corresponding to the
 *t-statistics. Date:    28 January 2002
 *
 * Mod:     Modifications to glt_analysis and report_results for enhanced screen output:  Display of
 *p-values for individual stim function regression coefficients.  Display of t-stats and p-values
 *for individual linear constraints within a GLT. Date:    29 January 2002
 *
 * Mod:     Allow user to specify which input stimulus functions are part of the baseline model.
 *Date:    02 May 2002
 *
 * Mod:     Increased size of screen output buffer (again). Date:    02 December 2002
 *
 * Mod:     global variable legendre_polort replaces x^m with P_m(x) Date     15 Jul 2004 - RWCox
 *
 */
...

{
    ...
    int mfirst = 0, mlast = 0; /* column boundaries of baseline parameters for a block (run) */
    ...
    if (is_xfull)
    {
        matrix_destroy(&xfull); is_xfull = 0;
    }

    if (is_xfull_plus)
    {
        matrix_destroy(&xfull_plus); is_xfull_plus = 0;
    }
    ...
    /*----- Set up columns of X matrix corresponding to the baseline (null hypothesis) signal model
     * -----*/
    ...
    /*----- Set up columns of X matrix corresponding to time delayed versions of the input stimulus
     * -----*/
    ...
    /*----- Keep only those rows of the X matrix which correspond to usable time points (i.e., apply
     * censoring here) -----*/
    ...
    { int ii, nbad, *glist, *blist;
      ...
      for (ii = 0      ; ii < N  ; ii++)
      {
          glist[good_list[ii]] = 1;
      }
      for (nbad = ii = 0 ; ii < nt ; ii++)
      {
          if (glist[ii] == 0)
          {
              blist[nbad++] = ii;
...

{
    ...
    if (!ok)
    {
        matrix_destroy(&xtxinv_temp);  RETURN(0);
    }
    ;
...
/*
 * Convert t-values and F-values to p-value. These routines were copied and modified from:
 * mri_stats.c 16 May 2005: Change names from '_t2p' to '_t2pp' to avoid library name conflict on
 *Mac OS X 10.4 (stupid system).
 */
...

{
    ...
    int ib;                   /* block (run) index */
    int mfirst, mlast;        /* column boundaries of baseline parameters for a block (run) */
    int ibot, itop;
