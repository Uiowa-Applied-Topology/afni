/*balloon.c - Apply the balloon haemodynamic model of Buxton et al. to produce a model haemodynamic
 * time series from a time series of stimuli or behavioural responses. Copyright (c) 2005 Matthew
 * Belmonte
 *
 * Reference:
 *  RB Buxton, EC Wong, LR Frank. Dynamics of blood flow and oxygenation changes during brain
 ...
 * <belmonte@mit.edu>.
 *
 * [PT: 27 Dec 2021] Use printf(...) to display the program help, instead of fprintf(stderr, ...),
 *so that the Sphinx help docs can see it.
 *
 */
...
/*mean venous transit time at rest (seconds) - value from page 858, column 2 of Buxton & al. 1998*/
#define tau_0    2.0

/*resting oxygen extraction fraction - from the legend of Figure 2 (page 859) of Buxton & al. 1998*/
#define E_0      0.4
 ...
 * this value read from the "Flow" panel of Figure 2 (page 859) of Buxton & al. 1998*/
#define f_max    1.7
...
/*outflow exponent - from the legend of Figure 2 (page 859) of Buxton & al. 1998*/
#define f_alpha     0.5

/*outflow linear coefficient - from the constraint f_out(1.3) = 1.7 in the "Fout(V)" panel of Figure
 * 2 (page 859) of Buxton & al. 1998*/
#define f_beta      1.866
...

{
    ...
    if (t < t_rise)
    {
        return 1.0 + t * (f_max - 1.0) / t_rise;
    }
    if (t <= t_rise + t_sustain)
    {
        return f_max;
    }
 ...
 * page 857, column 2 of Buxton & al. 1998: "we modeled f_out(v) as a sum of a linear component and
 *a power law"*/
double f_out(v)
double v;
...
/*change in normalised blood volume over time - Equation 5 (page 857) of Buxton & al. 1998*/
...
/*compute the 1x1 Jacobian comprising (d/dv)(dv/dt), and also the second derivative d2v/dt2.*/
...
/*normalised blood volume as a function of time - computed by numerical solution of the differential
 * equation specified by compute_dvdt() above*/
double v(t)
double t;
...
/*change in normalised deoxyhaemoglobin concentration over time - Equation 5 (page 857) of Buxton &
 * al. 1998*/
...
/*compute the 1x1 Jacobian comprising (d/dq)(dq/dt), and also the second derivative d2q/dt2.*/
...
/*normalised deoxyhaemoglobin concentration as a function of time - computed by numerical solution
 * of the differential equation specified by compute_dqdt() above*/
double q(t)
double t;
 ...
 * in Figure 2 (page 859) of Buxton & al. 1998, percent signal increase is 0.9 for a
 *deoxyhaemoglobin concentration decrease of 0.15, and the legend states that V0 is 0.01.  The
 *legend for Figure 1 (page 858) states that the amplitude of percent signal change "scales directly
 *with the blood volume V0."
 * So we invert the deoxyhaemoglobin effect by subtracting it from 1, and then apply the 0.9/0.15
 *ratio from Figure 2, scaled by the ratio of our V_0 to Figure 2's V_0 of 0.01.*/
double BOLD(t)
double t;
...
/*return -1 if t1->delta < t2->delta, 0 if t1->delta = t2->delta, or 1 if t1->delta > t2->delta*/
...
char **argv;
{
    ...
    for (i = 0; i != num_evt; i++)
    {
        if (((!read_params_from_data_file) && (fscanf(evtfile, "%lf", &t) != 1)) ||
            (read_params_from_data_file && ((fscanf(evtfile, "%lf %lf %lf %lf", &t, envelope[i],
                                                    envelope[i] + 1, envelope[i] + 2) != 4))))
        {
            fclose(evtfile);
            fprintf(stderr,
...
/*no need to initialise model[] to 0, since calloc() accomplishes this for us (assuming IEEE or some
 * other floating-point representation in which zero is represented by bytes that all are zero)*/
    if (model == (double *)0)
    {
        perror((char *)0);
        exit(errno);
    ...
    if (read_params_from_data_file)
    {
        /*Because the data file specifies distinct heamodynamic parameters for each event, we
         * instantiate the balloon model separately for each event, by copying the rise, sustain,
         * and fall times from envelope[i] and computing the balloon model over the entire time
         * evolution of the event:*/
        /*inv: For all events evt[0..i-1], BOLD responses modelled with parameters envelope[0..i-1],
         * respectively, have been summed into model[evt[i].ceiling
         * .. num_acq-1].*/
        for (i = 0; i != num_evt; i++)
        {
            t_rise    = envelope[i][0];
            ...
            /*inv: the BOLD response for event time evt[i] at scan acquisitions evt[i].ceiling ..
             * evt[i].ceiling+j-1 has been summed into model[evt[i].ceiling ..
             * evt[i].ceiling+j-1].*/
    ...
    {
        /*If, on the other hand, the haemodynamic parameters are the same for every event, we can
         * economise by arranging the event times in order of nondecreasing delta - that is, so that
         * the events that occur closest to the following scan acquisition will be processed first -
         * and then instantiating the balloon model only once.  (The numerical integrator is most
         *efficient when BOLD(t) is evaluated in order of nondecreasing t.)*/
        qsort(evt, num_evt, sizeof(*evt), evt_compar);

        /*inv: For every event time evt[i], the BOLD responses for the scan acquisitions numbered
         * evt[i].ceiling .. evt[i].ceiling+j-1 have been summed into model[evt[i].ceiling ..
         * min{evt[i].ceiling+j-1, num_acq-1}].*/
        j = 0;
        create_balloon();
        do{
            /*inv: For event times evt[0..i-1], the BOLD response for the scan acquisition numbered
             * evt[i].ceiling+j has been summed into model[evt[i].ceiling+j] if evt[i].ceiling+j <
             * num_acq.*/
            for (i = 0; i != num_evt; i++)
            {
                if (evt[i].ceiling + j < num_acq)
                {
                    /*evt[i].delta is the fraction of a TR from the time of the event to the time of
                     * the first scan acquisition ator after the time of the event, and j is the
                     * number of scan acquisitions, at or after the time of the event, for which the
                     * haemodynamic response has already been computed.*/
                    model[evt[i].ceiling + j] += BOLD((j + evt[i].delta) * TR);
                }
            }
        }while (++j != num_acq);
