...
/* Take a time series and a list of points to keep, and fill in the un-kept points with values
 * linearly interpolated from the nearest kept points.
...
/* get the workspace for compute_psinv() for a matrix of size m X n, with some extra if desired.
 *//*--------------------------------------------------------------------------*/
...
/*! Compute the pseudo-inverse of a matrix stored in a 2D float array (in column-major order). The
 * input (rmat) is m X n, the output (pmat) is n X m. The workspace should be allocated by
 * get_psinv_wsp(), and can be free()-ed when the need for it is over.
 *//*--------------------------------------------------------------------------*/
...
static void compute_psinv(int m, int n, float *rmat, float *pmat, double *wsp)
{
    ...
    wpp = wsp; if (wpp == NULL)
    {
        wpp = (double *)get_psinv_wsp(m, n, 0);
    }
...
#pragma omp critical
        { if (first)
          {
              ERROR_message("SVD fails in compute_psinv()!\n"); first = 0;
          }
        }
 ...
 *   nr = number of rows = length of vectors nc = number of columns = number of vectors aar = nr X
 *nc array of vectors par = nc X nr pseudo-inverse of aar var = nr length vector to have columns of
 *aar projected out of (will be modified in place) wsp = workspace (at least nc floats long)
 *//*--------------------------------------------------------------------------*/
...
/* Project out TWO sets of vectors, one of which is fixed (aar) and one of which varies with each
 * voxel (bar):
 *   nr = number of rows = length of vectors nc = number of columns = number of vectors in the first
 *set aar = nr X nc array of vectors in the first set par = nc X nr pseudo-inverse of aar nb =
 * number of vectors in the second set if nb == 0, then this function just calls project_out_once()
 *bar = nr x nb array of vectors in the second set pbar = nb x nr workspace of floats to hold the
 *pseudo-inverse of bar (will be computed in here) var = nr length vector to have columns of aar
 *projected out of, and then columns of bar also (will be modified in place) wsp = workspace (at
 *least MAX(nc,nb) floats long)
 *//*--------------------------------------------------------------------------*/
...
static void vector_demean(int n, float *v)
{
    ...
    for (sm = 0.0f, ii = 0 ; ii < n ; ii++)
    {
        sm += v[ii];
    }
    for (sm /= n, ii = 0 ; ii < n ; ii++)
    {
        v[ii] -= sm;
    }
    return;
...
void TPR_process_data(TPR_input *tp)
{
    ...
    if (tp->num_CENSOR > 0)
    {
        ...
        if (TPR_verb > 1)
        {
            INFO_message("-CENSORTR commands:");
        }
        for (cc = 0 ; cc < tp->num_CENSOR ; cc++)
        {
            ...
            if (a < 0 || a >= nt || b < a)
            {
                if (TPR_verb > 1)
                {
                    ININFO_message(" -- skip %d..%d as it is out of data range or means nothing", a,
                                   b);
                }
                continue;
            }
    ...
    for (jj = ntkeep = 0 ; jj < nt ; jj++)
    {
        if (tp->censar[jj] != 0.0f)
        {
            ntkeep++;
        }
    }
    ...
    if (nbl > 1)
    {
        ...
        if (nerr > 0)
        {
            ERROR_exit("Cannot continue with such over-censoring!");
        }
    }
    ...
    if (tp->nstopband > 0)
    {
        ...
        for (tt = 0 ; tt < nbl ; tt++) /* each run (block) is filtered separately */
        {
            ...
            if (TPR_verb > 1)
            {
                ININFO_message("run #%d has %d time points", tt, ntt);
            }

            nf[tt] = ntt / 2; fmask[tt] = (int *)malloc(sizeof(int) * (nf[tt] + 2));
            for (jj = 0 ; jj <= nf[tt] ; jj++)
            {
                fmask[tt][jj] = 0;
            }
            ...
            {
                fbot = tp->stopband[ib].a; if (fbot < 0.0f)
                {
                    fbot = 0.0f;
                }
                else if (fbot > qtop)
                ...
                ftop = tp->stopband[ib].b; if (ftop < 0.0f)
                {
                    ftop = 0.0f;
                }
                else if (ftop > qtop)
                ...
                jbot = (int)rintf((fbot + dff) / df[tt]); if (jbot < 0)
                {
                    jbot = 0;
                }
                jtop = (int)rintf((ftop - dff) / df[tt]); if (jtop > nf[tt])
                {
                    jtop = nf[tt];
                }
                if (TPR_verb > 1)
     ...
     *      ortar vectors the wrong length dsortar time series the wrong length just too many orts
     *for the data length -----*/
    ...
    if (nbad > 0)
    {
        ERROR_exit("Cannot continue after above errors :-( :-( :-( !!");
    }
    ...
    if (nort_fixed > 0)
    {
        ...
        for (jj = 0 ; jj < nt * nort_fixed ; jj++)
        {
            ort_fixed_unc[jj] = 0.0f;
        }
    }
    ...
    if (nort_fixed > 0)
    {
        ...
        if (tp->verb)
        {
            INFO_message("Compute pseudo-inverse of fixed orts");
        }
    ...
    if (tp->verb)
    {
        INFO_message("Loading dataset%s", (tp->dsortar != NULL) ? "s" : "\0");
    }
    if (ntuse == nt)
    {
    ...
    if (tp->verb)
    {
        INFO_message("Starting project-orization");
    }
...
#pragma omp for
      for (vv = 0 ; vv < nvmask ; vv++)
      {
          ...
          if (is_vector_zero(ntuse, zar))
          {
              continue;                /* skip all zero data */
          }
          ...
          if (tp->polort >= 0)
          {
              for (zsum = 0.0f, kk = 0 ; kk < ntuse ; kk++)
              {
                  zsum += zar[kk];
              }
              for (zsum /= ntuse, kk = 0 ; kk < ntuse ; kk++)
              {
                  zar[kk] -= zsum;
              }
          }
...
#pragma omp critical
    ...
    if (ort_fixed != NULL)
    {
        free(ort_fixed); ort_fixed = NULL;
    }
    if (ort_fixed_psinv != NULL)
    {
        free(ort_fixed_psinv); ort_fixed_psinv = NULL;
    }
    ...
    if (tp->do_norm)
    {
        ...
        if (tp->verb)
        {
            INFO_message("normalizing time series");
        }
    ...
    if (tp->verb)
    {
        INFO_message("Convert results to output dataset");
    }
...
int main(int argc, char *argv[])
{
    ...
    while (iarg < argc && argv[iarg][0] == '-')
    {
        ...
        if (strcasecmp(argv[iarg], "-concat") == 0)
        {
            ...
            if (nbl > 0)
            {
                ERROR_exit("You can't use '-concat' twice!");
            }
            cim = mri_read_1D(argv[iarg]);
            if (cim == NULL)
            {
                ERROR_exit("Can't read from -concat file '%s'", argv[iarg]);
            }
            nbl = cim->nvox;
            if (nbl <= 1)
            {
                WARNING_message("-concat file '%s' has only 1 entry ==> ignoring this file",
                                argv[iarg]);
            ...
            else
            {
                ...
                for (tt = 1 ; tt < nbl ; tt++)
                {
                    ...
                    {
                        WARNING_message(
                            "-concat file '%s' has entries out of order ==> ignoring this file",
                            argv[iarg]);
        ...
        if (strcasecmp(argv[iarg], "-input") == 0)
        {
            ...
            if (nname > 1)
            {
                INFO_message("have %d input dataset names", nname);
            }
        ...
        if (strcasecmp(argv[iarg], "-polort") == 0)
        {
            ...
            if (tinp->polort > 20)
            {
                ERROR_exit("-polort value can't be over 20 :-(");
            }
            iarg++; continue;
        }
        ...
        if (strcasecmp(argv[iarg], "-ort") == 0)
        {
            ...
            for ( ; iarg < argc && argv[iarg][0] != '-' ; iarg++)
            {
                ...
                if (oim == NULL)
                {
                    ERROR_exit("-ort: can't read file '%s'", argv[iarg]);
                }
        ...
        if (strcasecmp(argv[iarg], "-dsort") == 0)
        {
            ...
            if (tinp->dsortar == NULL)
            {
                INIT_3DARR(tinp->dsortar);
            }
        ...
        if (strcasecmp(argv[iarg], "-stopband") == 0)
        {
            ...
            if (++iarg >= argc - 1)
            {
                ERROR_exit("Need 2 values after option '%s'", argv[iarg - 1]);
            }
            ...
            if (stop <= sbot)
            {
                ERROR_exit("-stopband: range %.5f %.5f is illegal :-(", sbot, stop);
            }
            ...
            strcasecmp(argv[iarg], "-bandpass") == 0)
        {
            ...
            if (ftop <= fbot)
            {
                ERROR_exit("-stopband: range %.5f %.5f is illegal :-(", fbot, ftop);
            }
        ...
        if (strcasecmp(argv[iarg], "-cenmode") == 0)
        {
            ...
            if (strcasecmp(argv[iarg], "KILL") == 0)
            {
                tinp->cenmode = CEN_KILL;
            }
            else if (strcasecmp(argv[iarg], "ZERO") == 0)
            {
                tinp->cenmode = CEN_ZERO;
            }
            else if (strcasecmp(argv[iarg], "NTRP") == 0)
            {
                tinp->cenmode = CEN_NTRP;
            }
            else
            {
                ERROR_message("unknown value '%s' after -cenmode", argv[iarg]);
            }
            iarg++; continue;
        }
        ...
        if (strcasecmp(argv[iarg], "-censor") == 0)
        {
            ...
            if (tinp->censar != NULL)
            {
                ERROR_exit("Can't use -censor more than once!");
            }
            cenim = mri_read_1D(argv[iarg]);
            if (cenim == NULL)
            {
                ERROR_exit("-censor can't read file '%s'", argv[iarg]);
            }
        ...
        if (strcasecmp(argv[iarg], "-censorTR") == 0)
        {
            ...
            *src = '\0'; /* cat all following options until starts with '-' */
            ...
            if (*src == '\0')
            {
                ERROR_exit("Bad (or no) argument after -CENSORTR");
            }
            ...
            for (ns = 0 ; ns < nsar->num ; ns++)    /* loop over substrings */
            {
                ...
                if (dpt != NULL)     /* found 'run:' */
                {
                    ...
                    else
                    {
                        ...
                        if (r <= 0) /* skip out */
                        {
                            ERROR_message("-CENSORTR %s -- run index '%d' is bad! [iarg=%d]",
                                          nsar->str[ns], r, iarg);
                            nerr++; continue;
                        }
                    }
                    ...
                    if (*cpt == '\0') /* skip out */
                    {
                        ERROR_message("-CENSORTR %s -- no data after run index! [iarg=%d]",
                                      nsar->str[ns], iarg);
                        nerr++; continue;
                    }
                }
                ...
                if (a < 0)                      /* skip out */
                {
                    ERROR_message("-CENSORTR %s -- time index '%d' is bad! [iarg=%d]",
                                  nsar->str[ns], a, iarg);
                    nerr++; continue;
                }
                ...
                if (r > 1)
                {
                    WARNING_message("-CENSORTR '%s' has run number > 1 ==> ignoring it",
                                    nsar->str[ns]);
                }
                else
                {
            ...
            if (nerr > 0)
            {
                ERROR_exit("Can't proceed after -CENSORTR errors! [iarg=%d]", iarg);
            }
    ...
    {
        ERROR_exit(
            "mask and input datasets are NOT on the same 3D grid -- do you need 3dresample?");
    }
    ...
    if (DSET_IS_TCAT(tinp->inset))
    {
        ...
        {
            WARNING_message(
                "-concat option is ignored, since input dataset was automatically catenated");
        ...
        if (!noblock)
        {
            ...
            for (tt = 1 ; tt < nbl ; tt++)
            {
                if (tinp->inset->tcat_len[tt - 1] < MIN_RUN)
                {
                    nerr++;
                }
            ...
            if (nerr > 0)
            {
                ERROR_exit("Auto-catenated dataset has runs with length < %d", MIN_RUN);
            }
            INFO_message("Auto-catenated dataset has %d runs", nbl);
        }
        else
        {
            INFO_message("Auto-catenated dataset treated as 1 run instead of %d",
                         tinp->inset->tcat_num);
        }
    }
