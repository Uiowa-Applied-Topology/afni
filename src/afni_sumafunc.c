                             ...
                             SUMA_irgba **map, int *nvused)
{
    ...
    if (map == NULL || !IM3D_VALID(im3d))
    {
        RETURN(-1);                                    /* that was easy */
    }
    if (nvused != NULL)
    {
        *nvused = 0;                   /* set default return value here */
    }
    ...
    fdset = im3d->fim_now; if (!ISVALID_DSET(fdset))
    {
        RETURN(-1);
    }
    ...
    if (im_thr != NULL && !AFNI_GOOD_FUNC_DTYPE(im_thr->kind))
    {
        im_thr = NULL;
    }
    ...
    if (im_thr != NULL)
    {
        ...
        if (scale_thr == 0.0 || im_thr->kind == MRI_float)
        {
            scale_thr = 1.0;
        }
    }
    ...
    { int ind;
      ...
      if (scale_factor == 0.0)
      {
          scale_factor = im3d->vinfo->fim_autorange;
      }
      if (scale_factor == 0.0)
      {
          scale_factor = 1.0;
    ...
    {
        /* if have an old one (that doesn't match current dataset grid), then delete it from the
         * Universe           */

        if (surf->vn != NULL)
        {
            SUMA_destroy_vnlist(surf->vn);
        }
    ...
    else
    {
        ...
        case MRI_short: {
            ...
            for (ii = 0 ; ii < nvox ; ii++) /* voxel cutting time */
            {
                ...
                if (ar_thr[jj] > -thresh && ar_thr[jj] < thresh)
                {
                    vlist[ii] = -1;
                }
            }
    ...
    case MRI_short: {
        ...
        for (ii = 0 ; ii < nvox ; ii++)
        {
            ...
            {
                for (lp = 0; lp < num_lp && ar_fim[jj] < fim_thr[lp]; lp++)
                {
                    ;                                                    /*nada*/
                }
                if (fim_ovc[lp] == 0)
                {
                    continue;                         /* uncolored pane */
                }
            ...
            else                  /* colorscale - 02 Feb 2003 */
            {
                ...
                kk = (int)(val + 0.49); if (kk >= NPANE_BIG)
                {
                    kk = NPANE_BIG - 1;
                }
    ...
    if (vlist != voxijk)
    {
        free(vlist);                    /* toss trash, if it is trash */
    }
    if (nout == 0)
    {
        free(mmm); RETURN(0);
...
/*! Find node in surface closest to given DICOM vector, with limitation that node's x is in range
 * xbot..xtop, etc.  Return value is node index into ixyz array (not necessarily node ID), -1 if
 * none is found.
 * -------------------------------------------------------------------------*/
...
/*! Set initial colors for surface menu items, for use later when the surface menus are actually
 * created. */
...
void AFNI_init_suma_color(int ss, char *bcol, char *lcol)      /* 06 Sep 2006 */
{
    ...
    if (lin_col < 0)
    {
        lin_col = MIN(6, GLOBAL_library.dc->ovc->ncol_ov - 1);
    }
...
/*! Make the widgets for one row of the surface control panel. The row itself will not be managed at
 * this time; that comes later. */
...
/*! Make surface widgets for an AFNI controller [19 Aug 2002]. Called only once to create the
 * initial set of widgets. AFNI_update_surface_widgets() is used to update the list when something
 * changes.
 * --------------------------------------------------------------------------*/
...
static void AFNI_make_surface_widgets(Three_D_View *im3d, int num)
{
    ...
    if (line_col < 0)
    {
        line_col = MIN(6, im3d->dc->ovc->ncol_ov - 1);
    }
    ...
    if (AFNI_noenv("AFNI_SUMA_START_ON"))
    {
        line_col = box_col = 0;
    }
    ...
    for (ii = 0 ; ii < num ; ii++)
    {
        if (ii < swid_ncol)
        {
            line_col = swid_lincol[ii]; box_col = swid_boxcol[ii];
        }
        else
        {
            line_col = lincol_default; box_col = boxcol_default;
        }
        if (AFNI_noenv("AFNI_SUMA_START_ON"))
        {
            line_col = box_col = 0;
...
void AFNI_update_surface_widgets(Three_D_View *im3d)
{
    ...
    if (!IM3D_OPEN(im3d) || im3d->ss_now == NULL)
    {
        EXRETURN;                  /* bad */
    }
    ...
    if (swid->nall < num)
    {
        ...
        swid->surf_bbox = (MCW_bbox **)XtRealloc((char *)swid->surf_bbox,
                                                 num * sizeof(MCW_bbox *));
        swid->surf_node_av = (MCW_arrowval **)XtRealloc((char *)swid->surf_node_av,
                                                        num * sizeof(MCW_arrowval *));
        swid->surf_line_av = (MCW_arrowval **)XtRealloc((char *)swid->surf_line_av,
                                                        num * sizeof(MCW_arrowval *));
        swid->surf_ledg_av = (MCW_arrowval **)XtRealloc((char *)swid->surf_line_av,
                                                        num * sizeof(MCW_arrowval *));
...
void AFNI_choose_surface_CB(Widget w, XtPointer cd, XtPointer cbs)
{
    ...
    /* Mac OS X mangles up the widgets on first display, so unmanage and remanage them -- this
     * solves that problem */
...
/* Callback for press of an arrowval on the surface controls. All this does is to redraw the images,
 * since that is where the actual info will be grabbed from the swid arrowvals.  */
