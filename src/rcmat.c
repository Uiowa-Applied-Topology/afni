...
float rcmat_avglen(rcmat *rcm)
{
    ...
    nn = rcm->nrc; len = rcm->len; if (nn == 0 || len == NULL)
    {
        return sum;
    }
    for (ii = 0 ; ii < nn ; ii++)
...
/*! Consider a rcmat struct as a symmetric matrix, and Choleski factor it in place.  Return value is
 * 0 if all is OK. A positive return indicates the row/column that had trouble. */
...
/*! Consider a rcmat struct as a lower triangular matrix, and solve the matrix-vector equation
 * [rcm][x] = [vec], in place. */
...
void rcmat_lowert_solve(rcmat *rcm, double *vec)
{
    ...
    if (nn > 999)
    {
        rcmat_lowert_solve_unrolled(rcm, vec); return;
    }
#endif
    rc  = rcm->rc;
    len = rcm->len;
...
/*! Consider a rcmat struct as a lower triangular matrix, and solve the matrix-vector equation
 * [rcm][x] = [vec], in place. Innermost vector loop unrolled.  (Doesn't seem to speed things up?!)
 */
...
void rcmat_lowert_solve_unrolled(rcmat *rcm, double *vec)
{
    ...
    for (ii = 0 ; ii < nn ; ii++)
    {
        ...
        case 7:
            sum = vv[ii] - RV(ii - 6) - RV(ii - 5) - RV(ii - 4) - RV(ii - 3) - RV(ii - 2) -
                  RV(ii - 1); break;

        case 8:
            sum = vv[ii] - RV(ii - 7) - RV(ii - 6) - RV(ii - 5) - RV(ii - 4) - RV(ii - 3) -
                  RV(ii - 2) - RV(ii - 1);
            break;
        ...
        case 11:
            sum = vv[ii] - RV(ii - 10) - RV(ii - 9) - RV(ii - 8) - RV(ii - 7) - RV(ii - 6) -
                  RV(ii - 5)
                  - RV(ii - 4) - RV(ii - 3) - RV(ii - 2) - RV(ii - 1);
            break;
...
/*! Consider a rcmat struct as an upper triangular matrix, and solve the matrix-vector equation
 * [rcm][x] = [vec], in place. */
...
/* The code below is not good for use inside OpenMP, since it uses static variables, and also uses
 * malloc/free repeatedly.
 *//*------------------------------------------------------------------------*/
...
static void set_rbotop(int npt, int nref, float *ref[])
{
    ...
    free_rbotop();
    if (npt < 1 || nref < 1 || ref == NULL)
    {
        return;
    }

    /* find first and last nonzero entries in each column;
     * N.B.: If column #j is all zero, then rbot[j] > rtop[j]. However, this means that we can't do
     *Choleski least squares. */

    rbot = (int *)malloc(sizeof(int) * nref);
    for (jj = 0 ; jj < nref ; jj++)
    {
        rj = ref[jj]; if (rj == NULL)
        {
            free_rbotop(); return;
        }
        for (ii = 0 ; ii < npt && rj[ii] == 0.0f ; ii++)
        {
            ;                                          /*nada*/
        }
        rbot[jj] = ii;
    ...
    {
        rj = ref[jj];
        for (ii = npt - 1; ii >= 0 && rj[ii] == 0.0f ; ii--)
        {
            ;                                            /*nada */
        }
        rtop[jj] = ii;
    ...
    {
        rj = ref[jj];
        for (sum = 0.0, ii = rbot[jj] ; ii <= rtop[jj] ; ii++)
        {
            sum += DM(rj[ii], rj[ii]);
        }
        if (sum == 0.0)
...
rcmat * rcmat_normeqn(int npt, int nref, float *ref[])
{
    ...
    set_rbotop(npt, nref, ref); if (rbot == NULL)
    {
        RETURN(NULL);
    }
    ...
    for (jj = 1 ; jj < nref ; jj++)
    {
        ...
        {
            rkb = rbot[kk]; rkt = rtop[kk]; if (rkb > rkt)
            {
                continue;                                            /* zero */
            }
            ibot = MAX(rkb, rjb); itop = MIN(rkt, rjt); rk = ref[kk];
            for (sum = 0.0, ii = ibot ; ii <= itop ; ii++)
            {
                sum += DM(rj[ii], rk[ii]);
            }
                     ...
                     int nref, float *ref[])
{
    ...
    STATUS("normal eqns");
    rcm = rcmat_normeqn(npt, nref, ref); if (rcm == NULL)
    {
        RETURN(NULL);
    }
...
/*! Create an rcmat struct from a set of float rows.  For each rr[ii], only rr[ii][jj] for jj=0..ii
 * (inclusive) will be accessed (since an rcmat is for symmetric matrices).
 *//*------------------------------------------------------------------------*/
...
rcmat * rcmat_from_rows(int nn, float *rr[])
{
    ...
    for (ii = 1 ; ii < nn ; ii++)
    {
        for (jj = 0 ; jj < ii && rr[ii][jj] == 0.0f ; jj++)
        {
            ;                             /*nada*/
        }
