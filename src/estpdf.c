...
/*
 * This program estimates the probability density function (PDF) corresponding to the distribution
 *of gray-matter and white-matter voxel intensities.  The estimate is formed as the sum of three
 *normal distributions, using the Simplex algorithm for non-linear optimization to estimate the
 *unknown parameters.
 *
 * The Simplex algorithm is adapted from: A Jump Start Course in C++ Programming
 *
 * File:    estpdf.c Author:  B. Douglas Ward Date:    27 May 1999
 */
...
int  hist_min;           /* minimum histogram bin value;  this is set to exclude the large
                          * population near zero */
int hist_max;            /* maximum histogram bin value;  this is set to exclude voxels with very
                          * high intensities */
...
void find_base_value(int nxyz, short *sfim)
{
    ...
    if (kk == 0)
    {
        fprintf(stderr, "** find_base_value: All voxels are zero!\n"); exit(1);
    }
    nbin = kk + 1;
    ...
    {
        if (fbin[kk] < fbin[kk - 1] && fbin[kk] < fbin[kk + 1])
        {
            break;
        }
    }
...
void generate_initial_guess(float *parameters)
{
    ...
    if ((gmean - bmean) < 0.25 * (wmean - bmean))
    {
        gmean = bmean + 0.25 * (wmean - bmean);
    }
    if ((wmean - gmean) < 0.25 * (wmean - bmean))
    {
        gmean = wmean - 0.25 * (wmean - bmean);
    }
...

{
    ...
    const float BIG_NUMBER = 1.0e+10;                           /* return when constraints are
                                                                 * violated */

    float b, bmean, bsigma, g, gmean, gsigma, w, wmean, wsigma; /* parameters */
    float deltah, deltam;                                       /* rough estimate of spread of
                                                                 * distribution */
    ...
    if ((b + g + w < 1.0) || (b + g + w > 2.0))
    {
        return BIG_NUMBER;
    }

    if ((bmean < hist_min) || (bmean > hist_max))
    {
        return BIG_NUMBER;
    }
    if ((gmean < hist_min) || (gmean > hist_max))
    {
        return BIG_NUMBER;
    }
    if ((wmean < hist_min) || (wmean > hist_max))
    {
        return BIG_NUMBER;
    }
    if ((gmean < bmean) || (gmean > wmean))
    {
        return BIG_NUMBER;
    }

    if ((gmean - bmean) < 0.10 * (wmean - bmean))
    {
        return BIG_NUMBER;
    }
    if ((wmean - gmean) < 0.10 * (wmean - bmean))
    {
        return BIG_NUMBER;
    }

    if ((bsigma < 0.01 * deltah) || (bsigma > 0.5 * deltah))
    {
        return BIG_NUMBER;
    }
    if ((gsigma < 0.01 * deltam) || (gsigma > 0.5 * deltam))
    {
        return BIG_NUMBER;
    }
    if ((wsigma < 0.01 * deltam) || (wsigma > 0.5 * deltam))
    {
        return BIG_NUMBER;
    }
...
void simplex_optimization(float *parameters, float *sse)
{
    ...
    while (!done)
    {
        /* find the worst vertex and compute centroid of remaining simplex, discarding the worst
         * vertex */
        eval_vertices(response, &worst, &next, &best);
        ...
        /* test the reflection against the best vertex and expand it if the reflection is better.
         *  if not, keep the reflection */
        ...
        {
            /* new response is between the best and next worst so keep reflection */
            replace(simplex, response, worst, test1, resp1);
        }
        else
        {
            ...
            {
                /* new contracted response is worse, so decrease step size and restart */
                num_iter      = 0;
                num_restarts += 1;
        ...
        /* compare relative standard deviation of vertex responses against a defined tolerance limit
         */
