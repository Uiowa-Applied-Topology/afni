...
/***********************************************************************
 * Plugin to nudge a dataset around. Makes a custom interface.
 * -- RWCox -- April 2000
 ************************************************************************/
...
static int iha = 1, ax1 = 0, ax2 = 1, ax3 = 2, hax1 = 1, hax2 = 2, hax3 = 3, adx = 1, ady = 2,
           adz = 3;
...
char * NUD_main(PLUGIN_interface *plint)
{
    ...
    if (undo_rmat != NULL)
    {
        free(undo_rmat); undo_rmat = NULL;
    }
    if (undo_svec != NULL)
    {
        free(undo_svec); undo_svec = NULL;
    }
    undo_nuse = 0;
    undo_ntop = 0;
    undo_nall = 1;
 ...
 *    Q = R3 R2 R1, where Ri is rotation about axis axi by angle thi. In these routines, the axis
 *codes (ax1,ax2,ax3, hax1,hax2,hax3, and adx,ady,adz) are globals, computed when the dataset is
 *loaded.
 * ----------------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------
 * Compute the rotation angles from the matrix [the signs of these may need to be munged]
 * -------------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------
 * Set the cumulative angles/shifts labels from the current state (stored in globals rmat and svec)
 * -------------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------
 * Write a 3drotate partial command to the screen corresponding to the current nudge -- 31 Aug 2000
 * -------------------------------------------------------------------------*/
...
static void NUD_undo_CB(Widget w, XtPointer client_data, XtPointer call_data)
{
    ...
    if (undo_nuse <= 1)
    {
        SENSITIZE(undo_pb, 0); SENSITIZE(doall_pb, 0);
    }
    SENSITIZE(redo_pb, 1);
...
/*-------------------------------------------------------------------
 * Callback for choose button - give the user some dataset choices. Criteria for datasets that can
 *be nudged:
...
/*      qset = ss->dsset_xform_table[id][vv] ;*/
        ...
        if (!DSET_INMEMORY(qset))
        {
            continue;
        }
        if (DSET_BRICK_TYPE(qset, 0) == MRI_complex)
        {
            continue;
        }
...
static void NUD_finalize_dset_CB(Widget w, XtPointer fd, MCW_choose_cbs *cbs)
{
    ...
    if (dset_ival >= DSET_NVALS(dset))
    {
        dset_ival = DSET_NVALS(dset) - 1;
    }
    ...
    * iha = left or right handed coordinate order in dataset ax1 = axis index for 'I'   hax1 = sign
    *for roll angle ax2 = axis index for 'R'   hax2 = sign for pitch angle ax3 = axis index for
    * 'A'   hax3 = sign for yaw angle   */
...
#if 0
    if (clipit && (mode == MRI_LINEAR || mode == MRI_NN))
    {
        clipit = 0;
    }
#endif
...
static void NUD_update_base(Widget w)
{
    ...
    if (dset == NULL || imbase == NULL || dset_ival >= DSET_NVALS(dset))
    {
        return;
    }
