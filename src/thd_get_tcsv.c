...
NI_ELARR * THD_get_many_tcsv(THD_string_array *dlist)
{
    ...
    for (id = 0 ; id < ndir ; id++)
    {
        if (THD_forbidden_directory(dlist->ar[id]))
        {
            continue;
        }
    ...
    if (epath != NULL)
    {
        ...
        do{
            ...
            if (THD_forbidden_directory(ename))
            {
                continue;                                /* 18 Sep 2020 */
            }
...
NI_ELARR * THD_get_all_tcsv(char *dname)
{
    ...
    {
        fname = qsar->str[ir]; if (fname == NULL)
        {
            continue;
        }
