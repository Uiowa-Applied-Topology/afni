 ...
 * Radial Basis Function (RBF) interpolation in 3D.
 ...
 + They are designed for repeated interpolations from the same set of knots, using different sets of
 +values each time.
 + The C2 polynomial with compact support (0 < r < 1) f(r) = (1-r)^4*(4r+1) is the RBF used here,
 +with a global linear polynomial.
...
/* RBF expansion i=nk-1 f(x) = sum      { alpha_i * RBF(x-q_i) } + b_0 + x*b_x +y*b_y + z*b_z i=0
 *
 * where the weights alpha_i for knot location q_i are chosen so that the expansion f(q_i) matches
 *the function f() at that location, for each knot q_i.  The (optional) global linear polynomial is
 *available to provide nonzero values of f(x) for x far away from any knots.
 *//*..........................................................................*/
...
/*! Evaluate RBF expansion on a set of points in rbg, given the set of knots in rbk, and given the
 * coefficients of the RBF (or the values at the fit points) in rbe, and store results in array val
...
#pragma omp for
        for (ii = 0 ; ii < npt ; ii++)
        {
            ...
            if (kfirst != NULL)
            {
                kbot = kfirst[ii]; ktop = klast[ii];
            }
            for (sum = 0.0f, jj = kbot ; jj <= ktop ; jj++)
            {
                xk = xt - xx[jj]; rr = xk * xk; if (rr >= rqq)
                {
                    continue;
                }
                yk = yt - yy[jj]; rr += yk * yk; if (rr >= rqq)
                {
                    continue;
                }
                zk = zt - zz[jj]; rr += zk * zk; if (rr >= rqq)
                {
                    continue;
                }
    ...
    if (verb)
    {
        ININFO_message("              Elapsed = %.1f", COX_clock_time() - ct);
    }
    RETURN(1);
}
 ...
 *  that is, convert them from values at each fit point to the weights used for the RBF centered at
 *each knot (plus the linear polynomial). Return value is 0 if bad things happened, 1 if good things
 *happened.
 *//*--------------------------------------------------------------------------*/
...
int RBF_setup_evalues(RBF_knots *rbk, RBF_evalues *rbe)
{
    ...
    if (rbe->code > 0)
    {
        RETURN(1);                  /* already contains RBF weights */
    }
    if (verb)
    {
                            ...
                            float *xx, float *yy, float *zz)
{
     ...
     *  xm = middle (median) of the x knot coordinates xd = scale (MAD) of the x knot distances from
     *xm;
     *        would be about L/4 if knots are uniformly spaced in 1D over a distance of L
     *  ==> 4*xd/nk is about inter-knot x distance in 1D uniform case
     *  ==> 4*(xd+yd+zd)/(3*cbrt(nk)) is about mean inter-knot distance in 3D RBF support radius is
     *chosen to be a small multiple of this distance */
     ...
     *   mat[ii][jj] = RBF( knot[ii] - knot[jj] )  for jj=0..ii. Call this matrix M.  It is positive
     *definite by the choice of the particular radial basis function being used.
     *
     * If we are NOT using linear polynomials, then the system of equations to solve for RBF knot
     *weights [alpha] given knot function values [v] is [M] [alpha] = [v] We Choleski decompose [M]
     * = [L] [Lt] so that we solve via [alpha] = [Ltinv][Linv] [v] This will be done in function
     *RBF_setup_evalues(), which converts values of the function to be interpolated at the knots [v]
     *into weights [alpha].
     *
     * If we ARE using a global linear polynomial, then the system of equations to solve is [ M  P ]
     *[alpha] = [v] [ Pt 0 ] [beta ] = [0] where [P] is an nk X 4 matrix, with entries P_{i0}=1
     *P_{i1}=x_i  P_{i2}=y_i  P_{i3}=z_i (knot coordinates) Pt = P-transpose [beta] = 4-vector of
     *weights for linear polynomial The purpose of this is to make the RBF part of the expansion not
     *contain any linear component -- that will be carried by the global linear polynomial. Without
     *this constraint, the system would be over-determined: nk equations with nk+4 unknowns.
     *
     * The first nk equations express the interpolation property:
     * the evaluated RBF sum plus the linear polynomial equals [v] at the knots.  The last 4
     *equations express the property that the [alpha] weights by themselves do NOT have any
     *component of a global linear polynomial.
     *
     * The symmetric compound (nk+4)X(nk+4) matrix is obviously not positive definite (since it has
     *0 values on the diagonal), so cannot be solved directly by Choleski factorization.  Instead,
     *we solve it via a bordering technique:
     *    [M][alpha] + [P][beta] = [v]
     *      ==> [alpha] = [Minv][v] - [Minv][P][beta]          (*) [Pt][alpha] = [0]
     *      ==> [Pt][Minv][v] = [Pt][Minv][P][beta]
     *      ==> [beta] = inv{ [Pt][Minv][P] }[Pt][Minv][v]     (**) Once [beta] is known from
     *Eqn(**), [alpha] is found by using Eqn(*). In details, given the Choleski factors of [M]:
     *   Compute 4x4 matrix [Q] = inv{ [Pt][Ltinv][Linv][P] }  (done below) Given [v], [ahat] =
     * [Ltinv][Linv][v]  (==[alpha] if not using [P]) [beta] = [Q][Pt][ahat] [alpha] =
     * [Ltinv][Linv]{ [v] - [P][beta] }
     * We then use [alpha] to evaluate the RBF at each output grid point, and use [beta] to evaluate
     *the global linear polynomial.
     *//*.....................................................................*/
    ...
    if (verb > 1)
    {
        ININFO_message("                 matrix computation");
    }
    ...
    if (verb > 1)
    {
        ININFO_message("                 matrix factorization");
    }
    ...
    if (rcm == NULL)
    {
        ...
        if (uselin)
        {
            free(P0); free(Px); free(Py); free(Pz);
        }
        RETURN(NULL);
    }
...
/*! For speedup, for each grid point compute the first and last knots that can affect its output
 * value.  This will save time when looping over knots to evaluate the RBF expansion.
 *//*----------------------------------------------------------------*/
...
void RBF_setup_kranges(RBF_knots *rbk, RBF_evalgrid *rbg)
{
    ...
    if (rbg->klast != NULL)
    {
        free(rbg->klast); rbg->klast = NULL;
    }
    if (rbg->kfirst != NULL)
    {
        free(rbg->kfirst); rbg->kfirst = NULL;
    }

    if (rbk->nknot > RBFKINT_MAX)
    {
        EXRETURN;                            /* too many to store r/
                                              *
                                              * /* load some local variables */
    }
    npt = rbg->npt;
...
#pragma omp for
    ...
    if (verb > 1)
    {
        ...
        for (ii = 0 ; ii < npt ; ii++)
        {
            ntot += (1.0f + rbg->klast[ii] - rbg->kfirst[ii]);
        }
