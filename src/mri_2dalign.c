...
/*--------------------------------------------------------------------
 * Inputs: imbase = base image for alignment imwt   = image of weight factors to align to (if NULL,
 *will generate one internally)
 *
 * Output: pointer to a MRI_2dalign_basis struct, for later use. The malloc-ed data in there can be
 *freed using routine mri_2dalign_cleanup.
 * ----------------------------------------------------------------------*/
...
MRI_2dalign_basis * mri_2dalign_setup(MRI_IMAGE *imbase, MRI_IMAGE *imwt)
{
    ...
    if (use_fine_fit)
    {
        ...
        if (imwt == NULL)
        {
            imww = mri_to_float(bim);                 /* 03 Oct 1997 */
        }
        else
        {
            imww = mri_to_float(imwt);
 ...
 * dx , *dy, and *phi are set to rotation parameters. Note that the returned image is floats.
 * -------------------------------------------------------------------------*/
...
void mri_2dalign_cleanup(MRI_2dalign_basis *basis)
{
    ...
    if (basis->fitim != NULL)
    {
        DESTROY_IMARR(basis->fitim);
    }
    if (basis->chol_fitim != NULL)
    {
        free(basis->chol_fitim);
    }

    if (basis->fine_fitim != NULL)
    {
        DESTROY_IMARR(basis->fine_fitim);
    }
    if (basis->chol_fine_fitim != NULL)
    {
        free(basis->chol_fine_fitim);
    }
