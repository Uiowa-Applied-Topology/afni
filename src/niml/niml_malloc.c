...
/*! Allow user to replace malloc(), realloc(), and free() functions used in NI_malloc(),
 * NI_realloc(), and NI_free().
...
typedef struct {
    void * pmt;  /* pointer to actually malloc-ed block (user ptr is +NEXTRA bytes later)   */
...
static void probe_track(NI_mallitem *ip, char *fn, int ln)
{
    ...
    for (ii = 0 ; ii < NEXTRA ; ii++)
    {
        if (fred[ii] != MAGIC)
        {
            ...
            if (fn != NULL)
            {
                fprintf(stderr, "   Caller=%s line#=%d\n", fn, ln);
            }
            break;
        }
    }
...
static void * realloc_track(NI_mallitem *ip, size_t n, char *fn, int ln)
{
    ...
    if (nfred == NULL)
    {
        return NULL;                  /* this is bad - real bad */
    }
...
/*!  Return a status string about the situation. This is stored in a static buffer, so don't free
 * it.
 * -------------------------------------------------------------------*/
