...
/*! Create a new vector, of the given type and length. Returns NULL if an error occurs.  Otherwise,
 * the vec and vec_range arrays are calloc()-ed, and the statistic struct is set to NULL.
 * -------------------------------------------------------------*/
...
void NI_set_vector_range(void *nvv)
{
    ...
    if (nv == NULL ||
        ...
        nv->vec_typ == NI_STRING)
    {
        return;
    }

    len = nv->vec_len; if (len <= 0)
    {
        return;
    }
