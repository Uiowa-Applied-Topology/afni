...
void addto_Dtable(char *str_a, char *str_b, Dtable *dt)
{
    ...
    if (dt == NULL || str_a == NULL || str_b == NULL)
    {
        return;
    }
...
void removefrom_Dtable_a(char *str_a, Dtable *dt)
{
    ...
    free((void *)str_bb); if (str_aa != NULL)
    {
        free((void *)str_aa);
    }
    return;
...
void removefrom_Dtable_b(char *str_b, Dtable *dt)
{
    ...
    free((void *)str_aa); if (str_bb != NULL)
    {
        free((void *)str_bb);
    }
    return;
...
/* Create lists of the pointers to the strings directly inside the Dtable.  Return value is number
 * of string pairs.  Since these are pointers INTO the Dtable, don't free() these!
...
int listize_Dtable(Dtable *dt, char ***list_a, char ***list_b)
{
    ...
    if (dt == NULL || list_a == NULL || list_b == NULL)
    {
        return 0;
    }
    ...
    for (nn = jj = 0 ; jj < ht->len ; jj++)
    {
        ...
        {
            sa = (char *)ht->ctab[jj][kk]; if (sa == NULL)
            {
                continue;
            }
            sb = (char *)ht->vtab[jj][kk]; if (sb == NULL)
            {
                continue;
            }
...
char * Dtable_to_nimlstring(Dtable *dt, char *name)
{
    ...
    NI_stream_close(ns);
    nn = strlen(stout);
    for (ii = nn - 1 ; ii > 0 && isspace(stout[ii]) ; ii--)
    {
        ;                                                 /* trailing blanks */
    }
    stout[ii + 1] = '\0';
