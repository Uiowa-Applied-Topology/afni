...
static int header_stuff_is_group(header_stuff *hs)    /* 24 Feb 2005 */
{
    ...
    if (strcmp(hs->name, "ni_group") == 0)
    {
        return 1;
    }
    atr = get_header_attribute(hs, "ni_form");
    if (atr != NULL && strcmp(atr, "ni_group") == 0)
    {
        return 1;
    }
    return 0;
...
int NI_write_procins(NI_stream_type *ns, char *str)
{
    ...
    if (str == NULL || !IS_STRING_CHAR(*str))
    {
        return -1;
    }
...
void NI_set_read_header_only(int r)
{
    read_header_only = r;
}                                                             /* 23 Mar 2003 */

int NI_get_read_header_only(void)
{
...
/*! Read an element (maybe a group) from the stream, waiting up to msec milliseconds for the header
 * to appear.  (After that, this function may wait a long time for the rest of the element to
 * appear, unless the data stream comes to a premature end.)
 *
 * Return is NULL if nothing can be read at this time.  Otherwise, use NI_element_type(return value)
 *to determine if the element read is a data element or a group element.
 *
 * Note that a header that is longer than ns->bufsize will never be read properly, since we must
 *have the entire header in the buffer before processing it.  This should only be a problem for
 *deranged users (e.g., Ziad).  If such a vast header is encountered, it will be flushed.
 *
 * If header start '<' and stop '>' are encountered, then this function will read data until it can
 *create an element, or until the data stream is bad (i.e., the file ends, or the socket closes).
 *
 * If NULL is returned, that can be because there is no data to read even in the buffer, or because
 *the input data stream has gone bad (i.e., will return no more data ever).  To check for the latter
 *case, use NI_stream_readcheck().
 *
 * If a "<ni_do ... />" or "<?ni_do ... ?>" element is encountered, it will not be returned to the
 *caller.  Instead, the actions it orders will be carried out in function NI_do(), and the function
...
void * NI_read_element(NI_stream_type *ns, int msec)
{
    ...
    /* if we have a socket that hasn't connected, then see if it can connect now            */
...
HeadRestart:                                       /* loop back here to retry */
    ...
    if (num_restart > 1 && mleft <= 0)
    {
        return NULL;                                  /* don't allow too many loops */
    }
#ifdef NIML_DEBUG
    ...
    /* see if we found '<>', which is meaningless, or a trailer '</stuff>', which is illegal here */
    ...
    /*----- If here, have parsed a header (and will not HeadRestart). First, expunge the data bytes
     * that were consumed to make the header; that is, we can then start reading data from
...

    {
        ...
        if (strcmp(hs->name, "?ni_do") == 0) /* 19 Apr 2005: special case! */
        {
            ...
            if (ns->bad == MARKED_FOR_DEATH || ns->buf == NULL)
            {
                return NULL;
            }
...

    {
        ...
        if (empty)
        {
            return ngr;         /* 03 Jun 2002: empty group is legal */
        }
...

    {
...

        {
            ...
            if (form != NI_TEXT_MODE)
            {
                ...
                if (strstr(nel->attr_rhs[ii], "lsb") != NULL)
                {
                    order = NI_LSB_FIRST;
                }
        ...
        if (form == NI_TEXT_MODE)
        {
            ii = NI_LTEND_MASK;                        /* end on '<' char  */
        }
        else if (swap)
        {
            ii = NI_SWAP_MASK;                         /* swap binary data */
        }
        else
        {
            ii = 0;                                    /* no special flag  */
        }
        ...
        /* 27 Mar 2003: allow for case where vec_len is inferred from how much data we read */
        ...
        /*-- Now scan for the end-of-element marker '</something>' and skip all input bytes up to
...
TailRestart:
        ...
        if (num_restart < 99)  /* don't loop forever, dude */
        {
            ...
            /* if we didn't find '<...>' at all, then if the I/O stream is bad, just exit;
 ...
 *  - ns->npos will be altered to reflect the current buffer position (one after the last character
 *processed) when all is done.
...
int NI_decode_one_double(NI_stream_type *ns, double *val, int ltend)
{
    ...
    if (ns == NULL || ns->bad == MARKED_FOR_DEATH || val == NULL)
    {
        return 0;
    }
...
Restart:
    ...
    while (ns->npos < ns->nbuf && IS_USELESS(ns->buf[ns->npos]))
    {
        ns->npos++;
    }

    /*-- check if we ran into the closing '<' prematurely (before any useful characters); if we did,
     * then we are done --*/
    ...
    /*-- An input value is decoded from a string of non-useless characters delimited by a useless
     * character (or by the element closing '<'). Note that the 1st character we are now at is
     * non-useless. Scan forward to see if we have a useless character later. --*/

    if (!need_data)   /* so have at least 2 characters */

    {
#ifdef NIML_DEBUG
        nn = ns->nbuf - ns->npos; if (nn > 19)
        {
            nn = 19;
        }
        ...
        {
            if (ns->buf[epos] == '<' || IS_USELESS(ns->buf[epos]))
            {
                break;
            }
        }
        ...
        /*- If the string of characters we have is not yet delimited, and it is too long to be a
         * number, throw out all the data in the buffer and quit. -*/

        if (need_data && epos - ns->npos > NVBUF)
        {
            clear_buffer(ns); return 0;
        }
    }
    ...
    if (need_data)
    {
        ...
        /*- read at least 1 byte, waiting up to 666 ms (unless the data stream goes bad) -*/
        ...
        if (nn >= 0)
        {
            goto Restart;           /* check if buffer is adequate now */
        }

        /*- if here, the stream went bad.  If there are still data bytes in the stream, we can try
         * to interpret them. Otherwise, must quit without success.                  -*/
    ...
    nn = epos - ns->npos; if (nn > NVBUF)
    {
        nn = NVBUF;                                        /* # bytes to read   */
    }
...
/*! From the NI_stream ns, starting at buffer position ns->npos, decode one string into newly
 * NI_malloc()-ed space pointed to by *str.
 *  - Parameter ltend !=0 means to stop at '<' character [07 Jan 2003].
 *  - ltend != 0 also means to skip lines starting with '#' [20 Mar 2003].
 *  - Return value of this function is 1 if we succeeded, 0 if not.
 *  - ns->npos will be altered to reflect the current buffer position (one after the last character
...
int NI_decode_one_string(NI_stream_type *ns, char **str, int ltend)
{
    ...
    if (ns == NULL || ns->bad == MARKED_FOR_DEATH || str == NULL)
    {
        return 0;
    }
...
Restart:
    ...
    if (num_restart > 2 && overbuf) /* 23 Nov 2007: auto-expand buffer */
    {
        ...
        if (nn > 0)
        {
            ...
            if (nn < 0)
            {
                return 0;      /*** buffer expand fails? give up ***/
            }
        }
    }
    overbuf = 0;
    ...
    *str = NI_malloc(char, nn + 1);         /* make the string */
    ...
    if (sp.j < ns->nbuf && IS_QUOTE_CHAR(ns->buf[sp.j]))
    {
        sp.j++;
    }
...
/*! Reset the unscanned bytes in the buffer to start at position 0 instead of position ns->npos;
 * then set ns->npos to 0.
 * ------------------------------------------------------------------------*/

void NI_reset_buffer(NI_stream_type *ns)
{
    if (ns == NULL || ns->npos <= 0 || ns->nbuf <= 0)
    {
        return;
    }
    if (ns->buf == NULL || ns->bad == MARKED_FOR_DEATH)
    {
        return;
    }
...
/*! Scan stream for an element header or trailer:'<characters>', starting at byte offset ns->npos,
 * and waiting msec milliseconds.
 *
 *  Returns with the stream buffer set so that the opening '<' is at ns->buf[ns->npos] and the
 *closing '>' is at ns->buf[q-1], where q is this function's return value.  Note that read
 *operations may change ns->npos from its value when this function was called.
 *
 *  If the return value is -1, then we couldn't find a '<stuff>' string. This may be due to:
 *    - there is no '<...>' in the buffer, and we can't read from the input stream; call
 *NI_readcheck(ns,0) to confirm this
 *    - time ran out (alas)
 *    - The '<...' part filled the entire buffer space.  In this case, all the input buffer is
 *thrown away - we don't support headers or trailers this long!
 *
...
Restart:                                           /* loop back here to retry */
    ...
    while (epos < ns->nbuf && ns->buf[epos] != goal)
    {
        epos++;
    }
    ...
    if (epos < ns->nbuf)
    {
         ...
         *   set the buffer position to this location, set the new goal, and scan for the new goal
         * --*/
     ...
     *   (a) if the goal was the opening '<', then throw away all data in the buffer, and get some
     *more data (b) if the goal was the closing '>', then we need more data in the buffer, but need
     *to keep the existing data (c) UNLESS the buffer is full AND npos is zero
    ...
    /*-- read at least nbmin bytes, waiting up to mleft ms (unless the data stream goes bad) --*/
    ...
    if (nn >= nbmin)
    {
        caseb = 1;                  /* got new data => force rescan */
    }
    if (nn >= 0)
    {
        goto Restart;               /* scan some more for the goal */
    }
...
{
    if (nmode > 0 && nmode <= NI_ATTMODE_LAST)
    {
        name_mode = nmode;
    }
    else
    {
        name_mode = NI_NAMEMODE_NORMAL;
    }
}
...
/*! Write an element (data or group) to a stream. Return value is number of bytes written to the
 * stream. If return is -1, something bad happened.  You should then check the stream with
 * NI_stream_goodcheck(), for example.
 *
 *  If the stream is temporarily unable to write (e.g., the socket buffer is full), then this
 *function will wait until it is ready. If you don't want that behavior, you should use
 *NI_stream_writecheck() before calling this function.
 * --------------------------------------------------------------------------*/
...
#undef  AF
#define AF
#define ADDOUT(q)    if (nout < 0) { AF;                                                           \
                                     fprintf(stderr, "NIML: write abort %s nout=%" PRId64 "\n", q, \
                                             nout); return -1; } else ntot += nout
    ...
    if (tt == NI_PROCINS_TYPE)
    {
        ...
        if (header_sharp)
        {
            nout = NI_stream_writestring(ns, "# "); ADDOUT("a");
        }
        ...
        {
            jj = NI_strlen(npi->attr_lhs[ii]); if (jj == 0)
            {
                continue;
            }
            ...
            jj = NI_strlen(npi->attr_rhs[ii]); if (jj == 0)
            {
                continue;
            }
    ...
    else if (tt == NI_GROUP_TYPE)
    {
        ...
        /* 24 Feb 2005: all group elements used to be named "ni_group", but no more; now they have
         * attribute ni_form="ni_group" */
        ...
        if (header_sharp)
        {
            nout = NI_stream_writestring(ns, "# "); ADDOUT("i");
        }
#if 1
        ...
        {
            jj = NI_strlen(ngr->attr_lhs[ii]); if (jj == 0)
            {
                continue;
            }
            ...
            jj = NI_strlen(ngr->attr_rhs[ii]); if (jj == 0)
            {
                continue;
            }
        ...
        if (header_sharp)
        {
            nout = NI_stream_writestring(ns, "# "); ADDOUT("t");
        }
#if 1
...
#define AF    NI_free(att) /* free att if we have to quit early now */
        ...
        if (header_sharp)
        {
            nout = NI_stream_writestring(ns, "# "); ADDOUT("y");
        }
            ...
            for (ll = -1, ii = 0 ; ii < nel->vec_num ; ii++)
            {
                if (nel->vec_typ[ii] != ll) /* not the previous type */
                {
                    if (ll >= 0)            /* write the previous type out now */
                    {
                        ...
                        if (jj > 1)
                        {
                            sprintf(btt, "%d*%s,", jj, NI_type_name(ll));
                        }
                        else
                        {
                            sprintf(btt, "%s,", NI_type_name(ll));
                        }
                    }
        ...
        {
            jj = NI_strlen(nel->attr_lhs[ii]); if (jj == 0)
            {
                continue;
            }
            ...
            if (strcmp(nel->attr_lhs[ii], "ni_form") == 0)
            {
                continue;
            }
            if (strcmp(nel->attr_lhs[ii], "ni_type") == 0)
            {
                continue;
            }
            if (strcmp(nel->attr_lhs[ii], "ni_dimen") == 0)
            {
                continue;
            }
            if (strcmp(nel->attr_lhs[ii], "ni_veclen") == 0)
            {
                continue;
            }
            if (strcmp(nel->attr_lhs[ii], "ni_vecnum") == 0)
            {
                continue;
            }
            if (strcmp(nel->attr_lhs[ii], "ni_delta") == 0)
            {
                continue;
            }
            if (strcmp(nel->attr_lhs[ii], "ni_origin") == 0)
            {
                continue;
            }
            if (strcmp(nel->attr_lhs[ii], "ni_units") == 0)
            {
                continue;
            }
            if (strcmp(nel->attr_lhs[ii], "ni_axes") == 0)
            {
                continue;
            }
            if (strcmp(nel->attr_lhs[ii], "ni_datasize") == 0)
            {
                continue;                                              /* 13 Apr 2004 */
            }
            ...
            if (kk > 0)
            {
                ...
                if (kk > att_len)
                {
                    att_len = kk; att = NI_realloc(att, char, att_len);
                }
                strcat(att, qtt); NI_free(qtt);
            }
...
#define AF    /* nothing to do if we have to quit early */
        ...
        /* first, terminate the header, and allocate space for the write buffer (1 row at a time) */
        ...
        {
            NI_dpr("NI_write_element: header_only case\n");
        }
#endif
        ...
        if (header_sharp)
        {
            nout = NI_stream_writestring(ns, "# "); ADDOUT("Q");
        }
...
int64_t NI_write_element_tofile(char *fname, void *nini, int tmode)
{
    ...
    if (fname == NULL || *fname == '\0' || nini == NULL)
    {
        return -1;
    }
    ...
    if (ns == NULL)
    {
        fprintf(stderr, "NIML: fail to open file %s for writing\n", fname); return -1;
    }
...
char * NI_write_element_tostring(void *nel)
{
    ...
    NI_stream_close(ns);
    jj = strlen(stout);
    for (ii = jj - 1 ; ii > 0 && isspace(stout[ii]) ; ii--)
    {
        ;                                                 /* trailing blanks */
    }
    stout[ii + 1] = '\0';
