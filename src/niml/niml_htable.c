...
void addto_Htable(char *str, void *vpt, Htable *ht)
{
    ...
    if (vpt == NULL)
    {
        removefrom_Htable(str, ht); return;
    }
...

    {
        for (kk = 0 ; kk < ht->ntab[jj] ; kk++)
        {
            ...
            else if (strcmp(key, ht->ctab[jj][kk]) == 0)
            {
                break;                                                  /* found it? */
            }
        }
...
void subsume_Htable(Htable *htold, Htable *htnew)
{
    ...
    if (htold == NULL || htold->ntot == 0 || htnew == NULL)
    {
        return;
    }
