...
/*! Read an un-headered file into a data element, guessing at its structure from the 1st non-comment
 * line.
 * -----------------------------------------------------------------*/
...
/*! Like system fgets(), but allows LF, CR, CR+LF, and LF+CR as end of line markers.  Converts all
 * of them to a single LF character, so that the results are as if the input was an honest-to-Allah
 * Unix file, instead of some bastard child of Microsoft.  (Not that I have any prejudices.)
 *//*--------------------------------------------------------------------------*/
...
static char * ni_fgets(char *buf, int nbuf, FILE *fp)
{
    ...
    do{
        ...
        {
            buf[nin++] = (char)LF;  /* Microsoft coverup */
        }
        if (cin == CR || cin == LF) /* end of line */
        {
            ...
            if ((cin == CR && qin != LF) || (cin == LF && qin != CR))
            {
                ungetc(qin, fp);
            }
...
static char * my_fgets(char *buf, int size, FILE *fts, int flags)
{
    ...
    if (qbuf == NULL)
    {
        qbuf = NI_malloc(char, LBUF); /* 1st time in */
    }
...

    {
        ...
        if (cflag && ll > 1 && ptr[ll - 2] == '\\')
        {
            cflag = 0;
        }
        if (cflag)
        {
            ptr[ll - 1] = ' ';          /* replace '\' with ' ' */
        }
    ...
    if (nbuf > 0)
    {
        return buf;                 /* return what we read already */
    }
