 ...
 *
 *  Calling with the same verb will replace the function - you can't have two callbacks for the same
 *verb.  If func is input as NULL, then this will remove a callback, if it was defined earlier; if
 *func is NULL and verb was not previously defined, then nothing happens.
 *
 *  However, you CAN register a callback for a builtin verb.  The normal processing will take place,
 *then the user callback will be invoked if that processing was good.  [This feature was added on 30
 *Dec 2003.]
 * -----------------------------------------------------------------------------*/
...
/*! Carry out an action ordered by a "ni_do" element received on the input stream.  Actions we know
 * about:
 *
...
int NI_do(NI_stream_type *ns, NI_element *nel)
{
    ...
    if (ns == NULL || nel == NULL || nel->type != NI_ELEMENT_TYPE)
    {
        return -1;
    }

    if (strcmp(nel->name, "ni_do") != 0 &&
        strcmp(nel->name + 1, "ni_do") != 0)
    {
        return -1;
    }
    ...
    if (verb == NULL)
    {
        verb = NI_get_attribute(nel, "verb");
    }

    object = NI_get_attribute(nel, "ni_object");
    if (object == NULL)
    {
        object = NI_get_attribute(nel, "object");
    }
    if (object == NULL)
    {
        object = NI_get_attribute(nel, "ni_obj");
    }
    if (object == NULL)
    {
        object = NI_get_attribute(nel, "obj");
    }

    if (verb == NULL || verb[0] == '\0')
    {
        return -1;                                          /* need a verb;  */
    }
    /* but not always */
    /* need an object  */
    /*******************************************/
