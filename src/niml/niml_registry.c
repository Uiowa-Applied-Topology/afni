...
/*! Create the (empty) underlying hash tables for indexing the list of registry elements. */
 ...
 *  - If len=0, then 1 byte will be malloc-ed, but this special case is a flag that no data will
 *actually be available at this location -- cf. functions NI_registry_idcode_to_len() and
 *NI_registry_ptr_to_len.
 ...
 *     be lost and the new idcode will win (that is, you can't have two different idcodes associated
...
void * NI_registry_add(char *idcode, char *name, void *vpt)
{
    ...
    if (idcode == NULL || *idcode == '\0' || vpt == NULL)
    {
        return NULL;
    }
 ...
 *  - Calling this with an entry created via NI_registry_add() will return NULL, since that function
 *is used for registering things that aren't to be malloc-ed by this library.
 * ---------------------------------------------------------------------*/
...
void * NI_registry_realloc(void *vpt, size_t newlen)
{
    ...
    if (vpt == NULL || registry_htable_ipt == NULL)
    {
        return NULL;
    }
    ...
    if (vpt_new == vpt)
    {
        return vpt;                     /* no change! */
    }

    /* remove the pointer-based entry from the index, then make a new pointer index
     *                 */
...
/*! For something added with NI_registry_add(), lets you replace the pointer with some other
 * pointer.
 * ---------------------------------------------------------------------*/
...
void NI_registry_free(void *vpt)
{
    ...
    if (rent == NULL)
    {
        return;                  /* stupid users must be punished somehow */
    }
...
/*! Given an idcode, get the data length that goes with it. Note that 0 is returned if the data ptr
 * was setup with len=0 OR* if the idcode can't be found in the registry.
 * ---------------------------------------------------------------------*/
...
/*! Given a data pointer, get the data length that goes with it. Note that 0 is returned if the data
 * ptr was setup with len=0 OR* if the data ptr can't be found in the registry.
 * ---------------------------------------------------------------------*/
...
/*! Given an idcode, get the name string that went with it. This is the pointer into the internal
 * registry_entry struct, so don't modify it!
 * ---------------------------------------------------------------------*/
