...
/* These typedefs are used to find the byte alignment the compiler imposes on the 'basic' types
 * supported by NIML. */
...
/*! The array of user-defined rowtypes, indexed by type code. Note that rowtypes are never deleted -
 ...
 *  (The '-1' is to allow for the String type, which isn't a basic type but is a builtin type.) */
...
static void setup_basic_types(void)
{
    ...
    if (rowtype_table != NULL)
    {
        return;                          /* don't run this twice */
    }
...
/* Define a NI_rowtype, which is an expression of a C struct type (with some restrictions).
 *  - tname = Name to call this thing (must be unique and can't be one of the builtin types listed
 *below).
 *  - tdef = Definition of the type:
 *     - A list of type names ('components') separated by commas.
 *     - A type name is one of the NIML basic types ("byte", "short", "int", "float", "double",
 * "complex", "rgb", "rgba"), or is "String", or is a previously defined tname from a another
 *NI_rowtype.
 *     - Recursive types cannot be defined; that is, no component of tdef can refer to the same
 *tname that is being defined now.
 *     - A type name may be preceded/appended by an integer count, as in "int,4*float" or
 * "int,float[4]".  These are equivalent to "int,float,float,float,float" (just shorter).
 *     - Variable dimension arrays may be defined, IF the size of the array is an int component of
 *this new rowtype which occurs before the array definition:
 *         - "int,float[#1]" defines the second component to be a float array whose length is given
 *by the value of the first component
 *         - This example is analogous to this C fragment:
 *            - typedef struct { int n; float *x; } far ;
 *            - far xfar ;
 *            - xfar.n = 373 ;
 *            - xfar.x = malloc(sizeof(float)*xfar.n) ;
 *         - You can't do something like "int,3*float[#1]" - only one repeat count per component is
 *allowed!  The right way to do this would be "int,float[#1],float[#1],float[#1]".
 *         - All variable components in instances of these arrays will be allocated with NI_malloc()
 *and should be released with NI_free().
 *         - The type of a variable dim array must be fixed in size;
 *           that is, that type itself cannot contain any var dim array or String components.
 *     - String components are also variable dimension arrays, but whose size is given by the usual
 *strlen().
 *
 * The intention is that a C struct type is mapped to a NI_rowtype.  Some examples:
 ...
 *
 * The return value is a positive integer code which can be used to identify this NI_rowtype (the
 *tname string can also be used for this purpose).  If -1 is returned, something bad transpired.
 *
 * Also see
...
int NI_rowtype_define(char *tname, char *tdef)
{
    ...
    if (strlen(tname) > 255)
    {
        ERREX("overlong typename");
    }
    if (tdef == NULL || *tdef == '\0')
    {
        ERREX("empty type definition");
    }
    ...
    if (rt->part_num == 0)
    {
        delete_rowtype(rt); ERREX("no components?");
    }

    /*** now loop over components, breaking them down into their parts, storing the part types and
     * their offsets into the C struct    ***/
    ...
    for (ii = 0 ; ii < rt->comp_num ; ii++)
    {
...

        {
            if (pointer_alignment > 1)          /* make sure cbase */
            {
                ...
                if (jd > 0)
                {
                    cbase += (pointer_alignment - jd);
                }
            }

            /* Note that this is the only case where a part_typ might end up as a derived type -
             * normally, part_typ will be a builtin type code (NI_BYTE .. NI_STRING). Note the
             *limitation that the type of variable dim arrays be a fixed dim type.
             *                        */
...

        {
            ...
            if (kd > almax)
            {
                almax = kd;                            /* keep track of largest alignment */
            }
            ...
            for (jj = 1 ; jj < np ; jj++, id++)
            {
...

                {
                    ...
                    if (kd > 1)       /* must move nn up if    */
                    {
                        ...
                        if (jd > 0)
                        {
                            nn += (kd - jd);       /* of jj bytes alignment */
                        }
                    }
                    rt->part_off[id] = nn;
...

                {
                    ...
                    /* qt->part_dim[jj] is the part index in qt of the dimension for this variable
                     * dim array part;
            ...
            /* now move the base offset up by the size of the current component (which may be bigger
     ...
     * at this point, cbase = next byte offset available after last part;
     * this would be the size, but may have to be pushed up to allow for byte alignment of this
     *rowtype     */
    ...
    rt->size = cbase;  /* this size is the sizeof(struct), and doesn't include var dim arrays or
                        * Strings, just the pointers to those things */

    /* 26 Dec 2002: Compute the sum of the part sizes (zero if this has variable dim arrays). If
     * rt->psiz == rt->size, then struct is stored without padding bytes. */
    ...
    /* 28 Oct 2004: Move assignment of the new rowtype code to the end, since a recursive call via
     * NI_rowtype_find_name() might have created a new rowtype before this one. An example
     * definition: "int,VECTOR_float_32,int".  */
    ...
    if (ROWTYPE_debug)
    {
        ...
        for (ii = 0 ; ii < rt->comp_num ; ii++)
        {
            fprintf(stderr, "%4d ", rt->comp_typ[ii]);
        }
        fprintf(stderr, "\n");
        ...
        for (ii = 0 ; ii < rt->comp_num ; ii++)
        {
            fprintf(stderr, "%4d ", rt->comp_dim[ii]);
        }
        fprintf(stderr, "\n");
        ...
        for (ii = 0 ; ii < rt->part_num ; ii++)
        {
            fprintf(stderr, "%4d ", rt->part_typ[ii]);
        }
        fprintf(stderr, "\n");
        ...
        for (ii = 0 ; ii < rt->part_num ; ii++)
        {
            fprintf(stderr, "%4d ", rt->part_off[ii]);
        }
        fprintf(stderr, "\n");
        ...
        for (ii = 0 ; ii < rt->part_num ; ii++)
        {
            fprintf(stderr, "%4d ", rt->part_siz[ii]);
        }
        fprintf(stderr, "\n");
        ...
        for (ii = 0 ; ii < rt->part_num ; ii++)
        {
            fprintf(stderr, "%4d ", rt->part_dim[ii]);
        }
        fprintf(stderr, "\n");
    }

    /* save this in the table of rowtypes, and return the numerical code for this new type */
 ...
 *  28 Oct 2004: If name is of form VECTOR_basictype_length, then a new rowtype is created on the
 *spot;
...
NI_rowtype * NI_rowtype_find_name(char *nn)
{
    ...
    if (busy)
    {
        return NULL;          /* cannot allow re-entry below here! */
    }
    ii = strlen(nn);
    ...
    { char bt[32], rt[64]; int tt, dd;
      ...
      if (nn[ii] != '_')
      {
          return NULL;                   /* bad end of basic type name */
      }
      ...
      busy = 0;
      if (tt >= ROWTYPE_OFFSET)
      {
          return rowtype_array[tt - ROWTYPE_BASE_CODE];
      }
    }
...
NI_rowtype * NI_rowtype_find_code(int nn)
{
    ...
    if (nn >= ROWTYPE_OFFSET)
    {
        nn = nn - ROWTYPE_BASE_CODE;
    }
    if (nn < 0 || nn >= rowtype_num)
    {
        return NULL;
    }
...
/*! Given a rowtype name, find its integer code. Returns -1 if the name isn't found in the rowtype
 * table.
 * ----------------------------------------------------------------------*/
...
/*! Given a rowtype code, find its string name. Returns NULL if the code isn't found in the rowtype
 * table, otherwise returns the pointer to the name inside the table (i.e., don't free this
 * string!).
 * ----------------------------------------------------------------------*/
...
/*! Given a rowtype code, find its alias name.  This is only valid for builtin types; for
 * user-defined types, the return value is the user-supplied type name string.  Don't free this
 * string!
 * ----------------------------------------------------------------------*/
...
char * NI_rowtype_code_to_alias(int nn)     /* 19 Feb 2003 */
{
    ...
    if (nn <= NI_NUM_BASIC_TYPES)
    {
        return type_alias[nn];
    }
 ...
 *  - Note that if the rowtype contains variable dim arrays, this size returned here is just the
 *size of the storage for the basic struct with pointers, not including the variable arrays.
 *  - See NI_rowtype_vsize() to get the size of a rowtype instance including its variable dim
 *arrays.
 * ----------------------------------------------------------------------*/
...
int NI_rowtype_code_to_size(int dtyp)
{
    ...
    if (dtyp < ROWTYPE_OFFSET)
    {
        return type_size[dtyp];
    }
    if (dtyp == last_dtyp)
    {
        return last_size;
...
/*! Compute the size of all the data in a struct defined in a NI_rowtype (not including padding),
 * for this instance of the struct, allowing for variable array parts.  Zero is returned if
...
int NI_rowtype_vsize(NI_rowtype *rt, void *dpt)
{
    ...
    if (!ROWTYPE_is_varsize(rt))
    {
        return rt->psiz;                          /* fixed dim struct */
    }
    if (dat == NULL)
    {
        return 0;                                 /* var size with no data? */
    }

    /* loop over parts, adding up part sizes, including var dim arrays and String parts */
...
/*! Encode 1 type value at the end of the text string wbuf (which is assumed to be plenty long).
 *  typ must be a fixed dim type code, or NI_STRING.  Structs with var dim arrays must be handled
 *separately.
 * ---------------------------------------------------------------------------*/

static int do_raw = 0;

void NI_set_raw_val_to_text(int ii)
{
    do_raw = ii;
}
...
void NI_val_to_text(NI_rowtype *rt, char *dpt, char *wbuf)
{
    ...
    /* for floating point outputs, first print to a temp string, then clip trailing and leading
     * blanks */
...
/*! Copy 1 fixed dim type (no String or var dim array parts here) value in binary format to the
 * wbuf.
...
int NI_has_String(NI_rowtype *rt)
{
    ...
    if (ROWTYPE_is_builtin_code(rt->code))
    {
        return rt->code == NI_STRING;
    }
    ...
    for (ii = 0 ; ii < rt->part_num ; ii++)
    {
        ...
        {
            if (rt->part_rtp[ii]->code == NI_STRING)
            {
                return 1;
            }
        }
        ...
        {
            if (NI_has_String(rt->part_rtp[ii]))
            {
                return 1;                                  /* recursion */
            }
        }
...
/*! Write one column of structs to the output stream.  Now superseded by NI_write_columns(). This
 * function is not used anywhere (I think).
 * --------------------------------------------------------------------------*/
...
/*! Write "columns" of data to a NI_stream.  Each column is an array of structs of some NI_rowtype
 ...
 *    - return value is number of bytes written to stream (-1 if something bad happened, 0 if can't
 *write to stream yet)
 *
 * Only the data is written to the stream - no header or footer. This function is adapted from the
 *1st edition of NI_write_element().
 * --------------------------------------------------------------------------*/
...
# define FREEUP    do{ NI_free(wbuf); NI_free(bbuf); NI_free(cbuf); \
    ...
    if (col_num <= 0 || col_len <= 0)
    {
        return 0;
    }
    if (col_typ == NULL || col_dat == NULL)
    {
        WCERR("a"); return -1;
    }
    ...
    if (ns->bad)                           /* not connected yet? */
    {
        ...
        if (jj < 1)
        {
            return jj;                   /* 0 is nothing yet, -1 is death */
        }
    }
#if 1
    ...
    {
        WCERR("d"); return jj;
    }                                      /* only exit if stream is actually bad */
#endif
    ...
    for (col = 0 ; col < col_num ; col++)
    {
        ...
        if (tmode != NI_TEXT_MODE && NI_has_String(rt[col]))
        {
            tmode = NI_TEXT_MODE;
        }
    }
     ...
     *   one compact (no padding) fixed-size rowtype, and binary output ==> can write all data
     *direct to stream at once --*/
    ...
    /* this macro take the 'nout' number of output bytes and adds into the running total ntot if all
     * was well;
...
# define ADDOUT(x)                                    \
    ...
    /*-- loop over output rows, format for output into wbuf, and then send to output stream --*/
    ...
    for (row = 0 ; row < col_len ; row++)
    {
        ...
        case NI_BINARY_MODE:  /* jj bytes of binary in wbuf */
            ...
            if (nout != jj)
            {
                NI_dpr("NI_write_columns: col#%d sends %d bytes; nout=%d\n", col, jj, nout);
            }
#endif
            ADDOUT("B");
            break;
        ...
        case NI_BASE64_MODE: { /* convert binary triples into base64 quads */
            ...
            /* bbuf = bb bytes of unprocessed data from last struct plus jj bytes of data from new
             * struct (bb will be 0 or 1 or 2)                     */
            ...
            if (bb > 0)
            {
                ...
                if (bb > 1)
                {
                    bbuf[1] = bbuf[nb3 + 1];    /* to front of bbuf */
                }
            }
        }
        break;
    ...
    /* in Base64 mode, we might have to clean up if there are any leftover bytes in bbuf, or at
     * least write an end of line           */
    ...
    if (tmode == NI_BASE64_MODE)
    {
        if (bb > 0)                 /* num leftover bytes of data */
        {
            ...
            if (bb == 2)
            {
                B64_encode2(a, b, w, x, y, z);
            }
            else
            {
                B64_encode1(a, w, x, y, z);
            }
            cbuf[0] = w; cbuf[1] = x;
...
/*! Read "columns" of data from a NI_stream.  Each column is an array of structs of some NI_rowtype
 * (including the builtin types):
 ...
 *       - if col_dpt[i] isn't NULL, it must point to space big enough to hold the input (col_num *
 * fixed size of rowtype #i)
 ...
 *
 * Only the data is read from the stream - no header or footer. This function is adapted from the
 *1st edition of NI_read_element().
 * --------------------------------------------------------------------------*/
...
# define FREEUP    do{ NI_free(rt); NI_free(vsiz); NI_free(fsiz); } while (0)
    ...
    if (jj < 0)
    {
        return jj;                       /* only exit if stream is actually bad */
    }
    /* create array of NI_rowtype for columns, etc. */
    ...
    /*-- Special (and fast) case:
     *   one compact (no padding) fixed-size rowtype, and binary input ==> can read all data direct
     *from stream at once --*/
...

    {
        ...
        for (nn = 1, ii = 0 ; ii < rt->part_num ; ii++)
        {
...

            {
                ...
                if (swap)
                {
                    NI_swap4(1, &dim);       /* byte-swap dim, which was */
                }
...
{
    if (rt == NULL || nrow <= 0 || dat == NULL)
    {
        return;                                           /* stupid inputs */
    }
    switch (rt->code)
    {
    case NI_RGB:
...
/*! Delete a column of rowtype structs, including any var dim arrays. Assumes everything was
 * allocated with NI_malloc(). After this is called, the cpt argument should be set to NULL.
 * ----------------------------------------------------------------------------*/
...
void * NI_copy_column(NI_rowtype *rt, int col_len, void *cpt)
{
    ...
    /* copy any var dim arrays inside, since the pointers in ndat right now still point to data in
     * dat, but we want ndat to be entirely self-contained!  */
...
/*! Return the length in bytes of a column of data (not counting padding). The pointer to the data
 * is needed since it might contain variable size data (String or vardim arrays).
 * ----------------------------------------------------------------------------*/
