...
{
    if (NI_element_type(nel) != NI_ELEMENT_TYPE || vstr == NULL)
    {
        return;
    }
...
void NI_set_attribute_from_veclab_array(NI_element *nel, char **vec_lab)
{
    ...
    if (NI_element_type(nel) != NI_ELEMENT_TYPE || nel->vec_num == 0)
    {
        return;
    }
 ...
 *  - If the header was "empty" (ended in "/>"), then no vectors will be allocated, and
 *nel->vec_num=0.
 *  - This function is used by NI_read_element() to create the data element after the header has
 *been parsed.
 *  - 27 Mar 2003: modified to allow vec_len=0, indicating vector length to be inferred from amount
 *of data
 * -------------------------------------------------------------------------*/
...
/*! Make an empty group element from parsed header info. The attributes in the header are assigned
 * to the group, and the group parts are initialized to nothing.
 * ---------------------------------------------------------------------------*/
 ...
 *  - The return value is NI_ELEMENT_TYPE, NI_GROUP_TYPE, NI_PROCINS_TYPE, or -1 if the type is
 *anything else or unknowable.
 * -------------------------------------------------------------------------*/
...
int NI_element_type(void *nini)
{
    ...
    {
        return NI_ELEMENT_TYPE;
    }
    if (ngr->type == NI_GROUP_TYPE)
    {
        return NI_GROUP_TYPE;
    }
    if (npi->type == NI_PROCINS_TYPE)
    {
        return NI_PROCINS_TYPE;
    }
...
/*! Return the name of a NI element.  If the input is bad, returns a NULL pointer.  Do not free this
 * pointer!  It points to the name string inside the element struct.
 * -------------------------------------------------------------------------*/
...
NI_element * NI_new_data_element(char *name, int veclen)
{
    ...
    if (name == NULL || name[0] == '\0' || veclen < 0)
    {
        return NULL;
    }
...
/*! Insert a label for column #cc into a data element. Note that if column #cc does not exist,
 * nothing will happen, so this function should be called AFTER NI_add_column(). [11 Sep 2018]
 * -------------------------------------------------------------------------*/

void NI_set_column_label(NI_element *nel, int cc, char *lab)
{
    if (nel == NULL || nel->vec_len <= 0)
    {
        return;
    }
    if (nel->type != NI_ELEMENT_TYPE)
    ...
    if (cc < 0 || cc >= nel->vec_num || lab == NULL)
    {
        return;
    }
    ...
    if (nel->vec_lab == NULL)   /* create empty label array */
    {
        ...
        for (pp = 0 ; pp < nel->vec_num ; pp++)
        {
            nel->vec_lab[pp] = NI_strdup("\0");
        }
    }

    if (nel->vec_lab[cc] != NULL)
    {
        NI_free(nel->vec_lab[cc]);
    }
 ...
 *  - arr = pointer to data values - must be an array of length veclen (same value as used in
 *NI_new_data_element() call)
 *  - if arr is NULL, then will add a zero-filled column of the given type to the data element
 *
 *  The data array is copied into the element.  If the element was specified with veclen=0, then
 *this function will do nothing. Since this function has no return value, the only way to check for
 *such an error is to see if nel->vec_num was incremented.  Or don't be so stupid as to make this
 *error.
 * -------------------------------------------------------------------------*/
...
void NI_add_column(NI_element *nel, int typ, void *arr)
{
    ...
    rt = NI_rowtype_find_code(typ); if (rt == NULL)
    {
        return;
    }
...
/*!
 * Like add_column, but inserts the column at nel->vec[icol] rather than at the end.
...
/*!
 * move a column from index ibefore to iafter if ibefore (or iafter) is (< 0 || > nel->vec_num) then
...
void NI_move_column(NI_element *nel, int ibefore, int iafter)
{
    ...
    nn = nel->vec_num;
    if (ibefore < 0 || ibefore >= nn)
    {
        ibefore = nn - 1;
    }
    if (iafter < 0 || iafter >= nn)
    {
        iafter = nn - 1;
    }
...
/*!
 * Do we really need to document this too?        [[Cox says YES]] Removes column irm from nel. If
 *irm < 0 or irm >= nel->vec_num irm = nel->vec_num -1
 */
 ...
 *   - You can use this to convert an element from empty to non-empty by entering newlen > 0 when
 *the original vec_len is 0.
 *   - But, you cannot use this to convert an element to empty from non-empty, by entering newlen ==
 * 0 when vec_len > 0!
 * --------------------------------------------------------------------------*/
...
void NI_alter_veclen(NI_element *nel, int newlen)
{
    ...
    if (nel->vec_len == 0)
    {
        NI_init_veclen(nel, newlen);                         /* 19 Sep 2008 */
    }
    oldlen = nel->vec_len; if (oldlen == newlen)
    {
        return;
    }
...
/*! As in NI_add_column(), but adding every stride-th element from arr. Thus, arr should be at least
 * nel->vec_len * stride elements long.
 * --------------------------------------------------------------------------*/
...
void NI_add_column_stride(NI_element *nel, int typ, void *arr, int stride)
{
    ...
    if (arr == NULL)
    {
        return;                        /* no input data ==> we're done */
    }
...
/*! ZSS; Fills an already created column with values up to vec_filled the values in arr are inserted
 * into nel->vec[nn]
 * --------------------------------------------------------------------------*/
                           ...
                           void *arr, int nn, int stride)
{
    ...
    if (nn < 0 || nn >= nel->vec_num)
    {
        return;
    }
    if (typ != nel->vec_typ[nn])
    {
        return;
    }
 ...
 *   - NOTE WELL: When the column type is NI_STRING, it is a mistake to call this function with dat
 *being a pointer to the C string to insert.  Instead, dat should be a pointer to the pointer to the
 ...
 *      - The last case illustrates the NI_insert_string() function, which can be used to simplify
 *insertion into a column of Strings; that function is just a simple wrapper to call
 *NI_insert_value() properly.
 *      - The reason the first example is WRONG is that dat is supposed to point to the data to be
 *stored.  In the case of a String, the data is the pointer to the C string.
 * --------------------------------------------------------------------------*/
...
void NI_insert_value(NI_element *nel, int row, int col, void *dat)
{
    ...
    if (row < 0 || row >= nel->vec_len)
    {
        return;
    }
    if (col < 0 || col >= nel->vec_num)
    {
        return;
    }
    ...
    if (ROWTYPE_is_varsize(rt))
    {
...

        {
            if (rt->part_typ[jj] == NI_STRING)                   /* a string part */
            {
                ...
                *apt = qpt;                                      /* reset *apt to copy of input
                                                                  * string */
            }
...
/* Return a float value. If the data type does not pass NI_IS_NUMERIC_TYPE, or if any other thing is
 * non-copasetic, then return value is 0.0f.
 *//*----------------------------------------------------------------------*/
...
float NI_extract_float_value(NI_element *nel, int row, int col)
{
    ...
    if (!NI_IS_NUMERIC_TYPE(nel->vec_typ[col]))
    {
        return 0.0f;
    }
...
/* Return a malloc()-ed string value from one value in a data element. Adapted from
 * NI_write_columns(). Result should be free()-ed when you are done with it.
...
/*! Remove an attribute, if it exists from a data or group element. ZSS Feb 09
* --------------------------------------------------------------------------*/
...
void NI_kill_attribute(void *nini, char *attname)
{
    ...
    if (tt < 0 || attname == NULL || attname[0] == '\0')
    {
        return;
    }
    ...
    if (tt == NI_ELEMENT_TYPE)
    {
        ...
        {
            if (strcmp(nel->attr_lhs[nn], attname) == 0)
            {
                break;
            }
        }
    ...
    else if (tt == NI_GROUP_TYPE)
    {
        ...
        {
            if (strcmp(ngr->attr_lhs[nn], attname) == 0)
            {
                break;
            }
        }
    ...
    else if (tt == NI_PROCINS_TYPE)
    {
        ...
        {
            if (strcmp(npi->attr_lhs[nn], attname) == 0)
            {
                break;
            }
        }
...
/*! Add an attribute to a data or group element. If an attribute with the same attname already
 * exists, then it will be replaced with this one.
 * --------------------------------------------------------------------------*/
...
/*! Copy all attributes from one element to the next. The function is not recursive and must have a
 * target nel with no attributes.    ZSS Oct 2010
 * --------------------------------------------------------------------------*/
...
void NI_copy_all_attributes(void *nisrc, void *nitrg)
{
    ...
    if (ttsrc == NI_ELEMENT_TYPE)
    {
        ...
        {
            /* don't allow for this now. Else, you need to clear all pre-existing ones, or just
             * common pre-existing ones then continue below */
            fprintf(stderr, "Error NI_copy_all_attributes:\n"
...
/*! Duplicate a niml group or element. Function tested with test program niccc and -dup option  ZSS
 * Oct 2010
 * --------------------------------------------------------------------------*/
...
/*! Duplicate niml element only. Better use function NI_duplicate                     ZSS Oct 2010
 * --------------------------------------------------------------------------*/
...
NI_element * NI_extract_columns(NI_element *nel, int nc, int *cc)
{
    ...
    for (ii = 0 ; ii < nc ; ii++)
    {
        jj = cc[ii]; if (jj < 0 || jj >= nel->vec_num)
        {
            continue;
        }
...
/*! Duplicate niml group only. Better use function NI_duplicate                        ZSS Oct 2010
 * --------------------------------------------------------------------------*/
...
/*! Get an attribute with the given LHS name.  Returns a pointer to the RHS field in the element if
 * the attribute name is found; otherwise returns NULL.  If the LHS is found, but the RHS is NULL,
 * returns a pointer to an empty C string ("\0").  Do not free() the result from this function,
 * since it points to the internal field of the element!
 * -------------------------------------------------------------------------*/
                                ...
                                int (*STRcomp)(const char *, const char *))
{
    ...
    if (tt == NI_ELEMENT_TYPE)
    {
        ...
        for (nn = 0 ; nn < nel->attr_num ; nn++)
        {
            if (STRcomp(nel->attr_lhs[nn], attname) == 0)
            {
                break;
            }
        }
    ...
    else if (tt == NI_GROUP_TYPE)
    {
        ...
        for (nn = 0 ; nn < ngr->attr_num ; nn++)
        {
            if (STRcomp(ngr->attr_lhs[nn], attname) == 0)
            {
                break;
            }
        }
    ...
    else if (tt == NI_PROCINS_TYPE)
    {
        ...
        for (nn = 0 ; nn < npi->attr_num ; nn++)
        {
            if (STRcomp(npi->attr_lhs[nn], attname) == 0)
            {
                break;
            }
        }
...
/*! Set the delta attribute for a data element. Do not call this function until NI_set_dimen() has
 * been called, unless there is only 1 dimension (which is the default).
 * -------------------------------------------------------------------------*/

void NI_set_delta(NI_element *nel, float *del)
{
    if (nel == NULL || nel->type != NI_ELEMENT_TYPE ||
        nel->vec_rank < 1 || del == NULL)
    {
        return;
    }
...
/*! Set the origin attribute for a data element. Do not call this function until NI_set_dimen() has
 * been called, unless there is only 1 dimension (which is the default).
 * -------------------------------------------------------------------------*/
...
/*! Set the units attribute for a data element. Do not call this function until NI_set_dimen() has
 * been called, unless there is only 1 dimension (which is the default).
 * -------------------------------------------------------------------------*/
...
/*! Set the axes attribute for a data element. Do not call this function until NI_set_dimen() has
 * been called, unless there is only 1 dimension (which is the default).
 * -------------------------------------------------------------------------*/
...
void NI_add_to_group(NI_group *ngr, void *nini)
{
    ...
    if (ngr == NULL || ngr->type != NI_GROUP_TYPE || tt < 0)
    {
        return;
    }
...
void NI_remove_from_group(NI_group *ngr, void *nini)     /* 16 Apr 2005 */
{
    ...
    if (ngr == NULL || ngr->type != NI_GROUP_TYPE || nini == NULL)
    {
        return;
    }
...
{
    if (ngr == NULL || ngr->type != NI_GROUP_TYPE)
    {
        return;
    }
    NI_free(ngr->name);
 ...
 *    - This is a 'shallow' search: if the group itself contains groups, these sub-groups are not
 *searched.
 *    - Return value of function is number of elements found (might be 0).
 *    - If something is found, then *nipt is an array of 'void *', each of which points to a
 *matching element.
...
/*! Return a list of all elements in a group that have a given name.
 *    - This is a 'deep' search: if the group itself contains groups, these sub-groups are searched,
 *etc.
 *    - If a group element has the name 'enam' AND a data element within that group has the name
 * 'enam' as well, they will BOTH be returned in this list.
 *    - Return value of function is number of elements found (might be 0).
 *    - If something is found, then *nipt is an array of 'void *', each of which points to a
 *matching element.
 *    - The returned elements might be group or data elements.
...
/*! add a ni_type attribute to the element            14 Jul 2006 [rickr] (based on
 * NI_write_element())
 * -------------------------------------------------------------------------*/
    ...
    for (prev = -1, ii = 0; ii < nel->vec_num; ii++)
    {
        if (nel->vec_typ[ii] != prev)   /* not the previous type */
        {
            if (prev >= 0)              /* apply previous type now */
            {
                ...
                if (count > 1)
                {
                    sprintf(posn, "%d*%s,", count, NI_type_name(prev));
                }
                else
                {
                    sprintf(posn, "%s,", NI_type_name(prev));
                }
            }
...
/* Return a preview string for the top of a data element [22 Jun 2020]. This string should be
 * free()-ed after you are done with it.
 *//*---------------------------------------------------------------------*/
