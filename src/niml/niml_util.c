...
void NI_dpr(char *fmt, ...)         /* print debug stuff */
{
    ...
    if (nms != NULL)
    {
        fprintf(dfp, "     NI_malloc status: %s\n", nms);
    }
...
void NI_sleep(int msec)
{
    ...
    if (msec <= 0)
    {
        return;                         /* can't wait into the past */
    }
    tv.tv_sec  = msec / 1000;
 ...
 *
 *  Note this will overflow an int after 24+ days.  You probably don't want to use this if the
 *program will be running continuously for such a long time.
 * -----------------------------------------------------------------*/
...
int string_index(char *targ, int nstr, char *str[])
{
    ...
    for (ii = 0 ; ii < nstr ; ii++)
    {
        if (str[ii] != NULL && strcmp(str[ii], targ) == 0)
        {
            return ii;
        }
    }
...
/*! Un-escape a C string inplace.  (This can be done since the replacement is always smaller than
 * the input.)  Escapes recognized are:
 *    -  &lt;   ->  <
 *    -  &gt;   ->  >
 *    -  &quot; ->  "
 *    -  &apos; ->  '
 *    -  &amp;  ->  &
 *  Also replace CR LF pair (Microsoft), or CR alone (Macintosh) with LF (Unix), per the XML
 *standard. Return value is number of replacements made.
 * --------------------------------------------------------------------------*/
...
int unescape_inplace(char *str)
{
...

    {
...

        {
            ...
            /* although the comments above don't mention it, we also look for XML style numeric
             * escapes of the forms &#32; (decimal) and &#xfd; (hex) */
        ...
        /* at this point, ii=index of last character used up in scan jj=index of last character
         * written to (jj <= ii) */
...
/*! Quotize (and escapize) one string, returning a new string. Approximately speaking, this is the
 * inverse of unescape_inplace().
 * --------------------------------------------------------------------------*/
...
char * quotize_string(char *str)
{
    ...
    if (lstr == 0)
    {
        out = NI_malloc(char, 4); strcpy(out, "\"\""); return out;
    }
...
/*! Quotize an array of strings into one string, separating substrings with sep (setting sep=0 means
 * use commas).
 * --------------------------------------------------------------------------*/
...
/*! Check a string for 'nameness' - that is, consists only of legal characters for a NIML 'Name' and
 * also starts with an alphabetic character.  Returns 1 if it is a Name and 0 if is not.
 * --------------------------------------------------------------------------*/
...
int NI_is_name(char *str)
{
    ...
    if (str == NULL || str[0] == '\0' || !isalpha(str[0]))
    {
        return 0;
    }
    ...
    {
        if (!IS_NAME_CHAR(str[ii]))
        {
            return 0;                        /* this one is bad */
        }
    }
 ...
 *   - the lev=2 trailing name is "author/of/NIML", and so on. That is, "lev" is the number of
...
char * NI_suck_file(char *fname)
{
    ...
    while (nbuf < MAXBUF)
    {
        ...
        nbuf += ii;                                                     /* add in how many bytes we
                                                                         * now have */
