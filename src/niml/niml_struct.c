...
/* This macro does the basic stuff necessary to delete a struct from the hash table and from memory.
 * It is used at the very end of the NI_free_struct() function. Type specific code is also needed to
 * delete any memory used by sub-structs or sub-arrays.
 * -------------------------------------------------------------*/
...
/*! Decrement the reference counter, and destroy the struct (recursively in some cases) if the
...
/* This macro copies the basic elements of a struct, from struct qold to struct qnew.  Of course,
 * the new struct gets a new idcode.  This macro may be used after creating a new struct with
 * NI_new(), for example.
 * -------------------------------------------------------------*/
...
/* This macro makes a new struct of type TTYPE, copies the basic elements, and points ndnew to the
 * new struct, for eventual return from NI_copy_struct().  This macro is used only in that function.
 * -------------------------------------------------------------*/
