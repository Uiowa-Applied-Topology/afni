                                     ...
                                     char *file, int msec)
{
    ...
    if (ii <= 0)
    {
        FAILED; NI_stream_close(ns); return NULL;
    }
...
static int read_URL_http(char *url, int msec, char **data)
{
    ...
    if (!cflag)
    {
        buf = (char *)malloc(QBUF); nall = QBUF;
    }
    nuse = 0; first = 1;
...
void NI_set_URL_ftp_ident(char *name, char *pwd)
{
    ...
    ll = strlen(name); if (ll < 1 || ll > 511)
    {
        return;
    }
    ll = strlen(pwd); if (ll < 1 || ll > 511)
    {
        return;
    }
 ...
 * however, staging is always done through a temporary file. This function works simply by
 *creating/running a script file to run the command line ftp program.  Clumsy, but simpler than
...
static int read_URL_ftp(char *url, char **data)
{
    ...
    *h = '\0'; if (hostname[0] == '\0')
    ...
    buf = (char *)malloc(nuse); if (buf == NULL)
    {
        unlink(qname); return -1;
    }
