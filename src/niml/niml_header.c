 ...
 *
 *  Return value is an intpair with the .i component indicating the start position of the string in
 *the data and the .j indicating the byte AFTER the end of the string.  If the .i component is
...
intpair find_string(int nst, int nch, char *ch)
{
    ...
    if (nst >= nch || nch < 2 || ch == NULL)
    {
        return ans;                                             /* bad input */
    }
    for (ii = nst; ii < nch && !IS_STRING_CHAR(ch[ii]); ii++)
    {
        ;                                                   /* skip to start */
    }
    if (ii >= nch)
    ...
    if (IS_QUOTE_CHAR(ch[ii]))                              /* quoted string */
    {
        ...
        quot = ch[ii]; ii++;
        for (jj = ii ; jj < nch && ch[jj] != quot ; jj++)
        {
            ;                                               /* skip to close */
        }
    }
    ...
    {
        for (jj = ii + 1 ; jj < nch && IS_STRING_CHAR(ch[jj]) ; jj++)
        {
            ;                                                    /* to blank */
        }
    }
 ...
 *  if NULL is returned, something real bad happened (and *nused won't be assigned).
 * ----------------------------------------------------------------------------*/
...
header_stuff * parse_header_stuff(int ndat, char *dat, int *nused)
{
    ...
    id = ss.j; if (IS_QUOTE_CHAR(dat[id]))
    {
        id++;
    }
    ...
    while (1)
    {
        ...
        for ( ; id < ndat && SKIPABL(dat[id]) ; id++)
        {
            ;                                         /* skip blanks */
        }
        if (id >= ndat)
        {
            break;                             /* end of input found */
        }
        if (dat[id] == '>')
        {
            break;                                 /* ">" end found */
        ...
        if (IS_QUOTE_CHAR(dat[id]))
        {
            id++;                                   /* skip close quote */
        }
        while (id < ndat && SKIPABL(dat[id]))
        {
            id++;                                   /* skip blanks */
        }
        if (id >= ndat)
...
/*! Decode a single type field.  Return value is an intpair with the .i component being the type
 * code and the .j component being the number of characters consumed.  If the .i component is -1,
 * then no legal type was found (.j is still the number of chars used in the scan).
 * ----------------------------------------------------------------------*/
...
intpair decode_type_field(char *tf)
{
    ...
    if (tf == NULL || !isalpha(*tf))
    {
        return ans; /* prima facie bad */
    }
#if 1               /*** The new way! Look for full names, not initials - RWCcox ***/
    /* copy input into local string, as long as 'name' characters are found, then get the integer
     * code for this type name */

    for (jj = 0 ; jj < 255 && IS_NAME_CHAR(tf[jj]) ; jj++)
    {
        tname[jj] = tf[jj];
    }
    tname[jj] = '\0';
    ...
    /* check if tf[0] starts a full datum name, or if it is just an initial              */
...
/*! Decode a single string into a bunch of strings, separated by characters from the list in sep or
 * by a space.
...
NI_str_array * NI_decode_string_list(char *ss, char *sep)
{
    ...
    while (id < lss)
    {
        ...
        while (id < lss && strchr(sep, ss[id]) == NULL && !isspace(ss[id]))
        {
            id++;
        }
        if (id == jd)
...
/*! Like NI_decode_string_list, but will not use space as a delimiter ZSS Dec 2010
 * ----------------------------------------------------------------------*/
...
/*! Decode a string that gives a list of floats [10 Jun 2007]. Modified to allow ranges specified by
 * a/b/c [06 Mar 2019].
 *//*------------------------------------------------------------------*/
...
char * NI_encode_float_list(NI_float_array *far, char *sep)
{
...

    {
        ...
        for (ii = jj + 1 ; ii < num && ar[ii] == val ; ii++)
        {
            ;                                    /*nada*/
        }
        ...
        if (jj < num)
        {
            sprintf(car + strlen(car), "%c", cc);      /* add the separator */
        }
    }
...
NI_int_array * NI_decode_int_list(char *ss, char *sep)
{
    ...
    for (jj = 0 ; jj < sar->num ; jj++)
    {
        ...
        for (aa = vv, ii = 0 ; ii < nadd ; ii++, aa += da)
        {
            ar[num++] = aa;
        }
    }
...
char * NI_encode_int_list(NI_int_array *iar, char *sep)
{
    ...
    for (jj = 0 ; jj < num ; )
    {
        ...
        for (ii = jj + 1 ; ii < num && ar[ii] - ar[ii - 1] == 0 ; ii++)
        {
            ;                                     /*nada*/
        }
        if (ii > jj + 1)                          /* encode values [jj..ii-1] */
        ...
        {
            for (ii = jj + 1 ; ii < num && ar[ii] - ar[ii - 1] == 1 ; ii++)
            {
                ;                                               /*nada*/
            }
            if (ii == jj + 1)
        ...
        if (jj < num)
        {
            sprintf(car + strlen(car), "%s%c", fbuf, cc);
        }
        else
        {
            sprintf(car + strlen(car), "%s", fbuf);
        }
    }
...
/*! Decode a data type string into an array of integer codes. Returns NULL if the input is bad bad
 * bad.
 * ----------------------------------------------------------------------*/
