...
/*! Set the length of a line of output in base64; ll should be between 16 and 76 (inclusive). Will
...
void B64_to_binary(int nb64, byte *b64, int *nbin, byte **bin)
{
    ...
    *bin = (byte *)malloc(sizeof(byte) * (2 + 3 * nb64 / 4));
    ...
    {
        w = b64[ii++];
        while (!B64_goodchar(w) && ii < nb64)
        {
            w = b64[ii++];
        }
        if (w == '=')
        {
            break;                           /* premature end of input */
        }
        x = (ii < nb64) ? b64[ii++] : '=';
        while (!B64_goodchar(x) && ii < nb64)
        {
            x = b64[ii++];
        }
        y = (ii < nb64) ? b64[ii++] : '=';
        while (!B64_goodchar(y) && ii < nb64)
        {
            y = b64[ii++];
        }
        z = (ii < nb64) ? b64[ii++] : '=';
        while (!B64_goodchar(z) && ii < nb64)
        {
            z = b64[ii++];
        }
 ...
 *
 * Outputs: *nb64 = number of base64 bytes [*nb64==0 flags an error] b64 = pointer to newly
 *malloc()-ed space with bytes
 *
 * The output array (*b64) line length can be set by B64_set_linelen(n) where n is from 16 to 76.
 *The default is 72.  Note, however, that encoded bytes will always be written out in groups of 4.
 *The output array line separator can be the LF character only (Unix) or the CR-LF combination (DOS,
 *etc.).  This is controlled by B64_set_crlf(n) where n=1 for LF, n=2 for CR LF.  The default is LF.
 *The output array will be terminated with a line separator.  If you call B64_set_crlf(0) then this
 *will turn off the use of line separators. B64_set_crlf(-1) or (1) or (2) will turn line separators
 *back on.
 *
 * There will be an ASCII NUL character at the end of *b64 (i.e., it is a C string), but this
 *character is not included in the count nb64.
 *
 * Example:
 *   -  byte *b64 , *bin ;
 *   -  int  nb64=0 , nbin ;
...
void B64_to_base64(int nbin, byte *bin, int *nb64, byte **b64)
{
    ...
    if (nb64 == NULL || b64 == NULL)
    {
        return;
    }
    if (nbin <= 0 || bin == NULL)
    {
        *nb64 = 0; *b64 = NULL; return;
    }
