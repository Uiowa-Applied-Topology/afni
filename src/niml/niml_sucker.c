...
/*! Return a pointer to the idcode of a NIML element (group or data), if it has one.  Otherwise,
 * return NULL.  Do not modify or free() this string, since it points into the NIML element struct.
 * -----------------------------------------------------------------------------*/
...
void NI_suck_stream(char *sname, int msec, int *ndc, NI_objcontainer ***dc)
{
    ...
    if (ndc == NULL)
    {
        return;                /* clueless caller */
    }
    *ndc = 0;                  /* number of objects found thus far */
    if (dc == NULL)
    {
        return;                /* stupid caller */
    }
                               ...
                               NI_objconverter_func obj_to_elm)
{
    ...
    if (self_name == NULL || *self_name == '\0')
    {
        return;
    }
    if (elm_to_obj == (NI_objconverter_func)NULL)
    {
        return;
    }
    ...
    {
        if (strcmp(converters[cc].self_name, self_name) == 0)
        {
            break;
        }
    }
...
void NI_convert_elm_to_obj(NI_objcontainer *dc)
{
    ...
    if (strcmp(dc->type_name, "NI_ELEMENT") != 0 &&
        strcmp(dc->type_name, "NI_GROUP") != 0)
    {
        return;
    }
    ...
    for (cc = 0 ; cc < num_converters ; cc++)
    {
        if (strcmp(converters[cc].self_name, dc->self_name) == 0)
        {
            break;
        }
    }
