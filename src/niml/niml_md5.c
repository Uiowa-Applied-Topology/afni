...
/* #define USE_XOR ZSS: Apr. 15 2015 I have no idea at the moment why USE_XOR ends up causing SUMA
 * labels to get written on top of each other in the viewer. This happens on mac and linux versions.
 * This definition might also be causing other mysterious crashes in SUMA when opening the surface
 * controller as reported on the message borad. For now, it will stay off until we sort out why this
 * is causing grief. Valgrind did not point to anything fishy related to USE_XOR.
 */
#ifdef  USE_XOR
# define               XOR_NUM    1024
...
/*! Digest an array and returns the printable string of the result, stored in a static array
 * (length=32+1 bytes).
 * ------------------------------------------------------------------------*/
...
/*! Digest an array and returns the printable string of the result, stored in a malloc()-ed array
 * (length=32+1 bytes).
 * ------------------------------------------------------------------------*/
...
/*! Digest a C string and returns the printable string of the result, stored in a static array
 * (length=32+1 bytes).
 * ------------------------------------------------------------------------*/
...
/*! Digest a C string and returns the printable string of the result, stored in a malloc()-ed array
 * (length=32+1 bytes).
 * ------------------------------------------------------------------------*/
 ...
 * strlen(result) is 22 instead of 32 result is malloc()-ed and should be free()-d when appropriate
 * ------------------------------------------------------------------------------*/
...
/*! Return the MD5 hash of an array as a Base64 string, instead of a hex string.
 * strlen(result) is 22 instead of 32 result is malloc()-ed and should be free()-d when appropriate
 * ------------------------------------------------------------------------------*/
...
/*! Return the MD5 hash of a C string as a Base64 string, instead of a hex string.
* strlen(result) is 22 instead of 32 result is malloc()-ed and should be free()-d when appropriate
* ------------------------------------------------------------------------------*/
