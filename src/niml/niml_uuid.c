...
/*! Return a globally unique string (I hope).  This can be hashed to produce a unique idcode (cf.
 * UNIQ_idcode and UUID_idcode).
 *
 * Method: Generate a string from the system identifier information and the current time of day. The
 *output string is malloc()-ed, and should be free()-ed when no longer needed.
 *
...
#define NURR    32                     /* # bytes from /dev/urandom at a time */
    ...
    /* even if called twice in very rapid succession, at least ncall will differ, so we'll get
     * different ID codes  */
...
/*! Return a globally unique identifier (I hope).  This is a malloc()-ed string of length <= 31
 * (plus the NUL byte; the whole thing will fit into a char[32] array).  The output does not contain
 * any '/'s, so it could be used as a temporary filename.  Repeated calls to this function should
 * never return the same string.
 *
 * Method: Generate a string from the system identifier information and the current time of day. MD5
 *hash this to a 128 byte code. Base64 encode this to a 22 byte string. Replace '/' with '-'
 *        and '+' with '_'. Add 4 character prefix (1st 3 characters of environment variable
 *IDCODE_PREFIX plus '_').
 *
...
/*! Return the 3 character idcode prefix and cap with '\0', environment variable IDCODE_PREFIX is
 * not expected to change within a session.
 *//*--------------------------------------------------------------------*/
...
/*! Make an idcode-formatted malloc-ed string from an input string. Unlike UNIQ_idcode(), this will
...
/*! Fill a user-supplied buffer (length at least 32) with an idcode. That is, idc should point to a
 * char array of length 32 (or more).
 *//*--------------------------------------------------------------------*/
...
/*! Hash a string and return a malloc()-ed string (36+1 bytes) in the
...
/*! Hash a unique string and return a malloc()-ed string (36+1 bytes) in the
 * "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" format. The result should be unique worldwide, for all
 *time.
 *//*--------------------------------------------------------------------*/
