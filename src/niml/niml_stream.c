 ...
 *
 *  This list is needed so we can deal with the SIGURG signal, which we use as a message to shut a
 *socket down.  The signal call itself doesn't tell us which socket was the trigger, so we have to
 *search all the open sockets for a match:
...
static void remove_open_stream(NI_stream_type *ns)
{
    ...
    if (doing_atexit || nn <= 0 || ns == NULL)
    {
        return;                          /* bad input */
    }
    for (ii = 0 ; ii < nn ; ii++)        /* find input */
    {
        if (open_streams[ii] == ns)
        {
            break;
        }
    }
...
/*! Signal handler for SIGURG -- for incoming OOB data on a socket. We just close the NI_stream that
 * the socket is attached to. But first we have to find it!
 * --------------------------------------------------------------------*/
...
static void tcp_sigurg_handler(int sig)
{
    ...
    if (sig != SIGURG ||
        busy ||
        num_open_streams <= 0 || open_streams == NULL)
    {
        return; /* bad */
    }
    busy = 1;   /* prevent recursion! */

    /* find largest socket descriptor in list of streams, and make list of all open socket
     * descriptors in streams */
    ...
    for (ii = 0 ; ii < nn ; ii++)
    {
            ...
            open_streams[ii]->sd >= 0)
        {
            ...
            if (open_streams[ii]->sd > sdtop)
            {
                sdtop = open_streams[ii]->sd;
            }
        }
    }
    if (sdtop < 0)
...
/********************************************************************
 * Routines to manipulate TCP/IP stream sockets. See
 *http://www.manualy.sk/sock-faq/unix-socket-faq.html for info.
 * [PT: Mar 25, 2023] NB: the above link does not currently resolve. From searching online, perhaps
 *this is the modern form (or at least something analogously useful), which appears to exist in a
...
/*! Set a socket so that it will cutoff quickly when it is closed. See
 * http://www.manually.sk/sock-faq/unix-socket-faq.html for more information about this stuff.
 * --------------------------------------------------------------------------*/
...
#ifdef SO_REUSEADDR
    /* Let the address be reused quickly, in case of another connection from the same host on the
     * same port. */
 ...
 *
 * If it is dead, then readcheck will say we can read, but we won't actually get any bytes when we
 *try (using peek mode). Returns 1 if things are OK, 0 if not.
 * ---------------------------------------------------------------------*/
...
static int tcp_alivecheck(int sd)
{
    ...
    if (ii == 0)
    {
        return 1;                             /* can't read is OK     */
    }
    if (ii < 0)
    {
        return 0;                             /* some error is bad    */
    }
    errno = 0;
    ii    = tcp_recv(sd, bbb, 1, MSG_PEEK);   /* try to read one byte */
    if (ii == 1)
    {
        return 1;                             /* if we get it, good   */
    }
    if (errno)
    {
        PERROR("tcp_alivecheck");
    }
 ...
 *
 *   This function is used to "reach out" to a server that is supposed to be listening on the same
 *port. Returns socket id; if -1, some error occurred (e.g., nobody listening).
 * --------------------------------------------------------------------------*/
...
static int tcp_connect(char *host, int port)
{
    ...
    if (sd == -1)
    {
        PERROR("tcp_connect(socket)"); return -1;
    }
 ...
 *
 * This is intended for use by a server, which will wait for some other program to actively connect
 *to this port.  There is no security here -
 * connections will be taken from any IP address.
 *
 * This function does not actually form the connection.  That must be done separately.  Whether
 *someone is trying to connect can be checked for with the routine "tcp_readcheck" and then accepted
 *with "tcp_accept".
 *
 ...
 *
 * Return value is the attached socket (which is not the original socket!). If -1 is returned, some
 *error occurred.  If the accept works, then the original socket is still open and listening for
 *further attachments. Under many circumstances, you will want to close the original socket
 *immediately.  This can be done with CLOSEDOWN(sd), where sd is the input socket.
 *
 * If hostname is not NULL, then the char * it points to will be filled with a pointer to the
 *official name of the host that connected.
 *
 * If hostaddr is not NULL, then the char * it points to will be filled with a pointer to the
 *Internet address (in 'dot' form) of the host that connected.
 *
 * Both the char * pointers returned are from NI_malloc(), and should be NI_free()-d when no longer
 *needed.  If they aren't needed at all, just pass in NULL for these arguments.
 *
 * Note that this routine will block until somebody connects.  You can use tcp_readcheck(sd,0) to
 *see if anyone is waiting to connect before calling this routine.
 *
 * However, if someone connects and the IP address isn't on the trusted list, then the connection
...
static int tcp_accept(int sd, char **hostname, char **hostaddr)
{
    ...
    if (hostname != NULL)
    {
        ...
        if (hostp != NULL)
        {
            *hostname = NI_strdup(hostp->h_name);
        }
        else
        {
            *hostname = NI_strdup("UNKNOWN");                /* bad lookup */
        }
    }
...
/*! Return the Internet address (in 'dot' format, as a string) given the name of the host.  If NULL
 * is returned, some error occurrrrred.  The string is NI_malloc()-ed, and should be NI_free()-ed
...
char * NI_hostname_to_inet(char *host)
{
    ...
    hostp = gethostbyname(host); if (hostp == NULL)
    {
        return NULL;
    }
...
/*! Externally callable routine to add a host to the trusted list. If call with NULL, will just
 * initialize the default trusted host list.
 * ----------------------------------------------------------------------------*/
...
int NI_trust_host(char *hostid)
{
    ...
    /* if the trusted list is empty, see if we want to be completely trusting;
    ...
    /* to be trusted, hostid must start with same string as something in the trusted host_list array
     */
...
/****************************************************************
 * Routines to manipulate IPC shared memory segments for I/O [adapted from thd_iochan.c, 31 May 2002
 * -- RWCox]
 *****************************************************************/

/*---------------------------------------------------------------*/

/*!  Convert a string to a key, for IPC operations. Augment sum by port offset value (-np option)
 * -----------------------------------------------------------------*/
...
/*!  Connect to, or create if needed, a shmem segment. Returns the shmid >= 0 if successful; returns
 * -1 if failure.
 * -----------------------------------------------------------------*/
...
/*! Actually attach to the shmem segment. Returns the pointer to the segment start. NULL is returned
 * if an error occurs.
 * -----------------------------------------------------------------*/
...
/*! Create a SHMioc struct for use as a 2-way I/O channel, and return a pointer to it.  NULL is
 * returned if an error occurs.
 *
 * name = "shm:name:size1+size2" to connect a shared memory segment with buffers of length size1 and
 *size2 bytes. The creator process will write to the size1 buffer and read from the size2 buffer.
 *The acceptor process will reverse this.
 *       - The size strings can end in 'K' to multiply by 1024, or end in 'M' to multiply by
 *1024*1024.
...
static SHMioc * SHM_init(char *name, char *mode)
{
    ...
    if (do_create && name[ii] == ':' && name[ii + 1] != '\0')
    {
        ...
        else
        {
            ...
            else if (*kend == 'M' || *kend == 'm')
            {
                size1 *= 1024 * 1024; kend++;
            }
        }
...
/*! Check if the shmem segment is alive (has 2 attached processes). Returns 0 if not alive, 1 if
 * life is happy.
 * ---------------------------------------------------------------------------*/
...
/*! Check if the given SHMioc is ready for I/O.  If not, wait up to msec milliseconds to establish
...
static int SHM_goodcheck(SHMioc *ioc, int msec)
{
    ...
    if (ioc == NULL || ioc->bad == SHM_IS_DEAD)
    {
        return -1;
    }
    ...
    if (ioc->bad == SHM_WAIT_CREATE)
    {
        ...
        if (msec < 0)
        {
            msec = 999999999;                 /* a long time (11+ days) */
        }
...
/*! Check if the SHMioc is ready to have data read out of it. If not, the routine will wait up to
 * msec milliseconds for data to be available.  If msec < 0, this routine will wait indefinitely.
 * For shmem segments, the return value is how many bytes can be read (0 if none are available).
...
static int SHM_readcheck(SHMioc *ioc, int msec)
{
    ...
    if (ct - ioc->goodcheck_time > 2)     /* 23 Nov 2004 */
    {
        ...
        if (ii <= 0)                       /* not good yet */
        {
            ...
            if (ii <= 0)
            {
                return ii;                /* if still not good, exit */
            }
        }
    }
    else if (ioc->bad)
    ...
    for (ms = 0 ; ms < msec ; ms += dms)
    {
        ...
        dms = NEXTDMS(dms); dms = MIN(dms, msec - ms); NI_sleep(dms);
        ii  = SHM_goodcheck(ioc, 0); if (ii == -1)
        {
            return -1;
        }
    }
...
/*! Check if the SHMioc is ready to have data written into it. If not, the routine will wait up to
 * msec milliseconds for writing to be allowable.  If msec < 0, this routine will wait indefinitely.
 * The return value is the number of bytes that can be sent (0 if none, positive if some). -1 will
 * be returned if some unrecoverable error is detected.
 * -----------------------------------------------------------------------------*/
...
/*! Send nbytes of data from buffer down the SHMioc.  Return value is the number of bytes actually
 * sent, or is -1 if some error occurs.
 * ------------------------------------------------------------------------------*/
...
static int SHM_send(SHMioc *ioc, char *buffer, int nbytes)
{
    ...
    if (nwrite > nbytes)
    {
        nwrite = nbytes;                             /* how much to write */
    }
    ebot = *bend + 1; if (ebot >= bsize)
    {
        ebot = 0;                                    /* start at ebot */
    }
...
/*! Send (exactly) nbytes of data from the buffer down the SHMioc.  The only difference between this
 * and SHM_send is that this function will not return until all the data is sent, even if it takes
 * forever. Under these circumstances, it would be good if the reader process is still working.
 * ------------------------------------------------------------------------------*/
...
/*! Read up to nbytes of data from the SHMioc, into buffer.  Returns the number of bytes actually
 * read. This may be less than nbytes (may even be 0).  If an error occurs, -1 is returned.
 * ------------------------------------------------------------------------------*/
 ...
 * - Buffered input is also available using NI_stream_readbuf() to read data from an internal
 *buffer.  The advantage of buffered input is that it will return the number of bytes requested
 *(waiting, if needed), rather than just the number available at that moment.
 *
 * Several different types of streams are available.  The first two ("tcp:" and "shm:") are for
 *2-way interprocess communication. The later ones ("file:", "str:", "http:", "ftp:", and "fd:") are
 *for 1-way communication, either to read bytes or to write them. The formats for the "name"
 * input are described below:
 *
 * name = "tcp:host:port" to connect a socket to system "host"
 *           on the given port number.  One process should open in "w" mode and one in "r" mode.
 *
 * name = "shm:keyname:size1+size2" to connect to a shared memory segment created with "keyname" for
 *the ID and with I/O buffer sizes of size1 ("w" process to "r" process) and size2 ("r" process to
 * "w" process).
 ...
 *       - The total size of the shared memory segment will be size1+size2+36 bytes.  (Some systems
 *put an upper limit on this size.)
 *       - "keyname" is a string used to identify this shared memory segment
 *       - If you are communicating a lot of data between 2 processes on the same system, shm:
 * streams are usually much faster than tcp: streams.
 *       - Also see NI_stream_reopen() for a way to open a tcp: stream and then re-open it to
 *another tcp: port or to be a shm: stream.
 *
 * name = "file:filename" to open a file for I/O.
 *       - For this type of name ONLY, you can use "a" as the mode string to indicate that you want
 *to append to the file if it already exists.
 *
 * name = "str:" to read/write data from/to a string
 *
 * name = "http://hostname/filename" to read data from a Web server name = "ftp://hostname/filename"
 *  to read data from an FTP server
 *         - The data for these types is transferred all at once from the remote server and stored
 *in a memory buffer (much like the str: stream type).
 *         - Data can be read from this buffer using NI_stream_read().
 *         - When the stream is closed, the buffer is NI_free()-ed.
 *
 * name = "fd:integer" to read or write data from a pre-opened file descriptor (returned by the
 *open() function).
 ...
 *         - All other descriptors use fdopen() to open a FILE stream and then treat the result like
 *file:.  This means that if the descriptor comes from fileno() on a previously opened FILE stream,
 *you will have trouble if you mix I/O to this stream with NI_stream_read()/NI_stream_write().
 *         - You can use "stdin:", "stdout:", or "stderr:" as synonyms for "fd:0", "fd:1", and
 * "fd:2".
 *
 ...
 *         - shm: keyname determines the ID of the segment to create (which can be attached to by a
 *shm: "r" process).
 *         - file: filename is opened in write mode (and will be overwritten if already exists).
 *         - str: data will be written to a buffer in the NI_stream struct; you can later access
 *this buffer with the function NI_stream_getbuf(), and clear it with NI_stream_clearbuf().
 ...
 *         - shm: keyname determines the ID of the segment to attach to (which must be created by a
 *shm: "w" process).
 *         - file: filename is opened in read mode.
 *         - str: characters after the colon are the source of the input data (will be copied to
 *internal buffer);
 *                OR, you can later set the internal buffer string later with function
 *NI_stream_setbuf().
 *         - You can't open "fd:1" or "fd:2" (stdout or stderr) for reading.
 *         - ftp:/http: The remote files are fetched and loaded into memory.  After that, these
 *streams operate pretty much the same as str: streams for reading.
 *
 * For a file:, fd:, or str: stream, you can either read from or write to the stream, but not both,
 *depending on how you opened it ("r" or "w"). For a tcp: or shm: stream, once it is connected, you
 *can both read and write. The asymmetry in tcp: and shm: streams only comes at the opening (one
 *process must make the call using "w" and one must listen for the call using "r").
 *
 * The inputs "host" (for tcp:) and "filename" (for file:) are limited to a maximum of 127 bytes.
 *For str:, there is no limit for the "r" stream (but clearly you can't have any NUL bytes in
 *there).  For shm:, "keyname"
 * is limited to 127 bytes also.
 *
 * Since opening a socket or shared memory segment requires synchronizing two processes, you can't
 *read or write to a tcp: or shm: stream immediately.  Instead you have to check if it is "good"
 * first.  This can be done using the function NI_stream_goodcheck().
 *
 * After a tcp: "r" stream is good, then the string ns->name contains the IP address of the
 *connecting host, in "dot" form (e.g., "201.202.203.204"); here, "ns" is the NI_stream returned by
 *this routine.
 * - You can use the NI_add_trusted_host() function to set a list of IP addresses from which the
 *NIML library will accept connections.
 * - Systems not on the trusted list will have their sockets closed immediately after the connection
 *is accepted.  Nothing will be read from these sockets.
 *
...
NI_stream NI_stream_open(char *name, char *mode)
{
    ...
    if (dfp == NULL)
    {
        ...
        if (eee != NULL)
        {
            ...
            if (dfp == NULL)
            {
                dfp = stderr; eee = "stderr [defaulted]";
            }
    ...
    if (strncmp(name, "tcp:", 4) == 0)
    {
        ...
        if (!sigurg)
        {
            signal(SIGURG, tcp_sigurg_handler); sigurg = 1;
        }
    ...
    if (strncmp(name, "shm:", 4) == 0)
    {
        ...
        if (ioc == NULL)
        {
            return NULL;               /* this is bad bad bad */
        }
    ...
    if (strncmp(name, "file:", 5) == 0)
    {
        ...
        if (NI_strlen(name) > 255 || NI_strlen(fname) < 1)
        {
            return NULL;
        }
        ...
        {
            fmode = do_create ? (char *)"wb" : (char *)"rb";
        }
    ...
    if (strncmp(name, "str:", 4) == 0)
    {
         ...
         * This is because we don't count the terminal NUL in nbuf (number of readable bytes), but
         *do count it in bufsize (size of the buf array) */
...
/*! Re-open a NI_stream on a different channel.  This is only possible if the input original stream
 * (ns) is tcp: type.
 *   - The new stream (nname) can be of the form "tcp::port", which will reopen the stream to the
 *same host on the new port.
 *   - Or the new stream can be of the form "shm:key:size1+size2", but only if the existing stream
 *was opened to localhost.
 *   - The main purpose of this function is to let a process connect to a "standard" port on a
 *server process, then tell the server to reopen on another port (or to use shm:).  In this way, the
 *standard port can be freed up quickly for reuse.
 *
 *  If necessary, this function will wait until the connection to the other program is ready.  Then
 *it will exchange the information with the other program about changing things, and will again wait
 *until the new connection is established.  Assuming all goes well, then when this function returns,
 *the input stream (ns) will be modified so that it now refers to the new connection.
 *
...
int NI_stream_reopen(NI_stream_type *ns, char *nname)
{
    ...
    if (ns == NULL || ns->type != NI_TCP_TYPE)
    {
        return 0;                                         /* bad input stream */
    }
    ...
    /* send message on old stream to other program, telling it to open the new stream */
     ...
     * close the old stream and replace its contents with the contents of the new stream */
    ...
    *ns = *nsnew;
    ...
    /* 10 Jun 2005: at this point, nsnew is in the open streams list, but the pointer nsnew is about
     * to die, and so we must munge the open streams list around now to make sure that nsnew is
     * removed and ns is re-added! */
...
/*! Return 1 if it is legal to read from this stream, 0 if it isn't. This doesn't say anything about
 * if it is practical to read at this moment; for that, use NI_stream_readcheck().
 * -------------------------------------------------------------------------*/

int NI_stream_readable(NI_stream_type *ns)
{
    if (ns == NULL || ns->bad == MARKED_FOR_DEATH)
    {
        return 0;
    }
...
/*! Return 1 if it is legal to write to this stream, 0 if it isn't. This doesn't say anything about
 * if it is practical to write at this moment; for that, use NI_stream_writecheck().
 * -------------------------------------------------------------------------*/
...
/*! Return the name set in the NI_stream header.  (This is the pointer to the internal string, so
 * don't free it!)
 * -------------------------------------------------------------------------*/
...
/*! Get the input buffer size for a NI_stream. Returns -1 if the stream is bad, or has been
 * sentenced to death.
 * -------------------------------------------------------------------------*/
...
/*! Return the output string buffer for a NI_stream of str: type. If the input is not a "w" str:
 * stream, then NULL is returned. Otherwise a pointer to the internal buffer is returned. This will
...
/*! Clear the buffer of a str: writing NI_stream.  This is intended to let you write anew without
...
/*! Reset the input string buffer for a NI_stream of str: type. If the input is not a "r" str:
 * stream, then nothing happens. Otherwise, the current contents of the buffer are discarded, and
 *the buffer is replaced with a copy of the input string.
 * -------------------------------------------------------------------------*/
 ...
 *
 * If not, wait up to msec milliseconds to establish the connection to the other end; if msec < 0,
 *will wait nearly forever. Returns 1 if ready; 0 if not (but may become good later);
 * -1 if an error occurs. Possible -1 errors are:
 ...
 *   - ns is marked for death The only cases in which 0 is returned is if the NI_stream is tcp: or
 *shm: and the stream is waiting for a connection from the other program.  These are also the only
 *cases in which input parameter msec is actually used.
 * -------------------------------------------------------------------------*/
...
/*! Close a NI_stream, but don't free the insides. If (flag&1 != 0) send a "close_this" message to
 * the other end. If (flag&2 != 0) use TCP OOB data to send a SIGURG to the other end. If (flag&4 !=
 * 0) don't remove from open_stream list [only from atexit()]
 * -----------------------------------------------------------------------------*/

void NI_stream_close_keep(NI_stream_type *ns, int flag)
{
    if (ns == NULL || !isgraph(ns->orig_name[0]))
    {
        return;
    }
    ...
    case NI_FD_TYPE:
        if (ns->fp != NULL && ns->io_mode == NI_OUTPUT_MODE)
        {
            fflush(ns->fp);
        }
        break;
...
/*! Close a NI_stream.  Note that this will also free what ns points to. Don't use this pointer
 * again. Use the NI_STREAM_CLOSE macro to call this function and then also set the pointer "ns" to
 * NULL.
 * -------------------------------------------------------------------------*/
...
/* Close a NI_stream with a "close_this" message and also using TCP OOB data (for socket streams,
 * that is) to notify the other end. */
...
int NI_stream_hasinput(NI_stream_type *ns, int msec)
{
    ...
    if (ns->npos < ns->nbuf)
    {
        return 1;                            /* check if has data in buffer */
    }
 ...
 *
 * If not, the routine will wait up to msec milliseconds for data to be available.  If msec < 0,
 *this routine will wait nearly forever. The return value is 1 if data is ready, 0 if not; -1 will
 *be returned if some unrecoverable error is detected:
...
int NI_stream_readcheck(NI_stream_type *ns, int msec)
{
    ...
    case NI_TCP_TYPE:
        ...
        if (!ii)
        {
            return -1;                         /* if not open, error exit  */
        }
 ...
 *
 * If not, the routine will wait up to msec milliseconds for writing to be allowable.  If msec < 0,
 *this routine will wait nearly forever. The return value is 1 if data can be sent, 0 if not;
 * -1 will be returned if some unrecoverable error is detected:
...
int NI_stream_writecheck(NI_stream_type *ns, int msec)
{
    ...
    case NI_TCP_TYPE:
        if (ns->bad)                           /* not marked as good */
        {
            ...
            if (ii == -1)
            {
                return -1;                          /* some error when checking */
            }
            if (ii == 0)                            /* not good yet, */
            {
                ii = NI_stream_goodcheck(ns, msec); /* so wait for it to get good */
 ...
 *
 * Return value is the number of bytes actually sent, or is -1 if some error occurs (which means
 *that the NI_stream is bad).  If 0 is returned, this means you tried to write to something that is
 *temporarily unavailable.
 *
 * - tcp: We use blocking sends, so that all the data should be sent properly unless the connection
 *to the other end fails for some reason (e.g., the planet explodes in a fiery cataclysm of
 *annihilation).
 * - shm: We also block until everything can be written, even if it requires filling the shared
 *memory buffer many times and waiting for the reading process to empty it many times.
 * - file: Everything should be written, unless the filesystem fills up. If nothing at all gets
 *written, -1 is returned.
 * - str: Everything will be written, or the program will crash. Do not include the NUL byte at the
 *end of the string in the nbytes count.
 * ------------------------------------------------------------------------------*/
...
int NI_stream_write(NI_stream_type *ns, char *buffer, int nbytes)
{
    ...
    if (ns == NULL || ns->bad ||
        buffer == NULL || nbytes < 0 || ns->bad == MARKED_FOR_DEATH)
    {
        return -1;
    }
    ...
    if (ns->type != NI_TCP_TYPE)
    {
        ...
        if (ii < 0)
        {
            return ii;                  /* if not, vamoose the ranch  */
        }
    }
    ...
    case NI_TCP_TYPE:
        ...
        /* turn off SIGPIPE signals, which will otherwise be raised if we send to a socket when the
         * other end has crashed */

        if (!nosigpipe)
        {
            signal(SIGPIPE, SIG_IGN); nosigpipe = 1;
        }

#if 0
        /* 03 Mar 2002: wait until we can write fer shur */
        do{
            ii = tcp_writecheck(ns->sd, 1);
        } while (ii == 0);
        if (ii < 0)
        {
            return -1;
        ...
        if (nsent < nbytes || errno != 0)
        {
            PERROR("NI_stream_write(send)");
        }
        if (nsent == 0)
        {
            fprintf(stderr, "tcp send: 0/%d\n", nbytes); nsent = -1;
        }
        return nsent;
 ...
 *
 * Returns the number of bytes actually read.  For both the case of sockets and files, this may be
 *less than nbytes (may even be 0). If an error occurs and no data is read, -1 is returned.
 *
 * For tcp: streams, if no data is available, this function will wait until something can be read.
 *If this behavior is undesirable, then you should use NI_stream_readcheck() before calling this
 *function in order to see if any data is available.
 *
 * For shm: streams, will return immediately if no data is available.
 *
 * For file: streams, this function simply tries to read from the file. Whether or not it succeeds,
 *it will return immediately. It should never return -1; if it returns 0, this means end-of-file.
 * ---------------------------------------------------------------------------*/
...
int NI_stream_read(NI_stream_type *ns, char *buffer, int nbytes)
{
    ...
    if (ns == NULL || ns->bad || buffer == NULL || nbytes < 0)
    {
        return -1;
    }
    ...
    case NI_TCP_TYPE:
        ii = NI_stream_goodcheck(ns, 1); if (ii != 1)
        {
            return ii;
        }
#if 0
...
/*! Try to fill up the stream's input buffer. Don't call this function until NI_stream_goodcheck()
 * is 1!
 *
 * - minread = Minimum number of bytes to read. Will wait until we get at least this many, until the
 *stream is bad or the buffer is full. If minread=0, then may read nothing (but will try).
 *
 * - msec    = Maximum amount of time to wait to satisfy minread, in milliseconds.  If msec<0, will
 *wait nearly forever. If msec=0, will return after 1st read attempt, even if nothing was obtained.
 *
 *  Returns number of bytes read (-1 if input stream goes bad before any data is read).  If the
 *input stream goes bad AFTER some data is read, there is no indication of that (until the next time
...
int NI_stream_fillbuf(NI_stream_type *ns, int minread, int msec)
{
    ...
    if (ns->type == NI_STRING_TYPE)
    {
        return -1;                                   /* goofy input */
    }
    if (ns->type == NI_REMOTE_TYPE)
    {
        return -1;                                   /* goofy input */
    }
    if (ns->nbuf >= ns->bufsize)
    {
    ...
    while (1)
    {
        ...
        if (ngood < 0)
        {
            break;                           /* data stream gone bad, so exit */
        }
        ii = 0;
                                             ...
                                             /* try to fill buffer completely */
        {
            ...
            if (ii > 0)                /* we got data! */
            {
                ...
                /* if buffer is full, or we have all the data that was asked for, then exit */

                if (ns->nbuf >= ns->bufsize || ntot >= minread)
                {
                    break;
                }
            }
        ...
        if (NI_clock_time() - start_msec >= msec)
        {
            break;
        }
    ...
    /* if didn't get any data, and if the NI_stream was bad, return -1 as a flag of displeasure */
...
/*! Buffered read from a NI_stream.  Unlike NI_stream_read(), will try to read all nbytes of data,
 * waiting if necessary.  Also works through the internal buffer, rather than directly to the
 * stream.
 *
 *  Return value is number of bytes read.  May be less than nbytes if the stream closed (or was used
 *up) before nbytes of data was read. Will return -1 if something is rotten.
 * -------------------------------------------------------------------------*/
...
int NI_stream_readbuf(NI_stream_type *ns, char *buffer, int nbytes)
{
    ...
    if (nbytes == 0)
    {
        return 0;                                       /* that was real easy */
    }
    if (buffer == NULL || nbytes < 0)
    {
        return -1;                                      /* stupid caller */
    }
    if (ns->buf == NULL || ns->bufsize == 0)
    {
        return -1;                                      /* shouldn't happen */
    }
    if (!NI_stream_readable(ns))
    {
        return -1;                                      /* stupid stream */
    }
    ...
    if (nout == 0 && ii < 0)
    {
        nout = -1;                          /* no data and an I/O error */
    }
    return nout;
}
 ...
 *    - Will stop at a '<'.
 *
 *  Return value is number of bytes put into the buffer.  May be less than nbytes if the stream
 *closed (or was used up, or hit a '<') before nbytes of data was read.  Will return -1 if something
 *is rotten.
 * -------------------------------------------------------------------------*/
...
int NI_stream_readbuf64(NI_stream_type *ns, char *buffer, int nbytes)
{
     ...
     * this is done 4 input bytes at a time, which are decoded to 3 output bytes
     *                   */
...
Base64Reread:
    ag = bg = cg = 0;
    num_reread++; if (num_reread > 5)
    {
        goto Base64Done;                                 /* done waiting! */
    }
    ...
    if (num_reread > 1 || ns->nbuf - ns->npos < 4)
    {
        ...
        if (ns->nbuf < 4)
        {
            goto Base64Done;                  /* can't get no satisfaction! */
        }
    }

    /*** Copy valid Base64 bytes out of buffer (skipping others), converting them to binary as we
     * get full quads, putting the results into buffer.
     *
     *   Exit loop if we hit a '<' character (end of NIML element), or hit an '=' character (end of
     *Base64 data stream).
     *
     *   Jump back to Base64Reread (above) if we run out of data in the buffer before we fulfill the
     *caller's demand for nbytes of output. ***/
    ...
    while (1)
    {
        ...
        if (bpos == ns->nbuf)
        {
            goto Base64Reread;                  /* not enuf data yet */
        }
        /* repeat to fill x */
        ...
        if (x == '<')
        {
            ns->npos = bpos - 1; goto Base64Done;
        }
        if (bpos == ns->nbuf)
        {
            goto Base64Reread;
        ...
        if (z == '=')                              /* got to the end of Base64? */
        {
            ...
            if (ag)
            {
                buffer[nout++] = a; ag = 0; if (nout >= nbytes)
                {
                    goto Base64Done;
                }
            }
            if (bg)
            {
                buffer[nout++] = b; bg = 0; if (nout >= nbytes)
                {
                    goto Base64Done;
                }
            }
        ...
        buffer[nout++] = c; cg = 0; if (nout >= nbytes)
        {
            goto Base64Done;
        }

        /* now, loop back to decode the next 4 bytes;
         * BUT, if we don't have at least 4 bytes in the input buffer, must do a re-read first!
         *                               */
    ...
    /* At this point:
     *  have finished reading and decoding, have nout bytes in output buffer, and might have some
     *good bytes left that need to be saved */

Base64Done:
    ns->b64_numleft = 0;
    if (ag)
    {
        ns->b64_left[ns->b64_numleft++] = a;
    }
    if (bg)
    {
        ns->b64_left[ns->b64_numleft++] = b;
    }
    if (cg)
    {
        ns->b64_left[ns->b64_numleft++] = c;
    }
