...
/*****
*      mri_to_complex      = creates a complex image from input    (7D SAFE) mri_to_complex_ext  =
* extended version allows a new size    (NOT 7D SAFE) mri_pair_to_complex = creates a complex image
*from 2 inputs (7D SAFE) mri_complex_to_pair = creates 2 float images from 1 complex (7D SAFE)
*****/
...
/************************************************************************
 * xnew = new x dimension ynew = new y dimension if altern is non-zero, multiplies element (i,j) by
 *(-1)^(i+j) [to shift center of FFT space to center of grid]
 *************************************************************************/
    ...
    for (jj = 0 ; jj < jtop ; jj++)
    {
        ...
        for (ii = 0 ; ii < itop ; ii++)
        {
            ...
            case MRI_byte: { byte *qar = MRI_BYTE_PTR(oldim);
                             nar[ii + jbnew].r = qar[ii + jbold]; } break;
            ...
            case MRI_complex: { complex *qar = MRI_COMPLEX_PTR(oldim);
                                nar[ii + jbnew] = qar[ii + jbold]; } break;
            }
        }
    }
...
MRI_IMAGE * mri_pair_to_complex(MRI_IMAGE *rim, MRI_IMAGE *iim)
{
    ...
    if (rim == NULL || iim == NULL || rim->nvox != iim->nvox)
    {
        RETURN(NULL);
    }
    ...
    for (ii = 0 ; ii < nvox ; ii++)
    {
        car[ii].r = rar[ii]; car[ii].i = iar[ii];
    }
...
MRI_IMARR * mri_complex_to_pair(MRI_IMAGE *cim)
{
    ...
    if (cim == NULL || cim->kind != MRI_complex)
    {
        RETURN(NULL);
    }
    ...
    for (ii = 0 ; ii < nvox ; ii++)
    {
        rar[ii] = car[ii].r; iar[ii] = car[ii].i;
    }
