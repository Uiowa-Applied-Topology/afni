...
static RwcBoolean write_output = False;           /* 21 Jun 2006 [dg] -force rewrite as in 3drefit
                                                   * by rickr */
...
void ZCAT_read_opts(int argc, char *argv[])
{
    ...
    while (nopt < argc)
    {
        ...
        if (strncmp(argv[nopt], "-datum", 6) == 0)
        {
            ...
            {
                ZCAT_datum = MRI_short;
            }
            else if (strcmp(argv[nopt], "float") == 0)
            {
                ZCAT_datum = MRI_float;
            }
...
int main(int argc, char *argv[])
{
    ...
    if (ZCAT_datum < 0)
    {
        ZCAT_datum = DSET_BRICK_TYPE(dset, 0);
    }
    ...
    for (iv = 0 ; iv < DSET_NVALS(new_dset) ; iv++)
    {
        ...
        /* at this point, fvol holds the correctly scaled values for the output dataset;
        ...
        /* if being frugal with memory (why?), save the data to disk for each output sub-brick */
        if (ZCAT_frugal)
        {
