...
int main(int argc, char *argv[])
{
    ...
    if (verb)
    {
        fprintf(stderr, "++3dSpatNorm: loading dataset\n");
    }
    ...
    if (SpatNormDxyz)
    {
        if (verb)
        {
            fprintf(stderr, "Overriding default resampling\n");
        }
    ...
    if (OrigSpace)
    {
        if (verb)
        {
            fprintf(stderr, "++3dSpatNorm: Output in Original space\n");
        }
        mri_free(imout);
        imout      = imout_orig;
    ...
    else
    {
        if (verb)
        {
            fprintf(stderr, "++3dSpatNorm: Output in SpatNorm space\n");
        }
    }
