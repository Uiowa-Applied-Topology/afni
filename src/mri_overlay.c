...
void mri_overlay_2D(MRI_IMAGE *imbase, MRI_IMAGE *imover, int ix, int jy)
{
    ...
    if (ix + nxov > nxba)
    {
        nxxov = nxba - ix;                  /* row too long for base? */
    }
    nyyov = nyov;                           /* number of overlay rows */
    if (jy + nyov > nyba)
    {
        nyyov = nyba - jy;                  /* too many rows for base? */
    }
    ...
    if (imov != imover)
    {
        mri_free(imov);                   /* throw away converted overlay? */
    }
    EXRETURN;
}
