...
int main(int argc, char *argv[])
{
    ...
    while (iarg < argc && argv[iarg][0] == '-')
    {
        ...
        if (strcmp(argv[iarg], "-metric") == 0)
        {
            ...
            if (metric_thresh <= 1.0)
            {
                fprintf(stderr, "** Illegal value after -metric\n"); exit(1);
            }
            iarg++; continue;
        }
        ...
        if (strcmp(argv[iarg], "-area") == 0)
        {
            ...
            if (area_thresh <= 1.0)
            {
                fprintf(stderr, "** Illegal value after -area\n"); exit(1);
            }
            iarg++; continue;
        }
        ...
        if (strcmp(argv[iarg], "-base") == 0)
        {
            ...
            if (base <= 0)
            {
                fprintf(stderr, "** Illegal value after -base\n"); exit(1);
            }
            iarg++; continue;
        }
        ...
        if (strcmp(argv[iarg], "-hyval") == 0)
        {
            ...
            if (hyval <= 0)
            {
                fprintf(stderr, "** Illegal value after -hyval\n"); exit(1);
            }
            iarg++; continue;
        }
    ...
    if (base <= 0 || (hyval <= 0 && hyper))
    {
        ...
        if (hyval <= 0 && hyper && hh > base + 1)
        {
            ...
            if (verbose)
            {
                printf("-- Hyperintensity value set to %d\n", hyval);
            }
        }
    }
    ...
    if (verbose)
    {
        printf("-- Blasted %d voxels at or below base\n", pp);
    }
    ...
    /* for each slice:
     *   build a mask of blocking points (zero voxels in the slice) find some outermost points flood
     *fill from the outermost points compute a metric of "annularity" of the filled points if the
     *slice passes the metric, blast out the filled points
     */
    ...
    for (kk = ktop ; kk >= 0 ; kk--)
    {
        ...
        if (cc < 0.1 * nxy)
        {
            if (verbose)
            {
                printf("-- Slice %3d: skipping because not enough zeros\n", kk);
            }
            continue;
        }
        ...
        if (ii < ic && jj < jc && BB(ii, jj) == 0)
        {
            DRAW_2dfiller(nx, ny, ii, jj, bar);
        }
        ...
        /* we don't scan in from top/bottom since those might be clipped off in Talairach
         * coordinates, which would be disastrous! */

        /* compute metric for marked points:
         *   metric = average( r**2 ) / average(r)**2 this should be nearly 1 for an annular region
         */
        ...
        if (nsum < 10)
        {
            if (verbose)
            {
                printf("-- Slice %3d: skipping because not enough boundary fillin\n", kk);
            }
            continue;
        }
        ...
        if (verbose)
        {
            printf("-- Slice %3d: metric =%8.4f  area =%8.0f\n", kk, metric, area);
        }
    ...
    if (!didit)
    {
        fprintf(stderr, "** No slices were trimmed -- end of run!\n"); exit(1);
    }
    ...
    if (hyper && hyval > base + 1)
    {
        ...
        if (verbose)
        {
            printf("-- Processed %d voxels at or above hyval\n", pp);
        }
    }
...
void remove_isolated_stuff(THD_3dim_dataset *qset)
{
    ...
    do{
        ...
        if (verbose)
        {
            printf("-- Blasted %d voxels for being too isolated\n", nblast);
        }
    } while (nblast > 0);
 ...
 *   nx = 1st dimension ny = 2nd dimension ix = start point jy = end point ar = array, with 0's
 *everywhere except 1's as barriers to flooding
 *
...
#define AR(i, j)    ar[(i) + (j) * nx]
    ...
    for (ii = ip + 1; ii < nx && AR(ii, jp) == 0; ii++)
    {
        AR(ii, jp) = 2;
    }
    for (ii = ip - 1; ii >= 0 && AR(ii, jp) == 0; ii--)
    {
        AR(ii, jp) = 2;
    }
    for (jj = jp + 1; jj < ny && AR(ip, jj) == 0; jj++)
    {
        AR(ip, jj) = 2;
    }
    for (jj = jp - 1; jj >= 0 && AR(ip, jj) == 0; jj--)
    {
        AR(ip, jj) = 2;
    }
