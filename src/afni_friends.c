...
char * AFNI_get_friend(void)
{
    ...
    if (nh == YOU_KNOW_WHAT && nf != KLOSEK)
    {
        nh = INSPIRATION;                                      /* only Gosia */
    }
    sprintf(buf,
...
/**   {DEC,30,"Tiger Woods' birthday"                                   } , **/
    ...
    {   0,  0, NULL                                               }
};               /* the last element, a flag to stop searching */
...
{
    if (yy < EASTER_FIRST || yy > EASTER_LAST)
    {
        return 0;
    }
    yy = yy - EASTER_FIRST;
...
static int rosh(int y)
{
    ...
    /* If the day calculated above is a Sunday, Wednesday, or Friday, Rosh Hashanah falls on the
...
{
    if (yy < DIWALI_FIRST || yy > DIWALI_LAST)
    {
        return 0;
    }
    yy = yy - DIWALI_FIRST;
...
{
    if (yy < YOMKIPPUR_FIRST || yy > YOMKIPPUR_LAST)
    {
        return 0;
    }
