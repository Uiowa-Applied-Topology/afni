 ...
 *
 *    prelim   : initial prefix ndigits  : number of digits used for zero-padded index in the name
 *index    : index will be the second part of the name label    : if set, include the label string,
 *with space or #
 *               replaced by _ ext      : if set, include .ext
 *
 * If prelim is NULL, free local memory and return NULL.
 *
 * return a stactic char pointer, that should not be freed [12 Apr 2024 rickr]
 */
                          ...
                          char *label, char *ext, int do_bids_deriv)
{
    ...
    if (osize > splen)
    {
        ...
        if (!sprefix)
        {
            ERROR_exit("failed to alloc %d bytes for prefix", osize);
        }
        splen = osize;
    }
    ...
    if (lablen > sllen)
    {
        ...
        if (!slabel)
        {
            ERROR_exit("failed to alloc %d bytes for label", lablen);
        }
        sllen = lablen;
    }
...
int main(int argc, char *argv[])
{
    ...
    if (nval == 0)
    {
        ERROR_exit("no volumes to output?");
    }
