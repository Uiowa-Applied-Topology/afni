...
static MRI_IMAGE * mri_make_xxt(MRI_IMAGE *fim)
{
    ...
    nn = fim->nx; mm = fim->ny;    if (nn < mm || mm < 2)
    {
        return NULL;
    }
    xx = MRI_FLOAT_PTR(fim);               if (xx == NULL)
    {
        return NULL;
    }
    n1 = nn - 1;
...
static float mri_svd_max(MRI_IMAGE *fim)
{
    ...
    aim = mri_make_xxt(fim); if (aim == NULL)
    {
        return -1.0f;
    }
    mm = aim->nx;
 ...
 *  - tau > 0.0
 *  - sv non-NULL ==> it gets the fim->ny pre-shrink singular values in ascending order
 *  - return value is number of non-zero singular values after shrinkage; if -1 is returned, then
 *something bad happened and the input is unchanged
 *//*--------------------------------------------------------------------------*/
...
int mri_svd_shrink(MRI_IMAGE *fim, float tau, float *sv)
{
    ...
    if (fim == NULL || fim->kind != MRI_float || tau <= 0.0f)
    {
        return -1;
    }
    ...
    /** convert to singular values [ascending order], and then to scaling values for eigenvectors
     *  **/
    ...
    for (ii = 0 ; ii < mm ; ii++)
    {
        ...
        if (sv != NULL)
        {
            sv[ii] = (float)sval[ii]; /* save singular values */
        }
...
int mri_abs_shrink(MRI_IMAGE *fim, float tau)
{
    ...
    nvox = fim->nvox; xx = MRI_FLOAT_PTR(fim); if (xx == NULL)
    {
        return;
    }
    tpos = tau; tneg = -tpos;
...
/* Shrink in SVD, then shrink in abs. Inputs aim, bim, eim, fim are modified in place.
 *//*--------------------------------------------------------------------------*/
                         ...
                         MRI_IMAGE *bim, MRI_IMAGE *fim)
{
    ...
    /** form [yaim] = [aim] + ac*([aim]-[bim]) [yeim] = [eim] + ac*([eim]-[fim]) **/
...
MRI_IMARR * mri_pc_pursuit(MRI_IMAGE *dim, float lam, float mubot)
{
    ...
    if (lam <= 0.0f)
    {
        lam = 1.0f / sqrtf((float)dim->nx);
    }

    /* get largest singular value of [dim] */

    mu = mri_svd_max(dim);     if (mu <= 0.0f)
    {
        return NULL;
    }
    mu *= 0.911f; if (mu < mubot)
    {
        mu = mubot;
    }
