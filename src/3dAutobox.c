...
/*
 * [PT: Oct 15, 2018]
 + added '-extent_ijk_to_file FF' option to get slice numbers of auto-bboxing in a nice text file.
 +
 + [PT: Oct 15, 2018]
 + change where+how input dset check occurs, so subbrick selection is possible
 +
 + [PT: Oct 15, 2018]
...
/*
 * A function that takes a dataset as input, and based on its orientation outputs a string of 'i',
 * 'j' and 'k' values. For example, the orientation 'RPI' -> 'ijk', and 'AIL' -> 'jki'.
 */
...
int main(int argc, char *argv[])
{
...

    {
        ...
        /* Constrain each index to be in [0, L-1], where L is matr len. Some checks here might be
         * extraneous but... they are all valid
         */
        if (DO_NPAD_SAFELY)
        {
            if (xm < 0)
