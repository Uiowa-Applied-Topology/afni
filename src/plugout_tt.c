...
/***************************************************************
 * Sample plugout program, that registers with AFNI to be notified every time the Talairach-Tournoux
 *coordinates change.  Note that this will only occur when the lowest numbered active AFNI
 *controller is in the Talairach view. T-T coordinates are
 *    -x axis = L   +x axis = R
 *    -y axis = P   +y axis = A
 *    -z axis = I   +z axis = S Note that x and y are each flipped from the DICOM standard, which
 *AFNI uses internally. The values reported to this plugout program are in the T-T system, not the
 *DICOM system. RWCox, June 1997
 ****************************************************************
 * Usage: plugout_tt [-host name] [-v] Options:
 *  -host name  Means to connect to AFNI running on the remote computer 'name'.  The default is to
 *connect on the current host.  If the connection is to "localhost" (the default), then shared
 *memory is used, otherwise a TCP/IP socket is used.
 *  -v          Verbose mode: prints out lots of stuff.
 *
 * Note that AFNI must be run with the "-yesplugouts"
 * option to allow it to talk to this program.  See the output of "afni -help" for (a little) more
 *information.
 ****************************************************************
 * The file "thd_trusthost" controls which systems are allowed to connect to AFNI.  This is
 *controlled by setting the environment variables AFNI_TRUSTHOST_1 through AFNI_TRUSTHOST_99 to the
 *IP addresses (not names) of hosts from which AFNI should accept plugout connections.  For example,
 *setenv AFNI_TRUSTHOST_1 123.45.67.89 Note that 127.0.0.1 (localhost) is always trusted.
 ****************************************************************
 * Compilation:
 *  If using the Makefile that came with AFNI, then make plugout_tt Otherwise cc -o plugout_tt -O
 *plugout_tt.c thd_iochan.c -I.
 *
 * "thd_iochan.c" contains the routines that do the TCP/IP socket and IPC shared memory stuff.
 ****************************************************************/
...
static int  afni_port      = 0; /* Init. before parsing command line ZSS June 2011 */
...
/*===================================================================
 * Main program:
 *   Read command line for options Call afni_io routine forever
 * =====================================================================*/
...
int main(int argc, char *argv[])
{
    ...
    afni_port = get_port_named("PLUGOUT_TT_PORT"); /* ZSS June 2011 */
    ...
    if (argc == 2 && strncmp(argv[1], "-help", 5) == 0)
    {
        printf("Usage: plugout_tt [-host name] [-v]\n"
               ...
               "                %d, but if two copies of this are running on\n"
...
/*===================================================================
 * This routine handles all communications with AFNI. The only input is the global variable
 *afni_host, which determines on which system AFNI is running. The output is -1 if an error
 *occurred, 0 if everything is OK.
 * =====================================================================*/

/***** Mode flags for determining what afni_io does. The routine progress through 5 stages:
 *
 *     1) Open a control connection to AFNI;
 *   then 2) Wait until AFNI also opens the control connection;
 *   then 3) Send a control string to AFNI saying what kind of information we want, wait for an
 *acknowledgment, close the control connection, and open a data connection to AFNI;
 *   then 4) Wait for AFNI to also open the data connection;
 *   then 5) See if AFNI sends any data, and if so, process it!  *****/
...
int afni_io(void)
{
    ...
    if (afni_mode == AFNI_OPEN_CONTROL_MODE)
    {
        ...
        /** Note that the control channel is always a TCP/IP channel to port
         * #get_port_named("AFNI_PLUGOUT_TCP_0") (used to be #7955) on the AFNI host system **/
    ...
    if (afni_mode == AFNI_OPEN_DATA_MODE)
    {
         ...
         *    use shared memory (shm:) on ".", use TCP/IP (tcp:) on other computer systems;
         * Note that the TCP/IP port number can be anything that isn't already in use;
         * Note that the shm control name (here "test_plugout") is a string that will be converted
         *to an IPC key (in function string_to_key in iochan.c).       **/
         ...
         * each command ends with a newline character, except (possibly) the last command;
         * the command buffer is a C string, which ends with an ASCII NUL character;
        ...
        if (afni_name[0] == '\0')
        {
            strcpy(afni_name, "aManCalledHorse");
        }
        ...
        /** the return value is the number of bytes sent, or -1 indicating a fatal error transpired.
         *    **/
        ...
        else
        {
            ...
            else
            {
                ...
                if (afni_verbose)
                {
                    fprintf(stderr, "AFNI data channel created\n");
                }
            }
        }
    }
    ...
    if (afni_mode == AFNI_WAIT_DATA_MODE)
    {
        ...
        else if (ii > 0)                       /* ready to go! */
        {
            ...
            if (afni_verbose)
            {
                fprintf(stderr, "AFNI data channel is open\n");
            }
        }
        else
        {
    ...
    if (afni_mode == AFNI_CONTINUE_MODE)
    {
        ...
        /** ii <  0  ==>  a fatal error has happened ii == 0  ==>  no data is ready ii >  0  ==>
         *  data is ready to read from the channel **/
        ...
        /** at last! "process" the data from AFNI (in this case, just print it out) **/
