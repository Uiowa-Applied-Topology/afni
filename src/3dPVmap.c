...
int main(int argc, char *argv[])
{
    ...
    while (nopt < argc && argv[nopt][0] == '-')
    {
        ...
        if (strcmp(argv[nopt], "-mask") == 0)
        {
            if (++nopt >= argc)
            {
                ERROR_exit("-mask needs an argument!");
            }
    ...
    if (nopt >= argc)
    {
        ERROR_exit("No input dataset name?");
    }
    ...
    {
        ERROR_exit("input dataset too short");
    }
    ...
    if (maskname != NULL)
    {
        ...
        else
        {
            ...
            if (mask == NULL)
            {
                ERROR_exit("Can't make automask :(");
            }
    ...
    if (pvim == NULL)
    {
        ERROR_exit("Can't compute PVmap :(");
    }
    DSET_unload(inset);
    ...
    cpt = strstr(uvpref, ".nii"); if (cpt != NULL)
    {
        *cpt = '\0';
    }
    cpt = strstr(uvpref, ".HEAD"); if (cpt != NULL)
    {
        *cpt = '\0';
    }
    cpt = strrchr(uvpref, '+'); if (cpt != NULL)
    {
        *cpt = '\0';
    }
    strcat(uvpref, ".1D");
