...
int main(int argc, char *argv[])
{
    ...
    if (unesc)  /* let's leave argv alone, so dup the string */
    {
        ...
        if (!tnew)
        {
            fprintf(stderr, "** cannot dup targetstring?!\n"); exit(1);
        }
        for (ii = 0, jj = 0; ii < ntarg; ii++, jj++)
        {
            if (tnew[ii] == '\\' && tnew[ii + 1] == 't')
            {
                tnew[jj] = '\t'; ii++;
            }
            else if (tnew[ii] == '\\' && tnew[ii + 1] == 'n')
            {
                tnew[jj] = '\n'; ii++;
            }
            else if (tnew[ii] == '\\' && tnew[ii + 1] == 'r')
            {
                tnew[jj] = '\r'; ii++;
            }
            else if (ii > jj)
    ...
    if (ntarg < njstr)
    {
        if (use_newstr)
        {
            jstr[ntarg] = '\0'; njstr = ntarg;
        }
        else
        {
            jstr[0] = newchar; njstr = 1;
        }
    }
    ...
    for (ff = 0 ; ff < nfname ; ff++)
    {
        ...
        for (nfind = ii = 0 ; ii < nbuf - ntarg ; ii++)
        {
            ...
            {
                for (jj = 1; jj < ntarg && fbuf[ii + jj] == targ[jj] ; jj++)
                {
                    ;            /* nada */
                }
