/*      This program displays functional EPI of the form 64x64 up to 512x512 in X11 windows. Keys
 * and subWindow are added. The ar is malloc(). It uses 8 or 12 bitplanes and own solid color map,
 * or any RGB mode. Extras: image marker, continuous X,Y position. EPI part based on Eric C. Wong
...
#define MCOLORS           856                   /* maximum # of colors: NCOLORS + NUM_STD_COLORS
                                                 * (16) */
                                                /*                    + MAX_EXTRA_COLORS (32) */
...
struct _key xtkeys[N_KEYS] = { { "Rot",                      kROT,
                                 Ims_rot },
                               { "Help",                     kHLP,
                                 Im_help },
                               { "AvIm",                     kAVR,
                               ...
                               { "Norm",                     kNRM,
                                 Im_norm },
                               { "Average: from this image", kAV1,
                                 Av_im1 },
                               { "Average: to this one.   ", kAV2,
                                 Av_im2 },
                               { "Ref. level first image",   kIR1,
                                 Ref_im1 },
                               { "Ref. level last  image",   kIR1,
                                 Ref_im2 },
                               { "set threshold",            kFI1,
                                 FIM_selection },
                               { "ref/ort file ",            kFI2,
                               ...
                               { "    ",                     kFT3,
                                 FFT_selection },
                               { "Reference  image",         kSC1,
                                 SCA_selection },
                               { "Scale this image ",        kSC2,
                                 SCA_selection } };
...
char *formt[4][4] = {
    { "%s.%d",      "%s.%02d",      "%s.%03d",
      "%s.%04d" },
    { "%s_%d.%d",   "%s_%d.%02d",   "%s_%d.%03d",
      "%s_%d.%04d" },
    { "%s_%02d.%d", "%s_%02d.%02d", "%s_%02d.%03d",
      "%s_%02d.%04d" },
    { "%s_%03d.%d", "%s_%03d.%02d", "%s_%03d.%03d",
      "%s_%03d.%04d" }
};
...
/* ---------------- */
{
    ...
    ProgramName = argv[0];  if (Xdef_Name == NULL)
    {
        Xdef_Name = ProgramName;
    }
...
/* -------- */
{
    ...
    fprintf(stderr, "\n    -sp #_of_degree  - range of color spectrum [0-%d] degree (def %d)",
            M_SP_COL, N_SPCTR);
...
/***************************************************************************/
    ...
    {
        if (strncmp(Argv[k + nopt], "-extra", 3) == 0)
        {
            k++;                                         /* skip -extra */
        }
    ...
    for (i = 0; i < Im_frst - 1; i++)
    {
        f_name[i] = f_name[Im_frst - 1];
    }
    ...
    if (allim[0]->ny > 256)
    {
        im_height = 512;                                  /* new for 512 image size */
    }
    ...
    if (RWC_ideal != NULL)
    {
        ...
        if (RWC_ideal->len < npoints)
        {
            ...
            if (RWC_imover != NULL)
            {
                free(RWC_imover); RWC_imover = NULL;
            }
        }
        else
        {
...
/* ---------- Open the display. --------- */
    ...
    if (AJ_PseudoColor)
    {
        if (!(XAllocColorCells(theDisp, CMap, True, plane_masks, nplmsk,
                               pixels, ucolors)))
        {
            FatalError("XAllocColorCells problem. AJ");
        }
    }
    else
    {
...
/* ----------------------- */
{
    ...
    for (i = LAST_K; i < N_KEYS; i++)
    {
        XUnmapWindow(theDisp, key[i].wid);
    }
    back_to_main = 0;
    while (1)
    {
        ...
        else
        {
            ...
            case '9': {
                ...
                else
                {
                    diff_im = fim_dif;
                    avr_grp = fim_avr;
                    if (itmp > npoints - Av_length)
                    {
                        itmp = npoints - Av_length;
                    }
                }
                if (itmp != Im_Nr)
                {
...
/* -------------------- */
{
    ...
    {
        AJ_StoreColors(theDisp, CMap, MYgrey, ncolors, 0);
    }
}
...
/* ----------------- */
{
    ...
    {
        AJ_StoreColors(theDisp, CMap, MYcol, ncolors, 1);
    }
}
...
/* -------------------- */
{
    ...
    for (ex = 0; ex < ncolors; ex++)
    {
        x_arr[ex] = SQUE_NR * (ex - w) / ncolors + w;
    }
...
/* -------------- */
{
    ...
    if (AJ_PseudoColor)
    {
        if (cx > NUM_STD_COLORS)
        {
            return extra_color_x11[cx - NUM_STD_COLORS - 1];
        }

        if (color_x11[cx - 1] + 1)
        {
            return color_x11[cx - 1];
        }
        else
        {
...
/* ---------------------------------- */
{
    ...
    if (AJ_PseudoColor)
    {
        image = Load_Any_ind(im_arr, x, y);
    }
    else
    {
        image = Load_Any_RGB(im_arr, x, y);
    }
...
/* ---------------------------------- */
{
    ...
    if (!image)
    {
        FatalError("Unable create image in Load_R_G_B_Arr().");
    }

    Image = (char *)malloc(image->bytes_per_line * Hight);
    if (!Image)
    {
        FatalError("Not enough memory for the Image");
    }
    image->data = Image;
...
/* ---------------------------------- */   /* 8 and 12 bit planes work OK. */
{
    ...
    if (!image)
    {
        FatalError("Unable create image in Load_index_Arr().");
    }
...
/* ----------------------------------------------------------------
 *  Read file subroutine fo use in C.        A.Jesmanowicz, MCW 1991 return error :	0 - OK, 1 -
 * opening problem, 2 - file longer then array.
 *   fname :	file name.
 *   size  :	on input - max size of the arr or 0 for any length, on output- real size of the file
 *(and arr in bytes).
...
/* ----------------------------------------------------------------
 *  Write file subroutine fo use in C.       A.Jesmanowicz, MCW 1991 return error :	0 - OK, 1 -
 * opening problem, 2 - negative length.
 *   fname :	file name.
...
/* ------------ */
{
    ...
    if (!RWC_framehide)
    {
        if (x_mag == 1)
        {
            dd = 0;               /* extra correction for 64 and 128 */
        }
        else
        {
            dd = 1;
...
/* ---------------------------------- */
{
    if (AJ_PseudoColor)
    {
        Load_Next_ind(Image, im_arr, x, y);
    }
    else
    {
...
/* ----------------------------- */
{
    ...
    /** RWCox: 2 Aug 1996 load data from images into "val" array here, instead of in main loop.
     *  this allows loading of 'base' **/
...
/* ------------- */
{
    ...
    if (rot_direct == 1)                            /* clockwise rotation */
    {
        ...
        if (!avr_grp)
        {
            Put_image(Im_Nr);                   /* put single image here */
        }
        /* rotate other data */
        for (n = 0; n < N_im; n++)
        {
        ...
        if (avr_grp)
        {
            Put_image(Im_Nr);           /* put average image after all rot */
        }
    }
...
/* ------------------- */
{
    ...
    case kSC2:
        ...
        for (i = 0; i < k; i++)
        {
            nowim[i] = (int)(((float)nowim[i]) * SCA_ratio + .5);
        }
...
/* ---------------- */
{
                         ...
                         &rx, &ry, &x, &y, &key))
    {
...

        {
            ...
            if (c_im > npoints - Av_length)
            {
                c_im = npoints - Av_length;
            }
            if (im != c_im)
            {
                Im_Nr = c_im;
...
/* AJJ  OK in relation to the root window XGetWindowAttributes(theDisp, theWindow, &wat);
...

{
    ...
    if (RWC_refs == NULL || RWC_voxcor == NULL)
    {
        ...
        if (RWC_voxcor != NULL)
        {
            free_voxel_corr(RWC_voxcor);
        }
        if (RWC_refs != NULL)
        {
            free_references(RWC_refs);
        }
        if (RWC_ideal != NULL)
        {
            RWC_free_time_series(RWC_ideal); RWC_ideal = NULL;
...

{
    ...
    for (i = FIM_first_key ; i <= FIM_last_key ; i++)
    {
        ...
        {
            XMapWindow(theDisp, key[i].wid);                 /* first press */
        }
    }
...

{
    ...
    case kFI3:
        ...
        case 1: {
            ...
            if (RWC_ideal->fname != NULL)
            {
                free(RWC_ideal->fname);
            }
        ...
        case 5: {
            ...
            if (rot_direct == -1 && kFI3_show_ref > 0)
            {
                kFI3_show_ref--;
            }
            if (rot_direct == 0 && kFI3_show_ref < 2)
            {
                kFI3_show_ref++;
            }

            if (kold == kFI3_show_ref)
            {
                XBell(theDisp, 100); return 0;
            }
...
/*** this code is for rescaling the plot to range 0..10000 ***/
        ...
        case 10: {
            ...
            if (LSQ_code < LSQ_NONE)
            {
                LSQ_code = LSQ_LASTCODE;
            }
            if (LSQ_code > LSQ_LASTCODE)
            {
                LSQ_code = LSQ_NONE;
            }
            ...
            old_im = -1;
            if (old_imed != LSQ_imedit[LSQ_code] && Im_Nr < npoints)
            {
                Put_image(Im_Nr);
            }
    ...
    /*** if we get to here, the user changed something, so mark the FIM calculations to be redone,
 ...
 *      from the command line options (in FIM_opt_* variables), OR from the X11 defaults database
 *(in the fim_* variables), OR from the default tables (in overfim.h)
 ***/
...
/*** add a color with the given r,g,b values to the extra_colors table (see routine STD_colors)
 ***/
...
/*------------------------------------------------------------------------
 * Returns position of highest set bit in 'ul' as an integer (0-31), or returns -1 if no bit is set.
 * --------------------------------------------------------------------------*/
/* ------------------------- */
static int
