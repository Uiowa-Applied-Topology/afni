...
/* Functions to deal with the mat44 elements in an AFNI dataset dataxes structure.  Note that the
 * mat44 (4x4 matrix) type is defined in nifti1_io.h, and that the last row of the matrix is ALWAYS
 * [0 0 0 1]. It is to be applied to column vectors of the form [ x y z 1 ]', to define an affine
 * transformation of (x,y,z).
 *
 * The library nifti1_io.c includes nifti1_mat44_inverse() and some other utility functions.
 *
 * -- RWCox - 07 Dec 2005
 * -----------------------------------------------------------------------------*/
...
/*! Compute the mat44 elements of a dataset dataxes struct, given the other elements that define the
 * coordinate system.  Also see function THD_daxes_from_mat44(), which does the reverse.
 *
 *  On input to this function, the XXdel, XXorg, nXX, and to_dicomm elements of dax must have been
 *set already, for XX=xx,yy,zz.  Return value is
...
void THD_set_dicom_box(THD_dataxes *dax)
{
    ...
    if (dax == NULL || !ISVALID_MAT44(dax->ijk_to_dicom))
    {
        return;
    }
...
/*! Given the ijk_to_dicom index to DICOM transformation in the header, AND the nxx,nyy,nzz values,
 * load the legacy dataxes information:
 ...
 *    - to_dicomm = Orthogonal matrix transforming from dataset coordinates to DICOM coordinates
 *
...
int THD_daxes_from_mat44(THD_dataxes *dax)
{
    ...
    /* table to convert NIfTI-1 orientation codes (1..6) into AFNI orientation codes (0..5, and in a
     * different order) */
    ...
    if (dax->nxx < 1 || dax->nyy < 1 || dax->nzz < 1)
    {
        return -1;
    }

    /* use the NIfTI-1 library function to determine best orientation;
     * but, must remember that NIfTI-1 x and y are reversed from AFNI's, so we must negate the x and
     *y rows of the matrix before func call */
    ...
    /* grid offset int i-direction is projection of last column of ijk_to_dicom matrix (the shifts)
     * along the direction of the first column of the matrix (the i-column) */
    ...
    ss = sqrt(xx * xx + yy * yy + zz * zz); if (ss == 0.0f)
    {
        ss = 1.0f;
    }
    dax->xxorg = (xx * aa + yy * bb + zz * cc) / ss;
    if (ORIENT_sign[dax->xxorient] == '-')
    {
        dax->xxorg = -dax->xxorg;
    }
 ...
 *    - ri,rj,rk: new grid spacing along each direction Outputs:
                         ...
                         int *ninew, int *njnew, int *nknew)
{
    ...
    di = MAT33_CLEN(old_mat, 0); if (di == 0.0f)
    {
        di = 1.0f;
    }
    dj = MAT33_CLEN(old_mat, 1); if (dj == 0.0f)
    {
        dj = di;
    }
    dk = MAT33_CLEN(old_mat, 2); if (dk == 0.0f)
    {
        dk = di;
    }
    ...
    if (ri <= 0.0f)
    {
        ri = 1.0f;                /* don't allow negative spacing    */
    }
    if (rj <= 0.0f)
    {
        rj = ri;                  /* along output (i,j,k) directions */
    }
    if (rk <= 0.0f)
    {
        rk = ri;
    ...
    /* copy input matrix to output, then scale the upperleft 3x3 to allow for new spacings */
    ...
    *nknew = (int)rint(nk / fk);
    ...
    /* now compute offset (fourth column) in new_mat so that the input and output grids have the
     * same (x,y,z) centers;
    ...
    /* adjust so that MAT44_VEC(new_mat,fi,fj,fk,...) is same result as
     * MAT44_VEC(old_mat,di,dj,dk,...)    */
