...
/*****************************************************************************
 * The routine that generates a C function for a completely unrolled FFT. Basically the same as
 *csfft_cox() with print statements replacing the actual computations. The resulting code also needs
 *the csfft_trigconsts() function above.
 ******************************************************************************/
    ...
    while (n > m)
    {
        ...
        for (i0 = 0; i0 < m; i0++)
        {
            for (i1 = i0; i1 < n; i1 += i3)
            {
                ...
                {
                    printf(
                        "   f1 = xcx[%d].r * csp[%d].r - xcx[%d].i * csp[%d].i ; /* twiddles */\n",
                        i1 + m, k, i1 + m, k);
