...
static int mybsearch_int(int tt, int nar, int *ar)
{
    ...
    /* at the start of this loop, we've already checked indexes ii and jj, so check the middle of
     * them (kk), and if that doesn't work, make the middle the new ii or the new jj -- so again we
     * will have checked both ii and jj when the loop iterates back. */
    ...
    while (jj - ii > 1)
    {
        ...
        if (nn < 0)
        {
            ii = kk;            /* must be above kk */
        }
        else
        {
            jj = kk;            /* must be below kk */
        }
    }
...
static int string_search(char *targ, int nstr, char **str)
{
    ...
    if (targ == NULL || *targ == '\0' || str == NULL || nstr < 1)
    {
        return -1;
    }

    ttt = strdup(targ); npt = strstr(ttt, ".nii"); if (npt != NULL)
    {
        *npt = '\0';
    }
    for (ii = 0 ; ii < nstr ; ii++)
    {
        if (str[ii] != NULL)
        {
            sss = strdup(str[ii]); npt = strstr(sss, ".nii"); if (npt != NULL)
            {
                *npt = '\0';
            }
            if (strcmp(ttt, sss) == 0)
            {
                free(sss); free(ttt); return ii;
            }
            free(sss);
        }
    }
...
MRI_shindss * GRINCOR_read_input(char *fname)
{
    ...
    if (atr == NULL)
    {
        GQUIT("ndset attribute missing");
    }
    ndset = (int)strtod(atr, NULL);
    if (ndset < 1)
    {
        GQUIT("ndset attribute has illegal value");
    }
     ...
     *   sizeof(datum) * number of vectors per dataset number of datasets sum of per dataset vector
     *lengths */

    nbytes_needed = 0;
    for (ids = 0 ; ids < ndset ; ids++)
    {
        nbytes_needed += nvals[ids];
    }
    ...
    if (atr != NULL)
    {
        ...
        if (nbytes_dfname <= 0 && strstr(dfname, "/") != NULL)
        {
            ...
            if (nbytes_dfname > 0)
            {
                free(dfname); dfname = strdup(tnam);
            }
        }
    }
    ...
    if (nnode)
    {
        if (nnode->num != 2)
        {
            GQUIT("LRpair_nnode must have 2 values");
        }
    ...
    if (ninmask)
    {
        if (ninmask->num != 2)
        {
            GQUIT("LRpair_ninmask must have 2 values");
        }
...
void GRINCOR_meanvec(MRI_shindss *shd, int ids, float *vv)
{
    if (shd->datum == 1)
    {
        GRINCOR_meanvec_sbyte(shd, ids, vv);
    }
    else
    {
        GRINCOR_meanvec_short(shd, ids, vv);
    }
    return;
}
...
void GRINCOR_dotprod_short(MRI_shindss *shd, int ids, float *vv, float *dp)
{
    ...
    if (nvals % 2 == 0) /* even number of samples */
    {
        for (iv = 0 ; iv < nvec ; iv++)
        {
            ...
            {
                sum += vv[ii] * svv[ii] + vv[ii + 1] * svv[ii + 1];
            }
            dp[iv] = sum * fac;
        }
    }
...
void GRINCOR_dotprod_sbyte(MRI_shindss *shd, int ids, float *vv, float *dp)
{
    ...
    if (nvals % 2 == 0) /* even number of samples */
    {
        for (iv = 0 ; iv < nvec ; iv++)
        {
            ...
            for (ii = 0 ; ii < nvals ; ii += 2)
            {
                sum += vv[ii] * bvv[ii] + vv[ii + 1] * bvv[ii + 1];
            }
            dp[iv] = sum * fac;
        }
    }
...
void GRINCOR_dotprod(MRI_shindss *shd, int ids, float *vv, float *dp)
{
    if (shd->datum == 1)
    {
        GRINCOR_dotprod_sbyte(shd, ids, vv, dp);
    }
    else
    {
        GRINCOR_dotprod_short(shd, ids, vv, dp);
    }
    return;
}
...
#pragma omp for
      for (ids = 0 ; ids < ndset ; ids++)
      {
          if (verb > 3)
          {
              fprintf(stderr, " +   start correlation on dataset #%d\n", ids);
          }
    ...
    if (debug || verb > 8)
    {
        ...
        for (ids = 0 ; ids < shd->ndset ; ids++)
        {
            ...
            {
                if (!isfinite(ddp[ids][iv]))
                {
                    ddp[ids][iv] = 0.0f; nbad++;
                }
            }
            if (nbad > 0)
            {
...
void GRINCOR_standardize_vector(int nv, float *vv)
{
    ...
    if (USE_TRANGE)
    {
        ibot = tbot; if (ttop < itop)
        {
            itop = ttop;
        }
    }

    for (vs = 0.0f, ii = ibot ; ii <= itop ; ii++)
    {
        vs += vv[ii];
    }
    vs /= (itop - ibot + 1);
                            ...
                            MCW_cluster *nbhd, int voxijk, float *tsar)
{
    ...
    if (nbhd != NULL)
    {
        ...
        {
            xx = aa + nbhd->i[jj]; if (xx < 0 || xx >= nx)
            {
                continue;
            }
            yy = bb + nbhd->j[jj]; if (yy < 0 || yy >= ny)
            {
                continue;
            }
            zz = cc + nbhd->k[jj]; if (zz < 0 || zz >= nz)
            {
                continue;
            }
...
/* extract, from the dd-th dataset, the nijk vectors node-indexed in vijk[], into array tsar
 * (pre-malloc-ized, please, to nvals*nijk long).
 *//*--------------------------------------------------------------------------*/
...
/*  Replace               With AFNI_NIML_PORT        get_port_named("AFNI_GroupInCorr_NIML");
...
void GI_message(char *mess, int ecode)  /* Apr 2013 */
{
    ...
    if (mess != NULL && *mess != '\0')
    {
        if (nel != NULL)
        {
            NI_set_attribute(nel, "text", mess);
        }
        switch (ecode)
        {
...
int main(int argc, char *argv[])
{
    ...
    while (nopt < argc)
    {
        ...
        if (strcasecmp(argv[nopt], "-clust") == 0)  /* 23 May 2012 [patched 13 Oct 2015] */
        {
            ...
            if (++nopt >= argc)
            {
                ERROR_exit("GIC: need an argument after option '%s'", argv[nopt - 1]);
            }
            cpref = argv[nopt];
            if (*cpref == '\0')
            {
                ERROR_exit("Illegal prefix after option '%s'", argv[nopt - 1]);
            }
            cname = (char *)malloc(sizeof(char) * (strlen(cpref) + 32)); qq = 0;
            sprintf(cname, "%s.NN1_1sided.niml", cpref); clust_NN1_1sided = AFNI_suck_file(cname);
            if (clust_NN1_1sided != NULL)
            {
                qq++;
            }
            sprintf(cname, "%s.NN1_2sided.niml", cpref); clust_NN1_2sided = AFNI_suck_file(cname);
            if (clust_NN1_2sided != NULL)
            {
                qq++;
            }
            sprintf(cname, "%s.NN1_bisided.niml", cpref); clust_NN1_bisided = AFNI_suck_file(cname);
            if (clust_NN1_bisided != NULL)
            {
                qq++;
            }
            sprintf(cname, "%s.NN2_1sided.niml", cpref); clust_NN2_1sided = AFNI_suck_file(cname);
            if (clust_NN2_1sided != NULL)
            {
                qq++;
            }
            sprintf(cname, "%s.NN2_2sided.niml", cpref); clust_NN2_2sided = AFNI_suck_file(cname);
            if (clust_NN2_2sided != NULL)
            {
                qq++;
            }
            sprintf(cname, "%s.NN2_bisided.niml", cpref); clust_NN2_bisided = AFNI_suck_file(cname);
            if (clust_NN2_bisided != NULL)
            {
                qq++;
            }
            sprintf(cname, "%s.NN3_1sided.niml", cpref); clust_NN3_1sided = AFNI_suck_file(cname);
            if (clust_NN3_1sided != NULL)
            {
                qq++;
            }
            sprintf(cname, "%s.NN3_2sided.niml", cpref); clust_NN3_2sided = AFNI_suck_file(cname);
            if (clust_NN3_2sided != NULL)
            {
                qq++;
            }
            sprintf(cname, "%s.NN3_bisided.niml", cpref); clust_NN3_bisided = AFNI_suck_file(cname);
            ...
            else
            {
                ...
                if (clust_NN1_1sided != NULL)
                {
                    ...
                    strcpy(clatr[qq], "AFNI_CLUSTSIM_NN1_1sided ==> ");
                ...
                if (clust_NN1_2sided != NULL)
                {
                    ...
                    strcpy(clatr[qq], "AFNI_CLUSTSIM_NN1_2sided ==> ");
                    strcat(clatr[qq], clust_NN1_2sided); qq++;
                    free(clust_NN1_2sided);
                }
                if (clust_NN1_bisided != NULL)
                {
                    clatr[qq] = (char *)malloc(sizeof(char) * (strlen(clust_NN1_bisided) + 128));
                    strcpy(clatr[qq], "AFNI_CLUSTSIM_NN1_bisided ==> ");
                    strcat(clatr[qq], clust_NN1_bisided); qq++;
                    free(clust_NN1_bisided);
                }
                if (clust_NN2_1sided != NULL)
                {
                    clatr[qq] = (char *)malloc(sizeof(char) * (strlen(clust_NN2_1sided) + 128));
                    strcpy(clatr[qq], "AFNI_CLUSTSIM_NN2_1sided ==> ");
                    strcat(clatr[qq], clust_NN2_1sided); qq++;
                    free(clust_NN2_1sided);
                }
                if (clust_NN2_2sided != NULL)
                {
                    clatr[qq] = (char *)malloc(sizeof(char) * (strlen(clust_NN2_2sided) + 128));
                    strcpy(clatr[qq], "AFNI_CLUSTSIM_NN2_2sided ==> ");
                    strcat(clatr[qq], clust_NN2_2sided); qq++;
                    free(clust_NN2_2sided);
                }
                if (clust_NN2_bisided != NULL)
                {
                    clatr[qq] = (char *)malloc(sizeof(char) * (strlen(clust_NN2_bisided) + 128));
                    strcpy(clatr[qq], "AFNI_CLUSTSIM_NN2_bisided ==> ");
                    strcat(clatr[qq], clust_NN2_bisided); qq++;
                    free(clust_NN2_bisided);
                }
                if (clust_NN3_1sided != NULL)
                {
                    clatr[qq] = (char *)malloc(sizeof(char) * (strlen(clust_NN3_1sided) + 128));
                    strcpy(clatr[qq], "AFNI_CLUSTSIM_NN3_1sided ==> ");
                    strcat(clatr[qq], clust_NN3_1sided); qq++;
                    free(clust_NN3_1sided);
                }
                if (clust_NN3_2sided != NULL)
                {
                    clatr[qq] = (char *)malloc(sizeof(char) * (strlen(clust_NN3_2sided) + 128));
                    strcpy(clatr[qq], "AFNI_CLUSTSIM_NN3_2sided ==> ");
                    strcat(clatr[qq], clust_NN3_2sided); qq++;
                    free(clust_NN3_2sided);
                }
                if (clust_NN3_bisided != NULL)
                {
                    clatr[qq] = (char *)malloc(sizeof(char) * (strlen(clust_NN3_bisided) + 128));
                    strcpy(clatr[qq], "AFNI_CLUSTSIM_NN3_bisided ==> ");
                    strcat(clatr[qq], clust_NN3_bisided); qq++;
                    free(clust_NN3_bisided);
                }
                if (qq > 0 && clust_mask != NULL)
                {
                    clatr[qq] = (char *)malloc(sizeof(char) * (strlen(clust_mask) + 128));
                    strcpy(clatr[qq], "AFNI_CLUSTSIM_MASK ==> "); strcat(clatr[qq], clust_mask);
                    qq++;
                    free(clust_mask);
                }
                if (verb)
                {
                    INFO_message("%d -clust attributes read for use in AFNI", qq);
                }
            }
            nopt++; continue;
        }
        ...
        if (strcasecmp(argv[nopt], "-batchGRID") == 0) /* Aug 2012 */
        {
            if (bmode)
            {
                ERROR_exit("GIC: can't use '-batch' twice!");
            }
            if (++nopt >= argc + 1)
            {
                ERROR_exit("GIC: need 2 arguments after option '%s'", argv[nopt - 1]);
            }
        ...
        if (strcasecmp(argv[nopt], "-batch") == 0) /* Feb 2011 */
        {
            ...
            if (strcasecmp(argv[nopt], "IJK") == 0)
            {
                bmode = IJK_MODE;
            }
            else if (strcasecmp(argv[nopt], "IJKAVE") == 0)
            {
                bmode = IJK_MODE;
            }
            else if (strcasecmp(argv[nopt], "XYZ") == 0)
            {
                bmode = XYZ_MODE;
            }
            else if (strcasecmp(argv[nopt], "XYZAVE") == 0)
            {
                bmode = XYZ_MODE;
            }
            else if (strcasecmp(argv[nopt], "IJKPV") == 0)
            {
                bmode = IJKPV_MODE;
            }
            else if (strcasecmp(argv[nopt], "XYZPV") == 0)
            {
                bmode = XYZPV_MODE;
            }
            else if (strcasecmp(argv[nopt], "MASKAVE") == 0)
            {
                bmode = MASKAVE_MODE;
            }
            else if (strcasecmp(argv[nopt], "MASKPV") == 0)
            {
                bmode = MASKPV_MODE;
            }
            else if (strcasecmp(argv[nopt], "VECTORS") == 0)
            {
                bmode = VECTORS_MODE;
            }
            else if (strcasecmp(argv[nopt], "NODE") == 0)
            {
                bmode = NODE_MODE;
            }
            else
            {
                ERROR_exit("GIC: don't understand '-batch' method '%s'", argv[nopt]);
            }
        ...
        if (strcasecmp(argv[nopt], "-seedrad") == 0)
        {
            if (++nopt >= argc)
            {
                ERROR_exit("GIC: need 1 argument after option '%s'", argv[nopt - 1]);
            }
...
#if 0 /* This is now handled in AFNI_prefilter_args(). ZSS, June 2011 Delete soon.
       *                                                */
        ...
        if (strcasecmp(argv[nopt], "-read") == 0) /* 09 Feb 2016 */
        {
            ...
            {
                WARNING_message(
                    "Option '-read' given AFTER '-setA' => '-read' is ignored for '-setA'");
            }
            if (shd_BBB != NULL)
            {
                WARNING_message(
                    "Option '-read' given AFTER '-setB' => '-read' is ignored for '-setB'");
            }
        ...
        if (strcmp(argv[nopt], "-center") == 0) /* 15 Jul 2011 */
        {
            ...
            else if (strcasecmp(argv[nopt], "VALS") == 0)
            {
                ...
                if (++nopt > argc)
                {
                    ERROR_exit("Need a second argument after '%s %s'", argv[nopt - 2],
                               argv[nopt - 1]);
                }
        ...
        if (strcasecmp(argv[nopt], "-covariates") == 0) /* 20 May 2010 */
        {
            ...
            if (covnel != NULL)
            {
                ERROR_exit("GIC: can't use -covariates twice!");
            }
            ...
            if (covnel == NULL)
            {
                ...
                ERROR_message(
                    "GIC: Try re-running this program with the extra option -DAFNI_DEBUG_TABLE=YES");
            ...
            if (nbad > 0)
            {
                ERROR_exit("GIC: Cannot continue :-(");
            }
            nopt++; continue;
        }
        ...
        if (strcasecmp(argv[nopt], "-scale") == 0) /* 19 Sep 2012 */
        {
            ...
            if (sclnel != NULL)
            {
                ERROR_exit("GIC: can't use -scale twice!");
            }
        ...
        if (strcasecmp(argv[nopt], "-useA") == 0)
        {
            ...
            if (use_AAA != NULL)
            {
                ERROR_exit("GIC: you can't use -useA twice!");
            }
        ...
        if (strcasecmp(argv[nopt], "-useB") == 0)
        {
            ...
            if (use_BBB != NULL)
            {
                ERROR_exit("GIC: you can't use -useB twice!");
            }
        ...
        if (strcasecmp(argv[nopt], "-setA") == 0)
        {
            ...
            if (shd_AAA != NULL)
            {
                ERROR_exit("GIC: can only use '-setA' once!");
            }
            if (++nopt >= argc)
            {
                ERROR_exit("GIC: need 1 argument after option '%s'", argv[nopt - 1]);
            ...
            else if (STRING_HAS_SUFFIX(fname, ".grpincorr"))
            {
                ...
                if (verb)
                {
                    INFO_message("GIC: Added '.niml' to end of -setA filename");
                }
            }
            else if (STRING_HAS_SUFFIX(fname, ".grpincorr."))
            {
            ...
            if (shd_AAA == NULL)
            {
                ERROR_exit("GIC: Cannot continue after -setA input error");
            }
            if (verb)
            {
                INFO_message("GIC: -setA opened, contains %d datasets, %d time series, %s bytes",
                             shd_AAA->ndset, shd_AAA->nvec,
                             commaized_integer_string(shd_AAA->nbytes));
            }
            qlab_AAA = fname;
            cpt      = strchr(qlab_AAA, '.'); if (cpt != NULL && cpt != qlab_AAA)
            {
                *cpt = '\0';
            }
            nopt++; continue;
        ...
        if (strcasecmp(argv[nopt], "-setB") == 0)
        {
            ...
            if (shd_BBB != NULL)
            {
                ERROR_exit("GIC: can only use '-setB' once!");
            }
            if (++nopt >= argc)
            {
                ERROR_exit("GIC: need 1 argument after option '%s'", argv[nopt - 1]);
            ...
            else if (STRING_HAS_SUFFIX(fname, ".grpincorr"))
            {
                ...
                if (verb)
                {
                    INFO_message("GIC: Added '.niml' to end of -setB filename");
                }
            }
            else if (STRING_HAS_SUFFIX(fname, ".grpincorr."))
            {
            ...
            if (shd_BBB == NULL)
            {
                ERROR_exit("GIC: Cannot continue after -setB input error");
            }
            if (verb)
            {
                INFO_message("GIC: -setB opened, contains %d datasets, %d time series, %s bytes",
                             shd_BBB->ndset, shd_BBB->nvec,
                             commaized_integer_string(shd_BBB->nbytes));
            }
            qlab_BBB = fname;
            cpt      = strchr(qlab_BBB, '.'); if (cpt != NULL && cpt != qlab_BBB)
            {
                *cpt = '\0';
            }
            nopt++; continue;
    ...
    if (do_nocov && mcov == 0)
    {
        do_nocov = 0;                                /* 17 May 2012 */
    }
    if (do_spcov && mcov == 0)
    {
        do_spcov = 0;              /* 28 Nov 2012 */
    }
    ...
    if (seedrad == 0.0f && (bmode == IJKPV_MODE || bmode == XYZPV_MODE))
    {
        ...
        if (bmode == IJKPV_MODE)
        {
            bname = "IJKAVE"; bmode = IJK_MODE;
        }
        else
        {
            bname = "XYZAVE"; bmode = XYZ_MODE;
        }
    ...
    else if (shd_BBB->ndset != shd_AAA->ndset)
    {
        ttest_opcode_max = 1;
    }
    ...
    if (sclnel != NULL)
    {
        ...
        if (nbad)
        {
            ERROR_exit("GIC: Can't continue after such simple misteaks :-(");
        }
        if (verb)
        {
            INFO_message("GIC: Setting up scale factors for datasets");
        }
...
#define BXX(i, j)    bxx[(i) + (j) * (nB)] /* i=0..nB-1 , j=0..mcov */
    ...
    if (mcov > 0)
    {
        ...
        if (verb)
        {
            INFO_message("GIC: Setting up regression matrices for covariates");
        }
        ...
        case CENTER_DIFF: {
            ...
            {
                for (sum = 0.0f, kk = 0 ; kk < shd_AAA->ndset ; kk++)
                {
                    sum += AXX(kk, jj);
                }
            ...
            if (shd_BBB != NULL && ttest_opcode != 2) /* unpaired */
            {
                ...
                for (jj = 1 ; jj <= mcov ; jj++) /* average the columns */
                {
                    for (sum = 0.0f, kk = 0 ; kk < shd_BBB->ndset ; kk++)
                    {
                        sum += BXX(kk, jj);
                    }
        ...
        case CENTER_DIFF_MEDIAN: { /* 23 May 2012 */
            float *qar; int nqar;
            nqar = shd_AAA->ndset;
            if (shd_BBB != NULL && shd_BBB->ndset > nqar)
            {
                nqar = shd_BBB->ndset;
            }
        ...
        case CENTER_SAME_MEDIAN: { /* 23 May 2012 */
            ...
            for (jj = 1 ; jj <= mcov ; jj++) /* average the columns */
            {
                ...
                for (kk = 0 ; kk < shd_BBB->ndset ; kk++)
                {
                    qar[kk + shd_AAA->ndset] = BXX(kk, jj);
                }
        ...
        if (impr == NULL)
        {
            ERROR_exit("GIC: Can't process setA covariate matrix?! :-(");
        }
        ...
        if (shd_BBB != NULL && ttest_opcode != 2) /* un-paired case */
        {
            ...
            if (impr == NULL)
            {
                ERROR_exit("GIC: Can't process setB covariate matrix?! :-(");
            }
    ...
    /*--- scan through all the data, which will make it be page faulted into RAM, which will make
     * the correlation-izing process faster;
     *    the downside is that this may take quite a while, which is boring, but it's better to wait
     *now than to wait for the 1st result, IMHO ---*/
...
#define BSTEP    64
    if (!shd_AAA->did_read || (shd_BBB != NULL && !shd_BBB->did_read))
    {
        ...
        if (verb)
        {
            INFO_message("GIC: page faulting (reading) data into memory");
        }
        if (!shd_AAA->did_read)
        {
            if (shd_AAA->datum == 1)
            {
                qv = (char *)shd_AAA->bv[0];
            }
            else
            {
                qv = (char *)shd_AAA->sv[0];
            }
            if (verb)
            {
            ...
            {
                sum += *qv;
                if (verb && (pp / BSTEP) % vstep == vstep - 1)
                {
                    vstep_print();
                }
            }
        ...
        if (shd_BBB != NULL && !do_apair && !shd_BBB->did_read)
        {
            if (shd_BBB->datum == 1)
            {
                qv = (char *)shd_BBB->bv[0];
            }
            else
            {
                qv = (char *)shd_BBB->sv[0];
            }
            if (verb)
            {
        ...
        if (verb == 666)
        {
            INFO_message(" data sum = %g", sum);            /* e.g., never */
        }
    }
#undef BSTEP
    ...
    if (do_ztest || verb > 8)   /* 11 Feb 2016 */
    {
        ...
        if (shd_AAA->datum == 1)
        {
            for (pp = 0 ; pp < shd_AAA->ndset ; pp++)
            {
                ...
                for (qq = 0 ; qq < nqq && bv[qq] == 0 ; qq++)
                {
                    ;                                 /*nada*/
                }
                if (qq == nqq)
                {
                    WARNING_message("-setA dataset #%d is all zero :-(", pp); nzz++;
                }
            }
        }
        else
        ...
        if (shd_BBB != NULL && !do_apair)
        {
            if (shd_BBB->datum == 1)
            {
                for (pp = 0 ; pp < shd_BBB->ndset ; pp++)
                {
                    ...
                    if (qq == nqq)
                    {
                        WARNING_message("-setB dataset #%d is all zero :-(", pp); nzz++;
                    }
                }
            }
            else
    ...
    if (!bmode)            /* not in batch mode */
    {
        ...
        /* loop until AFNI connects (answers the call), printing '.' every so often to keep the user
         * mollified and distracted */
        ...
        if (kk <= 0)
        {
            fprintf(stderr, " ** Connection times out :-(\n"); exit(1);
        }
    }
    ...
    bricklabels = (char *)calloc(sizeof(char),
                                 (5 * MAX_LABEL_SIZE + 16) * (nout + nsaar + nspcov + 1));
    ...
    if (bricklabels[kk - 1] == ';')
    {
        bricklabels[kk - 1] = '\0';                           /* truncate last ';' */
    }
    ...
    if (do_sendall)
    {
        ...
        for (kk = 0 ; kk < ndset_AAA ; kk++)
        {
            if (shd_AAA->dslab != NULL)
            {
                sprintf(buf, "A_%.10s", shd_AAA->dslab[kk]);
            }
            else
            {
                sprintf(buf, "%.9s#%02d", label_AAA, kk);
            }
        ...
        {
            if (shd_BBB->dslab != NULL)
            {
                sprintf(buf, "B_%.10s", shd_BBB->dslab[kk]);
            }
            else
            {
                sprintf(buf, "%.9s#%02d", label_BBB, kk);
            }
            sprintf(bricklabels + strlen(bricklabels), "%s_zcorr ; ", buf);
        }
    ...
    /* ZSS: set surface attributes [note Ziad's TERRIBLE use of spaces] Perhaps, but   at least he
     * wraps at 80             . */
...

    {
        if (verb > 1)
        {
            INFO_message("GIC: Sending setup information to %s", pname);
        }
        ...
        if (covimar != NULL && TalkToAfni)
        {
            ...
            if (verb > 1)
            {
                INFO_message("GIC: Sending covariate 1D vectors to %s", pname);
            }
            for (jj = 0 ; jj < IMARR_COUNT(covimar) ; jj++)
            {
                ...
                if (nn < 0)
                {
                    ERROR_exit("GIC: Can't send 1D vector to %s :-(", pname);
                }
...
#pragma omp parallel
    ...
    if (ctnam != NULL && strncmp(ctnam, "1D:", 3) == 0)
    {
        ...
        if (ctimm != NULL && ctimm->nx >= shd_AAA->ndset)
        {
            ctarA = MRI_FLOAT_PTR(ctimm);
        }
    }
    ...
    {
        MRI_IMAGE *ctimm = mri_1D_fromstring(ctnam);
        if (ctimm != NULL && ctimm->nx >= shd_BBB->ndset)
        {
            ctarB = MRI_FLOAT_PTR(ctimm);
        }
    }
#endif
...

    {
...

        {
            ...
            if (nelcmd == NULL)
            {
                ...
                if (kk < 1)
                {
                    ...
                    NI_sleep(111);                                    /* give AFNI a moment to do
                                                                       * whatever */
                    GI_stream = NI_stream_open(nsname, "w");
...

        {
            ...
            if (bmode == LIST_MODE) /* Aug 2012 -- special case */
            {
                ...
                int         ix, jy, kz;
                if (blist_index >= blist_num)
                {
                    goto GetOutOfDodge;                       /* done */
                }
            ...
            case IJK_MODE: { /* i j k */
                ...
                {
                    float fi, fj, fk;
                    x = (float)strtod(bsar->str[1], NULL); if (do_lpi)
                    {
                        x = -x;
                    }
                    y = (float)strtod(bsar->str[2], NULL); if (do_lpi)
                    {
                        y = -y;
                    }
                    ...
                    k < 0 || k >= nz)
                {
                    if (bmode == IJK_MODE || bmode == IJKPV_MODE)
                    {
                        ERROR_message("GIC: bad batch command line: %s (%d,%d,%d) illegal", bname,
                                      i, j, k);
                    }
                    else
                    {
                        ...
                        ERROR_message("GIC: bad batch command line: %s (%g,%g,%g) illegal", bname,
                                      x, y, z);
                    }
                    goto LoopBack;
                }
                qijk = THREE_TO_IJK(i, j, k, nx, nxy);
                if (bmode == XYZPV_MODE || bmode == IJKPV_MODE)
                {
                    cname = "SETREF_ijk_pv";
                }
                else
            ...
            case MASKAVE_MODE: {
                ...
                if (mset == NULL)
                {
                    ERROR_message("GIC: bad batch command line: can't open mask dataset '%s'",
                                  bsar->str[1]);
                    goto LoopBack;
                }
                if (shd_AAA->nvox != DSET_NVOX(mset))
                {
                    ERROR_message("GIC: bad batch command line: mask dataset '%s' is wrong size",
                                  bsar->str[1]);
                ...
                if (!DSET_LOADED(mset))
                {
                    ERROR_message("GIC: bad batch command line: can't load dataset '%s'",
                                  bsar->str[1]);
                    DSET_delete(mset); goto LoopBack;
                }
                ...
                if (nmask == 0)
                {
                    ERROR_message("GIC: bad batch command line: dataset '%s' mask is all zero",
                                  bsar->str[1]);
                    free(mask); goto LoopBack;
                }
                if (bmode == MASKPV_MODE && nmask > 1)
                {
                    cname = "SETREF_listijk_pv";
                }
                else
                {
                    cname = "SETREF_listijk";
                }
            ...
            case VECTORS_MODE: { /* 1Dfilename */
                ...
                if (vim == NULL)
                {
                    ERROR_message("GIC: bad batch command line: can't open 1D file '%s'",
                                  bsar->str[1]);
                    goto LoopBack;
                }
                ...
                {
                    ERROR_message(
                        "GIC: bad batch command line: 1D file '%s' is %d X %d, but should be at least %d X %d",
                ...
                {
                    jar = iar + kk * ntim;
                    for (ii = 0 ; ii < shd_AAA->nvals[kk] ; ii++)
                    {
                        var[qq++] = jar[ii];
                    }
                }
                for (kk = 0 ; kk < ndset_BBB ; kk++)
                {
                    ...
                    for (ii = 0 ; ii < shd_BBB->nvals[kk] ; ii++)
                    {
                        var[qq++] = jar[ii];
                    }
                }
...
BatchFinalize:
            ...
            if (verb > 2)
            {
                INFO_message("GIC: generated %s command", nelcmd->name);
            }
        ...
        if (strncmp(nelcmd->name, "SETAPAIR_ijk", 12) == 0)
        {
            ...
            if (atr == NULL)
            {
                atr = NI_get_attribute(nelcmd, "node");
            }
            if (atr == NULL)
            {
                atr = NI_get_attribute(nelcmd, "ijk");
            }
            ...
            if (verb > 1)
            {
                ININFO_message("GIC:  dataset index=%d  node index=%d ** Apair **", voxijkB,
                               voxindB);
            }
            if (voxindB < 0)
            {
        ...
        if (strncmp(nelcmd->name, "SETREF_ijk", 10) == 0)
        {
            ...
            if (do_apair) /* Apr 2013: allow change of Apair location at same time */
            {
                ...
                if (atr == NULL)
                {
                    atr = NI_get_attribute(nelcmd, "node_apair");
                }
                if (atr == NULL)
                {
                    atr = NI_get_attribute(nelcmd, "ijk_apair");
                }
                if (atr != NULL)
                {
                    voxijkB = (int)strtod(atr, NULL);
        ...
        else if (strcmp(nelcmd->name, "SETREF_vectors") == 0)
        {
...

            {
                ...
                {
                    for (ii = 0 ; ii < nvals_AAA[kk] ; ii++, jj++)
                    {
                        seedvec_AAA[kk][ii] = cv[jj];
                    }
                }
                for (kk = 0 ; kk < ndset_BBB ; kk++)
                {
                    for (ii = 0 ; ii < nvals_BBB[kk] ; ii++, jj++)
                    {
                        seedvec_BBB[kk][ii] = cv[jj];
                    }
                }
            }
...

            {
                ...
                for (kk = 0 ; kk < ndset_AAA ; kk++)
                {
                    ...
                    for (ii = 0 ; ii < nvals_AAA[kk] ; ii++)
                    {
                        seedvec_AAA[kk][ii] = cv[ii];
                    }
                }
                for (kk = 0 ; kk < ndset_BBB ; kk++)
                {
                    cv = (float *)nelcmd->vec[kk + ndset_AAA];
                    for (ii = 0 ; ii < nvals_BBB[kk] ; ii++)
                    {
                        seedvec_BBB[kk][ii] = cv[ii];
                    }
                }
            }
        ...
        else if (strncmp(nelcmd->name, "SETREF_listijk", 14) == 0)
        {
            ...
            if (verb > 2)
            {
                ININFO_message("GIC:  %s: %d good vectors in list", bname, nijk);
            }
        ...
        if (verb > 3)
        {
            ININFO_message("GIC:  start correlation-izing for %s", label_AAA);
        }
        ...
        if (shd_BBB != NULL && redoB)
        {
            if (verb > 3)
            {
                ININFO_message("GIC:  start correlation-izing for %s", label_BBB);
            }
        ...
        if (ctarA != NULL)
        {
            for (kk = 0 ; kk < ndset_AAA ; kk++)
            {
                for (jj = nvec / 2 ; jj < nvec ; jj++)
                {
                    dotprod_AAA[kk][jj] += ctarA[kk];
                }
            }
        }
        if (ctarB != NULL)
...

        {
            ...
            if (dosix)
            {
                if (verb > 3)
                {
                    ININFO_message("GIC:  start 1-sample t-test-izing for %s", label_AAA);
                }
                GRINCOR_many_ttest(nvec, ndset_AAA, dotprod_AAA,
                                   0, NULL, neldar_AAA, nelzar_AAA);
                if (verb > 3)
                {
                    ININFO_message("GIC:  start 1-sample t-test-izing for %s", label_BBB);
                }
...

        {
            ...
            if (do_spcov)
            {
                if (verb > 3)
                {
                    ININFO_message("GIC:  start Spearman-ization");
                }
        ...
        if (!bmode && do_shm > 0 && strcmp(afnihost, "localhost") == 0 && !shm_active)
        {
            ...
            /* using nport in nsnew below is no longer necessary, but it does not hurt. ZSS June
             * 2011               */
        ...
        if (!bmode)
        {
            if (verb > 3)
            {
                ININFO_message("GIC:  sending results to %s", pname);
            }
        ...
        if (do_apair)
        {
            redoB = 0; /* Apr 2013 -- don't redo setB unless needed */
        }
...
/** The following routines are for the t-to-z conversion, and are adapted from mri_stats.c to be
 * parallelizable (no static data).
 * =========================================================================*/
 ...
 *           - numy = 0 ==> a 1 sample test of first sample against mean=0 DISABLED   - numy = 1 ==>
 * a 1 sample test of first sample against mean=yar[0]
...
float_pair ttest_toz(int numx, float *xar, int numy, float *yar, int opcode)
{
...

    {
        ...
        for (ii = 0 ; ii < numx ; ii++)
        {
            val = xar[ii] - yar[ii] - avx; sdx += val * val;
        }
        if (sdx > 0.0f)
        {
            tstat = avx / sqrtf(sdx / ((numx - 1.0f) * numx));
        }
        else if (avx > 0.0f)
        {
            tstat = 19.0f;
 ...
 *    0 ==> unpaired, pooled variance 1 ==> unpaired, unpooled variance (not yet implemented) 2 ==>
 * paired (numA==numB required)
 *
 *  xA      = numA X (mcov+1) matrix -- in column-major order psinvA  = (mcov+1) X numA matrix -- in
 *column-major order xtxinvA = (mcov+1) X (mcov+1) matrix = inv[xA'xA]
 *//*-------------------------------------------------------------------------*/
                 ...
                 float *outvec, float *workspace)
{
    ...
    if (testA || testAB)
    {
        ...
        for (jj = 0 ; jj < nA ; jj++)
        {
            ...
            for (ii = 0 ; ii < mm ; ii++)
            {
                val += XA(jj, ii) * betA[ii];
            }
            zdifA[ii] = val; ssqA += val * val;
        }
        if (testA)
        {
            varA = ssqA / (nA - mm); if (varA <= 0.0f)
            {
                varA = VBIG;
            }
        }
    ...
    if (testB || testAB)
    {
        ...
        for (jj = 0 ; jj < nB ; jj++)
        {
            ...
            for (ii = 0 ; ii < mm ; ii++)
            {
                val += XB(jj, ii) * betB[ii];
            }
            zdifB[ii] = val; ssqB += val * val;
        }
        if (testB)
        {
            varB = ssqB / (nB - mm); if (varB <= 0.0f)
            {
                varB = VBIG;
            }
        }
    ...
    if (testAB)
    {
        ...
        {
            varAB = (ssqA + ssqB) / (nA + nB - 2 * mm); if (varAB <= 0.0f)
            {
                varAB = VBIG;
            }
...
GICOR_setup * GRINCOR_setup_dataset(NI_element *nel)
{
    ...
    atr = NI_get_attribute(nel, "geometry_string"); if (atr == NULL)
    {
        return NULL;
    }
    ...
    dset = EDIT_geometry_constructor(atr, pre);    if (dset == NULL)
    {
        return NULL;
    }
    ...
    else                                        /* make index list of voxels */
    {
        ...
        if (atr != NULL)
        {
            ii = (int)strtod(atr, NULL); nn = MIN(nn, ii);
        }
        giset->nivec = nn;
    ...
    { ATR_string *aatr; int nn;
      ...
      for (nn = 0 ; ; nn++)
      {
          ...
          if (nnatr == NULL || *nnatr == '\0')
          {
              break;
          }
          cpt = strstr(nnatr, " ==> ");
          if (cpt == NULL || cpt == nnatr || cpt - nnatr > 256)
          {
              continue;
          }
    ...
    if (sdsetv[0] == NULL)  /* create output sets */
    {
        for (i = 0; i < 2; ++i)
        {
            if (targetv[i])
            {
                ...
                for (ii = 0; ii < SDSET_VECLEN(sdsetv[i]); ++ii)
                {
                    Ti[ii] = ii;
                }
...
/*! Rank-order a float array, with ties getting the average rank. The output overwrites the input.
...
/*! To Spearman (rank-order) correlate x[] with r[], first do rv = spearman_rank_prepare(n,r) ;
 *  then corr = spearman_rank_corr(n,x,rv,r) ;
...
static float spearman_correlate(int n, float *x, float rv, float *r, int *b, float *c)
{
    ...
    xv = spearman_prepare(n, x, b, c); if (xv <= 0.0f)
    {
        return 0.0f;
    }
    for (ii = 0, ss = 0.0f ; ii < n ; ii++)
    {
        ss += x[ii] * r[ii];
    }
