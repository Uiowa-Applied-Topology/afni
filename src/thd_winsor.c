                             ...
                             int keep_zero, int clipval, byte *mask)
{
    ...
    if (inset == NULL || DSET_BRICK_TYPE(inset, 0) != MRI_short)
    {
        return NULL;
    }
    DSET_load(inset);
    if (DSET_BRICK_ARRAY(inset, 0) == NULL)
    {
        return NULL;
    }
    ...
    for (krep = 0 ; krep < nrep ; krep++)
    {
        ...
        for (kk = 0 ; kk < nz ; kk++) /* loops over 3D voxel indices */
        {
            for (jj = 0 ; jj < ny ; jj++)
            {
                ...
                {
                    if (mask != NULL && !mask[ijk])
                    {
                        shout[ijk] = shin[ijk]; continue;
                    }
    ...
    KILL_CLUSTER(cl);
    free(tmp);
    if (shin != DSET_BRICK_ARRAY(inset, 0))
    {
        free(shin);
    }
...
static void qsrec_sh(int n, short *ar, int cutoff)
{
    ...
    while (mst > 0)
    {
        ...
        if ((i - left) > cutoff)
        {
            stack[mst++] = left; stack[mst++] = i - 1;
        }
        if ((right - i) > cutoff)
        {
            stack[mst++] = i + 1; stack[mst++] = right;
        }
