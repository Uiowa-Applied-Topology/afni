...
/*! For each cluster of points in clustar, extract the average time series from dataset dset.  Put
 * the results into an nt X nc float image, where nt = number of dataset bricks, nc = number of
 * clusters.
 * -------------------------------------------------------------------------*/
                                       ...
                                       THD_3dim_dataset *dset)
{
    ...
    if (clust == NULL || !ISVALID_DSET(dset))
    {
        RETURN(NULL);
    }
