...
/***************************************************************************
 * Routines to open and initialize plugins.  These should only be called at the very end of AFNI
 *initialization, since they will call each plugin's internal initializer function, which may make
 *requests to get AFNI data structures.
 ****************************************************************************/
...
AFNI_plugin_array * PLUG_get_all_plugins(char *dname)
{
    ...
    {
        fname = rlist->ar[ir]; if (fname == NULL)
        {
            continue;
        }
        if (strstr(fname, "plug") == NULL)
        {
            continue;
        }
        if (strstr(fname, "plug_nlfit.") != NULL)
        {
            continue;                                      /* 13 Jul 2020 */
        }
...
AFNI_plugin * PLUG_read_plugin(char *fname)
{
    ...
    do {
        ...
        if (nin == 0)
        {
            strcpy(plin->seqcode, plint->seqcode);               /* 06 Aug 1999 */
        }
        nin++;
    } while (plint != NULL);
...
/*--------------------------------------------------------------------
 * Routine to read in all plugins in the desired list of directories 29 Mar 2001: pname = argv[0] =
 ...
 **/
    ...
    if (_dyld_present() == 0)
    {
        RETURN(NULL);                         /* 05 Sep 2001: Mac OSX */
    }
#endif
    ...
    if (epath == NULL && pname != NULL && strchr(pname, '/') != NULL) /* 29 Mar 2001 */
    {
        ...
        *tp = '\0';
        if (strlen(ep) > 0)
        {
            epath = ep;                   /* got some path */
        }
        else
        {
            free(ep);                     /* got zipperoni */
        }
    }
    ...
    do{
        ...
        if (ii < 1 || id < 1)
        {
            break;                                        /* none --> end of work */
        }
        epos += id;                                       /* char after last scanned */

        if (!THD_is_directory(ename))
        {
            continue;                                     /* 21 May 2002 -rcr */
        }
        /* 02 Feb 2002: did we check this one already? */

        for (ii = 0 ; ii < qlist->num ; ii++)
        {
            if (THD_equiv_files(qlist->ar[ii], ename))
            {
                break;
            }
        }
 ...
 *  2) Use "add_option_to_PLUGIN_interface" to create an option line in the AFNI interface menu.
 *There is no built-in limit to the number of option lines that may be added to an AFNI interface
 *menu.
 *  2(abcdef) Use "add_number_to_PLUGIN_interface"  , and
 *         "add_string_to_PLUGIN_interface"  , and
 *         "add_dataset_to_PLUGIN_interface" , et cetera, to add control parameter choosers to the
 *most recently created option line.  Up to 6 choosers may be added to an option line.
 ...
 *
 * label       = C string to go on the button that activates this interface (will be truncated to 15
 *characters)
 *
 * description = C string to go on the interface control panel popped-up when the button above is
 *pressed -- this has no meaning for call_type = PLUGIN_CALL_IMMEDIATELY
 *
 * help        = C string to be popped up if the user presses "Help" on the interface control panel
 * -- this has no meaning for call_type = PLUGIN_CALL_IMMEDIATELY. If this is NULL, then there will
 *be no help available.
 *
 * call_type   = int that describes how the plugin is to be called from AFNI:
 *                 PLUGIN_CALL_IMMEDIATELY means to call APL_main as soon as the activating button
 *is pressed;
 *                 PLUGIN_CALL_VIA_MENU means to have AFNI popup a menu to control the input
 *parameters passed to APL_main.
 *
 * call_func   = routine that AFNI should call when the user activates this plugin.  The routine
 *will be passed the the pointer "PLUGIN_interface *" created herein, which can be interrogated with
 *the get_*_from_PLUGIN_interface routines.  The call_func should return a "char *", which is NULL
 *if everything is OK, and points to an error message that AFNI will display if something bad
 *happened.
 *
 * The value returned is the pointer to the new interface struct.
 *
 * Note that the three input strings are copied by AFNI, and so could be freed after this routine
 *returns.
 * ----------------------------------------------------------------------------*/
...
void PLUTO_set_runlabels(PLUGIN_interface *plint, char *rlab, char *dlab)
{
    ...
    if (rlab != NULL)
    {
        MCW_strncpy(plint->run_label, rlab, PLUGIN_LABEL_SIZE);
    }
    if (dlab != NULL)
    {
        MCW_strncpy(plint->doit_label, dlab, PLUGIN_LABEL_SIZE);
    }
    return;
}
...
void PLUTO_set_butcolor(PLUGIN_interface *plint, char *sq)
{
    ...
    if (strncmp(sq, "hot", 3) == 0)
    {
        sq = MCW_hotcolor(NULL);
    }
 ...
 *
 * plint      = PLUGIN_interface * which will have the option added label     = C string to be
 *displayed in the menu describing this option tag       = C string to be passed to the plugin when
 *this option is used mandatory = TRUE  (1)  if this option is always passed to the plugin FALSE (0)
 *if the user may or may not select this option MAYBE (-1) if it is to be turned on, but it can be
 *turned off
 * -----------------------------------------------------------------------------*/
...
/*-------------------------------------------------------------------------
 * Routine to add a number-type "chooser" to the most recently created option within a plugin
 *interface.  "Numbers" are always passed to the plugin in float format, but are specified here
 *using an integer range with a decimal shift, sort of like the Motif Scale Widget.
 *
 * label     = C string to go in the menu, next to the "chooser" for the integer.
 *
 * bot, top, = Smallest and largest integer values allowed in the chooser.
 * decim     = Number of decimals to shift to left for display of value. For example, bot=1 top=100
 *with decim=2 will actually specify a range of 0.01 to 1.00.  One function of decim is simply to
 *set the increments between values in the chooser.  With decim=2, the increment is 0.01.
 *
 * defval    = Integer value for the default.  For example, if decim=2, and defval=43, then the
 *floating point default value (as it appears to the user) is actually 0.43.
 *
 * editable  = TRUE (1) if the user will be allowed to type in any value in the chooser.  For
 *example, this would allow the input of 0.43721. FALSE (0) if the user is restricted to the range
 *of values given by bot/10**decim to top/10**decim, in steps of 1/10**decim.
 * ---------------------------------------------------------------------------*/
                                    ...
                                    int defval, int editable)
{
    ...
    if (plint == NULL || plint->option_count == 0)
    {
        EXRETURN;
    }
...
/*-------------------------------------------------------------------
 * Routine to add a string-type "chooser" to the most recently created option within a plugin
 *interface.
 *
 * label     = C string to go in the menu, next to the "chooser" for the string.
 *
 * num_str   = Count of the number of strings provided in strlist.
 * strlist   = strlist[i] is a pointer to the i'th string value that the user is to choose from, for
 *i=0...num_str-1.
 *
 * defval    = If num_str > 0:
 *               Integer from 0...num_str-1 indicating which string in strlist is the default value.
 *If num_str == 0:
 *               Gives width of field supplied for string input.
 *
 * Note that if num_str is <= 0, then instead of being presented with a menu of fixed strings, the
 *user will have to type in a string.
 * ---------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------
 * Routine to add a dataset "chooser" to the most recently created option within a plugin interface.
 *
 * label = C string to go in the menu, next to the "chooser" for the dataset.
 *
 * The _mask inputs are bitwise ORs (|) of dataset type masks. These are used to specify the types
 *of datasets that can be passed to the plugin.  The first two _mask inputs below cannot both be
 *zero (for then no dataset would be allowed into the plugin!).
 *
 * anat_mask = Chosen from the list in 3ddata.h, which is currently ANAT_SPGR_MASK   ANAT_FSE_MASK
 *ANAT_EPI_MASK ANAT_MRAN_MASK   ANAT_CT_MASK    ANAT_SPECT_MASK ANAT_PET_MASK    ANAT_MRA_MASK
 *ANAT_BMAP_MASK ANAT_DIFF_MASK   ANAT_OMRI_MASK and ANAT_ALL_MASK, which will allow any anatomical
 *dataset.  Entering 0 for this mask will mean that no anatomical datasets will be choosable.
 *
 * func_mask = Similar mask for functional dataset types, chosen from FUNC_FIM_MASK   FUNC_THR_MASK
 *FUNC_COR_MASK   FUNC_TT_MASK and FUNC_ALL_MASK, which will allow any functional dataset. Entering
 *0 for this mask will mean that no functional datasets will be choosable.
 *
 * ctrl_mask = An additional mask to specify further exactly which datasets should be choosable.
 *Mask options are:
 *
 *             SESSION_ALL_MASK    = If this is set, then the choice of datasets will be drawn from
 *all sessions now loaded into AFNI. By default, only the "current"
 *                                   session will be included.
 *
 *             DIMEN_3D_MASK       = Masks that define whether 3D and/or DIMEN_4D_MASK 3D+time (4D)
 *datasets are allowable. DIMEN_ALL_MASK
 *
 *             WARP_ON_DEMAND_MASK = If this is set, then datasets that may not have a BRIK file
 *will be included in the list of datasets to choose from. In this case, the plugin must be ready to
 *deal with the warp-on-demand routines that return one slice at a time. By default, only datasets
 *with actual BRIKs will be included.
 *
 *             BRICK_BYTE_MASK     = Masks that define what type of data BRICK_SHORT_MASK should be
 *stored in the sub-bricks BRICK_FLOAT_MASK      of the allowable datasets. BRICK_COMPLEX_MASK
 *BRICK_RGB_MASK BRICK_ALLTYPE_MASK BRICK_ALLREAL_MASK
 *
...
/*-----------------------------------------------------------------------
 * Routine to add a timeseries "chooser" to the most recently created option within a plugin
 *interface.
 *
 * label = C string to go in the menu, next to the "chooser" for the dataset.
 * -------------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------
 * Routine to add a *.[tc]sv "chooser" to the most recently created option within a plugin
 *interface.
 *
 * label = C string to go in the menu, next to the "chooser" for the dataset.
 * -------------------------------------------------------------------------*/
...
void PLUTO_set_initcolorindex(int i)
{
    initcolorindex = i;
}                                                             /* 10 Oct 2007 */

/*-----------------------------------------------------------------------
 * Routine to add a color overlay  "chooser" to the most recently created option within a plugin
 *interface -- 11 Jul 2001 - RWCox.
 *
 * label = C string to go in the menu, next to the "chooser" for the color.
 * -------------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------------
 * Decide if a dataset prefix is OK.  The string itself is checked for legality, then the list of
 *all datasets is checked for duplicates.
 * ----------------------------------------------------------------------------*/
...
void PLUG_setup_widgets(PLUGIN_interface *plint, MCW_DC *dc)
{
    ...
    /* 'LessTif Widormous'
     * With 64bit LessTif, some widgets get created with enormous sizes. To complicate matters, the
     *problem occurred randomly so tracking it is a frustrating exercise. It looks like adding a
     *few, harmless, XmNheight and XmNwidth parameters at widget creation fixed the problem. Those
     *parameters have no effect on the final plugin's look so we won't bother to find out which of
     *them was necessary for fixing the problem.
    ...
    /* now that we have the label, find its sizes and make sure the shell doesn't get too small */
    ...
    if (plint->run_label[0] == '\0')
    {
        strcpy(plint->run_label, PLUG_run_label);
    }
    if (plint->doit_label[0] == '\0')
    {
        strcpy(plint->doit_label, PLUG_doit_label);
    }
    ...
    /**** create a Scrolled Window and Form to hold the user input option widgets, if they will be
     * needed ****/
...
#define LPAD    0     /* 29 Mar 2002: used to be 1 */
    ...
    for (iopt = 0 ; iopt < plint->option_count ; iopt++)
    {
        ...
        /* this callback will change the appearance of the option's row of widgets when the toggle
         * button is pressed */
        ...
        for (ib = 0 ; ib < opt->subvalue_count ; ib++)
        {
            ...
            case PLUGIN_OVERLAY_COLOR_TYPE: {
                ...
                if (CPU_IS_64_BIT())
                {
                    ...
                    /* get the very top rowcol holding the optmenu rowcol, see function
            ...
            /** string type:
             *    make an arrowval if a finite number of choices, or a label+textfield if the user
            ...
            {
                XtSetSensitive(ow->chtop[ib], False);
            }
    ...
    /**** Now that we've created all the rows, and have found all the widest widgets in each column,
...
#define LUCK          5
    ...
    if (wframe != NULL)
    {
        ...
        ib = plint->option_count;                              /* too many options --> will use
                                                                * vertical scrollbar */
        if (ib > OPC_MAXMAX)
        {
...
void PLUTO_turnoff_options(PLUGIN_interface *plint)
{
    ...
    if (plint == NULL || plint->wid == NULL ||
        plint->call_method == PLUGIN_CALL_IMMEDIATELY)
    {
        EXRETURN;
    }
...
void PLUG_action_CB(Widget w, XtPointer cd, XtPointer cbs)
{
    ...
    if (close && !badrun)
    {
        PLUG_delete_window_CB(w, cd, cbs);
    }
    ...
    if (help)
    {
        ...
        if (nl < 10)
        {
            MCW_popup_message(plint->wid->label,
                              plint->helpstring, MCW_USER_KILL);
        }
        else
        {
            new_MCW_textwin(plint->wid->label,
...
/*------------------------------------------------------------------------
 * What happens when the user selects "Close" from the window menu in a plugin interface menu
 *window.
 * --------------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------------
 * Routine to take the interface widgets and put their values into place, so that the plugin can get
 *them out again.
 * ----------------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------
 * Routines to get values from a plugin interface, when passed to a plugin.
 * -------------------------------------------------------------------------*/
...
char * get_description_from_PLUGIN_interface(PLUGIN_interface *plint)
{
    ...
    {
        RETURN(plint->description);
    }
}

/*-----------------------------------------------------------------------
 * Get the next chosen option, and return its string "tag". If there is no "next chosen option",
 *return NULL.
 * -------------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------
 * Return a string encapsulating information about how the plugin was called.  This string is
 *created with malloc() and should be free()-ed when it is used up.  If NULL is returned, an error
    ...
    for (iopt = 0 ; iopt < plint->option_count ; iopt++)
    {
        ...
        for (jsv = 0 ; jsv < opt->subvalue_count ; jsv++)
        {
            ...
            case PLUGIN_NUMBER_TYPE: {
                ...
                if (val != NULL)
                {
                    outbuf = THD_zzprintf(outbuf, "%g", *val);
                }
                else
                {
                    outbuf = THD_zzprintf(outbuf, "?");
            ...
            case PLUGIN_STRING_TYPE: {
                ...
                if (val != NULL)
                {
                    BUFIT(val); outbuf = THD_zzprintf(outbuf, "%s", buf);
                }
                else
                {
                    outbuf = THD_zzprintf(outbuf, "?");
...
/*-------------------------------------------------------------------------
 * Find out what the next subvalue type is, without actually retrieving it. This will be one of the
 *PLUGIN_*_TYPE codes.  If something is wrong, the return value will be ILLEGAL_TYPE.  This can be
 *used to detect that no more subvalues are present in the option currently being processed.
 * ---------------------------------------------------------------------------*/
...
int peek_callvalue_type_from_PLUGIN_interface(PLUGIN_interface *plint)
{
    ...
    if (plint->opnum >= plint->option_count)
    {
        RETURN(ILLEGAL_TYPE);
    }
 ...
 * "type".  If it is not, return NULL, otherwise return a "void *", which must be properly
 *de-referenced to get the true value:
 *
 *   type                      output is really
 *   ------------------        ----------------
 *   PLUGIN_NUMBER_TYPE        float *         points to number PLUGIN_STRING_TYPE        char *
 *          duh PLUGIN_DATASET_TYPE       MCW_idcode *    duh PLUGIN_TIMESERIES_TYPE    MRI_IMAGE **
 *    duh PLUGIN_OVERLAY_COLOR_TYPE int *           points to overlay index
 *
 * Following this are convenience routines to do similar work on specific data types.
 * ---------------------------------------------------------------------------*/
...
void * get_callvalue_from_PLUGIN_interface(PLUGIN_interface *plint, int type)
{
    ...
    if (opt->subvalue[isv].data_type != type)
    {
        RETURN(NULL);
    }
 ...
 *   1) Retrieve the pointers to the data structures associated with the button.
 *   2) Select the datasets that are allowable and make a list of their names and idcodes.
...
void PLUG_choose_dataset_CB(Widget w, XtPointer cd, XtPointer cbs)
{
    ...
    /*--- 23 Nov 1996: loop over dataset links and patch their titles to include an indicator of the
     * dataset type    ---*/
    ...
    if (!av->multi)
    {
        if (!ISZERO_IDCODE(old_idcode))
        {
            for (id = 0 ; id < num_dset ; id++)
            {
                if (EQUIV_IDCODES(old_idcode, av->dset_link[id].idcode))
                {
                    break;
                }
            }
    ...
    else
    {
        ...
        for (qd = 0 ; qd < num_old ; qd++)
        {
            if (!ISZERO_IDCODE(old_chosen[qd]))
            {
                for (id = 0 ; id < num_dset ; id++)
                {
                    if (EQUIV_IDCODES(old_chosen[qd], av->dset_link[id].idcode))
                    {
                        break;
                    }
                }
    ...
    for (id = 0 ; id < num_dset ; id++)
    {
        strlist[id] = av->dset_link[id].title;
    }
...
/*-----------------------------------------------------------------------
 * Called when the user actually selects a dataset from the chooser. This routine just changes the
 *original pushbutton label, and notes the index of the choice in the right place, for later
 *retrieval.
 * -------------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------
 * Determine if a dataset passes the type_mask and ctrl_mask criteria for acceptability.
 * -------------------------------------------------------------------------*/
...
int PLUGIN_dset_check(int type_mask, int ctrl_mask, THD_3dim_dataset *dset)
{
    ...
    if (((1 << dset->func_type) & type_mask) == 0)
    {
        RETURN(0);
    }
    ...
    if (!DSET_INMEMORY(dset) && (ctrl_mask & WARP_ON_DEMAND_MASK) == 0)
    {
        RETURN(0);
    }
    ...
    if (itmp == MRI_byte && (ctrl_mask & BRICK_BYTE_MASK) == 0)
    {
        RETURN(0);
    }
    if (itmp == MRI_short && (ctrl_mask & BRICK_SHORT_MASK) == 0)
    {
        RETURN(0);
    }
    if (itmp == MRI_float && (ctrl_mask & BRICK_FLOAT_MASK) == 0)
    {
        RETURN(0);
    }
    if (itmp == MRI_complex && (ctrl_mask & BRICK_COMPLEX_MASK) == 0)
    {
        RETURN(0);
    }
    if (itmp == MRI_rgb && (ctrl_mask & BRICK_RGB_MASK) == 0)
    {
        RETURN(0);
    }
...
/*-------------------------------------------------------------------------
 * Loop over a bunch of dataset links and patch their titles to include an indicator of the dataset
 *type, etc.
...
/*------------------------------------------------------------------------
 * Popup a chooser list of datasets that meet some criteria. Note that only one such chooser will be
 *popped up at any given time. This routine is for use in user-written plugins with custom
 *interfaces.
 * 23 October 1998 -- RWCox
 *
 * w = widget to popup near
 *
 * vv = view type of datasets (cf. VIEW_*_TYPE from 3ddata.h) [one way to select this is
 *plint->im3d->vinfo->view_type]
 *
 ...
 *              If this function pointer is not NULL, it will be called to check if each dataset
 *should be allowed in.  A zero return value means don't allow; any other return value means dset
 *will be in the displayed list.  If chk_func is NULL, then all datasets known to AFNI will be
 *allowed.  [The function PLUTO_dset_check() may be useful inside chk_func.]
 *
 * cb_func  = void function( int num, THD_3dim_dataset ** dslist, void * cd ) ;
 *              This function pointer must not be NULL.  It will be called when the user makes a
 *choice on the popup chooser. The value num will be the number of datasets chosen.
 *              dslist[i] will be a pointer to the i-th dataset, for i=0..num-1.  If multi was 0,
 *then num will be 1 and the single selected dataset will be pointed to by dlist[0].
 *
 * cd = A pointer to anything the user likes.  It will be passed to chk_func and cb_func, as
 *described above.  (Can be NULL.)
 * --------------------------------------------------------------------------*/
                              ...
                              void_func *cb_func, void *cd)
{
    ...
    if (w == NULL || cb_func == NULL ||
        vv < FIRST_VIEW_TYPE || vv > LAST_VIEW_TYPE)
    {
        EXRETURN;
    }
...
#if 0
            if (chk_func != NULL && chk_func(dset, cd) == 0)
            {
                continue;                                            /* skip */
            }
#else
...
/*-----------------------------------------------------------------------
 * Called when the user actually selects a dataset from the chooser. Will call the user's pitiful
 *and loathsome routine.
 * 23 October 1998 -- RWCox
 * -------------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------
 * Called when the user actually selects a timeseries from the chooser. This routine just changes
 *the original pushbutton label, and notes the index of the choice in the right place, for later
 *retrieval.
 * -------------------------------------------------------------------------*/
...
void PLUG_finalize_timeseries_CB(Widget w, XtPointer fd, MCW_choose_cbs *cbs)
{
    ...
    XtVaGetValues(w, XmNuserData, &av, NULL);
    if (plint == NULL || av == NULL || av->tsimar == NULL)
    {
        EXRETURN;
    }
    if (cbs->reason != mcwCR_timeseries)
    {
        EXRETURN;                                    /* error */
    }
...
/*-----------------------------------------------------------------------
 * Called when the user actually selects a tcsv from the chooser. This routine just changes the
 *original pushbutton label, and notes the index of the choice in the right place, for later
 *retrieval.
 * -------------------------------------------------------------------------*/
...
void PLUG_finalize_tcsv_CB(Widget w, XtPointer fd, MCW_choose_cbs *cbs)
{
    ...
    XtVaGetValues(w, XmNuserData, &av, NULL);
    if (plint == NULL || av == NULL || av->elarr == NULL)
    {
        EXRETURN;
    }
    if (cbs->reason != mcwCR_tcsv)
    {
        EXRETURN;                              /* error */
    }
    /** store the choice, and change the widget label **/
...
void AFNI_plugin_button(Three_D_View *im3d)
{
    ...
    if (exten == NULL ||
        !IM3D_VALID(im3d) || im3d->type != AFNI_3DDATA_VIEW)
    {
        EXRETURN;
    }
    ...
    /*-- make arrays to hold the plugin buttons (etc.), so that plugins can be started from layouts
     * (in afni_splash.c) --*/
...
#define MENU_BUT(pl)                                                        \
    ...
    /*-- 28 Nov 2000: allow user to specify that button should be alphabetized --*/
...
/*------------------------------------------------------------------------
 * Routine that actually starts up a plugin when the user makes his or her choice from the menu
 *created above.
 * --------------------------------------------------------------------------*/
...
void PLUG_startup_plugin_CB(Widget w, XtPointer cd, XtPointer cbd)
{
    ...
    if (plint->im3d == NULL)
    {
        plint->im3d = AFNI_find_open_controller();
    }
        ...
        cbs->event->type == ButtonRelease)
    {
        ...
        if (xx + ww + 3 >= sw && ww <= sw)
        {
            xx = sw - ww;                             /* make sure is on screen */
        }
        if (yy + hh + 3 >= sh && hh <= sh)
        {
            yy = sh - hh;
...
/*---------------------------------------------------------------------------
 * Routine to add a dataset to the AFNI global collection. Return value is 0 if all is OK, 1 if bad
 *things happened.
 * "action_flag" is the OR (|) of various possibilities:
 *    DSET_ACTION_NONE         == do nothing DSET_ACTION_MAKE_CURRENT == make this the currently
 *viewed dataset
 * -----------------------------------------------------------------------------*/
                   ...
                   THD_3dim_dataset *dset, int action_flag)
{
    ...
    if (plint == NULL || !ISVALID_3DIM_DATASET(dset))
    {
        RETURN(1);
    }
...
/*----------------------------------------------------------------------
 * Routine to force AFNI to redisplay controllers that are attached to a given dataset.  (Feb 1998)
 * ------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------
 * Routine to inform AFNI that some dataset names are now different, so stuff should be done about
 *it.
 * ------------------------------------------------------------------------*/
...
static int PLUTO_strncmp(char *aa, char *bb, int nn)       /* 09 Oct 2000 */
{
    ...
    for (ii = 0 ; ii < nn ; ii++, aa++, bb++)
    {
        if (*aa == '\0' && *bb == '\0')
        {
            return 0;                               /* got to end all same! */
        }
        if (*aa == '\0' || *bb == '\0')
        {
            return 1;                               /* premature end of one */
        }
        if (isspace(*aa) || isspace(*bb))
        {
            continue;                               /* don't compare blanks */
        }
...
int PLUTO_string_index(char *target, int num, char *source[])
{
    ...
    if (num <= 0 || source == NULL || target == NULL)
    {
        return -1;
    }
    ...
    for (ii = 0 ; ii < num ; ii++)
    {
        if (PLUTO_strncmp(target, source[ii], PLUGIN_STRING_SIZE) == 0)
        {
            return ii;
        }
    }
...
/*-----------------------------------------------------------------------
 * Function to let an IMMEDIATE mode plugin let AFNI know where its toplevel shell is -- 22 Sep 2000
 * -- RWCox.
 * -------------------------------------------------------------------------*/
...
void PLUTO_popup_meter(PLUGIN_interface *plint)
{
    ...
    if (plint == NULL || plint->wid == NULL ||
        plint->wid->shell == NULL || plint->wid->meter != NULL)
    {
        EXRETURN;
    }
...
/*-----------------------------------------------------------------------
 * Routine to popup an image. Inputs:
 *   handle = (a) value returned from previous call to this routine (b) or NULL to start a new image
 *display window im     = pointer to MRI_IMAGE structure (a) NULL to popdown the image display
 *window (b) non-NULL to display this image in the window
 *
 * Output is a "handle" that can be used to control the next usage of this routine (whether to send
 *an image to a new window or replace it in an old one).
 * -------------------------------------------------------------------------*/
...
XtPointer PLUGIN_imseq_getim(int n, int type, XtPointer handle)
{
    ...
    /*--- return a copy of the image (since the imseq will delete it when it is done) ---*/
...
#define NKLING    5
        ...
        else if (xev->button == Button3)
        {
            ...
            else
            {
                ...
                if (nkl == NKLING)
                {
                    AFNI_speak("Deesist at once", 0); nkl++;
                }
            }
        }
    }
...
/*-----------------------------------------------------------------------
 * 28 April 2000: Open an image sequence display window. The input image(s) are copied, so they can
 *be destroyed after these calls. PLUTO_imseq_popim(im,kfunc,kdata) starts the image window off with
 *a single image;
 *       return value is a "handle" that is used in other calls;
 *       if kfunc is not NULL, it is a function that will be called with argument kdata when the
 *imseq window is closed (e.g., this can be used to set the saved handle to NULL)
 *PLUTO_imseq_popup(imar,kfunc,kdata) starts the window off with an array of images
 *PLUTO_imseq_retitle(handle,string) changes the window manager title
 *PLUTO_imseq_rekill(handle,kfunc,kdata) changes the kill function/data to kfunc/kdata;
 *    PLUTO_imseq_addto(handle,im) adds the single image "im" to the display
 *PLUTO_imseq_setim(handle,n) sets the image index to 'n'
 *    PLUTO_imseq_destroy(handle) pops down the image viewer;
 *       destroys the internal copies of the images;
 *       calls kfunc(kdata) if kfunc is not NULL (so do PLUTO_imseq_rekill(handle,NULL,NULL); before
...
void * PLUTO_imseq_popup(MRI_IMARR *imar, generic_func *kfunc, void *kdata)
{
    ...
    if (imar == NULL || IMARR_COUNT(imar) == 0)
    {
        return NULL;
    }
...
void PLUTO_imseq_retitle(void *handle, char *title)
{
    ...
    if (psq == NULL || psq->seq == NULL || title == NULL)
    {
        return;
    }
...
void PLUTO_imseq_addto(void *handle, MRI_IMAGE *im)
{
    ...
    if (psq == NULL || psq->seq == NULL || im == NULL)
    {
        return;
    }
...
{
    if (plint == NULL || str == NULL || !AFNI_VERBOSE)
    {
        return;
    }
...
/* Set the addresses of the main vol2surf globals.  Note that the plugin options pointer is stored
 * as (void *) so that vol2surf.h will not need to percolate up to afni.h.  09 Sep 2004 [rickr]
 */
#include "vol2surf.h"
...
#ifndef MATRIX_INCLUDED                /* matrix.h is now included and matrix_initialize prototype
                                        * conflicts with this one. ZSS Nov. 21 2014 */
...
void * MCW_onen_i_estel_edain(void *n)
{
};                                          /* dummy routine */
...
/***********************************************************************
 * Routines that are always compiled, since they are used in a few places in AFNI that are not
 *plugin-specific.
 ************************************************************************/
...
/*----------------------------------------------------------------------------
 * Routine to find a dataset in the global sessionlist, given its idcode. If this returns NULL, then
 *you are SOL.
 * ------------------------------------------------------------------------------*/
...
THD_3dim_dataset * PLUTO_find_dset(MCW_idcode *idcode)
{
    ...
    if (idcode == NULL || ISZERO_IDCODE(*idcode))
    {
        RETURN(NULL);
    }
...
THD_slist_find PLUTO_dset_finder(char *idc)
{
    ...
    BADFIND(find); if (idc == NULL || *idc == '\0')
    {
        return find;
    }
...
/*!
 * Look for a dataset by name and append it to session in question Search if needed for exact name
 *matches under paths from find_afni_file() and then try get_atlas()
 *
 * Returns a 0 only when a dataset was found. Non-zero indicates stupidity, error, etc. ZSS Oct.
 * 2013
 * ------------------------------------------------------------------------*/
int AFNI_append_dset_to_session(char *fname, int sss)
{
    ...
    if (sss < 0 || sss >= GLOBAL_library.sslist->num_sess)
    {
        RETURN(2);
    }

    this_ss = GLOBAL_library.sslist->ssar[sss];
    if (!ISVALID_SESSION(this_ss))
    {
        RETURN(3);
    }

    /* not sure what to do with GLOBAL_library session yet.
     * why not add to it too? Also, why not add to global if sss == -1 ? For now, return merrily */
    if (this_ss == GLOBAL_library.session)
    {
        RETURN(4);
    }
    ...
    if (this_ss->num_dsset < THD_MAX_SESSION_SIZE)
    {
        ...
        if (!new_dset) /* try again, this works better if name is not complete */
        {
        ...
        if (ISVALID_DSET(new_dset))
        {
            ...
            if (find.dset == NULL)
            {
                ...
                if (find.dset != NULL &&
                    find.dset->view_type != new_dset->view_type)
                {
                    find.dset = NULL;
                }
            }
 ...
 *   nbin = # of bins in hist[] bot  = bottom of hist[0] bin   } bin size is top  = top of
 *hist[nbin-1] bin } (top-bot)/nbin hist = array of counts in each bin xlab } labels for x-axis,
 *ylab }            y-axis tlab }        and top of graph (NULL => skip this label)
 *
 *   njist = number of extra histograms [can be 0] jist  = "extra" histograms to plot atop hist (if
 *jist == NULL, this plot is skipped) Graph is popped up and then "forgotten" -- RWCox - 30 Sep
 *1999.
 * -------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------
 * Return p10 as a power of 10 such that p10 <= fabs(x) < 10*p10 unless x == 0, in which case return
 *0.
 * ------------------------------------------------------------------------*/
...
void PLUTO_set_xypush(int a, int b)
{
    xpush = a; ypush = b;
}
...
/*-----------------------------------------------------------------
 * Plot a scatterplot.
 *   npt  = # of points in x[] and y[] x    = x-axis values array y    = y-axis values array xlab }
 * labels for x-axis, ylab }            y-axis tlab }        and top of graph (NULL => skip this
 *label) nlin = number of straight lines to plot alin,blin,clin = slope, intercept, color of lines
                           ...
                           int nlin, float *alin, float *blin, float_triple *clin)
{
    ...
    pbot = p10(xbot); ptop = p10(xtop); if (ptop < pbot)
    {
        ptop = pbot;
    }
    if (ptop != 0.0 && xpush > 0)
    ...
    else
    {
        nnax = 1; mmax = 10;
        dx   = 0.02 * (xtop - xbot);
        if (xtop - floorf(xtop) > 0.001f)
        {
            xtop += dx;
        }
        if (xbot - floorf(xbot) > 0.001f)
        {
            xbot -= dx;
        }
    }
    ...
    pbot = p10(ybot); ptop = p10(ytop); if (ptop < pbot)
    {
        ptop = pbot;
    }
    if (ptop != 0.0 && ypush > 0)
    ...
    else
    {
        nnay = 1; mmay = 10;
        dy   = 0.02 * (ytop - ybot);
        if (ytop - floorf(ytop) > 0.001f)
        {
            ytop += dy;
        }
        if (ybot - floorf(ybot) > 0.001f)
        {
            ybot -= dy;
        }
    }
...
#define DSQ    0.0011111
    ...
    if (dsq <= 0.0 || dsq >= 0.01)
    {
        ...
        else if (dsq > 5.0f * DSQ)
        {
            dsq = 5.0f * DSQ; set_thick_memplot(0.002f);
        }
    }
...
/*----------------------------------------------------------------------
 * Force the change of opacity in all open image viewers 06 Jun 2019 -- RWCox.
 * ------------------------------------------------------------------------*/
...
double PLUTO_elapsed_time(void) /* in seconds */
{
    ...
    gettimeofday(&new_tval, NULL);/*@@@BUG: */
