 ...
 *of FMRI noise, 0 < lam < rho < 0.9. The maximum bandwidth of the matrix is chosen so that the last
...
/* Generate ARMA11(a=ap,lam=lm) vectors of length nlen, nvec of them, with standard deviation set to
 * sg.  If sg <= 0, then the vectors are L2-normalized (sum of squares=1).
 *//*-------------------------------------------------------------------------*/

static float tdof = 0.0f;
void mri_genARMA11_set_tdof(float ttt)
{
    tdof = ttt;
}
...
MRI_IMAGE * mri_genARMA11(int nlen, int nvec, float ap, float lm, float sg)
{
    ...
    if (nlen <= 3)
    {
        ERROR_message("ARMA11 nlen < 4");    RETURN(NULL);
    }
    if (fabs(aa) >= 0.98)
    {
        ERROR_message("ARMA11 a too big");   RETURN(NULL);
    }
    if (fabs(lam) >= 0.97)
    {
        ERROR_message("ARMA11 lam too big"); RETURN(NULL);
    }
