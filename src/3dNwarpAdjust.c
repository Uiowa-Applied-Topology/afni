...
int main(int argc, char *argv[])
{
    ...
    while (iarg < argc && argv[iarg][0] == '-')
    {
        ...
        if (strcasecmp(argv[iarg], "-nwarp") == 0 || strcasecmp(argv[iarg], "-warp") == 0)
        {
            ...
            for (kk = iarg ; kk < argc && argv[kk][0] != '-' ; kk++, nwset++)
            {
                ;                                                        /*nada*/
            }
            if (nwset < 5)
            {
                ERROR_exit("Need at least 5 datasets after '%s'", argv[iarg - 1]);
            }
            ...
            for (kk = 0 ; kk < nwset ; kk++)
            {
                ...
                DSET_COPYOVER_REAL(dset_nwarp[kk]);
                if (dset_nwarp[kk] == NULL)
                {
                    ERROR_exit("can't open warp dataset '%s' :-(", argv[iarg + kk]);
                }
                if (DSET_NVALS(dset_nwarp[kk]) < 3)
                {
                    ERROR_exit("dataset '%s' isn't a 3D warp", argv[iarg + kk]);
                }
#if 0
                if (kk == 0)
                {
                    nx = DSET_NX(dset_nwarp[0]); ny = DSET_NY(dset_nwarp[0]);
                    nz = DSET_NZ(dset_nwarp[0]); nxyz = nx * ny * nz;
                }
                else if (DSET_NX(dset_nwarp[kk]) != nx ||
                         DSET_NY(dset_nwarp[kk]) != ny ||
                         DSET_NZ(dset_nwarp[kk]) != nz)
                {
                    ERROR_exit("warp dataset '%s' doesn't match with grid size %dx%dx%d",
                               argv[iarg + kk], nx, ny, nz);
                }
#endif
            }
            ...
            for (npad = kk = 0 ; kk < nwset ; kk++)
            {
                ...
                if (pad_xm > 0 || pad_xp > 0 || pad_ym > 0 || pad_yp > 0 || pad_zm > 0 ||
                    pad_zp > 0)
                {
                    THD_3dim_dataset *qset;
                    if (verb)
                    {
                        INFO_message("extending input warp #%d: %d %d %d %d %d %d",
            ...
            nx   = DSET_NX(dset_nwarp[0]); ny = DSET_NY(dset_nwarp[0]); nz = DSET_NZ(dset_nwarp[0]);
            nxyz = nx * ny * nz;
            if (npad == 0 && verb)
            {
        ...
        if (strcasecmp(argv[iarg], "-source") == 0)
        {
            ...
            for (kk = iarg ; kk < argc && argv[kk][0] != '-' ; kk++, nsset++)
            {
                ;                                                        /*nada*/
            }
            if (nsset < 5)
            ...
            for (kk = 0 ; kk < nsset ; kk++)
            {
                ...
                if (DSET_NVALS(dset_src[kk]) > 1)
                {
                    ERROR_exit("dataset '%s' has more than 1 sub-brick", argv[iarg + kk]);
                }
                if (kk == 0)
                {
                    nxs = DSET_NX(dset_src[0]); nys = DSET_NY(dset_src[0]);
                    nzs = DSET_NZ(dset_src[0]); nxyzs = nxs * nys * nzs;
                }
                else if (DSET_NX(dset_src[kk]) != nxs ||
                         DSET_NY(dset_src[kk]) != nys ||
                         DSET_NZ(dset_src[kk]) != nzs)
                {
                    ERROR_exit("source dataset '%s' doesn't match with grid size %dx%dx%d",
                               argv[iarg + kk], nxs, nys, nzs);
                }
            }
            iarg += nsset; continue;
        }
    ...
    {
        ERROR_exit("Number of -source datasets %d doesn't match number of -nwarp datasets %d",
                   nsset, nwset);
    }
    ...
    {
        INFO_message(
            "warp grid = %dx%dx%d is bigger than source grid = %dx%dx%d (this is NOT a problem)",
    ...
    if (verb)
    {
        fprintf(stderr, "++ Computing mean warp");
    }
    ...
    if (verb)
    {
        fprintf(stderr, "++ Inverting mean warp");
    }
    ...
    {
        INFO_message("========== Beginning adjustment process ==========");
    }
    ...
    for (kk = 0 ; kk < nwset ; kk++)
    {
        ...
        if (verb > 1)
        {
            ININFO_message("  compose with mean inverse");
        }
        BB = IW3D_compose(AA, WWbin, MRI_WSINC5);
        IW3D_destroy(AA);
        if (verb > 1)
        {
            ININFO_message("  convert back to dataset warp");
        }
        ...
        if (verb > 1)
        {
            ININFO_message("  write out adjusted warp dataset");
        }
        ...
        if (dset_src != NULL)
        {
            ...
            if (verb > 1)
            {
                ININFO_message("  re-warp from source");
            }
            dset_www = THD_nwarp_dataset(dset_nwarp[kk], dset_src[kk], NULL,
                                         "BondJamesBond", MRI_WSINC5, MRI_WSINC5, 0.0f, 1.0f, 1,
                                         NULL);
            bb = (float *)DSET_ARRAY(dset_www, 0);
            if (verb > 1)
            {
                ININFO_message("  add to mean");
            }
            for (ii = 0 ; ii < nxyzs ; ii++)
            {
                sbar[ii] += bb[ii];
            }
            if (verb > 1)
            {
                ININFO_message("  delete www dataset");
            }
            DSET_delete(dset_www);
            if (verb > 1)
            {
                ININFO_message("  delete src dataset");
            }
        ...
        if (verb > 1)
        {
            ININFO_message("  delete warp dataset from memory");
        }
