...
/*
 * This program calculates a functional image from an AFNI 3D+time data set, by calculating the
 *correlation between the image time series and one or more reference ("ideal") time series.
 *
 * File:     3dfim.c Date:     06 September 1996
 *
 * Incorporated "ort" time series into 3dfim program. BDW       12 December 1996
 *
 * Added -percent option for calculating relative effect of reference waveform upon observed time
 *series. BDW       19 May 1997
 *
 * Corrected reference to "ort" time series data structure. BDW       05 Sept 1997
 *
 * Print a more explicit error message when ideal or "ort" time series are not of sufficient length.
 *BDW       14 January 1998
 *
 * Mod:     Added changes for incorporating History notes. Date:    10 September 1999
 *
    ...
    for (ivec = 0 ; ivec < ny_ref ; ivec++)
    {
        ...
        for (it = itbot ; it < ntime ; it++)
        {
            if (tsar[it] < SO_BIG)
            {
                ...
                if (max_percent > 0.0) /* 19 May 1997 */
                {
                    if (tsar[it] > ref_ts_max[ivec])
                    {
                        ref_ts_max[ivec] = tsar[it];
                    }
                    if (tsar[it] < ref_ts_min[ivec])
                    {
                        ref_ts_min[ivec] = tsar[it];
                    }
                }
            }
        }
    ...
    for (it = itbot ; it < ntime ; it++)
    {
        ...
        if (nupdt == ngood_ref)
        {
            ...
            if (ny_ref == 1)
            {
                ...
                /*--- get correlation coefficient (pcor) into vval, scale into brick array (with
...
typedef struct {
    ...
    float              max_percent;               /* max. percentage change due to ideal time
                                                   * series,  19 May 1997 */
} line_opt;
...
void get_line_opt(int argc, char *argv[], line_opt *opt)
{
     ...
     * coding technique is to examine each argv, and if it matches something we expect (e.g.,
     * -ideal), process it, then skip to the next loop through ('continue' statements in each
     *strncmp if). Note that the 'while' will increment the argument index (nopt)  --- */
...
#  define DBOPT(str)    /* nothing */
        ...
        if (strncmp(argv[nopt], "-ideal", 5) == 0)
        {
            ...
            { /* --- one file --- */
                ...
                if (ideal == NULL)
                {
                    Syntax("Unable to read ideal time series.");
                }
        ...
        if (strncmp(argv[nopt], "-percent", 8) == 0)
        {
            ...
            if (++nopt >= argc)
            {
                Syntax("-percent needs an argument");
            }
            ftemp = strtod(argv[nopt], NULL);
            if (ftemp <= 0.0)
            {
        ...
        if (strncmp(argv[nopt], "-ort", 4) == 0)
        {
            ...
            { /* --- one file --- */
                ...
                if (ort == NULL)
                {
                    Syntax("Unable to read ort time series.");
                }
    ...
    if (!strcmp(input_filename, ""))
    {
        Syntax("No input file name was given.");
    }
    if (opt->idts->num == 0)
    {
        Syntax("No ideal response vector was defined!");
    }
    if (!strcmp(opt->prefix_name, ""))
    {
        Syntax("No prefix name was specified.");
    }
...
int main(int argc, char *argv[])
{
    ...
    if (!ok)
    {
        Syntax("Failure to write functional data set.");
    }
