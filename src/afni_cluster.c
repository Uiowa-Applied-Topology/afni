...
void set_vedit_cluster_label(Three_D_View *im3d, int ll)
{
    ...
    strcpy(lab, clubutlab[0]); if (ll == 0)
    {
        lab[0] = '*';
    }
    MCW_set_widget_label(im3d->vwid->func->clu_clear_pb, lab);

    strcpy(lab, clubutlab[1]); if (ll == 1)
    {
        lab[0] = '*';
    }
                          ...
                          XtPointer client_data, XtPointer call_data)
{
    ...
    cwid = im3d->vwid->func->cwid; if (cwid == NULL)
    {
        EXRETURN;
    }

    bval = MCW_val_bbox(cwid->usemask_bbox);
    if (bval == !im3d->vednomask)
    {
        EXRETURN;
    }
...
static void AFNI_cluster_choose_CB(Widget wc, XtPointer cd, int nval, void **val)
{
    ...
    if (!IM3D_OPEN(im3d) || nval != 3 || val == NULL)
    {
        EXRETURN;                                     /* bad bad */
    }
...
void AFNI_clu_CB(Widget w, XtPointer cd, XtPointer cbs)
{
    ...
    if (w == im3d->vwid->func->clu_cluster_pb)
    {
        ...
        if (im3d->vedset.code == VEDIT_CLUST)
        {
            ...
            ccsiz = im3d->vedset.param[3]; if (ccsiz < 1)
            {
                ccsiz = 40;
            }
        }
...
/*! Make the widgets for one row of the cluster display/control panel. The row itself will not be
 * managed at this time; that comes later. */
...
/* Stuff to set linkRbrain max cluster count from right-click popup menu on the 'linkRbrain' button
 * [09 Sep 2015]
 *//*-------------------------------------------------------------------------*/
...
static void AFNI_clus_make_widgets(Three_D_View *im3d, int num)
{
    ...
    if (!IM3D_OPEN(im3d) || im3d->vwid->func->cwid != NULL)
    {
        EXRETURN;                                                      /* bad */
    }
    ...
    if (maxclu_default < 0)
    {
        maxclu_default = scrolling ? 15 : SHRUTI_NUM;
    }
    ...
    { char *blab = "Save->Mask??";   /* 14 Jun 2014 */
      ...
      MCW_reghint_children(cwid->save_as_mask_bbox->wrowcol,
                           "Change Save buttons to Mask buttons?");
      cwid->save_as_mask = 0; }
 ...
 * ;*/
    ...
    if (AFNI_yesenv("AFNI_CLUSTERIZE_OLD"))
    {
        use_old_3dclust = 1;
    }
    ...
    { char *ppp = getenv("AFNI_CLUSTER_PREFIX");
      if (!THD_filename_pure(ppp) || strlen(ppp) > 61)
      {
          ppp = "Clust";
      }
    ...
    if (swtop != NULL)
    {
        ...
        hy *= 2; cmax = im3d->dc->height - 128; if (hy > cmax)
        {
            hy = cmax;
        }
...
static void AFNI_cluster_widgize(Three_D_View *im3d, int force)
{
    ...
    if (!force)
    {
        ...
        if (!MCW_widget_visible(im3d->vwid->func->cwid->wtop))
        {
            EXRETURN;
        }
        if (IM3D_SHFT_CTRL_DRAG(im3d))
        {
            EXRETURN;
        }
    }
...
int AFNI_clus_find_xyz(Three_D_View *im3d, float x, float y, float z)
{
    ...
    for (ii = 0 ; ii < nclu ; ii++)
    {
        ...
        for (jj = 0 ; jj < npt ; jj++)
        {
            if (xi == cl->i[jj] && yi == cl->j[jj] && zi == cl->k[jj])
            {
                RETURN(ii);
            }
        }
...
static void AFNI_clus_viewpoint_CB(int why, int np, void *vp, void *cd)
{
    ...
    case RECEIVE_ALTERATION: {  /* some dataset changed? */
        ...
        if (nch > 0)
        {
            AFNI_clus_auxdsetlabel(im3d);           /* re-label */
        }
    }
    break;
...
void AFNI_clus_update_widgets(Three_D_View *im3d)
{
    ...
    if (pval >= 0.0f && ctab != NULL)
    {
        ...
        if (csiz > 0)
        {
            sprintf(rrr + strlen(rrr), " Cluster thresh(alpha=0.10) =%6d\n", csiz);
        }
        else
        {
            strcat(rrr, " Cluster thresh(alpha=0.10) =   N/A\n");
        }
        csiz = find_cluster_thresh(0.05f, pval, ctab);
        if (csiz > 0)
        {
            sprintf(rrr + strlen(rrr), " Cluster thresh(alpha=0.05) =%6d\n", csiz);
        }
        else
        {
            strcat(rrr, " Cluster thresh(alpha=0.05) =   N/A\n");
        }
        csiz = find_cluster_thresh(0.01f, pval, ctab);
        if (csiz > 0)
        {
            sprintf(rrr + strlen(rrr), " Cluster thresh(alpha=0.01) =%6d\n", csiz);
        }
        else
        {
            strcat(rrr, " Cluster thresh(alpha=0.01) =   N/A\n");
        }
#else
    ...
    if (cwid->nall < nclu)
    {
        ...
        cwid->clu_see_bbox = (MCW_bbox **)XtRealloc((char *)cwid->clu_see_bbox,
                                                    nclu * sizeof(MCW_bbox *));
        for (ii = cwid->nall ; ii < nclu ; ii++)
        {
            MAKE_CLUS_ROW(ii);
        }
        cwid->nall = nclu;
...
static void AFNI_clus_finalize_dataset_CB(Widget w, XtPointer cd, MCW_choose_cbs *cbs)
{
    ...
    cwid = im3d->vwid->func->cwid;
    if (cwid == NULL || !cwid->is_open)
    {
        POPDOWN_strlist_chooser; EXRETURN;
    }

    ibut = cdds.ncode;
    if (ibut < 0 || ibut >= MAX_CLU_AUXDSET)
    {
        POPDOWN_strlist_chooser; EXRETURN;
    }

    ival = cbs->ival;
    if (ival < 0 || ival >= cdds.ndset)
    {
        POPDOWN_strlist_chooser; EXRETURN;
    }
    dset = cdds.dset[ival];
...
static void AFNI_clus_finalize_scat1D_CB(Widget w, XtPointer cd, MCW_choose_cbs *cbs)
{
    ...
    if (!IM3D_OPEN(im3d) || cbs == NULL)
    {
        POPDOWN_timeseries_chooser; EXRETURN;
    }
    cwid = im3d->vwid->func->cwid;
    if (cwid == NULL || !cwid->is_open)
    {
...
void AFNI_clus_action_CB(Widget w, XtPointer cd, XtPointer cbs)
{
    ...
    if (w == cwid->savemask_pb || w == cwid->whermask_pb)   /* 01 May 2008 */
    {
        ...
        if (!THD_filename_pure(ppp) || strcmp(ppp, "-") == 0)
        {
            ppp = "Clust";
        }
...
#define WSIZ    4096
        if (do_wami) /* 04 Aug 2010 */
        {
            ...
            if (jtop >= clar->num_clu)
            {
                strcpy(ct, " ");
            }
            else
            {
                sprintf(ct, " [first %d clusters]", jtop);
            }
            ...
            if (inv)
            {
                MCW_invert_widget(cwid->whermask_pb);
            }
            SHOW_AFNI_READY;
        }
    ...
    if (w == cwid->histrange_pb)
    {
        ...
        float        fvec[2];
        if (cwid->hbot < cwid->htop)
        {
            fvec[0] = cwid->hbot; fvec[1] = cwid->htop;
        }
        else
        {
            fvec[0] = fvec[1] = 0.0f;
    ...
    if (w == cwid->linkrbrain_pb)  /* 11 Feb 2014 */
    {
        ...
        if (nclu == 0 || cld == NULL || do_linkrbrain == 0)
        {
            EXRETURN;
        }
        ...
        {
            free(slist); EXRETURN;
        }                                          /* couldn't create coordinate table */
#undef  WSIZ
#define WSIZ    4096
        ...
        if (inv)
        {
            MCW_invert_widget(cwid->linkrbrain_pb);
        }
        SHOW_AFNI_READY;
        EXRETURN;
    ...
    for (ii = 0 ; ii < MAX_CLU_AUXDSET ; ii++)
    {
        ...
        /* there are no actions to take when a Aux Dset toggle is pressed, or when a From/To
         * arrowval is altered -- these things will be read out when the user actuates a Save or
         * Plot cluster button. */
...

    {
        if (w == cwid->clu_jump_pb[ii])
        {
            ...
            MAT44_VEC(im3d->fim_now->daxes->ijk_to_dicom, xx, yy, zz, px, py, pz);
            if ((intptr_t)666 == (intptr_t)cbs)
            {
                AFNI_creepto_dicom(im3d, px, py, pz);
            }
            else
            {
        ...
        else if (w == cwid->clu_plot_pb[ii] || w == cwid->clu_save_pb[ii])
        {
            ...
            for (nds = ids = 0 ; ids < MAX_CLU_AUXDSET ; ids++)
            {
                ...
                if (imaq != NULL && IMARR_COUNT(imaq) > 0)        /* did we get it? */
                {
                    ...
                    if (nxar[nds] > nxx)
                    {
                        nxx = nxar[nds];            /* longest time series */
                    }
                    if (ibot[nds] < ibx)
                    {
                        ibx = ibot[nds];            /* smallest From */
                    }
                    if (itop[nds] > itx)
                    {
                        itx = itop[nds];                               /* largest To */
                    }
                    nysum  += nyar[nds];                               /* total num time series */
                    nxysum += nyar[nds] * (itop[nds] - ibot[nds] + 1); /* total data pts */
                    nds++;                                             /* add one to the count of
                                                                        * active datasets */
                }
            ...
            if (dohist)
            {
                ...
                if (nbin > kk)
                {
                    nbin = MAX(kk, 8);          /* final setting of num bins */
                }
                ...
                for (nval = ids = 0 ; ids < nds ; ids++) /* build histogram! */
                {
                    ...
                    for (kk = 0 ; kk < nyar[ids] ; kk++)
                    {
                        ...
                        for (jj = ibot[ids] ; jj <= itop[ids] ; jj++)
                        {
                            val = far[jj]; if (val < hbot || val > htop)
                            {
                                continue;
                            }
...

                {
                    ...
                    {
                        PLUTO_histoplot_f(nbin, hbot, htop, hbin[0], xlab, ylab, tlab, nds - 1,
                                          hbin + 1);
                    }
                }
...

                {
                    ...
                    else
                    {
                        ...
                        if (fp != stdout)
                        {
                            ...
                            if (ff)
                            {
                                WARNING_message("Over-wrote file %s", fnam);
                            }
                            else
                            {
                                INFO_message("Wrote file %s", fnam);
                            }
                        }
                    }
            ...
            if (MCW_val_bbox(cwid->detrend_bbox) == 1 && (domean || dopc || domedn))
            {
                ...
                for (ids = 0 ; ids < nds ; ids++)
                {
                    ...
                    for (qq = 0 ; qq < nyar[ids] ; qq++)
                    {
                        ...
                        if (domedn)
                        {
                            THD_generic_detrend_L1(itop[ids] - ibot[ids] + 1, far, pp, 0, NULL,
                                                   NULL);
                        }
                        else
                        {
                            THD_generic_detrend_LSQ(itop[ids] - ibot[ids] + 1, far, pp, 0, NULL,
                                                    NULL);
                        }
                    }
                }
            ...
            {
                imaq = imar[ids]; im[ids] = sim[ids] = NULL; if (imaq == NULL)
                {
                    continue;
                }
                    ...
                    else                      /* no Scat.1D file? */
                    {
                        ...
                        if (cwid->splotim != NULL)
                        {
                            WARNING_message("Scat.1D file [%s] too short [%d] for dataset [%d]",
                                            cwid->splotim->name, cwid->splotim->nx,
                                            nix + ibot[ids]);
                        }
                    }
                        ...
                        if (p025 < pcor && p975 > pcor)
                        {
                            if (strlen(tlab) > 30)
                            {
                                sprintf(tlab + strlen(tlab),
                                        "\\esc\\red  %s=%.2f\\in[%.2f..%.2f]_{95%%}", rlab, pcor,
                                        p025, p975);
                            }
                            else
                            {
                                sprintf(tlab + strlen(tlab),
                                        "\\esc\\red  %s=%.3f\\in[%.3f..%.3f]_{95%%}", rlab, pcor,
                                        p025, p975);
                            }
                            if (p025 * p975 > 0.0f)
                            {
            ...
            if (nim > 0)
            {
                ...
                for (ids = 0 ; ids < nds ; ids++)
                {
                    if (im[ids] != NULL)
                    {
                        if (ibot[ids] < ib)
                        {
                            ib = ibot[ids];         /* smallest bot */
                        }
                        if (itop[ids] > it)
                        {
                            it = itop[ids];         /* largest top */
                        }
                        if (ids0 < 0)
                        {
                            ids0 = ids;
                ...
                else                   /* Save (the world) [26 Oct 2015] */
                {
                    ...
                    else
                    {
                        ...
                        for (ids = 0 ; ids < nds ; ids++)
                        {
                            ...
                            {
                                fprintf(fp, "#  %s[%d..%d]\n",
                                        DSET_PREFIX(cwid->auxdset[inds[ids]]), ibot[ids],
                                        itop[ids]);
                            }
                        }
                        for (qq = 0 ; qq < ibt ; qq++)
                        {
                            for (cc = 0 ; cc < numyar ; cc++)
                            {
                                fprintf(fp, " %14g", yar[cc][qq]);
                            }
                            fprintf(fp, "\n");
                        }
                        fflush(fp);
...

        {
            ...
            if (ISVALID_DSET(fset) && fset->dblk->vedim != NULL && clar != NULL)
            {
                ...
                if (shifted) /* turn all below #ii to same state [12 Jul 2021] */
                {
                    ...
                    for (kk = ii + 1 ; kk < nclu ; kk++)
                    {
                        CLUST_SET(im3d, kk, sss);
                    }
                }
                im3d->vedskip = 1;
        ...
        else if (w == cwid->clu_flsh_pb[ii])
        {
            ...
            if (!CLUST_SEE(im3d, ii))
            {
                EXRETURN;                    /* 16 Oct 2015 */
            }
            STATUS("flashing");
...
#define NFLASH    3
                ...
                for (jj = 0 ; jj < NFLASH ; jj++)
                {
                    ...
                    /* extract data in the ii'th cluster's voxels and store the values the cluster
                     * struct, leaving zeros behind -- this turns the cluster "off" in the images */
                    MCW_vol_to_cluster(vm->nx, vm->ny, vm->nz,
                                       vm->kind, mri_data_pointer(vm), clar->clar[ii]);

                    /* REDISPLAY_FLASH code means that other actions that might happen during image
                     * viewer redisplay are turned off */
                    AFNI_set_viewpoint(im3d, -1, -1, -1, REDISPLAY_FLASH);
                    ...
                    /* shove the data values stored above in the cluster struct back into the
                     * volume, which turns the cluster back "on"  */
...
static MRI_IMARR * AFNI_cluster_timeseries(Three_D_View *im3d, int ncl, int ids)
{
    ...
    if (!IM3D_OPEN(im3d))
    {
        RETURN(NULL);                                                   /* bad viewer */
    }
    if (ids < 0 || ids >= MAX_CLU_AUXDSET)
    {
        RETURN(NULL);                                                   /* bad index */
    }
    cwid = im3d->vwid->func->cwid;  if (cwid == NULL)
    {
        RETURN(NULL);                                                   /* bad widgets */
    }
    if (MCW_val_bbox(cwid->auxdset_bbox[ids]) == 0)
    {
    ...
    if (!ISVALID_DSET(dset))
    {
        RETURN(NULL);                                                   /* bad dataset */
    }
    clar = im3d->vwid->func->clu_list;
    if (clar == NULL || ncl < 0 || ncl >= clar->num_clu)
    {
        RETURN(NULL);                                                   /* bad cluster */
    }
    cl  = clar->clar[ncl];
    npt = cl->num_pt;                      if (npt < 1)
    {
        RETURN(NULL);                                                   /* bad cluster */
    }
...
/* Callback for arrowvals on the linkrbrain correlation type (tasks/genes) report panel just dummy
 * function for now */
...
static char * AFNI_clus_3dclust(Three_D_View *im3d, char *extraopts)
{
...

    {
        ...
        if (im3d->vwid->func->clu_mask != NULL) /* 02 Aug 2011 */
        {
            if (im3d->vednomask)
            {
                sprintf(cmd + strlen(cmd), " -no_inmask");
            }
            else
            {
                sprintf(cmd + strlen(cmd), " -inmask");
            }
        }
        ...
        if (extraopts != NULL)
        {
            sprintf(cmd + strlen(cmd), " %s", extraopts);
        }
...
void AFNI_enforce_throlayx(Three_D_View *im3d)
{
    ...
    if (ithr >= DSET_NVALS(im3d->fim_now))
    {
        EXRETURN;
    }
...
#define GET_MASK_IDC                                                   \
    ...
    dset = im3d->fim_now; if (!ISVALID_DSET(dset))
    {
        EXRETURN;
    }
...
/* Interpolate function y(x) at x=xout, given y(xa)=ya and y(xb)=yb. Assume y(x) = A * x^b, so that
 * log(y(x)) = log(A) + b*log(x). In our application, x=p and y=C(p,alpha) for a fixed alpha.
 *//*--------------------------------------------------------------------------*/
...
/* Find the significance level of a cluster with csiz voxels, which was thresholded at pval.  Return
 * values can be x <= 0   ==> cluster alpha value is greater than |x|
 * x == 0   ==> input error (alpha is N/A) x >  0   ==> cluster alpha value is smaller than x
 *//*--------------------------------------------------------------------------*/
...
float find_cluster_alpha(int csiz, float pval, CLU_threshtable *ctab)
{
    ...
    if (csiz <= 1 || ctab == NULL || pval > ctab->pthr[0])
    {
        return 0.0f;
    }
    ...
    for (ipthr = 1 ; ipthr < npthr && pval <= pthr[ipthr] ; ipthr++)
    {
        ;                                                            /*nada*/
    }
    if (ipthr == npthr)
...
int find_cluster_thresh(float aaa, float pval, CLU_threshtable *ctab)
{
    ...
    if (aaa <= 0.0f || ctab == NULL || pval > ctab->pthr[0])
    {
        return 0;
    }
...
char * get_alpha_string(int csiz, float pval, Three_D_View *im3d)
{
    ...
    if (ctab == NULL)
    {
        return "N/Csim";                   /* no ClustSim */
    }
    if (pval < 0.0f)
    {
        return "N/stat";                   /* not a statistic */
    }
    if (csiz <= 1)
    {
        return "N/Clus";                   /* not a cluster?! */
    }
    if (pval < 1.e-11f)
    {
        return "<<0.01";                   /* == smallest allowed */
    }
...
CLU_threshtable * CLU_get_thresh_table(Three_D_View *im3d)
{
    ...
    if (sig < 0)
    {
        RETURN(NULL);            /* should never transpire */
    }
    pfun = (int)im3d->vedset.param[5]; if (pfun)
    {
        sig = 0;
    }
