...
/*----------------------------------------------------------------
 * given a prefix and a list of files, create a set of datablocks for all datasets that correspond;
    ...
    char *prefixname, THD_string_array *regfile_list)
{
    ...
    if (prefixname == NULL || strlen(prefixname) == 0 ||
        regfile_list == NULL || regfile_list->num <= 0)
    {
        return dblk_arr;
    }
