...
int main(int argc, char *argv[])
{
    ...
    while (nopt < argc)
    {
        ...
        if (strcasecmp(argv[nopt], "-band") == 0)
        {
            ...
            if (nopt + 2 >= argc)
            {
                ERROR_exit("Need 2 arguments after %s", argv[nopt]);
            }
            if (nband >= BMAX)
            {
                ERROR_exit("Too many -band options given :-(");
            }
        ...
        if (strcasecmp(argv[nopt], "-TR") == 0 || strcasecmp(argv[nopt], "-del") == 0)
        {
            if (nopt + 1 >= argc)
            {
                ERROR_exit("Need 1 argument after %s", argv[nopt]);
            }
        ...
        if (strcasecmp(argv[nopt], "-nodata") == 0)
        {
            if (nopt + 1 >= argc)
            {
                ERROR_exit("Need at least 1 argument after %s", argv[nopt]);
            }
            ntime = (int)strtod(argv[++nopt], NULL);
            if (ntime < 9)
            {
                ERROR_exit("Value after -nodata is not allowable: %d", ntime);
            }
            nopt++;
        ...
        if (strcasecmp(argv[nopt], "-input1D") == 0)
        {
            ...
            if (tim == NULL)
            {
                ERROR_exit("Can't read -input1D file '%s'", argv[nopt]);
            }
            ntime = tim->nx; mri_free(tim);
            if (ntime < 9)
            {
                ERROR_exit("Length of -input1D file is not allowable: %d", ntime);
            }
            nopt++; continue;
        }
        ...
        if (strcasecmp(argv[nopt], "-input") == 0)
        {
            ...
            if (nopt >= argc)
            {
                ERROR_exit("Need 1 or more arguments after -input");
            }
            ...
            if (!dtforce)
            {
                DSET_UNMSEC(dset); dt = DSET_TR(dset);
            }
