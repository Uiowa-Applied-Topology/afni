...
/**-------------------------------------------------------------------
 *  Carry out a general affine transform on an image, using bicubic interpolation:
 *     xnew = a11 * xold + a12 * yold + xshift ynew = a21 * xold + a22 * yold + yshift If the input
 *image is MRI_complex, the output will be also;
...
#define THIRTYSIX    2.7777778e-2 /* 1./36.0, actually */
            ...
            if (val < bot)
            {
                nar[ii + jj * nx] = bot;             /* too small! */
            }
            else if (val > top)
            {
                nar[ii + jj * nx] = top;             /* too big!   */
            }
            else
            {
                nar[ii + jj * nx] = val;             /* just right */
            }
        }
    }
    ...
    if (im != imfl)
    {
        mri_free(imfl);               /* throw away unneeded workspace */
    }
    MRI_COPY_AUX(newImg, im);
    RETURN(newImg);
}
 ...
 *     aa  = shift in x bb  = shift in y phi = angle in radians Sort of a replacement for mri_rotate
 *in mri_warp.c; supposed to be faster.  If the input image is MRI_complex, the output will be also;
 * otherwise, the output will be MRI_float.
 * ----------------------------------------------------------------------**/
...
void ft_xshear(float a, float b, int nx, int ny, float *f)
{
    ...
    if (ny % 2 == 1)                              /* we work in pairs, so */
    {
        ...
        for (jj = 0 ; jj < nx ; jj++)
        {
            zz[jj] = 0.0;                         /* of rows, make an extra */
        }
    }
...
void ft_yshear(float a, float b, int nx, int ny, float *f)
{
...

    {
        if (jj < nx - 1)
        {
            for (ii = 0; ii < ny; ii++)
            {
                fj0[ii] = f[jj + ii * nx]; fj1[ii] = f[jj + 1 + ii * nx];
            }
        }
        else
        {
        ...
        if (jj < nx - 1)
        {
            for (ii = 0; ii < ny; ii++)
            {
                f[jj + ii * nx] = fj0[ii]; f[jj + 1 + ii * nx] = fj1[ii];
            }
        }
        else
        {
