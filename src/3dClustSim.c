...
static double athr_mega[22] = { 0.100, 0.095, 0.090, 0.085, 0.080, 0.075, 0.070, 0.065, 0.060,
                                0.055,
...
/* the output:
 *   2D table of cluster size threshold as function of per-voxel threshold and volumetric alpha
...
/* max_table_1sid[nnn][ipthr][cc] is the count of how often the maximum cluster had exact size cc
 * voxels (out of niter trials), at the pthr[ipthr] voxel-wise threshold p-value, for the nnn-th NN
 * level (1-3), for 1-sided voxel-wise thresholding. Mutatis mutandis for max_table_2sid and
 * max_table_bsid (bi-sided threshold) */

static int **max_table_1sid[4], **max_table_2sid[4], **max_table_bsid[4];

/* cmx_table_1sid[nnn][ipthr] = max cluster size found over all simulations for 1-sided clustering
 * with NN level nnn at voxel-wise threshold pthr[ipthr].  That is, it is the largest value of cc
 * such that max_table_1sid[nnn][ipthr][cc] > 0.  [23 Dec 2015] */

static int *cmx_table_1sid[4], *cmx_table_2sid[4], *cmx_table_bsid[4];

/* csiz_1D_NN1[ipthr][iter] = max cluster size found at the ipthr-th threshold in the iter-th
 * iteration.  [23 Dec 2015] */
    ...
    while (nopt < argc)
    {
        ...
        if (strcmp(argv[nopt], "-inset") == 0) /* 02 Feb 2016 */
        {
            ...
            for ( ; nopt < argc && argv[nopt][0] != '-' ; nopt++)
            {
                ...
                {
                    ERROR_message("-inset '%s': grid size doesn't match other datasets",
                                  argv[nopt]);
                    nbad++;
                }
                inset = (THD_3dim_dataset **)realloc(inset,
                                                     sizeof(THD_3dim_dataset *) * (num_inset + 1));
            ...
            if (num_inset == 0)
            {
                ERROR_exit("no valid datasets opened after -inset :-(");
            }
            if (nbad > 0)
            {
                ERROR_exit("can't continue after above -inset problems");
            }
            continue;
        }
        ...
        if (strcasecmp(argv[nopt], "-insdat") == 0)
        {
            ...
            for (ii = nopt ; ii < argc && argv[ii][0] != '-' ; ii++)
            {
                ;                                               /*nada*/
            }
            nfile = ii - nopt;
        ...
        if (strcmp(argv[nopt], "-nxyz") == 0)
        {
            nopt++; if (nopt + 2 >= argc)
            {
                ERROR_exit("need 3 arguments after -nxyz");
            }
            nx = (int)strtod(argv[nopt++], NULL); if (nx <= 0)
            {
                ERROR_exit("illegal nx value");
            }
            ny = (int)strtod(argv[nopt++], NULL); if (ny <= 0)
            {
                ERROR_exit("illegal ny value");
            }
            nz = (int)strtod(argv[nopt++], NULL); if (nz <= 0)
            {
                ERROR_exit("illegal nz value");
            }
            continue;
        }
        ...
        if (strcmp(argv[nopt], "-dxyz") == 0)
        {
            nopt++; if (nopt + 2 >= argc)
            {
                ERROR_exit("need 3 arguments after -dxyz");
            }
            dx = strtod(argv[nopt++], NULL); if (dx <= 0.0f)
            {
                ERROR_exit("illegal dx value");
            }
            dy = strtod(argv[nopt++], NULL); if (dy <= 0.0f)
            {
                ERROR_exit("illegal dy value");
            }
            dz = strtod(argv[nopt++], NULL); if (dz <= 0.0f)
            {
                ERROR_exit("illegal dz value");
            }
            continue;
        }
        ...
        if (strcmp(argv[nopt], "-tdof") == 0) /* 26 May 2015 -- hidden option */
        {
            nopt++; if (nopt >= argc)
            {
                ERROR_exit("need argument after -tdof!");
            }
            tdof = (float)strtod(argv[nopt], NULL);
            if (tdof > 0.0f && tdof < 4.0f)
            {
                ERROR_exit("illegal value after -tdof");
            }
            nopt++; continue;
        }
        ...
        if (strcmp(argv[nopt], "-mask") == 0)
        {
            ...
            if (mask_dset == NULL)
            {
                ERROR_exit("can't open -mask dataset!");
            }
            mask_vol = THD_makemask(mask_dset, 0, 1.0, 0.0);
            if (mask_vol == NULL)
            {
                ERROR_exit("can't use -mask dataset!");
            }
            ...
            if (mask_ngood < minmask)
            {
                if (minmask > 2 && mask_ngood > 2)
                {
                    ERROR_message("-mask has only %d nonzero voxels; minimum allowed is %d.",
                                  mask_ngood, minmask);
                    ERROR_message(
                        "To run this simulation, please read the CAUTION and CAVEAT in -help,");
                    ERROR_message(
                        "and then you can use the '-OKsmallmask' option if you so desire.");
            ...
            if (verb)
            {
                INFO_message("%d voxels in mask (%.2f%% of total)",
        ...
        if (strcasecmp(argv[nopt], "-fwhm") == 0)
        {
            nopt++; if (nopt >= argc)
            {
                ERROR_exit("need argument after -fwhm");
            }
        ...
        if (strcasecmp(argv[nopt], "-acf") == 0)
        {
            nopt++; if (nopt + 2 >= argc)
            {
                ERROR_exit("need 3 arguments after -acf");
            }
        ...
        if (strcasecmp(argv[nopt], "-fwhmxyz") == 0)
        {
            nopt++; if (nopt + 2 >= argc)
            {
                ERROR_exit("need 3 arguments after -fwhmxyz");
            }
            fwhm_x = (float)strtod(argv[nopt++], &ep);
        ...
        if (strcmp(argv[nopt], "-pthr") == 0 || strcmp(argv[nopt], "-pval") == 0)
        {
            ...
            else                              /* a list of values */
            {
                ...
                if (npthr <= 0)
                {
                    ERROR_exit("No positive values found after %s", argv[nopt - 1]);
                }
                pthr = (double *)realloc(pthr, sizeof(double) * npthr);
                for (ii = 0 ; ii < npthr ; ii++)
                {
                    ...
                    {
                        ERROR_exit("value '%s' after '%s' is illegal!", argv[nopt + ii],
                                   argv[nopt - 1]);
                    }
                }
                if (npthr > 1)
                {
                    for (ii = 0 ; ii < npthr ; ii++)
                    {
                        pthr[ii] = -pthr[ii];                     /* sort into */
                    }
                    qsort_double(npthr, pthr);                    /* descending */
                    for (ii = 0 ; ii < npthr ; ii++)
                    {
                        pthr[ii] = -pthr[ii];                     /* order */
                    }
                    for (ii = 1 ; ii < npthr ; ii++)
                    {
                        if (pthr[ii] == pthr[ii - 1])
                        {
                            WARNING_message("duplicate value %g after '%s'", pthr[ii],
                                            argv[nopt - 1]);
                        }
                    }
                }
            ...
            else                              /* a list of values */
            {
                ...
                if (nathr > 1)
                {
                    ...
                    for (ii = 1 ; ii < nathr ; ii++)
                    {
                        ...
                        {
                            WARNING_message("duplicate value %g after '%s'", athr[ii],
                                            argv[nopt - 1]);
                        }
                    }
                }
        ...
        if (strcmp(argv[nopt], "-cmd") == 0)
        {
            nopt++; if (nopt >= argc)
            {
                ERROR_exit("need argument after -cmd!");
            }
            cmd_fname = strdup(argv[nopt]);
            if (!THD_filename_ok(cmd_fname))
            {
                ERROR_exit("bad -cmd option!");
            }
            nopt++; continue;
        }
...
#define INSET_PRELOAD
    ...
    if (num_inset > 0)      /* 02 Feb 2016 */
    {
        ...
        for (niter = qq = 0 ; qq < num_inset ; qq++)
        {
            niter += nval_inset[qq];
#ifdef INSET_PRELOAD
            ININFO_message("loading -inset '%s' with %d volumes", DSET_HEADNAME(inset[qq]),
                           DSET_NVALS(inset[qq]));
            DSET_load(inset[qq]);
        ...
        if (nbad > 0)
        {
            ERROR_exit("Can't continue without all -inset datasets :-(");
        }
        if (niter < 100)
        {
    ...
    else if (xinset != NULL)             /*--- 29 Aug 2016 ---*/
    {
        ...
        if (do_ssave)
        {
            WARNING_message("-insdat turns off -ssave"); do_ssave = 0;
        }
        if (do_acf)
        {
            WARNING_message("-insdat turns off -acf"); do_acf = 0;
        }
    }
    ...
    if (mask_vol == NULL && do_ball)
    {
        ...
        if (verb)
        {
            INFO_message("%d voxels in BALL mask (%.1f%% of total)",
                         mask_ngood, 100.0 * mask_ngood / (double)mask_nvox);
        }
    }
    ...
    sigmax = FWHM_TO_SIGMA(fwhm_x); if (nx < 2)
    {
        sigmax = 0.0f;
    }
    sigmay = FWHM_TO_SIGMA(fwhm_y); if (ny < 2)
    {
        sigmay = 0.0f;
    }
    sigmaz = FWHM_TO_SIGMA(fwhm_z); if (nz < 2)
    {
        sigmaz = 0.0f;
    }
...
void generate_fim_inset(float *fim, int ival)
{
    ...
    else
    {
        ...
        {
            if (qval < nval_inset[qq])
            {
                break;                               /* inset[qq] to use */
            }
            qval -= nval_inset[qq];
        }
...
void generate_fim_acf(float *fim, unsigned short xran[])
{
    ...
    /* the function generates images in pairs, but 3dClustSim only consumes them one at a time --
     * thus the artifice of the 'aim'
     * and 'bim' images, which are created together, then aim is consumed right away and bim is
     *consumed on the next call here. */
...
void generate_fim_padded(float *fim, float *pfim, unsigned short xran[])
{
    ...
    for (ii = 0 ; ii < nxyz_pad ; ii++)
    {
        pfim[ii] = zgaussian_sss(xran);
    }
...
void generate_image(float *fim, float *pfim, unsigned short xran[], int iter)
{
    ...
    if (num_inset == 0 && xinset == NULL && tdof > 0.0f)
    {
        ...
        for (ii = 0 ; ii < nxyz ; ii++)
        {
            ...
            {
                zhat  = fim[ii] * zfac;
                denom = 1.0f - zhat * zhat * tfac; if (denom < 0.1f)
                {
                    denom = 0.1f;
                }
...
/* 'shave' == 'short save' == saving generated images for reuse in the sumup phase of the program.
 * Only the "in-mask" part is saved, and values are scale to shorts. Memory is malloc-ed for small
 * needs, and is mmap-ed for large needs.
 *//*-------------------------------------------------------------------------*/
...
#pragma omp critical
    ...
    if (mask_vol != NULL)
    {
        for (jj = ii = 0 ; ii < nxyz ; ii++)
        {
            if (mask_vol[ii])
            {
                shar[jj++] = (short)(fim[ii] * SHAVE_FAC);
            }
        }
    }
    else
    {
        for (ii = 0 ; ii < nxyz ; ii++)
        {
            shar[ii] = (short)(fim[ii] * SHAVE_FAC);
        }
    }
    return;
}
...
int find_largest_cluster_NN1(byte *mmm, int ithr)
{
    ...
    while (1)
    {
        ...
        /* loop over points in cluster, checking their neighbors, growing the cluster if we find any
         * that belong therein */
...
void gather_stats_NN1_1sid(int ipthr, float *fim, byte *bfim, int *mtab, int ithr, int iter)
{
    ...
    if (siz > max_cluster_size)
    {
        siz = max_cluster_size;
    }
                       ...
                       float *fim, byte *bfim, int ithr)
{
    ...
    for (ipthr = ipthr_bot ; ipthr <= ipthr_top ; ipthr++)
    {
        ...
        if (do_athr_sum < 2)
        {
            continue;           /* skip 2-sided and bi-sided */
        }
...
#pragma omp critical
              { if (fa_1sid_NN1[ithr])
                {
                    rfa_1sid_NN1[iathr] += drfa;
                }
                if (fa_1sid_NN2[ithr])
                {
                    rfa_1sid_NN2[iathr] += drfa;
                }
                if (fa_1sid_NN3[ithr])
                {
                    rfa_1sid_NN3[iathr] += drfa;
                }
                if (do_athr_sum > 1)
                {
                    if (fa_2sid_NN1[ithr])
                    {
                        rfa_2sid_NN1[iathr] += drfa;
                    }
                    if (fa_2sid_NN2[ithr])
                    {
                        rfa_2sid_NN2[iathr] += drfa;
                    }
                    if (fa_2sid_NN3[ithr])
                    {
                        rfa_2sid_NN3[iathr] += drfa;
                    }
                    if (fa_bsid_NN1[ithr])
                    {
                        rfa_bsid_NN1[iathr] += drfa;
                    }
                    if (fa_bsid_NN2[ithr])
                    {
                        rfa_bsid_NN2[iathr] += drfa;
                    }
                    if (fa_bsid_NN3[ithr])
                    {
                        rfa_bsid_NN3[iathr] += drfa;
                    }
                }
              }
          }
...
/* from the max_table, get the threshold cluster size for a given per-voxel threshold, for a given
...
double get_one_clust_thresh(int **mtnn, int ipthr, double aval)
{
    ...
    else
    {
        for (ii = 1 ; ii < itop ; ii++)
        {
            if (alpha[ii] >= aval && alpha[ii + 1] <= aval)
            {
                break;
            }
        }
    ...
    if (alo >= 1.0)
    {
        alo = 1.0 - 0.1 / niter;                      /* unlikely */
    }
    if (ahi <= 0.0)
    {
        ahi = 0.1 / niter;              /* should not be possible */
    }
    if (ahi >= alo)
    {
        ahi = 0.1 * alo;                /* should not be possible */
    }
    jj  = log(-log(1.0 - aval));
    alo = log(-log(1.0 - alo));
    ahi = log(-log(1.0 - ahi));
...
#pragma omp barrier  /* all threads wait until the above is finished */
        ...
        if (do_pad)
        {
            pfim = (float *)malloc(sizeof(float) * nxyz_pad);   /* 12 May 2015 */
        }
        else
        {
            pfim = NULL;
...
#pragma omp for
        ...
        free(fim); free(bfim); if (pfim != NULL)
        {
            free(pfim);
        }
        ...
        if (!do_athr_sum)
        {
            ...
            {
                free(acf_tar[ithr]);
                if (acf_bim[ithr] != NULL)
                {
                    mri_free(acf_bim[ithr]);
                }
            }
        }
    ...
    if (do_ssave > 0)
    {
        DSET_delete(ssave_dset);                /* 24 Apr 2014 */
    }
    ...
    /* For each table, find the largest cluster size cc such that max_table_xxxx[nnn][ipthr][cc] > 0
     * [23 Dec 2015] */
      ...
      for (nnn = 1 ; nnn <= 3 ; nnn++)
      {
          for (ipthr = 0 ; ipthr < npthr ; ipthr++)
          {
              for (cc = max_cluster_size ;
                   cc >= 1 && max_table_1sid[nnn][ipthr][cc] == 0 ; cc--)
              {
                  ;                                                   /*nada*/
              }
              cmx_table_1sid[nnn][ipthr] = cc;
              for (cc = max_cluster_size ;
                   cc >= 1 && max_table_2sid[nnn][ipthr][cc] == 0 ; cc--)
              {
                  ;                                                   /*nada*/
              }
              cmx_table_2sid[nnn][ipthr] = cc;
              for (cc = max_cluster_size ;
                   cc >= 1 && max_table_bsid[nnn][ipthr][cc] == 0 ; cc--)
              {
                  ;                                                   /*nada*/
              }
    ...
    { double *alpha, aval, ahi, alo;
...

      {
...

          {
              ...
              for (ipthr = 0 ; ipthr < npthr ; ipthr++)
              {
                  ...
                  for (iathr = 0 ; iathr < nathr ; iathr++)
                  {
                      ...
                      if (clust_thresh[ipthr][iathr] > cmax)
                      {
                          cmax = clust_thresh[ipthr][iathr];
                      }
                  }
#else
                  for (itop = ii = 1 ; ii <= max_cluster_size ; ii++)
              ...
              if (do_niml) /* output in NIML format */
              {
                  ...
                  if (prefix != NULL)
                  {
                      strcpy(fname, pname); strcat(fname, "niml");
                  }
                  else
                  {
                      strcpy(fname, "stdout:");
              ...
              if (nnn * 10 + mmm == 11)
              {
                  clust_thresh_1sid_NN1 = clust_thresh;
              }
              if (nnn * 10 + mmm == 12)
              {
                  clust_thresh_2sid_NN1 = clust_thresh;
              }
              if (nnn * 10 + mmm == 13)
              {
                  clust_thresh_bsid_NN1 = clust_thresh;
              }
              if (nnn * 10 + mmm == 21)
              {
                  clust_thresh_1sid_NN2 = clust_thresh;
              }
              if (nnn * 10 + mmm == 22)
              {
                  clust_thresh_2sid_NN2 = clust_thresh;
              }
              if (nnn * 10 + mmm == 23)
              {
                  clust_thresh_bsid_NN2 = clust_thresh;
              }
              if (nnn * 10 + mmm == 31)
              {
                  clust_thresh_1sid_NN3 = clust_thresh;
              }
              if (nnn * 10 + mmm == 32)
              {
                  clust_thresh_2sid_NN3 = clust_thresh;
              }
              if (nnn * 10 + mmm == 33)
              {
                  clust_thresh_bsid_NN3 = clust_thresh;
              }
#endif
          ...
          if (amesg != NULL)
          {
              /* WARNING_message cannot handle "%s" with a very long string (>16K), so break this up
               * a little                      20 Feb 2014 [rickr] */
