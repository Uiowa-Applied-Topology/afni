...
int main(int argc, char *argv[])
{
    ...
    if (strcmp(argv[iarg], "-") != 0)
    {
        ...
        if (ifp == NULL)
        {
            ERROR_exit("Can't open input file '%s'", argv[iarg]);
        }
    ...
    else
    {
        ...
        if (ii == 0)
        {
            ERROR_exit("Can't create directory '%s'", prefix);
        }
    ...
    if (ofp == NULL)
    {
        ERROR_exit("Can't open file '%s' for output", imagename);
    }
...
GotLine:
    ...
    if (sar->str[0][0] == '#')
    {
        ...
        else if (strncasecmp(cpt, "gam", 3) == 0 && dpt != NULL)
        {
            ...
            if (qam > 0.0f)
            {
                gam = qam; INFO_message("Set gamma = %f", gam);
            }
        }
        goto GetLine;
    }
    ...
    if (sar->num > 1)
    {
        ...
        for (ii = 1 ; ii < sar->num ; ii++)
        {
            fprintf(ofp, " %s", sar->str[ii]);
        }
        fprintf(ofp, "</small>\n");
    }
    fprintf(ofp, "</td>\n");
...
static void GAL_imageize(char *iname, char *prefix, int lab)
{
    ...
    inim = mri_read(iname); if (inim == NULL)
    {
        return;
    }
