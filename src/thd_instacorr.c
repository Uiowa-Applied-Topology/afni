...
static MRI_vectim * THD_instacorr_tsprep(ICOR_setup *iset, THD_3dim_dataset *dset)
{
    ...
    if (iset->ftop <= iset->fbot)
    {
        iset->ftop = 999999.9f;                             /* infinity */
    }
    dvec = (float **)malloc(sizeof(float *) * nmmm);
    for (iv = 0 ; iv < nmmm ; iv++)
    {
        dvec[iv] = VECTIM_PTR(mv, iv);
    }
...
int THD_instacorr_prepare(ICOR_setup *iset)
{
    ...
    if (iset == NULL || !ISVALID_DSET(iset->dset))
    {
        EM("no dataset"); RETURN(0);
    }
    if (DSET_NVALS(iset->dset) < 4)
    {
        EM("nvals < 4"); RETURN(0);
    }
    if (ISVALID_DSET(iset->eset) &&
        (DSET_NVOX(iset->eset) != DSET_NVOX(iset->dset) ||
         DSET_NVALS(iset->eset) != DSET_NVALS(iset->dset)))
    {
        EM("eset <> dset"); RETURN(0);
    }
...
float * THD_instacorr_getseed(ICOR_setup *iset, int ijk)
{
    ...
    if (iset == NULL || iset->mv == NULL || ijk < 0)
    {
        RETURN(NULL);
    }
    ...
    if (sblur != 0.0f)
    {
        ...
        {
            qi = ii + smask->i[kk]; if (qi < 0 || qi >= nx)
            {
                continue;
            }
            qj = ij + smask->j[kk]; if (qj < 0 || qj >= ny)
            {
                continue;
            }
            qk = ik + smask->k[kk]; if (qk < 0 || qk >= nz)
            {
                continue;
            }
            qjk = qi + qj * nx + qk * nxy;
            ...
            if (qq >= 0)
            {
                ...
                if (gblur)
                {
                    float rad = smask->mag[kk]; wt = exp(-fac * rad * rad);
                }
                else
                {
                    wt = 1.0f;
...
MRI_IMAGE * THD_instacorr(ICOR_setup *iset, int ijk)
{
    ...
    if (iset != NULL)
    {
        ININFO_message(" iset->mv=%p", (void *)iset->mv);
    }
#endif
    ...
    iter_count = iset->iter_count; if (iter_count <= 0)
    {
        iter_count = 1;
    }
    ...
    if (iter_count > 1)
    {
        nsar = (float *)malloc(sizeof(float) * nvals);
    }
...
/*! Compute the instant correlation from tsar as the seed series, from time indexes ibot to itop-1
 * -- mri_free() result when you are done with it
 *//*-------------------------------------------------------------------------*/
...
MRI_IMAGE * THD_instacorr_section(ICOR_setup *iset, float *tsar, int ibot, int itop)
{
    ...
    if (iset == NULL || iset->mv == NULL || tsar == NULL)
    {
        RETURN(NULL);
    }
    if (ibot < 0)
    ...
    if (itop > iset->mv->nvals - 1)
    {
        itop = iset->mv->nvals - 1;
    }
    if (itop - ibot < 5)
    {
        RETURN(NULL);                  /* too short to correlate */
    }
#if 0
