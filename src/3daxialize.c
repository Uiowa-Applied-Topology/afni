...
static RwcBoolean write_output = False;           /* 08 Aug 2006 [dg] -force rewrite as in 3drefit
                                                   * by rickr */
...
static int        AXIAL_frugal = 0;               /* Saving sub-bricks one at a time is optional 18
                                                   * Mar 2009 [dg] */
...
int main(int argc, char *argv[])
{
    ...
    if (verbose)
    {
        INFO_message("Loading input dataset %s", argv[iarg]);
    }
    ...
    {
        write_output = 1;         /* have to write everything out now*/
    }
