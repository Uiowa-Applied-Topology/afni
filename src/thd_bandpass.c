...
/*! Check THD_bandpass_vectors() input parameters for OK-ness. Returns 1 if OK, 0 if not OK.  If
 * verb!=0, prints an info message.
 *//*------------------------------------------------------------------------*/
...
int THD_bandpass_OK(int nx, float dt, float fbot, float ftop, int verb)
{
    ...
    if (ftop > ICOR_MAX_FTOP)
    {
        return 1;                         /* 26 Feb 2010 */
    }
    if (nx < 9)
    {
        return 0;
    ...
    if (ftop <= fbot || ftop > ICOR_MAX_FTOP)
    {
        ftop = ICOR_MAX_FTOP;
    }
    if (bpwrn && dt > 60.0f)
...
/*! Return the number of degrees of freedom that would remain after bandpassing (the dimension of
 * the subspace the data will be projected into).
 *
 *  Returns twice the length of the passband index range. Returns 0 if life is bad.
 *
...
int THD_bandpass_remain_dim(int nx, float dt, float fbot, float ftop, int verb)
{
    ...
    if (nx < 9)
    {
        if (verb)
        {
            WARNING_message("length %d too short for bandpassing", nx);
        }
        return 0;
    }
 ...
 * - To do a highpass only, set ftop to something larger than the Nyquist frequency (e.g.,
 *999999.9).  To do a lowpass only, set fbot to 0.0.
 * - However, the 0 and Nyquist frequencies are always removed.
 * - Return value is the number of linear dimensions projected out. If 0 is returned, something
 *funky happened.
 *//*------------------------------------------------------------------------*/
                         ...
                         int qdet, int nort, float **ort)
{
    ...
    if (jtop >= nby2)
    {
        jtop = nhalf;                   /* can't go past Nyquist! */
    }
    if (jbot >= jtop + 1)
    {
        jbot = 0; jtop = nhalf;
...

    {
        ...
        if (jbot >= 1)
        {
            ndof += 2 * jbot - 1;          /* DOF for low freq */
        }
        ...
        for (iv = 0 ; iv < nvec ; iv += 2)
        {
            ...
            else
            {
                ...
                for (jj = 0 ; jj < nlen ; jj++)
                {
                    zar[jj].r = xar[jj]; zar[jj].i = yar[jj];
                }
            }
            for (jj = nlen ; jj < nfft ; jj++)
            {
                zar[jj] = Zero;        /* zero fill */
            }
            ...
            else
            {
                for (jj = 0 ; jj < nlen ; jj++)
                {
                    xar[jj] = zar[jj].r; yar[jj] = zar[jj].i;
                }
            }
    ...
    if (nort > 0 && ort != NULL)
    {
        ...
        /* must bandpass copy of orts first -- via recursion (so we don't re-introduce any of the
         * removed frequencies) */
         ...
         * this is more efficient than computing the nlen X nlen projection matrix [I]-[Q][P] and
         *then applying to each vector, since that would require about nlen*nlen flops per vector,
         *whereas the 2 matrix multiplications by [P] then [Q] will require about 2*nlen*nort flops
         *per vector, a clear win if nort < nlen/2. */
...
/*! Remove spikes from a time series, in a very simplistic way. Return value is the number of spikes
 * that were squashed [RWCox].
 *//*-----------------------------------------------------------------------*/
...
THD_3dim_dataset * THD_despike9_dataset(THD_3dim_dataset *inset, byte *mask)
{
    ...
    if (!ISVALID_DSET(inset) || DSET_NVALS(inset) < 9)
    {
        RETURN(NULL);
    }
