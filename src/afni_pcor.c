 ...
 *    input:  numref = number of reference vectors to allow for output: pointer to the data
 *structure
 ***/
...
PCOR_references * new_PCOR_references(int numref)
{
    ...
    /*** allocate storage for Cholesky factor (an array of rows, row #ii is length ii+1, for
     * ii=0..numref-1) ***/
 ...
 *    input:  vec = pointer to nref values of the reference vectors at the new time point ref =
 * references data structure output: ref is updated;
 *              the Cholesky factor is modified via the Carlson algorithm, the alpha, f, and g
 *factors are saved for later use
 ***/
...
/***
 * create a new voxel partial correlation data structure inputs:  numvox = number of voxels in image
 *numref = number of reference vectors to allow for output:  pointer to voxel_corr data structure
 ***/
 ...
 *   "fit" is an array of pointers to floats, of length nref. If fit[j] != NULL, then it points to
 *an array of size nvox that will get the coefficient for reference #j (j=0..nref-1).  [June 1995]
 * ***/
...
void PCOR_get_lsqfit(PCOR_references *ref, PCOR_voxel_corr *vc, float *fit[])
{
    ...
    for (vox = 0 ; vox < nv ; vox++)
    {
        ...
        {
            sum = VCH(vc, vox, jj);
            for (kk = jj + 1 ; kk < nr ; kk++)
            {
                sum -= ff[kk] * RCH(ref, kk, jj);
            }
 ...
 *   08 Sep 1999:
 *     basaver = 0 ==> baseline for percent change calculation is the bottom of the curve [old
 *method]
                   ...
                   float *coef, float *bline, int basaver)
{
    ...
    /*** Work:                               jj=nr-2 x(t) = fit[nr-1] * ref(t,nr-1) + SUM fit[jj] *
     * ref(t,jj) jj=0
     *
     *   The percent change due to ref(t,nr-1) is computed by scaling this ref to run from 0 to 1
     *(that's why rmin and rmax are used), and by computing the baseline as fit[nr-1]*rmin + the sum
    ...
    for (vox = 0 ; vox < nv ; vox++)
    {
        ...
        if (coef != NULL)
        {
            coef[vox] = (base > 0.0) ? ff[nr - 1] * rdif / base
                                                  : 0.0;
        }
        if (bline != NULL)
        {
