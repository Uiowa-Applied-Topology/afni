...
/*---------------------------------------------------------------
 * Routine to extract a slice (fixed value of 3rd dimension) from a previously set up FD_brick
 *structure. November 1995: may return an image of any legal type, not just shorts.
 * -----------------------------------------------------------------*/
...
MRI_IMAGE * FD_brick_to_mri(int kslice, int ival, FD_brick *br)
{
    ...
    case MRI_byte: {
        ...
        {
            for (ii = 0 ; ii != ei ; ii += di)
            {
                ar[pp++] = bar[ii + (jj + base)];
            }
        }
    }
    break;
