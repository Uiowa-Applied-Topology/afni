...
/***
 *   recursive calculation of partial correlation coefficients for a lot of voxels at once. --
 * RWCox, Feb 1994
 ***/
...
void update_references(float *vec, references *ref)
{
    ...
    for (jj = 0 ; jj < nr ; jj++)
    {
        zz[jj] = (ref_float)vec[jj];
    }

#ifdef OV_DEBUG2
    for (jj = 0 ; jj < nr ; jj++)
    {
        qinput[jj] += SQR(zz[jj]);                          /* for later */
    }
    REF_DUMP(ref, "before update");
    fprintf(stderr, "  input vec= ");
    for (jj = 0 ; jj < nr ; jj++)
    {
        fprintf(stderr, "%11.4e ", zz[jj]);
    }
    fprintf(stderr, "\n");
#endif
    ...
    for (jj = 0 ; jj < nr ; jj++)
    {
        fprintf(stderr, "%11.4e ", qinput[jj]);
    }
    fprintf(stderr, "\n");
#endif
...
/*** update all voxels with a new array of data inputs:  vdata = array[nvox] of new data for each
 * voxel ref   = pointer to references structure to use vc    = pointer to correlation data
 * structure output:  updated vc
 ***/
 ...
 *   if cothresh > 0, then voxels whose |coef| is less than cothresh * max|coef| will be also be set
 *to zero
 ***/
