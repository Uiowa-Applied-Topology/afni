...
extern void set_HTTP_user_agent(char *ua)
{
    if (ua == NULL || *ua == '\0')
    {
        http_user_agent = "read_URL";
    }
    else
    {
        http_user_agent = strdup(ua);
    }
}
...
/*---------------------------------------------------------------------
 * Open an "http://" URL in host, port, and filename pieces. Wait up to msec milliseconds for
 * network functions to occur. If an error occurs, return NULL, otherwise the caller can read from
 * this IOCHAN (using iochan_recv, etc.).
 * -----------------------------------------------------------------------*/
...
IOCHAN * open_URL_hpf(char *host, int port, char *file, int msec)
{
    ...
    if (ii <= 0)
    {
        FAILED; IOCHAN_CLOSE(ioc); return NULL;
    }
...
/*----------------------------------------------------------------------
 * Open an "http://" URL and prepare to read it (but the caller must actually do the reading).  If
 * NULL is returned, an error occurred.
 * ------------------------------------------------------------------------*/
...
IOCHAN * open_URL_http(char *url, int msec)
{
    ...
    if (url == NULL || strstr(url, HTTP) != url)
    {
        return NULL;
    }
...
char *af_strnstr(char *s1, char *s2, size_t n)
{
    ...
    if (s1 && (n1 = strlen(s1)) > n)
    {
        c1 = s1[n]; s1[n] = '\0';
    }
    if (s2 && (n2 = strlen(s2)) > n)
    {
        c2 = s2[n]; s2[n] = '\0';
    }
...
/*
 * A very simple parsing of HTTP1.1 header fields. Assumes buf and hname are all upper case.
 * Function does not know to avoid searching beyond end of headers             ZSS Mar. 2011
 */
...
char *HTTP_header_val(char *head, char *hname, size_t max_head)
{
    ...
    if (!af_strnstr(head, "HTTP/1.1", 36))
    {
        return NULL;
    }
    if (max_head <= 0)
...
/*---------------------------------------------------------------
 * Read an "http://" URL, with network waits of up to msec milliseconds allowed.  Returns number of
 * bytes read -- if this is > 0, then *data will be a pointer to malloc-ed bytes holding the
 * contents of the file.
 *
 * If the file is gzip-ed, then it will be un-gzip-ed before being loaded into memory.  This uses
 * temporary files in $TMPDIR or
 * /tmp, which must have space to hold the compressed and uncompressed file.  If the file is not
 * compressed, then input is directly to memory and no temporary files are used.
 *
 * read_URL_http11 is a new version that is meant to handle HTTP1.1 header and return as soon as
 * entity body is read
 * -----------------------------------------------------------------*/
...
int page_append(char *buf, int n_buf, URL_PAGE *up, int null_term)
{
    if (up->N_page + n_buf > up->N_alloc)
    {
        do {
            up->N_alloc += QBUF;
        } while (up->N_alloc <= up->N_page + n_buf);
...
int page_scan_head(URL_PAGE *up)
{
    ...
    for (i = 0; i < up->N_head; ++i)
    {
        up->page[i] = toupper(up->page[i]);
    }
...
/* return a copy of the header. Caller must free pointer */
char *page_header_copy(URL_PAGE *up)
{
    char *hcp = NULL;

    if (!up->page || !up->head_complete)
    {
        return NULL;
    }
...
/* return a pointer to the beginning of the content. Do not free this pointer */
...
int page_received(URL_PAGE *up)
{
    ...
    if (up->head_complete)
    {
        ...
        up->N_cont = HTTP_header_long_val(up->page, "CONTENT-LENGTH:",
                                          up->N_head, -1);
        if (up->N_cont >= 0 && up->N_page >= up->N_head + up->N_cont)
        {
            return 1;
        }
    }
...
/*!
 * Read an HTTP/1.1 url url: The URL msec: Number of msec to wait before abandoning all hope data: A
 * non null pointer to a null char *pointer that will hold the content of the response. At call
 * time, data != NULL and *data = NULL head: If not null, *head will contain all the headers in the
 * response
 *
...
int read_URL_http(char *url, int msec, char **data)
{
    ...
    if (use_http_ver == 11)
    {
        return read_URL_http11(url, msec, data, NULL);
    }

    /* sanity check */

    if (url == NULL || data == NULL || msec < 0)
    {
        return -1;
    }
    ...
    if (!cflag)
    {
        buf = AFMALL(char, QBUF); nall = QBUF;
    }
    nuse = 0; first = 1;
...
void set_URL_ftp_ident(char *name, char *pwd)       /* 05 Apr 1999 */
{
    ...
    ll = strlen(name); if (ll < 1 || ll > 127)
    {
        return;
    }
    ll = strlen(pwd); if (ll < 1 || ll > 127)
    {
        return;
    }
...
int read_URL_ftp(char *url, char **data)
{
    ...
    if (url == NULL || data == NULL || strstr(url, FTP) != url)
    {
        return -1;
    }
    ...
    *h = '\0'; if (hostname[0] == '\0')
    ...
    mkstemp(sname);            if (sname[0] == '\0')
    {
        return -1;
    }
    sp = fopen(sname, "w"); if (sp == NULL)
    {
        return -1;
    }
    ...
    buf = AFMALL(char, nuse); if (buf == NULL)
    {
        unlink(qname); return -1;
    }
...
/*-------------------------------------------------------------------
 * Read a URL (ftp:// or http://) into memory.  The return value is the number of bytes read, and
 * *data points to the data. If the return value is negative, then something bad happened.
 * ---------------------------------------------------------------------*/
...
/*------------------------------------------------------------------
 * Read a URL and save it to disk in tmpdir.  The filename it is saved in is returned in the
 * malloc-ed space *tname. The byte count is the return value of the function;
...
int read_URL_tmpdir(char *url, char **tname)
{
    ...
    if (ll > 3 && strcmp(fname + (ll - 3), ".gz") == 0)
    {
        fname[ll - 3] = '\0';
    }
