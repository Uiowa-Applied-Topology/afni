...
/*-----------------------------------------------------------------------
 * This routine is used to hide the Xt warnings. It simply does nothing -- it replaces the default
 *Xt warning handler.
 * -------------------------------------------------------------------------*/
...
int main(int argc, char *argv[])
{
    ...
    /* initialize defaults for command line arguments that aren't related to dataset construction
     *    */
    ...
    /* 05 Feb 2001: [10 May 2001: moved up above auto-save] if geometry not loaded at all, maybe use
     * globals imdx, etc. */
    ...
    if (!(geomparent_loaded || geometry_loaded))
    {
            ...
            user_inputs.xorient == user_inputs.zorient)
        {
            ...
            acod = toupper(MRILIB_orients[0]); icod = ORCODE(acod); if (icod >= 0)
            {
                user_inputs.xorient = icod;
            }
            acod = toupper(MRILIB_orients[2]); icod = ORCODE(acod); if (icod >= 0)
            {
                user_inputs.yorient = icod;
            }
            acod = toupper(MRILIB_orients[4]); icod = ORCODE(acod); if (icod >= 0)
            {
                user_inputs.zorient = icod;
            }
    ...
    /** Mar 1997: if any FOV or SLAB command line inputs are used, then require that they all be
     * given.            **/
    ...
    /* now that MRILIB_tr has been applied, check for use of siemens slice timing 13 Apr 2011
     * [rickr] */
    ...
    if (argopt.ncolor <= 2)
    {
        argopt.ncolor = INIT_ngray;
    }
    if (argopt.gamma <= 0.0)
    {
        argopt.gamma = INIT_gamma;
    }
...
/*
 *          XmNmarginHeight    , 1 , XmNmarginWidth     , 1 ,
 */
...
/*---------------------------------------------------------------------
 * Read any command line arguments and interpret them into the user_inputs data structure.
 * -----------------------------------------------------------------------*/
...
static to3d_data default_user_inputs = {
    ...
    240.0,                  0.9375,0.9375, 0.9375, 0.9375,              /* FOV; x,y,z sizes; z
                                                                         * spacing */
...
void T3D_initialize_user_data(void)
{
    ...
    { char *eee = my_getenv("AFNI_TO3D_ZPAD"), *fff;
      if (eee != NULL)
      {
          ...
          if (zpad < 0.0)
          {
              ERROR_exit("AFNI_TO3D_ZPAD is negative");
          }
          zpad_mm = (*fff == 'm');
      }
    }
    ...
    while (nopt < Argc && Argv[nopt][0] == '-')
    {
        ...
        for (ii = FIRST_ANAT_TYPE ; ii <= LAST_ANAT_TYPE ; ii++)
        {
            if (strncmp(&(Argv[nopt][1]),
                        ANAT_prefixstr[ii], THD_MAX_PREFIX) == 0)
            {
                break;
            }
        }
        ...
        {
            if (strncmp(&(Argv[nopt][1]),
                        FUNC_prefixstr[ii], THD_MAX_PREFIX) == 0)
            {
                break;
            }
        }
        ...
        if (strncmp(Argv[nopt], "-view", 4) == 0)
        {
            ...
            if (++nopt >= Argc)
            {
                ERROR_exit("-view needs a type");
            }

            str = Argv[nopt]; if (str[0] == '+')
            {
                str++;
            }
            ...
            {
                if (strcmp(str, VIEW_codestr[ii]) == 0)
                {
                    break;
                }
            }
        ...
        if (strncmp(Argv[nopt], "-zpad", 5) == 0)
        {
            ...
            if (++nopt >= Argc)
            {
                ERROR_exit("-zpad needs a count");
            }
            if (zpad > 0.0)
            {
                fprintf(stderr, "++ WARNING: second -zpad option found!\n");
            }
            zpad = strtod(Argv[nopt], &eee);
            if (zpad < 0.0)
            {
                ERROR_exit("-zpad is negative");
            }
            zpad_mm = (*eee == 'm');
            nopt++; continue;
        }
        ...
        if (strncmp(Argv[nopt], "-session", 4) == 0)
        {
            if (++nopt >= Argc)
            {
                ERROR_exit("-session needs a name");
            }
        ...
        if (strncmp(Argv[nopt], "-prefix", 4) == 0)
        {
            if (++nopt >= Argc)
            {
                ERROR_exit("-prefix needs a name");
            }
        ...
        if (strncmp(Argv[nopt], "-dlabel", 4) == 0)
        {
            if (++nopt >= Argc)
            {
                ERROR_exit("-dlabel needs a name");
            }
        ...
        if (strncmp(Argv[nopt], "-geomparent", 6) == 0)
        {
            if (++nopt >= Argc)
            {
                ERROR_exit("-geomparent needs a headerfile");
            }
        ...
        if (strncmp(Argv[nopt], "-anatparent", 6) == 0)
        {
            if (++nopt >= Argc)
            {
                ERROR_exit("-anatparent needs a headerfile");
            }
        ...
        if (strcmp(Argv[nopt], "-save_outliers") == 0)
        {
            if (++nopt > Argc)
            {
                ERROR_exit("-save_outliers needs a filename");
            }
        ...
        if (strncmp(Argv[nopt], "-orient", 4) == 0)
        {
            ...
            if (++nopt >= Argc)
            {
                ERROR_exit("-orient needs a code");
            }
            if (strlen(Argv[nopt]) != 3)
            {
                ERROR_exit("Illegal -orient code");
            }
        ...
        if (strncmp(Argv[nopt], "-zorigin", 7) == 0)
        {
            if (++nopt >= Argc)
            {
                ERROR_exit("-zorigin needs a value");
            }
...
#undef USE_OLD_DCODE
            ...
            strncmp(Argv[nopt], "-xfov", 5) == 0 || strncmp(Argv[nopt], "-xslab", 5) == 0)
        {
            ...
            if (++nopt >= Argc)
            {
                WarningError("need an argument after -xFOV/-xSLAB!");
            }
            ...
            /* Step 4: try to get the second <dircode>.  If that fails, then get the second <number>
             * first, then the second <dircode>. If the second <dircode> works, however, then the
             *second
            ...
            if (dcod2 < 0)
            {
                ...
                {
                    WarningError(
                        "orientation code or dimension should follow : or - in -xFOV/-xSLAB!");
                }
                xin_top = val;
                ptr     = ptr2;
                acod    = toupper(*ptr); dcod2 = ORCODE(acod);
                ...
                {
                    WarningError(
                        "orientation code should follow second dimension in -xFOV/-xSLAB!");
                }
            }
            else
            {
             ...
             *   however, must be opposite if the two <number>s are identical. For example 120A-120A
             *makes no sense, but 120A-120P is reasonable, and 10A-30A is also reasonable.  */
             ...
             * is smaller, reverse the first direction code and negate the first number.  For
             *example, 10A:30A becomes -10P:30A, which is equivalent.  If the first number is
             *larger, then reverse the second code and number.  For example, 30A:10A becomes
             *30A:-10P. Why do this?  Then the first direction code correctly expresses the sense of
             *the input direction. The sum of numbers becomes the cross-slab thickness, needed to
             *compute various stuff later. */
            ...
            strncmp(Argv[nopt], "-yfov", 5) == 0 || strncmp(Argv[nopt], "-yslab", 5) == 0)
        {
            ...
            if (++nopt >= Argc)
            {
                WarningError("need an argument after -yFOV/-ySLAB!");
            }
            ...
            if (dcod2 < 0)
            {
                ...
                {
                    WarningError(
                        "orientation code or dimension should follow : or - in -yFOV/-ySLAB!");
                }
                yin_top = val;
                ptr     = ptr2;
                acod    = toupper(*ptr); dcod2 = ORCODE(acod);
                ...
                {
                    WarningError(
                        "orientation code should follow second dimension in -yFOV/-ySLAB!");
                }
            }
            else
            {
            ...
            strncmp(Argv[nopt], "-zfov", 5) == 0 || strncmp(Argv[nopt], "-zslab", 5) == 0)
        {
            ...
            if (++nopt >= Argc)
            {
                WarningError("need an argument after -zFOV/-zSLAB!");
            }
            ...
            if (dcod2 < 0)
            {
                ...
                {
                    WarningError(
                        "orientation code or dimension should follow : or - in -zFOV/-zSLAB!");
                }
                zin_top = val;
                ptr     = ptr2;
                acod    = toupper(*ptr); dcod2 = ORCODE(acod);
                ...
                {
                    WarningError(
                        "orientation code should follow second dimension in -zFOV/-zSLAB!");
                }
            }
            else
            {
        ...
        if (strncmp(Argv[nopt], "-statpar", 5) == 0)
        {
            ...
            if (++nopt >= Argc)
            {
                ERROR_exit("need an argument after -statpar!");
            }
        ...
        if (strncmp(Argv[nopt], "-Torg", 5) == 0)
        {
            if (nopt + 1 >= Argc)
            {
                ERROR_exit("need 1 argument after -Torg");
            }
        ...
        if (strncmp(Argv[nopt], "-time:zt", 8) == 0 || strncmp(Argv[nopt], "-time:tz", 8) == 0)
        {
            ...
            if (nopt + 4 >= Argc)
            {
                ERROR_exit("need 4 arguments after -time: options");
            }
            ...
            if (TR == 0.0)
            {
                TR = 1.0; user_inputs.tunits = UNITS_SEC_TYPE;
            }
    ...
    /* 14 Sep 1999: manufacture a command line for History, but with (possibly) fewer image inputs
     */
...
/*---------------------------------------------------------------------
 * read the voxshape button box and set the appropriate arrowvals to sensitive or insensitive
 * -----------------------------------------------------------------------*/
...
/*------------------------------------------------------------------------
 * read the images from the remaining command line arguments, and start the setup of the
 *3dim_dataset
 * --------------------------------------------------------------------------*/
...
void T3D_read_images(void)
{
    ...
    if (gnim < 1)
    {
        ERROR_exit("NO INPUT IMAGE FILES?");
    }
    ...
    /**------ Perform various sanity checks if the user is trying to create a time dependent
     * dataset.  ------**/

    time_dep = (user_inputs.ntt > 0);
    if (time_dep)
    {
        ntt = user_inputs.ntt; nzz = user_inputs.nzz;
    }
    ...
    /* maybe there is siemens slice timing info       13 Apr 2011 [rickr] Do not fully test or apply
     * times until MRILIB_tr has been applied, but if user wants to populate FROM_IMAGE, check that
     * times exist. Also, be sure there is a tpattern to inspect.   4 May 2011 [rickr] */
...

        {
            ...
            else                                /* must convert data */
            {
                ...
                case MRI_float: { /** convert to floats **/
                    ...
                    /* if requested and appropriate, convert negative shorts as unsigned */
                    if (uconvert)
                    {
                        ushort_num += mri_float_convert_ushort(shim);
                    }
                }
            ...
            else
            {
                ...
                /*-- step the kzz,ltt indices forward, depending on the order in which the slices
                 * are coming in (-time:zt or :tz) --*/
    ...
    if (use_MRILIB_slicespacing && fabs(MRILIB_slicespacing - imdz) > 0.01l)
    {
        ...
        if (imdz > 0.0)
        {
            fprintf(stderr, " instead of slice thickness=%g", imdz);
        }
        fprintf(stderr, "\n");
    ...
    if (commandline != NULL)
    {
        tross_Append_History(dset, commandline);
    }
...
XtPointer T3D_getim(int n, int type, FD_brick *br)
{
    ...
    if (n < 0 || n >= br->n3 || type == isqCR_getoverlay)
    {
        return NULL;
    }
                      ...
                      XtPointer client_data, XtPointer call_data)
{
    ...
    else              /* 05 Feb 2001: allow for zero-padding in z-direction */
    {
        ...
        if (commandline != NULL)
        {
            tross_Append_History(qset, commandline);
        }
...
void T3D_data_to_widgets(void)
{
    ...
    { int ii, num, bl; char buf[256];
      ...
      for (ii = MAX_STAT_AUX - 1 ; ii >= 0 && user_inputs.stat_aux[ii] == 0.0 ; ii--)
      {
          ;                                                                     /* nada */
      }
      num = ii + 1;
...
RwcBoolean T3D_check_data(RwcBoolean perr)
{
    ...
    else
    {
        for (ii = 0 ; ii < ll ; ii++)
        {
            if (iscntrl(user_inputs.output_filename[ii]) ||
                ...
                user_inputs.output_filename[ii] == '/')
            {
                break;
            }
        }
    ...
    else
    {
        for (ii = 0 ; ii < ll ; ii++)
        {
            if (iscntrl(user_inputs.session_filename[ii]) ||
                isspace(user_inputs.session_filename[ii]))
            {
                break;
            }
        }
    ...
    /*-- if the image type is functional, check that the anatomy parent dataset name has been set
     * --*/
    ...
    if (user_inputs.need_stat_aux)
    {
        ...
        {
            if (user_inputs.stat_aux[ii] <= 0.0)
            {
                break;
            }
        }
                          ...
                          XEvent *ev, RwcBoolean *continue_to_dispatch)
{
    ...
    if (lev->type != LeaveNotify || w == NULL)
    {
        return;
    }
                            ...
                            XtPointer client_data, XtPointer call_data)
{
    ...
    if (old_name == NULL)
    {
        old_name = XtNewString("Elvis Lives!!!");
    }
    ...
    {
        old_name = new_name; geomparent_loaded = 0; EXRETURN;
    }
    if (strcmp(new_name, old_name) == 0)
    {
        myXtFree(new_name); geomparent_loaded = 0; EXRETURN;
    }
    ...
    if (strstr(new_name, "/") == NULL)
    {
        ...
        if (w != NULL)
        {
            sess = XmTextFieldGetString(wset.session_file_textfield);
        }
        else
        {
            sess = XtNewString(user_inputs.session_filename);
        }
        DEBLANK(sess);
        if (strlen(sess) > 0)
        {
    ...
    /* but first, load all data from the widgets into the user_inputs (this will mean that the
     * T3D_data_to_widgets call later won't clobber any un-altered but currently un-saved data
     * fields    */
...
#ifdef ALLOW_NONCONTIG
    if (geom_daxes->zzskip == 0)
    {
        user_inputs.voxcontig = VOXCONTIG_YES;
    }
    else
    {
        user_inputs.voxcontig = VOXCONTIG_UNIF;
    }
#endif
                           ...
                           XtPointer client_data, XtPointer call_data)
{
    ...
    if (strlen(new_name) == 0)
    {
        old_name = new_name; EXRETURN;
    }
    if (strcmp(new_name, old_name) == 0)
    {
        myXtFree(new_name); EXRETURN;
    }
 ...
 *   <dircode>
 *   <number>
 * val will be WAY_BIG if the number isn't given, otherwise it will be the number;
 * dcode will be < 0 if the dircode isn't given, otherwise it will be the direction code. The return
 *value is the number of characters used from the input string.
 * --------------------------------------------------------------***/
...
/* convert ushorts to short range in float image (part of converting unsigned shorts to floats)  9
 * Jul 2013 [rickr]
 *
