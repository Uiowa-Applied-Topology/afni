...
int main(int argc, char *argv[])
{
    ...
    if (argc < 2)
    {
        TFIM_syntax("try tfim -help for usage");
    }
    ...
    if (TF_pthresh > 0.0)
    {
        ...
        for (ii = 0 ; ii < npix ; ii++)
        {
            if (TF_use_bval == -1)
            {
                thr = stinv(TF_pthresh, dofar[ii]);
            }
            if (fabs(tspar[ii]) < thr)
            {
                difar[ii] = 0.0;
            }
        }
...
void TFIM_getopts(int argc, char *argv[])
{
    ...
    while (nopt < argc)
    {
        ...
        if (strncmp(argv[nopt], "-prefix", 5) == 0)
        {
            if (++nopt >= argc)
            {
                TFIM_syntax("-prefix needs a name!");
            }
        ...
        if (strncmp(argv[nopt], "-pthresh", 5) == 0)
        {
            ...
            if (++nopt >= argc)
            {
                TFIM_syntax("-pthresh needs a value!");
            }
        ...
        if (strncmp(argv[nopt], "-eqcorr", 5) == 0)
        {
            ...
            if (++nopt >= argc)
            {
                TFIM_syntax("-eqcorr needs a value!");
            }
        ...
        if (strncmp(argv[nopt], "-base1", 5) == 0)
        {
            ...
            if (++nopt >= argc)
            {
                TFIM_syntax("-base1 needs a value!");
            }
            if (TF_use_bval == -1)
            {
                TFIM_syntax("-base1 with -set1 illegal!");
            }

            TF_bval = strtod(argv[nopt], &ch);
            if (*ch != '\0')
            {
                TFIM_syntax("value after -base1 is illegal!");
            }
        ...
        if (strncmp(argv[nopt], "-set1", 5) == 0)
        {
            if (TF_use_bval == 1)
            {
                TFIM_syntax("-set1 with -base1 illegal!");
            }
            TF_use_bval = -1;

            for (kk = nopt + 1 ; kk < argc ; kk++)
            {
                if (strncmp(argv[kk], "-set2", 5) == 0)
                {
                    break;
                }
            }
            ...
            if (kk >= argc)
            {
                TFIM_syntax("-set1 not followed by -set2");
            }
            if (kk - 1 - nopt <= 0)
            {
                TFIM_syntax("-set1 has no files after it");
            }
        ...
        if (strncmp(argv[nopt], "-set2", 5) == 0)
        {
            if (++nopt >= argc)
            {
                TFIM_syntax("-set2 has not files after it");
            }
    ...
    /* if paired t-test, subtract set1 from set2 to convert it into the equivalent base level test
     * vs. 0 */
