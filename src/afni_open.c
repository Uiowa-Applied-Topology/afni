...
int is_archive_pn(SUMA_PARSED_NAME *FN)
{
    ...
    else if (!strcmp(FN->Ext, ".tgz"))
    {
        a = 1 | 2; /* With compression flag?
                    * Not sure if we should handle this here Could have an is_compressed query
                    *separately...*/
    }
    return a;
}
...
/* List of text file types, including some fairly specific to AFNI (like NIML, netcc, grid, asc,
 * spec, etc.); don't include .HEAD here, because afni_open can open dsets with AFNI, too. */
...
int is_url_pn(SUMA_PARSED_NAME *FN)
{
    ...
    if (FN->OnDisk)
    {
        return 0;           /* should not be on disk */
    }
    if (strstr(FN->NameAsParsed, "http:") == FN->NameAsParsed)
    {
        return 1;
    }
    if (strstr(FN->NameAsParsed, "file:") == FN->NameAsParsed)
    {
        return 1;
    }
    if (strstr(FN->NameAsParsed, "afni.nimh.nih.gov") == FN->NameAsParsed)
    {
        return 1;
    }
    return 0;
...
int is_local_html(SUMA_PARSED_NAME *FN)
{
    ...
    if (!FN->OnDisk)
    {
        return 0;            /* should be on disk */
    }
    if (!strcmp(FN->Ext, ".html"))
    {
        return 1;
    }
    return 0;
...
int ao_with_afniweb(char *fname)
{
    ...
    if (is_url(fname))
    {
        return ao_with_downloader(fname, 0);
    }
    ...
    if (is_pdf(fname))
    {
        ...
        {
            return ao_with_pdf_viewer(fname);
        }
        else
        {
...
int main(int argc, char **argv)
{
    ...
    for (i = 0; i < fnamev->num; ++i)
    {
        ...
        /* Now comes the decision stream There is no one function fits all here, we will need to
         * think about how we go about deciding what is what. Two field types are of most import:
         * FN->StorageMode (or FN->StorageModeNm) FN->TypeExt Also, what should be done might be
         *dictated by the size of the file at hand, and whether or not it is in binary versus ascii
         *mode. Those fields are not a part of SUMA_PARSED_NAME, but we can easily add them. We
         *still need things like:
         * is_url, is_pdf, is_script, is_..., is_surface_dset, is_surface, is_xmat, is_prog,
         *is_archive, is_web_archive
         */
        if (is_url_pn(FN))
        {
            if (!is_archive_pn(FN))
