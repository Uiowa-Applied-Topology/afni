...
int main(int argc, char *argv[])
{
    ...
    if (newid)
    {
        dset->idcode = MCW_new_idcode();          /* 11 May 2005 */
    }
    ...
    /* we cannot strdup if we will later use strcat()  17 Apr 2013 [rickr] plus, fname already has
     * space for suffix padding options.infile_name = nifti_strdup(fname) ; */
    ...
    if (floatize)
    {
        ...
        if (verb)
        {
            INFO_message("Converting dataset to floats (in memory)");
        }
        ...
        {
            fac = DSET_BRICK_FACTOR(dset, ii); if (fac == 0.0f)
            {
                fac = 1.0f;
            }
            ...
            if (fac == 1.0f && (tt == MRI_float || tt == MRI_complex))
            {
                continue;
            }
    ...
    if (denote)
    {
        THD_anonymize_write(1);          /* sets a flag for attribute output */
    }
