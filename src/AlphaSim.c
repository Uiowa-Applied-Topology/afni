...
/*
 * This program estimates the statistical significance levels and statistical power of FMRI datasets
 *by Monte Carlo simulation of random image generation, Gaussian filtering, intensity thresholding,
 *and minimum cluster size thresholding.
 *
 * File:    AlphaSim.c Author:  B. D. Ward Date:    18 June 1997
 *
 * Mod:     Changed random number generator function from rand to drand48. Date:    26 August 1997
 *
 * Mod:     Corrected problem: attempt to close a file which was not open. Date:    21 June 1999
 *
 * Mod:     Corrected problem with count overflow. Date:    30 July 1999
 *
 * Mod:     Added -mask option.  (Adapted from: 3dpc.c) Date:    15 June 2000
 *
 * Mod:     Added call to AFNI_logger. Date:    15 August 2001
 *
 * Mod:     Set MAX_NAME_LENGTH equal to THD_MAX_NAME. Date:    02 December 2002
 *
 * Mod:     Added -max_clust_size, to override MAX_CLUSTER_SIZE. Also, increased default from 1000
 *to 10000. Date:    12 Apr 2006 [rickr]
 *
 * Mod:     Added AFNI_BLUR_* environment variable stuff. Date:    03 Apr 2007 [RWC]
 *
 * Mod:     Add -fast and -nxyz and -dxyz options. Date:    10 Jan 2008 [RWC]
 *
 * Mod:     Modify to use OpenMP to parallelize simulations. Date     09 Jun 2009 [RWC]
 *
 * Mod:     Modify to compute extreme value approximation to Alpha(i). Date:    10 Jun 2009 [RWC]
 *
 */
                 ...
                 char **outfilename, int *seed)
{
    ...
    while (nopt < argc)
    {
        ...
        if (strcmp(argv[nopt], "-nxyz") == 0)
        {
            nopt++; if (nopt + 2 >= argc)
            {
                AlphaSim_error("need 3 arguments after -nxyz ");
            }
            *nx = (int)strtod(argv[nopt++], NULL); if (*nx <= 0)
            {
                AlphaSim_error("illegal n1 value");
            }
            *ny = (int)strtod(argv[nopt++], NULL); if (*ny <= 0)
            {
                AlphaSim_error("illegal n2 value");
            }
            *nz = (int)strtod(argv[nopt++], NULL); if (*nz <= 0)
            {
                AlphaSim_error("illegal n3 value");
            }
            continue;
        }
        ...
        if (strcmp(argv[nopt], "-dxyz") == 0)
        {
            nopt++; if (nopt + 2 >= argc)
            {
                AlphaSim_error("need 3 arguments after -dxyz ");
            }
            *dx = strtod(argv[nopt++], NULL); if (*dx <= 0)
            {
                AlphaSim_error("illegal d1 value");
            }
            *dy = strtod(argv[nopt++], NULL); if (*dy <= 0)
            {
                AlphaSim_error("illegal d2 value");
            }
            *dz = strtod(argv[nopt++], NULL); if (*dz <= 0)
            {
                AlphaSim_error("illegal d3 value");
            }
            continue;
        }
        ...
        if (strncmp(argv[nopt], "-nx", 3) == 0)
        {
            ...
            if (nopt >= argc)
            {
                AlphaSim_error("need argument after -nx ");
            }
        ...
        if (strncmp(argv[nopt], "-ny", 3) == 0)
        {
            ...
            if (nopt >= argc)
            {
                AlphaSim_error("need argument after -ny ");
            }
            sscanf(argv[nopt], "%d", &ival);
            if (ival <= 0)
            {
        ...
        if (strncmp(argv[nopt], "-nz", 3) == 0)
        {
            ...
            if (nopt >= argc)
            {
                AlphaSim_error("need argument after -nz ");
            }
            sscanf(argv[nopt], "%d", &ival);
            if (ival <= 0)
            {
        ...
        if (strncmp(argv[nopt], "-dx", 3) == 0)
        {
            ...
            if (nopt >= argc)
            {
                AlphaSim_error("need argument after -dx ");
            }
        ...
        if (strncmp(argv[nopt], "-dy", 3) == 0)
        {
            ...
            if (nopt >= argc)
            {
                AlphaSim_error("need argument after -dy ");
            }
            sscanf(argv[nopt], "%f", &fval);
            if (fval <= 0.0)
            {
        ...
        if (strncmp(argv[nopt], "-dz", 3) == 0)
        {
            ...
            if (nopt >= argc)
            {
                AlphaSim_error("need argument after -dz ");
            }
            sscanf(argv[nopt], "%f", &fval);
            if (fval <= 0.0)
            {
        ...
        if (strcmp(argv[nopt], "-mask") == 0)
        {
            ...
            if (nopt >= argc)
            {
                AlphaSim_error("need argument after -mask!");
            }
            ...
            if (mset == NULL)
            {
                AlphaSim_error("can't open -mask dataset!");
            }

            mask_vol = THD_makemask(mset, 0, 1.0, 0.0);
            if (mask_vol == NULL)
            {
                AlphaSim_error("can't use -mask dataset!");
            }
            ...
            if (mc == 0)
            {
                AlphaSim_error("mask is all zeros!");
            }
            if (*quiet < 2)
            {
        ...
        if (strncmp(argv[nopt], "-fwhm", 6) == 0)
        {
            ...
            if (nopt >= argc)
            {
                AlphaSim_error("need argument after -fwhm ");
            }
            sscanf(argv[nopt], "%f", &fval);
            if (fval < 0.0)
            {
        ...
        if (strncmp(argv[nopt], "-fwhmx", 6) == 0)
        {
            ...
            if (nopt >= argc)
            {
                AlphaSim_error("need argument after -fwhmx ");
            }
            sscanf(argv[nopt], "%f", &fval);
            if (fval < 0.0)
            {
        ...
        if (strncmp(argv[nopt], "-fwhmy", 6) == 0)
        {
            ...
            if (nopt >= argc)
            {
                AlphaSim_error("need argument after -fwhmy ");
            }
            sscanf(argv[nopt], "%f", &fval);
            if (fval < 0.0)
            {
        ...
        if (strncmp(argv[nopt], "-fwhmz", 6) == 0)
        {
            ...
            if (nopt >= argc)
            {
                AlphaSim_error("need argument after -fwhmz ");
            }
            sscanf(argv[nopt], "%f", &fval);
            if (fval < 0.0)
            {
        ...
        if (strncmp(argv[nopt], "-sigma", 7) == 0)
        {
            ...
            if (nopt >= argc)
            {
                AlphaSim_error("need argument after -sigma ");
            }
            sscanf(argv[nopt], "%f", &fval);
            if (fval < 0.0)
            {
        ...
        if (strncmp(argv[nopt], "-sigmax", 7) == 0)
        {
            ...
            if (nopt >= argc)
            {
                AlphaSim_error("need argument after -sigmax ");
            }
            sscanf(argv[nopt], "%f", &fval);
            if (fval < 0.0)
            {
        ...
        if (strncmp(argv[nopt], "-sigmay", 7) == 0)
        {
            ...
            if (nopt >= argc)
            {
                AlphaSim_error("need argument after -sigmay ");
            }
            sscanf(argv[nopt], "%f", &fval);
            if (fval < 0.0)
            {
        ...
        if (strncmp(argv[nopt], "-sigmaz", 7) == 0)
        {
            ...
            if (nopt >= argc)
            {
                AlphaSim_error("need argument after -sigmaz ");
            }
            sscanf(argv[nopt], "%f", &fval);
            if (fval < 0.0)
            {
        ...
        if (strncmp(argv[nopt], "-ax", 3) == 0)
        {
            ...
            if (nopt >= argc)
            {
                AlphaSim_error("need argument after -ax ");
            }
            sscanf(argv[nopt], "%d", &ival);
            if (ival <= 0)
            {
        ...
        if (strncmp(argv[nopt], "-ay", 3) == 0)
        {
            ...
            if (nopt >= argc)
            {
                AlphaSim_error("need argument after -ay ");
            }
            sscanf(argv[nopt], "%d", &ival);
            if (ival <= 0)
            {
        ...
        if (strncmp(argv[nopt], "-az", 3) == 0)
        {
            ...
            if (nopt >= argc)
            {
                AlphaSim_error("need argument after -az ");
            }
            sscanf(argv[nopt], "%d", &ival);
            if (ival <= 0)
            {
        ...
        if (strncmp(argv[nopt], "-zsep", 5) == 0)
        {
            ...
            if (nopt >= argc)
            {
                AlphaSim_error("need argument after -zsep ");
            }
            sscanf(argv[nopt], "%f", &fval);
            if (fval < 0.0)
            {
        ...
        if (strncmp(argv[nopt], "-rmm", 4) == 0)
        {
            ...
            if (nopt >= argc)
            {
                AlphaSim_error("need argument after -rmm ");
            }
            sscanf(argv[nopt], "%f", &fval);
            if (fval <= 0.0)
            {
                INFO_message("-rmm set to %g ==> NN clustering", fval);
            }
            *rmm = fval;
            nopt++;
            continue;
        ...
        if (strncmp(argv[nopt], "-pthr", 5) == 0 || strcmp(argv[nopt], "-pval") == 0)
        {
            ...
            if (nopt >= argc)
            {
                AlphaSim_error("need argument after -pthr ");
            }
            sscanf(argv[nopt], "%f", &fval);
        ...
        if (strncmp(argv[nopt], "-iter", 5) == 0 || strcmp(argv[nopt], "-niter") == 0)
        {
            ...
            if (nopt >= argc)
            {
                AlphaSim_error("need argument after -iter ");
            }
            sscanf(argv[nopt], "%d", &ival);
            if (ival <= 0)
            {
        ...
        if (strncmp(argv[nopt], "-out", 4) == 0)
        {
            ...
            if (nopt >= argc)
            {
                AlphaSim_error("need argument after -out ");
            }
                            ...
                            char *outfilename)
{
    ...
    if (nx <= 0)
    {
        AlphaSim_error("Illegal value for nx ");
    }
    if (ny <= 0)
    {
        AlphaSim_error("Illegal value for ny ");
    }
    if (nz <= 0)
    {
        AlphaSim_error("Illegal value for nz ");
    }
    if (dx <= 0.0)
    {
        AlphaSim_error("Illegal value for dx ");
    }
    if (dy <= 0.0)
    {
        AlphaSim_error("Illegal value for dy ");
    }
    if (dz <= 0.0)
    {
        AlphaSim_error("Illegal value for dz ");
    }
    ...
    if (power && (zsep <= 0.0))
    {
        AlphaSim_error("Illegal value for zsep ");
    }
    ...
    if (niter <= 0)
    {
        AlphaSim_error("Illegal value for niter ");
    }
...
#pragma omp critical (PRINTF)
    if (nzzz < 0.05f * nxyz)
    {
        WARNING_message("nzzz=%d mean=%f sd=%f zthr=%f p=%f q=%f", nzzz, mean, sd, zthr, p, q);
    }
#endif
}
...
/*
 * where rmm = cluster connection radius (mm) nx = number of voxels along x-axis ny = number of
 *voxels along y-axis nz = number of voxels along z-axis dx = voxel size along x-axis dy = voxel
 *size along y-axis dz = voxel size along z-axis
 */
...
#pragma omp critical (PRINTF)
            ...
            if (filter)
            {
                gaussian_filter(nx, ny, nz, dx, dy, dz, rmm,
            ...
            if (egfw)
            {
                estimate_gfw(nx, ny, nz, dx, dy, dz,
            ...
            if (power)
            {
                get_activation_region(nx, ny, nz, ax, ay, az, pthr, zsep,
                                      fim, arfim);
            }
        ...
        if (arfim != NULL)
        {
            free(arfim);               /* toss the local trash */
        }
        free(fim);
