 ...
 * - 05 Nov 2001: increased size of hist array to nhist+1 (from nhist), to store properly elements
 *[0..nhist] (d'oh).
 * ----------------------------------------------------------------------------*/
...
float THD_cliplevel(MRI_IMAGE *im, float mfrac)
{
    ...
    default:
        ...
        {
            fac = mri_maxabs(im); if (fac < 1.0e-100)
            {
                RETURN(0.0f);
            }
    ...
    case MRI_float: {    /* 20 Dec 2006: do the float->int conversion inline */
        ...
        fac = mri_max(im); if (fac < 1.e-100)
        {
            free(hist); RETURN(0.0f);
        }
        sfac = nhist / fac;
        for (ii = 0 ; ii < nvox ; ii++)
        {
    ...
    for (kk = 0, ii = nhist - 1 ; ii >= ib && kk < qq ; ii--)
    {
        kk += hist[ii];
    }

    /*-- median adjustment algorithm:
     *   we find a cut level so that it equals mfrac times the median of all the values above the
     *cut level. ---*/

    ncut = ii; qq = 0;   /* qq is iteration count */
    do{
        for (npos = 0, ii = ncut; ii < nhist; ii++)
        {
            npos += hist[ii];                                     /* num >= cut */
        }
 ...
 * (2) also, if mfrac < 0, then find 90% point on CDF of absolute values and return smaller of this
 *or the THD_cliplevel() result. Purpose is to find some reasonable place to threshold an image for
 *visual effect only.
 * ---------------------------------------------------------------------------*/
...
float THD_cliplevel_abs(MRI_IMAGE *im, float mfrac)
{
    ...
    fim = mri_to_float(im); if (fim == NULL)
    {
        RETURN(0.0f);
    }
    far = MRI_FLOAT_PTR(fim);
    for (ii = 0 ; ii < fim->nvox ; ii++)
    {
        far[ii] = fabsf(far[ii]);
    }
    if (mfrac < 0.0f)
    ...
    if (dotwo)
    {
        ...
        if (tv == 0.0f)
        {
            for ( ; ii < fim->nvox && far[ii] == 0.0f ; ii++)
            {
                ;                                          /* nada */
            }
            if (ii < fim->nvox)
...
static clipvec get_octant_clips(MRI_IMAGE *im, float mfrac)
{
    ...
    ii = (int)rint(0.01 * nx); if (ii < 1)
    {
        ii = 1;                                    /* 1% quadrant overlap */
    }
    jj = (int)rint(0.01 * ny); if (jj < 1)
    {
        jj = 1;
    }
    kk = (int)rint(0.01 * nz); if (kk < 1)
    {
        kk = 1;
    }
    icm = ic - ii; icp = ic + ii; if (icm < 0)
    {
        icm = 0;
    }
    if (icp > it)
    ...
    if (cv.clip_000 < val)
    {
        cv.clip_000 = val;                      /* don't let   */
    }
    if (cv.clip_100 < val)
    {
        cv.clip_100 = val;                      /* clip levels */
    }
    if (cv.clip_010 < val)
    {
        cv.clip_010 = val;                      /* get too     */
    }
    if (cv.clip_110 < val)
    {
        cv.clip_110 = val;                      /* small!      */
    }
    if (cv.clip_001 < val)
    {
        cv.clip_001 = val;
    ...
    /* (x0,y0,z0) = center of lowest octant (x1,y1,z1) = center of highest octant */
...
/*! Return the cliplevel at any point, tri-linearly interpolated between octant centers. [24 Oct
 * 2006] */
...
/*! Return a cliplevel that varies gradually across the image. At this time [24 Oct 2006], the
 * clipping varies trilinearly between octants.  A fancier method would use higher order
...
#define CB    0.10f
    ...
    for (qc = 0 ; qc < NC ; qc++)
    {
        ...
        free(mask);
        ININFO_message("  clfrac=%.2f nmask=%d (%.1f%%)", cc, nmask[qc],
                       (100.0f * nmask[qc]) / (float)(im->nvox));
    }
