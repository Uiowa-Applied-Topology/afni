 ...
 *(AFNI_CONTINUE_MODE), which is for normal transmission of images (one at a time) to AFNI.  This
 *operation is handled in the separate routine AFNI_send_image (infra).
 *
...
void AFNI_send_image(int nim)
{
    ...
    if (nim == 0)
    {
        if (AFNI_verbose)
        {
            fprintf(stderr, "Sending 1st info+image to AFNI.\n");
        }
