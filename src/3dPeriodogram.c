...
int main(int argc, char *argv[])
{
    ...
    while (nopt < argc && argv[nopt][0] == '-')
    {
        ...
        if (strcasecmp(argv[nopt], "-taper") == 0)
        {
            if (++nopt >= argc)
            {
                ERROR_exit("-taper needs an argument!");
            }
            taper = (float)strtod(argv[nopt], NULL);
            if (taper < 0.0f || taper > 1.0f)
            {
                ERROR_exit("-taper value out of range 0..1");
            }
            nopt++; continue;
        }
                         ...
                         void *ud, int nbrik, float *val)
{
    ...
    if (val == NULL)
    {
...

        {
            ...
            if (nallz > 0)
            {
                ININFO_message("%d/%d input voxel%s were all zero", nallz, nvox,
                               ((nallz == 1)?"\0":"s"));
            }
            if (noutz > 0)
            {
                ININFO_message("%d/%d output voxel%s were all zero", noutz, nvox,
                               ((noutz == 1)?"\0":"s"));
            }
    ...
    for (kk = 0 ; kk < npts && ts[kk] == 0.0f ; kk++)
    {
        ;                                             /*nada*/
    }
    if (kk == npts)
    {
        for (kk = 0 ; kk < nbin ; kk++)
        {
            val[kk] = 0.0f;                           /* input was all zero */
        }
        nallz++;
        EXRETURN;
    }
