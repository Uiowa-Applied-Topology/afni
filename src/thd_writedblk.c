...
{
    if (compress_mode == COMPRESS_NOFILE)
    {
        THD_enviro_write_compression();
    }
...
void THD_enviro_write_order(void)
{
    ...
    if (strcmp(hh, LSB_FIRST_STRING) == 0)
    {
        output_order = LSB_FIRST; return;
    }
    if (strcmp(hh, MSB_FIRST_STRING) == 0)
    {
        output_order = MSB_FIRST; return;
    }
 ...
 *   - All attributes must now be set prior to calling this, via function
...
RwcBoolean THD_write_datablock(THD_datablock *blk, RwcBoolean write_brick)
{
    ...
    if (DBLK_IS_MINC(blk))
    {
        WRITE_ERR("MINC with bad name extension?");
    }
    /* 29 Oct 2001 */
    if (DBLK_IS_ANALYZE(blk))
    {
        WRITE_ERR("ANALYZE but bad name extension?");
    }
    /* 27 Aug 2002 */
    if (DBLK_IS_NIFTI(blk))
    {
        WRITE_ERR("NIFTI but bad name extension?");
    }
    /* 28 Aug 2003 */
    ...
    if (dkptr->byte_order <= 0)
    {
        dkptr->byte_order = native_order;
    }
     ...
     *     if none exist, cannot write, but is OK if some but not all exist, cannot write, and is an
     *error --*/
    ...
    case STORAGE_BY_BRICK: {
        ...
        for (ibr = 0 ; ibr < nv ; ibr++)
        {
            ...
            if (do_mripurge)
            {
                mri_unpurge(DBLK_BRICK(blk, ibr));
            }
            ...
            if (do_mripurge)                     /* 31 Jan 2007 */
            {
                if (!purge_when_done)
                {
                    mri_purge(DBLK_BRICK(blk, ibr));
                }
                else
                {
                    mri_clear(DBLK_BRICK(blk, ibr));
                }
            }
        ...
        if (force_gzip)
        {
            compress_mode = csave;                /* 02 Mar 2001 */
        }
