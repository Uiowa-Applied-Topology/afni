...
/* being good and cleaning up before erroring out can be a pain, and messy, lets make a variadic
 * macro that does it for us. */
...
double * calc_fecm_power(MRI_vectim *xvectim, double shift, double scale, double eps, long max_iter)
{
    ...
    /* -- xv_int is an intermediary of the A'v matrix product */
    ...
    while ((v_err > eps) && (power_it < max_iter))
    {
        ...
        /* calculate the error, norms, and sums for the next iteration */
        v_prev_sum   = 0.0;
        v_new_norm   = 0.0;
        v_new_sum_sq = 0.0;
        v_err        = 0.0;
        ...
        /* normalize the new vector, calculate the error between this vector and the previous, and
         * get the sum */
        v_new_sum_sq = 0.0;
        for (lout = 0; lout < xvectim->nvec; lout++)
        {
    ...
    /* the eigenvector that we are interested in should now be in v_prev, free all other power
     * iteration temporary vectors */
    if (v_new != NULL)
    {
        free(v_new);
        v_new = NULL;
...
double * calc_full_power_sparse(MRI_vectim *xvectim, double thresh,
                                double sparsity, double shift, double scale, double eps,
                                long max_iter,
                                long binary, long mem_bytes)
{
    ...
    while ((v_err > eps) && (power_it < max_iter))
    {
        ...
        /* begin by adding in the correlation for the diagonal element (i.e. 1.0) */
        for (ii = 0; ii < xvectim->nvec; ii++)
        {
                                 ...
                                 double scale, double eps, long max_iter, long binary)
{
    ...
    /* we will use another array to help us quickly calculate iterator so that we only need to store
     * the unique part of the array */
...
#pragma omp parallel if( xvectim->nvec > 999 )
            {
                ...
                /* iterate through and build the correlation matrix, this can be done at the same
                 * time as the first iteration of the power method, so lets do it! */
...
#pragma omp critical(dataupdate)
                    ...
                    for (thr_lin = 0; thr_lin < thr_lout; thr_lin++)
                    {
                        ...
                        /* only need to add in non-zero values, this hopefully will reduce some of
                         * the competition for the critical section */
                        if (car >= thresh)
                        {
                            /* use a critical section to make sure that the values do not get
                             * corrupted */
#pragma omp critical(dataupdate)
                            {
                                nvals = nvals + 1;
                            ...
                            /* incorporate the correlation in the weight_matrix, since each value is
                             * only calculated once, we don't need a critical sections */
...
double * calc_full_power_min_mem(MRI_vectim *xvectim, double thresh, double shift, double scale,
                                 double eps, long max_iter, long binary)
{
    /* CC - we need a few arrays for calculating the power method */
    double *v_prev = NULL;
    double *v_new  = NULL;
...
#pragma omp for schedule(static, 1)
            ...
            {  /*----- outer voxel loop -----*/
                ...
                for (lin = lout; lin < xvectim->nvec; lin++)
                {
                    ...
                    if (car >= thresh)
                    {
                        /* use a critical section to make sure that the values do not get corrupted
                         */
#pragma omp critical(dataupdate)
                        {
                            nvals = nvals + 1;
...
/* 3dECM was created from 3dAutoTCorrelate by R. Cameron Craddock */
...
int main(int argc, char *argv[])
{
    ...
    /* CC - number of voxels with zero variance that were masked out */
    int rem_count = 0;
    ...
    if (nopt >= argc)
    {
        ERROR_EXIT_CC("Need a dataset on command line!?");
    }
    ...
    /* if fecm is specified default to scale = 0.5, shift = 1.0 to be consistent with Wink et. al.
     */
    if (do_fecm == 1)
    {
    ...
    else if (do_autoclip)
    {
        ...
        if (nmask < 2)
        {
            ERROR_EXIT_CC("Only %d voxels in -automask!", nmask);
        }
    }
    ...
    else
    {
        ...
        if (mask == NULL)
        {
            ERROR_EXIT_CC("Can't allocate mask?!");
        }
    ...
    if (xvectim == NULL)
    {
        ERROR_EXIT_CC("Can't create vectim?!");
    }
    ...
    /*-- CC now that the data is in an easy to use form, go through and remove all of the zero
     * variance voxels */
    for (ii = 0; ii < xvectim->nvec; ii++)
    {
    ...
    if (rem_count > 0)
    {
        ...
        if (xvectim == NULL)
        {
            ERROR_EXIT_CC("Can't create vectim (var filter)?!");
        }
     ...
     * avoid duplicating memory usage, this exists regardless of whether there was a mask ---*/
    ...
    /* -- CC normalize input data to zero mean and unit variance this procedure does not change time
     * series that have zero variance -- */
    THD_vectim_normalize(xvectim);   /* L2 norm = 1 */

    /* if using sparsity calculate an initial guess for the correlation threshold that will give
     * that sparsity, from the p-value */
    /* THD_pval_to_stat
     */
    ...
    if ((do_full == 1) || (do_sparsity == 1) || (do_thresh == 1))
    {
        ...
        eigen_vec = calc_full_power_sparse(xvectim, thresh, sparsity, shift,
                                           scale, eps, max_iter, do_binary,
                                           (mem_bytes - running_mem));
    }
    else
    {
