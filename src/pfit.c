 ...
 *
 * On input, fit[] contains the initial estimates of the fitting parameters.  On output, it contains
 *the final estimate.
 *
...
#define HH(k, j)    ehess[(k) + (j) * nfit]
         ...
         *   note that only the lower triangle of the Hessian is computed (HH(jj,kk) for jj >= kk),
         *since the matrix is symmetric and that's all we'll need ***/
        ...
        for (ii = 0 ; ii < nfit ; ii++)   /* in the ii-th row */
        {
            ...
            for (kk = 0 ; kk < ii ; kk++)
            {
                sum -= HH(ii, kk) * HH(ii, kk);
            }
            if (sum <= 0.0)
            {
                fprintf(stderr, "Choleski fails: row %d\n", ii); exit(1);
            }
            HH(ii, ii) = sqrt(sum);
        }
        ...
        {
            sum = egrad[ii];
            for (jj = ii + 1 ; jj < nfit ; jj++)
            {
                sum -= HH(jj, ii) * egrad[jj];
            }
            egrad[ii] = sum / HH(ii, ii);
