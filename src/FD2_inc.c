...
/*! Stores one image (1D to 7D). Why 7D, you ask?  Well, I originally only had 2D images here. When
 * extending AFNI from 3D to 3D+time and buckets, I thought that I might use 4D images (x,y,z,t) as
 * the basic element. Instead, I decided to use an array of 3D images (in a THD_datablock), but by
 * then I'd extended this typedef to allow (x,y,z,t,u,v,w) dimensioned arrays.  I don't think
 * anyplace ever uses more than 3D images, though.
 */
...
void mri_free(MRI_IMAGE *im)
{
    ...
    if (im->fname != NULL)
    {
        free(im->fname); im->fname = NULL;
    }
    im->fondisk = 0;
    if (im->name != NULL)
    {
        free(im->name); im->name = NULL;
    }
...
MRI_IMAGE *mri_to_float(MRI_IMAGE *oldim)
{
    ...
    if (oldim == NULL || mri_data_pointer(oldim) == NULL)
    {
        return NULL;
    }
    ...
    MRI_TYPE kind, int make_space)
{
    ...
    if (nx < 1)
    {
        nx = 1;           /* 18 Mar 2005: fix stupid user problems */
    }
    if (ny < 1)
    {
        ny = 1;
    ...
    newim->was_swapped = 0;                                   /* 07 Mar 2002 - flag that bytes were
                                                               * swapped */
    ...
    if (make_space)
    {
        newim->im = calloc(newim->pixel_size, npix);
    }
    else
    {
        newim->im = NULL;
 ...
 *
 * If( head < 0 ) then file length must match size else file length must == n*size + head for some
 * integer n, which will be the number of slices to read from the file.
 */
 ...
 *  \return NULL if an image couldn't be read, otherwise a pointer to an MRI_IMAGE with data,
 * dimensions, etc.
 */
...
MRI_IMAGE *mri_read(char *fname)
{
    ...
    { char str[5] = "AFNI";
      ...
      rewind(imfile); vv = fread(str, 1, 4, imfile);                        /* check for "IMGF" or
                                                                             * "GEMS" */

      if (str[0] == 'G' && str[1] == 'E' && str[2] == 'M' && str[3] == 'S') /* 12 Feb 2004 */
      {
          char buf[4096]; int bb, cc;                                       /* search for IMGF in
                                                                             * 1st 4K */
          rewind(imfile); cc = fread(buf, 1, 4096, imfile); cc -= 4;
          for (bb = 4; bb < cc ; bb++)
          {
              if (buf[bb] == 'I' && buf[bb + 1] == 'M' && buf[bb + 2] == 'G' && buf[bb + 3] == 'F')
              {
                  break;
              }
          }
      ...
      if (str[0] == 'I' && str[1] == 'M' && str[2] == 'G' && str[3] == 'F')
      {
          ...
          if (length == 0)
          {
              ...
              if (hdroff > 0)                /* read from image header */
              {
                  ...
                  if (swap)
                  {
                      myswap_4(&dxx); myswap_4(&dyy); myswap_4(&dzz);
                  }
...

                  {
                      ...
                      if (kk == 1 || kk == 2 || kk == 3)
                      {
                          MRILIB_zoff = -MRILIB_zoff;
                      }

                      /* Same for x offset; [20 Dec 2001] This must be at the middle of the TLHC
                       * voxel, so we must move a little bit towards the TRHC edge;
                       * We only use the result if the x-coordinate doesn't change significantly
                       * between the TRHC and BRHC, to avoid problems with oblique slices.
                       *         */
                      ...
                      if (ii == 1 || ii == 2 || ii == 3)
                      {
                          MRILIB_xoff = -MRILIB_xoff;
                      }
                      use_MRILIB_xoff = (fabs(xyz[qq + 2] - xyz[qq + 5]) < 0.01 * dxx);

                      /* Same for y offset;
                       * This must be at the middle of the TLHC voxel, so we must move a little bit
                       * towards the BRHC edge;
                       * We only use the result if the y-coordinate doesn't change significantly
                       * between the TLHC and TRHC. */
    ...
    if (eee == NULL)
    {
        eee = getenv("FD2_FLOATPAIR_IMAGES");
    }
    if (eee == NULL)
    {
        eee = getenv("FLOATPAIR_IMAGES");
    }
...
The_Old_Way:
    ...
    default:
        ...
        if (im == NULL)
        {
            im = mri_try_7D(imfile, &skip);                     /* 7D format  */
        }
        if (im == NULL)
        {
            im = mri_try_pgm(imfile, &skip);                    /* PGM format */
        }
        if (im != NULL)
        {
            break;
...
Ready_To_Roll:
    ...
    if (swap)
    {
        ...
        if (fpim != NULL)
        {
            swap_fourbytes(fpim->nvox, fpdata);
        }
 ...
 *  \return NULL if an image could not be read, otherwise the address of a new MRI_IMAGE structure
 */
 ...
 *  \param skip is a pointer to an int that will be set to the number of bytes to skip from the file
 * start to find the image data
 *  \return NULL if the file doesn't work for "Cox MRI" format;
 *          otherwise, the return is a pointer to an MRI_IMAGE ready to have its data read from
 * imfile.
 */
...
static MRI_IMAGE *mri_try_mri(FILE *imfile, int *skip)
{
    ...
    ch = getc(imfile); if (ch != 'M')
    {
        return NULL;                                     /* check for MRI */
    }
    ch = getc(imfile); if (ch != 'R')
    {
        return NULL;
    }
    ch = getc(imfile); if (ch != 'I')
    {
        return NULL;
    }
 ...
 *
 *  \param imfile is a pointer to an open FILE
 *  \param skip is a pointer to an int; *skip will be set to the byte offset at which to start
 * reading data
 *  \return A pointer to an MRI_IMAGE ready to have its data read in (if the file is a PGM file), or
 * NULL.
 */
...
static MRI_IMAGE *mri_try_pgm(FILE *imfile, int *skip)
{
    ...
    ch = getc(imfile); if (ch != 'P')
    {
        return NULL;                                   /* check for magic */
    }
    ch = getc(imfile); if (ch != '5')
    {
        return NULL;
    }
    ...
    NUMSCAN(maxval); if (maxval <= 0 || maxval > 255)
    {
        return NULL;
    }
...
/*--------------------------------------------------------------
 * Read a pile of images from one file. Modified 4/4/95 to read short or byte data. Modified
 * 10/02/95 to allow byte swapping with 3Ds:
...
MRI_IMARR * mri_read_3D(char *tname)
{
    ...
    /** 13 Apr 1999: modified to allow actual hglobal < -1 as long as hglobal+himage >= 0       **/
 ...
 * \param fname is the name of the file to read.  This file might be in one of these formats:
 ...
 *
 * \return A pointer to an array of 2D images.  If nothing could be read, NULL is returned.
 */
...
MRI_IMAGE * mri_read_just_one(char *fname)
{
    ...
    if (imar->num != 1)
    {
        DESTROY_IMARR(imar); return NULL;
    }
 ...
 *
 *  This was developed in the days when FD/FD2/fim ruled the world, and those programs (AJ's legacy)
 * only deal with square images that are a power of 2 in size.
 *  \date 01 Oct 1995
 */
 ...
 *
 *  This implements the facility for the user to define MCW_IMSIZE_1 (or AFNI_IMSIZE_1) et cetera,
 * for pre-defining a relationship between a file size in bytes and a 3D: prefix.  This function is
 * only called once to setup the table.
 *  \date 07 Nov 95
 */
 ...
 *  \return A new "filename" with 3D header attached if it fits. If not, return a copy of the
 * filename.  In any case the returned string should be free()-d when it is no longer needed.
 */
...
/***************************************************************************
 * From http://www.mayo.edu/bir/Analyze_Pages/AnalyzeFileInfo.html Describes the ANALYZE 7.5 file
 * format (.img/.hdr pairs).
 * ----------------------------------------------------------------
 * The image database is the system of files that the ANALYZE package uses to organize and access
 * image data on the disk. Facilities are provided for converting data from a number of sources for
 * use with the package. A description of the database format is provided to aid developers in
 * porting images from other sources for use with the ANALYZE TM system. An ANALYZE image database
 * consists of at least two files:
 *
 * an image file a header file
 *
 * The files have the same name being distinguished by the extensions .img for the image file and
 *.hdr for the header file. Thus, for the image database heart, there are the UNIX files heart.img
 *and heart.hdr. The ANALYZE programs all refer to this pair of files as a single entity named
 *heart.
 *
 * Image File
 *
 * The format of the image file is very simple containing usually uncompressed pixel data for the
 *images in one of several possible pixel formats:
 *
 * 1 bit            packed binary (slices must begin on byte boundaries) 8 bit            8 bits per
 *pixel (unsigned char) 16 bit           16 bits per pixel (signed short) 32 bit           32 bits
 *per pixel signed integers, or floating point 64 bit           64 bits per pixel; double precision,
 *floating point, or complex.
 * 24 bit           RGB , 8-bits per channel Red, Green, Blue.
 *
 * Header File
 *
 * The header file is represented here as a `C' structure which describes the dimensions and history
 *of the pixel data. The header structure consists of three substructures:
 *
 *   header_key         describes the header image_dimension    describes image sizes data_history
 *      optional
 ****************************************************************************/
...
struct image_dimension {                /* off + size      */
     ...
     *   pixdim[1] - voxel width pixdim[2] - voxel height pixdim[3] - interslice distance
     *       ...etc
     */
...
/***************************************************************************
 * The header format is flexible and can be extended for new user-defined data types. The essential
 * structures of the header are the header_key and the image_dimension.
 *
 * The required elements in the header_key substructure are:
 *
 *   int sizeof_header      Must indicate the byte size of the header file.
 *   int extents            Should be 16384, the image file is created as contiguous with a minimum
 * extent size.
 *   char regular           Must be `r' to indicate that all images and volumes are the same size.
 *
 * The image_dimension substructure describes the organization and size of the images. These
 * elements enable the database to reference images by volume and slice number. Explanation of each
 * element follows:
 *
 *   short int dim[]; = array of the image dimensions dim[0]      = Number of dimensions in
 * database; usually 4 dim[1]      = Image X dimension; number of pixels in an image row dim[2]
 *      = Image Y dimension; number of pixel rows in slice dim[3]      = Volume Z dimension; number
 * of slices in a volume dim[4]      = Time points, number of volumes in database.
 *
 *        char vox_units[4] = specifies the spatial units of measure for a voxel char cal_units[4] =
 * specifies the name of the calibration unit
 *
 *       short int datatype = datatype for this image set Acceptable values for datatype are one of
 * the ANDT_* defines above
 *
 *       short int bitpix   = number of bits per pixel; 1, 8, 16, 32, 64 short int dim_un0  = unused
 *
 *       float pixdim[]     = Parallel array to dim[], giving real world measurements in mm and ms.
 *        pixdim[1]         = voxel width in mm pixdim[2]         = voxel height in mm pixdim[3]
 *         = slice thickness in mm float vox_offset   = byte offset in the .img file at which voxels
 * start. This value can be negative to specify that the absolute value is applied for every image
 * in the file float cal_max,     = specify the range of calibration values cal_min int glmax, glmin
 *   = The maximum and minimum pixel values for the entire database
 *
 * The data_history substructure is not required, but the orient field is used to indicate
 * individual slice orientation and determines whether the Movie program will attempt to flip the
 * images before displaying a movie sequence.
 *
 *   orient = slice orientation for this dataset.
 *              0 = transverse unflipped 1 = coronal unflipped 2 = sagittal unflipped 3 = transverse
...
MRI_IMARR * mri_read_analyze75(char *hname)
{
    ...
    { short xyzuv[5], xx, yy, zz;
      ...
      if (xyzuv[3] == 0 && xyzuv[4] == 0)
      {
          ...
          if (doswap)
          {
              myswap_2(&xx); myswap_2(&yy); myswap_2(&zz);
          }
...
/* 28 Aug 1996: return value changed from void to int, which will be the number of files written to
 * disk (either 0 or 1 at the present time).             */
...
int mri_write_pnm(char *fname, MRI_IMAGE *im)
{
    ...
    if (im->kind != MRI_byte && im->kind != MRI_rgb)
    {
        return 0;
    }
...
/* ----------------------------------------------------------------------
 * This is an interface for processing GEMS 4.x image files (see ge4_header.h for structure
 *contents).
 * ----------------------------------------------------------------------
 * int ge4_read_header( char * filename, ge4_header * H )
 *
 *     The basic point is to pass an empty ge4_header structure in, along with a file name.  This
 *function fills the structure.
 *
 *     One exception: if H->image is non-null, this function will read the image into that location.
 *
...
void mri_scale_inplace(float fac, MRI_IMAGE *im)
{
    ...
    if (im == NULL || fac == 1.0 || fac == 0.0)
    {
        return;
    }
    vp = mri_data_pointer(im); if (vp == NULL)
    {
        return;
    }
    nvox = im->nvox;
