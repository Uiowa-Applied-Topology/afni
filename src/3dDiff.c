...
int main(int argc, char *argv[])
{
    ...
    /* TODO: make error summary for all user mistakes simultaneously. Strategy could be to count up
     * arguments, store each error message in a 2D char array, and then print it to the terminal in
     * the desired format*/
    ...
    while (iarg < argc && argv[iarg][0] == '-')
    {
        ...
        if (strncmp(argv[iarg], "-tol", 4) == 0)
        {
            if (iarg >= argc)
            {
                diff3d_crash("Need value after -tol");
            }
        ...
        if (strncmp(argv[iarg], "-mask", 5) == 0)
        {
            if (++iarg >= argc)
            {
                diff3d_crash("Need dset after -mask");
            }
        ...
        if (strncmp(argv[iarg], "-a", 2) == 0)
        {
            if (++iarg >= argc)
            {
                diff3d_crash("Need dset after -a");
            }
        ...
        if (strncmp(argv[iarg], "-b", 2) == 0)
        {
            if (++iarg >= argc)
            {
                diff3d_crash("Need dset after -b");
            }
    ...
    if (ds1 == NULL)
    {
        diff3d_crash("Can't open dataset %s", a_fname);
    }
    if (mask_fname)
    {
        DSET_mallocize(ds1);
    ...
    if (ds2 == NULL)
    {
        diff3d_crash("Can't open dataset %s", b_fname);
    }
    if (mask_fname)
    {
        DSET_mallocize(ds2);
    ...
    /* Note: in practice, the user would have to scroll up to see this as a header, so we'll treat
     * it as a footer instead */
    ...
    if (quiet)
    {
        /* We should short-circuit early and exit, so if we made it here then there should be no
         * differences; leaving check in place in case some future JBT or collaborator messes that
         * up. */
    ...
    if (long_report)
    {
        ...
        else
        {
            if (nt > 1)
            {
                ...
                /* Calculate the minimum nonzero number of differing elements*/
                for (i = 0; i < nt; ++i)
                {
                    val = counts[i];
                    if (val != 0)
