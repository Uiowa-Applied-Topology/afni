 ...
 *
 * This file contains the basic DCT (Discrete Cosine Transform) transformation subroutine.
 *
 * This implementation is based on Appendix A.2 of the book "Discrete Cosine Transform---Algorithms,
 *Advantages, Applications" by K.R. Rao and P. Yip (Academic Press, Inc, London, 1990). It uses
 *scaled fixed-point arithmetic instead of floating point.
 */
 ...
 *multiply all the constants by DCT_SCALE and convert them to integer constants (thus retaining
 *LG2_DCT_SCALE bits of precision in the constants).  After doing a multiplication we have to divide
 *the product by DCT_SCALE, with proper rounding, to produce the correct output.  The division can
 *be implemented cheaply as a right shift of LG2_DCT_SCALE bits.  The DCT equations also specify an
 *additional division by 2 on the final outputs; this can be folded into the right-shift by shifting
 *one more bit (see UNFIXH).
 *
 * If you are planning to recode this in assembler, you might want to set LG2_DCT_SCALE to 15.  This
 *loses a bit of precision, but then all the multiplications are between 16-bit quantities (given
 *8-bit JSAMPLEs!) so you could use a signed 16x16=>32 bit multiply instruction instead of full
 *32x32 multiply.  Unfortunately there's no way to describe such a multiply portably in C, so we've
 *gone for the extra bit of accuracy here.
 */
...

{
    if (pureDCT)
    {
        reference_fwd_dct(data, dest);
    }
    else
    {
        mp_fwd_dct_fast(data, dest);
    }
}
 ...
 *
 * A 2-D DCT can be done by 1-D DCT on each row followed by 1-D DCT on each column.
 *
 * Results: None
 *
...
{
    int16 *data          = (int16 *)data2d; /* this algorithm wants a 1-d array */
    ...
    /*
     * Each iteration of the inner loop performs one 8-point 1-D DCT. It reads from a *row* of the
     *input matrix and stores into a *column*
     * of the output matrix.  In the first pass, we read from the data[] array and store into the
     *local workspace[].  In the second pass, we read from the workspace[] array and store into
     *data[], thus performing the equivalent of a columnar DCT pass with no variable array indexing.
     */
    ...
    for (pass = 1; pass >= 0; pass--)
    {
        ...
        {
            /*
             * many tmps have nonoverlapping lifetime -- flashy register colourers should be able to
             *do this lot very well
             */
...

{
    ...
    for (i = 0; i < 8; i++)
    {
        for (j = 0; j < 8; j++)
        {
             ...
             * s is quite often x.5 (at least for i and/or j = 0 or 4) and setting the rounding
             *threshold exactly to 0.5 leads to an extremely high arithmetic implementation
             *dependency of the result;
             * s being between x.5 and x.500001 (which is now incorrectly rounded downwards instead
             *of upwards) is assumed to occur less often (if at all)
             */
        }
    }
}
