...
TTRR_controls * New_TTRR_controls(char *atname)
{
    ...
    if ((tto_count = atlas_n_points(atname)) <= 0)
    {
        return ttlc;
    }
...
void TTRR_resetup()
{
    ...
    if (!ttc)
    {
        return;      /* hasn't been setup yet, just wait for setup  */
    }
    dc = ttc->dc;
...
static void TTRR_setup_widgets(MCW_DC *dc)
{
    ...
    if (dc == NULL || ttc != NULL)
    {
        EXRETURN;                               /* might be better to check ttc->dc */
    }
    SHOW_AFNI_PAUSE;
    ...
    if (maxlength < 20)
    {
        maxlength = 20;              /* looks better with some trailing ....*/
    }
    ttc->reg_num = 0;
    for (ii = 0 ; ii < n_points ; ii++)
    {
        ...
        if (tto_list[ii].tdlev == 2)
        {
            strcpy(lbuf, "[G] ");
        }
        else if (tto_list[ii].tdlev == 4)
        {
            strcpy(lbuf, "[A] ");
        }
        if ((tto_list[ii].tdlev == 2) || (tto_list[ii].tdlev == 4))
        {
            strcat(lbuf, Atlas_name_choice(&tto_list[ii]));
        }
        else
        {
            strcpy(lbuf, Atlas_name_choice(&tto_list[ii]));
        }
        ...
        /* only create a few colormenu widgets first, because XtRealizeWidget() is so slow with many
         * widgets, and the impatient user is likely to be unhappy with us  */
...
#define CMMAX    17 /* vertical size = CMMAX colormenus high */
    ...
    /* but make sure window is at least wide enough for the Method and Hemisphere(s) widgets */
...
{
    if (!ttc->av_invert || av == NULL || av->ival == av->old_ival)
    {
        return;
    }

    if (av->ival == 0 ||
        (av->ival != 0 && av->old_ival == 0))
    {
        MCW_invert_widget(av->wrowcol);
    }
...
/*------------------------------------------------------------------------
 * What happens when the user selects "Close" from the window menu in a plugin interface menu window
 * --------------------------------------------------------------------------*/
...
/*------------------------------------------------------------------------
 * Return the current state of the "show atlas colors" in a static struct (i.e., do NOT free()
 *this!).
 * --------------------------------------------------------------------------*/
...
TTRR_params * TTRR_get_params(void)
{
    ...
    if (ttc == NULL)
    {
        RETURN(NULL);                /* report nothing */
    }
    if (ttc->meth_av->ival == TTRR_METH_OFF)
    {
        RETURN(NULL);
    }
...
#define NLBUF    1024
    if (fp != NULL)
    {
        ...
        while (1)
        {
            ...
            if (ns == 1)
            {
                freeup_strings(ns, stok); continue;
            }                                             /* skip */
            if (stok[0][0] == '#' ||
                (stok[0][0] == '/' && stok[0][1] == '/'))
            {
            ...
            if (ns == 2)
            {
                color = stok[1];            /* overlay color name */
            }
            else
            {
                color = stok[2];
