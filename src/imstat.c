...
int main(int argc, char *argv[])
{
    ...
    for (kim = 0 ; kim < IMARR_COUNT(imar) ; kim++)
    {
        ...
        for (ii = 0 ; ii < npix ; ii++)
        {
            if (flar[ii] > im_max2 && flar[ii] < im_max)
            {
                im_max2 = flar[ii];
            }
            if (flar[ii] < im_min2 && flar[ii] > im_min)
            {
                im_min2 = flar[ii];
            }
        }
    ...
    if (pix_prefix != NULL)
    {
        ...
        if (pix_prefix[ii - 1] == '.')
        {
            sprintf(fname, "%smean", pix_prefix);
        }
        else
        {
            sprintf(fname, "%s.mean", pix_prefix);
        }
        ...
        if (pix_prefix[ii - 1] == '.')
        {
            sprintf(fname, "%ssdev", pix_prefix);
        }
        else
        {
            sprintf(fname, "%s.sdev", pix_prefix);
        }
        ...
        if (pix_prefix[ii - 1] == '.')
        {
            sprintf(fname, "%scvar", pix_prefix);
        }
        else
        {
            sprintf(fname, "%s.cvar", pix_prefix);
        }
