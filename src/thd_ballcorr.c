...
/* Modified 12 Nov 2020 to use the fftnf_OMP routine (fftn_omp.c), which is FFTs that have been made
 * thread-safe for OpenMP usage. Not that any individual FFT is parallelized, but you can call the
 * FFT function from multiple threads. Which is what function mri_fft_3Dconvolve_OMP() does. Unlike
 * the original csfft.c function or the original fftn.c function, which are not thread safe.
 *//*-------------------------------------------------------------------------*/
...
MRI_IMAGE * make_ball_mask_float(float brad, float dx, float dy, float dz)
{
    ...
    if (irad < MINRAD || jrad < MINRAD || krad < MINRAD)
    {
        return NULL;
    }
...
int_triple MRI_ball_mask_overlapation(MRI_IMAGE *aim, float brad)
{
    ...
    if (aim == NULL || brad <= 0.0f)
    {
        return ijkout;                                /* bad inputs */
    }
    ...
    for (kk = krad ; kk < nz ; kk++)
    {
        for (jj = jrad ; jj < ny ; jj++)
        {
            for (ii = irad ; ii < nx ; ii++)
            {
                ...
                if (!fmask[ijk])
                {
                    far[ijk] = 0.0f;                 /* use the automask */
                }
#endif
                if (far[ijk] > ftop)
                {
                    ftop = far[ijk];                 /* find the max */
                }
            }
        }
    }
...
/* INFO_message("about to mri_free(fim)") ; */
    ...
    if (fc == 0.0f)
    {
        return ijkout;               /* no positive values? */
    }
...
int_triple THD_ball_mask_overlapation(THD_3dim_dataset *dset, float brad)
{
    ...
    if (!ISVALID_DSET(dset) || brad <= 0.0f)
    {
        return ijkout;
    }
 ...
 * ab = quintuple of floats arad     = radius along axis of rotation ax,ay,az = 3-vector defining
 *axis of rotation brad     = radius along 2 axis perpendicular to rotation axis brad == 0 resets to
 *brad = arad de = triple of floats dx,dy,dz = grid spacings to use along each axis (any 0 value is
 *replaced by 1) spheroid is prolate if arad > brad ; oblate if arad < brad
 *//*-------------------------------------------------------------------------*/
...
MRI_IMAGE * make_spheroid_mask_float(float_quint ab, float_triple de)
{
    ...
    if (arad <= 0.0f)
    {
        return NULL;                /* why do I have to deal with this crap? */
    }
    ...
    for (ijk = kk = 0 ; kk < kdim ; kk++)
    {
        ...
        for (jj = 0 ; jj < jdim ; jj++)
        {
            ...
            for (ii = 0 ; ii < idim ; ii++, ijk++)
            {
                ...
                aqq = xc * ax + yac; aqq = aqq * aqq;      /* aqq = L^2 of (xc,yc,zc) along
                                                            * (ax,ay,az) */
                bqq = xqq - aqq;                           /* bqq = L^2 of (xc,yc,zc) perp to
                                                            * (ax,ay,az) */
                                     ...
                                     float dth, int nthbot, int nthtop)
{
    ...
    if (!ISVALID_DSET(dset) || arad <= 0.0f)
    {
        return ijkout;
    }

    dx = DSET_DX(dset); if (dx == 0.0f)
    {
        dx = 1.0f;
    }
    else
...
/* Replaces mri_fft_3D() in mri_fft_complex.c -- note that alt arg is ignored FFT lengths are in
 * Lxx, Lyy, Lzz; however, Lxx = 0 ==> no FFT in that direction (etc.).
 *//*--------------------------------------------------------------------------*/
...
/* ININFO_message("ii=%d kk=%d",ii,kk) ; */
                  ...
                  for (jj = 0 ; jj < fy ; jj++)
                  {
                      cbig[jj] = far[jj * fx + joff];          /* copy data */
                  }
                  csfft_OMP(Sign, fy, cbig);                   /* FFT in buffer */
                  for (jj = 0 ; jj < fy ; jj++)
                  {
                      far[jj * fx + joff] = cbig[jj];              /* copy back */
                  }
              }
          }
        }
...
/* ININFO_message("ii=%d jj=%d",ii,jj) ; */
                  ...
                  for (kk = 0 ; kk < fz ; kk++)
                  {
                      cbig[kk] = far[kk * fxy + koff];
                  }
                  csfft_OMP(Sign, fz, cbig);
                  for (kk = 0 ; kk < fz ; kk++)
                  {
                      far[kk * fxy + koff] = cbig[kk];
                  }
              }
          }
        }
...
/* Replaces mri_fft_3Dconvolve() in mri_fft_complex.c [Oct 2020] Convolve (via FFT) image aim with
 * bim. Note output image will be at least as big as than the sum of the two sizes.
 *//*--------------------------------------------------------------------------*/
...
static MRI_IMAGE * mri_fft_3Dconvolve_OMP(MRI_IMAGE *aim, MRI_IMAGE *bim)
{
    ...
    /* 1) convert A image to complex 2) zero pad it to fit the FFT size 3) FFT that Then repeat
     * these steps for the B image */
