...
/**** Complex-to-Complex in-place FFT functions. Prototypes below, after some inclusions. If
 * USE_FFTN is enabled, any length is allowed;
 *    otherwise, limited to powers of 2 combined with powers of 3 and 5 (no more than 3^3 and 5^5,
 *though). The original csfft_cox() functions are somewhat faster when applicable, and so are used
 *in preference to the FFTN functions. However, the interface is still via csfft_cox(), described
 *below. ****/
 ...
 * Pentium II 400 MHz:  58% (32) to 36% (256)  [gcc -O3 -ffast-math] SGI R10000 175 MHz:  47% (32)
 *to 18% (256)  [cc -Ofast] HP PA-8000 200 MHz:  58% (32) to 40% (256)  [cc +O3 +Oaggressive]
 * ------------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------
 * Initialize csfft trig constants table.  Adapted from AJ's code. Does both mode=1 and mode=-1
 *tables.
 * ----------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------
 * Complex-to-complex FFT in place:
 *   mode = -1 or +1 (1/idim scaling on inverse [+1] if sclinv was set in the call to
 *csfft_scale_inverse) idim = dimension (power of 2, possibly with a factor of 3^p * 5^q also, for
 *p,q=0..RMAX) xc   = input/output array Automagically re-initializes itself when idim changes from
 *previous call.  By AJ (Andrzej Jesmanowicz), modified by RWCox.
 * ----------------------------------------------------------------------*/

/*- Macro to do 1/N scaling on inverse:
 *    IF it is ordered by the user, and IF mode is positive, and IF this is not a recursive call.
 *  -*/
...
void csfft_cox(int mode, int idim, complex *xc)
{
    ...
    if (idim % 3 == 0)
    {
        rec++; fft_3dec(mode, idim, xc); rec--; SCLINV; return;
    }
    if (idim % 5 == 0)
    {
        rec++; fft_5dec(mode, idim, xc); rec--; SCLINV; return;
    }
 ...
 *   mode = -1 or +1 (NO SCALING ON INVERSE!) idim = dimension (power of 2) nvec = number of vectors
 *to do (vectors are contiguous) xc   = input/output array Automagically re-initializes itself when
 *idim changes from previous call.  By AJJesmanowicz, modified by RWCox.
 * ----------------------------------------------------------------------*/
...
/****************************************************************************
 * Everything from here on down is for the unrolled FFT routines. They were generated with program
 *fftprint.c.
 *****************************************************************************/
...
/*----------------------------------------------------------------
 * Do a 64 FFT using fft32 and decimation-by-2 (complete unrolling wasn't as efficient)
 * ------------------------------------------------------------------*/
...
static void fft64(int mode, complex *xc)
{
    ...
    if (cs == NULL)
    {
        ...
        for (k = 1 ; k < 32 ; k++)
        {
            cs[k].r = cos(k * th); cs[k].i = sin(k * th);
        }
    }
...
/*================================================================
 * Decimation by 4 routines below from 12 Aug 1999 -- RWCox On a 400 MHz PII, about 8% faster than 2
 *decimation by 2 steps. Added fft512, fft1024, and fft2048 also, which are 50-80%
...
/*------------------------------------------------------------------
 * fft_4dec: do a decimation by 4 for N=1024 and 2048. At most RMAX levels of recursion are allowed.
 * --------------------------------------------------------------------*/
...
/*=======================================================================
 * The following radix-3 and radix-5 routines are by RWCox -- Aug 1999. They are not inside unrolled
 *code.
 * =========================================================================*/

/*------------------------------------------------------------------
 * fft_3dec: do a decimation-by-3, plus a power-of-2. At most RMAX levels of recursion are allowed,
 *which means that at most 3**RMAX can be a factor of idim.
 * --------------------------------------------------------------------*/
...
/*------------------------------------------------------------------
 * fft_5dec: do a decimation-by-5, plus a power-of-2. At most RMAX levels of recursion are allowed,
 *which means that at most 5**RMAX can be a factor of idim.
 * --------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------
 * Return the smallest integer >= idim for which we can do an FFT. That is, 2^a * 3^b * 5^c for 0 <=
 * b,c <= RMAX and a >= 0.
 * ----------------------------------------------------------------------*/
...
int csfft_nextup(int idim)
{
    ...
    /*-- build table of allowable powers of 3 and 5 [tf], the powers of 2 just less than them [dn],
     * and their ratios tf/dn                     [rat]. Then sort tf and dn to be increasing in
     * rat.     --*/
...
/*----------------------------------------------------------------------
 * return the next legal value that has only 1 power of 3 and/or 5, and that is also even
 * ------------------------------------------------------------------------*/
...
int csfft_nextup_one35(int idim)
{
    ...
    do{
        ...
        /* skip if divisible by 9 or 25 (too many powers of 3/5), or if not divisible by 2 (result
         * should be even)      */
        if (jj % 9 == 0 || jj % 25 == 0 || jj % 2 == 1)
        {
            jj++;
        }
        else
