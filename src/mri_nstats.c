...
/*! Input = 1D float array, and an NSTAT_ code to compute some statistic. Output = statistic's
 * value.
 *//*------------------------------------------------------------------------*/
...
float mri_nstat(int code, int npt, float *far, float voxval, MCW_cluster *nbhd)
{
    ...
    case NSTAT_NZNUM: {
        ...
        if (code != NSTAT_NZNUM)
        {
            outval /= npt;                      /* fractional number of points that are non-zero */
        }
    }
    break;
    ...
    case NSTAT_VAR: {
        ...
        if (npt == 1)
        {
            break;                                /* will return 0.0 */
        }
        for (mm = 0.0, ii = 0 ; ii < npt ; ii++)
        {
            mm += far[ii];
        }
        mm /= npt;
        ...
        else if (code == NSTAT_VAR)
        {
            outval = vv;
        }
        else if (mm != 0.0f)
        {
            outval = sqrt(vv) / fabsf(mm);
        }
    }
    break;
    ...
    case NSTAT_OSFILT:
        /* [PT: 12 Feb 2025] order statistics filter, which is like a weighted mean combined with a
         * median; this uses the same recipe used in AFNI GUI Image -> Disp -> Project, which is
         * defined by osfilt_proj() in afni_transforms.c */
    {
...
/*!
 * A specialized function for speeding up computations for segmentation computes  5 statistic
 * values: mean, median, sigma, mad, and skew and stores them in fv5
 */
/*------------------------------------------------------------------------*/
...
{
    /*             fv6[6]={average_diff, min_diff, max_diff, average_adiff, min_adiff, max_adiff} */
...
void mri_nstat_fwhmxyz_usevar(int i)
{
    fwhm_use_variance = i;
}
                            ...
                            MRI_IMAGE *im, byte *mask, MCW_cluster *nbhd)
{
    ...
    kk  = xx + yy * nx + zz * nxy;
    if (npt < 6 || kk < 0 || kk >= nxy * nz || !INMASK(kk))
    {
        return fw_xyz;
    }
    ...
    for (ii = 0 ; ii < npt ; ii++)
    {
        aa = xx + nbhd->i[ii]; if (aa < 0 || aa >= nx)
        {
            continue;
        }
        bb = yy + nbhd->j[ii]; if (bb < 0 || bb >= ny)
        {
            continue;
        }
        cc = zz + nbhd->k[ii]; if (cc < 0 || cc >= nz)
        {
            continue;
        }
        kk = aa + bb * nx + cc * nxy;
    ...
    for (ii = 0 ; ii < npt ; ii++)
    {
        ...
        kk = aa + bb * nx + cc * nxy;     if (!INMASK(kk))
        {
            continue;
        }
        arg = far[kk];
                                  ...
                                  float *ws)
{
    ...
    if (npt < 9 || kk < 0 || kk >= nxyz || !INMASK(kk))
    {
        return fw_xyz;
    }
...
/*! Compute a local statistic at each voxel of an image, possibly with a mask; 'local' is defined
 * with a neighborhood; 'statistic' is defined by an NSTAT_ code.
 * ----------------------------------------------------------------------------*/
...
/*
 * Function to turn ijk 1D index on iset into its equivalent i,j,k on a different gridset gset
 *
                               ...
                               int *ii, int *jj, int *kk)
{
     ...
     * m_nind3.ijk[1], m_nind3.ijk[2], m_ncoord.xyz[0], m_ncoord.xyz[1], m_ncoord.xyz[2]);
     */
    ...
    *kk      = m_nind3.ijk[2];
    ...
    /*
     * if (ijk < 10) fprintf(stderr,"HR    [%d %d %d] [%f %f %f]\n", m_nind3.ijk[0], m_nind3.ijk[1],
     * m_nind3.ijk[2], m_ncoord.xyz[0], m_ncoord.xyz[1], m_ncoord.xyz[2]);
     */
    return 1;
}
...
/*fprintf(stderr,"%p, %p, %d, %p", dset, nbhd, ncode, code);*/
    if (dset == NULL || nbhd == NULL || ncode < 1 || code == NULL)
    {
        RETURN(NULL);
    }
/*fprintf(stderr,"%d\n", nbhd->num_pt);*/
    npt = nbhd->num_pt; if (npt == 0)
    {
        RETURN(NULL);
    }

    /* check for stupid reduction parameters allowing = 1.0 for testing purposes*/
    if (redx && redx[0] < 1.0 && redx[1] < 1.0 && redx[2] < 1.0)
    {
        redx = NULL;
    }
    ...
    dx = fabsf(DSET_DX(dset)); if (dx <= 0.0f)
    {
        dx = 1.0f;
    }
    dy = fabsf(DSET_DY(dset)); if (dy <= 0.0f)
    {
        dy = 1.0f;
    }
    dz = fabsf(DSET_DZ(dset)); if (dz <= 0.0f)
    {
        dz = 1.0f;
    }
    ...
    for (iv = 0 ; iv < nvin ; iv++)
    {
        ...
        if (verb && nxyz_o > 999)
        {
            INFO_message("Start sub-brick [%d]", iv);
        }
#endif
        ...
        if (vstep)
        {
            fprintf(stderr, "++ voxel loop [%d]:", iv);
        }
#endif
...
#pragma omp for
...

                {
                    ...
                    else if (code[cc] == NSTAT_LIST) /* Just all the neighbors mam*/
                    {
                        ...
                        if (nbar)
                        {
                            for (pp = 0; pp < nbar_num; ++pp)
                            {
                                aar[cc + pp][ijk] = (float)nbar[pp];
                            }
                        }
                        cc += (nbhd->num_pt - 1);      /* number of sub-bricks added, minus 1
                                                        * Do not use nbar_num-1 because at
                                                        * volume edges you will not have as many
                                                        * neighbors as elsewhere*/
                    }
        ...
        for (cc = 0 ; cc < ncode ; cc++)
        {
            ...
            EDIT_substscale_brick(oset, iv * ncode + cc, MRI_float, aar[cc],
                                  localstat_datum, -1.0);
            if (localstat_datum != MRI_float)
            {
                free(aar[cc]);                             /* 13 Jul 2009 */
            }
        }
