...
/* powell_newuoa.f -- translated by f2c (version 19961017). You must link the resulting object file
 * with the libraries:
...
/*    This subroutine seeks the least value of a function of many variable s,*/
/*    by a trust region method that forms quadratic models by interpolatio n.*/
...
/*    Initial values of the variables must be set in X(1),X(2),...,X(N). T hey*/

/*       will be changed to the values that give the least calculated F.
 */
/*    RHOBEG and RHOEND must be set to the initial and final values of a t rust*/
/*      region radius, so both must be positive with RHOEND<=RHOBEG. Typic ally*/
/*      RHOBEG should be about one tenth of the greatest expected change t o a*/
/*      variable, and RHOEND should indicate the accuracy that is required in*/
/*       the final values of the variables. */

/*CCC     The value of IPRINT should be set to 0, 1, 2 or 3, which control s the*/

/*CCC       amount of printing. Specifically, there is no output if IPRINT
 * =0 and*/
/*CCC       there is output only at the return if IPRINT=1. Otherwise, eac h new*/
/*CCC       value of RHO is printed, with the best vector of variables so far and*/
/*CCC       the corresponding value of the objective function. Further, ea ch new*/
/* CCC       value of F with its variables are output if IPRINT=3. */

/*    MAXFUN must be set to an upper bound on the number of calls of CALFU N.*/
/*    The array W will be used for working space. Its length must be at le ast*/
                             ...
                             doublereal *vlag, doublereal *w, integer *icode)
{
    ...
    STATIC doublereal diffa = 0, diffb = 0, diffc = 0, hdiag = 0, alpha = 0, delta = 0, recip = 0,
                      reciq = 0,
                      fsave = 0;
...
/*    The arguments N, NPT, X, RHOBEG, RHOEND, IPRINT and MAXFUN are ident ical*/
/*       to the corresponding arguments in SUBROUTINE NEWUOA. */

/*    XBASE will hold a shift of origin that should reduce the contributio ns*/
...
/*    XPT will contain the interpolation point coordinates relative to XBA SE.*/
...
/*      H, this factorization being ZMAT times Diag(DZ) times ZMAT^T, wher e*/
...
/*    Begin the initialization procedure. NF becomes one more than the num ber*/
/*    of function values so far. The coordinates of the displacement of th e*/
 ...
 */
/*    after this calculation. The least function value so far and its inde x*/
/*     are required. */
...
/*    Set the off-diagonal second derivatives of the Lagrange function s and*/
...
/* Computing MIN */
    ...
    if (0 && DEBUG)
    {
        fprintf(stderr, "  sqrt(dsq=%.14g)=%.14g\n", dsq, d__2);
    }
...
/*    Shift XBASE if XOPT may be too far from XBASE. First make the change s*/
...
/*     The following instructions complete the shift of XBASE, includi ng */
...
/*    then BIGDEN calculates an alternative model step, XNEW being used fo r*/
/*     working space. */
...
/*    Update FOPT and XOPT if the new F is the least value of the objectiv e*/
...
/*    If a trust region step makes a small change to the objective function n,*/
/*    then calculate the gradient of the least Frobenius norm interpolant at*/
/*    XBASE, and store it in W, using VLAG for a vector of right hand side s.*/
...
/*     norm interpolant, making the replacement if the test is sat isfied. */
...
/* Computing MIN */
        ...
        if (0 && DEBUG)
        {
            fprintf(stderr, "  sqrt(distsq=%.14g)=%.14g\n", distsq, d__2 * 10.0);
        }
...
/*    The calculations with the current value of RHO are complete. Pick th e*/
...
L490:
    if (rho > *rhoend)
    {
        ...
        if (DEBUG)
        {
            fprintf(stderr, "  rho=%.14g delta=%.14g\n", rho, delta);
        }
...
/* L540: */
        ...
        if (DEBUG)
        {
            fprintf(stderr, " ; final set f=%.14g\n", f);
        }
    }
...
/* CC      END IF */
    ...
    if (DEBUG)
    {
        fprintf(stderr, " ; return f=%.14g with rho=%.14g\n", f, rho);
    }
    return 0;
} /* newuob_ */
                             ...
                             doublereal *hd, doublereal *hs, doublereal *crvmin)
{
    ...
    /* Local variables */
    STATIC doublereal qadd = 0, half = 0, qbeg = 0, qred = 0, qmin = 0, temp = 0, qsav = 0,
                      qnew = 0, zero = 0;
...
/*    N is the number of variables of a quadratic objective function, Q sa y.*/
/*    The arguments NPT, XOPT, XPT, GQ, HQ and PQ have their usual meaning s,*/
...
/*    The following instructions act as a subroutine for setting the vecto r*/
 ...
 */
/*    in the KNEW-th row of the new matrix ZMAT, but, if IFLAG is set to o ne,*/
/*    then the first column of ZMAT will be exchanged with another one lat er.*/
                             ...
                             doublereal *wvec, doublereal *prod)
{
    /* System generated locals */
    integer xpt_dim1 = 0, xpt_offset = 0, bmat_dim1 = 0, bmat_offset = 0, zmat_dim1 = 0,
            zmat_offset = 0, wvec_dim1 = 0, wvec_offset = 0, prod_dim1 = 0, prod_offset = 0,
            i__1    = 0,
            i__2    = 0;
    doublereal d__1 = 0;
    ...
    STATIC doublereal ss = 0, denold = 0, denmax = 0, densav = 0, dstemp = 0, sumold = 0,
                      sstemp = 0,
...
/*    KNEW is the index of the interpolation point that is going to be mov ed.*/
/*    D will be set to the step from XOPT to the new point, and on entry i t*/
/*      should be the D that was calculated by the last call of BIGLAG. Th e*/
...
/*    BETA will be set to the value that will occur in the updating formul a*/
...
/*    D is calculated in a way that should provide a denominator with a la rge*/
...
/*     Seek the value of the angle that maximizes the modulus of DENOM. */
    ...
    for (i__ = 1; i__ <= i__1; ++i__)
    {
        ...
        if (0 && DEBUG)
        {
            fprintf(stderr, "  cos(angle=%.14g)=%.14g sin()=%.14g\n", angle, par[1], par[2]);
        }
        for (j = 4; j <= 8; j += 2)
        {
                             ...
                             doublereal *s, doublereal *w)
{
    ...
    STATIC integer    iu = 0;
    STATIC doublereal sp = 0, ss = 0, taubeg = 0, tauold = 0, cf1 = 0, cf2 = 0, cf3 = 0, cf4 = 0,
                      cf5 = 0, taumax = 0,
...
/*    The step D is calculated in a way that attempts to maximize the modu lus*/
 ...
 */
/*    beginning with the multiplication of S by the second derivative matr ix.*/
