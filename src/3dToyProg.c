...
THD_3dim_dataset * New_Dataset_From_Scratch(char *prefix)
{
    ...
    ENTRY("New_Dataset_From_Scratch"); /* The use of ENTRY and RETURN will allow for the display of
     ...
     *    1st dimension (i) is from Right To Left 2nd dimension (j) is from Anterior to Posterior
     *3rd dimension (k, slice) is from Inferior to Superior Number of voxels along each dimension
     *is:
     *    Ni = 32 Nj = 32 Nk = 10;
     ...
     *    Dj = 6.0;
     *    Dk = 5.2;
     * Voxel origin in mm DICOM RAI Xorg = -12.7;         (Right == -ve X, Left == +ve X) Yorg =
     * 2.3;           (Anterior ==
     * -ve Y, Posterior == +ve Y) Zorg = -4.5;          (Inferior == -ve Z, Superior == +ve Z)
     *
    ...
    /* You can create a dataset with non-cardinal orientation (oblique) by directly specifying the
     * 3x4 matrix (A) that converts voxel index (i,j,k) to  millimeter RAI coordinates (x,y,z) with:
    ...
    /* Now let us fill up this dataset with time series, and say we want the results to be stored in
     * MRI_short. I use short because in this example, one is forced to consider what the scaling
     * factor, if any should be before inserting the time series into the volume. Here I know that
     * my values are between -1.0 and 1.0 so a factor of MRI_TYPE_maxval[MRI_short] is just fine for
     * all nvals sub-bricks
     */

    /* Get the dataset ready to receive 100 values per voxel, and say we have a time axis sampling
     * frequency of 0.5 Hz, TR = 2sec */
    nvals = 100;
    ...
    /* Allocate space for the storage of each sub-brick and setup brick factors */
    for (tt = 0; tt < nvals; ++tt)
    {
        /* By passing NULL below, the function will allocate space for one sub-brick and return */
        ...
        for (jj = 0; jj < DSET_NY(oset); ++jj)
        {
            for (ii = 0; ii < DSET_NX(oset); ++ii)
            {
                ...
                THD_insert_series(nijk, oset, nvals,
                                  ...
                                  0);        /* Values in ts should be scaled before conversion to
                                              * volume storage type. This would have been 1 if oset
                                              * was a volume of floats. */
                ++nijk;
            }
        }
    }
                                         ...
                                         int datum)
{
    ...
    vout = (float *)calloc(nvox, sizeof(float)); /* Allocate for one whole volume, one sub-brick
                                                  * that is */
    ...
    if (datum == MRI_float)  /* store results as float */
    {
        ...
        vout = NULL;  /* do not free vout, oset uses the same pointer because column types is oset
                       * are float */
    ...
    else    /* store results as short */
    {
        ...
        /* Store the results using short precision and set your own scale (here out of laziness I
         * used the same scale as for sub-brick 0) */
    ...
    /* Add your own special attribute -- Note that private attributes are not preserved when making
     * copies of datasets               */
...
void Dataset_Navigation(THD_3dim_dataset *dset)
{
    ...
    /* The most general, though not necessarily most efficient, way to go from voxel (i,j,k) to
     * (x,y,z) is by using matrix A:               */

    /* Load matrix into augmented 4x4 matrix A */
    MAT44_TO_AFF44(A, dset->daxes->ijk_to_dicom_real);

    /* Load i,j,k into an array. For this toy case, pick middle voxel in volume to be sure we have
     * something for any input dataset. Otherwise it is possible to fall outside the grid. */
    ...
    /* To go from voxel 1D index (nijk) (entire volume in one array), to 3D index (i,j,k)
     *                                         */
    ni  = DSET_NX(dset);
                       ...
                       void *ud, int nbriks, float *val)
{
    ...
    if (val == NULL)
    {
        ...
        /* This is when you perform any setup you don't want to repeat each time the function is
         * called for a new voxel. Such a setup typically involves allocating for temporary arrays
         */
...
/* If you follow a comparable syntax in your help, Then you can use AFNI's automatic option
 * completion for your program. */
...
int main(int argc, char *argv[])
{
    ...
    while (iarg < argc && argv[iarg][0] == '-')
    {
        ...
        if (strncmp(argv[iarg], "-mask", 5) == 0)
        {
            if (iarg >= argc)
            {
                ERROR_exit("Need dset after -mask");
            }
        ...
        if (strncmp(argv[iarg], "-prefix", 6) == 0)
        {
            if (iarg >= argc)
            {
                ERROR_exit("Need name after -prefix");
            }
            prefix = argv[++iarg];
            iarg++; continue;
            continue;
        ...
        if (strcmp(argv[iarg], "-datum") == 0)
        {
            if (iarg >= argc)
            {
                ERROR_exit("Need datum type after -datum");
            }
            ++iarg;
    ...
    if (mask_dset)
    {
        ...
        if (mcount <= 0)
        {
            ERROR_exit("No voxels in the mask!\n");
        }
