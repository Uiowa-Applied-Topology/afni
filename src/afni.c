 ...
 *and are released under the Gnu General Public License, Version 2 (or later).  See the file
 *README.Copyright for details.
 ******************************************************************************/
...
/* the definition below is used in some header files, and it indicates that the main() program is in
 * this file */
...
/* if the compiler wants to 'show off' the name of this edition of AFNI, then turn that macro into a
 * string. */
 ...
 * .Xdefaults file, or other resource sources.  AFNI does not come with an "app-defaults" file,
 *since that would be too much like work.
...
/* The trick to using multiple Xt translations in the fallback resources above is to separate them
 * not with '\n' but with '\\n'.  Ugghhhhhhh.  */
...
static void process_XXX_options(int argc, char *argv[])
{
    ...
    while (nopt < argc)
    {
        ...
        if (strncasecmp(argv[nopt], "-XXX", 4) != 0)
        {
            nopt++; continue;
        }
        ...
        if (strcasecmp(argv[nopt], "-XXXnpane") == 0) /* 06 May 2016 */
        {
            ...
            {
                WARNING_message("value '%s' after '%s' is illegal -- ignoring", argv[nopt],
                                argv[nopt - 1]);
            }
            else
            {
...
/*----------------------------------------------------------------------------
 * Global variables that used to be local variables in main(), but since the advent of the splash
 *screen and startup code in MAIN_workprocess() are needed in more than one place.
 * ------------------------------------------------------------------------------*/
...
void AFNI_parse_args(int in_argc, char *in_argv[])
{
    ...
    if (env != NULL)
    {
        ...
        if (SESSTRAIL < 0)
        {
            SESSTRAIL = 0;                     /* 24 Aug 2000 */
        }
        else if (SESSTRAIL > 4)
        {
            SESSTRAIL = 4;                     /* 30 May 2018 */
        }
    }
    ...
    while (narg < argc)
    {
            ...
            strcasecmp(argv[narg], "-BIDS") == 0)    /* 01 Feb 2018 */
        {
                 ...
                 aa++, narg += inc_narg)
            {
...

                {
                    ...
                    {
                        cmd = qsar->str[qq]; if (cmd == NULL)
                        {
                            continue;
                        }
                        bb = strlen(cmd);   if (bb < 2)
                        {
                            continue;
                        }
                        if (cmd[bb - 1] == '/')
                        ...
                        bysub_dir          = (char **)realloc(bysub_dir,
                                                              sizeof(char *) * (num_bysub_dir + 1));
            ...
            if (narg < argc && strcmp(argv[narg], "-") == 0)
            {
                narg++;
            }
            continue;
        ...
        if (strcmp(argv[narg], "-layout") == 0)
        {
            if (narg + 1 >= argc)
            {
                ERROR_exit("need an argument after -layout!");
            }
        ...
        if (strcmp(argv[narg], "-comsep") == 0)
        {
            ...
            if (++narg >= argc)
            {
                ERROR_exit("need an argument after -comsep");
            }
            cc = argv[narg][0];
        ...
        if (strcmp(argv[narg], "-com") == 0)
        {
            ...
            if (++narg >= argc)
            {
                ERROR_exit("need an argument after -com!");
            }
            cm = argv[narg]; ll = strlen(cm); cs = strchr(cm, comsep);
            if (ll > 1024)
            {
                ERROR_message("argument after -com is too long:\n'%s'", argv[narg]);
            }
            else if (ll < 3)
            {
                ERROR_message("argument after -com is too short:\n'%s'", argv[narg]);
            }
            else if (cs == NULL)
            {
                if (COM_num < MAX_N_COM)
                {
                    COM_com[COM_num++] = strdup(argv[narg]);
                }
                else
                {
                    ERROR_message("Too many commands (max allowed=%d)", MAX_N_COM);
                }
            }
            else  /* 22 Feb 2007: break into sub-commands */
            {
                cq = cm = strdup(argv[narg]);
                for (ii = ll - 1 ; isspace(cm[ii]) ; ii--)
                {
                    cm[ii] = '\0';                                /* trim end */
                }
                cs = strchr(cm, comsep);
                while (1)
                {
                    *cs = '\0'; /* NUL terminate command at separator */
                    ...
                    if (ll > 2 && ll <= 1024)
                    {
                        if (COM_num < MAX_N_COM)
                        {
                            COM_com[COM_num++] = strdup(cm);
                        }
                        else
                        {
                            ERROR_message("Too many commands in total.");
                        }
                    }
                    cm = cs + 1; if (*cm == '\0')
                    {
        ...
        /*     This flag disables use of the "done" and X buttons. Close afni via plugout_drive or
         * kill.                  */
        ...
        {
            GLOBAL_argopt.yes_niml--;
            if (GLOBAL_argopt.yes_niml < 0)
            {
                GLOBAL_argopt.yes_niml = 0;
            }
            narg++; continue; /* go to next arg */
        }
        ...
        if (strncmp(argv[narg], "-tbar", 5) == 0)
        {
            if (narg + 1 >= argc)
            {
                ERROR_exit("need an argument after -tbar!");
            }
        ...
        if (strcasecmp(argv[narg], "-big") == 0)
        {
            narg++; continue;
        }
        if (strcasecmp(argv[narg], "-plus") == 0)
        {
            narg++; continue;
        }
        if (strcasecmp(argv[narg], "-minus") == 0)
        {
            narg++; continue;
        }
        if (strcasecmp(argv[narg], "-norm") == 0)
        {
            narg++; continue;
        }
        ...
        if (strncmp(argv[narg], "-R", 2) == 0)
        {
            ...
            else
            {
                ...
                if (ll > 0)
                {
                    GLOBAL_argopt.recurse = ll;
                }
                else
                {
                    ERROR_exit("illegal -R option!");
                }
            }
            narg++; continue; /* go to next arg */
        }
        ...
        if (strncmp(argv[narg], "-orient", 4) == 0)
        {
            if (narg + 1 >= argc)
            {
                ERROR_exit("need an argument after -orient!");
            }
        ...
        if (strncmp(argv[narg], "-ignore", 4) == 0)
        {
            ...
            if (narg + 1 >= argc)
            {
                ERROR_exit("need an argument after -ignore!");
            }

            val = strtod(argv[++narg], NULL);
            if (val >= 0)
            {
                GLOBAL_argopt.ignore = (int)val;
            }
            else
            {
                fprintf(stderr,
        ...
        if (strncmp(argv[narg], "-im1", 4) == 0)
        {
            ...
            if (narg + 1 >= argc)
            {
                ERROR_exit("need an argument after -im1!");
            }

            val = strtod(argv[++narg], NULL);
            if (val >= 1)
            {
                GLOBAL_argopt.ignore = (int)(val - 1.0);
            }
            else
            {
                fprintf(stderr,
        ...
        if (strncmp(argv[narg], "-dy", 3) == 0)
        {
            ...
            if (narg + 1 >= argc)
            {
                ERROR_exit("need an argument after -dy!");
            }

            val = strtod(argv[++narg], NULL);
            if (val > 0)
            {
                GLOBAL_argopt.dy = val;
            }
            else
            {
                fprintf(stderr,
        ...
        if (strncmp(argv[narg], "-dz", 3) == 0)
        {
            ...
            if (narg + 1 >= argc)
            {
                ERROR_exit("need an argument after -dz!");
            }

            val = strtod(argv[++narg], NULL);
            if (val > 0)
            {
                GLOBAL_argopt.dz = val;
            }
            else
            {
                fprintf(stderr,
        ...
        if (strncmp(argv[narg], "-gamma", 4) == 0)
        {
            ...
            if (narg + 1 >= argc)
            {
                ERROR_exit("need an argument after -gamma!");
            }

            val = strtod(argv[++narg], NULL);
            if (val > 0)
            {
                GLOBAL_argopt.gamma = val;
            }
            else
            {
                fprintf(stderr,
        ...
        if (strncmp(argv[narg], "-gsfac", 4) == 0)
        {
            ...
            if (narg + 1 >= argc)
            {
                ERROR_exit("need an argument after -gsfac!");
            }

            val = strtod(argv[++narg], NULL);
            if (val != 0)
            {
                GLOBAL_argopt.gsfac = val;
            }
            else
            {
                fprintf(stderr,
        ...
        if (strncmp(argv[narg], "-ncolor", 3) == 0)
        {
            ...
            if (narg + 1 >= argc)
            {
                ERROR_exit("need an argument after -ncolor!");
            }

            val = strtod(argv[++narg], NULL);
            if (val > 2)
            {
                GLOBAL_argopt.ncolor = val;
            }
            else
            {
                fprintf(stderr,
...
/*-----------------------------------------------------------------------
 * This routine is used if hiding Xt warnings is enabled. It does very little -- it replaces the
 *default Xt warning handler.
 * -------------------------------------------------------------------------*/
 ...
 *
 * Some compilers require __attribute__((noreturn)) for a function passed to XtAppSetErrorHandler.
 *But in that case, exit() might be required, rather than return.
 *
 * Since AFNI_handler is also used for XtAppSetWarningHandler, it should not exit().  So add this
 *new function just for XtAppSetErrorHandler.
 * -------------------------------------------------------------------------*/
 ...
 *   - which says something cute and dies This catenation of events is for Jerzy 'the Mad Pole'
 * Bodurka.
 * ------------------------------------------------------------------------------*/
...
{
    int ii, lt;
    if (targ == NULL || *targ == '\0' || ns <= 0 || ss == NULL)
    {
        return 0;
    }
    lt = strlen(targ);
    for (ii = 0 ; ii < ns ; ii++)
    {
        if (ss[ii] != NULL && strncmp(ss[ii], targ, lt) == 0)
        {
            return 1;
        }
    }
...
int main(int argc, char *argv[])
{
...
#ifdef USE_TRACING
    if (check_string("-trace", argc, argv))
    {
        DBG_trace = 1;
    }
    if (check_string("-TRACE", argc, argv))
    ...
    /* check the processed environment, the afni way: machdep/prefilter and the common main() way:
     * prefilter/machdep  19 Sep 2013 [rickr] */
    ...
    /* Since AFNI_DETACH is applied before machdep() or other my_getenv calls, -D cannot be used to
     * apply this env var, so add an option.
     *                                                7 Mar 2014 [rickr] */
...
/*** INFO_message("after prefilter: argc=%d argv=%p",argc,(void *)argv) ; ***/
    ...
    if (AFNI_VERBOSE && check_string("-q", argc, argv))
    {
        GLOBAL_argopt.quiet = 1;
    }

    /** 12 Dec 2002: scan for "-rt" now, to see if we want to start the version check **/
    ...
    if (argc > 1)
    {
        AFNI_logger("afni", argc, argv);          /* 14 Aug 2001 */
    }
#endif
    ...
    { char *sf = AFNI_get_friend();
      REPORT_PROGRESS(sf);
      REPORT_PROGRESS("\n\n");
      if (check_string("-friend", argc, argv))
      {
          exit(0);
      }
    }
...
/*   REPORT_PROGRESS( ANNOUNCEMENT ) ;*/
    ...
    { char *ep = getenv("AFNI_FONTSIZE");
      if (ep != NULL)
      {
          if (strcasecmp(ep, "minus") == 0)
          {
              XXX_MINUS_FONTS;
          }
          else if (strcasecmp(ep, "plus") == 0)
          {
              XXX_PLUS_FONTS;
          }
          else if (strcasecmp(ep, "big") == 0)
          {
              XXX_BIG_FONTS;
          }
          else if (strcasecmp(ep, "norm") == 0)
          {
              XXX_NORM_FONTS;
          }
      }
    ...
    if (new_FALLback != NULL)        /* if found any -XXX options, merge them */
    {
        ...
        if (xrdb_pg == NULL)
        {
            for (qq = 0 ; FALLback[qq] != NULL ; qq++)
            {
                ...
                {
                    if (equiv_FALLback(new_FALLback[pp], FALLback[qq]))
                    {
                        break;
                    }
                }
...
#undef XXXSIZ
    ...
    if (MAIN_shell == NULL)
    {
        ERROR_exit("Cannot initialize X11");
    }
    ...
    { Display *dpy = XtDisplay(MAIN_shell);
      ...
      if (xsv != NULL)
      {
          sprintf(msg, "[%s v %d]", xsv, xvr); REPORT_PROGRESS(msg);
      }
    }
#endif
    ...
    /* if we used xrdb to set X11 resources, re-set them back to their old state so that other AFNIs
     * don't use these new settings by default   */
    ...
    if (!GLOBAL_argopt.skip_afnirc)
    {
        ...
        if (sysenv != NULL)
        {
            AFNI_process_environ(sysenv);                 /* 12 Apr 2000 */
        }
...
static RwcBoolean MAIN_workprocess(XtPointer fred)
{
    ...
    if (PRINT_TRACING)
    {
        char str[256]; sprintf(str, "MAIN_calls=%d", MAIN_calls); STATUS(str);
    }
    ...
    case 1:
        ...
        if (check_string("-com", MAIN_argc, MAIN_argv) == 0 &&
            ...
            AFNI_yesenv("AFNI_STARTUP_SOUND"))
        {
    ...
    case 10:
        ...
        if (!nosplash)
        {
            AFNI_sleep(1);             /* burn some time to let splash windo popup */
        }
        break;
    ...
    case 13: {
        ...
        { int  nad = (int)AFNI_numenv("AFNI_AdptMeanWidth1D"); /* 30 Sep 2016 */
          ...
          if (nad > 3 && nad != 9 && nad != 19 && nad < 100)
          {
              if (nad % 2 == 0)
              {
                  nad++; INFO_message("increased AFNI_AdptMeanWidth1D to %d", nad);
              }
        ...
        F2D_null();                                               /* should be last 2D func - 20 Oct
                                                                   * 2020 */
        ...
        if (MAIN_im3d->type == AFNI_3DDATA_VIEW)
        {
            ...
            if (GLOBAL_library.plugins != NULL)
            {
                nplug = GLOBAL_library.plugins->num;
            }
    ...
    case 14: {
        ...
        /* 23 Sep 2000: this function will be called 0.123 seconds from now to initialize the window
         * layouts, if any  */
        ...
        /* 21 Jan 2003: this function will be called 0.246 seconds from now to run the startup
         * script commands, if any */
        ...
        if (MAIN_im3d->vwid->func->do_setup)
        {
            ...
            if (ii > 0 && ii < THR_top_expon)
            {
                AFNI_set_thresh_itop(MAIN_im3d, ii);
            }
            MAIN_im3d->vwid->func->do_setup = 0;
        }

        /* this function will be called 1.666 seconds from now to finalize anything else that needs
         * fixing up once AFNI is fully started   */
        ...
        if (AFNI_yesenv("AFNI_THRESH_AUTO"))
        {
            ...
            if (new_thresh > 0.0f)
            {
                AFNI_set_threshold(MAIN_im3d, new_thresh);
            }
        }
...
/*-------------------------------------------------------------------------
 * Callback for the quit button.  If called with the widget == NULL, resets the button to the
 *lowercase state.
 * ---------------------------------------------------------------------------*/
...
{
    int pp;
    if (im3d == NULL || vers_pixmap == XmUNSPECIFIED_PIXMAP)
    {
        return;
    }
...
/*----------------------------------------------------------------------
 * This function is called about 1 s after AFNI startup is completed. It's original purpose was to
 *make sure that the help window was popped down - the help initializing routine does this, too, but
 *it didn't work properly on the old Tektronix X-terminal at MCW. Thus, the timeout - waiting a
 *little made things work OK.
 * ------------------------------------------------------------------------*/
...
void AFNI_startup_timeout_CB(XtPointer client_data, XtIntervalId *id)
{
    ...
    /*--- test geometry of main window [08 Aug 2016] if it is negative territory, move it back to
     * positive land;
    ...
    /* No need to warn anymore, -np can be set by environment variables too. */
    {
        fprintf(stderr,
...

    {
                           ...
                           * -DUSING_LESSTIF was not used when compiling  */
...
#ifdef USE_TRACING
      if (eee != NULL && strcmp(eee, "rwcox") == 0)
      {
          atexit(clock_time_atexit);
      }
#endif
    }
...
/*----------------------------------------------------------------------
 *  Return the underlay or overlay brick and the sub-brick index that the user had selected in the
 * 'Underlay' and 'Overlay' menus.
 *
 *  So whether or not the user has bkgd:ULay or bkgd:OLay is irrelevant to what gets returned.
 *
 *  The returned structure is brr brr->dset will contain the dset selected as underlay if type  ==
 *  isqCR_getulayim and         the dset selected as overlay if type  ==  isqCR_getolayim
 *
...
FD_brick *Get_FD_Brick_As_Selected(FD_brick *br, int type, int *rival)
{
    ...
    if (brr == NULL)
    {
        brr = br;               /* I wish it was 'brr' right now -- July 2014 */
    }
    ...
    /*    This is for allowing montage to cycle through sub-bricks */
    {
        ...
        if (ival < 0 || ival >= DSET_NVALS(brr->dset))
        {
            RETURN(NULL);
        }
    }

    if (type == isqCR_getqimage)
    {
        ival = -1;                                     /* get empty image */
    }
    else if (ival >= DSET_NVALS(brr->dset))
    {
        ival = brr->dset->dblk->nvals - 1;
    }
    ...
    *rival = ival;
     ...
     *DSET_PREFIX(im3d->anat_now), DSET_PREFIX(im3d->fim_now), banat, ival, type, type ==
     * isqCR_getulayim ? "getULAY" :
...
/*
 * Return the overlay or underaly dset currently selected and set the sub-brick number.
 *
 * Peculiar input:
 * UOlay:  'O' --> Return the overlay
 *         'U'     Return the underlay AsSet: 1 --> Return the dataset that is set by the 'Underlay'
 *                    or 'Overlay' selectors.
 *              0 --> Return the 'displayed' overlay or underlay dset. This means, take into account
 *the bkgd:ULay or bkgd:OLay setting.
 */
                              ...
                              byte AsSet, int *rival)
{
    ...
    if (AsSet)  /* as selected in 'Underlay' and 'Overlay' */
    {
        if (!(brt = Get_FD_Brick_As_Selected(br,
                                             UOlay == 'O' ? isqCR_getolayim : isqCR_getulayim,
                                             rival)))
        {
 ...
 *        1 if Axial montage 2 if Sagittal montage 4 if Coronal montage
 */
...
/*----------------------------------------------------------------------
 * Fill the various possible string values used in an imseq getlabel. Return value is 0 if things
 *are good, 1 if things are bad.
 * ------------------------------------------------------------------------*/
...
int AFNI_brick_to_label_parts(int n, FD_brick *br, char **lpart)
{
    ...
    if (lpart == NULL || im3d->type != AFNI_3DDATA_VIEW)
    {
        return 1;
    }
    ...
    for (ii = strlen(str) - 1 ; ii > 0 && str[ii] == '0' ; ii--)
    {
        str[ii] = '\0';
    }
    if (str[ii] == '.')
...
/*----------------------------------------------------------------------
 * routine to extract a plane of data from a 3D brick (used as a "get_image" routine for an
 *MCW_imseq)
 * ------------------------------------------------------------------------*/
...
XtPointer AFNI_brick_to_mri(int n, int type, FD_brick *br)
{
    ...
    if (PRINT_TRACING)
    {
        char str[1024]; sprintf(str, "n=%d type=%d", n, type); STATUS(str);
    }
    ...
    if (type == graCR_getseries)
    {
        ...
        im = FD_brick_to_series(n, br); if (im == NULL)
        {
            RETURN(NULL);
        }
        im->flags = 1;
    ...
    if (type == isqCR_getoverlay)
    {
        ...
        if (!AFNI_noenv("AFNI_VALUE_LABEL"))
        {
            AFNI_do_bkgd_lab(im3d);
        }
        RETURN((XtPointer)im);
    }
    ...
    if (type == isqCR_getmemplot)
    {
        ...
        if (do_surf)
        {
            ...
            for (ks = 0 ; ks < nsurf ; ks++) /* 14 Aug 2002: loop over surfaces */
            {
                ...
                if (ag == NULL)
                {
                    continue;                 /* skip this non-existent one */
                }
...
/** INFO_message("SUMA_mask draw: xyz rel = %g %g %g",xrel,yrel,zrel) ; **/
                ...
                if (skip_boxes && skip_lines)
                {
                    continue;                   /* nothing to do? */
                }

                /** 21 Mar 2002:
                 *  We calculate plotting coordinates in "fdfind" coordinates, which are floating
                 *point indexes into the FD_brick.  However, these run from 0..n1-1 (in x), which
                 *are the centers of the voxels.  In turn these must be mapped to screen locations.
                 ...
                 *    ---------------------
                 ...
                 |
                 |  Thus voxel index i maps to screen location (i+0.5)/n1. Previously, I forgot the
                 |+0.5, which didn't matter much, until the introduction of the image zoom feature
                 |last week. **/
                ...
                if (skip_boxes)
                {
                    STATUS("finding slice planes");
                }
                else
                {
                    STATUS("drawing node boxes");
                }
                    ...
                    if (!skip_boxes)
                    {
                        for (ii = 0 ; ii < nn ; ii++)
                        {
                            ...
                            if (nodx >= xb && nodx <= xt)           /* inside?  */
                            {
                                ...
                                if (firstb)
                                {
                                    plotline_memplot(s1 * (fv.xyz[0] - rxm),
                                                     1.0 - s2 * (fv.xyz[1] - rxm),
                                                     s1 * (fv.xyz[0] + rxp),
                                                     1.0 - s2 * (fv.xyz[1] + rxp));
                                    firstb = 0;
                                ...
                                plotrect_memplot(s1 * (fv.xyz[0] - rxm),
                                                 1.0 - s2 * (fv.xyz[1] - rxm),
                                                 s1 * (fv.xyz[0] + rxp),
                                                 1.0 - s2 * (fv.xyz[1] + rxp));
                                ...
                                if (ks == kbest && nod[ii].id == ibest) /* 24 Feb 2003 */
                                {
                                    ...
                                    plotline_memplot(s1 * (fv.xyz[0] - rxm),
                                                     ...
                                                     s1 * (fv.xyz[0] + rxp),
                                                     1.0 - s2 * (fv.xyz[1] + 0.5));
                                    plotline_memplot(s1 * (fv.xyz[0] + 0.5),
                                                     1.0 - s2 * (fv.xyz[1] - rxm),
                                                     s1 * (fv.xyz[0] + 0.5),
                                                     1.0 - s2 * (fv.xyz[1] + rxp));
                                }
                ...
                /* 10 Mar 2002:
                 * For each triangle that crosses the plane of the slice, plot a line segment at the
                 *intersection of the plane and triangle. The plane is along DICOM axis #kkk at
                 *coordinate xyz;
                ...
                if (!skip_lines && ag->num_ijk > 0 && ag->ijk != NULL)
                {
                                                        ...
                                                        /* slice center, top & bot edges  */
                    {
                        if (ilev == 0)
                        {
                            if (skip_lcen)
                            {
                                continue; /* don't do center plane */
                            }
                            xlev = xyz;
                        ...
                        for (ii = 0 ; ii < ntr ; ii++)
                        {
                            ...
                            id = SUMA_find_node_id(ag, tr[ii].id); if (id < 0)
                            {
                                continue;
                            }
                            jd = SUMA_find_node_id(ag, tr[ii].jd); if (jd < 0)
                            {
                                continue;
                            }
                            kd = SUMA_find_node_id(ag, tr[ii].kd); if (kd < 0)
                            {
                                continue;
                            }
                            ...
                            {
                                fvijk[0].xyz[0] += xrel; fvijk[1].xyz[0] += xrel;
                                fvijk[2].xyz[0] += xrel;
                                fvijk[0].xyz[1] += yrel; fvijk[1].xyz[1] += yrel;
                                fvijk[2].xyz[1] += yrel;
                                fvijk[0].xyz[2] += zrel; fvijk[1].xyz[2] += zrel;
                                fvijk[2].xyz[2] += zrel;
                            }
                            ...
                            if (jj == 0 || jj == 7)
                            {
                                continue;       /* all have same sign */
                            }

                            /* setup id,jd,kd so fvijk[id] is on one side of plane, and so that
                             * fvijk[jd] and fvijk[kd] are on other side */
                            ...
                            /* linearly interpolate between fvijk[id] and fvijk[jd] to find the
                             * point where this line hits the slice plane */
                            ...
                            if (fvp.xyz[0] < daxes->xxmin ||
                                ...
                                fvp.xyz[2] > daxes->zzmax)
                            {
                                continue;                 /* 08 Jan 2004 */
                            }
...
#define PSY(j)    (1.0 - s2 * ((j) + 0.5))
        ...
        type == isqCR_getulayim || type == isqCR_getolayim)
    {
        ...
        if (!(brr = Get_FD_Brick_As_Selected(br, type, &ival)))
        {
            RETURN(NULL);
        }
        ...
        if (ival < 0)
        {
            RETURN((XtPointer)im);               /* return fake image */
        }
...
void AFNI_set_valabel(FD_brick *br, int nsl, MRI_IMAGE *im, char *blab)
{
    ...
    /* if the input image slice index (nsl) doesn't match the current location of the crosshairs,
     * then we don't care about this image */
...
THD_3dim_dataset * AFNI_read_images(int nf, char *fname[])
{
    ...
    if (nf < 1)
    {
        ERROR_exit("No images on command line!?");
    }
    ...
    if (nz == 1)
    {
        nz = 2;            /* special case for just one image */
    }
    ...
    for (lf = 0 ; lf < nf ; lf++)
    {
        ...
        for (kim = 0 ; kim < arr->num ; kim++)
        {
            ...
            else
            {
                ...
                if (shim == NULL)
                {
                    ERROR_exit("Illegal datum convert!");
                }
                mri_free(im);
            }
    ...
    /*- 19 Oct 1999: if we are doing a -tim read, then have to setup the time and z dimensions -*/
    ...
    else                                                /* 19 Oct 1999: make up a lot of bricks and
                                                         * attach them all */
                                                        /* 20 Oct 1999: allow for the 3rd dimension
                                                         * as well         */
    ...
    if (dx > 0.0)
    {
        dset->daxes->xxdel = dx;             /* 29 Jul 2002 */
    }
    if (dy > 0.0)
    {
        dset->daxes->yydel = dy;
...
void AFNI_jumpto_clus_nearby(Three_D_View *im3d, int dci)    /* 29 Apr 2019 */
{
    ...
    clar = im3d->vwid->func->clu_list; if (clar == NULL)
    {
        return;
    }
    nclu = clar->num_clu;              if (nclu == 0)
    {
        return;
    }
...
void AFNI_seq_send_CB(MCW_imseq *seq, FD_brick *br, ISQ_cbs *cbs)
{
    ...
    case isqCR_destroy: {
        ...
        /* July 1996: redraw if we just lost a crosshair montage (it would have been in the z
    ...
    case isqCR_buttonpress: {
        ...
        case Button1: {      /* set viewpoint? set InstaCorr? */
            ...
            /* April 1996:  only use this button press if it is inside the confines of the brick */
                ...
                cbs->nim >= 0 && cbs->nim < br->n3)
            {
                ...
                if (doing_icor)
                {
                    ...
                    if (qq > 0 && im3d->giset == NULL)
                    {
                        AFNI_icor_setref_locked(im3d);                                  /* 15 May
                                                                                        * 2009 */
                    }
                }
            }
    ...
    case isqCR_keypress: {
        ...
        case '}': {    /* Change the threshold slider up or down */
            ...
            else if (nff > THR_top_value)
            {
                nff = THR_top_value;
            }
            if (nff != fff)
            {
                AFNI_set_threshold(im3d, nff);
            }
        }
        break;
    ...
    case isqCR_button2_points: {
...

        {
            ...
            /* translate coordinates to dataset xyz indices, casting out any that are outside the
             * dataset brick */
...
void AFNI_gra_send_CB(MCW_grapher *grapher, FD_brick *br, GRA_cbs *cbs)
{
    ...
    /*** 27 Jan 2004:
     *   User toggled WinAver on in one graph window, so we toggle it off in the other windows.
     *    ***/
...
/*! Report on datasets with mixed type sub-bricks, as they tend to cause problems. */
...
/*                dset = gss->dsset_xform_table[qd][vv] ;*/
        ...
        for (id = 0; id < num_bysub_dir; id++)
        {
            ADDTO_SARR(dlist, bysub_dir[id]);
        }
        ...
        if (qlist != NULL)
        {
            DESTROY_SARR(dlist); dlist = qlist;
        }
        ...
        for (id = 0 ; id < num_ss ; id++)
        {
            ...
            if (THD_is_directory(dname))  /* directory? read session(s) */
            {
                ...
                for (qss = 0 ; qss < num_bysub ; qss++) /* bysub [01 Feb 2018] */
                {
                    if (first)
                    {
                        fprintf(stderr, "[bysub/BIDS]"); first = 0;
                    }
...
/*                 dset = new_ss->dsset_xform_table[qd][vv] ;*/
                    ...
                    if (do_css)
                    {
                        THD_append_sessions(css, new_ss);
                    }
                    num_css++;
...
#define NCANON    3 /* added 22 Oct 2019 */
         ...
         * then, make any datasets that don't exist but logically descend from the warp and anatomy
         *parents just assigned */
...

    {
        ...
        {
            /** 23 Mar 2001: modified code to deal with an array of datasets, rather than just one
             * at a time **/
...
void AFNI_controller_panel_CB(Widget wcall, XtPointer cd, XtPointer cbs)
{
    ...
    if (!IM3D_OPEN(im3d) || im3d->vwid->prog->panel_pb == NULL)
    {
        EXRETURN;
    }
...
void AFNI_crosshair_visible_CB(MCW_arrowval *av, XtPointer client_data)
{
    ...
    if (omold != im3d->vinfo->xhairs_orimask)
    {
        AFNI_process_viewpoint(im3d);
    }
                       ...
                       XtPointer client_data, XtPointer call_data)
{
    ...
    if ((RwcBoolean)bval == im3d->vinfo->xhairs_periodic)
    {
        EXRETURN;
    }
                        ...
                        XtPointer client_data, XtPointer call_data)
{
    ...
    if ((RwcBoolean)bval == im3d->vinfo->xhairs_all)
    {
        EXRETURN;
    }
...
void AFNI_time_index_step_CB(Widget w, XtPointer cd, int nval, void **val)
{
    ...
    if (!IM3D_VALID(im3d) || nval != 3 || val == NULL)
    {
        EXRETURN;
    }
                        ...
                        XEvent *ev, RwcBoolean *continue_to_dispatch)
{
    ...
    case ButtonPress: {
        ...
        if (event->button == Button3)
        {
            ...
            thrin = im3d->vinfo->thr_olayx; if (thrin < 0 || thrin > 2)
            {
                thrin = 0;
            }
                      ...
                      XtPointer client_data, XtPointer call_data)
{
    ...
    /* handle case of button press of already open window by bringing that window to the top */
    ...
    if (snew != NULL && (brnew->n1 < 2 || brnew->n2 < 2))
    {
        EXRETURN;
    }
    ...
    if (snew != NULL)
    {
        ...
        *snew = open_MCW_imseq(im3d->dc, (get_ptr)AFNI_brick_to_mri,
        ...
        if (mirror || m2m || c2c)
        {
            ...
            if (m2m)
            {
                opt.scale_range = ISQ_RNG_MINTOMAX;
            }
            if (c2c)
            {
                opt.scale_range = ISQ_RNG_CLIPPED;
            }
...
#undef II
        ...
        if (event != NULL)
        {
            WID2EV((*snew)->wtop, event);                    /* 23 Nov 2010 */
        }
    ...
    if (gnew != NULL && DSET_GRAPHABLE(brnew->dset))
    {
        ...
        *gnew           = gr;
        ...
        if (afni48_good)
        {
            ...
            if (w == gr_xyz)
            {
                pm = afni48graaxi_pixmap;
            }
            else if (w == gr_yzx)
            {
                pm = afni48grasag_pixmap;
            }
            else if (w == gr_zxy)
            {
                pm = afni48gracor_pixmap;
            }
        ...
        if (event != NULL)
        {
            WID2EV(gr->fdw_graph, event);                    /* 23 Nov 2010 */
        }
                     ...
                     XEvent *ev, RwcBoolean *continue_to_dispatch)
{
    ...
    if (ev->type != ButtonPress || !IM3D_OPEN(im3d))
    {
        EXRETURN;
    }
    if (event->button != Button3)
    ...
    if (w == pb_xyz && ISQ_REALZ(sxyz))
    {
        GETWIN(sxyz->wtop);
    }
    else if (w == pb_yzx && ISQ_REALZ(syzx))
    {
        GETWIN(syzx->wtop);
    }
    else if (w == pb_zxy && ISQ_REALZ(szxy))
    {
        GETWIN(szxy->wtop);
    }
    else if (w == gr_xyz && GRA_REALZ(gxyz))
    {
        GETWIN(gxyz->fdw_graph);
    }
    else if (w == gr_yzx && GRA_REALZ(gyzx))
    {
        GETWIN(gyzx->fdw_graph);
    }
    else if (w == gr_zxy && GRA_REALZ(gzxy))
    {
        GETWIN(gzxy->fdw_graph);
    }
...
void AFNI_redisplay_func_ignore(int ig)
{
    ignore_redisplay_func = ig;
}
...
void AFNI_range_setter(Three_D_View *im3d, MCW_imseq *seq)
{
    ...
    if (!IM3D_VALID(im3d) || !ISQ_VALID(seq))
    {
        EXRETURN;
    }
    ...
    br = (FD_brick *)im3d->b123_ulay; if (br == NULL)
    {
        EXRETURN;
    }
    ds = br->dset;                    if (ds == NULL)
    {
        EXRETURN;
    }
...
/*! Define the view_setter code, which says which image viewer plane was used to set the current
 * viewpoint.  [26 Feb 2003]
 * --------------------------------------------------------------------------*/
...
void AFNI_check_for_multiple_vedits(Three_D_View *im3d)
{
    ...
    for (qq = 0 ; qq < MAX_CONTROLLERS ; qq++)
    {
        ...
        if (qq3d->vedset.code <= 0 || qq3d->vedset.ival < 0)
        {
            continue;
        }
...
#define EXRR    do{ recurse--; EXRETURN; } while (0)
    ...
    if (recurse > 1)
    {
        return;                /* changed from 3 to 1 [16 Sep 2009] */
    }
    recurse++;
    ...
    if (!IM3D_VALID(im3d) || !ISVALID_3DIM_DATASET(im3d->anat_now))
    {
        EXRR;
    }
        ...
        ihave && im3d->vinfo->func_visible && !doflash)
    {
        ...
        if (VEDIT_good(im3d->vedset))
        {
            ...
            else if (changed)
            {
                ...
                if (rrr != NULL && *rrr != '\0')
                {
                    im3d->vwid->func->clu_rep = strdup(rrr);
                }
 ...
 * (return NULL if none;  the result must be mri_free-d by the user) NOTE: most of the code in this
 *function is no longer executed under 'normal' circumstances
 * ------------------------------------------------------------------------------*/
...
MRI_IMAGE * AFNI_overlay(int n, FD_brick *br)
{
    ...
    int               do_xhar = 0;                                                             /* 22
                                                                                                ...
                                                                                                * 2002
                                                                                                */
    MRI_IMAGE *rgbov          = NULL;                                                          /* 30
                                                                                                * Jan
                                                                                                * 2003
                                                                                                */
    int jill = im3d->vinfo->see_ttatlas && AFNI_yesenv("AFNI_JILL_TRAVESTY");                  /*
                                                                                                * Jill
                                                                                                *is
                                                                                                *trouble
                                                                                                */
    ...
    if (!ovgood)
    {
        RETURN(NULL);             /* nothing is asked of us! */
    }
    ...
    if (im3d->vinfo->func_visible)
    {
        ...
        /* in the olden days, fov might be shorts (indexes to colors), or might be RGB, but now it
         * is always RGBA. However, the older codes below are built around short (indexed) overlays,
         * and so we set aside the RGBA output to build the (obsolescent) short overlays, and then
         * merge the RGBA and short overlay images later */

        /* so if by some weird chance (or later code change), we still get a color-indexed overlay
         * image back here, convert it to RGB now  */
        ...
        if (jill)
        {
            if (rgbov == NULL)
            {
                INFO_message("AFNI_func_overlay is NULL");
            }
            else
            {
                INFO_message("AFNI_func_overlay is %s", MRI_TYPE_NAME(rgbov));
            }
        }
        ...
        im3d->anat_now->view_type == VIEW_TALAIRACH_TYPE)
    {
        ...
        /* if rgbov is non-NULL, the return will be rgbov again, after suitable editing; otherwise,
         * the return is a new image;
        ...
        if (tov != NULL && tov->kind == MRI_short) /* should not happen */
        {
            if (jill)
            {
                WARNING_message("AFNI_ttatlas_overlay returned a short-valued image!");
            }
            mri_free(rgbov);
        ...
        else if (tov != NULL && IS_RGB_TYPE(tov->kind) && tov != rgbov)
        {
            if (jill && rgbov != NULL)
            {
                ININFO_message(" replacing rgbov with AFNI_ttatlas_tov combo");
            }
            else if (jill)
            {
                ININFO_message(" making rgbov = AFNI_ttatlas_tov output");
            }
            if (rgbov != NULL)
            {
                mri_free(rgbov);               /* might happen??? */
            }
            rgbov = tov;
        }
    ...
    /*----- now set up short (indexed) overlay image as the functional overlay (if present), or as a
     * new blank image (otherwise) -----*/
    ...
    if (do_xhar)
    {
        ...
        /** April 1996: Only put crosshairs on if image number matches current slice number of
         * viewpoint.
         *              (This allows for the montage multislice view) **/

        /** July 1996: Allow for multiple crosshairs to indicate the location of montage multislice
         * views. **/

        /** Aug 1996: Allow for periodic (wrap) or non-periodic montages. Also, if in "Single" mode
         * and also are graphing, then only draw the grapher frame, not the crosshairs. **/
        ...
        if (n == ib.ijk[2] || im3d->vinfo->xhairs_all)
        {
            ...
            if (im3d->vinfo->xhairs_show_montage)          /* in "Multi" mode */
            {
                ...
                {
                    char str[256];
                    sprintf(str, "montage xhairs: ax   =%d ay   =%d az =%d", ax, ay, az);
                    STATUS(str);
                    sprintf(str, "                iskip=%d idown=%d iup=%d", iskip, idown, iup);
                    STATUS(str);
                    sprintf(str, "                jskip=%d jdown=%d jup=%d", jskip, jdown, jup);
                    STATUS(str);
                    sprintf(str, "orimask=%d ox=%d oy=%d", im3d->vinfo->xhairs_orimask, ox, oy);
                    STATUS(str);
                }
            }
            else                                           /* in "Single" Mode */
        ...
        (im3d->vwid->marks->tag_visible == True))
    {
        ...
        if (tem == NULL)
        {
            tem = myXtNew(AFNI_ovtemplate);              /* once only */
        }
        np = MAX(nx, ny) / 64; np = MAX(np, 2);
        if (np != npold)
        {
            npold = np; AFNI_make_tagmask(np, 0, tem);
        }
    ...
    /* 30 Jan 2003:
     * If the functional overlay is in RGB format, then must meld that with the short color index
     *image */
...
XmString AFNI_crosshair_label(Three_D_View *im3d)
{
    ...
    else
    {
        ...
        if (dset_obliquity(im3d->anat_now, &angle) == 1)
        {
            sprintf(obl, " *");
        }
        else
...
/*-------------------------------------------------------------------------
 * handle the selection of a marker name by the user (using the toggle buttons from the control
 *panel or the popup menu)
 * ---------------------------------------------------------------------------*/
                     ...
                     XtPointer client_data, XtPointer call_data)
{
    ...
    if (!IM3D_OPEN(im3d) || im3d->anat_now->markers == NULL)
    {
        EXRETURN;
    }
    ...
    /* bval      = index of toggle that is set (-1 if none) other_tog = pointer to other set of
     * toggles;
...
int AFNI_first_tog(int ntog, Widget *tog)
{
    ...
    for (ib = 0 ; ib < ntog ; ib++)
    {
        if (XmToggleButtonGetState(tog[ib]))
        {
            break;
        }
    }
...
/*------------------------------------------------------------------------
 * handle the changing arrowvals for marker display controls (set colors and sizes of markers)
 * --------------------------------------------------------------------------*/
...
void AFNI_make_ptmask(int size, int gap, AFNI_ovtemplate *tem)
{
    ...
    for (ix = -size ; ix <= size ; ix++)
    {
        ...
        if (ax != size)
        {
            PUTPIX(size, ix); CHKPIX; PUTPIX(-size, ix); CHKPIX;
        }
        if (ax > gap)
        {
            PUTPIX(ix, 0); CHKPIX; PUTPIX(0, ix); CHKPIX;
        }
    }
                        ...
                        XtPointer client_data, XtPointer call_data)
{
    ...
    if (bval < 0 || bval == im3d->vinfo->view_type)
    {
        EXRETURN;
    }
    if (im3d->anat_dset[bval] == NULL)
    {
        EXRETURN;
    }
...
/*---------------------------------------------------------------------
 * setup for viewing, given the choices in im3d->vinfo (the indexes of the desired session and
 *datasets, that is)
 * -----------------------------------------------------------------------*/
 ...
 */
    ...
    /*--- if the old dataset has markers and the marker panel is open, shut it down now ---*/
 ...
 */
...

    {
        ...
        /* copy the non-empty labels into the toggle labels, and make the toggle buttons active
         * (panel AND popup) */
    ...
    /* 27 Jan 2004: set coordinate to center of dataset if we were formerly looking at the dummy
     * dataset */
    ...
    if (im3d->vinfo->func_visible)
    {
            ...
            FLDIF(thbot, im3d->fim_thrbot) || FLDIF(thtop, im3d->fim_thrtop))
        {
            ...
            if (ovim != NULL && thim != NULL)
            {
                ...
                if (fac > 0.0f)
                {
                    im3d->fim_thresh_min *= fac; im3d->fim_thresh_max *= fac;
                }
            }
            if (im3d->fim_thresh_min <= im3d->fim_thresh_max)
            {
                char str[256]; int_pair mij = mri_threshold_minmax_indexes();
                sprintf(str, "OLay thresholded range: %f : %f", im3d->fim_thresh_min,
                        im3d->fim_thresh_max);
 ...
 *
 * 02 Nov 1996: set up view specific viewing stuff in im3d, rather than in the datasets.  This is to
 *allow for the possibility that more than one im3d may be looking at the same dataset at once.
 *
...
void AFNI_setup_viewing(Three_D_View *im3d, RwcBoolean rescaled)
{
...

    {
        ...
        /*-- access data in dataset im3d->fim_now IF AND ONLY IF 1) its actual data axes are the
         * same as the wod_daxes 2) it has actual data 3) the user hasn't officially declared for
         * warp-on-demand 4) the dataset doesn't have a warp to bring it into alignment with the
         * current anat dataset (self_warp)    --*/
        ...
        /*- 27 Aug 2002: see if there is a self_warp from fim_now back to anat_now; if so, install
         * it;
         *               this is a coordinate-to-coordinate transformation, and requires
         *warp-on-demand viewing              -*/
        ...
        /* first time in for this controller (or if so ordered), set fim_index and thr_index to
         * reasonable values (IMHO)
         * -- modified 12 Jan 2017 to define 'reasonable' more reasonably */

        { static int first = 1, ffim[MAX_CONTROLLERS]; int qq;
          if (first)
          {
              first = 0; for (qq = 0; qq < MAX_CONTROLLERS; qq++)
              {
                  ffim[qq] = 1;
              }
          }
          ...
          if (ffim[qq] || im3d->vinfo->func_init_subbricks)
          {
              ...
              if (otp.i >= 0)
              {
                  im3d->vinfo->fim_index = otp.i;
              }
              if (otp.j >= 0)
              {
                  im3d->vinfo->thr_index = otp.j;
              }
              ffim[qq] = 0;
          }
        }
    ...
    { RwcBoolean have_fim = ISVALID_3DIM_DATASET(im3d->fim_now);
      ...
      /** Oct 1996: increase decim by 1 to allow for new precision 0..999 of scale (used to be
       * 0..99) **/
      /** Nov 1997: the scale precision is now set by macro THR_top_expon, and its settings are done
       * in routine AFNI_set_thresh_top **/
    ...
    /*--- set the function type bboxes based on the current viewing set up (which may have changed
    ...
    /*--- Sep 1995: turn "Go to atlas position" button on image popup on or off ---*/
    ...
    {
        if (ISREADY_EITHER_ICOR(im3d))
        {
            ENABLE_INSTACORR(im3d);
        }
        else
        {
            DISABLE_INSTACORR(im3d);
        }
    }
     ...
     * fim_now can be NULL if you switch to tlrc view and for some reason, the transform fails on
     *the fim image. We have seen this happen when:
     * 1st volume in list is anat and has a +orig only.
     * 2nd volume in list is anat2 and has a +orig and +tlrc 3rd volume in list is a functional dset
     *(selected as overlay) You switch to anat2 and select TLRC view, BOOM. Our guess is that since
     *the first dset has no tlrc xform, the third one, which is the overlay seems to get no TLRC
     *daddy, even if the second anat has a tlrc xform . Should be able to test this hypothesis by
     *expressly setting the anat parent of all volumes to that of the anat with the TLRC xform....
     *
...
/*-----------------------------------------------------------------------
 * Tell if AFNI_transform_vector can take a vector from old_dset to new_dset coordinates -- 09 Jul
 *2001 -- RWCox.
 * -------------------------------------------------------------------------*/
...
{
    if (old_dset == NULL || new_dset == NULL)
    {
        return 0;
    }
    ...
    if (old_dset->warp_parent == new_dset)
    {
        return 1;
    }
...
THD_fvec3 AFNI_transform_vector(THD_3dim_dataset *old_dset,
                                ...
                                THD_3dim_dataset *new_dset)
{
    if (old_dset == NULL || new_dset == NULL || old_dset == new_dset)
    {
        return old_fv;
    }
    ...
    /*-- 09 Jul 2001:
     *   If old_dset is in +tlrc/+acpc and new_dset is in +orig, see if can find anat_parent to do
     *the job for us --*/
    ...
    { THD_warp *swarp = AFNI_find_warp(new_dset, old_dset);
      if (swarp != NULL)
      {
          return AFNI_forward_warp_vector(swarp, old_fv);
      }
      swarp = AFNI_find_warp(old_dset, new_dset);
      if (swarp != NULL)
      {
          return AFNI_backward_warp_vector(swarp, old_fv);
      }
    }
...
/*------------------------------------------------------------------------
 * 09 May 2001: fix a Solaris stupidity, where the scale is resized improperly when the Define
 *Function panel is opened!
 * --------------------------------------------------------------------------*/
...
void AFNI_define_CB(Widget w, XtPointer client_data, XtPointer call_data)
{
    ...
    if (w == im3d->vwid->view->define_marks_pb)
    {
...

        {
            ...
            /*** I don't know why this is needed, but it prevents the marks panels geometry from
             * getting screwed up, so it's here ***/
...
#define REMANAGE_MARKS    0
        ...
        if (needsX11Redraw())
        {
            forceExpose(im3d->vwid->top_form, 0); FIX_TOPFORM_HEIGHT(im3d);
        }
#else
        FIX_TOPFORM_HEIGHT(im3d);
#endif
                         ...
                         XtPointer client_data, XtPointer call_data)
{
    ...
    /*----- allow transformation if edits are allowed, AND the markers are flagged for it, AND all
...
void AFNI_crosshair_relabel(Three_D_View *im3d)
{
    ...
    xstr = AFNI_crosshair_label(im3d); if (xstr == NULL)
    {
        EXRETURN;
    }
                           ...
                           XtPointer client_data, XtPointer call_data)
{
    ...
    if (w == im3d->vwid->imag->crosshair_dicom_pb)
    {
        val = cord_dicom;
    }
    else if (w == im3d->vwid->imag->crosshair_spm_pb)
    {
        val = cord_spm;
    }
                      ...
                      XtPointer client_data, XtPointer call_data)
{
             ...
             CAN_TALTO(im3d))
    {
        { /* initialize labels */
            ...
            {
                if (AFNI_yesenv("AFNI_DATASET_BROWSE"))
                {
                    MCW_set_browse_select(1);
                }
             ...
             CAN_TALTO(im3d))
    {
...

        {
            ...
            if (tlab != NULL)
            {
                ...
                if (eee != NULL)
                {
                    if (*eee == 'B' || *eee == 'b' || isdigit(*eee))
                    {
                        MCW_textwin_setbig(1);
                    }
                    else if (*eee == 'S' || *eee == 's' || *eee == '-')
                    {
                        MCW_textwin_setbig(-1);
                    }
                }
             ...
             w != NULL && ISREADY_EITHER_ICOR(im3d))
    {
        ...
        if (ii >= 0 && jj >= 0 && kk >= 0)
        {
            ...
            AFNI_jumpto_dicom(im3d, im3d->vinfo->xi_icor, im3d->vinfo->yj_icor,
                              im3d->vinfo->zk_icor);
#endif
        }
    }
...
void AFNI_talto_CB(Widget w, XtPointer cd, MCW_choose_cbs *cbs)
{
    ...
    if (!IM3D_OPEN(im3d) || im3d->type != AFNI_3DDATA_VIEW)
    {
        EXRETURN;
    }
    ...
    nn = cbs->ival;
    if (nn < 0 || nn >= atlas_n_points(Current_Atlas_Default_Name()))
    {
        EXRETURN;
    }
    ...
    /* 09 Jul 2001: if not now viewing in Talairach coordinates, then transform vector to current
     * coordinates */
...
void AFNI_htmlwami_killfun(XtPointer pp)
{
    ...
    /* not there yet MCW_unregister_hint( im3d->vwid->imag->pop_whereami_twin->wtext ) ;
     * MCW_unregister_help( im3d->vwid->imag->pop_whereami_twin->wtext ) ;
     */
...
void AFNI_alter_wami_text(Three_D_View *im3d, char *utlab)
{
    ...
    if (!im3d || !im3d->vwid || !im3d->vwid->imag)
    {
        EXRETURN;
    }
    ...
    {
        if (!im3d->vwid->imag->pop_whereami_twin)
        {
            EXRETURN;
        }
...
char * AFNI_ttatlas_query(Three_D_View *im3d)
{
    ...
    if (!IM3D_OPEN(im3d) || !CAN_TALTO(im3d))
    {
        return NULL;
    }
...
void AFNI_see_ttatlas_CB(Widget w, XtPointer cd, XtPointer cb)
{
    ...
    if (newsee == im3d->vinfo->see_ttatlas)
    {
        return;
    }
...
void AFNI_mnito_CB(Widget w, XtPointer cd, MCW_choose_cbs *cbs)
{
    ...
    if (last_mnito_string != NULL)
    {
        free(last_mnito_string);
    }
    ...
    if (nn != 5)
    {
        BEEPIT; WARNING_message("bad 'MNI To' entries!?"); EXRETURN;
    }
...
/*  tv = THD_mni_to_tta( tv ) ; */ /* Talairach coords */
    ...
    LOAD_FVEC3(tv, xout, yout, zout); /* load vector with new coordinates in dset's std space of
                                       * tlrc view */
...
void AFNI_jumpto_CB(Widget w, XtPointer cd, MCW_choose_cbs *cbs)
{
    ...
    if (last_jumpto_xyz_string != NULL)
    {
        free(last_jumpto_xyz_string);
    }
    ...
    if (nn != 5)
    {
        BEEPIT; WARNING_message("bad Jumpto entries!?"); EXRETURN;
    }
    ...
    if (nn < 0)
    {
        BEEPIT; WARNING_message("Jumpto failed!");
    }
...
int AFNI_jump_and_seed(Three_D_View *im3d, float xx, float yy, float zz)
{
    ...
    if (ii >= 0 && ii < daxes->nxx &&
        jj >= 0 && jj < daxes->nyy && kk >= 0 && kk < daxes->nzz)
    {
        /* Note that the locations of the last click should be set per im3d, perhaps within function
         * AFNI_icor_setref_anatijk(). This current static storage might fail whith multiple
         * controllers.                                               */
...
int AFNI_creepto_dicom(Three_D_View *im3d, float xx, float yy, float zz)
{
    ...
    if (ndd < 2)
    {
        ii = AFNI_jumpto_dicom_OLD(im3d, xx, yy, zz); RETURN(ii);
    }
...
void AFNI_jumpto_thminmax_CB(Widget w, XtPointer cd, XtPointer cb)
{
    ...
    if (ijk == -777 || im3d->fim_thrindex < 0)
    {
        EXRETURN;
    }
    if (ijk < 0)
    {
        BEEPIT; SENSITIZE(w, False); EXRETURN;
    }
...
void AFNI_jumpto_ijk_CB(Widget w, XtPointer cd, MCW_choose_cbs *cbs)
{
    ...
    if (last_jumpto_ijk_string != NULL)
    {
        free(last_jumpto_ijk_string);
    }
...
void AFNI_jumpto_ijk_olay_CB(Widget w, XtPointer cd, MCW_choose_cbs *cbs)
{
    ...
    if (last_jumpto_ijk_olay_string != NULL)
    {
        free(last_jumpto_ijk_olay_string);
    }
...
void AFNI_sumato_CB(Widget w, XtPointer cd, MCW_choose_cbs *cbs)
{
    ...
    if (last_sumato_string != NULL)
    {
        free(last_sumato_string);
    }
    ...
    if (ii < 0)
    {
        BEEPIT; WARNING_message("SUMA To fails -- bad entry?!"); EXRETURN;
    }
                             ...
                             XtPointer client_data, XtPointer call_data)
{
    ...
    markers = im3d->anat_now->markers;
    if (markers == NULL || markers->aflags[1] != MARKACTION_WARP)
    {
        BEEP_AND_RETURN;
    }

    vnew = WARPED_VIEW(im3d->vinfo->view_type); /* view index of new dataset */
    if (!ISVALID_VIEW(vnew))
    {
        BEEP_AND_RETURN;
    }
    ...
    /*----- This new dataset may replace a current dataset, and if so, THAT dataset may have a warp
     * child, and so on.  Mark those datasets for destruction, mark their anatomy children for
     * destruction, and destroy them -----*/
...
/*   GLOBAL_library.sslist->ssar[sss]->dsset_xform_table[aaa][vnew] = new_dset ;*/
    ...
    /* reload active datasets, to allow for destruction that may have occurred (this code is copied
     * from AFNI_initialize_view) */
 ...
 */
     ...
     *    ==> if the input was the original view, then destroy any other marker sets in the original
     *view and make their datasets anatomical children of this one ---*/
...
/*         dss = im3d->ss_now->dsset_xform_table[id][0] ;*/
            ...
            if (!ISVALID_3DIM_DATASET(dss) || dss == im3d->anat_now)
            {
                continue;
            }
            ...
            if (dss->markers != NULL)
            {
                SINGLE_KILL(dss->kl, dss->markers);
            }
            dss->markers = NULL;
    ...
    /*--- now that we have a new potential parent (the warped dataset), and maybe some links to its
     * warp_parent (done just above), try to make some descendants (followup warps) from it
     *     ---*/
    ...
    if (GLOBAL_argopt.auto_purge == True)
    {
        AFNI_purge_unused_dsets();
    }

    /* Jan 31, 1995: force adoption of any dataset that was orphaned at Andersonville */
    ...
    for (sss = 0 ; sss < GLOBAL_library.sslist->num_sess ; sss++)
    {
        ...
        {
            AFNI_force_adoption(ss, GLOBAL_argopt.warp_4D);
        }
    }
 ...
 * Compute the warp.
 *
 * Notice the convention: the warp is always stored as Dicom-to-Dicom coordinates.  For actual use,
 *it must be converted into the correct form for 3dmm-to-3dmm, or 3dfind-to-3dfind, etc.
 *
 * The reason for this convention is that anatomical warps will be applied to functional datasets,
 *which will in general be oriented differently from the anatomical dataset that the warp was
 *computed upon.  By keeping the warp in the generic Dicom system, this application of a warp to
 *another dataset will be possible.
 * ---------------------------------------------------------------------*/
...
THD_warp * AFNI_make_warp(Three_D_View *im3d)
{
    ...
    /*--- make a new warp, and then construct it, based on the type of marker set we have here ---*/
    ...
    case MARKSET_BOUNDING: {
        ...
        /* from these, compute the scalings needed in each direction and the shift needed posterior
         * to the PC */
         ...
         *    posterior to the PC, they also contain shifts to align stuff to the nominal PC
         *location. N.B.: these are maps from AC-PC aligned coordinates to the Talairach system.
         *Maps from the original data to the Talairach system will be computed later (in
         *AFNI_concatenate_warps). */
...
#undef MAKE_MAP
    ...
    case MARKSET_ALIGN: {
        ...
        /*--- at this point, have:
         *       new origin in rr ;
         *       new axes directions in alpha,beta,gamma. Now construct the transformation between
        ...
        /* load bot & top with largest possible excursions from origin (the ALIGNBOX dimensions were
         * added 3/25/95)  */
...
RwcBoolean AFNI_marks_quality_check(RwcBoolean make_report, Three_D_View *im3d)
{
    ...
    case MARKSET_ALIGN: {
        ...
        /*--- now, consider the ray from the AC posterior margin (acpos) in the gamma direction, and
         * the ray from the AC superior edge (acsup) in the beta direction.  Nominally, these rays
         * should intersect.  Find their points of closest approach (rr1,rr2).  The average of these
         * is the Talairach center of coordinates (rr). ------------------------------------*/
        ...
        /*-- Use the trace of the rotation matrix to find the total rotation angle [suggested by M.
         * Klosek] --*/
...
/*------------------------------------------------------------------
 * Create a new dataset that has the geometry specified as the warp from the parent.  The actual
 *data will not be filled in yet.
 * --------------------------------------------------------------------*/
                                  ...
                                  THD_warp *warp_init, float resam_vox)
{
    ...
    /*----- It is possible that this warp is one in a succession of warps.  In that case, the actual
     * transformation is to be done directly from the "adam" dataset, rather than in succession from
     * the parent.  The reason for this is to avoid repeated interpolation.  Thus, we first scan
     * backward along the line of descent, and create the total warp from "adam" to the new dataset
     * -----*/
    ...
    *warp_total = *warp_init;
    ...
    if (warp_total->type < FIRST_WARP_TYPE ||
        warp_total->type > LAST_WARP_TYPE)
    {
        RETURN(NULL);                                         /* error! */
    }
#ifdef AFNI_DEBUG
    ...
    /*----- We now determine the bounding box of the new dataset. This depends on the warp type:
    ...
    /*--- 12 case Talairach mapping (sizes chosen to include borders of Atlas figures) ---*/
...
#undef GETVAL
    ...
    case WARP_AFFINE_TYPE: {
        ...
        /*--- transform each of the 8 corner locations in the adam dataset to the aligned system,
         * and determine the outer limits of the new datablock ---*/
        ...
        /* If the warp had any data in it about the region to map to, apply that data. */
        ...
        {
            /* 3/25/95: use the bot & top INSTEAD of the corners (old version used in ADDITION to
             * corners) */
...
void AFNI_copy_statistics(THD_3dim_dataset *dsold, THD_3dim_dataset *dsnew)
{
    ...
    if (!ISVALID_3DIM_DATASET(dsold) || !ISVALID_3DIM_DATASET(dsnew))
    {
        EXRETURN;
    }
...
/****************************************************************************
* The NAME2* calls enable corresponding AFNI_* environment variables (in addition to .Xdefaults
*values).  Just to make such thing greppable, here is a list of variables, generated by:
*
*    grep NAME2 *.c | grep -e '("' -e '( "' \
*         | awk -F\" '{printf "AFNI_%s\n", $2}' | sort | uniq | column
*
*       AFNI_bigscroll                  AFNI_graph_ort_thick AFNI_crosshairgap AFNI_graph_text_color
*AFNI_fim_ignore                 AFNI_init_fov AFNI_fim_polort AFNI_marksgap AFNI_gamma
*AFNI_markssize AFNI_graph_backg_color AFNI_montage_periodic AFNI_graph_boxes_color AFNI_ncolors
*AFNI_graph_boxes_thick AFNI_ncolovr AFNI_graph_data_color AFNI_ovcrosshair AFNI_graph_data_thick
*AFNI_ovmarksprimary AFNI_graph_dplot_color AFNI_ovmarkssecondary AFNI_graph_dplot_thick
*AFNI_pbar_hide AFNI_graph_ggap AFNI_pbar_pos_pane_count AFNI_graph_grid_color AFNI_pbar_posfunc
*AFNI_graph_grid_thick AFNI_pbar_sgn_pane_count AFNI_graph_gthick               AFNI_purge
*AFNI_graph_ideal_color          AFNI_resam_anat AFNI_graph_ideal_thick          AFNI_resam_func
*AFNI_graph_ignore_color         AFNI_resam_thr AFNI_graph_matrix               AFNI_resam_vox
...
*
*       AFNI_ovdef01                    AFNI_ovlab01 pbar_pos_pane01_ov01 pbar_pos_pane01_thr01
...
void AFNI_load_defaults(Widget w)
{
    ...
    if (cpt != NULL)
    {
        ...
        {
            if (strcmp(cpt, RESAM_shortstr[ii]) == 0)
            {
                break;
            }
        }

        if (ii <= LAST_RESAM_TYPE)
        {
            INIT_resam_anat = ii;
        }
        myXtFree(cpt);
    ...
    for (ii = NPANE_INIT + 1 ; ii <= NPANE_MAX ; ii++)
    {
        ...
        for (jj = 0 ; jj < ii ; jj++)
        {
            pov[jj] = (jj % INIT_ncolovr) + 1;
        }

        for (jj = 0 ; jj <= ii ; jj++)
        {
            INIT_pval_pos[ii][jj] = pthr[jj];
        }
        for (jj = 0 ; jj < ii ; jj++)
        {
            INIT_ovin_pos[ii][jj] = pov[jj];
        }
    }
    ...
    for (ii = NPANE_MIN ; ii <= NPANE_MAX ; ii++)
    {
        ...
        if (pthr[0] != 1.0 || pthr[jj] != 0.0)
        {
            continue;
        }
        for (jj = 1 ; jj <= ii ; jj++)
        {
            if (pthr[jj] == BAD || pthr[jj] >= pthr[jj - 1])
            {
                break;
            }
        }
    ...
    for (ii = NPANE_INIT + 1 ; ii <= NPANE_MAX ; ii++)
    {
        ...
        for (jj = 0 ; jj <= ii ; jj++)
        {
            INIT_pval_sgn[ii][jj] = pthr[jj];
        }
        for (jj = 0 ; jj < ii ; jj++)
        {
            INIT_ovin_sgn[ii][jj] = pov[jj];
        }
    }
    ...
    for (jj = 0 ; jj < ii ; jj++)
    {
        INIT_ovin_pos[ii][jj] = RGBCYC_FIRST + jj + 1;
    }
    for (jj = 0 ; jj < ii ; jj++)
    {
        INIT_ovin_sgn[ii][jj] = RGBCYC_FIRST + jj + 1;
    }
#endif
...
void AFNI_sonnet_CB(Widget w, XtPointer client_data, XtPointer call_data)
{
    ...
    if (NO_frivolities || !IM3D_OPEN(im3d))
    {
        return;
    }
...
/*! Add init function to last registered function. This function should be called just after
 * AFNI_register_nD_function(). [21 Jul 2003] */
...
void AFNI_fix_scale_size_direct(Three_D_View *im3d)
{
    ...
    if (abs(sel_actual - sel_height) < 4)
    {
        return;                                   /* it's OK */
    }
    /* do the work */
...
int AFNI_find_session(char *dname)
{
    ...
    if (ic == GLOBAL_library.sslist->num_sess)
    {
        RETURN(-1);
    }
