/*
 * LiteClue.c - LiteClue widget See LiteClue documentation Version 1.4
 *
 * Copyright 1996 COMPUTER GENERATION, INC.,
 *
 * The software is provided "as is", without warranty of any kind, express or implied, including but
 *not limited to the warranties of merchantability, fitness for a particular purpose and
 *noninfringement. In no event shall Computer Generation, inc. nor the author be liable for any
 ...
 *
 * Author:
 * Gary Aviv Computer Generation, Inc., gary@compgen.com www.compgen.com/widgets
 *
...
/* Revision History:
 * $Log$
 * Revision 1.18  2009/03/05 21:55:24  rickr Cput
 *
 * Revision 1.17  2005/10/18 22:00:53  rwcox Cput
 *
 * Revision 1.16  2003/12/19 22:39:00  rwcox Cput
 *
 * Revision 1.15  2003/12/16 16:13:12  rhammett Cput
 *
 * Revision 1.14  2003/08/05 17:20:46  rwcox Cput
 *
 * Revision 1.13  2000/12/21 16:10:54  cox AFNI
 *
 * Revision 1.3  1997/11/06 16:26:48  cox Fixes so it won't crash so easily
 *
 * Revision 1.2  1997/11/05 21:28:04  cox empty log message ***
 *
 * Revision 1.13  1997/07/07 14:55:04  gary Cancel timeouts when XcgLiteClueDeleteWidget is called
 *to prevent errant timeout event on deleted widget.
 *
 * Revision 1.12  1997/06/20 20:09:09  gary Add XcgLiteClueDispatchEvent to enable clues for
 *insensitive widgets.
 *
 * Revision 1.11  1997/06/15 14:10:24  gary Add XcgLiteClueDispatchEvent to enable clues for
 *insensitive widgets.
 *
 * Revision 1.10  1997/04/14 13:02:33  gary Attempt to fix problem when we get multiple enter events
 *bu no leave event.
 *
 * Revision 1.9  1997/03/10 14:42:41  gary Attempt to fix problem when we get multiple enter events
 *bu no leave event. Add C++ wrapper to allow linking with C++ programs. (In HView.h)
 *
 * Revision 1.8  1997/01/17 13:44:14  gary Support of cancelWaitPeriod resource: this is a period
 *from the point a help popdown occurs in which the normal waitPeriod is suspended for the next
 *popup
 *
 * Revision 1.7  1996/12/16 22:35:38  gary Fix double entry problem
 *
 * Revision 1.6  1996/11/18 14:52:21  gary remove some compile warnings pointed out by a user
 *
 * Revision 1.5  1996/11/12 20:56:43  gary remove some compile warnings
 *
 * Revision 1.4  1996/10/20  13:38:16  gary Version 1.2 freeze
 *
 * Revision 1.3  1996/10/19 16:16:30  gary Compile warning removed with cast
 *
 * Revision 1.2  1996/10/19 16:07:38  gary a) R4 back compatibility b) Delay before pop up of help,
 *waitPeriod resource (def 500 ms). Thanks to J Satchell for this.
 * c) Button press in watched widget pops down help
 *
 * Revision 1.1  1996/10/18 23:14:58  gary Initial
 *
 *
 * $log Cancel timeouts when XcgLiteClueDeleteWidget is called to prevent errant timeout event on
 *deleted widget.
 * $log
 */

/******************************************************************
 * Modified by RWCox of MCW, to draw a rectangle around the text, and to make the widget popup on
 *screen if it would go off. These changes can be found by searching for the string "RWC". These
 *modifications are released to the public domain.
 *******************************************************************/
...
static Boolean setValues(Widget _current, Widget _request, Widget _new, ArgList args,
                         Cardinal *num_args);
...
void RWC_fixup_fontset(Widget cww, Widget www) /* 08 Jan 2021 -- does not work */
{
    ...
    if (cw == NULL || cw->liteClue.fontset != NULL)
    {
        return;
    }
    ...
    if (www != (Widget)NULL)
    {
        ...
        if (xfset != (XFontSet)NULL)
        {
            cw->liteClue.fontset = xfset;
        }
    }
...
static void compute_font_info(XcgLiteClueWidget cw)
{
    ...
    cw->liteClue.font_baseline = -logical.y;    /* y offset from top to baseline, don't know why
                                                 * this is returned as negative */
...
static void compute_font_info(XcgLiteClueWidget cw)
{
    ...
    cw->liteClue.font_baseline = oret.ascent; /* y offset from top to baseline, don't know why this
...
/* a routine to halt execution and force a core dump for debugging analysis when a public routine is
 * called with the wrong class of widget
 */
...
/*
 * Find the target in the widget list. Return context pointer if found, NULL if not
 */
                         ...
                         Cardinal *num_args)
{
    ...
    /* values of cw_new->liteClue.cancelWaitPeriod and cw_new->liteClue.waitPeriod are accepted
     * without checking */
...
#define BorderPix    3
    ...
    if (cw->liteClue.fontset == NULL)
    {
        return;                                 /* If fontset not found [RWC - 08 Jan 2021] */
    }
...
/** RWCox change to widget location **/
    { int scw = WidthOfScreen(XtScreen(cw)), sch = HeightOfScreen(XtScreen(cw));
      ...
      if (newx + logical.width > scw)
      {
          newx = scw - logical.width - 2 * BorderPix;
      }
      if (newx < 0)
      {
          newx = 0;
...
/*
 * Pointer enters watched widget, set a timer at which time it will popup the help
 */
                        ...
                        Boolean *continue_to_dispatch)
{
    ...
    /* check for two enters in a row - happens when widget is exposed under a pop-up */
    if (cw->liteClue.interval_id != (XtIntervalId)0)
    {
        return;
    }
    ...
    /* if a help was recently popped down, don't delay in popping up help for next watched widget
     */
 ...
 *
 * Func:	A widget will be added to the LiteClue watched list. Clues are given for sensitive
 *watched widgets when the pointer enters its window. If the widget is already watched, the passed
 *text replaces its current clue text. If text is null, the widget is still added, if it is not
 *already in the list, but no clue will appear. Text may be specified with XcgLiteClueAddWidget in a
 *subsequent call. When text is null and the widget is already in the list, its text is not changed.
 *When a widget will is added to the watched list, it automatically becomes sensitive. Otherwise,
 *its sensitivity is not changed. A watched widget which is not sensitive retains its context but
 *clues are suppressed. None of this affects the behaviour of the watched widget itself. LiteClue
 *monitors enter and leave events of the watched widget's window passively.
 *
 * Input:	w - LiteClue widget watch - the widget to give liteClues for text - pointer to liteClue
 *text. (May be NULL) size - size of text. May be zero in which case a strlen will be done.
 *  option - option mask, future use, zero for now. Output:
 *
 * Return:
 *
 * ;-
 ...
 *
 * Func:	A widget is deleted from the watched list and its resources are freed. LiteClue is no
 *longer given for the widget. If the widget is not watched, nothing is done.
 *
 * Input:	w - LiteClue widget watch - the widget to delete Output:
 *
 * Return:
 *
 * ;-
 ...
 *
 * Func:	When a watched widget is sensitive, a clue is poped up when the pointer enters its
 *window. When a watched widget is insensitive, the widget is retained in the watched list but no
 *clue is poped. The sensitivity of a watched widget relative to clues is set or reset by this
 *function. The Xt sensitivity of the watched widget is not altered by this function.
 *
 * Input:	w - LiteClue widget watch - the widget to make sensitive or insensitive or NULL to
 *change all watched widgets sensitive - True or False Output:
 *
 * Return:
 *
 * ;-
 ...
 * Func:	When a watched widget is sensitive, a clue is poped up when the pointer enters its
 *window. When a watched widget is insensitive, the widget is retained in the watched list but no
 *clue is poped. The sensitivity state of a watched widget relative to clues is returned by this
 *function. The Xt sensitivity of a widget is a totally independent concept.
 *
 * Input:	w - LiteClue widget watch - the widget for which to get sensitivity state. If NULL first
 *watched widget is used. If there are no watched widgets, False is returned. Output:
 *
 ...
 *
 * Func:	This function may be used to enable clues for insensitive watched widgets. Normally,
 *XtAppMainLoop (which calls XtDispatchEvent) will not deliver EnterNotify and LeaveNotify events to
 *widgets that are not sensitive (XtSetSensitive). This prevents clues from popping up for these
 *widgets. To bypass this limitation, you can break out XtAppMainLoop and add a call to
 ...
 *  }
 *
 * Input:	w - LiteClue widget event - received event, normally from call to XtAppNextEvent.
 *
 * Output: void
 *
 * Return:	True - event was dispatched to non-sensitive watched widget. False - not a EnterNotify
 *or LeaveNotify event or window in event is not a non-sensitive watched widget.
 *
 * ;-
 */
