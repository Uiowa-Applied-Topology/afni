...
/*********************************************************************
 * These are the template routines to form functional images recursively from various data types.
 *
 ...
 *
 *    cc -c -DDTYPE=short afni_pcor.c mv -f afni_pcor.o afni_pcor_short.o
 *
 * In the example above, the resulting routine will be named PCOR_update_short and will take as
 *input a "short *"
...
/**----------------------------------------------------------------------
 *  innermost loop expansion is for speedup if nref is small, if enabled UPZZ updates zz for each
 *row element  > This pair is performed for UPCH updates the Cholesky row element > each
 *non-diagonal element UPLL updates the Cholesky diagonal element
 * -------------------------------------------------------------------------**/
...
#  define UPLL(j)    VCH(vc, vox, j) += bq * zz * zz
     ...
     *   The above loop over jj is just unrolled manually (using the UP?? macros) in order to gain
     *speed.
     * ----------------------------------------------------------------***/
