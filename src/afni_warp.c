...
void AFNI_set_ignore_vedit(int ii)
{
    ignore_vedit = ii; return;
}

/*------------------------------------------------------------------------*/

/* return pointer to the 3D image that would be displayed for the ival-th sub-brick of the dataset
 * -- includes the possibility for vedit (on-the-fly volume editing; currently == clusterize-ation).
...
MRI_IMAGE * AFNI_dataset_displayim(THD_3dim_dataset *dset, int ival)
{
    ...
    if (!DSET_INMEMORY(dset))
    {
        return NULL;
    }

    typ = DSET_BRICK_TYPE(dset, ival);
    if (!AFNI_GOOD_DTYPE(typ))
    {
        return NULL;
    }
...
/*------------------------------------------------------------------------
 * Return a slice from a dataset, possibly to be warped on-the-fly from its parent:
 *   dset        = input dataset fixed_axis  = 1, 2, or 3 (for x,y,z) fixed_index = value of frozen
 *index ival        = index of value (or sub-brick) to extract resam_mode  = used if
 * "warp-on-the-fly" is used
 *
 * Output is in the MRI_IMAGE format, and will be nyy * nzz for fixed_axis = 1 nzz * nxx for
 *fixed_axis = 2 nxx * nyy for fixed_axis = 3
 * --------------------------------------------------------------------------*/
                               ...
                               int ival, int resam_mode)
{
    ...
    if (ival == DSET_THRESH_VALUE(dset))
    {
        resam_mode = RESAM_NN_TYPE;
    }
#endif
    ...
    if (0 && do_vedit)           /* 23 Aug 2012 - volume edit ==> must self-warp */
    {
        ...
        if (dset->vox_warp != NULL)
        {
            dset->vox_warp->type = ILLEGAL_TYPE;
        }
    }
    ...
    if (parent_dset == dset && do_vedit)
    {
        ...
        if (bar == NULL)
        {
            ERROR_message("vedim array is NULL!?");
        }
    }
 ...
 * 25 Jul 2001 -- moved the flipping stuff to new function AFNI_slice_flip() -- RWCox This function
 *is how AFNI gets a slice for **
...
/*-----------------------------------------------------------------
 * Get the #kslice-th slice from sub-brick #ival with resampling mode resam, and flip it so that it
 *is oriented along directions (ax_1,ax_2,ax_3) -- 25 Jul 2001 - RWCox
 * -------------------------------------------------------------------*/
                            ...
                            THD_3dim_dataset *dset)
{
    ...
    if (dset == NULL || kslice < 0)
    {
        RETURN(NULL);                               /* should not happen! */
    }
    daxes = CURRENT_DAXES(dset);

    /* determine which axis kslice refers to, which determines fixed_index, and also compute dsl_1
     * and dsl_2 = axis codes for image returned from AFNI_dataset_slice() */
    ...
    /*--- the image may need to be flippo-ed to be in the orientation that the FD_brick wants ---*/
