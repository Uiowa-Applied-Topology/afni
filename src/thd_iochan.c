...
/*****************************************************************
 * Routines to manipulate IOCHANs, something RWCox invented as an abstraction of socket or shmem
 *inter-process communication.
 *
 * The function iochan_init creates an IOCHAN, and returns a pointer to it. The function
 *iochan_goodcheck() checks if an IOCHAN is ready (connected at both ends); if not, it will wait
 *until the connection is made, or until a given amount of time has passed. The functions
 *iochan_readcheck() and iochan_writecheck() will check if an IOCHAN is good for I/O, and is ready
 *to accept reads and writes, respectively. The functions iochan_send() and iochan_recv() will
 *transmit and receive messages, respectively.  There are also the routines iochan_sendall() and
 *iochan_recvall() that will block until the specified number of bytes is transmitted or received,
 *respectively. The function iochan_ctl() allows control of internal IOCHAN parameters.
 *
 * Socket and shmem IOCHANs are not fully symmetric:
 * Sockets are bidirectional, but shmem IOCHANs can only communicate in one direction. Sending data
 *to a socket will result in all the data being transmitted, even if the process must block. Sending
 *data to a shmem may result in only part of the data being transmitted (if the buffer space
 *available isn't big enough to hold the whole message).
 *
 * Shmem IOCHANs are implemented as a circular buffer with two int indices "bstart" and "bend"; the
 *data between the bstart-th and bend-th bytes of the buffer (inclusive) are ready to be read.
 *Writing to the buffer involves increasing bend, and is done only by the "writing" process. Reading
 *from the buffer involves increasing bstart, and is done only by the
 * "reading" process.  That is, the two processes do not write into the same control variables.
 * (Unless they both try to read AND write into the same IOCHAN.)
 *
 * June 1997:
 * Shmem IOCHANs now can be bidirectional.  This is ordered by using the new "size1+size2"
 * specification.
 ******************************************************************/
 ...
 *
 * Return values are
 *   -1 = some error occurred (socket closed at other end?) 0 = socket is not ready to read 1 =
 * socket has data
 * ---------------------------------------------------------------------*/
...
int tcp_readcheck(int sd, int msec)
{
    ...
    if (sd < 0)
    {
        STATUS("tcp_readcheck: illegal sd"); return -1;
    }                                           /* bad socket id */
    ...
    if (ii == -1)
    {
        PERROR("Socket gone bad? tcp_readcheck[select]");
    }
    return ii;
}
...
int tcp_writecheck(int sd, int msec)
{
    ...
    if (sd < 0)
    {
        STATUS("tcp_writecheck: illegal sd"); return -1;
    }                                           /* bad socket id */
    ...
    if (ii == -1)
    {
        PERROR("Socket gone bad? tcp_writecheck[select]");
    }
    return ii;
}

/*------------------------------------------------------------------------
 * Version of recv() that will try again if interrupted by a signal before any data arrives.
 * --------------------------------------------------------------------------*/
#ifdef USE_TCP_RECV
...
/*------------------------------------------------------------------------
 * Check if an already active socket is still alive. If it is dead, then readcheck will say we can
 *read, but we won't actually get any bytes when we try (using peek mode). Returns 1 if things are
 *OK, 0 if not.
 * --------------------------------------------------------------------------*/
...
int tcp_alivecheck(int sd)
{
    ...
    if (errno)
    {
        PERROR("Socket gone bad? tcp_alivecheck[tcp_recv]");
    }
    return 0;                                 /* no data ==> death!   */
}

/*------------------------------------------------------------------------
 * Open a socket to the given host, to the given TCP port. Returns socket id; if -1, some error
 *occurred (e.g., nobody listening).
 * --------------------------------------------------------------------------*/
...
int tcp_connect(char *host, int port)
{
    ...
    if (host == NULL || port < 1)
    {
        STATUS("tcp_connect: illegal inputs"); return -1;
    }
    ...
    if (sd == -1)
    {
        PERROR("Can't create? tcp_connect[socket]"); return -1;
    }
    ...
    /*@@@BUG: sin.sin_addr.s_addr = ((struct in_addr *)(hostp->h_addr))->s_addr; */
...
/*--------------------------------------------------------------------------
 * Set up to listen for a connection on a given port.  This does not actually form the connection.
 *That must be done separately. Whether someone is trying to connect can be checked for with the
 *routine
...
int tcp_listen(int port)
{
    ...
    if (port < 1)
    {
        STATUS("tcp_listen: illegal port"); return -1;
    }
...
/*--------------------------------------------------------------------------
 * Accept incoming connection on a socket.  Return value is the attached socket (which is not the
 *original socket!).  If -1 is returned, some error occurred.  If the accept works, then the
 ...
 *
 * Both the char * pointers returned are from malloc, and should be free-d when no longer needed. If
 *they aren't needed at all, just pass in NULL for these arguments.
 *
...
int tcp_accept(int sd, char **hostname, char **hostaddr)
{
    ...
    if (sd_new == -1)
    {
        PERROR("Can't accept? tcp_accept[accept]"); return -1;
    }
...
int shm_create(char *key_string, int size)
{
    ...
    if (shmid < 0)
    {
        ...
        if (pron)
        {
            fprintf(stderr, "key_string=%s key=%d size=%d\n",
...
int shm_size(int shmid)
{
    ...
    if (ii < 0)
    {
        PERROR("Can't check? shm_size[shmctl]");  return -1;
    }
    return buf.shm_segsz;
}

/*----------------------------------------------------------------
 * Find the number of attaches to a shmem segment. Returns -1 if an error occurs (like the segment
 *was destroyed).
 * ------------------------------------------------------------------*/
...
int shm_nattach(int shmid)
{
    ...
    if (shmid < 0)
    {
        STATUS("shm_nattach: illegal shmid"); return -1;
    }
    errno = 0;
    ii    = shmctl(shmid, IPC_STAT, &buf);
    if (ii < 0)
...
/****************************************************************
 * The IOCHAN routines, which call the tcp_ or shm_ routines, as needed.
 *****************************************************************/

/*---------------------------------------------------------------
 * Create an IOCHAN struct, and return a pointer to it.  NULL is returned if an error occurs.
 *
 * name = "tcp:host:port" to connect a socket to system "host"
 *           on the given port.
 *     = "shm:name:size" to connect a shared memory segment with the given name and size in bytes
 *(size can end with K or M to denote kilobytes or megabytes). With this mode, the creator and
 *acceptor processes cannot both read and write -- one of them must be the writing process (normally
 *the creator), and one must be the reading process (normally the acceptor).
 *     = "shm:name:size1+size2" to connect a shared memory segment with buffers of length size1 and
 *size2. The creator process will write to the size1 buffer and read from the size2 buffer.  The
 *acceptor process will reverse this.
 *
 * mode = "create" to open a new channel tcp: host must be specified shm: size must be > 0 (if in
 *the form size1+size2, both must be > 0)
 *     = "accept" to log into a channel created by someone else tcp: host is ignored (but must be
 *present) shm: size is ignored (but must be present) (similarly, size1+size2 must be present, but
 *their actual values are ignored)
 *
 * The inputs "host" (for tcp:) and "name" (for shm:) are limited to a maximum of 127 bytes.
 *
 * After an tcp: accept IOCHAN is good, then the string ioc->name contains the IP address of the
 *connecting host, in "dot" form (e.g., "201.201.201.201"); here, "ioc" is the IOCHAN * returned by
 *this routine.
 * -----------------------------------------------------------------*/
...
/*-------------------------------------------------------------------------
 * Check if the shmem segment is alive (has 2 attached processes). Returns 0 if not alive, 1 if life
 *is happy.
 * ---------------------------------------------------------------------------*/
...
/*-------------------------------------------------------------------------
 * Check if the given IOCHAN is ready for I/O.  If not, wait up to msec milliseconds to establish
...
int iochan_goodcheck(IOCHAN *ioc, int msec)
{
    ...
    else if (ioc->bad == SHM_WAIT_ACCEPT)
    {
        ...
        if (ioc->bad && shm_nattach(ioc->id) > 1)
        {
            ioc->bad = 0;
        }
    }
...
/*-----------------------------------------------------------------------
 * Close an IOCHAN.  Note that this will free what ioc points to. Use the IOCHAN_CLOSE macro to also
 *set the pointer "ioc" to NULL.
 * -------------------------------------------------------------------------*/
...
void iochan_set_cutoff(IOCHAN *ioc)
{
    ...
    if (ioc->type == TCP_IOCHAN && ioc->id >= 0)
    {
        tcp_set_cutoff(ioc->id);
    }
    return;
}

/*---------------------------------------------------------------------------
 * Check if the IOCHAN is ready to have data read out of it. If not, the routine will wait up to
 *msec milliseconds for data to be available.  If msec < 0, this routine will wait indefinitely. For
 *sockets, the return value is 1 if data is ready, 0 if not (for sockets, no indication of how much
 *can be read is available). For shmem segments, the return value is how many bytes can be read (0
...
int iochan_readcheck(IOCHAN *ioc, int msec)
{
    ...
    if (ii == 0)                          /* not good yet */
    {
        ...
        if (ii != 1)
        {
            return 0;                    /* if still not good, exit */
        }
    }
    ...
    if (ioc->type == TCP_IOCHAN)
    {
        ii = tcp_alivecheck(ioc->id); if (!ii)
        {
            return -1;
        }
        ...
        if (ii < 0)
        {
            error_string = "iochan_readcheck: socket is bad";
        }
        return ii;
    }
    ...
    if (ioc->type == SHM_IOCHAN)
    {
        ...
        /** Compute the number of readable bytes into nread.  This routine should be called by the
         * "reading" process.  It will then be waiting until the "writing" process increments
         *ioc->bend.   **/
        ...
        for (ms = 0 ; ms < msec ; ms += dms)
        {
            ...
            dms = NEXTDMS(dms); dms = MIN(dms, msec - ms); iochan_sleep(dms);
            ii  = iochan_goodcheck(ioc, 0); if (ii == -1)
            {
                return -1;
            }
        }
...
/*---------------------------------------------------------------------------
 * Read and discard data from the IOCHAN until it is clear. Returns -1 if an error occurs, otherwise
 *returns the number of bytes discarded (which may be 0).
 * -----------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------
 * Check if the IOCHAN is clear -- that is, if the data sent to it has already been read.  This is
 *logically impossible for a TCP channel, so will normally only be used for shmem segments.  It
 *would be called by the writer process to see if the reader process has cleared the data all out.
...
int iochan_clearcheck(IOCHAN *ioc, int msec)
{
    ...
    if (ii == 0)
    {
        return 1;                        /* not good yet, so can be no data */
    }
    /** tcp: ==> use the Unix "select" mechanism **/

    if (ioc->type == TCP_IOCHAN)
    {
        return tcp_readcheck(ioc->id, msec) == 0;
    }
...
/*---------------------------------------------------------------------------
 * Check if the IOCHAN is ready to have data written into it. If not, the routine will wait up to
 *msec milliseconds for writing to be allowable.  If msec < 0, this routine will wait indefinitely.
 *For sockets, the return value is 1 if data can be sent, 0 if not (for sockets, no indication is
 *given of how much data can be sent). For shmem segments, the return value is the number of bytes
 *that can be sent (0 if none, positive if some).
 * -1 will be returned if some unrecoverable error is detected.
 * -----------------------------------------------------------------------------*/
...
int iochan_writecheck(IOCHAN *ioc, int msec)
{
    ...
    if (ioc->type == TCP_IOCHAN)
    {
        ...
        if (ii == -1)
        {
            error_string = "iochan_writecheck: socket not ready";
        }
        return ii;
    }
    ...
    if (ioc->type == SHM_IOCHAN)
    {
        ...
        /** This routine is called by the "writing" process.  It will wait until the reading process
         * increments ioc->bstart.    **/
...
/*----------------------------------------------------------------------------
 * Send (at most) nbytes of data from buffer down the IOCHAN.  Return value is the number of bytes
 *actually sent, or is -1 if some error occurs.
 * (Zero bytes might be sent under some circumstances.)
 *
 * Tcp: IOCHANs use blocking sends, so that all the data should be sent properly unless the
 *connection to the other end fails for some reason.
 *
 * Shm: IOCHANs may not be able to send all the data, if the buffer is nearly full.  This function
 *will send what it can, and return immediately (will not wait for space to clear up).  If no space
 *is available in the buffer, nothing will be sent (0 will be returned).
 * ------------------------------------------------------------------------------*/
...
int iochan_send(IOCHAN *ioc, char *buffer, int nbytes)
{
    ...
    if (ioc->type == TCP_IOCHAN)
    {
        ...
        if (ioc->sendsize <= 0 || nbytes <= ioc->sendsize)
        {
            ...
            if (nsent == -1)
            {
                PERROR("Can't use socket? tcp[send]");
            }
            if (nsent < 0)
            {
        ...
        else
        {
            ...
            do{
                while (tcp_writecheck(ioc->id, 1) == 0)
                {
                    ;                                      /* spin */
                }
...
/*----------------------------------------------------------------------------
 * Send (exactly) nbytes of data from the buffer down the IOCHAN.  The only difference between this
 *and iochan_send is that this function will not return until all the data is sent, even if it takes
...
/*----------------------------------------------------------------------------
 * Read up to nbytes of data from the IOCHAN, into buffer.  Returns the number of bytes actually
 *read.  For both the case of sockets and shmem segments, this may be less than nbytes (may even be
...
/*----------------------------------------------------------------------------
 * Read as much data as possible from the iochan, looping until nothing is left -- 22 May 2001 --
 * RWCox.
 * ------------------------------------------------------------------------------*/
...
int iochan_recvloop(IOCHAN *ioc, char *buffer, int nbytes)
{
    ...
    while (1)
    {
        ...
        if (jj < 1)
        {
            break;          /* stop if nothing more comes in */
        }
        nbuf += jj;
        if (nbuf >= nbytes)
        {
...
/*----------------------------------------------------------------------------
 * Receive (exactly) nbytes of data from the buffer down the IOCHAN.  The only difference between
 *this and iochan_recv is that this function will not return until all the data is received, even if
 *it takes forever. Under these circumstances, it would be good if the writer process is still
 *working.
 * ------------------------------------------------------------------------------*/
...
int iochan_recvall(IOCHAN *ioc, char *buffer, int nbytes)
{
    ...
    while (1)
    {
        ...
        /* Only slow down if we are receiving little bits, otherwise any large buffer that is broken
         * into small pieces could take a very long time, even such as a single anatomical volume.
         *         17 Jul 2015 [rickr]   */
...
/*-----------------------------------------------------------------
 * Send some control command to the IOCHAN.  At present the only command available is to limit the
 *size of TCP/IP sends.
 * ------------------------------------------------------------------*/
 ...
 *            - should be opened with "create" in the caller before this call, and then the child
 *process will open with
 *              "accept".
 * name_out = name of output IOCHAN; will be written to only
 *            - will be opened with "create" in the child and then child will wait for someone to
 * "accept"
 * Return value is the pid of the forked process; (pid_t)-1 if something bad happened at startup. If
 *something bad happens later, the child will _exit(1), and it will also close both IOCHANs it
 *opened.  You can detect the child exit with waitpid(), and can detect the closing of the name_in
 *IOCHAN using iochan_writecheck(), as in this fragment:
 *
 *   pid_t qpid , ppid ;
 *   IOCHAN *ioc ;
 *   char * data ;
 ...
 *   }
 *
 * As in the example, the usual way to use this would have name_in be a shm IOCHAN, and name_out be
 *a tcp IOCHAN; this would then let a main program relay socket data through a separate process, so
 *if the socket freezes up, then the main program can merrily continue (by using iochan_writecheck
 *to see if the shm IOCHAN is available for output).
 *
 * -- 23 May 2001 -- RWCox
 * -------------------------------------------------------------------*/
...
static void iochan_fork_sigfunc(int sig)  /* for when the child dies */
{
    ...
    case SIGTERM:
        if (ioc_kill_1 != NULL)
        {
            iochan_close(ioc_kill_1);
        }
        if (ioc_kill_2 != NULL)
        {
            iochan_close(ioc_kill_2);
        }
...
#define MBUF    1048576        /* 1 Megabyte */
    ...
    if (ppid != 0)       /* the parent process */
    {
        ...
        if (qpid == ppid)
        {
            ppid = (pid_t)(-1);                 /* if it did, return error */
        }
        return ppid;
    }
...

    {
        ...
        if (jj < 0)                                   /* bad news?  */
        {
            if (errno)
            {
                perror("forked readcheck");
            }
            else
            {
                fprintf(stderr, "forked readcheck abort: jj=%d!\n", jj);
            }
        ...
        if (kk <= 0)
        {
            if (errno)
            {
                perror("forked writecheck");
            }
            else
            {
                fprintf(stderr, "forked writecheck abort: kk=%d!\n", kk);
            }
            sss = iochan_error_string();
            if (sss != NULL)
            {
        ...
        if (kk < 0)                                   /* bad news?  */
        {
            if (errno)
            {
                perror("forked sendall");
            }
            else
            {
                fprintf(stderr, "forked sendall abort: kk=%d!\n", kk);
            }
            sss = iochan_error_string();
            if (sss != NULL)
            {
...
double COX_clock_time(void) /* in seconds */
{
    ...
    gettimeofday(&new_tval, NULL);
    /*@@@BUG: gettimeofday(&new_tval, &tzone); */
