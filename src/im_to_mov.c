...
int suck_file(char *fname, char **fbuf)
{
    ...
    if (fname == NULL || fname[0] == '\0' || fbuf == NULL)
    {
        return 0;
    }

    len = THD_filesize(fname); if (len <= 0)
    {
        return 0;
    }

    buf = (char *)malloc(sizeof(char) * (len + 4)); if (buf == NULL)
    {
        return 0;
    }
    ...
    ii = read(fd, buf, len); close(fd); if (ii <= 0)
    {
        free(buf); return 0;
    }
    *fbuf = buf; return ii;
...
void file_copy(char *fname, int ncopy, char **fcopy)
{
    ...
    if (fname == NULL || ncopy < 1 || fcopy == NULL)
    {
        return;
    }
    ...
    {
        if (fcopy[ii] == NULL || fcopy[ii][0] == '\0')
        {
            continue;
        }
        fp = fopen(fcopy[ii], "w"); if (fp == NULL)
        {
            continue;
        }
...
MRI_IMAGE * mri_rgb_scaledown_by2(MRI_IMAGE *inim)
{
    ...
    if (inim == NULL || inim->kind != MRI_rgb)
    {
        return NULL;
    }
    ...
    for (jj = 0 ; jj < nyout ; jj++)
    {
        ...
        for (ii = 0 ; ii < nxout ; ii++)
        {
            ...
            OUT_R(ii, jj) = (IN_R(pp, qq) + 2 * IN_R(p1, qq) + 2 * IN_R(p2, qq) + IN_R(p3, qq)
                             + 2 * IN_R(pp, q1) + 4 * IN_R(p1, q1) + 4 * IN_R(p2, q1) + 2 * IN_R(p3,
                                                                                                 q1)
                             + 2 * IN_R(pp, q2) + 4 * IN_R(p1, q2) + 4 * IN_R(p2, q2) + 2 * IN_R(p3,
                                                                                                 q2)
                             + IN_R(pp, q3) + 2 * IN_R(p1, q3) + 2 * IN_R(p2, q3) + IN_R(p3,
                                                                                         q3)) / 36;
            OUT_G(ii, jj) = (IN_G(pp, qq) + 2 * IN_G(p1, qq) + 2 * IN_G(p2, qq) + IN_G(p3, qq)
                             + 2 * IN_G(pp, q1) + 4 * IN_G(p1, q1) + 4 * IN_G(p2, q1) + 2 * IN_G(p3,
                                                                                                 q1)
                             + 2 * IN_G(pp, q2) + 4 * IN_G(p1, q2) + 4 * IN_G(p2, q2) + 2 * IN_G(p3,
                                                                                                 q2)
                             + IN_G(pp, q3) + 2 * IN_G(p1, q3) + 2 * IN_G(p2, q3) + IN_G(p3,
                                                                                         q3)) / 36;
            OUT_B(ii, jj) = (IN_B(pp, qq) + 2 * IN_B(p1, qq) + 2 * IN_B(p2, qq) + IN_B(p3, qq)
                             + 2 * IN_B(pp, q1) + 4 * IN_B(p1, q1) + 4 * IN_B(p2, q1) + 2 * IN_B(p3,
                                                                                                 q1)
                             + 2 * IN_B(pp, q2) + 4 * IN_B(p1, q2) + 4 * IN_B(p2, q2) + 2 * IN_B(p3,
                                                                                                 q2)
                             + IN_B(pp, q3) + 2 * IN_B(p1, q3) + 2 * IN_B(p2, q3) + IN_B(p3,
                                                                                         q3)) / 36;
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    while (iarg < argc && argv[iarg][0] == '-')
    {
        ...
        if (strcasecmp(argv[iarg], "-npure") == 0)
        {
            if (++iarg >= argc)
            {
                ERROR_exit("Need arg after -npure");
            }
            if (strstr(argv[iarg], "1D") != NULL)
            {
                ...
                if (npurim == NULL)
                {
                    ERROR_exit("Cannot read -npure file %s", argv[iarg]);
                }
            }
            else
            {
                ...
                if (npure < 1)
                {
                    ERROR_exit("Illegal value after -npure");
                }
            }
            iarg++; continue;
        }
        ...
        if (strcasecmp(argv[iarg], "-nfade") == 0)
        {
            if (++iarg >= argc)
            {
                ERROR_exit("Need arg after -nfade");
            }
            if (strstr(argv[iarg], "1D") != NULL)
            {
                ...
                if (nfadim == NULL)
                {
                    ERROR_exit("Cannot read -nfade file %s", argv[iarg]);
                }
            }
            else
            {
                ...
                if (nfade < 0)
                {
                    ERROR_exit("Illegal value after -nfade");
                }
            }
            iarg++; continue;
        }
        ...
        if (strcasecmp(argv[iarg], "-down") == 0)
        {
            if (++iarg >= argc)
            {
                ERROR_exit("Need arg after -down");
            }
            down = (int)strtod(argv[iarg], NULL);
            if (down < 1 || down > 4)
            {
                ERROR_exit("Illegal value after -down");
            }
            iarg++; continue;
        }
        ...
        if (strcasecmp(argv[iarg], "-prefix") == 0)
        {
            if (++iarg >= argc)
            {
                ERROR_exit("Need arg after -prefix");
            }
            prefix = strdup(argv[iarg]);
            if (!THD_filename_ok(prefix))
            {
                ERROR_exit("Bad name after -prefix");
            }
            iarg++; continue;
        }
    ...
    if (nimag < 2)
    {
        ERROR_exit("Need at least 2 input images!");
    }
    ...
    if (nbad > 0)
    {
        ERROR_exit("Can't continue after such blatant stupidities");
    }
    ...
    if (do_loop)
    {
        im0 = mri_copy(IMARR_SUBIM(in_imar, 0));
    }
...

    {
        ...
        { int   alen; char *alf, *oof, *par, *frate;
          ...
          if (fpar == NULL)
          {
              ERROR_exit("Can't create MPEG parameter file %s", par);
          }
