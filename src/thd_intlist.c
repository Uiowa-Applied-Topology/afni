    ...
    *nret = -1;
    ...
    else
    {
        *nret = nuni;
        ...
        if (itop < ibot)
        {
            for (ii = ibot; ii >= itop; ii = ii + step)
            {
                subv[itmp] = ii; ++itmp;
            }
        }
    ...
    ret[0] = *nret;
    for (ii = 1; ii <= ret[0]; ++ii)
    {
        ret[ii] = subv[(ii - 1) % (nuni)];
    }
...
/*! Get an integer list in the range 0..(nvals-1), from the character string str.  If we call the
 * output pointer fred, then fred[0] = number of integers in the list (> 0), and fred[i] = i-th
 * integer in the list for i=1..fred[0]. If on return, fred == NULL or fred[0] == 0, then something
 * is wrong, and the caller must deal with that.
 *
 ...
 *
 *   See also MCW_get_thd_intlist, or MCW_get_labels_intlist, which allow the use of sub-brick
 *labels for selection.
 *
 *   Do not update this function anymore, update MCW_get_labels_intlist instead. ZSS Dec 09
 * -------------------------------------------------------------------*/
 ...
 *
 * lbl : Selector string. To specify labels with space characters ' ' in the name, replace space
 *with the '_' character. On the first pass, the function will match the string as is, but if
 *nothing is found, a second pass is attempted, replacing ' ' in DSET_LABEL with
 *       '_' and matching again.
 *       lbl should match one and only one label.
 * dset: dataset isb : to be set to the index of the matching sub-brick
 *       -1 if no match returns nused, the number of characters used up in lbl 0 if no labels were
 *found.
 *
 *          ZSS Dec 09
 */
    ...
    *isb = -1;
    ...
    do {
        for (ii = 0; ii < N_labels; ++ii)
        {
             ...
             */
            /* note match and keep looking for longer one (tokens might be easier than strings with
             * unknown terminators)      31 Dec 2012 [rickr] */
...
/*
 * Return the sub-brick indices referenced in str This is a new version of MCW_get_intlist, which
 *allows the use of sub-brick labels
 *
...
{
    /* test for brick_lab is not needed, and breaks sub-brick selection of NIfTI datasets 6 Jan 2009
     * [rickr]
     *
     * return( (dset && dset->dblk && dset->dblk->brick_lab) ?
     *          MCW_get_labels_intlist (dset->dblk->brick_lab, DSET_NVALS(dset), str) : NULL );
     */
...
/*
 * A slightly modified version of MCW_get_intlist, which can scan for label matching instead of just
 *numbers. If labels == NULL, then it functions just like MCW_get_intlist
 *
 *                            ZSS Dec 09
 */
int * MCW_get_labels_intlist(char **labels, int nvals, char *str)
{
    ...
    if (show_labs == -1)
    {
        show_labs = AFNI_yesenv("AFNI_SHOW_LABEL_TO_INDEX");
    }
...
/* ----------------------------------------------------------------------
 * Prepare to apply mastery <> selector to dataset by setting master_bot/top (for <a> or <a..b>) or
...
int thd_get_labeltable_intlist(THD_3dim_dataset *dset, char *str)
{
    ...
    if (!workstr)
    {
        ERROR_message("TGLI: failed to cp <> string"); RETURN(1);
    }
    next = strtok(workstr, ",");
    err  = 0;
