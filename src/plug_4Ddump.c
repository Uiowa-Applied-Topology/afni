...
static fXYZ * extract_xyz(char *fname, int x_col_loc, int y_col_loc, int z_col_loc, int ncols,
                          int *nrows, int *Err);
...
static char * EXTRACT_main(PLUGIN_interface *plint)
{
    ...
    if (nprfx == 1 && (int)strlen(ud->strout) == 0)
    {
        nprfx = 0;
    }
...
{/*extract_xyz*/
    ...
    if (x_col_loc <= 0 || x_col_loc > ncols || y_col_loc <= 0 || y_col_loc > ncols ||
        z_col_loc <= 0 || z_col_loc > ncols)
    {
        *Err = 1;
...
{/*extract_index*/
    ...
    for (i = 0; i < sz; ++i)
    {
        ...
        if (divstuff.rem == temp.rem)
        {
            if (divstuff.rem != 0)
            {
                indxvect[divstuff.quot] = tmp;
            }
            else
            {
                indxvect[divstuff.quot - 1] = tmp;
            }
        }
    }
