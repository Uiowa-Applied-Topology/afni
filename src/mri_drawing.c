...
**
** The character drawing routines are by John Walker Copyright (C) 1994 by John Walker,
**kelvin@fourmilab.ch
** -----------------------------------------------------------------------------*/
...
void mri_draw_force_opaque(int fo)
{
    force_opaque = fo;
}
               ...
               int x0, int y0, int x1, int y1, byte r, byte g, byte b)
{
    ...
    if (ppmd_lineclip)
    {
        ...
        if (cx0 == cx1 && cy0 == cy1)
        {
            ASSPIX(pixels, cx0, cy0, r, g, b); return;
        }
    } /* end of clip */
...
void ppmd_polyspline(byte *pixels, int cols, int rows,
                     int x0, int y0, int nc, int *xc, int *yc, int x1, int y1, byte r, byte g,
                     byte b)
{
 ...
 *
 * The	following  character  definitions are derived from the (public domain) Hershey plotter  font
 *database,  using  the  single-stroke Roman font.
 *
 * Each  character  definition	begins	with 3 bytes which specify the number of X, Y plot pairs
 *which follow, the negative  of  the  skip before  starting  to  draw  the  characters, and the
 *skip after the character.  The first plot pair moves the pen to that location  and subsequent
 *pairs  draw  to	the  location given.  A pair of 192, 0 raises the pen, moves to the location
 *given by the following  pair, and resumes drawing with the pair after that.
 *
 * The  values  in  the  definition  tables are 8-bit two's complement signed numbers.  We  declare
 *the  table  as  "unsigned  char"  and manually  sign-extend  the  values because C compilers
 *differ as to whether the type "char" is signed or unsigned, and  some  compilers don't  accept the
 *qualifier "signed" which we would like to use for these items.  We specify negative numbers as
 *their  unsigned  two's complements  to  avoid  complaints  from compilers which don't like
...
/*! Table used to look up sine of angles from 0 through 90 degrees. The value returned is the sine *
 * 65536.  Symmetry is used to obtain sine and cosine for arbitrary angles using this table. */
 ...
 * angle degrees to the X axis, with letters height pixels  high (descenders will extend below the
 * baseline).  ppmd_line does the actual drawing */
                ...
                while (--cn > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (pen)
                        {
                            ...
                            /* Note that up until this  moment  we've  been working	in  an	arbitrary
                             * model coordinate system with  fixed  size  and  no  rotation. Before
                             * drawing	the  stroke,  transform  to viewing coordinates to	honour	the
