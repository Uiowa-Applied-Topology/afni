...
/* This stuff is to catch and ignore SEGV errors that can happen in Extract_Gradient_Maxima_3D --
 * that is, the function just ends there -- no harm, no foul, no edge enhancement.
 *//*-----------------------------------------------------------------------*/
...
/* Enhance the interior edges of a 3D volume (brain image). Positive return value = number of input
 * voxels altered. Negative return value = error code
 *//*-----------------------------------------------------------------------*/
...
int mri_interior_edgeize(MRI_IMAGE *inim, int erode, float frac)
{
    ...
    if (inim == NULL || inim->kind != MRI_float)
    {
        RETURN(-1);
    }
    ...
    nxi = inim->nx; if (nxi < 32)
    {
        RETURN(-2);
    }
    nyi = inim->ny; if (nyi < 32)
    {
        RETURN(-3);
    }
    nzi = inim->nz; if (nzi < 32)
    {
        RETURN(-4);
    }
    ...
    if (xm > 1 || ym > 1 || zm > 1 || xp < nxi - 2 || yp < nyi - 2 || zp <= nzi - 2)
    {
        ...
        if (nxyzi > (int)(1.5f * nnn))
        {
            cim = mri_zeropad_3D(-xm, xp - (nxi - 1),
                                 -ym, yp - (nyi - 1),
                                 -zm, zp - (nzi - 1), inim);
            ININFO_message("   mri_interior_edgeize: cropping %d..%d %d..%d %d..%d", xm, xp, ym, yp,
                           zm, zp);
        }
    }
    if (cim == NULL)
    ...
    if (cropped)
    {
        ININFO_message("   mri_interior_edgeize: input=%dx%dx%d cropped=%dx%dx%d", nxi, nyi, nzi,
                       nx, ny, nz);
    }
    ...
    indims[1] = ny;
    indims[2] = nz;
    border[0] = nx / 5; if (border[0] < 10)
    {
        border[0] = 10;
    }
    border[1] = ny / 5; if (border[1] < 10)
    {
        border[1] = 10;
    }
    border[2] = nz / 5; if (border[2] < 10)
    {
        border[2] = 10;
    }
     ...
     *         Note that the default signal handler in AFNI or otherwise causes the program to die
     *die die, which I'm trying to avoid */

    old_handler = signal(SIGSEGV, intedge_sig);

    /* Step 1: setjmp() sets up to return back here, and returns 0 when it is called all innocently
     * here */
    ...
    {
        /* Step 2: If setjmp() returns a nonzero value, so control gets to here, setjmp() was
         * actually just being longjmp() leaping back here, which happens if the signal handler was
         * invoked -- if instead we just normally returned from the signal handler,
         * Extract_Gradient_Maxima_3D would take control back and do the seg fault over again, and
         * an infinite loop would take over the galaxy, which is bad for everybody concerned */
