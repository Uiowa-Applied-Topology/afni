 ...
 *
 * File:    3dmerge.c Author:  R. W. Cox
 *
 * Mod:     Changes to implement "-doall" command option. Author:  B. D. Ward Date:    04 February
 *1998
 *
 * Mod:     Changes to implement "-1dindex" and "-1tindex" options. Author:  RWCox Date:    12 Nov
 *1998
 *
 * Mod:     Allow use of THD_open_dataset instead of THD_open_one_dataset. Author:  RWCox Date:
 *    21 Feb 2001
 *
 * Mod:     Check for loaded dataset in merge operations Author:  RWCox Date:    02 Nov 2001
 * -----------------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------
 * read the arguments, load the global MRG_ variables, and return the index of the argument that we
 *stopped at (that is, the first input filename)
 * ----------------------------------------------------------------------*/
    ...
    while (nopt < argc && argv[nopt][0] == '-')
    {
            ...
            strncmp(argv[nopt], "-1filter_blur", 13) == 0)
        {
            ...
            if (doblur)
            {
                if (nopt >= argc)
                {
                    ERROR_exit("Need 1 argument after -1filter_blur");
                }
            }
            else if (nopt + 1 > argc)
            {
...
#define  PREDEFINED_MASK    (MASK('r') | MASK('x') | MASK('y') | MASK('z') \
            ...
            if (nee > 0)
            {
                exit(1);          /* can't use this expression! */
            }
    ...
    if (MRG_rank)
    {
        ...
        if (prefix[0] == '\0')
        {
            strcpy(prefix, MRG_output_prefix);
        }
...
int main(int argc, char *argv[])
{
    ...
    if (MRG_ivfim < 0)
    {
        fprintf(stderr, "++ default -1dindex = 0\n");
    }
    if (MRG_ivthr < 0)
    {
        fprintf(stderr, "++ default -1tindex = 1\n");
    }
    ...
    if (file_count == 1)
    {
...
#if 1
        /** 17 Sep 1998: Move the creation of the new dataset to AFTER the editing operations, so
         * that any dataset parameter changes in EDIT_one_dataset will properly be propagated to the
         * new dataset. The creation shown here is just to check if the output dataset exists
         * already.                   **/
        ...
        if (MRG_keepthr && ISFUNC(new_dset) && FUNC_HAVE_THR(new_dset->func_type))
        {
            ...
            if (MRG_thdatum >= 0)
            {
                output_thdatum = MRG_thdatum;
            }
            else
            {
                output_thdatum = input_thdatum;
            }
        }
        else
        {
...
#   endif
    ...
    {
        output_datum = DSET_BRICK_TYPE(dset, ival);
    }
    ...
    for (iv = iv_bot; iv < iv_top; iv++)
    {
        ...
        if (!MRG_be_quiet)
        {
            printf("-- merging results into sub-brick %d\n", ivout);
        }
        ...
        /*   [at this point, is_int is true if all] inputs were integers and unscaled.  ] 08 Jan
         * 1998 */
        ...
        if ((rmm >= dx || rmm >= dy || rmm >= dz) && ptmin > 1)
        {
            if (!MRG_be_quiet)
            {
                printf("-- editing merger for cluster size\n");
            }
        ...
        if (MRG_cflag_gthr != THFLAG_NONE)
        {
            ...
            /* if all datasets were fico, then output is fico, and needs to get the
             * degrees-of-freedom parameters stataux */
