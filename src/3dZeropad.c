                         ...
                         int *add_I, int *add_S)
{
    ...
    {
        /* otherwise, we want to add the negative mod nvoxels (but -v1 % m is negative, grrrrrr...)
         */
    ...
    /* Check about switching add_L to add_R, etc.  That is, ensure that if padding is done along a
     * particular axis, that it is done on side *away* from the matrix origin, by swapping the add_?
     * value if it is nonzero.
     */
    for (ii = 0 ; ii < 3 ; ii++)
    {
...
int main(int argc, char *argv[])
{
    ...
    while (iarg < argc && argv[iarg][0] == '-')
    {
        ...
        if (strcmp(argv[iarg], "-master") == 0)
        {
            ...
            {
                fprintf(stderr,
                        "** 3dZeropad: Can't use -master with -I,-S,-A,-P,-R,-L, or -mm!\n");
                exit(1);
            }
            if (mset != NULL)
            {
    ...
    if (mset != NULL)
    {
        ...
        {
            fprintf(stderr,
                    "** 3dZeropad: Master and Input datasets don't have same voxel size!\n");
            nerr++;
        }
    ...
    if (add_z != 0)
    {
        ...
        case ORI_R2L_TYPE:
        case ORI_L2R_TYPE:
            if (add_R != 0 && add_R != add_z)
            {
                fprintf(stderr, "++ 3dZeropad: -z overrides -R\n");
            }
            if (add_L != 0 && add_L != add_z)
            {
                fprintf(stderr, "++ 3dZeropad: -z overrides -L\n");
            }
            add_R = add_L = add_z;
            break;
        ...
        case ORI_P2A_TYPE:
        case ORI_A2P_TYPE:
            if (add_P != 0 && add_P != add_z)
            {
                fprintf(stderr, "++ 3dZeropad: -z overrides -P\n");
            }
            if (add_A != 0 && add_A != add_z)
            {
                fprintf(stderr, "++ 3dZeropad: -z overrides -A\n");
            }
            add_P = add_A = add_z;
            break;
        ...
        case ORI_I2S_TYPE:
        case ORI_S2I_TYPE:
            if (add_I != 0 && add_I != add_z)
            {
                fprintf(stderr, "++ 3dZeropad: -z overrides -I\n");
            }
            if (add_I != 0 && add_S != add_z)
            {
                fprintf(stderr, "++ 3dZeropad: -z overrides -S\n");
            }
            add_I = add_S = add_z;
            break;
        }
