 ...
 *assumes that point is above facet (major damage if below) for Delaunay triangulations, Use
 *qh_setdelaunay() to lift point to paraboloid and scale by 'Qbb' if needed Do not use options
 *'Qbk', 'QBk', or 'QbB' since they scale the coordinates.
 *
 * returns:
 ...
 *coplanar point exit exit if pre STOPpoint requested find horizon and visible facets for point make
 *new facets for point to horizon make hyperplanes for point compute balance statistics match
 ...
 *partition qh.GOODvertex sets facet->newfacet for qh_findbestnew() in qh_partitionpoint()
 *
 * notes:
