 ...
 * >-------------------------------</a><a name="TOP">-</a>
 *
 * qhull.c Quickhull algorithm for convex hulls
 *
 ...
 *
 * see:
 *  to add individual points to either qh.num_points use qh_addpoint()
 *
 *  if qh.GETarea qh_produceoutput() returns qh.totarea and qh.totvol via qh_getarea()
 *
 * design:
 ...
 *    if point is not outside of the hull, uses qh_partitioncoplanar() else if facet specified,
 * assumes that point is above facet (major damage if below) for Delaunay triangulations,
 *    Use qh_setdelaunay() to lift point to paraboloid and scale by 'Qbb' if needed Do not use
 * options 'Qbk', 'QBk', or 'QbB' since they scale the coordinates.
 *
 * returns:
 ...
 *
 * design:
 *  check point in qh.first_point/.num_points if checkdist if point not above facet partition
 * coplanar point
 *      exit exit if pre STOPpoint requested find horizon and visible facets for point make new
 ...
 * requested merge non-convex new facets check for using qh_findbestnew() instead of qh_findbest()
 * partition outside points from visible facets delete visible facets check polyhedron if requested
 * exit if post STOPpoint requested reset working lists of facets and vertices
 */
boolT qh_addpoint(pointT *furthest, facetT *facet, boolT checkdist)
{
    ...
    if (qh ONLYgood && !(goodvisible + goodhorizon) && !qh GOODclosest)
    {
        ...
        /* last point of outsideset is no longer furthest.  This is ok since all points of the
         * outside are likely to be bad */
        qh_resetlists(False /*qh visible_list newvertex_list newfacet_list */);
        return True;
    }
    zzinc_(Zprocessed);
 ...
 *
 * qh_buildhull() construct a convex hull by adding outside points one at a time
 *
 * returns:
 *
 * notes:
 ...
 * outsidesets of facets vertices= vertices in qh.facet_list (not partitioned)
 *
 * returns:
 *  builds facet->outsideset does not partition qh.GOODpoint if qh.ONLYgood && !qh.MERGING,
 *    does not partition qh.GOODvertex sets facet->newfacet for qh_findbestnew() in
 * qh_partitionpoint()
 *
 * notes:
 ...
 * from point to facet if dist NULL,
 *    searches for bestfacet and does nothing if inside if qh.findbestnew set,
 ...
 * qh.max_outside else append point one before end of coplanar set else update qh.max_outside
 */
 ...
 *    (either exhaustive search of new facets or directed search from facet) if qh.NARROWhull retain
 * coplanar and nearinside points as outside points if point is outside bestfacet if point above
 * furthest point for bestfacet append point to outside set (it becomes the new furthest) if outside
 * set was empty move bestfacet to end of qh.facet_list (i.e., after qh.facet_next) update
 * bestfacet->furthestdist else append point one before end of outside set else if point is coplanar
 * to bestfacet if keeping coplanar points or need to update qh.max_outside partition coplanar point
 * into bestfacet else if near-inside point
 *    partition as coplanar point into bestfacet else is an inside point if keeping inside points
 ...
 * to a horizon facet or a new facet if allpoints (not used), repartitions coplanar points
 *
 * returns:
 ...
 *
 * qh_precision( reason ) restart on precision errors if not merging and if 'QJn'
 */
 ...
 *
 * qh_printsummary( fp ) prints summary to fp
 *
 * notes:
