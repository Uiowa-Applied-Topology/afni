 ...
 * >-------------------------------</a><a name="TOP">-</a>
 *
 * rboxlib.c Generate input points
 *
 * notes:
 *   For documentation, see prompt[] of rbox.c 50 points generated for 'rbox D4'
 *
 * WARNING:
...
struct rboxT {
    ...
    int     isinteger;
    double  out_offset;
    jmp_buf errexit;      /* exit label for rboxpoints, defined by setjmp(), called by
                           * qh_errexit_rbox() */
};
 ...
 *
 *  Parse arguments into variables Determine the number of points Generate the points
 */
int qh_rboxpoints(FILE *fout, FILE *ferr, char *rbox_command)
{
    ...
    int issphere = 0, isaxis = 0, iscdd = 0, islens = 0, isregular = 0, iswidth = 0,
        addcube = 0;
    ...
    if (rbox_inuse)
    {
        qh_fprintf_rbox(rbox.ferr, 6188,
    ...
    *command = '\0';
    ...
    while (*s)
    {
        ...
        case 'D':
            ...
            if (dim < 1 ||
                dim > MAXdim)
            {
                qh_fprintf_rbox(rbox.ferr, 6189,
                                "rbox error: dimension, D%d, out of bounds (>=%d or <=0)", dim,
                                MAXdim);
    ...
    if (islens)
    {
        ...
        if (radius <= 1.0)
        {
            qh_fprintf_rbox(rbox.ferr, 6191,
    ...
    if ((s = first_point))
    {
        while (s && *s) /* 'P' */
        {
            ...
            while (*++s)
            {
                ...
                if (*s != ',')
                {
                    qh_fprintf_rbox(rbox.ferr, 6194,
            ...
            {
                qh_fprintf_rbox(rbox.ferr, 6195,
    ...
    else if (isregular && !islens)
    {
        if (dim != 2 && dim != 3)
        {
            qh_fprintf_rbox(rbox.ferr, 6197,
    ...
    else if (isregular && islens && dim != 2)
    {
        if (dim != 3)
        {
            qh_fprintf_rbox(rbox.ferr, 6198,
                            "rbox error: regular points can be used only in 2-d and 3-d\n\n");
            qh_errexit_rbox(qh_ERRinput);
        }
        angle     = 0.0;
    ...
    else
    {
        ...
        for (i = 0; i < numpoints; i++)
        {
            ...
            else if (isspiral)
            {
                if (dim != 3)
                {
                    qh_fprintf_rbox(rbox.ferr, 6199,
...
int roundi(double a)
{
    if (a < 0.0)
    {
        if (a - 0.5 < INT_MIN)
        {
            qh_fprintf_rbox(rbox.ferr, 6200,
    ...
    else
    {
        if (a + 0.5 > INT_MAX)
        {
            qh_fprintf_rbox(rbox.ferr, 6201,
...
void out2n(double a, double b)
{
    if (rbox.isinteger)
    {
        qh_fprintf_rbox(rbox.fout, 9405, "%d %d\n", roundi(a + rbox.out_offset),
                        roundi(b + rbox.out_offset));
    }
    else
    {
...
void out3n(double a, double b, double c)
{
    if (rbox.isinteger)
    {
        qh_fprintf_rbox(rbox.fout, 9407, "%d %d %d\n", roundi(a + rbox.out_offset),
                        roundi(b + rbox.out_offset), roundi(c + rbox.out_offset));
    }
    else
    {
        qh_fprintf_rbox(rbox.fout, 9408, qh_REAL_3n, a + rbox.out_offset, b + rbox.out_offset,
                        c + rbox.out_offset);
    }
} /* out3n */
