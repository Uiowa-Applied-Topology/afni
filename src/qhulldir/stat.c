 ...
 *
 * notes:
 *  (otherwise, 'gcc -O2' uses too much memory) uses qhstat.next
 */
 ...
 *
 * qh_nostatistic( index ) true if no statistic to print
 */
 ...
 *
 * qh_printallstatistics( fp, string ) print all statistics with header 'string'
 */
 ...
 *
 * qh_printstatistics( fp, string ) print statistics to a file with header 'string'
 ...
 *
 * qh_printstatlevel( fp, id ) print level information for a statistic
 *
 * notes:
...
#define NULLfield    "       "
    ...
    {
        qh_fprintf(fp, 9365, "%7.3g",
                   (realT)qhstat stats[id].i / qhstat stats[(unsigned char)(qhstat count[id])].i);
    }
 ...
 *
 * qh_printstats( fp, index, nextindex ) print statistics for a zdoc group
 *
 * returns:
 ...
 * >-------------------------------</a><a name="stddev">-</a>
 *
 * qh_stddev( num, tot, tot2, ave ) compute the standard deviation and average from statistics
 *
 *  tot2 is the sum of the squares notes:
 *  computes r.m.s.:
 *    (x-ave)^2
