 ...
 * >-------------------------------</a><a name="TOP">-</a>
 *
 * qset.c implements set manipulations needed for quickhull
 *
...
/*** uncomment here and qhull_a.h if string.h does not define memcpy()
 #include <memory.h>
 */
 ...
 * >--------------------------------<a name="setaddnth">-</a>
 *
 * qh_setaddnth( setp, nth, newelem) adds newelem as n'th element of sorted or unsorted *setp
 *
 * notes:
 * setp and newelem must be defined setp may be a temp set nth=0 is first element errors if nth is
 *out of bounds
 *
 * design:
 *  expand *setp if empty or full move tail of *setp up one insert newelem
 */
 ...
 *
 * setaddsorted( setp, newelem ) adds an newelem into sorted *setp
 *
 * notes:
 * setp and newelem must be defined setp may be a temp set nop if newelem already in set
 *
 * design:
 *  find newelem's position in *setp insert newelem
 */
 ...
 * >-------------------------------<a name="setappend">-</a>
 *
 * qh_setappend( setp, newelem) append newelem to *setp
 *
 * notes:
 * setp may be a temp set setp and newelem may be NULL
 *
 * design:
 *  expand *setp if empty or full append newelem to *setp
 *
 */
 ...
 *
 * qh_setappend_set( setp, setA) appends setA to *setp
 *
 * notes:
 * setp can not be a temp set setp and setA may be NULL
 *
 * design:
 *  setup for copy expand *setp if it is too small append all elements of setA to *setp
 */
 ...
 *
 * qh_setappend2ndlast( setp, newelem ) makes newelem the next to the last element in *setp
 *
 * notes:
 * setp must have at least one element newelem must be defined setp may be a temp set
 *
 * design:
 *  expand *setp if empty or full move last element of *setp up one insert newelem
 */
 ...
 * >-------------------------------<a name="setcheck">-</a>
 *
 * qh_setcheck( set, typename, id ) check set for validity report errors with typename and id
 *
 * design:
 ...
 * >-------------------------------<a name="setcompact">-</a>
 *
 * qh_setcompact( set ) remove internal NULLs from an unsorted set
 *
 * returns:
 *  updated set
 *
 * notes:
 *  set may be NULL it would be faster to swap tail of set into holes, like qh_setdel
 *
 * design:
 *  setup pointers into set skip NULLs while copying elements to start of set update the actual size
 */
 ...
 * >-------------------------------<a name="setcopy">-</a>
 *
 * qh_setcopy( set, extra ) make a copy of a sorted or unsorted set with extra slots
 *
 * returns:
 *  new set
 *
 * design:
 *  create a newset with extra slots copy the elements to the newset
 *
 */
 ...
 * >-------------------------------<a name="setdel">-</a>
 *
 * qh_setdel( set, oldelem ) delete oldelem from an unsorted set
 *
 * returns:
 *  returns oldelem if found returns NULL otherwise
 *
 * notes:
 ...
 *
 * design:
 *  locate oldelem update actual size if it was full move the last element to the oldelem's location
 */
 ...
 * >-------------------------------<a name="setdellast">-</a>
 *
 * qh_setdellast( set) return last element of set or NULL
 *
 * notes:
 *  deletes element from set set may be NULL
 *
 * design:
 ...
 * >-------------------------------<a name="setdelnth">-</a>
 *
 * qh_setdelnth( set, nth ) deletes nth element from unsorted set 0 is first element
 *
 * returns:
 ...
 *
 * design:
 *  setup points and check nth delete nth element and overwrite with last element
 */
 ...
 *
 * qh_setdelnthsorted( set, nth ) deletes nth element from sorted set
 *
 * returns:
 ...
 *
 * design:
 *  setup points and check nth copy remaining elements down one update actual size
 */
 ...
 *
 * qh_setdelsorted( set, oldelem ) deletes oldelem from sorted set
 *
 * returns:
 ...
 *
 * design:
 *  locate oldelem in set copy remaining elements down one update actual size
 */
 ...
 *
 * qh_setduplicate( set, elemsize ) duplicate a set of elemsize elements
 *
 * notes:
 *  use setcopy if retaining old elements
 *
 * design:
 *  create a new set for each elem of the old set create a newelem append newelem to newset
 */
 ...
 * >-------------------------------<a name="setequal">-</a>
 *
 * qh_setequal(  ) returns 1 if two sorted sets are equal, otherwise returns 0
 *
 * notes:
 *  either set may be NULL
 *
 * design:
 *  check size of each set setup pointers compare elements of each set
 */
 ...
 *
 *  if skipelemB is NULL, can skip any one element of setB
 *
 * design:
 *  setup pointers search for skipelemA, skipelemB, and mismatches check results
 */
 ...
 *
 * design:
 *  setup pointers search for mismatches while skipping skipA and skipB
 */
 ...
 * >-------------------------------<a name="setfree">-</a>
 *
 * qh_setfree( setp ) frees the space occupied by a sorted or unsorted set
 *
 * returns:
 *  sets setp to NULL
 *
 ...
 * >-------------------------------<a name="setfree2">-</a>
 *
 * qh_setfree2( setp, elemsize ) frees the space occupied by a set and its elements
 *
 * notes:
 *  set may be NULL
 *
 * design:
 *  free each element free set
 */
 ...
 *
 * qh_setfreelong( setp ) frees a set only if it's in long memory
 *
 * returns:
 ...
 * >-------------------------------<a name="setin">-</a>
 *
 * qh_setin( set, setelem ) returns 1 if setelem is in a set, 0 otherwise
 *
 * notes:
 ...
 * >-------------------------------<a name="setindex">-</a>
 *
 * qh_setindex( set, atelem ) returns the index of atelem in set.
 *  returns -1, if not in set or maxsize wrong
 *
 * notes:
 *  set may be NULL and may contain nulls. NOerrors returned (qh_pointid, QhullPoint::id)
 *
 * design:
 *  checks maxsize scans set for atelem
 */
 ...
 * >-------------------------------<a name="setlarger">-</a>
 *
 * qh_setlarger( oldsetp ) returns a larger set that contains all elements of *oldsetp
 *
 * notes:
 *  the set is at least twice as large if temp set, updates qhmem.tempstack
 *
 * design:
 ...
 * >-------------------------------<a name="setlast">-</a>
 *
 * qh_setlast(  ) return last element of set or NULL (use type conversion)
 *
 * notes:
 *  set may be NULL
 *
 ...
 * >-------------------------------<a name="setnew">-</a>
 *
 * qh_setnew( setsize ) creates and allocates space for a set
 *
 * notes:
 ...
 * >-------------------------------<a name="setprint">-</a>
 *
 * qh_setprint( fp, string, set ) print set elements to fp with identifying string
 *
 * notes:
 *  never errors
 */
 ...
 * >-------------------------------<a name="setreplace">-</a>
 *
 * qh_setreplace( set, oldelem, newelem ) replaces oldelem in set with newelem
 *
 * notes:
 ...
 * >-------------------------------<a name="setsize">-</a>
 *
 * qh_setsize( set ) returns the size of a set
 *
 * notes:
 ...
 * >-------------------------------<a name="settemp">-</a>
 *
 * qh_settemp( setsize ) return a stacked, temporary set of upto setsize elements
 *
 * notes:
 *  use settempfree or settempfree_all to release from qhmem.tempstack see also qh_setnew
 *
 * design:
 *  allocate set append to qhmem.tempstack
 *
 */
 ...
 *
 * qh_settempfree( set ) free temporary set at top of qhmem.tempstack
 *
 * notes:
 *  nop if set is NULL errors if set not from previous   qh_settemp
 *
 * to locate errors:
 *  use 'T2' to find source and then find mis-matching qh_settemp
 *
 * design:
 *  check top of qhmem.tempstack free it
 */
 ...
 *
 * qh_settempfree_all(  ) free all temporary sets in qhmem.tempstack
 *
 * design:
 *  for each set in tempstack free set free qhmem.tempstack
 */
 ...
 * >-------------------------------<a name="settemppop">-</a>
 *
 * qh_settemppop(  ) pop and return temporary set from qhmem.tempstack
 *
 * notes:
 ...
 *
 * qh_settemppush( set ) push temporary set unto qhmem.tempstack (makes it temporary)
 *
 * notes:
 ...
 *
 * qh_settruncate( set, size ) truncate set to size elements
 *
 * notes:
 *  set must be defined
 *
 ...
 *
 * design:
 *  check size update actual size of set
 */
void qh_settruncate(setT *set, int size)
{
    if (size < 0 || size > set->maxsize)
    {
        qh_fprintf(qhmem.ferr, 6181,
 ...
 * >-------------------------------<a name="setunique">-</a>
 *
 * qh_setunique( set, elem ) add elem to unsorted set unless it is already in set
 *
 * notes:
 *  returns 1 if it is appended
 *
 * design:
 *  if elem not in set append elem to set
 */
 ...
 * >-------------------------------<a name="setzero">-</a>
 *
 * qh_setzero( set, index, size ) zero elements from index on set actual size of set to size
 *
 * notes:
 *  set must be defined the set becomes an indexed set (can not use FOREACH...)
 *
 * see also:
 *  qh_settruncate
 *
 * design:
 *  check index and size update actual size zero elements starting at e[index]
 */
