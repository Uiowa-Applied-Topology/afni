 ...
 *
 * qh_copypoints( points, numpoints, dimension) return qh_malloc'd copy of points
 */
 ...
 *
 * qh_crossproduct( dim, vecA, vecB, vecC ) crossproduct of 2 dim vectors C= A x B
 *
 * notes:
 *  from Glasner, Graphics Gems I, p. 639 only defined for dim==3
 */
 ...
 *
 * design:
 *  construct qm_matrix by subtracting apex from points compute determinate
 */
 ...
 *
 * notes:
 *  calculate roundoff error according to Lemma 3.2-1 of Golub and van Loan "Matrix Computation"
 *  use sqrt(dim) since one vector is normalized or use maxsumabs since one vector is < 1
 */
 ...
 * >-------------------------------</a><a name="divzero">-</a>
 *
 * qh_divzero( numer, denom, mindenom1, zerodiv ) divide by a number that's nearly zero mindenom1=
 * minimum denominator for dividing into 1.0
 *
 * returns:
 *  quotient sets zerodiv and returns 0.0 if it would overflow
 *
 * design:
 ...
 *
 * qh_facetarea( facet ) return area for a facet
 *
 * notes:
 ...
 *
 * qh_facetcenter( vertices ) return Voronoi center (Voronoi vertex) for a facet's vertices
 *
 * returns:
 ...
 *
 *  moves good, visible facets (and some other visible facets) to end of qh facet_list
 *
 * notes:
 *  uses qh visit_id
 *
 ...
 *
 * qh_gram_schmidt( dim, row ) implements Gram-Schmidt orthogonalization by rows
 *
 * returns:
 *  false if zero norm overwrites rows[dim][dim]
 *
 * notes:
 *  see Golub & van Loan Algorithm 6.2-2 overflow due to small divisors not handled
 *
 * design:
 ...
 *
 * qh_inthresholds( normal, angle ) return True if normal within qh.lower_/upper_threshold
 *
 * returns:
 *  estimate of angle by summing of threshold diffs angle may be NULL smaller "angle" is better
 *
 * notes:
 *  invalid if qh.SPLITthresholds
 *
 * see:
 *  qh.lower_threshold in qh_initbuild() qh_initthresholds()
 *
 * design:
 *  for each dimension test threshold
 */
...
REALepsilon %g REALmin %g REALmax %g -REALmax %g\n",
    ...
    for (k = 0; k < dimension; k++)
    {
        ...
        /* calculation of qh NEARzero is based on error formula 4.4-13 of Golub & van Loan, authors
         * say n^3 can be ignored and 10 be used in place of rho */
 ...
 *
 * notes:
 *  assumes at least pointsneeded points in points maximizes determinate for x,y,z,w, etc.
 ...
 *
 * qh_minabsval( normal, dim ) return minimum absolute value of a dim vector
 */
 ...
 * >-------------------------------</a><a name="mindiff">-</a>
 *
 * qh_mindif ( vecA, vecB, dim ) return index of min abs. difference of two vectors
 */
 ...
 *
 * qh_orientoutside( facet  ) make facet outside oriented via qh.interior_point
 *
 * returns:
 ...
 *
 * design:
 *  compute scale and shift factors apply to last coordinate of each point
 */
 ...
 *
 * design:
 *  for all facets on qh.newfacet_list if two facets are in different quadrants set issharp
 */
