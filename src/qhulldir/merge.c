 ...
 *
 * design:
 *  for each facet in samecycle for each unseen vertex in facet->vertices append to result
 */
 ...
 *
 * qh_checkconnect() check that new facets are connected new facets are on qh.newfacet_list
 *
 * notes:
 *  this is slow and it changes the order of the facets uses qh.visit_id
 *
 * design:
 ...
 *
 * qh_checkzero( testall ) check that facets are clearly convex for qh.DISTround with qh.MERGEexact
 *
 *  if testall, test all facets for qh.MERGEexact post-merging else test qh.newfacet_list
 *
 *  if qh.MERGEexact, allows coplanar ridges skips convexity test while qh.ZEROall_ok
 *
 * returns:
 ...
 *
 * qh_compareangle( angle1, angle2 ) used by qsort() to order merges by angle
 */
 ...
 *
 * qh_comparemerge( merge1, merge2 ) used by qsort() to order merges
 */
 ...
 *
 * qh_comparevisit( vertex1, vertex2 ) used by qsort() to order vertices by their visitid
 */
 ...
 *
 * qh_copynonconvex( atridge ) set non-convex flag on other ridges (if any) between same neighbors
 *
 * notes:
 *  may be faster if use smaller ridge set
 *
 * design:
 *  for each ridge of atridge's top facet if ridge shares the same neighbor set nonconvex flag
 */
 ...
 *
 * qh_degen_redundant_facet( facet ) check facet for degen. or redundancy
 *
 * notes:
 ...
 *
 * design:
 *  test for redundant neighbor test for degenerate facet
 */
 ...
 *
 * design:
 *  test for degenerate facet test for redundant neighbor test for degenerate neighbor
 */
 ...
 *
 * qh_forcedmerges( wasmerge ) merge duplicated ridges
 *
 * returns:
 ...
 *
 * qh_hashridge( hashtable, hashsize, ridge, oldvertex ) add ridge to hashtable without oldvertex
 *
 * notes:
 *  assumes hashtable is large enough
 *
 * design:
 *  determine hash value for ridge without oldvertex find next empty slot for ridge
 */
 ...
 *
 * qh_makeridges( facet ) creates explicit ridges between simplicial facets
 *
 * returns:
 ...
 *
 * qh_mark_dupridges( facetlist ) add duplicated ridges to qh.facet_mergeset facet->dupridge is true
 *
 * returns:
 ...
 *
 * qh_maydropneighbor( facet ) drop neighbor relationship if no ridge between facet and neighbor
 *
 * returns:
 *  neighbor sets updated appends degenerate facets to qh.facet_mergeset
 *
 * notes:
 ...
 *
 * qh_mergecycle_facets( samecycle, newfacet ) finish merge of samecycle into newfacet
 *
 * returns:
 *  samecycle prepended to visible_list for later deletion and partitioning each facet->f.replace ==
 * newfacet
 *
 ...
 *
 * qh_mergecycle_neighbors( samecycle, newfacet ) add neighbors for samecycle facets to newfacet
 *
 * returns:
 ...
 *
 * notes:
 *  mindist/maxdist may be NULL (only if both NULL) traces merge if fmax_(maxdist,-mindist) >
 * TRACEdist
 *
 * see:
 *  qh_mergecycle()
 ...
 *
 * qh_mergefacet2d( facet1, facet2 ) in 2d, merges neighbors and vertices of facet1 into facet2
 *
 * returns:
 ...
 *
 * qh_mergeneighbors( facet1, facet2 ) merges the neighbors of facet1 into facet2
 *
 * see:
 ...
 *
 * qh_mergeridges( facet1, facet2 ) merges the ridge set of facet1 into facet2
 *
 * returns:
 *  may delete all ridges for a vertex sets vertex->delridge on deleted ridges
 *
 * see:
 *  qh_mergecycle_ridges()
 *
 ...
 *
 * qh_mergevertex_neighbors( facet1, facet2 ) merge the vertex neighbors of facet1 to facet2
 *
 * returns:
 ...
 *
 * qh_mergevertices( vertices1, vertices2 ) merges the vertex set of facet1 into facet2
 *
 * returns:
 ...
 *
 * qh_newvertices( vertices ) add vertices to end of qh.vertex_list (marks as new vertices)
 *
 * returns:
 *  vertices on qh.newvertex_list vertex->newlist set
 */
 ...
 *
 * qh_remove_extravertices( facet ) remove extra vertices from non-simplicial facets
 *
 * returns:
 ...
 *
 * qh_renameridgevertex( ridge, oldvertex, newvertex ) renames oldvertex as newvertex in ridge
 *
 * returns:
 *
 * design:
 ...
 *
 * qh_test_vneighbors() test vertex neighbors for convexity tests all facets on qh.newfacet_list
 *
 * returns:
 ...
 *
 * qh_tracemerge( facet1, facet2 ) print trace message after merge
 */
 ...
 *
 * qh_tracemerging() print trace message during POSTmerging
 *
 * returns:
 ...
 *
 * qh_updatetested( facet1, facet2 ) clear facet2->tested and facet1->ridge->tested for merge
 *
 * returns:
 *  deletes facet2->center unless it's already large if so, clears facet2->ridge->tested
 *
 * design:
 ...
 *
 * design:
 *  for each neighbor of vertex add ridges that include the vertex to ridges
 */
