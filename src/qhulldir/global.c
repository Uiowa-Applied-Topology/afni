 ...
 * >-------------------------------</a><a name="TOP">-</a>
 *
 * global.c initializes all the globals of the qhull application
 *
 * see README
 *
...
#else
qhT qh_qh;              /* all global variables. Add "= {0}" if this causes a compiler error. Also
 ...
 *
 * qh_version version string by year and date
 *
 ...
 *
 * qh_appendprint( printFormat ) append printFormat to qh.PRINTout unless already defined
 */
 ...
 *
 * qh_clear_outputflags() Clear output flags for QhullPoints
 */
 ...
 * >-------------------------------</a><a name="clock">-</a>
 *
 * qh_clock() return user CPU time in 100ths (qh_SECtick) only defined for qh_CLOCKtype == 2
 *
 * notes:
 *  use first value to determine time 0 from Stevens '92 8.15
 */
 ...
 *
 * qh_freebuffers() free up global memory buffers
 *
 * notes:
 ...
 *
 * qh_freeqhull( allmem ) see qh_freeqhull2 if qh_QHpointer, frees qh_qh
 */
 ...
 *
 * design:
 * free global and temporary memory from qh_initbuild and qh_buildhull free buffers free statistics
 */
 ...
 *
 *  called before error handling initialized qh_errexit() may not be used
 */
 ...
 *
 * qh_init_qhull_command( argc, argv ) build qh.qhull_command from argc/argv
 *
 * returns:
 ...
 *
 * qh_initflags( commandStr ) set flags and initialized constants from commandStr
 *
 * returns:
 ...
 *
 * qh_initqhull_outputflags initialize flags concerned with output
 *
 * returns:
 ...
 *
 * qh_initthresholds( commandString ) set thresholds for printing and scaling from commandString
 *
 * returns:
 ...
 * >-------------------------------</a><a name="option">-</a>
 *
 * qh_option( option, intVal, realVal ) add an option description to qh.qhull_options
 *
 * notes:
