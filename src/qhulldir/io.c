 ...
 * >-------------------------------</a><a name="dfacet">-</a>
 *
 * dfacet( id ) print facet by id, for debugging
 *
 */
 ...
 * >-------------------------------</a><a name="dvertex">-</a>
 *
 * dvertex( id ) print vertex by id, for debugging
 */
 ...
 *
 * qh_compare_vertexpoint( p1, p2 ) used by qsort() to order vertices by point id
 */
 ...
 *
 * qh_compare_facetarea( p1, p2 ) used by qsort() to order facets by area
 */
 ...
 *
 * qh_compare_facetmerge( p1, p2 ) used by qsort() to order facets by number of merges
 */
 ...
 *
 * qh_compare_facetvisit( p1, p2 ) used by qsort() to order facets by visit id or id
 */
 ...
 *
 * qh_copyfilename( dest, size, source, length ) copy filename identified by qh_skipfilename()
 *
 * notes:
...
void qh_countfacets(facetT *facetlist, setT *facets, boolT printall,
                    ...
                    int *numcoplanarsp, int *numtricoplanarsp)
{
    facetT *facet, **facetp;
                ...
                if (vertex->visitid != qh vertex_visit && !vertex->seen)
                {
                    ...
                    FOREACHneighborA_(vertex)
                    {
                        if (neighborA->seen)
                        {
                            ...
                            {
                                if (!neighborA->tricoplanar || qh_setunique(&tricenters,
                                                                            neighborA->center))
                                {
                                    count++;
                                }
 ...
 *
 * qh_eachvoronoi_all( fp, printvridge, isUpper, innerouter, inorder ) visit all Voronoi ridges
 *
 *  innerouter:
 *    see qh_eachvoronoi()
 *
 ...
 *
 * notes:
 *  Not used for qhull.exe same effect as qh_printvdiagram but ridges not sorted by point id
 */
int qh_eachvoronoi_all(FILE *fp, printvridgeT printvridge, boolT isUpper, qh_RIDGE innerouter,
                       boolT inorder)
{
    facetT * facet;
    vertexT *vertex;
...
void qh_geomplanes(facetT *facet, realT *outerplane, realT *innerplane)
{
    ...
    if (qh MERGING || qh JOGGLEmax < REALmax / 2)
    {
        ...
        {
            radius -= qh JOGGLEmax * sqrt((realT)qh hull_dim); /* already accounted for in
                                                                * qh_outerinner() */
        }
        *outerplane += radius;
        *innerplane -= radius;
 ...
 */
setT *qh_markvoronoi(facetT *facetlist, setT *facets, boolT printall, boolT *isLowerp,
                     int *numcentersp)
{
    int     numcenters = 0;
    facetT *facet, **facetp;
 ...
 *
 * qh_order_vertexneighbors( vertex ) order facet neighbors of a 2-d or 3-d vertex by adjacency
 *
 * notes:
 ...
 *
 * design:
 *  test for printing facet call appropriate routine for format or output results directly
 */
 ...
 *
 * qh_printbegin(  ) prints header for all output formats
 *
 * returns:
 ...
 *
 * design:
 *  count facets and related statistics print header for format
 */
 ...
 * >-------------------------------</a><a name="printend">-</a>
 *
 * qh_printend( fp, format ) prints trailer for all output formats
 *
 * see:
 ...
 *
 * qh_printend4geom( fp, facet, numridges, printall ) helper function for qh_printbegin/printend
 *
 * returns:
 *  number of printed ridges
 *
 * notes:
 *  just counts printed ridges if fp=NULL uses facet->visitid must agree with qh_printfacet4geom...
 *
 * design:
 *  computes color for facet from its normal prints each ridge of facet
 */
 ...
 *
 *  sorted by id same order as qh_printpoints_out if no coplanar/interior points
 */
 ...
 *
 * qh_printfacet( fp, facet ) prints all fields of a facet to fp
 *
 * notes:
 ...
 *
 * qh_printfacet2geom( fp, facet, color ) print facet as part of a 2-d VECT for Geomview
 *
 *  notes:
 ...
 *
 * qh_printfacet3geom_points( fp, points, facet, offset ) prints a 3-d facet as OFF Geomview object.
 ...
 *
 * qh_printfacet3math( fp, facet, notfirst ) print 3-d Maple or Mathematica output for a facet
 *
 * notes:
 ...
 *
 * qh_printfacet3vertex( fp, facet, format ) print vertices in a 3-d facet as point ids
 *
 * notes:
 *  prints number of vertices first if format == qh_PRINToff the facet may be non-simplicial
 */
 ...
 *
 * qh_printfacetheader( fp, facet ) prints header fields of a facet to fp
 *
 * notes:
 *  for 'f' output and debugging Same as QhullFacet::printHeader()
 */
 ...
 *
 * qh_printfacetridges( fp, facet ) prints ridges of a facet to fp
 *
 * notes:
 ...
 *
 * qh_printline3geom( fp, pointA, pointB, color ) prints a line as a VECT prints 0's for qh.DROPdim
 *
 * notes:
 ...
 *
 * notes:
 *  nop if point is NULL prints id unless it is undefined (-1) Same as QhullPoint's printPoint
 */
 ...
 *
 * qh_printpoint3( fp, point ) prints 2-d, 3-d, or 4-d point as Geomview 3-d coordinates
 */
...
/*----------------------------------------
 * -printpoints- print pointids for a set of points starting at index see geom.c
 */
 ...
 *
 * qh_printridge( fp, ridge ) prints the information in a ridge
 *
 * notes:
 *  for qh_printfacetridges() same as operator<< [QhullRidge.cpp]
 */
 ...
 *
 * qh_printspheres( fp, vertices, radius ) prints 3-d vertices as OFF spheres
 *
 * notes:
 ...
 *
 * qh_printvdiagram( fp, format, facetlist, facets, printall ) print voronoi diagram
 # of pairs of input sites
 #indices site1 site2 vertex1 ...
 #
 #  sites indexed by input point id point 0 is the first input point vertices indexed by 'o' and 'p'
 #order vertex 0 is the 'vertex-at-infinity'
 #    vertex 1 is the first Voronoi vertex
 #
 # see:
 #  qh_printvoronoi() qh_eachvoronoi_all()
 #
 # notes:
 #  if all facets are upperdelaunay, prints upper hull (furthest-site Voronoi diagram)
 */
 ...
 *  qh_eachvoronoi_all()
 */
int qh_printvdiagram2(FILE *fp, printvridgeT printvridge, setT *vertices, qh_RIDGE innerouter,
                      boolT inorder)
{
    int      totcount = 0;
 ...
 *
 * qh_printvertex( fp, vertex ) prints the information in a vertex Duplicated as operator<<
 * [QhullVertex.cpp]
 */
...
void qh_printvneighbors(FILE *fp, facetT *facetlist, setT *facets, boolT printall)
{
    ...
    qh_countfacets(facetlist, facets, printall, &numfacets, &numsimplicial,
                   &totneighbors, &numridges, &numcoplanars, &numtricoplanars); /* sets
                                                                                 * facet->visitid */
 ...
 *
 * see:
 *  qh_printvdiagram() qh_eachvoronoi()
 */
 ...
 *
 * notes:
 *  the user may use a different function parameter unbounded is UNUSED
 */
 ...
 *
 * qh_readfeasible( dim, curline ) read feasible point from current line and qh.fin
 *
 * returns:
 *  number of lines read from qh.fin sets qh.FEASIBLEpoint with malloc'd coordinates
 *
 * notes:
 *  checks for qh.HALFspace assumes dim > 1
 *
 * see:
 *  qh_setfeasible
 */
...
coordT *qh_readpoints(int *numpoints, int *dimension, boolT *ismalloc)
{
    ...
    if (qh CDDinput)
    {
        while ((s = fgets(firstline, qh_MAXfirst, qh fin)))
        {
            ...
            else if (!memcmp(firstline, "begin", (size_t)5) || !memcmp(firstline, "BEGIN",
                                                                       (size_t)5))
            {
                break;
            }
 ...
 *
 * qh_setfeasible( dim ) set qh.FEASIBLEpoint from qh.feasible_string in "n,n,n" or "n n n" format
 *
 * notes:
 *  "n,n,n" already checked by qh_initflags() see qh_readfeasible()
 */
 ...
 *
 * qh_skipfacet( facet ) returns 'True' if this facet is not to be printed
 *
 * notes:
 ...
 *
 * qh_skipfilename( string ) returns pointer to character after filename
 *
 * notes:
 *  skips leading spaces ends with spacing or eol if starts with ' or " ends with the same, skipping
 * \' or \"
