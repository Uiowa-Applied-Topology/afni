 ...
 *
 * qh_intcompare( i, j ) used by qsort and bsearch to compare two integers
 */
static int qh_intcompare(const void *i, const void *j)
{
 ...
 * >--------------------------------</a><a name="memalloc">-</a>
 *
 * qh_memalloc( insize ) returns object of insize bytes qhmem is the global memory structure
 *
 * returns:
    ...
    if (insize <= qhmem.LASTsize)
    {
        ...
        if ((object = *freelistp))
        {
            ...
            *freelistp     = *((void **)*freelistp); /* replace freelist with next object */
            ...
            {
                qh_fprintf(qhmem.ferr, 8141,
                           "qh_mem %p n %8d alloc quick: %d bytes (tot %d cnt %d)\n", object, n,
                           outsize, qhmem.totshort,
                           qhmem.cntshort + qhmem.cntquick - qhmem.freeshort);
            }
#endif
            return object;
        }
        else
        {
            ...
            if (outsize > qhmem.freesize)
            {
                ...
                *((void **)newbuffer) = qhmem.curbuffer; /* prepend newbuffer to curbuffer list */
    ...
    else                      /* long allocation */
    {
        ...
        if (qhmem.IStracing >= 5)
        {
            qh_fprintf(qhmem.ferr, 8057, "qh_mem %p n %8d alloc long: %d bytes (tot %d cnt %d)\n",
                       object, qhmem.cntlong + qhmem.freelong, outsize, qhmem.totlong,
 ...
 * >--------------------------------</a><a name="memfree">-</a>
 *
 * qh_memfree( object, insize ) free up an object of size bytes size is insize from qh_memalloc
 *
 * notes:
        ...
        *freelistp         = object;
        ...
        if (qhmem.IStracing >= 5)
        {
            qh_fprintf(qhmem.ferr, 8142, "qh_mem %p n %8d free short: %d bytes (tot %d cnt %d)\n",
                       object, idx, outsize, qhmem.totshort,
                       qhmem.cntshort + qhmem.cntquick - qhmem.freeshort);
        }
#endif
    }
    else
    {
        ...
        if (qhmem.IStracing >= 5)
        {
            qh_fprintf(qhmem.ferr, 8058, "qh_mem %p n %8d free long: %d bytes (tot %d cnt %d)\n",
                       object, qhmem.cntlong + qhmem.freelong, insize, qhmem.totlong,
                       qhmem.cntlong - qhmem.freelong);
        }
    }
} /* memfree */
 ...
 *
 * qh_memfreeshort( curlong, totlong ) frees up all short and qhmem memory allocations
 *
 * returns:
 *  number and size of current long allocations
 *
 * see:
 *  qh_freeqhull(allMem) qh_memtotal(curlong, totlong, curshort, totshort, maxlong, totbuffer);
 */
 ...
 * >--------------------------------</a><a name="meminit">-</a>
 *
 * qh_meminit( ferr ) initialize qhmem and test sizeof( void*)
 */
 ...
 * >-------------------------------</a><a name="memsize">-</a>
 *
 * qh_memsize( size ) define a free list for this size
 */
 ...
 *
 * qh_memstatistics( fp ) print out memory statistics
 *
...
void qh_memstatistics(FILE *fp)
{
    ...
    {
        qh_fprintf(qhmem.ferr, 6211,
                   "qh_memstatistics internal error: totfree %d not equal to freelist total %d\n",
 ...
 * >-------------------------------</a><a name="NOmem">-</a>
 *
 * qh_NOmem turn off quick-fit memory allocation
 *
 * notes:
