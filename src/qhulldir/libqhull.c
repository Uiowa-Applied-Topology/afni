 ...
 * >-------------------------------</a><a name="TOP">-</a>
 *
 * libqhull.c Quickhull algorithm for convex hulls
 *
...
void qh_partitionall(setT *vertices, pointT *points, int numpoints)
{
    ...
    if (!qh BESToutside)                                        /* matches conditional for
                                                                 * qh_partitionpoint below */
    {
        distoutside          = qh_DISToutside;                  /* multiple of qh.MINoutside &
                                                                 * qh.max_outside, see user.h */
...
void qh_partitioncoplanar(pointT *point, facetT *facet, realT *dist)
{
    ...
    if (bestdist > qh max_outside)
    {
        if (!dist && facet != bestfacet)
        {
            ...
            if (angle < 0)
            {
                /* typically due to deleted vertex and coplanar facets, e.g., RBOX 1000 s Z1 G1e-13
                 * t1001185205 | QHULL Tv */
                zinc_(Zpartflip);
                trace2((qh ferr, 2058,
...
void qh_partitionpoint(pointT *point, facetT *facet)
{
    ...
    else if (qh DELAUNAY || bestdist >= -qh MAXcoplanar)  /* for 'd', bestdist skips upperDelaunay
                                                           * facets */
    {
