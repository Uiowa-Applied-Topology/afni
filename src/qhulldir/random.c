 ...
 *
 *  build command from argc/argv max_size is at least
 *
 * returns:
 *  a space-delimited string of options (just as typed) returns false if max_size is too short
 *
 * notes:
 *  silently removes makes option string easy to input and output matches qh_argv_to_command_size()
 *
 *  argc may be 0
 */
 ...
 *
 * notes:
 *  argc may be 0 actual size is usually shorter
 */
 ...
 * >-------------------------------</a><a name="rand">-</a>
 *
 * qh_rand() qh_srand( seed ) generate pseudo-random number between 1 and 2^31 -2
 *
 * notes:
 ...
 *
 * qh_randomfactor( scale, offset ) return a random factor r * scale + offset
 *
 * notes:
