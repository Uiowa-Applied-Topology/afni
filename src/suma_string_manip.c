/*
 * Assortment of functions to maniplate strings, particularly those from help output.
 *
 * Many of those functions originated in SUMA/ but they are now part of libmri.a.
 *
 * While the functions herein use SUMA_[c,m,re]alloc, you're OK using either free or SUMA_free on
 *returned pointers because the SUMA's allocation functions are the same as AFNI's mcw_[c,m,re]alloc
 *
 */
...
/*
 * SUMA_EscapeChars ("Hallo_Baby%Hallo", "_%", "//") returns Hallo\\_Baby\\%Hallo
...
/*
 * Insert string 'ins' at pointer 'pos' inside of string '*s' which has at most *nalloc characters.
 *Returns the string with the insertion, and reallocates and updates nalloc if needed. Call with
 *something like:
                  ...
                  int nmax, int multiline, FILE *fout)
{
    ...
    if (postscript)
    {
        fprintf(fout, "%s", postscript);
    }
    return;
}

/*!
 * Append s2 to s1 but without exceeding nmax characters for s1
 *
...
/*
 * Find 1st location in cur that begins with string opt. Blanks are ignored.
 *
 * If term is not null, then string opt must be followed by one of the characters in term.
 *
 * If bracketers is not null, accept an opening bracket as a valid starting character. bracketers
 *must have an even number of characters with each pair containing the opening/closing characters.
 *
 * Function returns pointer to beginning of opt in the line, and sets number of blanks preceding it
 */
...
/*! My version of Bob's Decode a single string into a bunch of strings, separated by characters from
 * the list in sep.
 *  - Passing sep in as NULL means to use "," as the separator.
 *  - In each sub-string, leading and trailing blanks will be excised.
 *  - This can result in 0 length strings (e.g., "1,,2," will result in the second and fourth output
...
NI_str_array * SUMA_NI_decode_string_list(char *ss, char *sep)
{
    ...
    while (id < lss)
    {
        ...
        /* skip ahead until ss[id] is a separator [or a space - 10 Dec 2002 ZSS I don't like that
         * one, gives me funny looking results with legitimate spaces , line below was:
         ...
         * id++; continue;
         * But that is a bad idea in cases when parsing strings that have something like "...;;..."
         * where
         * ';;' indicates an empty string. That can come up for column range of data elements when a
...
#if 0
        while (nn > 0 && isspace(ss[jd + nn - 1]))
        {
            nn--;                                    /* clip trailing blanks */
        }
#endif
        sar->str[num] = NI_malloc(char, (nn + 1) * sizeof(char)); /* output string  */
        if (nn > 0)
        {
            memcpy(sar->str[num], ss + jd, nn);      /* copy sub-string    */
        }
...
char * SUMA_NI_get_ith_string(char *ss, char *sep, int i)
{
    ...
    if (ss == NULL || ss[0] == '\0' || i < 0)
    {
        SUMA_RETURN(NULL);                                       /* bad input */
    }
    if (sep == NULL || sep[0] == '\0')
    {
        sep = ",";                                  /* default sep */
    ...
    while (id < lss)
    {
        ...
        /* new sub-string runs from ss[jd] to ss[id-1] */
        {
            ...
            if (nn > 0)
            {
                memcpy(str, ss + jd, nn);   /* copy sub-string    */
            }
...
int  SUMA_NI_find_in_cs_string(char *ss, char *sep, char *str)
{
    ...
    if (ss == NULL || ss[0] == '\0' || str == NULL)
    {
        SUMA_RETURN(i);                                           /* bad input */
    }
    if (sep == NULL || sep[0] == '\0')
    {
        sep = ",";                                  /* default sep */
    ...
    while (id < lss)
    {
        ...
        /* new sub-string runs from ss[jd] to ss[id-1] */
        if (nn == strlen(str)) /* a strict search, might want to allow for blanks at some point ...
                                */
        {
            if (!strncmp(str, ss + jd, strlen(str)))
            {
                SUMA_RETURN(num);
            }
        }
...
void SUMA_Show_NI_str_ar(NI_str_array *nisa, FILE *out)
{
    ...
    if (!nisa)
    {
        SS = SUMA_StringAppend_va(SS, "NULL struct");
    }
    else
    {
...
/*!
 * \brief take a bunch of strings stored in NI_str_array and turn them into a composite string Free
 *result with SUMA_free
...
/*!
 * \brief Inverse of SUMA_NI_str_ar_2_comp_str free output with SUMA_free_NI_str_array
 */
...
int SUMA_NI_str_array_find(char *targ, NI_str_array *sar, int partial, int ci)
{
    ...
    if (!partial)
    {
        ...
        else
        {
            for (ii = 0 ; ii < sar->num ; ii++)
            {
                if (strcasecmp(targ, sar->str[ii]) == 0)
                {
                    SUMA_RETURN(ii);
                }
            }
    ...
    else
    {
        if (!ci)
        {
            for (ii = 0 ; ii < sar->num ; ii++)
            {
                if (strstr(sar->str[ii], targ) == NULL)
                {
                    SUMA_RETURN(ii);
                }
            }
        ...
        else
        {
            for (ii = 0 ; ii < sar->num ; ii++)
            {
                if (!AFNI_strcasestr(sar->str[ii], targ))
                {
                    SUMA_RETURN(ii);
                }
            }
...
/*!
 * \brief returns the iith string in a sep separated composite string cs free result with SUMA_free
 */
char *SUMA_Get_Sub_String(char *cs, char *sep, int ii)
{
    ...
    if (ii < 0)
    {
        SUMA_SL_Err("Bad index"); SUMA_RETURN(s);
    }
    if (!cs)
    {
        SUMA_SL_Err("NULL input"); SUMA_RETURN(s);
    }
   #if 0 /* old slow way */
...
int SUMA_Find_Sub_String(char *cs, char *sep, char *ss)
{
    ...
    if (!ss)
    {
        SUMA_SL_Err("Bad string"); SUMA_RETURN(ii);
    }
    if (!cs)
    {
        SUMA_SL_Err("NULL input"); SUMA_RETURN(ii);
    }
...
SUMA_Boolean SUMA_Set_Sub_String(char **cs, char *sep, int ii, char *str)
{
    ...
    if (ii < 0)
    {
        SUMA_SL_Err("Bad index"); SUMA_RETURN(NOPE);
    }
    if (!cs || !str)
    {
        SUMA_SL_Err("NULL input %p %p", cs, str);
        if (LocalHead)
        {
            SUMA_DUMP_TRACE("Why");
        }
...
/*!
 * \brief removes a string in a sep separated composite string cs The function does not reallocate
 *for cs returns 0 fail 1 strn found and removed
 *         -1 strn not found
 */
...
/*!
 * \brief Reads in a sequence of numbers of an undetermined length Not for reading in large numbers
 *of numbers!
 * \param op (char *) pointing to the beginning of a blank delimited series of numbers
 * \param opend (char **) if not NULL, *opend will contain the value of op at the end of successful
 *reads
 * \param tp (SUMA_VARTYPE) SUMA_int, SUMA_float, SUMA_double supported at the moment
 * \return ans (void*) if  tp == SUMA_int then ans is (SUMA_IVEC *) tp == SUMA_float then ans is
 *(SUMA_FVEC *) tp == SUMA_double then ans is (SUMA_DVEC *)
 * \sa SUMA_strtol_vec
 * \sa SUMA_SringToNum
 */
    ...
    *nread = 0;
    ...
    case SUMA_int:
    {
        ...
        while (endptr && endptr != op && *nread < nvals)
        {
            ...
            /* if (LocalHead && *nread < 10) fprintf(SUMA_STDERR,">>>%d<<<\t", ivec[*nread]); */
            ++(*nread);
            op = endptr;
            lv = strtol(op, &endptr, 10);
        }
...
/*
 * Break long lines into ones that are no longer than mxln You need to handle the freeing of the
...
/*
 *
 * A convenience version of SUMA_Offset_SLines which absolves you of having to free the offset
 *strings at all.
 *
 * Now, if you feel moved to clean, DO NOT FREE the strings outside of this function. To clean the
 *allocated space, just call:
...
char *SUMA_Offset_SLines(char *si, int off)
{
    ...
    if (!sov)
    {
        sov = (char **)SUMA_calloc(Nmax, sizeof(char *));
    }
 ...
 * "That's one nice pen."
 *
 * Note that 's' cannot be a constant string because the function will need to write into it.
 *
 */
char *SUMA_Sphinx_DeRef(char *s, char *r)
{
    ...
    if (!strcmp(r, ":LIT:")) /* special case for non Sphinx directive */
    {
        ...
        while (ss)
        {
            ...
            if (nso && !SUMA_IS_PURE_BLANK(so[nso - 1]))
            {
                so[nso++] = ':';
            }
            s += strlen(r);
 ...
 *
 * Function handles reallocation if sw is longer than sc. Just make sure *sp can be reallocated.
 *
 */
...
/*
 * Split a string into multiple ones using string sc as a deliminter.
 *
...
/*
 * Cut out chunk of string 's' bracketed by 'sc0' and 'sc1', but do leave any portion between 'save'
 * and 'sc1', if any exist.
 *
...
void SUMA_Clipping_Planes_Help(FILE *fout, int forweb)
{
     ...
     *
     * s0 = SUMA_copy_string(s);
     * fprintf(fout,"\nEdited  for  SPHINX::\n\n%s\n    -------\n", SUMA_Sphinx_String_Edit(&s0,SPX,
     *forweb?3:0));
     *
     * if (forweb) {
 ...
 * \param off: (int) Number of blank characters to insert at the beginning of each line that does
 *not begin with :NOF:
...
/*
 * A function that allows me to format help strings for display in SUMA as was done in the past, and
 *for fancier SPHINX formatted output.
 *
 * \param suser (char **): Pointer to user's string
 * \param targ (TFORM) the format, 0 for regular, 1 for sphinx
 * \param off: (int) Number of blank characters to insert at the beginning of each line that does
 *not begin with :NOF:
 * \return: s (char *): Edited string. Note that usually s = *suser, unless reallocation was
 *necessary. In that case the function takes care of freeing suser and resetting it to new pointer.
 *
...
char *SUMA_Sphinx_String_Edit(char **suser, TFORM targ, int off)
{
    ...
    case TXT:   /* Default C output */
        ...
        sprintf(stmp, "\\|"); /* to avoid compile warning for direct use of "\|" in SUMA_Swap_String
                               * below */
...
/*
 * Take the help output of program prog and return it as a string in sphinx format.
 */
...
char *sphinxize_prog_shelp(char *prog, char *oh, int verb)
{
    ...
    ohc = SUMA_copy_string(oh); /* make copy to avoid corrupting oh in approx_str_sort_all_popts */
...
/*
 * Check if string begins with sphinx directives used in SUMA's code
 */
...
/*
 * Check if string begins with AFNI  sphinx directives used in SUMA's code
 */
...
char *SUMA_Sphinx_LineSpacer(char *s, TFORM targ)
{
    ...
    while (s[ns])
    {
        ...
        {
            bln = 0;
            while (s[ns + bln + 1] && SUMA_IS_PURE_BLANK(s[ns + 1 + bln]))
            {
                ++bln;
            }
            if (bln > 0 && s[ns + 1 + bln] == ':' &&
                ...
                !SUMA_Known_Sphinx_ADir(s + ns + 1 + bln))
            {
                ...
                case SPX: /* remove all spaces */
                    /* remove preceding new line just to keep superfluous new line characters that
                     * were there for the purpose of keeping the output width short. Do not remove
                     * the newline if there is two of them in a row, or there is certain punctuation
                     * before the newline.*/
                    if (nso > 1 && so[nso - 1] == '\n' &&
                        (so[nso - 2] != '\n' && so[nso - 2] != ':'))
                    {
 ...
 *
 * This function will reallocate for whatever is in us and set *us accordingly.
 *
 * The function returns *us, whether or not *us was changed.
 */
char *SUMA_Sphinx_SetVars(char **us, TFORM targ)
{
    static char FuncName[] = { "SUMA_Sphinx_SetVars" };
    insertion * ins        = NULL;
    int         N_ins = 0, ntok = 0, ns = 0, nso = 0, ii = 0, jj = 0, oo = 0, N_ins_alloc,
                nextra = 0;
    ...
    while ((ss = strstr(s, ":=")))
    {
        ...
        if (strstr(ss, tok))
        {
            ...
            {
                /* Looks like sphinx likes // for absolute path references like .. image::
                 * //Users/home/abin/...
...
/*
 * A function that formats the content of a printf statement to handle Sphinx markups as well as our
 *own, before writing the results.
 *
 * \param targ: (int) 0 -- Regular output 1 -- Sphinx output
 * \param off: (int) Number of blank characters to insert at the beginning of each line that does
 *not begin with :NOF:
 ...
 *
 * \sa  macros: sphinx_printf(int targ, char *str, ...) and  sphinx_fprintf(int targ, FILE *fout,
 *char *str, ...)
 *
 * Examples:
 * {
