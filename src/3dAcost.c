...
int main(int argc, char *argv[])
{
    ...
    while (iarg < argc && argv[iarg][0] == '-')
    {
        if (strcmp(argv[iarg], "-xrange") == 0)
        {
            if (++iarg >= argc - 1)
            {
                ERROR_exit("no arguments after '%s'!", argv[iarg - 1]);
            }
        ...
        if (strncmp(argv[iarg], "-mask", 5) == 0)
        {
            ...
            if (iarg + 1 >= argc)
            {
                ERROR_exit("-mask needs a filename!");
            }
    ...
    if (mask_dset != NULL)
    {
        ...
        if (mcount <= 5)
        {
            ERROR_exit("Mask is too small: %d voxels", mcount);
        }
    ...
    if (xyall <= 5)
    {
        ERROR_exit("Too few voxels to continue!");
    }
...
#if 0
            for (ii = 0 ; ii < nbin * nbin ; ii++)
            {
                xyc[ii] = sqrtf(xyc[ii]);
            }
            ftop = mri_max(fim); if (ftop == 0.0)
            {
                ftop = 1.0;
            }
