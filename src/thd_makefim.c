...
/*-------------------------------------------------------------------------
 * Routine to create a 3D 'fim' dataset from a 3D+time dataset, using a user supplied function to
 *process each time series.
 * "user_func" should like
 *
 * void user_func( double tzero , double tdelta , int npts , float ts[] , double ts_mean , double
 *ts_slope , void * ud , float * val )
 *
 * where tzero  =  time at ts[0] tdelta =  time at ts[1] (i.e., ts[k] is at tzero + k*tdelta);
 *                   tzero and tdelta will be in sec if this is truly "time"
 *       npts   =  number of points in ts array ts     =  one voxel time series array, ts[0] ..
 * ts[npts-1];
 *                   note that this will always be a float array, and that ts will start with the
 * "ignore"-th point of the actual voxel time series.
 *      ts_mean =  mean value of ts array ts_slope =  slope of ts array;
 *                   this will be inversely proportional to tdelta (units of 1/sec);
 *                   if "detrend" is nonzero, then the mean and slope will been removed from the ts
 *array ud     =  the user_data pointer passed in here -- this can contain whatever control
 *information the user wants val    =  pointer to return value for this voxel;
 *                   note that this must be a float
 *
 * Before the first timeseries is passed, user_func will be called with arguments ( 0.0 , 0.0 , nvox
 *, NULL , 0.0 , 0.0 , user_data , NULL ) where nvox = number of voxels that will be processed. This
 *is to allow for some setup (e.g., malloc, PLUTO_popup_meter, ...).
 *
 * After the last timeseries is passed, user_func will be called again with arguments ( 0.0 , 0.0 ,
 *0 , NULL , 0.0 , 0.0 , user_data , NULL ) This is to allow for cleanup (e.g., free of malloc,
 * ...).  Note that the only difference between these "notification" calls is the third argument.
 *
 * The inputs to the present routine are old_dset   = pointer to old dataset;
 *                 note that this dataset must not be warp-on-demand new_prefix = string to use as
 *filename prefix new_datum  = type of data to store in output brick;
 *                 if negative, will use datum from old_dset ignore     = number of data points to
 *ignore at the start detrend    = if nonzero, this routine will detrend (a+b*t) each time series
 *before passing it to user_func user_func  = discussed above user_data  = discussed above
 *
                                         ...
                                         void *user_data)
{
    ...
    if (!ISVALID_3DIM_DATASET(old_dset))
    {
        return NULL;
    }
    ...
    if (new_datum < 0)
    {
        new_datum = old_datum;                   /* output datum = input */
    }
    if (new_datum == MRI_complex)
    {
        return NULL;                           /* but complex = bad news */
    }
    ...
    /*------ make floating point output brick (only at the end will scale to byte or shorts)
     * ------*/
    ...
    if (DSET_TIMEUNITS(old_dset) == UNITS_MSEC_TYPE)
    {
        tdelta *= 0.001;
    }
    if (tdelta == 0.0)
    ...
    /*------- The output is now in fout[ii], ii=0..nvox-1. We must now put this into the output
     * dataset -------*/
    ...
    /*** output is bytes (byte = unsigned char) we have to create a scaled sub-brick from fout ***/
