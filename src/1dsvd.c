...
int main(int argc, char *argv[])
{
    ...
    for ( ; iarg < argc ; iarg++)
    {
        ...
        if (tim == NULL)
        {
            ERROR_exit("Can't read 1D file %s\n", argv[iarg]);
        }
        if (nx == 0)
        {
            nx = tim->nx;
