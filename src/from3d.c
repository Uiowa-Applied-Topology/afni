 ...
 *
 * File:    from3d.c Author:  B. Douglas Ward Date:    30 August 1996
 *
...
void F3D_initialize_user_data(int Argc, char *Argv[],
                              RwcBoolean *verbose, RwcBoolean *nsize, RwcBoolean *raw,
                              RwcBoolean *do_floats,
    ...
    *tlast     = BIGNUMBER;
    ...
    while (nopt < Argc && Argv[nopt][0] == '-')
    {
        ...
        if (strncmp(Argv[nopt], "-zfirst", 4) == 0)
        {
            if (++nopt >= Argc)
            {
                ERROR_exit("-zfirst needs an argument");
            }
        ...
        if (strncmp(Argv[nopt], "-zlast", 4) == 0)
        {
            if (++nopt >= Argc)
            {
                ERROR_exit("-zlast needs an argument");
            }
            ftemp  = strtod(Argv[nopt], NULL);
            *zlast = (int)ftemp;
            nopt++;
        ...
        if (strncmp(Argv[nopt], "-tfirst", 4) == 0)
        {
            if (++nopt >= Argc)
            {
                ERROR_exit("-tfirst needs an argument");
            }
            ftemp   = strtod(Argv[nopt], NULL);
            *tfirst = (int)ftemp;
            nopt++;
        ...
        if (strncmp(Argv[nopt], "-tlast", 4) == 0)
        {
            if (++nopt >= Argc)
            {
                ERROR_exit("-tlast needs an argument");
            }
            ftemp  = strtod(Argv[nopt], NULL);
            *tlast = (int)ftemp;
            nopt++;
...
int main(int argc, char *argv[])
{
    ...
    THD_diskptr *     dskptr;
    int        nx, ny, nz, nv;
    RwcBoolean verbose, nsize, raw, do_floats;
    int        ok;
    ...
    if (!ok)
    {
        ERROR_exit("Unable to load data block");
    }
    ...
    if (zfirst > nz)
    {
        ERROR_exit("No data selected -- zfirst too large.");
    }
    if (zlast < 1)
    {
        ERROR_exit("No data selected -- zlast too small.");
    }
    if (tfirst > nv)
    {
        ERROR_exit("No data selected -- tfirst too large.");
    }
    if (tlast < 1)
    {
        ERROR_exit("No data selected -- tlast too small.");
    }
    ...
    if (verbose)
    {
        printf("Created %d 2D image files.\n", count);
    }
