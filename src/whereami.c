 ...
 * Main function added by Mike Angstadt on 1/12/05 usage: whereami x y z [output format] where x,y,z
 *are float coordinates in tlrc space and output format is a 0 or 1 0 (default) just outputs the
 *string as it would appear from the interactive AFNI Where am I? command 1 outputs the string as a
 *tab-delimited list of the form:
...
int print_atlas_reference(char *atname)
{
    ...
    else
    {
        ...
        if (refs)
        {
            refs = free_names_list(refs, N_refs);
        }
    }
...
int main(int argc, char **argv)
{
    ...
    while (iarg < argc)
    {
        ...
        if (!isnakedarg(argv[iarg]))
        {
            ...
            for (i = 1; i < arglen; ++i)
            {
                argv[iarg][i] = tolower(argv[iarg][i]);
            }
...
/*         print_atlas_list(get_G_atlas_list());*/
    ...
    if (shar)
    {
        ...
        else
        {
            ...
            /* analyze the matches, remember no left/right decisions made yet, and even if labels
             * are present, right/left sides may not have different ids in atlas...  */
    ...
    if (cmask)
    {
        free(cmask); cmask = NULL;
    }                                        /* Should not be needed beyond here */
...
/*           linkrbrain_str = parse_linkrbrain("linkrbrain.xml");*/
    ...
    for (ixyz = 0; ixyz < nxyz; ++ixyz)
    {
        ...
        if (!OldMethod)
        {
            ...
            if (atlas_sort)
            {
                if (output == 1)
                {
                    TT_whereami_set_outmode(TAB1_WAMI_ATLAS_SORT);
                }
                else
                {
                    TT_whereami_set_outmode(CLASSIC_WAMI_ATLAS_SORT);
                }
            }
            else
            {
                if (output == 1)
                {
                    TT_whereami_set_outmode(TAB1_WAMI_ZONE_SORT);
                }
                else
                {
                    TT_whereami_set_outmode(CLASSIC_WAMI_ZONE_SORT);
                }
            }
            ...
            if (space_dset)
            {
                if (LocalHead)
                {
                    INFO_message("Calling tt_whereami with space_dset");
                }
            ...
            else
            {
                ...
                if (LocalHead)
                {
                    INFO_message("Calling tt_whereami with srcspace %s",
                                 srcspace);
                }
    ...
    if (coord_list)
    {
        free(coord_list); coord_list = NULL;
    }
                ...
                int N_atlas_names, char **atlas_names, ATLAS_LIST *atlas_alist)
{
    ...
    for (iroi = 0; iroi < n_unq; ++iroi)
    {
        if (dobin)
        {
            ...
            if ((nonzero = THD_makedsetmask(mset, 0, 1.0, 0.0, cmask)) < 0)                     /*
                                                                                                 * get
                                                                                                 *all
                                                                                                 *non-zero
                                                                                                 *values
            ...
            for (isb = 0; isb < DSET_NVALS(ATL_DSET(atlas)); ++isb)
            {
                ...
                for (i = 0; i <= atlas->adh->maxkeyval; ++i)
                {
                    if (count[i])
                    {
                        if (ics[i] == 0)
                        {
                            continue;        /* don't count codes of 0 */
                        }
                        if (strcmp(
