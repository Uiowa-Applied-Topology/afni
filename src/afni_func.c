...
/*-------------------------------------------------------------------
 * This routine is also used by the macros AFNI_SEE_FUNC_ON and AFNI_SEE_FUNC_OFF as well as the
 *MCW_bbox that it is attached to as the callback.
 * ---------------------------------------------------------------------*/
...
float AFNI_get_autothresh(Three_D_View *im3d)
{
    ...
    if (!IM3D_OPEN(im3d) || !ISVALID_DSET(im3d->fim_now))
    {
        RETURN(-1.0f);
    }
...
void AFNI_set_pval(Three_D_View *im3d, float pval)
{
    ...
    if (!IM3D_OPEN(im3d) || pval <= 0.0f || pval >= 1.0f)
    {
        EXRETURN;
    }
...
void AFNI_set_qval(Three_D_View *im3d, float qval)
{
    ...
    if (!IM3D_OPEN(im3d) || qval <= 0.0f || qval >= 1.0f)
    {
        EXRETURN;
    }
    if (DSET_BRICK_STATCODE(im3d->fim_now, im3d->vinfo->thr_index) <= 0)
...
void AFNI_func_setqval_final_CB(Widget w, XtPointer cd, int nval, void **val)
{
    ...
    if (!IM3D_OPEN(im3d) || nval != 2 || val == NULL)
    {
        EXRETURN;
    }
...
void AFNI_func_alpha_CB(MCW_arrowval *av, XtPointer cd)
{
    ...
    if (!IM3D_OPEN(im3d) || !im3d->vinfo->thr_use_alpha)
    {
        EXRETURN;
    }
...
void AFNI_set_threshold(Three_D_View *im3d, float val)
{
    ...
    if (!IM3D_OPEN(im3d) || val < 0.0f || val > THR_top_value)
    {
        EXRETURN;
    }
    ...
    else if (olddec > THR_top_expon)
    {
        olddec = THR_top_expon;
    }
    newdec = olddec;
    ...
    if (val > 0.0f)
    {
        ...
        else if (newdec > THR_top_expon)
        {
            newdec = THR_top_expon;
        }
        if (newdec != olddec)
        {
 ...
 * 30 Oct 1996: changed scale factor from slider to threshold from 0.01 to 0.001 to allow for
 *increased precision of scale (0..999 instead of 0..99).
 * -------------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------
 * Called when the user drags the threshold scale at all, even without releasing it.  Used to update
 *the "p-value" interactively.
 * ----------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------
 * Used to set the pval (significance) label at the bottom of the threshold scale.  And the qval
 *(FDR) label.
 * -------------------------------------------------------------------------*/
...
void AFNI_set_thr_pval(Three_D_View *im3d)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (dec < 10)
            {
                sprintf(buf, "p=%3.1f-%1d", zval, dec);
            }
            else
            {
                sprintf(buf, "p=%1d.-%2d", (int)rint(zval), dec);
            }
        }
        ...
        if (zval > 0.0f)
        {
            ...
            if (qval > 0.0f && qval < 0.9999)
            {
                ...
                else
                {
                    ...
                    if (dec < 10)
                    {
                        sprintf(qbuf, " %3.1f-%1d", zval, dec);
                    }
                    else
                    {
                        sprintf(qbuf, " %1d.-%2d", (int)rint(zval), dec);
                    }
                }
                strcat(buf, "\nq="); strcat(buf, qbuf + 1);
...
void AFNI_hintize_pbar(MCW_pbar *pbar, float fac)
{
    ...
    if (pbar == NULL || fac == 0.0f)
    {
        EXRETURN;                                /* bad */
    }
    if (PBAR_FULLRANGE)
    {
        fac = 1.0f;                      /* 03 Jun 2014 */
    }
    if (pbar->bigmode)
    {
...
/*----------------------------------------------------------------------------
 * called when the pbar for the intensity mapping is adjusted (thresholds or colors)
 * ------------------------------------------------------------------------------*/
...
void AFNI_iab_pbar_CB(MCW_pbar *pbar, XtPointer cd, int reason)
{
    ...
    if (pbar == NULL || !pbar->three_level || !IM3D_OPEN(im3d))
    {
        EXRETURN;                                                          /* error */
    }
    switch (reason)
...
void AFNI_iab_av_CB(MCW_arrowval *av, XtPointer cd)
{
    ...
    else if (av == im3d->vwid->func->iab_bot_av)
    {
        if (av->ival != 2)
        {
            AFNI_iab_pbar_CB(pbar, im3d, pbCR_VALUE);
        }
    }
...
/** 3/27/95: changed to allow different initializations for the [0,1] and the [-1,1] range pbars.
 *  **/
...
void AFNI_setup_inten_pbar(MCW_pbar *pbar)
{
    ...
    for (i = 0 ; i <= np ; i++)
    {
        pbar->pval[i] = pbar->pval_save[np][i][jm];
    }
    for (i = 0 ; i < np ; i++)
    {
        pbar->ov_index[i] = pbar->ovin_save[np][i][jm];
    }
...
char * AFNI_inten_av_texter(MCW_arrowval *av, XtPointer cd)
{
    ...
    if (av->ival > NPANE_MAX)
    {
        strcpy(buf, "**");                                  /* label for  */
    }
    else
    {
        sprintf(buf, "%d", av->ival);                       /* number of  */
    }
    return buf;                                             /* pbar panes */
}

/*---------------------------------------------------------------------
 * Make a warped dataset whose grid corresponds to the anat_parent and whose data comes from the
 *data_parent. Note that the assumption is made that data_parent and the warp parent of the
 *anat_parent are both in the same coordinate system (up to the to_dicomm transformation of their
 *dataxes structs).
 * -----------------------------------------------------------------------*/
                                         ...
                                         THD_3dim_dataset *data_parent)
{
    ...
    if (!ISVALID_3DIM_DATASET(anat_parent) ||
        !ISVALID_3DIM_DATASET(data_parent))
    {
        RETURN(NULL);
    }
    ...
    if (DSET_NUM_TIMES(data_parent) > 1 && !GLOBAL_argopt.warp_4D)
    {
        RETURN(NULL);
    }
    ...
    *(new_dset->warp)   = IDENTITY_WARP; /* start with (Dicom) identity */
    ...
    /* set the axes for this new dataset (same as anatomy parent, since that's the meaning of this
     * routine) */
    ...
    *(new_dset->daxes) = *(anat_parent->daxes);   /* anatomy parent   */
    ...
    /* create a datablock and diskptr, in case the data is ever filled into memory (instead of wod)
     * and written to disk */
    ...
    /* oh yeah, set the new_dset kill list, copy statistics if available, and NULL out any unused
     * stuff */
 ...
 * as 'followers' (as in AFNI_follower_dataset).  That is, find any slots that are not now filled,
 *but are "downstream" from original view data that has an anatomy parent:
 *
 * SPGR:   orig  acpc  ----
 *         /^\                <--- anatomy parent
 |                 <--- relationship FIM:    orig  ----  ----
 |
 | In this picture, the first blank in the FIM line can be filled in, using the fim+orig dataset as
 |the data_parent and the spgr+acpc dataset as the anat_parent for the new fim+acpc dataset.
 |
 | The algorithm used to find slots that may be filled in is as simple as the picture above -- the
 |'orig' view is presumed to contain all the basic anatomy parent relationships.  This may not be
 |true in the future (e.g., when registering one dataset to another, the 'rgst' view will then have
 |a basic anatomy parent relationship).  I'll worry about that later, as well as about the
 |possibility that the newly created datasets may spawn further possible offspring (e.g., if they
 |themselves are anatomical images that fill in slots that will become anat_parents).
 |                                            -- RWCox, November, 1994 A.D.
 |
 | 3 June 1998:
 |  The original version of this is renamed to be AFNI_make_descendants_old, and is modified to do a
 |sweep starting at the "vbase" view.  Then we sweep twice, once from +orig and once again from
 |+acpc.  This is to allow for creation of descendants from +acpc that do not have ancestors at
 |+orig.
 | ---------------------------------------------------------------------------*/
...
/*                  **orig_row ;*/
    ...
    for (iss = 0 ; iss < ssl->num_sess ; iss++)
    {
        ...
        {
            continue;                        /* no good ==> skip */
        }
        if (ss == GLOBAL_library.session)
        {
            continue;                               /* 21 Dec 2001 */
        }
...
/*         orig_dset = ss->dsset_xform_table[jdd][vbase] ; */ /* 03 Jun 98 */
            if (!ISVALID_3DIM_DATASET(orig_dset) ||           /* no good   */
                ...
                orig_dset->anat_parent == orig_dset)
            {
                continue;                                     /* ==> skip  */
            }
            if (DSET_in_global_session(orig_dset))
            {
                continue;                                   /* 25 Dec 2001 */
            }
            ...
            /* check for a good find; if it doesn't happen, then skip (this eventuality should never
             * occur!) */

            if (find.dset == NULL ||
                ISFUNC(find.dset) ||     /* anat parent can't be a func! */
                find.view_index != vbase)
            {
                continue;
            }

            /* make a pointer to the row of datasets of the anat parent (this is like the SPGR row
            ...
            if (orig_dset == anat_parent_dset)
            {
                continue;                              /* 20 Jul 2010 */
            }
 ...
 *         &(ssl->ssar[find.sess_index]->dsset_xform_table[find.dset_index][0]) ;*/
            ...
            /* pointer to row of datasets being operated on now (like the FIM row in the picture
             * above)          */
            orig_row_key = jdd;
...
/*         if( orig_row == anat_parent_row ) continue ;*/  /* 14 Dec 1999 */
             ...
             * those that don't exist yet, but have entries in the anat_parent_row can be brought
             *into being now */
            temp_ss = ssl->ssar[find.sess_index];

            for (kvv = vbase + 1 ; kvv <= LAST_VIEW_TYPE ; kvv++)
            {
                if (GET_SESSION_DSET(ss, orig_row_key, kvv) != NULL)
                {
                    continue;
                }
                temp_anat_dset = GET_SESSION_DSET(temp_ss, anat_parent_row_key, kvv);
                if (temp_anat_dset == NULL)
                {
                    continue;
                }
...
/*            orig_row[kvv] = AFNI_follower_dataset( anat_parent_row[kvv] , orig_dset ) ;*/
                num_made++;
            }
...
/*-----------------------------------------------------------------------
 * Scan through a session and force any un-anat_parented functional datasets to have the first
 *available parent, if any.
 * (Note that only the dataset anat_parent pointer is set, not the anat_parent_name string, so that
 *if the dataset is saved, no record of this will be kept.)
 *
...
void AFNI_force_adoption(THD_session *ss, RwcBoolean do_anats)
{
    ...
    if (!ISVALID_SESSION(ss) || ss->num_dsset == 0 || ss->is_collection)
    {
        EXRETURN;
    }
    ...
    if (aset >= 0)
    {
        for (aa = 0 ; aa < ss->num_dsset ; aa++)
        {
            ...
            if (ISVALID_DSET(dset) &&
                ISANAT(dset) &&
                dset->markers != NULL && dset->markers->numset >= aset)
            {
                vv++;
            }
        }
...
/*         dset = ss->dsset_xform_table[ff][vv] ; */ /* function that needs parent */
            ...
            if (!do_anats && ISANAT(dset))
            {
                continue;                                /* 28 Jul 2003 */
            }
            if (DSET_in_global_session(dset))
            {
                continue;                              /* 25 Dec 2001 */
            }
...
/* Hollow out the overlay in place -- 21 Mar 2005 - RWCox. An interior pixel is defined as one whose
 * 4 nearest neighbors are all nonzero.  This editing is done in-place (in the input image).
 * -------------------------------------------------------------------------*/
...
static void mri_edgize(MRI_IMAGE *im)
{
    ...
    if (nx < 3 || ny < 3)
    {
        return;                     /* no interior pixels at all?! */
    }
    switch (im->kind)
    {
    case MRI_byte: {                             /* 09 Dec 2014 */
        ...
        ar    = MRI_BYTE_PTR(im);
        atemp = (byte *)malloc(sizeof(byte) * nxy); if (atemp == NULL)
        {
            return;
        }
    ...
    case MRI_short: {
        ...
        ar    = MRI_SHORT_PTR(im);
        atemp = (short *)malloc(sizeof(short) * nxy); if (atemp == NULL)
        {
            return;
        }
    ...
    case MRI_rgb: {
        ...
        atemp = (rgbyte *)malloc(sizeof(rgbyte) * nxy); if (atemp == NULL)
        {
            return;
        }
        ...
        for (jj = 1 ; jj < ny - 1 ; jj++)
        {
            ...
            for (ii = 1 ; ii < nx - 1 ; ii++)
            {
                if (!RGBZEQ(ajj[ii]) &&
                    ...
                    !RGBZEQ(ajj[ii + 1]) && !RGBZEQ(ajj[ii - 1]))
                {
                    RGBZAS(ar[ii + joff]);
                }
            }
...
static void mri_edgize_outer(MRI_IMAGE *im)
{
    ...
    case MRI_byte: {                             /* 09 Dec 2014 */
...
#endif
        atemp = (byte *)calloc(sizeof(byte), nxy); if (atemp == NULL)
        {
            return;
        }
        for (jj = 1 ; jj < ny - 1 ; jj++)
        {
            ...
            {
                ajj[nx - 1] = 1;
                if (ajm[nx - 1] == 0 || ajp[nx - 1] == 0 || ajj[nx - 2] == 0)
                {
                    atemp[nx - 1 + joff] = 1;
                }
            }
        ...
        for (ii = 1 ; ii < nx - 1 ; ii++)
        {
            ...
            {
                ajj[ii] = 1;
                if (ajp[ii] == 0 || ajj[ii + 1] == 0 || ajj[ii - 1] == 0)
                {
                    atemp[ii + joff] = 1;
                }
            }
        ...
        for (ii = 1 ; ii < nx - 1 ; ii++)
        {
            ...
            {
                ajj[ii] = 1;
                if (ajm[ii] == 0 || ajj[ii + 1] == 0 || ajj[ii - 1] == 0)
                {
                    atemp[ii + joff] = 1;
                }
            }
        ...
        /* now modify atemp so that only points that are zero AND are next to an edge point get a
         * nonzero value == 'outer' edge */
            ...
            for (ii = 1 ; ii < nx - 1 ; ii++)
            {
                if (ar[ii + joff] == 0) /* not inside a blob */
                {
                    if (ajm[ii - 1] != 0 || ajm[ii] != 0 || ajm[ii + 1] != 0 ||
                        ...
                        ajp[ii - 1] != 0 || ajp[ii] != 0 || ajp[ii + 1] != 0)
                    {
                        ar[ii + joff] = 2;
                    }
                }
...
/* Convert a paned color bar to a continuous color scale, for use with AFNI_newnewfunc_overlay [17
 * Jul 2019 - RWC]
 *//*---------------------------------------------------------------------*/
 ...
 * n = slice index , br_fim = data structure to extract slice from. Note that imseq requires that
 *the overlay be an image of shorts. This routine requires that the function and threshold images be
 *bytes, shorts, or floats (complex-valued functional images are not supported).
...
MRI_IMAGE * AFNI_func_overlay(int n, FD_brick *br_fim)
{
    ...
    { int ind;
      ...
      if (scale_factor == 0.0 || PBAR_FULLRANGE)
      {
          scale_factor = 1.0f;
      }
          ...
          im3d->vinfo->thr_use_alpha && pbar->bigmode)
      {
          ...
          AFNI_set_ignore_vedit(0);
          if (mri_equal(im_fim, im_noved))
          {
              mri_free(im_noved); im_noved = NULL;
          }
      }
...

    {
        ...
        if (im_thr != im_fim)
        {
            KILL_1MRI(im_thr);                   /* toss the trash */
        }
        RETURN(im_fim);
    }
    ...
    /**---------- 30 Jan 2003:
     *            if the pbar is in "big" mode, then create an RGB overlay in a separate function
     * ----------**/
...
#define ALWAYS_USE_BIGMODE    1              /* use the newnew function always [17 Jul 2019] */
...

    {
        ...
        else
        {
            if (pbar->big30)
            {
                reject_zero = VEDIT_good(im3d->vedset);
            }
        ...
        {
            flags |= NFO_POS_MASK;                              /* pos only FIM? */
        }
        if (im3d->vinfo->thr_use_boxed)
        {
            flags |= NFO_USE_BOXED;                             /* boxing day?   */
        }
    ...
    case MRI_short: {
        ...
        if (simult_thr)
        {
            ...
            for (ii = 0 ; ii < npix ; ii++)
            {
                ...
                {
                    for (lp = 0 ; lp < num_lp && ar_fim[ii] < fim_thr[lp] ; lp++)
                    {
                        ;                                                     /*nada*/
                    }
                    ar_ov[ii] = fim_ovc[lp];
    ...
    if (ii == npix)
    {
        KILL_1MRI(im_ov);               /* no nonzero values --> no overlay */
    }
...
CLEANUP:
    ...
    if (AFNI_yesenv("AFNI_EDGIZE_OVERLAY"))
    {
        mri_edgize(im_ov);
    }
#endif
                                 ...
                                 int flags)
{
    ...
    if (im_fim == NULL || fimbot >= fimtop || fimcolor == NULL)
    {
        RETURN(NULL);
    }
    ...
    for (ii = 0 ; ii < npix ; ii++)
    {
        ...
        if (ZREJ(val) || (zabove && val > fimtop) || (zbelow && val < fimbot))
        {
            continue;
        }
...
/* This function is only used to alpha fade the pbar image. However, the macros below are used in
 * AFNI_newnewfunc_overlay().
 *//*---------------------------------------------------------------------*/

/* th  = threshold at this voxel fac = scales threshold to be between 0 and 1 */
 ...
 * max value set to 222 to try to make sure there is some distinction between above-threshold voxels
 *and below threshold rebel scum */
...
void AFNI_alpha_fade_mri(Three_D_View *im3d, MRI_IMAGE *im)
{
    ...
    if (!IM3D_OPEN(im3d) || im == NULL || im->kind != MRI_rgb)
    {
        return;
    }
    if (im3d->vinfo->thr_use_alpha <= 0)
    {
        return;
    }
    ...
    for (kk = jj = 0 ; jj < ny ; jj++)
    {
        for (ii = 0 ; ii < nx ; ii++, kk += 3)
        {
            ...
            if (aa < 255.0f)
            {
                ...
                {
                    continue;                                     /* don't alter */
                }
                if (rf == 255.0f && gf == 255.0f && bf == 255.0f)
                {
                    continue;                                     /* black or white */
                }
 ...
 *  - fimcolor = continuous color scale Changed Nov 2018 to allow a mixture of the volume edited FIM
 *(im_fim) and the non-volume edited FIM (im_noved) to determine color and alpha. The output image
 *is in RGBA format.
 * -------------------------------------------------------------------------*/
                                    ...
                                    int flags, float alpha_floor, MCW_DC *dc)
{
    ...
    if (im_fim == NULL || fimbot >= fimtop || fimcolor == NULL || nfimc < 2)
    {
        RETURN(NULL);
    }
    ...
    for (ii = 0 ; ii < npix ; ii++)
    {
        ...
        if (ZREJ(vval) || (zabove && vval > fimtop) || (zbelow && vval < fimbot))
        {
            continue;
        }
        if (do_pos && vval <= 0.0f)
        {
            continue;                         /* 12 Dec 2014 */
        }
        jj = (int)(fac * (fimtop - vval));
#if 0
        INFO_message("v=%f -> jj=%d  fimbot=%f fimtop=%f fac=%f nfimc=%d", vval, jj, fimbot, fimtop,
                     fac, nfimc);
#endif
        if (jj < 0)
        {
    ...
    if (flags & NFO_ALIN_MASK)
    {
        alcode = 1;                               /* linear fade below thresh */
    }
    else if (flags & NFO_AQUA_MASK)
    {
        alcode = 2;                               /* quadratic fade */
    }
    else
    {
        alcode = 0;                               /* no fade = sharp cutoff */
    }
    if (dothr)
    {
        ...
        /* in the following code, note that alcode==0 implies ALFA() = 0.0 implies not visible,
        ...
        case MRI_short: {
            ...
            for (ii = 0 ; ii < npix ; ii++)
            {
                if (kf == MRI_byte)
                {
                    val = (float)bfim[ii]; vval = (float)bvim[ii];
                }
                else if (kf == MRI_short)
                {
                    val = (float)sfim[ii]; vval = (float)svim[ii];
                }
                else
                {
                    val = ffim[ii]; vval = fvim[ii];
                }
                ...
                else if (do_edge)
                {
                    if (!(do_pos && val <= 0.0f))
                    {
                        ear[ii] = 1;                      /* not faded or vedit-ed */
                    }
                }
            }
        }
        ...
        if (do_edge)
        {
            ...
            for (ii = 0 ; ii < npix ; ii++)
            {
                if (ear[ii])
                {
                    ovar[ii].r = rb; ovar[ii].g = gb; ovar[ii].b = bb; ovar[ii].a = 254;
                }
            }
            mri_free(eim);
        }
...
/*-----------------------------------------------------------------------
 * Make an overlay from the TT atlas n = slice index; (ax_1,ax_2,ax_3) = slice orientation codes fov
 * = functional overlay image previously computed if non-NULL, then fov will be overlaid, and will
 *be returned if NULL, then a new image will be created and returned This function is called from
 *AFNI_overlay, when making the combined overlay image for display in a viewer.
 * -- 25 Jul 2001 - RWCox
 * -------------------------------------------------------------------------*/
                                 ...
                                 MRI_IMAGE *fov)
{
    ...
    if (jill)
    {
        INFO_message("Starting AFNI_ttatlas_overlay");
    }
    ...
    if (ttp == NULL)
    {
        if (jill)
        {
            ININFO_message(" params are off - vamoose");
        }
        RETURN(NULL);
    }
 ...
 *   -- when the existing underlay has been altered in some hideous way if the calling widget "w" is
 *NULL, then the calling routine is required to do the redrawing later
 * ------------------------------------------------------------------------*/
...
void AFNI_underlay_CB(Widget w, XtPointer cd, XtPointer cb)
{
    ...
    if (w != NULL)
    {
        bval = AFNI_first_tog(LAST_UNDERLAY_TYPE + 1,
    ...
    if (bval == im3d->vinfo->underlay_type && w != NULL && IM3D_ULAY_COHERENT(im3d))
    {
        EXRETURN;
    }
    ...
    /* Feb 1998: if a receiver is open send it a message that something has altered */
...
void AFNI_choose_dataset_CB(Widget w, XtPointer cd, XtPointer cb)
{
    ...
    if (!IM3D_OPEN(im3d) || w == (Widget)NULL)
    {
        EXRETURN;
    }
                    ...
                    for (vv = FIRST_VIEW_TYPE ; vv <= LAST_VIEW_TYPE ; vv++)
                    {
                        ...
                        if (qset != NULL)
                        {
                            INFO_message("BAD dataset: type=%d view_type=%d ibk=%d bkt=%d",
                                         qset->type, qset->view_type, qset->dblk != NULL,
                                         qset->dblk->type);
                        }
                    }
                    first = 0;
    ...
    else
    {
        ...
        if (is_other)
        {
            ...
            ndset = cs->ndset; if (ndset < 1)
            {
                EXRETURN;
            }
            dset_list = cs->dset; if (dset_list == NULL)
            {
                EXRETURN;
            }
            cbfun = cs->cb; if (cbfun == NULL)
            {
                EXRETURN;
            }
            wpar = w;
        ...
        for (ii = 0 ; ii < ndset ; ii++)
        {
            ...
            else
            {
                for (vv = FIRST_VIEW_TYPE ; vv <= LAST_VIEW_TYPE ; vv++)
                {
                    dset = GET_SESSION_DSET(im3d->ss_now, ii, vv); if (ISVALID_DSET(dset))
                    {
                        break;
                    }
/*            dset = im3d->ss_now->dsset_xform_table[ii][vv]; if( ISVALID_DSET(dset) ) break;*/
        ...
        for (ii = 0 ; ii < ndset ; ii++)
        {
            if (is_other)
            {
                dset = dset_list[ii]; if (!ISVALID_DSET(dset))
                {
                    continue;
                }
            }
                              ...
                              XtPointer cd, MCW_choose_cbs *cbs)
{
    ...
    if (wcall == im3d->vwid->view->choose_sess_pb)
    {
        ...
        if (new_anat < 0)
        {
            new_anat = 0;                /* use 1st if no match */
        }
...
/*            if( ISVALID_3DIM_DATASET(ss_new->dsset_xform_table[ff][new_view]) ) break ;*/
            ...
            if (ff < ss_new->num_dsset)
            {
                new_func = ff;                         /* found one */
            }
        }
        if (new_func < 0)
        {
                          ...
                          THD_3dim_dataset *rset)
{
    ...
    if (AFNI_yesenv("AFNI_NO_OBLIQUE_WARNING"))
    {
        EXRETURN;
    }

    if (AFNI_yesenv("AFNI_ONE_OBLIQUE_WARNING") && num_warn)
    {
        EXRETURN;
    }
    ...
    else
    {
        ...
        else
        {
            warncombos = (char *)realloc(warncombos,
                                         sizeof(char) *
                                         (strlen(sidcombo) + strlen(warncombos) + 1));
...
/*----------------------------------------------------------------------
 * Routines to close and open a file selection dialog associated with the im3d viewer.
 * ------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------
 * After this is called, the calling routine must set the callbacks, window title, etc.,
 *appropriately.
 * ------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------
 * Start getting ready to read a new session in.  This is the CB for the "Read Sess" button.  We'll
 *get a filename from the user, and process it in another routine.
 * ------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------
 * Got a button press from the file selection dialog, so process it (maybe read in a new session!)
 * -----------------------------------------------------------------------*/
...
void AFNI_finalize_read_sess_CB(Widget w, XtPointer cd, XtPointer cb)
{
    ...
    if (!IM3D_OPEN(im3d) || cbs == NULL)
    {
        EXRETURN;                                    /* 04 Feb 2008 */
    }
    switch (cbs->reason)
    {
...
/*----------------------------------------------------------------
 * Start getting ready to read a new timeseries in.  This is the CB for the "Read 1D" button.  We'll
 *get a filename from the user, and process it in another routine.
 * ------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------
 * Got a button press from the file selection dialog, so process it (maybe read in a new
 *timeseries!)
 * -----------------------------------------------------------------------*/
...
/*        ss->dsset_xform_table[nn][vv] = dset ;*/
            ss->num_dsset++; nds++;
            if (vv == im3d->vinfo->view_type && na == -1)
            {
                na = nn;
            }
        }
    ...
    if (na >= 0)
    {
        im3d->vinfo->anat_num = na;           /* 1st new anat in current view */
    }
    if (nf >= 0)
    {
        im3d->vinfo->func_num = nf;           /* 1st new func in current view */
    }
...
void AFNI_rescan_timeout_CB(XtPointer client_data, XtIntervalId *id)
{
    ...
    if (!block_rescan)
    {
        AFNI_rescan_all_CB(NULL, NULL, NULL);
    }
...
/*!
 * Re-read the session indexed by "sss".  THE OLD WAY. Much of this code is taken from
 *AFNI_read_inputs. WARNING: this will do bad things if the user deletes the session directory or
 *the current active datasets within it before trying this.  On the other hand, if the user is that
 *stupid, bad things will probably have happened to him already (like being unable to open dataset
 *files, or being unable to tie his shoes correctly, or being named Mike Beauchamp).
 * ------------------------------------------------------------------------*/
...
static int AFNI_rescan_session_OLD(int sss)    /* the old way */
{
    ...
    if (!ISVALID_SESSION(old_ss) || old_ss->is_collection)
    {
        RETURN(0);
    }

    if (old_ss == GLOBAL_library.session)
    {
        RETURN(0);                                     /* 21 Dec 2001 */
    }
    if (!THD_is_directory(old_ss->sessname))
    {
        RETURN(0);                                        /* 02 Jun 2016 */
    }

    /*--- Make sure that the dataset choosers are closed. Since these are just instances of the
     * generic strlist chooser, and we can't tell what is being chosen just now, we'll just forcibly
     * close the strlist chooser no matter what. ---*/
...
/*               DSET_MARK_FOR_DEATH( old_ss->dsset_xform_table[ii][vv] ) ;*/
    ...
    /*--- but before actual deletion, find the active datasets in each main controller window ---*/
...
/*!
 * Re-read the session indexed by "sss".  THE NEW WAY. Much of this code is taken from
 *AFNI_read_inputs().
 *
 * WARNING:
 *  - This will do bad things if the user deletes the session directory or the current active
 *datasets within it before trying this.
 *  - On the other hand, if the user is that stupid, bad things will probably have happened to him
 *already (like being unable to open dataset files, or being unable to tie his shoes correctly).
 *
 * 28 Dec 2002: modified extensively to not clobber existing pointers to datasets, but instead to
 *insert new datasets into the existing session -- RWCox (MX&HNY)
 * ------------------------------------------------------------------------*/
...
static int AFNI_rescan_session_NEW(int sss)     /* the new way */
{
    ...
    new_ss = THD_init_session(old_ss->sessname);
    if (!ISVALID_SESSION(new_ss))
    {
        RETURN(0);
    }                                            /* this is BAD */

    /*--- scan datasets and remove those that already exist in this session ---*/
...
/*     new_dset = new_ss->dsset_xform_table[ii][vv] ;*/
    ...
    /*--- now scan survivors and put them at the end of the existing session ---*/

    for (ii = 0 ; ii < new_ss->num_dsset ; ii++)
    {
        for (vv = 0 ; vv <= LAST_VIEW_TYPE ; vv++) /* see if row is empty */
        {
            if (GET_SESSION_DSET(new_ss, ii, vv) != NULL)
            {
                break;
            }
        }
...
/*     AFNI_inconstancy_check(NULL,new_ss->dsset_xform_table[ii][vv]) ;*/  /* 06 Sep 2006 */
        ...
        if (nr >= THD_MAX_SESSION_SIZE)
        {
            break;                                 /* old session is full */
        }
...
/*       old_ss->dsset_xform_table[nr][vv] = new_ss->dsset_xform_table[ii][vv];*/
    ...
    for (ii = 0 ; ii < old_ss->num_dsset ; ii++)
    {
        for (vv = 0 ; vv <= LAST_VIEW_TYPE ; vv++)
        {
            ...
            if (temp_dset != NULL)
            {
                DSET_unload(temp_dset);
            }

/*     if( old_ss->dsset_xform_table[ii][vv] != NULL )
 * DSET_unload(old_ss->dsset_xform_table[ii][vv]);*/
        }
    }
        ...
        for (nadd = inew = 0 ; inew < nnew ; inew++)
        {
            ...
            for (jold = 0 ; jold < nold ; jold++)
            {
                ...
                if (oldim != NULL && oldim->name != NULL &&              /* break out of loop */
                    strcmp(oldim->name, newim->name) == 0)
                {
                    break;                                    /* when new == old */
                }
            }
        ...
        for (nadd = inew = 0 ; inew < nnew ; inew++)
        {
            ...
            for (jold = 0 ; jold < nold ; jold++)
            {
                ...
                if (oldel != NULL &&                                 /* break out of loop */
                    strcmp(oldel->filename, newel->filename) == 0)
                {
                    break;                                            /* when new == old */
                }
            }
...
/*----------------------------------------------------------------
 * Used to force redisplay when some datamode parameters have been altered
 * ------------------------------------------------------------------*/
...
void AFNI_do_many_writes(Widget wpar, XtPointer cd, MCW_choose_cbs *cbs)
{
    ...
    for (ib = 0 ; ib < cbs->nilist ; ib++)
    {
        ...
        if (DSET_WRITEABLE(dset))
        {
            ...
            if (!good)  /* bad news! */
            {
                ...
                /** otherwise, check if the dataset just done is the active anatomy or function in
                 * any open controller window **/
            }
            else
            {
                for (cc = 0 ; cc < MAX_CONTROLLERS ; cc++)
                {
                    ...
                    if (dset == qq3d->anat_now)
                    {
                        ccanat[cc] = 1;                       /* flag their  */
                    }
                    if (dset == qq3d->fim_now)
                    {
                        ccfunc[cc] = 1;                       /* controllers */
                    }
                }
            }
        }
    ...
    /** for any controllers whose active datasets were written out, we need to set the "See Brick"
     * buttons to be sensitive.     **/
...
void AFNI_writeout_dataset(THD_3dim_dataset *dset, char *prefix)
{
    ...
    if (!ISVALID_DSET(dset) || !THD_filename_ok(prefix))
    {
        EXRETURN;
    }
...
/*------------------------------------------------------------------
 * (re)compute and (re)write a dataset to disk, with the indicated geometric parameters.  Note that
 *the filenames for the save should already be initialized in the dset->dblk->diskptr.  Also, note
 *that the dataset's "permanent" dataxes will be remade to fit the new geometry.
 * --------------------------------------------------------------------*/
                                  ...
                                  THD_dataxes *daxes, int resam_mode)
{
    ...
    *(dset->daxes)     = *(daxes);               /* Make daxes permanent */
    ...
    /* purge the datablock that now exists, then delete the file on disk that now exists (if any) */

    DSET_unlock(dset);   /* Feb 1998 */
    PURGE_DSET(dset);
    COMPRESS_unlink(dkptr->brick_name);

    /* refashion its brick data structure, which requires first saving in a temporary array the
     * datum type for each sub-brick    */
    ...
    /*--- open the output file (N.B.: much of the following code is from THD_write_datablock) ---*/
 ...
 * it will mark for death the descendants of datasets that are about to be destroyed themselves (as
 *noted by their "death_marks").
 *
 * Note that this routine is not set up for recursive massacres:
 * that is, it doesn't try to find grandchildren that should also be killed.  That could be done by
 *calling this routine more than once, until no changes occur.
 *
 * Note also that this routine does not actually destroy any datasets. That is done by
 *AFNI_andersonville.
 *
 * 31 Jan 1995: altered to avoid destruction of a dataset without a warp parent, since that dataset
 *cannot be recreated.
 * 21 Dec 2001: modified to use DSET_MARK_FOR_DEATH(), which will not let a dataset marked for
 *immortality be killed.
 * --------------------------------------------------------------------*/
...
/****************************************************************
 * 3/24/95: range controls in func panel for user management of the conversion of pbar values
 *[-1..1] to thresholds for the coloring of functional data
 *****************************************************************/
...
/*-----------------------------------------------------------
 * find the autorange (default value) and make a label for the autorange control widget
 * -------------------------------------------------------------*/
...
void AFNI_range_bbox_CB(Widget w, XtPointer cd, XtPointer cb)
{
    ...
    if (!IM3D_OPEN(im3d) ||
        w != im3d->vwid->func->range_bbox->wbut[RANGE_AUTOBUT])
    {
        EXRETURN;
    }
...
void AFNI_perc_bbox_CB(Widget w, XtPointer cd, XtPointer cb)
{
    ...
    if (!IM3D_OPEN(im3d) ||
        w != im3d->vwid->func->perc_bbox->wbut[PERC_AUTOBUT])
    {
        EXRETURN;
    }
...
void AFNI_inten_bbox_CB(Widget w, XtPointer cd, XtPointer cb)
{
    ...
    if (!IM3D_OPEN(im3d) ||
        w != im3d->vwid->func->inten_bbox->wbut[PBAR_MODEBUT])
    {
        EXRETURN;
    }
...
void AFNI_force_bucket_label_resize(int ff)
{
    force_label_resize = ff;
}
...
char * AFNI_bucket_label_CB(MCW_arrowval *av, XtPointer cd)
{
    ...
    static char *     fmt[3] = { NULL, NULL, NULL }, sfmt[THD_MAX_SBLABEL],
                blab[26 + THD_MAX_SBLABEL];
    int        nlab;
    ...
    if (ISVALID_DSET(dset) && (dset != dset_last || force_label_resize))
    {
        ...
        for (kk = 0 ; kk < nvals ; kk++)
        {
            ...
            if (lab != NULL)
            {
                blw = strlen(lab); if (blw > mblw)
                {
                    mblw = blw;
                }
            }
        }
        if (mblw > THD_MAX_SBLABEL)
        {
            mblw = THD_MAX_SBLABEL;
        }
        nsiz_last = mblw;
    }
...
#define ISIZE    1024
    ...
    {
        DBG_trace = (DBG_trace + 1) % 3;                                     /* Aug 23 1998: cycle
                                                                              * between 0,1,2 */
        ...
        if (DBG_trace == 2)
        {
            STATUS("XSynchronize enabled");
        }
        else
        {
            STATUS("XSynchronize disabled");
        }
    }
#endif
    ...
    else if (w == im3d->vwid->dmode->misc_environ_pb)  /* 20 Jun 2000 */
    {
        ...
        if (plint == NULL)
        {
            ...
            if (plint == NULL)
            {
                BEEPIT; WARNING_message("WTF?!"); EXRETURN;
            }
    ...
    else if (w == im3d->vwid->dmode->misc_1dchain_pb)  /* 07 Aug 2001 */
    {
        ...
        if (cbs == NULL)
        {
            EXRETURN;               /* only for a setup call */
        }
        /* code below is from PLUG_startup_plugin_CB() in afni_plugin.c */
    ...
    else if (w == im3d->vwid->func->icalc_pb)  /* 18 Sep 2009 */
    {
        ...
        if (!im3d->vwid->func->iwid->is_open)
        {
...
int find_subbrick_with_label(THD_3dim_dataset *dset, char *lstr)
{
    ...
    if (!ISVALID_DSET(dset) || lstr == NULL || *lstr == '\0')
    {
        return -1;
    }
...
void AFNI_hidden_CB(Widget w, XtPointer cd, XtPointer cbs)
{
    ...
    else if (w != NULL && w == im3d->vwid->prog->hidden_music_pb)  /* 07 Aug 2019 */
    {
        if (GLOBAL_library.sound_player != NULL && GLOBAL_library.local_display)
        {
            ...
            if (nmusic < 99)
            {
                osfilt3_func(2 * nmusic, 0.0, 1.0, qar);            /* smooth */
            }
            else
            {
            ...
            if (ncall == 0)
            {
                mri_play_sound_notify(1);              /* only the first time */
            }
...
Retry_dd:
        ...
        {
            for (qq = 0 ; qq < ndold && dold[qq] != dd ; qq++)
            {
                ;                                                /* nada */
            }
            if (qq < ndold)
            {
                goto Retry_dd;                /* was recent */
            }
...
static void AFNI_find_poem_files(void)  /* 15 Oct 2003 */
{
    ...
    do{
        ...
        if (THD_is_directory(fdir))
        {
            strcat(fdir, "/"); udir = fdir;
        }
        else
        {
            udir = edir;
...
{
    if (!IM3D_OPEN(im3d) || fac < 0.0f || fac > 2.0f)
    {
        return;
    }
...
#ifdef WANT_RWCOX_IMAGE
        else if (!NO_frivolities && event->button == Button1)
        {
            RWCOX_popper();
        }
#endif
...
#ifdef USE_SKIT
        else if (!NO_frivolities && event->button == Button2)
        {
            SKIT_popper(im3d);
        }
        else if (!NO_frivolities && event->button == Button4)
        {
            AFNI_alter_controller_bg(im3d, 0.960000f);
        }
        else if (!NO_frivolities && event->button == Button5)
        {
            AFNI_alter_controller_bg(im3d, 1.041667f);
        }
#endif
    }
...
void AFNI_hidden_pts_CB(Widget w, XtPointer cd, MCW_choose_cbs *cbs)
{
    ...
    if (!ISVALID_3DIM_DATASET(dset_now))
    {
        EXRETURN;
    }
    ...
    case PTS_READ_XYZ:
        ...
        do {
            ...
            if (ii == 3)                           /* all 3 good! */
            {
                if (ijk_option)
                {
                    ADD_IVEC_TO_VLIST(sv, ijk_vec);
                }
                else
                {
                    ADD_FVEC_TO_VLIST(sv, xyz_vec);
                }
            }
