...
void despike7pp_func(int num, double to, double dt, float *vec)
{
    ...
    for (ii = 1 ; ii < num ; ii++)
    {
        dvv[ii - 1] = vec[ii] - vec[ii - 1];
    }
    despike7_func(num - 1, to, dt, dvv);
    for (ii = 1 ; ii < num ; ii++)
    {
        vec[ii] = vec[ii - 1] + dvv[ii - 1];
    }
    free(dvv); return;
}
...
void DES_despike25(int num, double to, double dt, float *vec)
{
    ...
    if (num < 25)
    {
        despike9_func(num, to, dt, vec); return;
    }
...
void hrfdecon_func(int num, double to, double dt, float *vec)
{
    ...
    nkern = 1 + (int)(16.0f / dt); if (nkern >= num / 2)
    {
        return;
    }
    ...
    nbase = 1 + (int)(num * dt / 100.0f); if (nbase < 3)
    {
        nbase = 3;
    }
...
void absfft_func(int num, double to, double dt, float *vec)
{
    ...
    for (ii = 0 ; ii < num ; ii++)
    {
        cx[ii].r = vec[ii] - (f0 + f1 * ii + f2 * ii * ii); cx[ii].i = 0.0;
    }
    for (      ; ii < ncx ; ii++)
    {
        cx[ii].r = cx[ii].i = 0.0;
...
void winsor9_box_func(int nx, int ny, double dx, double dy, float *ar)
{
    ...
    for (jj = 0 ; jj < ny ; jj++)
    {
        ...
        aa[6] = ajp[nx - 2]; aa[7] = ajp[nx - 1]; aa[8] = ajp[nx - 1];
        isort_float(9, aa);
        if (ar[nx - 1 + joff] < aa[2])
        {
            ar[nx - 1 + joff] = aa[2];
        }
        else if (ar[nx - 1 + joff] > aa[6])
        {
            ar[nx - 1 + joff] = aa[6];
        }
    }
...
void median21_box_func(int nx, int ny, double dx, double dy, float *ar)
{
    ...
    for (jj = 1 ; jj < ny - 1 ; jj++)
    {
        ...
        for (ii = 2 ; ii < nx - 2 ; ii++)
        {
            ...
            aa[3]  = ajm[ii - 2]; aa[4] = ajm[ii - 1]; aa[5] = ajm[ii]; aa[6] = ajm[ii + 1];
            aa[7]  = ajm[ii + 2];
            aa[8]  = ajj[ii - 2]; aa[9] = ajj[ii - 1]; aa[10] = ajj[ii]; aa[11] = ajj[ii + 1];
            aa[12] = ajj[ii + 2];
            aa[13] = ajp[ii - 2]; aa[14] = ajp[ii - 1]; aa[15] = ajp[ii]; aa[16] = ajp[ii + 1];
            aa[17] = ajp[ii + 2];
...
void winsor21_box_func(int nx, int ny, double dx, double dy, float *ar)
{
    ...
    for (jj = 1 ; jj < ny - 1 ; jj++)
    {
        ...
        for (ii = 2 ; ii < nx - 2 ; ii++)
        {
            ...
            if (ar[ii + joff] < aa[kbot])
            {
                ar[ii + joff] = aa[kbot];
            }
            else if (ar[ii + joff] > aa[ktop])
            {
                ar[ii + joff] = aa[ktop];
            }
        }
...
void fft2D_absfunc(int nx, int ny, double dx, double dy, float *ar)
{
    ...
    for (jj = 0 ; jj < ny ; jj++)
    {
        ...
        for (ii = 0; ii < nx  ; ii++)
        {
            cpt->r = *fpt * fi; cpt->i = 0.0; cpt++; fpt++; fi = -fi;
        }
        for (    ; ii < nxup; ii++)
        {
            cpt->r = cpt->i = 0.0; cpt++;
    ...
    for (ii = 0 ; ii < nxup ; ii++)
    {
        for (jj = 0 ; jj < nyup ; jj++)
        {
            cpt[jj] = cxar[ii + jj * nxup];
        }
        csfft_cox(-1, nyup, cpt);
        for (jj = 0 ; jj < nyup ; jj++)
        {
            cxar[ii + jj * nxup] = cpt[jj];
        }
    }
