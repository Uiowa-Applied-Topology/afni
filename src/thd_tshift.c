...
/*! Time shift a dataset to a common origin.  Stores the data on top of the input dataset!  Return
 * value is 0 for good processing, nonzero for errors. This routine is like doing the default
 * actions of 3dTshift (from which the code is adapted).  -- RWCox -- 15 Feb 2001
 * -------------------------------------------------------------------------------*/

/*-- At present, these static variables are not alterable from the outside; they correspond to the
 * command line options of 3dTshift. In the future, there may be a way to changes these to affect
 * things. --*/
...
static int    TS_mode   = MRI_FOURIER; /* one of MRI_FOURIER MRI_CUBIC MRI_QUINTIC MRI_HEPTIC
                                        * MRI_LINEAR             */
...
int THD_dataset_tshift(THD_3dim_dataset *TS_dset, int ignore)
{
    ...
    if (!ISVALID_DSET(TS_dset) || ignore < 0)
    {
        RETURN(1);
    }
    ...
    DSET_load(TS_dset); if (!DSET_LOADED(TS_dset))
    {
        RETURN(1);
    }
...

    {
...

        {
            ...
            for (fmin = fmax = far[ignore], jj = ignore + 1 ; jj < ntt ; jj++)
            {
                if (far[jj] < fmin)
                {
                    fmin = far[jj];                      /* range of data: after */
                }
                else if (far[jj] > fmax)
                {
                    fmax = far[jj];
            ...
            for (jj = ignore ; jj < ntt ; jj++)
            {
                if (far[jj] < fmin)
                {
                    far[jj] = fmin;                              /* clip to input range */
                }
                else if (far[jj] > fmax)
                {
                    far[jj] = fmax;
