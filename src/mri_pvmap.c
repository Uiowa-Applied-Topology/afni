...
/* This function computes the "correlation" of vector x with the pair of vectors a and b (presumed
 * to be L2 norm 1).
 *//*---------------------------------------------------------------*/
...
static float r2D(UAint n, float *a, float *b, float *x)
{
    ...
    if (sax > 1.0f)
    {
        sax = 1.0f;              /* should be oompossibull */
    }
#endif
    return sax;
}
...
/* Inputs:
 *   nx  = number of points per time series (>= 9) ny  = number of time series (>= 9) vlist[j] =
 * j-th time series (j=0..ny-1) Output:
 *   newly malloc-ed array, length ny, of the pvmap intensity (r2D) for each time series
 *//*-------------------------------------------------------------*/
...
static float * mri_veclist_to_pvmap(UAint nx, UAint ny, float **vlist)
{
    ...
    /* random number seed used to initialize iterating vectors for the principal vector pair */
...
/* Modified extensively to compute in time series length chunks of maximum length NXMAX, then
 * average (if more than one chunk). Longer time series tend to take a very long time to compute.
...
MRI_IMAGE * mri_vec_to_pvmap(MRI_IMAGE *inim)
{
    ...
    nx = (Aint)inim->nx; if (nx < 9)
    {
        RETURN(NULL);
    }
    ny = (Aint)inim->ny; if (ny < 9)
    {
        RETURN(NULL);
    }
    ...
    /* manufacture output image with no data 'empty', then put the output array into it as its data
     * (sneaky, huh?) */
