 ...
 *
 * File:    3dbucket.c Author:  R. W. Cox Date:    17 December 1997
 *
 *
 * Mod:     Changes to implement "-glueto" command option. Also, modified output to preserve
 *sub-brick labels. Author:  B. D. Ward Date:    04 February 1998
 *
 * Mod:     If more than one input dataset, copy command line history from the first input dataset
 *to the output bucket dataset. Date:    14 March 2002
 *
 * Mod:     When verifying view type extension for -glueto dataset, scan from the end (in case there
 *are extra '+' characters). Author:  R. C. Reynolds Date:    30 October 2003
 *
 */
/*---------------------------------------------------------------------------*/
    ...
    while (nopt < argc)
    {
            ...
            strncmp(argv[nopt], "-aglueto", 5) == 0) /* ZSS April 16 2010 */
        {
            ...
            if (ok)
            {
                ...
                {
                    if (!strncmp(str, VIEW_codestr[ii], 4))
                    {
                        break;
                    }
                }
            ...
            /*----- Note: no "continue" statement here.  File name will now be processed as an input
             * dataset -----*/
        }
        ...
        /* akin to 3dTcat, handle failure in open_one (todo: remove open_one from these programs) 18
         * Apr 2016 [rickr] */
        if (dset == NULL)
        {
...
int main(int argc, char *argv[])
{
    ...
    if (BUCK_verb)
    {
        printf("-verb: output will have %d sub-bricks\n", new_nvals);
    }
    ...
    for (iv = 1 ; iv < ninp ; iv++)
    {
        ...
        if (DSUB(iv)->dblk->brick_fdrcurve)
        {
            have_fdr = 1;
        }
    ...
    if (have_fdr)
    {
        ...
        if (BUCK_verb)
        {
            printf("-verb: adding fdrcurve list\n");
        }
    ...
    for (ids = 0 ; ids < ninp ; ids++)
    {
        ...
        for (iv = 0 ; iv < nv ; iv++)
        {
            ...
            if (!BUCK_dry)
            {
                ...
                if (have_fdr)
                {
                    ...
                    {
                        new_dset->dblk->brick_fdrcurve[ivout] = NULL;
                    }
                    ...
                    {
                        new_dset->dblk->brick_mdfcurve[ivout] = NULL;
                    }
                }
                ...
                if (BUCK_verb)
                {
                    printf("-verb: copied %s[%d] into %s[%d]\n",
        ...
        /** loop over all bricks in input dataset and unload them if they aren't going into the
         * output (not required, but is done to economize on memory) **/
        ...
        if (!BUCK_dry && nv < DSET_NVALS(dset))
        {
            for (kv = 0 ; kv < DSET_NVALS(dset) ; kv++) /* all input sub-bricks */
            {
                ...
                if (iv == nv)
                {
                    ...
                    if (BUCK_verb)
                    {
                        printf("-verb: unloaded unused %s[%d]\n",
    ...
    if (!BUCK_dry)
    {
        if (BUCK_verb)
        {
            if (have_fdr)
            {
                fprintf(stderr, "-verb: added %d of %d fdr curves\n",
                        nfdr, new_nvals);
            }
        ...
        if (BUCK_verb)
        {
            fprintf(stderr, "-verb: wrote output: %s\n", DSET_BRIKNAME(new_dset));
        }
    }
