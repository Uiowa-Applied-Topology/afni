...
void GRAF_get_setup(MCW_graf *gp, int *nh, int *xh, int *yh, int *spl)
{
    ...
    if (gp == NULL || nh == NULL || xh == NULL || yh == NULL || spl == NULL)
    {
        return;
    }
...
/*--------------------------------------------------------------------
 * Check to see if graph function has changed.  As a byproduct, saves the current function in the
 *backup place, so that if called twice in a row, the answer the second time is always no.
 * ----------------------------------------------------------------------*/
...
#  define MyCursor    None
        ...
        for (h = 0; h < gp->nhands; h++)
        {
            if (PTINRECT(mx * 2, (127 - my) * 2,
                         gp->hands[h].x - 5, gp->hands[h].y - 5, 11, 11))
            {
                break;
            }
        }
        ...
        if (h == gp->nhands)
        {
            return;                   /* meaningless click - not in a "hand" */
        }
        ...
        {
            if (!(mask & Button1Mask) && !(mask & Button3Mask))
            {
            ...
            {
                if (newx <= gp->hands[h - 1].x)
                {
                    newx = gp->hands[h - 1].x + 1;
                }
                if (newx >= gp->hands[h + 1].x)
                {
                    newx = gp->hands[h + 1].x - 1;
        ...
        if (grab)
        {
            XUngrabPointer(gp->dc->display, (Time)CurrentTime);
        }
...
void drawGraf(MCW_graf *gp, int erase)
{
    ...
    for (i = 0, pt = pts; i < 256; i += 2, pt++)
    {
        ...
        if (i == 0)
        {
            i = -1;     /* kludge to get sequence 0,1,3,5, ... 253,255 */
        }
    }
...
void GRAF_handle_CB(MCW_arrowval *av, XtPointer client_data)
{
    ...
    /* find (middle) point whose x-distance to previous and next points is minimal.  Delete that
     * point */
...
{
    /* given arrays of data points x[0..n-1] and y[0..n-1], computes the values of the second
     * derivative at each of the data points y2[0..n-1] for use in the splint function */
...
double EvalSpline(int xa[], int ya[], double y2a[], int n, double x)
{
    ...
    if (h == 0.0)
    {
        return ya[klo];         /* error condition, should not happen */
    }
    a = (xa[khi] - x) / h;
    b = (x - xa[klo]) / h;
...
void xvbcopy(char *src, char *dst, size_t len)
{
     ...
     *
     * if src=dst, they overlap completely, but nothing needs to be moved if src<dst and src+len>dst
     *then they overlap
     */
                             ...
                             float ybot, float ytop, int x, int y)
{
    ...
    AV_fval_to_char(xx, xbuf); xb = xbuf; if (*xb == ' ')
    {
        xb++;
    }
    AV_fval_to_char(yy, ybuf); yb = ybuf; if (*yb == ' ')
    {
        yb++;
    }
