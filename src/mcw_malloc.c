...
/*--------------------------------------------------------------------------
 * 24 Jan 2001: Modified heavily to use a hash table instead of a linear array to store the data
 *about each allocated block.
 * 16 Feb 2001: Modified to include some debug traceback info (if USE_TRACING is defined - searce
 *for TRAC to see code).
 * -- RWCox
 * ----------------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------
 * Find an empty entry in the hash table list [jj] and return a pointer to it.  Will create the
 *entry, if need be.
 * -------------------------------------------------------------------*/
...
/*----------------------------------------------------------------
 * Add an entry to the hash table, given the address, the user's size, and the filename and line
 *number.
 * ------------------------------------------------------------------*/
...
/* fprintf(stderr,"malloc_track(%u,%s,%d)\n",(unsigned int)n,fn,ln) ; */
      ...
      if (fred == NULL)                                    /* real bad news */
      {
          ...
          if (val > 0)
          {
              fprintf(stderr, "*** current total usage=%lld bytes\n", val);
          }
          goto IAMDONE;
      }
...
static void * calloc_track(size_t n, size_t m, char *fn, int ln)
{
    ...
    fred = malloc_track(nn, fn, ln); if (fred == NULL)
    {
        return NULL;
    }
    memset(fred, 0, nn);
...
#pragma omp critical (MCW_MALLOC_stat)
    { int jj, kk, nptr = 0; long long nbyt = 0;
      ...
      sprintf(buf, "chunks=%d bytes=%lld", nptr, nbyt); } /* end OpenMP critical */
    return buf;
}
      ...
      for (ii = jj = 0 ; jj < SLOTS ; jj++)
      {
          for (kk = 0 ; kk < nhtab[jj] ; kk++)
          {
              if (htab[jj][kk].pmt != NULL)
              {
                  if (size_sort)
                  {
                      ss[ii] = htab[jj][kk].psz;    /* save size */
                  }
                  else
                  {
 ...
 *  the halt is meant to be temporary. Had to add this for SUMA to get around the problem of using
 *allocate2D with large numbers of pointers. ZSS 03/04
 *  --------------------------------------- */
...
void * mcw_malloc(size_t n, char *fnam, int lnum)
{
    if (use_tracking)
    {
        return malloc_track(n, fnam, lnum);
    }
    else
    {
        return malloc(n);
...
void * mcw_calloc(size_t n, size_t m, char *fnam, int lnum)
{
    if (use_tracking)
    {
        return calloc_track(n, m, fnam, lnum);
    }
    else
    {
        return calloc(n, m);
...
#if 0
        { char buf[1024]; sprintf(buf, "called mcw_strdup() [%s line %d p=%p]", fnam, lnum, ssout);
          STATUS(buf); }
#endif
    }
    else
...
char * mcw_XtMalloc(RwcCardinal n, char *fnam, int lnum)
{
    if (use_tracking)
    {
        return (char *)malloc_track(n, fnam, lnum);
    }
    else
    {
        return (char *)RwcMalloc(n);
    }
}
...
char * mcw_XtCalloc(RwcCardinal n, RwcCardinal m, char *fnam, int lnum)
{
    if (use_tracking)
    {
        return (char *)calloc_track(n, m, fnam, lnum);
    }
    else
    {
        return RwcCalloc(n, m);
    }
}

#else  /* just something, anything [fake functions] */

long long mcw_malloc_total(void)
{
    return 0;
}
...
void mcw_malloc_dump_sort()
{
    return;
}

void mcw_malloc_dump()
{
    return;
}
