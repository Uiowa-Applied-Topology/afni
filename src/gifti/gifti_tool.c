 ...
 *
 * return  1 : success, but exit program 0 : success, continue
 *        -1 : failure, terminate
 */
static int process_opts(int argc, char *argv[], gt_opts *opts)
{
    ...
    /* for gifti_tool, non-writing functions will default to keeping the original Array Index Order
     * (so we overwrite the gifti_xml library default), while writing functions default to
     * rearranging 1 May 2017 [rickr] */
    ...
    {
        gifti_set_perm_by_iord(opts->perm_by_iord);
    }
...
int gt_write(gt_opts *opts)
{
    ...
    /* since we plan to write a dataset, default to converting to RowMajor order, unless the user
     * specified otherwise       1 May 2017 [rickr] */
    if (opts->perm_by_iord == -1)
    {
...
int gt_write_dataset(gt_opts *opts, gifti_image *gim)
{
        ...
        opts->encoding <= GIFTI_ENCODING_MAX)
    {
        ...
        {
            /* when modifying enconding, if we are changing _from_ external, then clear the external
             * filename and offset */
 ...
 * if name == MAKE_IM   : create a new dataset else if we have name : read one dataset
 *
 * Note that name may have the form dset[int list], where the integer list is used to create the
 *DataArray list.
 *
 ...
 *
 * Note that the DA list selection requires reading the dataset twice, first to compute the number
 *of DA elements.
 */
...
/* init any options that should not default to 0 (so 0 means something, or the default is non-zero)
 */
static int init_opts(gt_opts *opts)
{
...
/* if dlist contains 1 element, write out as 2-D list, else each DA must have only 1 dimension */
 ...
 *
 * This essentially transposes DA elements and DA data (Row/Column major order is not handled, as
 *that is within each DA element).  The point is to print the time series for each node on a single
 *line.
 *
...
int gt_modify_dset(gt_opts *opts, gifti_image *gim)
{
    ...
    if (!errs && opts->mod_ind_ord)
    {
        ...
        if (c > 0)
        {
            fprintf(stderr, "++ converted data to %s\n",
