...
static char *gifti_history[] = {
    ...
    "1.17 19 Feb, 2021: promote fabs() args explicitly to avoid warnings\n"
};

static char gifti_version[] = "gifti library version 1.17, 19 Feb, 2021";
...
/*! gifti_type_list is an array of gifti_type_ele structs which list, for each type, the bytes per
 * value, swapsize and corresponding name string (these type values are defined in nifti1.h) */
...
{
    G.verb = level;  return 1;
}

int gifti_get_indent(void)
{
    return gxml_get_indent();
}

int gifti_set_indent(int level)
{
    return gxml_set_indent(level);
}

int gifti_get_b64_check(void)
{
    return gxml_get_b64_check();
}

int gifti_set_b64_check(int level)
{
    return gxml_set_b64_check(level);
}

int gifti_get_update_ok(void)
{
    return gxml_get_update_ok();
}

int gifti_set_update_ok(int level)
{
    return gxml_set_update_ok(level);
}

int gifti_get_zlevel(void)
{
    return gxml_get_zlevel();
}
...
{
    return gxml_get_perm_by_iord();
}

int gifti_set_perm_by_iord(int level)
{
    return gxml_set_perm_by_iord(level);
}

int gifti_get_xml_buf_size(void)
{
    return gxml_get_buf_size();
}
...
/*----------------------------------------------------------------------
 *! This is the main dataset reading routine.  Read a GIFTI dataset and return the corresponding
 *gifti_image structure.
 *
 *  Reading data is optional, via the read_data flag. User variables should already be set (via
 *accessor functions).
 *
 *  return an allocated gifti_image struct on success, NULL on error
 *//*-------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------
 *! Similar to gifti_read_data, this function also takes an integer list of DataArray indices to
 *populate the gifti_image structure with.
 *
 *  The indices are be zero-based, can have repeats and can be in any order. A simple example to
 *read 3 DA elements (with 2 repeats) might be:
 *
 *    gifti_image * gim;
...
int gifti_free_image(gifti_image *gim)
{
    ...
    if (gim->version)
    {
        free(gim->version);  gim->version = NULL;
    }
...
int gifti_free_DataArray(giiDataArray *darray)
{
    ...
    if (darray->ext_fname)
    {
        free(darray->ext_fname); darray->ext_fname = NULL;
    }
...
/*----------------------------------------------------------------------
 *! free the CoordSystem array from a DataArray passing NULL is okay
 *//*-------------------------------------------------------------------*/
...
int gifti_free_CoordSystem(giiCoordSystem *cs)
{
    ...
    if (cs->dataspace)
    {
        free(cs->dataspace); cs->dataspace = NULL;
    }
    if (cs->xformspace)
    {
        free(cs->xformspace); cs->xformspace = NULL;
    }
...
/*----------------------------------------------------------------------
 *! initialize an existing DataArray structure given a list of name=value attribute pairs
 *
 *  if alen > 0, consider it the length of the attr list else         process until attr[i] == NULL
 *
 *  if add_to_extras, add any bad attribute pairs to ex_atrs else              whine about any bad
 *ones and return
 *//*-------------------------------------------------------------------*/
...
int gifti_valid_dims(const giiDataArray *da, int whine)
{
    ...
    /* verify that num_dim is not too big, the most significant dimension is not allowed to be 1
 ...
 *
 *  This function is meant to index into one of the gifti_*_list arrays, while being certain that
...
int gifti_rotate_DAs_to_front(gifti_image *gim, int nrot)
{
    ...
    if (G.verb > 3)
    {
        fprintf(stderr, "++ rotate darray[%d] (%d)\n", gim->numDA, nrot);
    }
 ...
 *
 *  if num_to_add > 0: add that many elements if defaults      : init each element with default
 *values
 *
 *  return 0 on success 1 on error
 *//*-------------------------------------------------------------------*/
 ...
 *
 *  this allocates memory for the p->name and p->value arrays, along with duplicating the passed
 *name/value strings
 *//*-------------------------------------------------------------------*/
...
int gifti_disp_nvpairs(const char *mesg, const nvpairs *p)
{
    ...
    if (mesg)
    {
        fputs(mesg, stderr); fputc(' ', stderr);
    }
 ...
 *
 *  if 'subs' is set, display the contents of the sub-structures, such as all of the DataArray
 *elements
 *//*-------------------------------------------------------------------*/
 ...
 *
 *  if in_mb is set, return the (rounded) number of megabytes (i.e. 17 is 17 MB)
 *//*-------------------------------------------------------------------*/
                       ...
                       giiDataArray ***list, int *len)
{
    ...
    *list = (giiDataArray **)calloc(*len, sizeof(giiDataArray *));
    ...
    if (nfound == 0)
    {
        free(*list); *list = NULL; *len = 0; return 0;
    }
 ...
 *
 *  define rows to be along the first dimension, and collapse the remaining dimensions as cols
 *//*-------------------------------------------------------------------*/
 ...
 *
 *  if mesg is set, print the message first if fp is not set, print to stdout
 *//*-------------------------------------------------------------------*/
 ...
 *
 *  if N < 2         : just return if N = 2 or N = 4: call explicit function for that size (speed)
 *else             : swap in a loop
 *//*-------------------------------------------------------------------*/
...
/*! Convert the ArrayIndexingOrder of the gifti_image DA elements to the given order.
 *                             28 Apr 2017 [rickr]
 *
 *  This is not just a tranpose, but a more general permutation of the data in memory, so allocate a
 *new data pointer.
 *
...
int gifti_convert_ind_ord(gifti_image *gim, int new_ord)
{
    ...
    for (dind = 0; dind < gim->numDA; dind++)
    {
        ...
        if (rv < 0)
        {
            return rv;          /* fail on any error */
        }
        if (rv > 0)
        {
            perm = 1;           /* note that it happened */
        }
    }
...
/*! Convert the ArrayIndexingOrder of the DataArry element to the specified order.
 *                          27 Apr 2017 [rickr]
 *
 *  This is not just a tranpose, but a more general permutation of the data in memory, so allocate a
 *new data pointer.
...
static int permute_by_index_order(void *dest, int new_ord, giiDataArray *da)
{
    ...
    {
    default: {
        /* don't need to fail after malloc, but this should be rare, and this is a good place for a
         * failure check */
    ...
    case 1: {   /* process as char (signed int8) */
        if (GIFTI_BAD_SIZEOF(mesg, char, 1))
        {
            return 1;
        }
    ...
    case 2: {   /* process as short (signed int16) */
        if (GIFTI_BAD_SIZEOF(mesg, short, 2))
        {
            return 1;
        }
    ...
    case 4: {   /* process as int (signed int32) */
        if (GIFTI_BAD_SIZEOF(mesg, int, 4))
        {
            return 1;
        }
    ...
    case 8: {   /* process as long long (signed int64) */
        if (GIFTI_BAD_SIZEOF(mesg, long long, 8))
        {
            return 1;
        }
...
/*! Allocate and fill the data array with data read from the given external file.
 *
 *  return 0 on success
 *//*-------------------------------------------------------------------*/
 ...
 *
 *  The files are assumed to be partitioned by DataArray entries.  So the list length must divide
 *numDA evenly.
 *
 *  External files are not checked for her, as this is independent of any read or write operation.
 *
 *  return 0 on success
 *//*-------------------------------------------------------------------*/
...
/*! Given a NIFTI_INTENT string, such as "NIFTI_INTENT_NODE_INDEX", return the corresponding
 * integral intent code.  The intent code is the macro value defined in nifti1.h.
 *
...
/*! Given a NIFTI_TYPE value, such as NIFTI_TYPE_INT16, return the corresponding macro label as a
 * string.  The dtype code is the macro value defined in nifti1.h.
 *//*-------------------------------------------------------------------*/
 ...
 *
 *  Allocate for a new struct, and fill it so that the contents are identical (sans pointers) to
 *orig.  Sub-structure arrays will need to be allocated, also.
 *
...
/*! copy any DataArray MetaData named 'name' from dest to src (replace old) (apply to list of DAs,
 * or to all)
 *
 *  return 0 on success, 1 on failure to find, -1 on error
 *//*-------------------------------------------------------------------*/
...
/* compare everything but darray (for diffs, only print if verb > 1)
 */
...
/*! return 1 if gifti_images are approximately equal (from a data standpoint) note: return value is
 * opposite from gifti_compare_gifti_images
 *
 *  verb  0-3 = quiet, state diff, state per DA, state all diffs
 *
 ...
 *      - numDA
 *      - labeltable
 *      - darray elements (pairwise) ignore version, meta, swapped, encoding, endian, ext_*,
 *ex_atrs, etc.
 *
 ...
 *
 *      transformation matrices : 16 values each (approx) coordinates             : 3 values per
 *node (approx) triangles               : exact (starting node can be any of the 3) data
 *                   : approx
 *
 * lverb = 0    : no print (no print anywhere) 1    : no print (print at higher level) 2    : print
 *DA level diff 3    : print all diffs
 *
 */
                         ...
                         int comp_data, int verb)
{
    ...
    /* if data is coordinates, compare percent diff if trianges, compare exactly, but with any
     * starting index else, compare percent diff */
...
/*! check pointers, compare lengths, then check the Names in each list, and see if there is a
 * matching Name=Value pari
 *
 ...
 *      0: if both pointers are NULL 1: if exactly one is NULL
 *//*-------------------------------------------------------------------*/
...
#undef GIFTI_SFD
#define GIFTI_SFD(a, b)    (fabs((double)(a) - (double)(b)) \
                            / (fabs((double)(a)) + fabs((double)(b))))

/*---------------------------------------------------------------------*/

/*! approximate comparison of raw data, returing the first location difference (>= 0 means
 * difference, -1 means "approximately equal")
 *
 * Compute a symmetric fractional difference:
 *
 *      SFD = abs(a-b)/(abs(a)+abs(b))
 *
 * length       - number of contiguous elements to test ni_type      - NIFTI_TYPE_*, denoting the
 *type of data to compare limit        - maximum SFD to be considered approximately equal
 *
 *              * if limit = 0.0, check is faster
 *
 *              * if limit >= 1.0, apply a default requiring almost all significant bits
 *
 *              * for integers, all bits are considered significant for real numbers, mantissa bits
 ...
 *
 *  require the type to be 1, 2 or 4-byte integers could cheat and compare as unsigned...
 *
 ...
 *
 *  if numDA > 0, allocate and initialize gim->darray if intent is a valid NIFTI_INTENT code, set it
 *in all DataArray elements if dtype is a valid NIFTI_TYPE, set it in all DA elements if dims is set
 *(MUST be of length 6), set the dims in all DA elements if alloc_data, allocate zero-filled data in
 *each DA element
 *
                                 ...
                                 const int *dims, int alloc_data)
{
    ...
    if (G.verb > 1)     /* maybe start with some chit-chat */
    {
        ...
        if (G.verb > 2)
        {
            ...
            {
                gifti_disp_raw_data(dims, DT_INT32, GIFTI_DARRAY_DIM_LEN,
                                    1, stderr);
            }
        }
    }
...
/*----------------------------------------------------------------------
 *! allocate nvals*nbyper bytes of (zero-filled) data in each DataArray (in dalist or simply in gim)
 *
 *  return 0 on success 1 on error
 *//*-------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------
 *! simply clear all contents of the passed gifti_image (being explicit with pointers)
 *
 *  return 0 on success 1 on error
 *//*-------------------------------------------------------------------*/
 ...
 *
 *  Allocate and copy all contents of the gifti_image structure and sub-structures.  If copy_data is
 *not set, all data pointers within DataArray elements will be left as NULL.
 *
...
gifti_image * gifti_copy_gifti_image(const gifti_image *gold, int copy_data)
{
    ...
    if (errs)
    {
        gifti_free_image(gnew); return NULL;
    }
 ...
 *
 *  for each DataArray if data exists, convert it (free old, allocate new) else, leave as NULL
 *//*-------------------------------------------------------------------*/
