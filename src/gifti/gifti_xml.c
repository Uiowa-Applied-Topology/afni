...
/* XMLCALL was added to expat in version 1.95.7 to define a calling convention, as cdecl
 */
...
/* decoding: converting characters A-Z, a-z, 0-9, +, /, to integers 0-63 (other characters are
 * considered invalid, though '=' is mapped to 0, to account for the end of input (didn't Arnold
...
/* note: the buffer needs to be large enough to contain any contiguous piece of (CDATA?) text, o.w.
 * it will require parsing in pieces */
...
static int free_xd_data(gxml_data *xd)
{
    if (xd->da_list)
    {
        free(xd->da_list); xd->da_list = NULL;
    }

    if (xd->xdata)
    {
        free(xd->xdata); xd->xdata = NULL;
    }                                                     /* xform matrix  */
    if (xd->zdata)
    {
        free(xd->zdata); xd->zdata = NULL;
    }                                                     /* compress buff */
    if (xd->ddata)
    {
        free(xd->ddata); xd->ddata = NULL;
    }                                                     /* Data buffer   */

    return 0;
}

/* verify that the current lengths match create a new list of NULL DA pointers create a 'taken'
 * list, matching xd->da_len (but unset) for each index, if it is not set, find one and copy
 */
...
/*! zlevel is the zlib compression level, with -1 implying the default (of 6, I believe) and value
 * levels being in {0..9} (with 0 meaning no compression) */
...
/*! perm_by_iord specifies whether to permute the data to row major index order (since this is C)
 * upon read (-1 is used to initialize to the default)      28 Apr 2017 [rickr] */
int gxml_get_perm_by_iord(void)
{
    return GXD.perm_by_iord;
}
...
/* increase LabelTable length by 1, and fill new entries (note that the Key (or Index) attribute is
 * required)
 *
...
/* get key and RGBA attributes, if they exist return 1 if key, 2 if key+rgba, 0 if neither, -1 on
 * error */
...
/* May divide Data, but apparently not attributes, perhaps because the Data section is longer than
 * the buffer is wide.
 *
 * if in Data:
 *      if prev_end_check if( ! char_is_whitespace(first) && concat_is_number() ) concatencate as
 *number to adjust previous number verify rest is whitespace return  (we don't expect to start a new
 *number) else apply previous number if( !char_is_whitespace(last) ) store trailing non-space in
 *concat_buf else prev_end_check = 0 apply number (though it may change later)
 */
 ...
 *      md->name[k], md->value[k], lt->label[k], da[k]->coordsys->dataspace,
 *da[k]->coordsys->xformspace
 *
 ...
 *
 * use intermediate buffer (ddata), length dlen+1, reprocess length doff ddata format, in bytes per
 *section:
 *       +------+----------+--------+---+
 *       | doff | copy_len | unused | 1 |  (last 1 assures null termination)
 *       +------+----------+--------+---+
 *
 * - while there are bytes left to process (rem_bytes_in > 0) copy_len = bytes to process now copy
 *that many bytes to ddata rem_bytes_out = number of output bytes left to fill in da->data doff =
 * decode_b64(xd, ddata, doff, da->data+dind, &rem_bytes_out) (returns number of unprocessed bytes,
 *in [0..3]) if( doff > 0 ) mv last doff bytes to beginning of ddata rem_bytes_in -= copy_len dind =
 * updated offset into output da->data buffer
 */
                              ...
                              const char *cdata, int cdlen)
{
    ...
    /* Copy cdata to local buffer in pieces, for storage of trailing characters from a previous
     * call.  Given that, processing data as is done with ASCII seems reasonable. */
    while (rem_bytes_in > 0)
    {
        ...
        /* copy the data to our intermediate buffer (if we allow bad characters, skipping them, do
         * it here) */
...
/* Copy the base64 data to the dest buffer, possibly noting, counting and/or skipping any invalid
 * characters, based on b64_check.
 *
...
static int append_to_data_ascii(gxml_data *xd, const char *cdata, int len)
{
    ...
    /* Copy cdata to local buffer in pieces, for null termination and for storage of trailing
     * characters that may need to be processed again (after more characters are read by the
     * parser).
     *                    */
    while (rem_len > 0)
    {
...
/*  given: source pointer, length, dest loc and nbytes to set, (cdata is null-terminated) modify:
 * needed (bytes) left for output return: nbytes unprocessed, so 0-3 (< 0 on error)
 *
 ...
 *
 *  note: the base64 defaults will be applied o EOL use is not allowed o padding is expected (using
 * '=')
 */
...
/* given: source pointer, remaining length, nvals desired, dest loc and type (cdata is
 * null-terminated) modify: nvals left for output, mod_prev for next call return: nbytes that may
 * still need to processed (< 0 on error)
 *
 * read failure happens only when no characters are processed
 */
                        ...
                        void *dptr, long long *nvals, int *mod_prev)
{
    ...
    case NIFTI_TYPE_COMPLEX64: {
        /* there are 2 pieces to each complex, so read and insert 1 at a time, and increment 'vals'
         * only after each second piece */
        static int piece = 0;
        float *    ptr   = (float *)dptr;
        p1   = cdata;
        prev = p1;
    ...
    case NIFTI_TYPE_RGB24: {
        /* there are 3 pieces to each RGB24, so read and insert 1 at a time, and increment 'vals'
         * only after each third piece */
        static int     piece = 0;
        unsigned char *ptr   = (unsigned char *)dptr;
        p1   = cdata;
        prev = p1;
...
/* decide how big a processing buffer should be (either for a small xform matrix or a Data element)
 */
...
static int gxml_write_gifti(gxml_data *xd, FILE *fp)
{
    ...
    if (gim->version)
    {
        fprintf(fp, " Version=\"%s\"", gim->version); first = 0;
    }
...
static int ewrite_data(gxml_data *xd, giiDataArray *da, FILE *fp)
{
    ...
    if (xd->dstore)
    {
        ...
        {
#ifdef HAVE_ZLIB                                              /* for compiling, higher level test
                                                               * elsewhere */
