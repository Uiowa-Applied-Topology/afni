...
/*! Convert a dataset to the MRI_vectim format, where each time series is a contiguous set of values
 * in an array, and the voxel indexes whence came the values are also stored.
 *//*------------------------------------------------------------------------*/
...
MRI_vectim * THD_dset_to_vectim(THD_3dim_dataset *dset, byte *mask, int ignore)
{
    ...
    nvals = DSET_NVALS(dset) - ignore; if (nvals <= 0)
    {
        RETURN(NULL);
    }
    nvox = DSET_NVOX(dset);
...
#pragma omp critical (MALLOC)
    ...
    if (mrv->fvec == NULL)
    {
        ERROR_message("THD_dset_to_vectim: out of memory B -- tried to get %lld bytes",
                      (long long)ntot);
        free(mrv->ivec); free(mrv); if (mmm != mask)
        {
            free(mmm);
        }
        RETURN(NULL);
    ...
    for (kk = iv = 0 ; iv < nvox ; iv++)
    {
        if (mmm[iv])
        {
            mrv->ivec[kk++] = iv;          /* build index list */
        }
    }
...
int THD_vectim_data_tofile(MRI_vectim *mrv, char *fnam)
{
    ...
    if (mrv == NULL || fnam == NULL)
    {
        return 0;
    }

    fp = fopen(fnam, "w"); if (fp == NULL)
    {
        return 0;
    }
...
int THD_vectim_reload_fromfile(MRI_vectim *mrv, char *fnam)
{
    ...
    if (mrv->fvec == NULL)
    {
        mrv->fvec = (float *)malloc(sizeof(float) * nf);
    }
...
MRI_vectim * THD_dset_to_vectim_stend(THD_3dim_dataset *dset, byte *mask, int start, int end)
{
    ...
    if (end < start)
    {
        end = DSET_NVALS(dset) - 1;
    }
    nvals = end - start + 1; if (nvals <= 0)
    {
        RETURN(NULL);
    }
    nvox = DSET_NVOX(dset);
                                        ...
                                        int ignore, int kzbot, int kztop)
{
    ...
    nvals = DSET_NVALS(dset); if (nvals <= 0)
    {
        RETURN(NULL);
    }
    nvox = DSET_NVOX(dset);
...
#pragma omp critical (MALLOC)
      ...
      if (mask == NULL)
      {
          AAmemset(mmm, 1, sizeof(byte) * nvox);
      }
      else
      {
          AAmemcpy(mmm, mask, sizeof(byte) * nvox);
      }
      if (kzbot > 0)
      {
                                 ...
                                 int ignore)
{
    ...
    for (id = 0; id < 2; ++id)
    {
        if (!ISVALID_DSET(dsetv[id]))
        {
            RETURN(NULL);
        }
        DSET_load(dsetv[id]); if (!DSET_LOADED(dsetv[id]))
        {
            RETURN(NULL);
        }
    ...
    nvals = DSET_NVALS(dsetv[0]) - ignore; if (nvals <= 0)
    {
        RETURN(NULL);
    }
...
#pragma omp critical (MALLOC)
    ...
    for (kk = iv = 0 ; iv < nvoxv[0] ; iv++)
    {
        if (mmmt[iv])
        {
            mrv->ivec[kk++] = iv;           /* build index list to 1st dset */
        }
    }
    mmmt = mmmv[1]; kk2 = 0;
...
MRI_vectim * THD_vectim_copy_nonzero(MRI_vectim *mrv)   /* 21 Sep 2010 */
{
    ...
    for (ngood = ii = 0 ; ii < nvec ; ii++)
    {
        ...
        for (jj = 0 ; jj < nvals && mar[jj] == 0.0f ; jj++)
        {
            ;                                             /*nada*/
        }
        if (jj < nvals)
    ...
    if (ngood == nvec)
    {
        RETURN(THD_vectim_copy(mrv));                   /* everything to do */
    }
...
/* Apply a function to each time series, presumably to edit it in-place.  For 3dGroupInCorr -- 10
 * May 2012 -- RWCox.
 *//*-------------------------------------------------------------------*/
...
void THD_vectim_applyfunc(MRI_vectim *mrv, void *vp)
{
    ...
    {
        fp(mrv->nvals, VECTIM_PTR(mrv, iv));
    }
}
...
void THD_vectim_vectim_dot(MRI_vectim *arv, MRI_vectim *brv, float *dp)
{
    ...
    if (arv == NULL || brv == NULL || dp == NULL)
    {
        return;
    }
    if (arv->nvec != brv->nvec || arv->nvals != brv->nvals)
    {
        return;
    }
...
#pragma omp critical (MALLOC)
    ...
    sav = spearman_rank_prepare(nvals, av); if (sav <= 0.0f)
    {
        sav = 1.e+9f;
    }
...
#pragma omp critical (MALLOC)
    ...
    AAmemcpy(av, vec, sizeof(float) * nvals);
    sav = quadrant_corr_prepare(nvals, av); if (sav <= 0.0f)
    {
        sav = 1.e+9f;
    }
...
#pragma omp critical (MALLOC)
    ...
    AAmemcpy(av, vec, sizeof(float) * nvals);
    sav = tictactoe_corr_prepare(nvals, av); if (sav <= 0.0f)
    {
        sav = 1.e+9f;
    }
...
#pragma omp critical (MALLOC)
    ...
    AAmemcpy(av, vec, sizeof(float) * nvals);
    sav = quantile_prepare(nvals, av); if (sav <= 0.0f)
    {
        sav = 1.e+9f;
    }
...
/* Special parameters:
 * abs: 0 --> Euclidean distance 1 --> City Block distance xform: String flags for transforming
 *distance. If string contains "n_scale", scale distance by number of values (dimensions) at each
 *voxel If string contains "inv", return the inverse of the distance
 */
...
{
    MCW_cluster *smask;
    int          sqq, qq, pp, pijk, nsar, nyar, nlen, nx, ny, nz, nxy, ii, ik, ij, qi, qj, qk, qjk,
                 mm, nmask;
    float **sar, **yar;

    ENTRY("THD_vectim_pearsonBC");

    if (mrv == NULL || par == NULL)
    {
        EXRETURN;
    }
    sqq = THD_vectim_ifind(sijk, mrv); if (sqq < 0)
    {
        EXRETURN;
    }
    if (srad >= 0.0f)
...
#pragma omp critical (MALLOC)
    ...
    {
        qi = ii + smask->i[mm]; if (qi < 0 || qi >= nx)
        {
            continue;
        }
        qj = ij + smask->j[mm]; if (qj < 0 || qj >= ny)
        {
            continue;
        }
        qk = ik + smask->k[mm]; if (qk < 0 || qk >= nz)
        {
            continue;
        }
        qjk = qi + qj * nx + qk * nxy;
    ...
    if (vstep)
    {
        fprintf(stderr, " + Voxel loop [nmask=%d]: ", nmask);
    }
#endif
    ...
    for (pp = 0 ; pp < mrv->nvec ; pp++)
    {
        if (pp == sqq)
        {
            par[pp] = 1.0f; continue;
        }
#ifdef USE_VSTEP
        if (vstep && pp % vstep == vstep - 1)
        {
            vstep_print();
        }
#endif
...
int THD_vectim_subset_average(MRI_vectim *mrv, int nind, int *ind, float *ar)
{
    ...
    if (mrv == NULL || nind <= 0 || ind == NULL || ar == NULL)
    {
        RETURN(0);
    }
    ...
    for (jj = 0 ; jj < nind ; jj++)
    {
        kk = THD_vectim_ifind(ind[jj], mrv); if (kk < 0)
        {
            continue;
        }
...
void THD_vectim_to_dset(MRI_vectim *mrv, THD_3dim_dataset *dset)
{
    ...
    if (mrv->nvals + mrv->ignore != DSET_NVALS(dset))
    {
        EXRETURN;
    }
...
/* The jj-th time point in the vectim goes to the tlist[jj]-th time point in the output dataset [06
 * Aug 2013].
 *//*--------------------------------------------------------------------*/
                                ...
                                THD_3dim_dataset *dset, int *tlist)
{
    ...
    if (mrv == NULL || !ISVALID_DSET(dset) || tlist == NULL)
    {
        EXRETURN;
    }
...
/* The ilist[jj]-th point in the vectim goes into the jj-th index in the output dataset [06 Feb
 * 2014].
 *//*--------------------------------------------------------------------*/
...
MRI_vectim * THD_dset_list_to_vectim(int nds, THD_3dim_dataset **ds, byte *mask)
{
    ...
    if (nds == 1)
    {
        RETURN(THD_dset_to_vectim(ds[0], mask, 0));
    }

    for (kk = 0 ; kk < nds ; kk++)
    {
        if (!ISVALID_DSET(ds[kk]))
        {
            RETURN(NULL);
        }
    }
                                              ...
                                              byte *mask, int nkeep, int *keep)
{
    ...
    for (kk = 0 ; kk < nds ; kk++)
    {
        ...
        if (DSET_NVALS(ds[kk]) != DSET_NVALS(ds[0]))
        {
            RETURN(NULL);
        }
    }
...
void THD_check_vectim(MRI_vectim *mv, char *fname)
{
    ...
    for (nbad = jj = 0 ; jj < nvec ; jj++)
    {
        ...
        for (ii = 1 ; ii < nvals && vpt[ii] == vz ; ii++)
        {
            ;                                           /*nada*/
        }
        if (ii == nvals)
    ...
    if (nbad > 0 && nvals > 1)
    {
        /* 16 Dec 2025: be quieter about warnings, with some cases moved to "just" info msgs. We
         * start warns-> info for 3dTproject usage, but might expand further.
         */
    ...
    for (nbad = ii = 0 ; ii < nvals ; ii++)
    {
        ...
        for (jj = 1 ; jj < nvec ; jj++)
        {
            vpt = VECTIM_PTR(mv, jj); if (vpt[ii] != vz)
            {
                break;
            }
        }
...
/* To discard the output from this function, use mri_clear_and_free() NOT mri_free() -- the
 * MRI_IMAGE data is just a pointer into the vectim, and trying to free it would be disastrous! [19
 * Nov 2021] To put ALL vectors into the temp image, use istart=numi=0.
 *//*-------------------------------------------------------------------*/
