 ...
 *
 * Mod:   2 June 1997 Added option to generate histogram for only those voxels which are above the
 *operator specified threshold value.
 *
...
int main(int argc, char *argv[])
{
    ...
    if (fbin == NULL)
    {
        HI_syntax("can't allocate histogram array!");
    }
...
void HI_read_opts(int argc, char *argv[])
{
    ...
    while (nopt < argc && argv[nopt][0] == '-')
    {
        ...
        if (strncmp(argv[nopt], "-nbin", 5) == 0)
        {
            ...
            if (HI_nbin == 0)
            {
                HI_nbin = NBIN_SPECIAL;
            }
            else if (HI_nbin < 1)
            {
                HI_syntax("illegal value of -nbin!");
            }
            nopt++; continue;
        }
        ...
        if (strncmp(argv[nopt], "-dind", 5) == 0)
        {
            ...
            if (HI_dind < 0)
            {
                HI_syntax("illegal value of -dind!");
            }
            nopt++; continue;
        }
        ...
        if (strncmp(argv[nopt], "-omit", 5) == 0)
        {
            ...
            if (cpt != NULL && *cpt != '\0')
            {
                HI_syntax("illegal value of -omit!");
            }
            HI_nomit++;
            if (HI_nomit == 1)
            {
        ...
        if (strncmp(argv[nopt], "-prefix", 6) == 0)
        {
            ...
            if (nopt >= argc)
            {
                HI_syntax("need argument after -prefix!");
            }
            HI_ni = argv[nopt];
            nopt++; continue;
        }
        ...
        if (strncmp(argv[nopt], "-mask", 5) == 0)
        {
            ...
            if (nopt >= argc)
            {
                HI_syntax("need argument after -mask!");
            }

            mset = THD_open_dataset(argv[nopt]);
            if (mset == NULL)
            {
                HI_syntax("can't open -mask dataset!");
            }
            HI_mask = THD_makemask(mset, 0, 1.0, 0.0);
            if (HI_mask == NULL)
            {
                HI_syntax("can't use -mask dataset!");
            }

            HI_mask_nvox = DSET_NVOX(mset);
            DSET_delete(mset);

            for (ii = mc = 0 ; ii < HI_mask_nvox ; ii++)
            {
                if (HI_mask[ii])
                {
                    mc++;
        ...
        if (strncmp(argv[nopt], "-roi_mask", 5) == 0)
        {
            ...
            if (nopt >= argc)
            {
                HI_syntax("need argument after -roi_mask!");
            }

            rset = THD_open_dataset(argv[nopt]);
            if (rset == NULL)
            {
                HI_syntax("can't open -roi_mask dataset!");
            }
            ...
            /* There can be a leak here if rset is not short But that's OK */
