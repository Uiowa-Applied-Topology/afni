/********************************************************************************
*  3dEdge Justin Knoll 5/23/00
*
*  Generates an edge-detected mask from the input 3d+time dataset. Needs boundary checking for
*non-sagittal datasets.
*
********************************************************************************/
...
/*******************************************************************************/
{
    printf(
        ...
        "                   threshold. Voxels in the time-averaged volume with\n"
        ...
        "                   threshold. Voxels in the RMS-derived mask are\n"
 ...
 */
{
    ...
    if (l % (nxx * nyy) > (nxx * (nyy - 1)))
    {
        count++;
    }
    if (l % (nxx * nyy) > (nxx * (nyy - 1) - 1))
    {
        count++;
    }
    ...
    if (((l % (nxx * nyy)) % nxx) == (nxx - 1))
    {
        count++;
    }
    if (((l % (nxx * nyy)) % nxx) == (nxx - 2))
    {
        count++;
    }
...
float neighborCount(unsigned char *mask, int location, int nxx, int nyy, int nvox)
{
    ...
    for (i = -2 ; i <= 2 ; i++)
    {
        for (j = -2 ; j <= 2 ; j++)
        {
            ...
            {
/*
 * the if condition below is the bitwise OR of the three variables. It is true if at least one
...
int centerOfMass(unsigned char *mask, int nxx, int nyy, int nvox)
{
    ...
    yMass /= mass;
    zMass /= mass;
    return (int)((int)(xMass + 0.49) + ((int)(yMass + 0.49) * nxx) +
                 ((int)(zMass + 0.49) * (nxx * nyy)));
}
...
unsigned char * edgeDetect(THD_3dim_dataset *inputDataset, float rmsThresh, float neighbors)
{
    ...
    double *       sums;
    double         rms = 0;
    unsigned char *newMask;    /* temporary mask used to prevent order-dependent errors in neighbor
                                * counting. */
...
/*
 * the for loop below switches to handle different subbrick types (float, byte, short), and stores
 *the square of the values at each voxel in sums[].
 */
    for (j = 0 ; j < ntimes ; j++)
    {
...
/* fill holes and remove outlayers based on number of nearest neighbors */
    for (i = 0 ; i < nvox ; i++)
    {
        if (neighborCount(mask, i, nxx, nyy, nvox) < neighbors)
        {
            newMask[i] = 0;
        }
        else
...
/*******************************************************************************/
int applyMask(THD_3dim_dataset *inputDataset, unsigned char *mask, char *prefix, int argc,
              char *argv[])
/* argc and argv are needed for to put the command-line args in the history. */
{
    ...
    for (j = 0 ; j < ntimes ; j++)
    {
        ...
        case MRI_byte:
        {
            ...
            EDIT_substitute_brick(outputDataset, j, DSET_BRICK_TYPE(inputDataset, j),
                                  outputTimeStep_b);
            break;
        }
