...
MRI_IMARR * THD_get_many_timeseries(THD_string_array *dlist)
{
    ...
    if (epath == NULL)
    {
        epath = my_getenv("AFNI_TS_PATH");                   /* 07 Oct 1996 */
    }
    if (epath == NULL)
    {
        epath = efake;                                      /* 07 Oct 1996 */
    }
    ...
    if (epath != NULL)
    {
        ...
        do{
            ...
            if (ii < qlist->num)
            {
                continue;                  /* skip to end of do loop */
            }
...
MRI_IMARR * THD_get_all_timeseries(char *dname)
{
    ...
    for (ir = 0 ; ir < rlist->num ; ir++)
    {
        ...
        {
            if (THD_filesize(fname) > max_fsize)
            {
                continue;                                  /* 20 Jul 2004 */
            }
