...
/*!  This function is intended to be called from within a fork()-ed off child, and it will cause the
 * child process to die after so many milliseconds have passed -- the intention is to prevent any
 * zombies from hanging around, in case the child fails to complete itself properly for some
 * hideously grotesque reason.
 *//*-------------------------------------------------------------------***/
...
/*! Function to check AFNI version.  Forks and returns to caller almost instantly.  The only output
 * is from the child process to stderr;
...
void THD_check_AFNI_version(char *pname)
{
    ...
    /* This function breaks MOTD usage. Ponder, do we use a new .afni.vctime.cmdline file?
     *                              30 Dec 2015 [RCR/DRG] */
    return;

    if (!AFNI_yesenv("AFNI_VERSION_CHECK") || machdep_be_quiet())
    {
        return;
    }
...
#  endif
    ...
    vvaa = strstr(vbuf, "AFNI_");   if (vvaa == NULL)
    {
        _exit(0);
    }
