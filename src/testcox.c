 ...
 * -- In any case, this program is probably useless except for the eventual biographer of AFNI.
 * -- RWCox
 */
...
int main(int argc, char *argv[])
{
    ...
    for (sum = 0, ibot = 0   ; ibot < 128 && sum < nperc ; ibot++)
    {
        sum += hbr[ibot];
    }
    for (sum = 0, itop = 255 ; itop > ibot && sum < nperc ; itop--)
    {
        sum += hbr[itop];
    }
    if (ibot >= itop)
