                   ...
                   int initialize, int nbin, int hist[])
{
    ...
    if (PRINT_TRACING)
    {
        char str[256]; sprintf(str, "hbot=%f htop=%f nbin=%d sbin=%f", hbot, htop, nbin, sbin);
        STATUS(str);
    }
    ...
    case MRI_short: {
        ...
        for (ii = 0 ; ii < npix ; ii++)
        {
            val = shar[ii]; if (val < hbot || val > htop)
            {
                continue;
            }
    ...
    case MRI_float: {
        ...
        for (ii = 0 ; ii < npix ; ii++)
        {
            val = flar[ii]; if (val < hbot || val > htop)
            {
                continue;
            }
            ih = (int)(sbin * (val - hbot)); hist[ih]++;
    ...
    if (lim != im)
    {
        mri_free(lim);              /* toss temporary array */
    }
    EXRETURN;
}
