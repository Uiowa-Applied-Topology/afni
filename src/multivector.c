...
/*********************************************************************
 * Routines to handle the multivector data type: a collection of 1D arrays of strings or floats, all
 *the same length.
 *
 * RWCox - May 1999
 **********************************************************************/
...
static int my_strequiv(char *s1, char *s2)
{
    ...
    ii = strlen(s1); ll = strlen(s2); if (ii != ll)
    {
        return 0;
    }
    for (ii = 0 ; ii < ll ; ii++)
...
multivector * multivector_read(char *fname)
{
    ...
    if (fp == NULL)
    {
        MERR("can't open file"); return NULL;
    }
    ...
    while (1)
    {
        ...
        if (ii < 0)
        {
            continue;               /* was all blanks; goto next line */
        }
        if (buf[0] != '#')
        {
            first = 1; break;
        }                                      /* not a header line ==> done */
        ...
        if (my_strequiv(ptr, "#TYPE"))
        {
            ...
            while (1)
            {
                ...
                if (my_strequiv(ptr, "STRING"))
                {
                    typ[ntyp++] = MV_STRING;
                }
                else if (my_strequiv(ptr, "FLOAT"))
                {
                    typ[ntyp++] = MV_FLOAT;
                }
                else
    ...
    while (1)
    {
        ...
        if (buf[0] == '#')
        {
            continue;                 /* a comment line; goto next line */
        }
        /* extract tokens from this line */

        pp[0] = strtok(buf, SEPCH); if (pp[0] == NULL)
        {
            continue;
        }
        ll = 1;
        while (1)
        {
            pp[ll] = strtok(NULL, SEPCH); if (pp[ll] == NULL)
            {
                break;
            }
            ll++;
        ...
        /* expand vector space for new row of data, convert tokens to values and store them in this
         * space */
...
void multivector_set_name(multivector *mv, char *nname)
{
    if (mv->name != NULL)
    {
        free(mv->name); mv->name = NULL;
    }

    if (nname != NULL)
    {
        mv->name = strdup(nname);
    }
    return;
}

/*-------------------------------------------------------------------
 * Write a multivector to disk. Returns 0 if it fails, 1 if it succeeds.
 * ---------------------------------------------------------------------*/
...
int multivector_write(char *fname, multivector *mv)
{
    ...
    if (!THD_filename_ok(fname) || mv == NULL)
    {
        return 0;
    }
...
#define BSTRIP    for (il = strlen(lbuf) - 1 ;                          \
    ...
    default:
        if (qval > 0.0)
        {
            sprintf(lbuf, "%-12.6e", qval);
        }
        else
        {
            sprintf(lbuf, "%-12.5e", qval);
        }
        break;
 ...
 * same as fval, but will attempt to keep the number len characters long. That's done by truncating
 *digits to the right of the decimal point, if one exists.
 * \sa MV_fval_to_char
 * \sa MV_format_fval      ZSS, RickR May 28 04
 */
char * MV_format_fval2(float fval, int len)
{
    static char bufm[10][32];
    static int  icall = 0;
    char *      buf   = NULL;
    int         wid;
    char *      pos = NULL;
    ...
    buf = (char *)bufm[icall]; /* This way we can make multiple calls formatting function inside one
                                * printf*
                                * call  ZSS Jan 2014 */
    ...
    if (pos)
    {
        return buf;    /* scientific notation, get out (ZSS, thanks to tip by Ben Singer Dec 12 05)
                        */
    }
    pos = strchr(buf, '.');
    if (!pos)
    {
        return buf;      /* can't do no'in */
    }
    wid = pos - buf;
    if (wid < len)
    {
    ...
    if (buf[len - 1] == '.')
    {
        buf[len - 1] = '\0';                 /* remove trailing period */
    }
    return buf;
}
