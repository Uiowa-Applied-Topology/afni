 ...
 *   - max_dist   = maximum distance for a point to be included in the mask Date   :  11 September
 *1996
 *
 * To correct error due to ambiguity in identification of clusters, voxel coordinates are now stored
 *as 3 separate short integers. BDW  06 March 1997
 *
...
/*! Like MCW_build_mask(), but adds the (0,0,0) point to the cluster, and then sorts by radius (so
 * the points closest to the origin are first in the array).  The radius from the origin is stored
...
/*     if((x==0.0)&&(y==0.0)&&(z==0.0)) mask->mag[ii] = 1.0;
 *   else
 */
...
/*! Like MCW_spheremask(), but builds a rhombic dodecahedron. Volume = 2 * radius**3.
 *//*--------------------------------------------------------------------*/
...
/*! Like MCW_spheremask(), but builds a truncated octahedron. Volume = 4 * radius**3.
 *//*--------------------------------------------------------------------*/
