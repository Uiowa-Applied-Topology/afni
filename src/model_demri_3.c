/*----------------------------------------------------------------------
 * This is for the 3-parameter DEMRI model from Bob Cox and Steve Fung, implemented for John Butman
 *and Steve Fung.
 *
    ...
    )
{
    ...
    if (first_call > 1)
    {
        return;                     /* fail, and be quick about it */
    }
    ...
    if (first_call)
    {
        ...
        if (P.debug)
        {
            disp_demri_params("ready to rock: ", &P);
        }
    ...
    if (P.per_min)
    {
        P.K /= 60.0;                    /* then convert to per minute */
    }
    if (g_use_ve)
    {
        P.ve = params[1];
    ...
    if (R1I_data_ptr)
    {
        ...
        if (P.debug > 3)
        {
            printf("Voxel index %d, R1I value %f\n", P.ijk, P.RIT);
        }
    }
    ...
    if (resid_ct_ptr)
    {
        ...
        if (P.debug > 3)
        {
            printf("Voxel index %d, rCt value %f\n", P.ijk, P.rct);
        }
    }
    ...
    {
        disp_demri_params("before compute_ts: ", &P);
    }
...
static int compute_ts(demri_params *P, float *ts, int ts_len)
{
    if (ct_from_cp(P, P->comp, P->mcp, P->nfirst, ts_len))
    {
        return 1;
    }

    if (c_from_ct_cp(P, ts_len))
    {
        return 1;
    }

    if (R1_from_c(P, ts_len))
    {
        return 1;
    }

    if (Mx_from_R1(P, ts, ts_len))
    {
        return 1;
    }
 ...
 *          ------                 * ( exp(kep*TF/2) - exp(-kep*TF/2) ) ] kep
 + (Cp[m] + Cp[n]) * (1 - exp(-kep*TF/2)) )
 +
 +  where sum is over k=m+1..n-1 (m is nfirst), and n is over m..len-1 note: Cp[n] = Ct[n] = 0, for
 +n in {0..m}  (m also, since that is time=0) note: (Cp[m]+Cp[n])*... reflects the first and last
 +half intervals, while the sum reflects all the interior, full intervals
 +
 +      Let P1  = Ktrans / kep P2  = exp(kep*TF/2) - exp(-kep*TF/2)
 +              = exp(-P3/2) - exp(P3/2) P3  = -kep*TF P4  = 1 - exp(-kep*TF/2)
 +              = 1 - exp(P3/2)
 +
 +  Ct[n] = P1*P2 * sum [ Cp[k] * exp(P3*(n-k)) ] + P1*P4 * (Cp[m]+Cp[n]) note: in exp_list, max
 +power is (P3*(len-1-m)), m is nfirst note: Ct is stored in P->comp
 +
 ...
 +
 +      note that n*TR = t (time, in seconds) note that Ct has already decayed by 1 TR of time,
 +hence +1
 +
 ** This is the only place that the dataset TR (which we label as TF, the inter-frame TR) is used in
 **this model.
 */
                      ...
                      int nfirst, int len)
{
    ...
    /* In a test, float accuracy was not lost until ~1 million products. Computing powers over the
     * range of a time series should be okay. */
    ...
    if (P->rct > 0)
    {
        ...
        if (resid < 0.0)
        {
            resid = 0.0;                /* clip it if negative */
        }

        /* elist is not long enough, even if we take it to nfirst-1 (since we must start at 1), so
         * be a little slow */
        /* note, Ct has already been decayed from the prior TR */
        for (n = 0; n < len; n++)
        {
            ct[n] += resid; resid *= dval;
        }
        ...
        if (P->debug > 2 && P->counter == 0)
        {
            ...
            if (resid < 0.0)
            {
                fprintf(stderr, "-- resid < 0.0, no decay curve\n");
            }
            else
            {
                fprintf(stderr, "++ decay curve:");
                for (n = 0; n < len; n++, resid *= dval)
                {
                    fprintf(stderr, "  %f", resid);
                }
                fputc('\n', stderr);
            }
        }
 ...
 *  R1[n] = RIT + r1 * C[n]
 *
 *  note: R1 will replace C in P->comp R1[i] will be constant P->RIT over i=0..nfirst-1
 */
static int R1_from_c(demri_params *P, int len)
{
    double *R1 = P->comp;
 ...
 *
 *      where P1   = 1 - exp(-RIT*TR) P1c  = 1 - exp(-RIT*TR)*cos0 e[n] = exp(-R1[n] * TR)
 *
 *  notes:  R1 is in P->comp The '1' is a placeholder for Mx(t=0), which should have been factored
 *out of the input time series. I can see no speed-up for e[n].  :'(
 */
static int Mx_from_R1(demri_params *P, float *ts, int len)
{
    double *R1 = P->comp;
...
static int get_env_params(demri_params *P)
{
    ...
    if (envp)
    {
        ...
        if (P->debug > 0)
        {
            printf("Set flipangle_factor_ptr\n");
        }
    }
    else
    {                    /* should I close any open datasets? */
    ...
    if (envp && P->debug > 1 && !errs)
    {
        disp_demri_params("env params set: ", P);
    }
...
static int get_Mp_array(demri_params *P, int *mp_len)
{
    ...
    if (P->debug)
    {
        fprintf(stderr, "-d reading Mp file, %s...\n", envp);
    }
    im = mri_read_1D(envp);
    if (!im)
    {
    ...
    if (im->nx == 1 && im->ny > 1)
    {
        ...
        if (!im)
        {
            fprintf(stderr, "** MP trans failure\n"); return 1;
        }
    ...
    *mp_len = im->nx;
    if (P->debug > 1)
    {
        fprintf(stderr, "-d Mp (len, ny) = (%d, %d)\n", im->nx, im->ny);
    }
 ...
 *          -----      [ ----------------------------------------------------  ] r1*TR      [
 *  1-exp(-RIB*TR)cos0 - (1-exp(-RIB*TR))*Mp(t)/Mp(0)    ]
 *
 *                 - RIB/r1
 *
 *  subject to Cp(t) >= 0
...
static int disp_demri_params(char *mesg, demri_params *p)
{
    ...
    if (!p)
    {
        fprintf(stderr, "demri_params: p == NULL\n"); return 1;
    }
