...
void EDIT_set_padval(int ftype, char *pval)
{
    ...
    if (pval == NULL || dsiz == 0)
    {
        use_padval = 0; typ_padval = -1; return;
    }
 ...
 *      nxbot = # to add on -x side (can be negative, to remove planes) nxtop = # to add on +x side,
 *etc. If all n?bot,n?top values are zero, just returns a copy of the input.  If too much is cut
 *off, or something else bad happens, returns NULL.
 *
 *     Adapted from thd_zeropad.c       -- 02 Feb 2001 -- RWCox Modified to allow uneven padding --
 * 09 Feb 2001 -- RWCox *****/
