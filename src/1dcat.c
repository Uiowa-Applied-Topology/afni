...
int main(int argc, char *argv[])
{
    ...
    while (narg < argc && argv[narg][0] == '-')
    {
        ...
        if (strcmp(argv[narg], "-form") == 0)
        {
            ...
            if (narg >= argc)
            {
                ERROR_exit("need argument after -form");
            }
            if (strcmp(argv[narg], "double") == 0)
            {
                oform = CCALC_DOUBLE;
            }
            else if (strcmp(argv[narg], "nice") == 0)
            {
                oform = CCALC_NICE;
            }
            else if (strcmp(argv[narg], "int") == 0)
            {
                oform = CCALC_INT;
            }
            else if (strcmp(argv[narg], "rint") == 0)
            {
                oform = CCALC_INT;
            }
            else if (strcmp(argv[narg], "fint") == 0)
            {
                oform = CCALC_FINT;
            }
            else if (strcmp(argv[narg], "cint") == 0)
            {
                oform = CCALC_CINT;
            }
        ...
        if (ncv != NULL && inim[jj] != NULL)  /* check for constant columns [04 Dec 2010] */
        {
            ...
            if (nonfixed)
            {
                ...
                if (hl != NULL && *hl == '#')
                {
                    ...
                    if (spt != NULL)
                    {
                        spt = strchr(spt, '#');       /* start of line 2 */
                    }
                    if (spt != NULL)
                    {
