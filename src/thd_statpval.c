...
/*---------------------------------------------------------------------------
 * This function is akin AFNI_set_pval(), but is meant to simply return the threshold that
 *corresponds to the input p-value, based on the dataset sub-brick statistic type and DF.
 *
                                ...
                                float pval, int as_1_sided)
{
    ...
    if (DSET_BRICK_STATCODE(dset, tindex) <= 0)
    {
        return -1.0;
    }
    ...
    /* If we want as_1_sided and it is a 2-sided test, double the p-value (unless the stat is F, for
     * which we never alter p).
     * */
                                ...
                                float thresh, int as_1_sided)
{
    ...
    /* If we want as_1_sided and it is a 2-sided test, halve the p-value (unless the stat is F, for
     * which we never alter p).
     * */
    if (as_1_sided && sid2 && (scode != FUNC_FT_TYPE))
    {
...
float THD_stat_to_pval(float thr, int statcode, float *stataux)
{
    ...
    if (stataux == NULL && statcode != FUNC_ZT_TYPE)
    {
        return pval;
    }
...
/* pval is 2-sided tail probability for FUNC_COR_TYPE  FUNC_TT_TYPE  FUNC_ZT_TYPE is 1-sided (upper)
 * tail probability for the others
 *//**************************************************************/
