...
/* The indexing into Storear has been swapped (columns became rows and rows columns. That's because
 * each subbrick was not stored in a vector on N elements, rather in N vectors of 1 element each.
 * That made the allocation process very slow, especially now that Bob has malloc and calloc going
 * through his own macros. */
...
static char * DUMP_main(PLUGIN_interface *plint)
{
    ...
    /* That output message was too long. AFNI was crashing, must ask Bob to allow more verbose
     * messages */

    /*sprintf(mssg , "            Dataset %s was dumped.\n"
     *            "%d voxels (%5f %% of total) met the boundary conditions.\n"
     *             , DSET_FILECODE(xset) , ndmp , (float)ndmp/(float)(ud->nxx * ud->nyy * ud->
     * nzz)*100.0) ;*/
...
static int Dumpit(extract_data *ud, THD_3dim_dataset *xset)
{
    ...
    Storear = (float **)allocate2D(ud->Nsub, nxyz, sizeof(float));  /* changed from :  allocate2D
                                                                     * (nxyz
                                                                     * ,ud->Nsub,sizeof(float))
                                                                     */
    ...
    if (1)
    {
        for (ii = 0 ; ii < nxyz ; ii++)
        {
            ...
            if (pass && ud->DoInt)
            {
                if (Storear[ud->intind - 1][ii] < ud->mini ||
                    Storear[ud->intind - 1][ii] > ud->maxi)                                       /*
                                                                                                   *
                                                                                                   *
                                                                                                   *
                                                                                                   *
            ...
            if (pass && ud->DoThres)
            {
                if (Storear[ud->thrind - 1][ii] < ud->minth ||
                    Storear[ud->thrind - 1][ii] > ud->maxth)                                       /*
                                                                                                    *
                                                                                                    *
                                                                                                    *
                                                                                                    *
            ...
            if (pass)
            {
                ...
                xpos = ii - (ypos * xset->daxes->nxx) -
                       (zpos * xset->daxes->nxx * xset->daxes->nyy);
