...
MRI_vectim * GRINCOR_extract_vectim(MRI_shindss *shd, int ids)
{
    ...
    if (shd->datum == 1)
    {
        mv = GRINCOR_extract_vectim_sbyte(shd, ids);
    }
    else
    {
        mv = GRINCOR_extract_vectim_short(shd, ids);
    }
    return mv;
}
...

{
    if (shd->datum == 1)
    {
        GRINCOR_insert_vectim_sbyte(shd, ids, mv);
    }
    else
    {
        GRINCOR_insert_vectim_short(shd, ids, mv);
    }
    return;
}
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 2)
    {
        usage_3dTransformGroupInCorr(2); exit(0);
    }
    ...
    if (prepfunc == NULL)
    {
        ERROR_exit("no -prep option given? :-(");
    }

    if (nopt >= argc)
    {
        ERROR_exit("No input filename on command line?!");
    }
    ...
    if (shd == NULL || nelshd == NULL)
    {
        ERROR_exit("TIC: Cannot continue after input error");
    }
