 ...
 *   n   = length of a row nup = length to use for FFTs (must be even) af  = shift for row f ag  =
 * shift for row g Input and output arrays are f[n] and g[n].  (Note: g may be NULL.)
 * ----------------------------------------------------------------------------*/
...
void wsinc5_shift(int n, float af, float *f)
{
    ...
    if (fabsf(af) < 0.0001f)
    {
        EXRETURN;                       /* little or nothing to do */
    }
...
/*---------------------------------------------------------------------------
 * Shift 1 row with with heptic Lagrange polynomial interpolation [Nov 1998]. Note that heptic
 *interpolation is about the same as Hamming-weighted 3-sidelobe sinc interpolation .
 * -----------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------
 * Shift 1 row with with quintic Lagrange polynomial interpolation [Nov 1998]. Note that quintic
 *interpolation is about the same as Hamming-weighted 2-sidelobe sinc interpolation .
 * -----------------------------------------------------------------------------*/
