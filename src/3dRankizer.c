...
int main(int argc, char *argv[])
{
    ...
    {
        WARNING_message("Too many input datasets!");
    }
    ...
    if (mask_dset != NULL)
    {
        ...
        if (mcount == nvox)
        {
            ININFO_message("Mask includes all voxels");
        }
    ...
    else     /* yes mask :( all this had to be fixed [09 Sep 2024 - RWC] */
    {
        ...
        for (fmin = far[0], ii = 1 ; ii < nvox ; ii++)
        {
            if (far[ii] < fmin)
            {
                fmin = far[ii];
        ...
        if (fmin > 0.0f)
        {
            fmin = 0.0f;                             /* 0  < all pos values */
        }
        else if (fmin == 0.0f)
        {
            fmin = -1.0f;                            /* -1 < all non-neg values */
        }
        else
        {
            fmin = 2.0f * fmin - 1.0f;               /*    < all neg values */
        }
        for (ii = 0 ; ii < nvox ; ii++)
        {
            if (!mmm[ii])
