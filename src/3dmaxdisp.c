...
int main(int argc, char *argv[])
{
    ...
    while (iarg < argc && argv[iarg][0] == '-')
    {
        ...
        if (strcmp(argv[iarg], "-matrix") == 0)
        {
            ...
            if (matim != NULL)
            {
                ERROR_exit("Can't use '%s' more than once :(", argv[iarg]);
            }
            if (++iarg >= argc)
            {
                ERROR_exit("no argument after '%s' :-(", argv[iarg - 1]);
            ...
            if (qim == NULL)
            {
                ERROR_exit("Can't read -matrix '%s' :-(", fname);
            }
            ...
            if (matim_nx < 12 && matim->nvox == 12) /* special case of a 3x4 array */
            {
                ...
                if (verb)
                {
                    INFO_message("-matrix: converting input 3x4 array to 1 row of 12 numbers");
                }
            }
            if (matim_nx < 12)
            {
                ERROR_exit("%d = Less than 12 numbers per row in -matrix '%s' :-(", matim_nx,
                           fname);
            }
            else if (matim_nx > 12)
            {
                WARNING_message("%d = More than 12 numbers per row in -matrix '%s'", matim_ny,
                                fname);
            }
            iarg++; continue;
        }
...
#define DSK(i, j, k)    dsk[(i) + (j) * nx + (k) * nxy]
    ...
    if (verb)
    {
        ININFO_message("  %s voxels in automask",
    ...
    for (nmsk = mm = 0 ; mm < nxyz ; mm++) /* create edges of the automask */
    {
        if (dsk[mm] == 0)
        {
            continue;                                      /* not in the mask */
        }
        ii = mm % nx; kk = mm / nxy; jj = (mm % nxy) / nx; /* voxel indexes */
        ip = ii + 1; im = ii - 1; if (ip >= nx || im < 0)
        {
            msk[mm] = 1; nmsk++; continue;
        }
        jp = jj + 1; jm = jj - 1; if (jp >= ny || jm < 0)
        {
            msk[mm] = 1; nmsk++; continue;
        }
        kp = kk + 1; km = kk - 1; if (kp >= nz || km < 0)
        {
            msk[mm] = 1; nmsk++; continue;
        }
        if (DSK(ip, jj, kk) && DSK(im, jj, kk) &&       /* if all 6 nbhrs  */
            DSK(ii, jp, kk) && DSK(ii, jm, kk) &&       /* are in automask */
            DSK(ii, jj, kp) && DSK(ii, jj, km))
        {
    ...
    if (verb)
    {
        ININFO_message("  %s edge voxels in automask", commaized_integer_string(num_dispvec));
    }
