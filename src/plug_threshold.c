/*plug_threshold.c - AFNI plugin that creates a mask separating brain from non-brain, and saves this
 * mask as a fim dataset. Copyright (c) 2000 Matthew Belmonte
 *
...
/*Most scanners, including GE Signa and Siemens Vision systems, do not use the full 16-bit dynamic
 * range.  THRESH_MAX_BRAIN can therefore be decreased to about 3000 to save memory, if desired.
 * It's set to 32766 here rather than to 32767 since we want to be able to add one to it without
 * causing a signed overflow.*/
#define THRESH_MAX_BRAIN     0x7ffe

/*A median-filter length of 9 seems to work for every data set that I've tested. If your scanner
 * gives a banded distribution of voxel intensities instead of a uniform distribution, you may need
 * to increase this value or apply some smoothing to the histogram.*/
...
/*Prune node 'n' from the tree rooted at 't->tree', rearranging the tree as necessary in order to
 * preserve the descendants of 'n'.*/
static void prune(btree *t, node *n)
{
    register node **prune_site, *largest;
    register int    ref_count_of_largest;

    prune_site = (n->parent == NULLTREE? &(t->tree): n ==
                  n->parent->left_child? &(n->parent->left_child): &(n->parent->right_child));
    ...
    else
    {
        ...
        for (largest = n->left_child; largest->right_child != NULLTREE;
        ...
        largest->subtree_ref_count = largest->ref_count + (largest->left_child ==
                                                           NULLTREE? 0:
                                                           largest->left_child->subtree_ref_count) +
                                     (largest->right_child ==
                                      NULLTREE? 0: largest->right_child->subtree_ref_count);
        destroy_node(n);
    }
}
...
/*Return the median of all the values in the given btree.  Do not alter the btree.*/
static int extract_median(btree *t)
{
    ...
    /*inv: 'left_count' is the number of values less than the median that have been excluded during
     * traversal of the path from 't->tree' to 'n', and
...
/*Insert the given value into the given btree and place it at the tail of the associated queue.*/
static void insert_newest(int v, btree *t)
{
    ...
    /*inv: 'p' is the parent of 'n'.  All 'subtree_ref_count' fields on the path from 't->tree' to
     * 'p' have been incremented.  The proper location for the new value 'v' lies somewhere in the
     *subtree rooted at 'n'.*/
    {
        n->subtree_ref_count++;
        p = n;
...
/*Find the set of voxels in img whose intensities are less than the given threshold and that are
 * contiguous with the given starting voxel (x,y,z). Label the corresponding voxels in mask_img with
 * the value INCLUDED+region_num.
 * (mask_img should have been initialised to UNVISITED by the calling routine. With UNVISITED
 *defined as 0, calloc() or bzero() suffices to accomplish this initialisation.  Multiple calls can
 *be made without intervening re-initialisations only in the case in which the regions being
 *labelled are unconnected.)  This routine returns the size in voxels of the labelled region.*/
            ...
            if (img[*sp] < threshold)
            {
                ...
                for (dz = -1; dz <= 1; dz++)
                {
                    for (dy = -1; dy <= 1; dy++)
                    {
                        for (dx = -1; dx <= 1; dx++)
                        {
                            if ((x + dx >= 0) && (x + dx < xdim) && (y + dy >= 0) &&
                                (y + dy < ydim) && (z + dz >= 0) && (z + dz < zdim) &&
                                (dx || dy || dz))
                            {
...
/*On entry, img is the averaged echo-planar image.  On exit, each voxel in img has been replaced by
 * a 1 if it's been identified as a brain voxel, or by a 0 otherwise.  Returns 0 if OK, 1 if out of
 * memory.*/
...
/*make all non-brain regions in mask_img INCLUDED, and leave all brain regions UNVISITED*/
    ...
    for (z = 0; z != zdim; z++)
    {
        for (y = 0; y != ydim; y++)
        {
            for (x = 0; x != xdim; x++)
            {
                ...
                {
                    region_size = THRESH_region_grow(mask_img, img, stack, region, x, y, z, xdim,
                                                     ydim, zdim, INCLUDED);
...
/*Scan from the right-hand end of the histogram until the median-filtered value of the histogram
 * definitely exceeds zero (right-hand edge of the brain peak). Then continue scanning left, keeping
 * track of the filtered histogram's minimum value, till its current value becomes very large
 * (right-hand edge of the air peak).  The position of this minimum is the intensity threshold for
...
static short *THRESH_compute(THD_3dim_dataset *dset, int verbose)
{
    ...
    /*Form an image each of whose voxels is the mean of the time series for the corresponding voxel
     * in the original image.  Construct a histogram of these mean intensities.*/
    for (z = 0; z != zdim; z++)
    {
        for (y = 0; y != ydim; y++)
        {
    ...
    /*inv: filter contains histogram[x..x+THRESH_FILTER_LEN-1], histo_max is the maximum
     * median-filtered value of histogram[x+1+THRESH_FILTER_LEN/2..
     * THRESH_MAX_BRAIN-(THRESH_FILTER_LEN+1)/2, brain_peak is the index at which histo_max occurs,
     * histo_min is the minimum median-filtered value of
     * histogram[x+1+THRESH_FILTER_LEN/2..brain_peak], and cutoff is the index at which histo_min
     * occurs.*/
    ...
    /*inv: filter contains histogram[x..x+THRESH_FILTER_LEN-1], histo_min is the minimum
     * median-filtered value of histogram[x+1+THRESH_FILTER_LEN/2..
...
/*Make sure that the input dataset exists and is stored in a format that we can understand
 * (MRI_short)*/
...
/*first line of dialogue box: input dataset*/
    ...
    PLUTO_add_dataset(plint, "Dataset", ANAT_SPGR_MASK | ANAT_EPI_MASK, 0,
                      DIMEN_4D_MASK | BRICK_SHORT_MASK);
