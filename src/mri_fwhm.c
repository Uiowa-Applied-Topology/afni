...
void FHWM_1dif_dontcheckplus(int i)
{
    dontcheckplus = i;
}
...
THD_fvec3 mriarr_estimate_FWHM_1dif(MRI_IMARR *imar, byte *mask, int unif)
{
    ...
    for (ii = 0 ; ii < nar ; ii++)
    {
        if (unif)
        {
            ...
            for (kk = 0 ; kk < nvox ; kk++)
            {
                sar[kk] = (sar[kk] - medar[kk]) * madar[kk];
            }
        }
 ...
 *   - A negative return value indicates an error condition in that direction (e.g., FWHM(z) == -1.0
 *when nz == 1).
 *   - Adapted from original 3dFWHM.c by BD Ward.
 *   - 31 Oct 2006 -- BOO!
 *   - A sister function is in SUMA_GeomComp.c if changes are made to this one, please notify the
 *half-gloved man.
 * -----------------------------------------------------------------------------*/
...
THD_fvec3 mri_estimate_FWHM_1dif(MRI_IMAGE *im, byte *mask)
{
    ...
    for (ixyz = 0; ixyz < nxyz; ixyz++)
    {
        if (GOOD(ixyz))
        {
            count++; arg = fim[ixyz]; fsum += arg; fsq += arg * arg;
        }
    }
    ...
    /*---- the model is the correlation of the neighbors in x is exp[ -dx^2 / (4 * sx^2) ] which is
     * the result of passing a Gaussian exp[-x^2/(2*sx^2)] convolution over a white noise field.
     * Then the covariance of the neighbors is 2*V * { 1 - exp[ -dx^2 / (4*sx^2) ] }
...
/* ININFO_message("dx: varxx/var=%g arg=%g",varxx/var,arg) ; */
    if (arg > 0.0 && arg < 1.0)
    {
        sx = 2.35482 * sqrt(-1.0 / (4.0 * log(arg))) * dx;
    }

    arg = 1.0 - 0.5 * (varyy / var);
    if (arg > 0.0 && arg < 1.0)
    {
        sy = 2.35482 * sqrt(-1.0 / (4.0 * log(arg))) * dy;
    }
/* ININFO_message("dy: varyy/var=%g arg=%g",varyy/var,arg) ; */
    ...
    if (arg > 0.0 && arg < 1.0)
    {
        sz = 2.35482 * sqrt(-1.0 / (4.0 * log(arg))) * dz;
    }
...
/*! Routine to estimate Gaussian FWHM of data brick, using differences between 1st and 2cd nearest
 * neighbors.
 *   - A negative return value indicates an error condition in that direction (e.g., FWHM(z) == -1.0
 *when nz == 1).
 * -----------------------------------------------------------------------------*/
...
THD_fvec3 mri_estimate_FWHM_12dif(MRI_IMAGE *im, byte *mask)
{
    ...
    /*--- y = cbrt(12*sqrt(48-120*r+81*r*r)+108*r-80), and then x = y/6 - 4/(3*y) - 1/3 is the real
     * solution to the equation (1-x^4)/(1-x) = r > 1 ;
...
THD_fvec3 mri_estimate_FWHM_12dif_MAD(MRI_IMAGE *im, byte *mask)
{
    ...
    for (ngood = ixyz = 0 ; ixyz < nxyz ; ixyz++)
    {
        if (GOOD(ixyz))
        {
            ngood++;
    ...
    /*--- Modified 11 Aug 2015, to use the mean absolute deviations (wx1 & wx2) instead of the
     * median absolute deviations (vx1 & vx2) if the computation fails for the median absolute
     * deviations (the backup) ---*/
    ...
    if (vx1 > 0.0 && vx2 > vx1)
    {
        ...
        else if (brg > 1.0f)
        {
            ...
            if (brg > 0.0 && brg < 1.0)
            {
                sx = 2.35482 * sqrt(-1.0 / (4.0 * log(brg))) * dx;
            }
        }
...
/* INFO_message("FWHM x: vx1=%f vx2=%f NOT GOOD",vx1,vx2) ; */
    ...
    if (vy1 > 0.0 && vy2 > vy1)
    {
        ...
        else if (brg > 1.0f)
        {
            ...
            if (brg > 0.0 && brg < 1.0)
            {
                sy = 2.35482 * sqrt(-1.0 / (4.0 * log(brg))) * dy;
            }
        }
...
/* INFO_message("FWHM y: vy1=%f vy2=%f NOT GOOD",vy1,vy2) ; */
    ...
    if (vz1 > 0.0 && vz2 > vz1)
    {
        ...
        else if (brg > 1.0f)
        {
            ...
            if (brg > 0.0 && brg < 1.0)
            {
                sz = 2.35482 * sqrt(-1.0 / (4.0 * log(brg))) * dz;
            }
        }
...
/*! Get FWHM estimates for each sub-brick in a dataset. Output image is 3xN where N=# of sub-bricks.
 * -----------------------------------------------------------------------------*/
...
void mri_fwhm_mom12_set_stdev_fac(double fff)
{
    mom12_stdev_fac = fff;
}
...
/*! New method using first and second moments of absolute values of differences, instead of only
 * second moments.  [05 Aug 2015]
 *//*----------------------------------------------------------------------*/
...
THD_fvec3 mri_FWHM_1dif_mom12(MRI_IMAGE *im, byte *mask)
{
    ...
    if (countx == 0)
    {
        countx = 1;               /* patch for possible stupidity in data */
    }
    if (county == 0)
    {
        county = 1;
...
#define SQEPI    0.626657 /* sqrt(PI/8) -- needed for 3rd moment */
    ...
    /*   Model: first differences are distributed like ef = Normal( 0 , 2*V*tau^2 ) where tau =
     * sqrt{ 1 - exp[-del^2/(4*s^2)] }
     *         V    = variance (var) of data del  = spacing of data in mm s    = sigma of smoothing
     *kernel
     *              = del * sqrt( -1/(4*log(1-tau^2)) [s(tau) is a monotonic decreasing function of
     *tau] FWHM = sqrt(8*log(2))*s = 2.35482*s and we assume that tau in turn is (independently)
     *random, distributed with density w(tau) over the interval [0,1].  That is, the density
     *function of df = ef/sqrt(2*V) is
     |1 int|  w(tau) 1/[tau*sqrt(2*pi)] * exp[-df^2/(2*tau^2)] d(tau)
     |0 Unless w(tau) is a delta function, df will be distributed non-normally. Then the moments of
     *the absolute value of df are related to the moments of tau by        |1 E[|df|^p] = int|
     *  w(tau) E[ |Normal(0,tau^2)|^p ] d(tau)
     |0
     *               = 2^(p/2) * Gamma((p+1)/2) / sqrt(pi) * E[tau^p] E[|df|]   = sqrt(2/pi) *
     * E[tau] E[|df|^2] =              E[tau^2] E[|df|^3] = sqrt(8/pi) * E[tau^3] E[|df|^4] =
     *          3 * E[tau^4] etc.
     *   (At this time, we don't use the 3rd or 4th moments, but someday ...?) The "standard"
     * (Forman) way to estimate FWHN (mri_estimate_FWHM_1dif) assumes s is constant; e.g., that
     *w(tau) = delta(tau-q), in which case E[|df|^2] = q^2, and so only the second moment is needed
     *to estimate tau=q (and thus FWHM).  In this routine, we use E[|df|] to estimate E[tau], and
     *then subtract off one standard deviation of tau sqrt(E[tau^2]-E[tau]^2) to weight the estimate
     *towards smaller tau and thus larger FWHM -- since larger FWHM will produce more
     * "false positive" clusters. Completely ad hoc, but it's something. */
    /*--------------------------------------------------------------------------*/
    ...
    { double mn, sg, ff;
      ...
      {
          sg = sqrt(sg);
          for (ff = mom12_stdev_fac ; sg * ff >= mn ; ff *= 0.888)
          {
              ;                                               /*nada*/
          }
          mn -= ff * sg;
      ...
      if (mn > 0.0 && mn < 1.0)
      {
          sx = 2.35482 * sqrt(-1.0 / (4.0 * log(1.0 - mn))) * dx;
      }
      ...
      if (mn > 0.0 && mn < 1.0)
      {
          sy = 2.35482 * sqrt(-1.0 / (4.0 * log(1.0 - mn))) * dy;
      }
      ...
      if (mn > 0.0 && mn < 1.0)
      {
          sz = 2.35482 * sqrt(-1.0 / (4.0 * log(1.0 - mn))) * dz;
      }
    }
...
/* Return the cluster of points over which we estimate the ACF. Use the central NCLU_BASE set of
 * points, and then fill out up to NCLU_GOAL from a selection of the outer points, so that we don't
 * end up with a vast cluster and slow estimation.
 *//*----------------------------------------------------------------------*/
...
int mri_estimate_ACF(MRI_IMAGE *im, byte *mask, MCW_cluster *clout)
{
    ...
    if (clout == NULL || clout->num_pt < 5)
    {
        RETURN(-1);
    }

    for (ppp = 0 ; ppp < clout->num_pt ; ppp++)
    {
        clout->mag[ppp] = 0.0f;
    }
    ...
    for (ixyz = 0 ; ixyz < nxyz ; ixyz++)
    {
        ...
        { int ppp, ip, jp, kp, qqq;
          ...
          {
              ip = ix + clout->i[ppp]; if (ip >= nx || ip < 0)
              {
                  continue;
              }
              jp = jy + clout->j[ppp]; if (jp >= ny || jp < 0)
              {
                  continue;
              }
              kp = kz + clout->k[ppp]; if (kp >= nz || kp < 0)
              {
                  continue;
              }
              qqq = THREE_TO_IJK(ip, jp, kp, nx, nxy); if (!GOOD(qqq))
              {
                  continue;
              }
...
float mriarr_estimate_FWHM_acf(MRI_IMARR *imar, byte *mask, int unif, float radius)
{
    ...
    nvals = IMARR_COUNT(imar); if (nvals < 2)
    {
        unif = 0;
    }
    nvox = IMARR_SUBIM(imar, 0)->nvox;
...
/*! Convert ACF cluster output to function vs radius             [09 Nov 2015] Exponential model
 * function vs. radius r is ACF(r) = a*exp(-r*r/(2*b*b)) + (1-a)*exp(-r/c) where a,b,c are the
 * estimated parameters returned in the float_quad. With restrictions that   0 <= a <= 1 ; b > 0 ; c
 * > 0 The last parameter return (d) is the FWHM of the curve fit.
 *//*--------------------------------------------------------------------------*/
...
#define CRAD(a)    (xx = dx * abs(acf->i[a]), \
    ...
    for (pp = 0 ; pp < nclu - 1 ; pp++)
    {
        ...
        for (qq = pp + 1 ; qq < nclu && rar[qq] <= rp ; qq++)
        {
            ;                                             /*nada*/
        }
        if (qq > pp + 1)
        {
            for (vs = 0.0f, ss = pp ; ss < qq ; ss++)
            {
                vs += aar[ss];
            }
            aar[pp] = vs / (qq - pp);
    ...
    for (pp = 0 ; pp < nclu && aar[pp] > 0.5f ; pp++)
    {
        ;                                             /*nada*/
    }
    if (pp == nclu)
