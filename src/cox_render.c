 ...
 * (2) Load grayscale+color bricks into it with function CREN_set_databytes() (3) Set the viewing
 *angles with CREN_set_viewpoint().
 *    (Optional: set axes ordering with CREN_set_axes().) (4) Create an image with CREN_render().
...
/*--------------------------------------------------------------------------
 * Create a new renderer. The return pointer is passed to all other CREN routines.
 * ----------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------
 * Set the rendering mode CREN_SUM_VOX   = integral of voxel data times opacity CREN_MIP_VOX   =
 * maximum voxel intensity CREN_MIP_OPA   = maximum opacity CREN_MINIP_VOX = minimum nonzero voxel
 *intensity [09 May 2006]
 * -----------------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------------
 * Set the interpolation mode CREN_NN      = nearest neighbor CREN_TWOSTEP = two-step CREN_LINEAR  =
 * linear
 * -------------------------------------------------------------------------------*/
 ...
 *  ncol    = number of colors (must be from 1 to 128, inclusive) rmap[i] = red   map for index
 *i=0..ncol-1 (for voxel values i+128) gmap[i] = green map for index i=0..ncol-1 (for voxel values
 *i+128) bmap[i] = blue  map for index i=0..ncol-1 (for voxel values i+128)
 * -------------------------------------------------------------------------------*/
...
void CREN_set_rgbmap(void *ah, int ncol, byte *rmap, byte *gmap, byte *bmap)
{
    ...
    if (ncol < 1 || rmap == NULL || gmap == NULL || bmap == NULL)
    {
        return;
    }
 ...
 *  opm[vv] = opacity of voxel value vv, for vv=0..127 opgrb   = (fixed) opacity of colored voxels,
 *with vv=128..255 Opacities range from 0.0 to 1.0
 * ------------------------------------------------------------------------*/
 ...
 *   -2 == -y direction 3 == +z direction
 *   -3 == -z direction and similarly for ajj, akk for the j and k directions.
                   ...
                   float di, float dj, float dk)
{
    ...
    if (abii < 1 || abii > 3 ||
        abjj < 1 || abjj > 3 ||
        abkk < 1 || abkk > 3 || abii + abjj + abkk != 6)
    {
        return;
    }
    ...
    ar->dx = fabs(di); if (ar->dx == 0.0)
    {
        ar->dx = 1.0;
    }
    ar->dy = fabs(dj); if (ar->dy == 0.0)
    {
        ar->dy = 1.0;
    }
    ar->dz = fabs(dk); if (ar->dz == 0.0)
    {
        ar->dz = 1.0;
    }
...
void CREN_dset_axes(void *ah, THD_3dim_dataset *dset)
{
    ...
    if (!ISVALID_CREN(ar) || !ISVALID_DSET(dset))
    {
        return;
    }
 ...
 *
 * Data bytes:
 *   values vv=0..127   correspond to grayscale 0..254 (vv << 1) values vv=128..255 correspond to
 *colormap index vv-128.
 * -------------------------------------------------------------------------------*/
...
void CREN_set_databytes(void *ah, int ni, int nj, int nk, byte *grim)
{
    ...
    if (ar->vox != NULL)
    {
        free(ar->vox); ar->vox = NULL;
    }
    if (ar->vtm != NULL)
    {
        free_Tmask(ar->vtm); ar->vtm = NULL;
    }
    ...
    for (ii = 0 ; ii < nvox && grim[ii] < 128 ; ii++)
    {
        ;                                             /* nada */
    }
...
/*----------------------------------------------------------------------------
 * Set the viewpoint of the user axI = axis about which rotation #I is to take place 0 == x , 1 == y
 *, 2 == z thI = angle of rotation (radians)
 * ------------------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------------
 * Actually render an image.  Returns NULL if an error occurs. The image is always in MRI_rgb
 *format.
 *
...
MRI_IMAGE * CREN_render(void *ah, THD_mat33 *rotm)
{
    ...
    /*-- we want diag{1/dx,1/dy,1/dz} [skewmat] [uu] diag{dab,dab,dab}
     *   which is the matrix which transforms from image index coords to volume index coords  --*/
...
#define U(i, j)    uu.mat[i][j]
    ...
    uabs = fabs(U(1, 2)); if (uabs > utop)
    {
        utop = uabs; kk = 1;
    }
    uabs = fabs(U(2, 2)); if (uabs > utop)
    {
        utop = uabs; kk = 2;
    }
 ...
 *   nx, ny, nz = dimensions of vol vol        = input 3D volume of bytes
 *
 *   fixdir = fixed direction (1=x, 2=y, 3=z) fixijk = fixed index da, db = shift in planar
 *coordinaes (non-fixed directions) ma, mb = dimensions of im im     = output 2D image
 *
 * Goal is im[a,b] = vol[ P(a-da,b-db,c=fixijk) ] for a=0..ma-1, b=0..mb-1, where P(a,b,c) is the
 *permutation of (a,b,c) that goes with fixdir:
 *   P(x,y,z) = (y,z,x) for fixdir == 1 P(x,y,z) = (z,x,y) for fixdir == 2 P(x,y,z) = (x,y,z) for
 *fixdir == 3 For values outside the range of vol[], im[] is set to 0.
 * =============================================================================*/
...
/* macro to compute the stepsizes and dimensions in the 3D array for each direction, given the fixed
 * direction */
                     ...
                     int ma, int mb, byte *im)
{
    ...
    da += 0.5; adel = (int)da; if (da < 0.0)
    {
        adel--;                                           /* floor(da+0.5) */
    }
    db += 0.5; bdel = (int)db; if (db < 0.0)
    {
        bdel--;                                           /* floor(db+0.5) */
    }
    abot = 0; if (abot < adel)
    {
        abot = adel;                                      /* range in im[] */
    }
    atop = na + adel; if (atop > ma)
    {
        atop = ma;
    }
                            ...
                            int ma, int mb, byte *im)
{
    ...
    adel = (int)da; if (da < 0.0)
    {
        adel--;                               /* floor(da) */
    }
    bdel = (int)db; if (db < 0.0)
    {
        bdel--;                            /* floor(db) */
    }
                      ...
                      int ma, int mb, byte *im)
{
    ...
    bb = (int)(256 * f_a_b + 0.499); if (bb == 256)
    {
        bb--;
    }
    b_a_b = (byte)bb;
    bb    = (int)(256 * f_ap_b + 0.499); if (bb == 256)
    {
        bb--;
    }
    b_ap_b = (byte)bb;
    bb     = (int)(256 * f_a_bp + 0.499); if (bb == 256)
    {
        bb--;
    }
    b_a_bp = (byte)bb;
    bb     = (int)(256 * f_ap_bp + 0.499); if (bb == 256)
    {
        bb--;
    }
    b_ap_bp = (byte)bb;
