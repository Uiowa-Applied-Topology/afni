...
PLUGIN_interface * PLUGIN_init(int ncall)
{
    ...
    PLUTO_add_number(plint,
                     ...
                     1,        /* smallest possible value */
                     1,        /* largest possible value (turned Off for the moment, supporting code
                                * is present)*/
                     0,        /* decimal shift (none in this case) */
                     1,        /* default value */
                     FALSE     /* allow user to edit value? */
...
static char * DELAY_main(PLUGIN_interface *plint)
{
    ...
    ud->Navg     = 1;                       /* Navg does not play a role for the p value, averaging
    ...
    ud->strout = PLUTO_get_string(plint);       /* strout is for the outiflename, which will be used
                                                 * after the debugging section */
    ...
    if ((ud->T == 0.0) && (ud->unt > 0))                 /* unt error Check */
    {
        ...
        return "***********************\n"
               ...
               "***********************\n";                 /*T must be specified, and > 0 in order
                                                             * to use polar units*/
    }
    ...
    new_dset = MAKER_4D_to_typed_fbuc(old_dset,                       /* input dataset */
                                      ...
                                      -1,                             /* negative value indicating
                                                                       * data type is like original
                                                                       * brick */
                                      ud->ignore,                     /* ignore count */
                                      1,                              /* detrend = ON Let BOB do
                                                                       * it*/
                                      NBUCKETS,                       /*Number of values at each
                                                                       * voxel*/
                                      (generic_func *)DELAY_tsfuncV2, /* timeseries processor
                                                                       * (bucket version)*/
    ...
    while (i < NBUCKETS)
    {
        ...
        case COFINDX:                               /* cross correlation coefficient value in
                                                     * results vector */
            EDIT_BRICK_LABEL(new_dset, i, "Corr. Coef.");
            EDIT_BRICK_ADDKEY(new_dset, i, "r");

            /* Here you must modify either ud->Nfit or ud->Nort or most likely ud->nsamp based on
             * ud->Navg */
            EDIT_BRICK_TO_FICO(new_dset, i, ud->nsamp - ud->ignore, ud->Nfit, ud->Nort);
            ++i;
            break;

        case VARINDX:                               /* FMRI time course variance value in results
                                                     * vector */
    ...
    {
        if (ud->outlogfile != NULL)
        {
            fclose(ud->outlogfile);                                             /* close outlogfile
                                                                                 */
        }
    }
                           ...
                           void *udp, int nbrick, float *buckar)
{
    ...
    if (buckar == NULL)
    {
        ...
        #if 0
            status = hilbertdelay_V2(ts, ud->rvec, ud->ln, ud->Nseg, ud->Pover, opt, ud->dtrnd, dtx,
                                     ud->biasrem, &tmp, &slp, &xcor, &tmp2, &vts, &vrvec);                                                                  /*
                                                                                                                                                             *
                                                                                                                                                             *
                                                                                                                                                             *
                                                                                                                                                             *
    ...
    /* In the old version, I had to check for a match between the lengths of the reference time
     * series and FMRI time series This is now done before the function is called. */
    ...
    if (is_vect_null(ts, npts) == 1) /* check for null vectors */
    {
        ...
        del      = 0.0;                 /* Set all the variables to Null and don't set xcorCoef to
                                         * an impossible value*/
    ...
    {                     /* ud->errcode == 0 outer loop */
        ...
        /* transform dtx from seconds to sampling units and correct for the number of points
         * ignored*/
        if (ud->Dsamp == YUP)
        {
        ...
        ud->errcode = hilbertdelay_V2(ts, ud->rvec, ud->ln, ud->Nseg, ud->Pover, opt, ud->dtrnd,
                                      dtx, ud->biasrem, &delu, &slp, &xcor, &xcorCoef, &vts,
                                      &vrvec); /* cleanup time */
        ...
        {                                      /*ud->errcode == 0 inner loop */
            ...
            if (xcorCoef < ud->co)
            {
                actv = 0;                                       /* determine if voxel is activated
                                                                 * using xcorCoef  */
            }
            if ((actv == 1) && (ud->out == YUP))                /* if voxel is truly activated,
                                                                 * write results to file without
                                                                 * modifying return value */
            {
                indexTOxyz(ud, ncall, &xpos, &ypos, &zpos);
                fprintf(ud->outwrite, "%d\t%d\t%d\t%d\t%f\t%f\t%f\t%f\t%f\n", ncall, xpos, ypos,
                        zpos, delu, del, xcor, xcorCoef, vts);
                if (ud->outts == YUP)
                {
                    writets(ud, ts);
                }
        ...
        else if (ud->errcode == ERROR_LONGDELAY)
        {
            ...
            del      = 0.0;                                 /* Set all the variables to Null and
                                                             * don't set xcorCoef to an impossible
                                                             * value*/
            xcorCoef = 0.0;                                 /*  because the data might still be OK
                                                             */
            xcor     = 0.0;
...
static void write_ud(hilbert_data_V2 *ud)
{
    ...
    fprintf(ud->outlogfile,
            "Plugin detrending = %d (Always 0, mandatory detrending is performed)\n", ud->dtrnd);
