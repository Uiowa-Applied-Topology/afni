...
static char * MHIST_main(PLUGIN_interface *plint)
{
    ...
    while ((tag = PLUTO_get_optiontag(plint)) != NULL)
    {
        ...
        if (strcmp(tag, "Source") == 0)
        {
            ...
            if (DEBUG)
            {
                fprintf(stderr, "++ Dataset #%d '%s' - %d..%d  ovc=%d\n",
    ...
    if (hrad >= 0.0)
    {
        ...
        for (dd = 0 ; dd < nd ; dd++)
        {
            ip = xx + di[dd]; if (ip < 0 || ip > nx1)
            {
                continue;
            }
            jp = yy + dj[dd]; if (jp < 0 || jp > ny1)
            {
                continue;
            }
            kp = zz + dk[dd]; if (kp < 0 || kp > nz1)
            {
                continue;
            }
            mmm[ip + jp * nx + kp * nxy]++;
...

    {
        ...
        if (DEBUG)
        {
            fprintf(stderr, "++ Dataset #%d -- sub-bricks [%d..%d]\n", id, ivbot, ivtop);
        }
    ...
    for (id = 0 ; id < num_dset ; id++)
    {
        ...
        if (DEBUG)
        {
            fprintf(stderr, "++ mri_histogram(#%d)\n", id);
        }
        ...
        if (smooth > 0)
        {
            ...
            for (jj = 0 ; jj <= nbin ; jj++)
            {
                ibot = jj - nwid; if (ibot < 0)
                {
                    ibot = 0;
                }
                itop = jj + nwid; if (itop > nbin)
                {
                    itop = nbin;
                }
                ws = wss = 0.0;
        ...
        if (maxcount > 0)
        {
            for (ii = 0 ; ii <= nbin ; ii++)
            {
                hbin[id][ii] = MIN(hbin[id][ii], maxcount);
            }
        }
#endif
    }
    if (DEBUG)
    {
        fprintf(stderr, "++ about to plot\n");
    }
    hrad = AFNI_numenv("AFNI_1DPLOT_THIK");
    if (hrad <= 0.0f || hrad >= 0.02f)
    {
        hrad = 0.004f;
    }
    ...
    plot_ts_xypush(0, -1);
    sprintf(buf, "#mask=%s #values=%s", commaized_integer_string(mcount),
            commaized_integer_string(tval));
