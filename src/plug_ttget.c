...
static char * TTget_URL(int code, float xx, float yy, float zz)
{
    ...
    ttahome = getenv("AFNI_TTAHOME"); if (ttahome == NULL)
    {
        ttahome = TTAHOME;
    }
    ...
    case 1:                        /* sagittal */
        ...
        {
            for (ii = 1 ; ii < SAGITTAL_NUM && xx > sagittal_xx[ii] ; ii++)
            {
                ;                                                         /* nada */
            }
            if (fabs(xx - sagittal_xx[ii - 1]) < fabs(xx - sagittal_xx[ii]))
            {
                ii--;
            }
        }
    ...
    case 2:                        /* coronal */
        ...
        {
            for (jj = 1 ; jj < CORONAL_NUM && yy > coronal_yy[jj] ; jj++)
            {
                ;                                                        /* nada */
            }
            if (fabs(yy - coronal_yy[jj - 1]) < fabs(yy - coronal_yy[jj]))
            {
                jj--;
            }
        }
    ...
    case 0:                        /* axial */
        ...
        {
            for (kk = 1 ; kk < AXIAL_NUM && zz > axial_zz[kk] ; kk++)
            {
                ;                                                    /* nada */
            }
            if (fabs(zz - axial_zz[kk - 1]) < fabs(zz - axial_zz[kk]))
            {
                kk--;
            }
        }
...
#define NUMSCAN(var)                                                                              \
    ...
    NUMSCAN(nx); if (nx <= 2 || id >= nn - 1)
    {
        free(ndata); return NULL;
    }
    NUMSCAN(ny); if (ny <= 2 || id >= nn - 1)
    {
        free(ndata); return NULL;
    }
    NUMSCAN(maxval);
    if (maxval <= 7 || maxval > 255 || id >= nn - 1)
    {
        free(ndata); return NULL;
    }
    ...
    if (maxval < 255)
    {
        ...
        for (ii = 0 ; ii < 3 * nx * ny ; ii++)
        {
            bar[ii] = (byte)(bar[ii] * fac);
        }
    }
 ...
 *is returned, some error occurrrrred.  The return string is malloc-ed and should be free-d someday.
 * ------------------------------------------------------------------*/
...
static char * self_to_inet(void)
{
    ...
    if (ipad == NULL)
    {
        gethostname(hname, 1048); ipad = xxx_name_to_inet(hname);
    }
    return ipad;
}
...
static int is_nih_host(void)
{
    ...
    if (strstr(ipad, "128.231.") == ipad)
    {
        return 1;
    }
    if (strstr(ipad, "137.187.") == ipad)
    {
        return 1;
    }

#if 1
    if (strstr(ipad, "130.14.") == ipad)
    {
        return 1;
    }
    if (strstr(ipad, "156.40.") == ipad)
    {
        return 1;
    }
    if (strstr(ipad, "165.112.") == ipad)
    {
        return 1;
    }
    if (strstr(ipad, "157.98.") == ipad)
    {
        return 1;
    }
#endif
...
#if 1
    if (strstr(ipad, "129.43.") == ipad)
    {
        return 1;
    }
    if (strstr(ipad, "199.249.158.") == ipad)
    {
        return 1;
    }
    if (strstr(ipad, "209.218.0.") == ipad)
    {
        return 1;
    }
    if (strstr(ipad, "130.14.") == ipad)
    ...
    if (strstr(ipad, "131.158.140.") == ipad)
    {
        return 1;
    }
    if (strstr(ipad, "150.148.11") == ipad)
    {
        return 1;
    }
    if (strstr(ipad, "150.148.21") == ipad)
    {
        return 1;
    }
#endif
...
PLUGIN_interface * PLUGIN_init(int ncall)
{
    ...
    if (ncall > 0 || !is_nih_host() || !can_get_testfile())
    {
        return NULL;
    }
...
static void TTget_recv(int why, int np, int *ijk, void *junk)
{
    if (orient == 0 ||
        !IM3D_OPEN(old_im3d) ||
        old_im3d->vinfo->view_type != VIEW_TALAIRACH_TYPE)
    {
        return;
    }
