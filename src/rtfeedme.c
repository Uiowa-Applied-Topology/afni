 ...
 * get_port_named("AFNI_CONTROL_PORT") and get_port_named("AFNI_TCP_PORT")
 */
#endif
 ...
 *
 * At any given moment, this routine is in one of a number of modes (the AFNI_mode variable). The
 *first time in, AFNI_mode == AFNI_OPEN_CONTROL_MODE.  In each mode, certain tasks must be
 *accomplished and this program must be synchronized with AFNI.  When the necessary deeds are done,
 *the routine advances to the next mode.  If the deeds cannot be done when this routine is called,
 *then it will stay in the same mode, and the next time it is called it will try to do them again.
 *This routine should be called repeatedly until it progresses to the last mode
 *(AFNI_CONTINUE_MODE), which is for normal transmission of images (one at a time) to AFNI.
 *
 * If an error occurs, so that this program can no longer talk to AFNI, then AFNI_mode is set to 0,
 *which means "do nothing further".  The rest of the data acquisition software will continue, but
 *these routines will be stopped dead.
 ******************************************************************************/
...
void AFNI_start_io(void)
{
    ...
    if (AFNI_mode <= 0 || AFNI_mode == AFNI_CONTINUE_MODE)
    {
        return;
    }

    /***** If we are at the first time in, try to open a control socket to talk to AFNI *****/
    ...
    if (AFNI_mode == AFNI_WAIT_CONTROL_MODE)
    {
        ...
        /** if ii == 0, then the channel is still pending, so do nothing; otherwise, take some
         * action.    **/
    ...
    /***** Send the control information, which says how we will talk to AFNI in the future (shmem or
     * TCP/IP), then close the control channel and open this new data channel *****/
    ...
    if (AFNI_mode == AFNI_OPEN_DATA_MODE)
    {
        ...
        {
            sprintf(AFNI_iochan, "shm:grv:%dM", RT_mega);
        }
        else
        {
            sprintf(AFNI_iochan, "shm:grv:50K"); /* 11 Dec 2002 */
        }
        ...
        else
        {
            ...
            else
            {
                if (AFNI_verbose)
                {
                    fprintf(stderr, "Entering AFNI_CATCHUP_MODE.\n");
                }
    ...
    /***** Wait for the data channel to be connected to AFNI, and then send any images that are
     * reconstructed and ready to go *****/
...
/* 11 Dec 2002: string to mark end of image data, to enable ending dataset(s) without closing data
 * channel */
...
Restart:
    ...
    num_chan = 0;
    for ( ; iarg < argc && strncmp(argv[iarg], "-break", 6) != 0 ; iarg++)
    {
        num_chan++;
    }
    if (num_chan == 0)
...
#define ERREX(ee)                                                                    \
        ...
        while (0)
            ...
            if (DSET_NX(RT_dset[0]) != DSET_NX(RT_dset[cc]))
            {
                ERREX("nx mismatch");
            }
            if (DSET_NY(RT_dset[0]) != DSET_NY(RT_dset[cc]))
            {
                ERREX("ny mismatch");
            }
            if (DSET_NZ(RT_dset[0]) != DSET_NZ(RT_dset[cc]))
            {
                ERREX("nz mismatch");
            }

            if (DSET_NVALS(RT_dset[0]) != DSET_NVALS(RT_dset[cc]))
            {
                ERREX("nvals mismatch");
            }
...
#define ADDTO_BUF    (strcat(RT_buf, RT_com), strcat(RT_buf, "\n"))
    ...
    if (DSET_NVALS(RT_dset[0]) == 1)
    {
        if (RT_3D)
        {
            strcat(RT_com, "3D");         /* 1 3D array, all at once */
        }
        else
        {
            strcat(RT_com, "2D+z");       /* 1 3D array, by slices */
        }
    }
    else
    {
        if (RT_3D)
        {
            strcat(RT_com, "3D+t");       /* multi 3D arrays, each all at once */
        }
        else
        {
            strcat(RT_com, "2D+zt");      /* multi 3D arrays, each by slices */
        }
    }
    ADDTO_BUF;
    ...
    if (DSET_NVALS(RT_dset[0]) > 1 && DSET_TR(RT_dset[0]) > 0.0)
    {
        ...
        if (DSET_TIMEUNITS(RT_dset[0]) == UNITS_MSEC_TYPE)
        {
            TR *= 0.001;
        }
    ...
    { float xorg, yorg, zorg;
      ...
      if (ORIENT_sign[xorc] == '+')
      {
          xorc = ORIENT_OPPOSITE(xorc);
      }
      if (ORIENT_sign[yorc] == '+')
      {
          yorc = ORIENT_OPPOSITE(yorc);
      }
      if (ORIENT_sign[zorc] == '+')
      {
          zorc = ORIENT_OPPOSITE(zorc);
      }
    ...
    if (RT_3D)
    {
        nbytes *= DSET_NZ(RT_dset[0]);
    }

    if (qar != NULL)
    {
        free(qar);                                   /* free old workspace */
    }
    ...
    if (!RT_3D)
    {
        ntran *= DSET_NZ(RT_dset[0]);                 /* volumes or slices */
    }
