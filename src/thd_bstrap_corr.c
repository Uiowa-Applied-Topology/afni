...
/*! atanh() of Correlation of x[] and y[], omitting index qq. For doing jackknife statistics on the
 * correlation. */
...
/* Bias-Corrected accelerated (BCa) bootstrap estimation of correlation and confidence intervals: M
 * Mudelsee, Mathematical Geology 35:651-665.
 *//*--------------------------------------------------------------------------*/
...
#pragma omp critical (MALLOC)
    ...
    for (kb = 0 ; kb < nboot ; kb++)
    {
        ...
        for (ii = 1 ; ii < npt ; ii++)
        {
            if (lrand48() < qboot)
            {
                qq = (lrand48() >> 3) % npt;                 /* random jump */
            }
            else
            {
                qq = (qq + 1) % npt;                         /* next point */
            }
    ...
    /* find location of rxy in the sorted rboot array, then use that to estimate the bias correction
     * parameter z0hat */

    qsort_float(nboot, rboot);
    for (kb = 0 ; kb < nboot && rboot[kb] < rxy ; kb++)
    {
        ;                              /*nada*/
    }
...
static void mean_vector(int n, int m, int xtyp, void *xp, float *uvec)
{
    ...
    if (nn < 1 || mm < 1 || xp == NULL || uvec == NULL)
    {
        return;
    }
...
/*! Compute the principal singular vector of a set of m columns, each of length n, stored in array
 * xx[i+j*n] for i=0..n-1, j=0..m-1. The singular value is returned, and the vector is stored into
 * uvec[].
 * tvec is a vector so that the sign of uvec dot tvec will be non-negative. If the return value is
 *not positive, something bad happened.
 *//*--------------------------------------------------------------------------*/
                                        ...
                                        /* so [A] = [X]'[X] = m x m */
    {
        ...
        for (jj = 0 ; jj < mm ; jj++)
        {
            ...
            for (kk = 0 ; kk <= jj ; kk++)
            {
                ...
                for (ii = 0 ; ii < n1 ; ii += 2)
                {
                    sum += xj[ii] * xk[ii] + xj[ii + 1] * xk[ii + 1];
                }
                if (ii == n1)
                {
                    sum += xj[ii] * xk[ii];
                }
                A(jj, kk) = sum; if (kk < jj)
                {
                    A(kk, jj) = sum;
                }
            }
...
#pragma omp critical (MALLOC)
     ...
     *               so [A][V] = [V] [S'][S] so eigenvectors of [A] are [V], but we want [U] note
     *that [X][V] = [U] [S] so pre-multiplying each column vector in [V] by matrix [X] will give the
     *corresponding column in [U], but scaled;
...
/* Return bootstrap BCa estimates for atanh(correlation) and stdev of that, plus the un-corrected
 * 'raw' correlation, in a triple of floats.
 *//*--------------------------------------------------------------------------*/
...
#pragma omp critical (MEMCPY)
    ...
    val = fabsf(rxy); if (val < 0.1f)
    {
        val = 0.1f;
    }
    val = fabsf(bcor - rxy) / val; if (val > 1.0f)
    {
        val = 1.0f;
    }
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 3)
    {
        ERROR_exit("need 2 file args");
    }

    if (AFNI_yesenv("NO_DEMEAN"))
    {
        acorr_demean = 0;
    }
    SET_RAND_SEED;
    ...
    aim = mri_read_1D(argv[1]); if (aim == NULL)
    {
        ERROR_exit("Can't read aim");
    }
    bim = mri_read_1D(argv[2]); if (bim == NULL)
    {
        ERROR_exit("Can't read bim");
    }
    if (aim->nx != aim->nx)
    {
        ERROR_exit("nx not same");
    }

    if (argc > 3)
    {
        nboot = (int)strtod(argv[3], NULL);
        if (nboot < 66 || nboot > 999999)
        {
            nboot = 666;
        }
    }
