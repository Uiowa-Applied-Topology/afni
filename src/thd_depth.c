                               ...
                               bool edges_are_zero_for_nz, bool debugMode)
{
    ...
    if ((errorNumber = getDistanceFieldDataSet(din, &dout, metric, do_sqrt, edges_are_zero_for_nz,
    ...
    if (!outputFileName)         /* Default output filename */
    {
        ...
        if (debugMode)
        {
            sprintf(appendage, "%s", strcat(appendage, "Debug"));
        }

        /* Allocate memory to output name buffer */
        if (!(outputFileName = (char *)malloc(strlen(searchPath) + strlen(prefix) +
                                              strlen(appendage) + 8)))
        {
...
int erosion(THD_3dim_dataset *din, float *outImg)
{
    ...
    {
        return ERROR_DATATYPENOTHANDLED;
    }
...
bool sixConnectedAllHi(BYTE *buffer, int index, int nx, int ny, int nz)
{
    ...
    return buffer[getIndex((x > 0)? x - 1:x, y, z, nx, ny,
                           nz)] && buffer[getIndex((x < nx - 1)? x + 1:x, y, z, nx, ny, nz)] &&
           buffer[getIndex(x, (y > 0)? y - 1:y, z, nx, ny, nz)] && buffer[getIndex(x,
                                                                                   (y <
                                                                                    ny - 1)? y +
                                                                                   1:y, z, nx, ny,
                                                                                   nz)] &&
           buffer[getIndex(x, y, (z > 0)? z - 1:z, nx, ny, nz)] && buffer[getIndex(x, y,
                                                                                   (z <
                                                                                    nz - 1)? z +
                                                                                   1:z, nx, ny,
                                                                                   nz)];
...
int afni_edt(THD_3dim_dataset *din, float *outImg, bool do_sqrt, bool edges_are_zero_for_nz,
             bool debugMode)
{
    /* Get dimensions in voxels */
    int    nz   = DSET_NZ(din);
...
ERROR_NUMBER processIndex(int index, int *inputImg, float **outImg, THD_3dim_dataset *din)
{
    ...
    nRow = nx * nz;                                           /*transpose XYZ to YXZ and blur Y
                                                               * columns with XZ Rows */
    for (v = 0; v < nVol; v++)                                /*transpose each volume separately */
    {
        flt *img3D = (flt *)calloc(nvox3D * sizeof(flt), 64); /*alloc for each volume to allow
                                                               * openmp */
    ...
    nRow = nx * ny;                                           /*transpose XYZ to ZXY and blur Z
                                                               * columns with XY Rows */
    for (v = 0; v < nVol; v++)                                /*transpose each volume separately */
    {
        flt *img3D = (flt *)calloc(nvox3D * sizeof(flt), 64); /*alloc for each volume to allow
                                                               * openmp */
...
ERROR_NUMBER getNonzeroIndices(int nvox, int *inputImg, int *numIndices, int **indices)
{
    ...
    *numIndices = 0;
    ...
    for (i = 0; i < nvox; ++i)
    {
        if ((voxelValue = inputImg[i]) > 0)
        {
            old = false;
            /* for (j=0; j<*numIndices; ++j) if ((*indices)[j]==voxelValue) old = true;    // Core
             * dump */
            for (j = 0; j < *numIndices; ++j)
            {
                if (buffer[j] == voxelValue)
                {
                    old = true;
                }
            }
