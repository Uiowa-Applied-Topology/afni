 ...
 *  Purpose:
 *
 *      AFNI 'reorder' plugin routine that parses the epoch map file for a requested shuffling of a
 *3D+Time dataSet. This function will return an array of indices representing the new (reorderd)
 *position of time-course values. The caller will then apply this order to all voxels in the
 *dataSet.
 *
 *      This version of the parseMap function, 'parseMap', 'collates'
 *      duplicated classes in the map file in the order that they appear (e.g., given a sequence of
 *classes [ D C A B A ], the reorderd order returned will be indices for [ A1 A2 B C D ]).
 *
 *      The "epoch map" is a series of entries (given in a text file) which indicates the
 *classification and, implicitly, the target order of epochs of a time-course. These maps are
 *companions to dataSets arising from specific sequences of stimulus presentation and, therefore,
 *should have a one-to-one correspondence to those dataSets (i.e., they have the same time length).
 *
 ...
 *
 ...
 #
 #      Each entry is either an 'EpochClass' or a '-'. The latter excludes the point from the
 #resulting reordering. 'EpochClass' is a single letter, [a-zA-Z], which classifies the current
 #epoch; the concatenated number is an increasing value from 1 to the epoch length and is used
 #mainly to delimit contiguous instances of the same class.
 #
 #      For example:
 #
 ...
 #          C1 C2 C3 C4 C5 B1 B2 B3 B4 B5 D1 D2 D3 D4 D5 A1 A2 A3 A4 A5
 # Resulting order:
 #   RingSize |               __A__
 #            |          __D__|
 #            |     __B__|
 ...
 #
 #
 #      The TR number of each entry is, implicitly, the line number of the entry; therefore, the
 #number of lines in the map file (except for comment lines) should equal the number of TRs in the
 #companion dataSet.
 #
 #  Usage:
 #
 ...
 #  Input Parameters:
 #
 #      myFile, char *, pointer to a NULL-terminated string, filename for an epoch map length, int
 #*, pointer to a value storing the number of time-course points in the dataSet Output Parameters:
 #
 #      int *, pointer to an array of length 'length' containing indices (on the interval [0,
 #(length-1)] for the new ordering of time-course data points. NULL is returned on any error. By
 #parameter:
 #      class, ClassInfo **, address of a pointer to an array of structures that will contain class
 #and class length info.
 #      classCount, integer, address of an integer to receive the length of 'class'
 #
 #  Side Effects:
 #
 #      The contents of 'length' will hold the length of the 3D+time dataSet to be processed by this
 #plugin; on return, this will contain the length of the array of indices returned by this function,
 #which in turn will also be the time-length of the reorderd 3D+time dataset. The return length can
 #only be altered (decreased) by excluding TR points from the reorderd dataSet (by specifying '-' in
 #the epoch map file). 'class' will be allocated to the length 'classCount' if parsing is
 #successful.
 #
 #  Pseudo Code:
 #
 #      Check input parameters. Count the number of entries in the file.
    ...
    *classCount  = 0;
    for (i = 0, line = 1; i < rawLength; line++)
    {
        ...
        if (0 == currentClass && 0 == *classCount) /* first class */
        {
            ...
            {
                printf(
                    "!! [AFNI/reorder] Invalid class numbering at line %d [Should start at 1] {1} !!\n",
                    line);
                free(classList);
                free(classNum);
                free(index);
            ...
            /* reallocate space */
            (*classKRH) = (ClassInfo *)realloc((void *)(*classKRH),
                                               (*classCount) * sizeof(ClassInfo));
            if (NULL == (*classKRH))
            {
                printf("!! [AFNI/reorder] Allocation error(4) !!\n");
                free(index);
        ...
        else if (currentClass != cBuf[0]) /* new class */
        {
            ...
            {
                printf(
                    "!! [AFNI/reorder] Invalid class numbering at line %d [Should start at 1] {2} !!\n",
                    line);
                free(classList);
                free(classNum);
                free(index);
...
/* Find the first occurrence of the lowest value class [currentClass]. Store the index positions of
 * that class sequence in the current positions available in the 'index' array.
 */
    ...
    for (i = 0, currentClass = 0; i < *length; /* quit when all indices are remapped */)
    {
        ...
        /* currenClassPos is the index of the start position for the current class to be remapped;
         * remap until class changes or class number returns to 1 */
