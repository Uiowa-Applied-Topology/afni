...
char * mri_clusterize_report(void)
{
    return report;
}
...
/*! Cluster-edit volume bim, possibly thresholding with tim, and produce a new output image.  [05
 * Sep 2006]
 *
                           ...
                           int posonly, byte *mask)
{
    ...
    if (report != NULL)
    {
        free((void *)report);  report = NULL;
    }
    if (clarout != NULL)
    {
        DESTROY_CLARR(clarout);
    }

    if (bim == NULL || mri_data_pointer(bim) == NULL)
    {
        RETURN(NULL);
    }
...
/*! Bi-Cluster-edit volume bim, possibly thresholding with tim, and produce a new output image.  [05
 * Sep 2006]
 * -----------------------------------------------------------------------*/
...
mri_cluster_detail mri_clusterize_detailize(MCW_cluster *cl, int icent_flag)
{
    ...
    if (vsum > 0.0f)
    {
        ...
        for (kk = 0 ; kk < cl->num_pt ; kk++)
        {
            ...
            if (vsum < wbest)
            {
                xmi = xqq; ymi = yqq; zmi = zqq; wbest = vsum;
            }
        }
    }
    cld.xmi = xmi; cld.ymi = ymi; cld.zmi = zmi;
...
MCW_cluster_array *find_clusters_NN1(MRI_IMAGE *cim)
{
    ...
    if (clust_arr->num_clu <= 0)
    {
        DESTROY_CLARR(clust_arr);
    }
    return clust_arr;
}
