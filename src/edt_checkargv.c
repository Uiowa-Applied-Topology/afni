...
/**--------------------------------------------------------------------
 * Check option # nopt to see if it is a legal dataset editing command. If so, put its value into
 * "edopt" and return the number of options consumed.  If not, return 0.  An illegal editing option
...
int EDIT_check_argv(int argc, char *argv[], int nopt, EDIT_options *edopt)
{
    ...
    if (strncmp(argv[nopt], "-2thresh", 6) == 0)
    {
        ...
        if (v1 >= v2)
        {
            ERROR_exit("Illegal values after -2thresh");
        }
        edopt->thresh = v2;
        edopt->thbot  = v1;
        CHECK_DONE;
    ...
    if (strncmp(argv[nopt], "-1erode", 7) == 0)
    {
        ...
        if (edopt->erode_pv > 1.0)
        {
            edopt->erode_pv /= 100.0;
        }
