...
time_series * RWC_blank_time_series(int npt)
{
    ...
    if (vec == NULL)
    {
        MALLOC_ERR("new time_series");
    }

    vec->ts = (float *)malloc(sizeof(float) * npt);
    if (vec->ts == NULL)
    {
        MALLOC_ERR("new time_series vector");
    }
...
time_series * RWC_read_time_series(char *fname)
{
    ...
    if (vec->fname == NULL)
    {
        MALLOC_ERR("new time_series fname");
    }
    ...
    if (vec->ts == NULL)
    {
        MALLOC_ERR("new time_series initial ts");
    }
    ...
    while (1)
    {
        ...
        if (used_tsar == alloc_tsar)
        {
            ...
            if (vec->ts == NULL)
            {
                MALLOC_ERR("expanding new time_series ts");
            }
        }
    ...
    if (vec->ts == NULL)
    {
        MALLOC_ERR("shrinking new time_series ts");
    }
